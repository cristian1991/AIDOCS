...
def record_hook_event(
    ...
    *,
    ...
    try:
        ...
        # #489: a telemetry breadcrumb no gate reads. Written behind the
        # evaluation so it never holds a hook in BEGIN IMMEDIATE on the
        # execution index (measured 2-19s under daemon contention). A queue
        # overflow is COUNTED and announced by hook_write_behind, never silent.
        hook_write_behind.submit(
            "hook_intercept",
            lambda: _log_record(
                runtime.hub,
                project_root,
...
def record_classification_event(
    ...
    *,
    ...
    try:
        ...
        # #489: traceability row, not enforcement evidence — write-behind.
        hook_write_behind.submit(
            "prompt_classified",
            lambda: _log_record(
                runtime.hub,
...
def decide_tool_use(
    ...
    *,
    ...
    # Test-retry mutation witness (r0b 903db95a): snapshot this edit
    # invocation's target bytes is taken AFTER the verdict, on allow only
    # (see the end of this function) — a refused edit never runs and would
    # otherwise stay an open overlap for the whole TTL.
    ...
    decision = _decision_from_gate_result(result)

    # Test-retry mutation witness (r0b 903db95a, moved here on 7ac4b3388):
    # snapshot this edit invocation's target bytes, keyed by the host
    # tool_use_id, AFTER the gate's terminal verdict and only on ALLOW — the
    # only verdict whose tool actually runs and therefore produces a
    # PostToolUse to close the witness. Opening it before the verdict left a
    # denied/frozen edit as an open overlap for the whole TTL, disqualifying
    # later legitimate edits of the same file. An `ask` opens nothing either:
    # the host may decline with no PostToolUse at all, and an approval that
    # runs without a fresh PreToolUse simply mints no witness — that call can
    # not unlock (fail closed), which is the safe direction.
    # Best-effort; never affects the decision.
    if decision.verdict == "allow":
        try:
            from .lifecycle_service import LifecycleService as _LcPre

            _LcPre(runtime).on_pre_tool_use_edit_preimage(
                tool_name=tool_name,
                tool_input=tool_input,
                project_root=project_root,
                payload=payload,
...
def _ups_prompt_event_id(runtime, project_root, payload, session_id):
    """The host's own per-prompt id, or "" (the bridge then derives a stable
    receive-window identity). NEVER the minted causal turn id: it rotates on
    every hook run, so a retried hook would double-append."""
    return str(payload.get("prompt_id") or "").strip()


def _ups_operator_bridge(runtime, project_root, payload, session_id, prompt):
    """#1061: allowed operator prompt -> operator-origin stream entry. Best-effort:
    the bridge can never block or alter the operator's own prompt."""
    try:
        from .operator_prompt_bridge import bridge_ups_prompt

        return bridge_ups_prompt(
            project_root,
            session_id=session_id,
            host_session_id=str(payload.get("session_id") or "").strip(),
            prompt=prompt,
            prompt_event_id=_ups_prompt_event_id(runtime, project_root, payload, session_id),
...
def _ups_operator_bridge_refusal(runtime, project_root, payload, prompt, reason_code):
    """#1061: blocked prompt -> named refusal audit, NO content entry."""
    try:
        from .operator_prompt_bridge import refuse_ups_prompt

        host = str(payload.get("session_id") or "").strip()
        session_id = resolve_managed_session(
            runtime.hub.managed_mode, project_root, host_session_id=host
        )
        return refuse_ups_prompt(
            project_root,
            session_id=session_id,
            host_session_id=host,
            prompt=prompt,
            prompt_event_id=_ups_prompt_event_id(runtime, project_root, payload, ""),
...
def _ups_origin_gate(payload, project_root, verified_grant_eligible, _why):
    ...
    _grant_eligible=False (fail closed) and the evidence-not-authority
    attribution marker (empire §XII: a prompt's content is evidence, never
    authority; aidocs §IX: origin decides grant eligibility). Returns (_grant_eligible, _origin_principal)."""
    ...
    if verified_grant_eligible is not None:
        ...
        # Evidence-not-authority attribution (empire §XII): the caller did NOT verify grant
...
def _ups_advise_rails(
    ...
    # keyword-only fallback. Detectors themselves stay fail-SAFE (§IX
...
def _run_user_prompt_core(
    ...
    *,
    ...
    if _safety_envelope is not None:
        # #1061 operator bridge: a REFUSED prompt never reaches a parked seat
        # with its content -- only a named refusal is audited.
        _ups_operator_bridge_refusal(
            runtime, project_root, payload, prompt,
            str(_safety_envelope.get("blocked_by") or "preflight_blocked"),
        )
    ...
    # reply/system text. Fail direction is DROP (§IX drop-on-doubt): a
    ...
    if _resolution_envelope is not None:
        _ups_operator_bridge_refusal(runtime, project_root, payload, prompt, "route_blocked")
    ...
    # #1061 OPERATOR BRIDGE. Every blocking verdict (login, secret, preflight,
    # origin gate, route validation) has ALREADY been rendered above; only an
    # operator-authored, authority-bearing prompt (the causal-turn mint rule)
    # becomes ONE operator-origin stream entry for this host's seat actor.
    # Receive event only: it mints nothing (grants stayed in the stages above).
    if session_id and _should_mint_causal_turn(_operator_text, _grant_eligible):
        _ups_operator_bridge(runtime, project_root, payload, session_id, prompt)
...
def on_session_start(
    ...
    *,
    ...
    try:
        ...
        # managed binding yet ⇒ no row; the explicit ai_session(connect)
        # attaches it (#1095 step 4: no XAACP call registers anyone any more).
        # On a cloud-bound project this forwards to the gate (no local shadow).
        ...
        # #1095 step 4: SessionStart is a lifecycle rail, so it retries owed
        # remote attaches (transport outbox) -- bounded, never from a tool call.
        ...
        pass
    # #1095: an old deployment's legacy aidocs-managed.json comes across HERE,
    # on the lifecycle rail -- managed-mode READS are SQLite-only and never
    # consult the file. A project that is not adopted refuses (no store is
    # created); either way a session start is never refused by this.
    try:
        from .aidocs_managed_store import AidocsManagedStore

        AidocsManagedStore().migrate_legacy(project_root)
    except Exception:
        pass
    # #1095: the skill catalog READ (list_skills, under ai_session skills_get)
    # no longer seeds/restores the empire library or migrates the legacy
    # skill-providers.json. Both come across HERE, on the lifecycle rail (and on
    # their write paths), through the ONE lifecycle helper -- fail-open and
    # step-independent: a session start is never refused by it.
    try:
        from .skill_store import run_skill_authority_lifecycle

        run_skill_authority_lifecycle(project_root)
    except Exception:
        pass
...
# ══════════════════════════════════════════════════════════════════════════
# HOST-AGNOSTIC NATIVE-TOOL LEASE LIFECYCLE (r0b 3e084f64). Every host adapter
# calls these two functions around a host-native tool; nothing host-specific
# lives here. The per-host attestation (which adapters provide a matched
# pre/post channel) is branch_transition_barrier.ATTESTED_NATIVE_LIFECYCLE_HOSTS;
# every host's presence is recorded so ai_git switch refuses while an
# unattested host is attached to the work tree.
# ══════════════════════════════════════════════════════════════════════════


#: What a host kind resolves to when it cannot be PROVEN. It is deliberately
#: not a host name and is never in ATTESTED_NATIVE_LIFECYCLE_HOSTS, so an
#: unproven host blocks ai_git switch exactly like an unresolved roster kind.
UNKNOWN_HOST_KIND = "unknown"


def resolve_hook_host_kind(payload: dict[str, object]) -> str:
    """The host kind a HOOK payload PROVES, else ``UNKNOWN_HOST_KIND``.

    ``ClaudeHookHandler`` is the SHARED hook entry point: Codex CLI installs the
    same hook document shape (``claude_hooks_install.CODEX_HOOKS``) and its
    events land in the same dispatcher, which stamps the request identity as
    ``claude_code`` unconditionally. Attestation may not inherit that default —
    a Codex actor recorded as claude_code would look attested while having no
    PostToolUse lifecycle at all. So this resolver never consults that stamp and
    NEVER falls back to a host name:

      1. an explicit host marker on the payload (adapters that know their host);
      2. the process environment, which the HOST set: CODEX_* => codex_cli,
         CLAUDECODE / CLAUDE_CODE_* => claude_code. BOTH present is ambiguous
         and resolves to unknown;
      3. otherwise unknown.
    """
    import os as _os

    for key in ("host_kind", "hostKind", "aidocs_host_kind"):
        marker = str((payload or {}).get(key) or "").strip()
        if marker:
            from . import branch_transition_barrier as _btb

            return _btb.canonical_host_kind(marker) or UNKNOWN_HOST_KIND
    codex = any(k.startswith("CODEX_") for k in _os.environ)
    claude = bool(_os.environ.get("CLAUDECODE") or _os.environ.get("CLAUDE_CODE_ENTRYPOINT"))
    if codex and not claude:
        return "codex_cli"
    if claude and not codex:
        return "claude_code"
    return UNKNOWN_HOST_KIND


def record_edit_terminal_event(runtime, project_root: Path, payload: dict, reason: str = "") -> bool:
    """PUBLIC entry point for a host event that TERMINATES an edit invocation
    without an ordinary PostToolUse — PostToolUseFailure above all.

    A host-native witness is retired only on positive terminal evidence keyed by
    the native invocation identity (native leases carry no pid and no heartbeat,
    so their expiry proves nothing). Call it with the same hook payload.
    Best-effort, never raises.
    """
    try:
        from .lifecycle_service import LifecycleService

        return LifecycleService(runtime).on_tool_use_terminal(
            tool_name=str((payload or {}).get("tool_name") or ""),
            project_root=project_root,
            payload=payload or {},
            reason=reason or "post_tool_use_failure",
        )
    except Exception:
        return False


def abort_edit_witness_after_allow(runtime, project_root: Path, payload: dict) -> int:
    """PUBLIC entry point for any pre-execution authority that converts the
    gate's ALLOW into a refusal AFTER decide_tool_use returned (the native
    mutation lease gate is the one such authority today).

    Call it with the same PreToolUse payload; it disposes this exact
    (session, lane, tool_use_id, host session_id) test-retry witness so a call that never runs
    cannot block later edits of the same file. Best-effort, returns the number
    of preimages disposed, never raises.
    """
    try:
        from .lifecycle_service import LifecycleService

        return LifecycleService(runtime).on_pre_tool_use_refused_after_allow(
            tool_name=str((payload or {}).get("tool_name") or ""),
            project_root=project_root,
            payload=payload or {},
        )
...
def native_tool_pre(
    ...
    *,
    ...
    """Record host presence and, for an attested host, register the native
    tool's shared lease BEFORE the tool may run (atomic with the barrier check).

    Returns '' when the tool may proceed, else the refusal reason (fail closed:
    a presence or lease store failure refuses)."""
    from . import branch_transition_barrier as _btb

    try:
        _btb.record_host_presence(project_root, host_kind, host_session_id)
        if denied or not _btb.native_tool_needs_lease(tool_name):
            return ""
        if host_kind not in _btb.ATTESTED_NATIVE_LIFECYCLE_HOSTS:
            return ""  # presence recorded; ai_git switch refuses while it lasts
        _btb.register_native_tool_lease(
            project_root,
            host_session_id=host_session_id,
        ...
        return ""
    except _btb.BarrierHeld as exc:
        return (
            f"AIDOCS branch-transition barrier: {_btb.describe(exc.holder)}. `{tool_name}` is "
            "refused until the switch finishes."
        )
    except Exception as exc:  # noqa: BLE001
        return (
            f"AIDOCS branch-transition barrier: native mutation lease could not be registered "
            f"({exc!r}); `{tool_name}` refused closed."
        )


def native_tool_post(
    *,
    host_kind: str,
    host_session_id: str,
    tool_use_id: str,
    ...
    """Release the native tool's lease (post / post-failure). A missed release
    fails CLOSED: the lease lingers until NATIVE_TOOL_TTL_S."""
    from . import branch_transition_barrier as _btb

    try:
        if host_kind in _btb.ATTESTED_NATIVE_LIFECYCLE_HOSTS and _btb.native_tool_needs_lease(tool_name):
            _btb.release_native_tool_lease(
                host_session_id=host_session_id,
                tool_use_id=tool_use_id,
                tool_name=tool_name,
                tool_input=tool_input,
