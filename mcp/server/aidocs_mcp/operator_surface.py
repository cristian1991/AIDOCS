...
def inspect_key(
    ...
    *,
    ...
    return {
        ...
        # The Advanced Raw / expert editor renders its control from THIS payload —
        # it cannot reach SETTINGS_CATALOG. Without allowed_values an enum setting
        # degrades to a free-text box, which is how a typed catalog ends up asking
        # the operator to hand-type JSON (2026-08-02).
        "allowed_values": (meta or {}).get("allowed_values"),
...
def expert_set(
    ...
    *,
    ...
    # #747 defect 2 -- TYPE THE VALUE BEFORE IT IS STORED.
    #
    # MEASURED 2026-08-18: expert_set(..., "false") for the BOOLEAN key
    # security.allow_allowlist_edit returned ok and stored the STRING 'false'.
    # A non-empty string is TRUTHY, so an operator using the dashboard's
    # free-text box to switch that DANGER-flagged authority OFF switched it ON.
    # The catalog has always declared the type; nothing at this boundary read it.
    #
    # _atomic_apply's readback guard cannot catch this: with no coercion the
    # string round-trips EQUAL to itself, so the write "verifies" successfully.
    #
    # The rule is the CLI's, moved to config_schema and shared rather than
    # copied. It is safe in the direction that matters: anything unrecognised
    # resolves to False, so garbage can never silently ENABLE an authority.
    from .config_schema import coerce_setting_value

    typed = coerce_setting_value(key, value)
    return _atomic_apply(project_root, {key: typed}, scope=scope, scope_key=scope_key)
