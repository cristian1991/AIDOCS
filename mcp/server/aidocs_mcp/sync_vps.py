...
SERVER HALF — WRITTEN SPEC (STOPPED, evidence below; not shipped here)
────────────────────────────────────────────────────────────────────────────
    ...
    * ``http`` is the injectable channel: ``http(method, url, body_dict|None)
    ...
    def submit_events(self, batch: list[SyncEvent]) -> list[SyncReceipt]:
        ...
        if status != 200 or not isinstance(body, dict):
            raise VpsSyncError(f"submit_events: HTTP {status}")
    ...
    def pull_events(self, since: str | None = None) -> list[SyncEvent]:
        ...
        if status != 200 or not isinstance(body, dict):
            raise VpsSyncError(f"pull_events: HTTP {status}")
...
def reconcile_outbox(project_root: Path, transport: VpsApiTransport) -> dict:
    ...
    summary = {"ok": True, "submitted": 0, "accepted": 0, "rejected": 0, "pulled": 0, "error": ""}
    ...
    try:
        ...
        return {**summary, "ok": False, "error": f"{type(exc).__name__}: {exc}"}
    return summary
