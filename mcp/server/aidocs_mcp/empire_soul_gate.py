...
OP_READ = "read"
OP_WRITE = "write"
#: Lawbook seal (plan lawbook-seal-door). Rides the SAME per-turn, session-
#: keyed, single-use, TTL-belted grant rows the souls use; the `soul_id` column
#: holds the lawbook id. It carries no lineage acceptance — a lawbook is public
#: law, not a seat's continuity — so only its consume skips that check.
OP_SEAL = "seal"
...
# ── registry-backed lineage detection (#167 Phase 3, aidocs-doctrine §IX) ──
...
# honor §IX. Kept small and distinctive so ordinary prompts never match.
...
# grant that survives word-less turns (unlike the single-use OP grants) and is
# scoped to the MANAGED SESSION: the project/session permission scope.
#
# SCOPE = session_id, and that is the WHOLE key (KING RULING 2026-07-30 + #599).
# Three things converge on it:
#
#   1. THE RULING. Grants are agent_context based — project/session perms — and
#      they SURVIVE compaction. The epoch axis is for DEDUP and SURFACING only.
#      So the second axis's only job (rotate the lineage on compaction) is a job
#      the ruling explicitly does not want done.
#   2. THE PROCESS SPLIT. The mint runs in the fresh UPS/PreToolUse hook process,
#      which stamps NO ambient identity; the consume runs in the MCP server
#      process, which does. An identity-keyed row cannot be looked up without the
#      identity, so the key must be something BOTH sides receive as an ARGUMENT.
#      `resolve_soul_session` (below) is documented as THE single resolver both
#      sides go through, and the managed session_id it returns is exactly that —
#      passed in, never ambient, identical on both sides. It is already the
#      PRIMARY KEY here.
#   3. #599's MARKER CONTRACT. The retired second axis was derived under a
#      fabricated host kind and fell back to a global constant when unresolvable.
#      Both are unlawful now: the canonical unresolved marker is an honest empty
#      string per axis, never a shared sentinel that collapses distinct callers
#      into one bucket.
#
# The retired axis cost NO isolation on removal, because it was derived under a
# CONSTANT kind and therefore separated nothing. The `epoch` column is retained
# (written empty) for forensic continuity of pre-existing rows; it is never read.
#
# HONEST LIMIT: two hosts that resolve to the SAME managed session still share a
# lineage. That is managed-session resolution's business
# (`ManagedModeService.get_mode` per-conductor mapping vs singleton fallback), not
# this module's, and nothing here can fix it. Pinned by
# tests/security/test_soul_lineage_scope_contract.py so it is never mistaken for
# closed.
_LINEAGE_SCHEMA = """
...
def _conn(project_root: Path) -> sqlite3.Connection:
    ...
    # #814/#755: this module already imports the schema memo from the canonical
    # connect but was still opening the handle raw, so soul_grants.sqlite3 sat
    # on the default ROLLBACK JOURNAL — a writer takes an EXCLUSIVE whole-file
    # lock and a concurrent reader gets SQLITE_BUSY ("database is locked").
    # foreign_keys=ON comes with it, which is CORRECTNESS: sqlite defaults it
    # OFF per connection, leaving every declared FK inert.
    conn = _canonical_connect(db)
    # The DDL ran on EVERY connect, not just on init (#756). The memo
    # re-verifies the file, so a deleted DB rebuilds its schema.
    if not _schema_already_ensured(db, "empire_soul_gate"):
        conn.execute(_SCHEMA)
        conn.execute(_LINEAGE_SCHEMA)
        _mark_schema_ensured(db, "empire_soul_gate")
    return conn
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            for lawbook in sorted(seal_ids or set()):
                conn.execute(
                    "INSERT OR REPLACE INTO sovereign_soul_grants "
                    "(session_id, soul_id, operation, grant_id, expires_at) "
                    "VALUES (?, ?, ?, ?, ?)",
                    (sid, lawbook, OP_SEAL, secrets.token_hex(16), expires),
                )
            # STANDING acceptance: naming a soul this turn accepts the seat
            # for its MANAGED SESSION — the project/session permission scope, see
            # the `_LINEAGE_SCHEMA` note. Read and write evocations both accept
            # (write implies read). This is ADDITIVE — a word-less turn does NOT
            # revoke a standing lineage — which is what lets the accepted seat
            # persist across ordinary turns AND across compaction.
            #
            # The retired second-axis column is written EMPTY: an honest empty per
            # #599, never a sentinel. It is retained for forensic continuity of
            # pre-existing rows and is never read.
...
def granted_soul_lineages(project_root: Path, session_id: str) -> set[str]:
    """The souls this managed session has ACCEPTED and still holds.

    Scoped to the managed ``session_id`` — the project/session permission scope —
    which is PASSED IN by both the mint (hook process) and the consume (server
    process) via ``resolve_soul_session``. It is never read from ambient identity,
    which is what makes it survive the process split.

    A standing acceptance therefore SURVIVES compaction (the epoch axis is dedup,
    not authority) and lapses when the managed session changes. An empty or
    unresolvable session holds NOTHING — an honest empty, never a bucket.
    """
    sid = (session_id or "").strip()
    if not sid:
        return set()
    try:
        ...
        try:
            rows = conn.execute(
                "SELECT soul_id FROM sovereign_soul_lineages WHERE session_id = ?",
                (sid,),
            ).fetchall()
        finally:
            conn.close()
...
def consume_grant_detail(
    ...
    *,
    ...
    """Atomically CONSUME (single-use) an exact (session, soul, operation)
    grant — the detail surface behind ``consume_grant``. ``consume=False`` is a
    PEEK (a two-phase preview): same checks, nothing deleted. Returns
    ...
    if not sid or not soul or operation not in (OP_READ, OP_WRITE, OP_SEAL):
    ...
    if operation != OP_SEAL and soul not in granted_soul_lineages(project_root, sid):
...
# ── #223: ONE session resolver + host-agnostic minter (Article VI) ─
...
def resolve_soul_session(
    ...
    try:
        ...
        return explain_managed_session(mm, Path(project_root), host_session_id=hsid)
    except Exception:
...
def mint_turn_grants(
    ...
    *,
    ...
    try:
        from .canonical_intent_registry import detect_lawbook_seal_grants_v2

        seal_ids = detect_lawbook_seal_grants_v2(prompt)
    except Exception:
        seal_ids = set()
    ...
    if not sid:
        ...
        for soul in sorted(set(read_souls) | set(write_souls) | set(seal_ids)):
            record_soul_act(
                record_event,
                project_root,
                session_id="",
