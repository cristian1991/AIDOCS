...
EMPTY_REASONS = frozenset(
    {
        ...
        "timed_out",
        "pattern_invalid",
        # #1039 S3: the path/glob SCOPE is outside the grammar (unbalanced
        # brace / class). Distinct from path_not_indexed on purpose.
        "pattern_unsupported",
    },
)
...
def _validate_scope_glob(scope: str | None) -> str | None:
    """Return the grammar error for an unparseable path/glob scope, else None
    (#1039 S3). ONE grammar for every search tool: `code_index_store.validate_glob`."""
    if not scope:
        return None
    from .code_index_store import GlobPatternError, validate_glob

    try:
        validate_glob(str(scope))
    except GlobPatternError as exc:
        return str(exc)
    return None


def _pattern_unsupported_envelope(scope: str, error: str, *, for_find: bool = False) -> dict[str, Any]:
    """#1039 S3: a scope the glob grammar cannot parse is `pattern_unsupported`
    — NEVER `path_not_indexed`. The latter told the operator a file was absent
    from the index when the real fault was an unbalanced brace in the glob."""
    param = "path" if for_find else "glob"
    return {
        "total_matches": 0,
        "results": [],
        "empty_reason": "pattern_unsupported",
        "error": f"unsupported {param} pattern {scope!r}: {error}",
        "next_action": (
            f"Fix the {param}= pattern — the grammar is: plain prefix ('src'), "
            "'*' / '?' within a segment, '**' across segments, '{a,b}' "
            "alternation (nesting allowed), '[...]' character class. "
            "Case folds on case-insensitive filesystems."
        ),
    }
...
def _text_empty_envelope(
    ...
    *,
    ...
    # #1039: an empty content search over a scope with refused source files
    # is NOT a clean empty — name them.
    try:
        out = hub.code.annotate_unindexed(root, out, scope)
    except Exception:
        pass
    return out
...
def register_code_tools(
    *,
    ...
    def ai_text_search(
        ...
        # #1039 S3: an unparseable glob is pattern_unsupported, never
        # "path not indexed".
        _glob_err = _validate_scope_glob(glob)
        if _glob_err is not None:
            return _pattern_unsupported_envelope(str(glob), _glob_err)
    ...
    def ai_get_symbol_snippet(
        ...
        # flagged vendored=True (§XXVI vendored-source class).
    ...
    def ai_find(
        ...
        if m in ("text", "regex", "string"):
            ...
            if use_regex:
                ...
                if _rx_err is not None:
                    return _pattern_invalid_envelope(query, _rx_err)
            # #1039 S3: same grammar, same refusal as ai_text_search.
            _glob_err = _validate_scope_glob(path)
            if _glob_err is not None:
                return _pattern_unsupported_envelope(str(path), _glob_err, for_find=True)
            matches = hub.code.search_text(
                root,
                query,
                glob=(path or None),
    ...
    def memory_promote(
        ...
        The trusted path runs THROUGH the archive (empire-doctrine §XII: evidence may enter
    ...
    def memory_capture(
        ...
        # ── Pre-add FIT check (Empire directive 2026-07-06, empire §XI/§XII two-tier
        ...
        # Empire-worthiness: CANDIDATE flag only — empire-doctrine §XII: evidence may enter
