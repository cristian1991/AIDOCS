...
class AidocsManagedStore(SQLiteIndexStoreBase):
    ...
    def init_db(self, project_root: Path) -> None:
        """WRITE-PATH init: schema + the legacy JSON ingest (which DELETES the
        file). Reads use ``ensure_schema`` only (#1095)."""
        self.ensure_schema(project_root)
        self._ingest_legacy_json(project_root)

    def ensure_schema(self, project_root: Path) -> None:
        """Idempotent structural schema only -- safe on a read path (#1095):
        no ingest, no delete, no row written."""
    ...
    def migrate_legacy(self, project_root: Path) -> None:
        """LIFECYCLE migration of the legacy aidocs-managed.json (SessionStart
        rail, #1095): schema + ingest + retire the file. The same work init_db
        does on the write path, named for the lifecycle caller. Never a read."""
    ...
    def get(self, project_root: Path) -> dict[str, Any]:
        ...
        if row is None:
            # SQLite is the SOLE authority: no row reads as INACTIVE. The legacy
            # JSON is never read back -- a stale or restored file must not
            # resurrect a binding (#1095, r0b2 4c948fb4-080). It comes across
            # only via init_db / migrate_legacy on a write or lifecycle rail.
            return {
                "active": False,
                "session_id": None,
                "activated_at": None,
