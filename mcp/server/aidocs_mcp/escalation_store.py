...
class EscalationStore:
    ...
    def _read_conn(self, project_root: Path):
    ...
    def init_db(self, project_root: Path) -> None:
        ...
        if _schema_already_ensured(path, "escalation"):
            return
        path.parent.mkdir(parents=True, exist_ok=True)
        # #746: aidocs_identity.sqlite3 is SHARED by five stores, so whichever
        # one gets there first CREATES the file -- and the journal mode it
        # creates it with is the one every later connection inherits, because
        # journal_mode is persisted in the FILE HEADER. Establishing WAL only in
        # whichever store "happened to be first" is establishing it by luck, so
        # EVERY creator routes through the one canonical connect (#755). On the
        # default rollback journal a writer locks the WHOLE file and any reader
        # gets SQLITE_BUSY -- the "database is locked" of #746.
        with _canonical_connect(path, durability=_Durability.RUNTIME) as conn:
            ...
            for ddl in (
                ...
                try:
                    ...
                    pass
            conn.commit()
        _mark_schema_ensured(path, "escalation")
    ...
    def get(
        ...
        conn = self._read_conn(project_root)  # READ-ONLY: never creates (#553)
        if conn is None:
            return None
        with closing(conn):
            row = conn.execute(
    ...
    def find_live_grant(
        ...
        *,
        ...
        now_iso = _iso_now()
        with _canonical_connect(self.db_path(project_root), row_factory=False) as conn:
            # _row_to_grant reads columns BY NAME, so this MUST stay a Row
            # factory. The #553 read-only sweep stripped row_factory from every
            # method it converted to _read_conn (which sets it internally) —
            # but this method was never converted, so it lost the line with
            # nothing replacing it and started handing plain tuples to
            # _row_to_grant ("TypeError: tuple indices must be integers").
            # Every live-grant lookup died, which silently disabled
            # operator-approval LIFT: an approved retry could not find its
            # grant and fell through to heuristic_judge to be re-blocked.
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
    ...
    def restore_prompt_submit_state(
        ...
        *,
        ...
        # BORROWED HANDLE (#756). restore_scoped_rows opens `with conn:` on the
        # handle it is GIVEN -- a TRANSACTION on someone else's connection, not
        # ownership. A ClosingConnection would close there, yanking the handle
        # out from under this method mid-restore; the prompt-submit fail-open
        # then turns the error into None, degrading a REFUSAL into "carry on"
        # (caught once by tests/security/test_prompt_mutation_plan.py). So this
        # method OWNS the handle explicitly: mark it borrowed so every nested
        # `with conn:` commits without closing, and close it in `finally`.
