...
## session_start (REMOVED 2026-04-30)

The agent-facing `session_start` MCP tool was removed. The startup/session-selection flow is now exposed as `project_bootstrap_or_resume` (eager, system-controlled). The internal `runtime.session_start` Python method is preserved and used by `runtime_bootstrap_orchestration_service.project_bootstrap_or_resume` — but agents do NOT call `session_start` directly. Calling it via MCP raises NotFoundError.
...
Outputs
- same shape as `ai_search`, but against the related project
...
Outputs
- same shape as `ai_bundle`, but against the related project
...
Outputs
- repeated `ai_get_symbol_snippet`-style entries for each partial definition
