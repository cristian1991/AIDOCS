...
//
// SECOND COPY of the declaration in `core/plugins/aidocs.js:30`. Kept identical
// by a contract test, NOT by hand — they have drifted before (CHANGELOG.md:2746,
// "CRITICAL — apply_patch missing from OC plugin GUARDED_TOOLS").
//
// A tool absent from this set is NOT GATED AT ALL — `tool.execute.before`
// returns before any verdict, refusal or trace. Parity with the Claude Code
// declaration `claude_hooks_install.PRETOOLUSE_MATCHER_TOKENS` is asserted in
// mcp/tests/security/test_opencode_guarded_tools_drift_674.py, which NAMES that
// symbol rather than restating its value. Edit BOTH copies or the test fails.
const GUARDED_TOOLS = new Set([
  "read", "edit", "write", "glob", "grep", "bash", "task",
  // `agent`: subagent spawn. #674 — the 2026-07-09 "agents cheat" bypass shape.
  "agent",
  "multiedit", "patch", "apply_patch", "search", "listdir",
  // `skill`: 2026-07-30 skill-surface redirect. Read-path only.
  "skill",
  "notebookedit", "webfetch",
  // `schedulewakeup`: the force-wakeup guard's own stamp path.
  "schedulewakeup",
...
// so AIDOCS's own MCP tool surface is carried across as a PREFIX rule.
const GUARDED_TOOL_PREFIXES = ["mcp__"]

// Alias normalization, mirroring `canonicalToolName` in core/plugins/aidocs.js:
// hosts spell the same tool several ways and an unnormalized name misses the
// exact-match Set — which is exactly how `apply_patch` went ungated before.
...
// SECOND COPY of core/plugins/lib/python.js:resolvePythonBin — kept identical
// by the plugin-parity contract test, not by hand.
//
// The interpreter AIDOCS installed for itself is preferred over a bare `python`
...
// Measured 2026-08-14: bare `python` resolved to a uv-managed CPython whose
// `_sqlite3` DLL would not load, every bridge call returned null, and the plugin
// silently lost ALL AIDOCS state — managed mode, skills, prompt context.
...
// AIDOCS state, which is how the interpreter fault above stayed invisible.
// Announce the FIRST failure per process on stderr; never repeat, never throw.
...
const ACTION_TOOL_DIRECTIVES = {
  edit: '`task_begin` → `ai_get_lines` (read) → `ai_replace` (mode=string/anchor/symbol/lines) or `ai_batch_edit` (write) → `task_complete`. Do NOT mix edit methods. Before editing: `ai_get_symbol_info(kind="signature")` or `kind="constructor"` to confirm signatures. CSS: `ai_trace(class, mode="css_class")`. DB: `ai_schema(entity, mode="entity")`.',
  trace: '`ai_find(query, mode="references")` → `ai_trace(query, mode="field_flow"|"css_class"|"api_to_ui")`. DB: `ai_schema("Source→Target", mode="trace_path")`.',
  understand: '`session_resume_bundle` (project/session/skills/plan overview) → `action_surface_current_session_bundle` (likely next tools) → `ai_find(query, mode="symbols")` → `ai_get_symbol_snippet`. Precision: `ai_get_symbol_info(kind="signature"|"constructor"|"enum"|"api"|"properties")`. Broad: `ai_bundle(concept, mode="subsystem")`. DB: `ai_schema(name, mode="entity")`.',
  read_error: '`ai_find(symbol, mode="symbols")` → `ai_find(symbol, mode="references")` → `ai_get_symbol_snippet`. DB: `ai_schema(entity, mode="entity")`.',
  ...
  return {
    ...
    "command.execute.before": async ({ command, sessionID }, output) => {
      ...
      "tool.execute.before": async (input, output) => {
        ...
        if (!isGuardedTool(normalizedTool)) {
...
if (typeof process !== "undefined" && process.env.NODE_ENV === "test") {
  ...
  // #674 / #840: the guarded-tool matcher, exported so a test can EXECUTE it
  // instead of regexing the file it lives in. Before this, the ONLY check on
  // this copy's matcher was a `re.search` over the source text — which passes
  // just as happily when the matcher is wrong as when it is right.
  isGuardedTool,
  GUARDED_TOOLS,
