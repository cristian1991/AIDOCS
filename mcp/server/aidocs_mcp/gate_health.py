...
def _decline_probe(now: float, state_dir: Path | None) -> dict:
    ...
    if not p.is_file():
        return {"status": "ok", "reason": "", "recent_count": 0, "last_line": ""}
    try:
        lines = [
            ln.strip()
    ...
    for ln in lines[-200:]:  # tail only; the log is append-only forever
        ...
        if ts is not None and (now - ts) <= DECLINE_RECENT_WINDOW_S:
            recent += 1
            last_recent_line = ln
    if recent:
        return {
            ...
            "reason": (
                ...
                f"{int(DECLINE_RECENT_WINDOW_S // 60)}min — every one is a tool "
                f"call the gate did NOT govern"
            ),
            "recent_count": recent,
