...
_log = logging.getLogger("aidocs_mcp.skill_provider")
...
def load_bundled_provider_skills(
    ...
    for file in sorted(provider.root_path.glob("*.md")):
        try:
            ...
            # mis-encoded files. Skip loudly instead of loading mangled text.
            text = file.read_text(encoding="utf-8")
        except UnicodeDecodeError as exc:
            _log.warning("bundled skill %s is not valid UTF-8 (%s) — skipped", file, exc)
            continue
        except OSError:
            continue
...
def load_external_provider_skills(
    ...
    for file in sorted(skills_dir.glob("*/SKILL.md")):
        try:
            # Strict UTF-8 (#479): never half-read a mis-encoded skill file.
            text = file.read_text(encoding="utf-8")
        except UnicodeDecodeError as exc:
            _log.warning("external skill %s is not valid UTF-8 (%s) — skipped", file, exc)
            continue
        except OSError:
            continue
        try:
