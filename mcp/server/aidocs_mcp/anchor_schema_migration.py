...
KNOWN_SOURCES = frozenset(
    {
        ...
        "capture_analyzer",  # memory_capture bounded analyzer (semantic_guess only)
        "kg_seed",  # KG triple seeded via seed_from_entity_facts
        "leaf_resolver",  # #375 Phase 3 smallest-leaf resolver (semantic_guess only)
    },
)


# #375 Phase 3 (memory-home ruling 2026-07-19): the smallest resolvable
# code-index leaf a memory anchors at. Finer than anchor_kind (whose CHECK
# constraint on legacy tables cannot be widened by ALTER): anchor_kind stays
# in its legal set; leaf_granularity carries the full ladder.
VALID_LEAF_GRANULARITY = frozenset(
    {
        "",  # legacy rows / unresolved
        "file",
        "module",
        "family",  # a related group (class + members, partial-class set)
        "class",
        "function",
        "method",
        "symbol",  # any other named leaf (field, const, component…)
    },
)
...
#
# Doctrine (2026-05-29 — Empire triage, clean-VPS Gate 2b cluster A):
...
_BOOTSTRAP_FULL_SCHEMA_DDL = """
    CREATE TABLE memory_symbol_anchors (
        ...
        leaf_granularity TEXT NOT NULL DEFAULT '',
...
# against.
_RFC4_PHASE_2_0_COLUMNS = (
    "drawer_id",
    "content_hash",
    "confidence",
    "source",
    # #375 Phase 3 leaf anchoring — rides the same idempotent migration.
    "leaf_granularity",
)
...
def migrate_memory_symbol_anchors(conn: sqlite3.Connection) -> dict:
    ...
    if not table_exists:
        ...
        _add("leaf_granularity", "leaf_granularity TEXT NOT NULL DEFAULT ''")
...
def validate_leaf_granularity(value: str) -> str:
    """Application-level CHECK for ``leaf_granularity`` (#375 Phase 3).
    Same rationale as ``validate_confidence`` — ALTER cannot add CHECKs."""
    if value not in VALID_LEAF_GRANULARITY:
        raise ValueError(
            f"invalid leaf_granularity {value!r}; "
            f"must be one of {sorted(VALID_LEAF_GRANULARITY)}",
        )
    return value
