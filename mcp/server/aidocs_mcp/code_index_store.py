...
from ._sqlite_connect import Durability  # noqa: E402
from ._sqlite_connect import connect as _canonical_connect  # noqa: E402
from ._sqlite_connect_sampler import sample_connect as _sample_connect  # noqa: E402

# The local _ClosingConnection subclass that used to live here is GONE, not
# merely bypassed: `_sqlite_connect.ClosingConnection` is the same fix (commit
# or roll back exactly as sqlite3 would, then close in `finally`, with borrowed
# handles exempt) made ONCE where every store can reach it. Two copies of one
# decision is how the pragma debt this migration pays got started -- a decision
# that must be re-made at every site is a decision that rots (#755, #756).


# #1039 S3 — ONE path grammar for every search tool.
#
# The operator's `ai_text_search(glob="tests/**/*.cs")` came back
# `path_not_indexed` on a Windows checkout where the file was indexed as
# `Tests/Unit/Foo.cs`, and `{A,B}/**` came back `path_not_indexed` too. Both
# were grammar gaps reported as an index gap — the worst answer a search tool
# can give, because the caller concludes the code is ABSENT (law 311bf3e6).
#
#   * braces `{a,b}` (nested allowed) are alternation;
#   * `[...]` is a character class within one segment;
#   * `**` crosses `/`, `*` / `?` stay within a segment;
#   * on a case-insensitive filesystem (Windows, default macOS) the match
#     folds case, because that is how the read tools — which go through the
#     OS — already behave; the same pattern must scope the same files;
#   * a pattern the grammar cannot parse raises GlobPatternError so the tool
#     edge can say `pattern_unsupported` instead of "not indexed".
_PATHS_CASE_INSENSITIVE: bool = os.name == "nt" or sys.platform == "darwin"

_GLOB_META = "*?[{"


class GlobPatternError(ValueError):
    """A glob the search grammar cannot parse (unbalanced `{`/`[` etc.)."""


def validate_glob(pattern: str) -> None:
    """Raise GlobPatternError when `pattern` is not in the search grammar.

    Cheap structural check — balanced braces and character classes — run
    at the tool edge BEFORE a search so an unparseable scope is named as
    such and never reported as an empty index."""
    pat = str(pattern or "")
    depth = 0
    i, n = 0, len(pat)
    while i < n:
        c = pat[i]
        if c == "[":
            close = pat.find("]", i + 2) if i + 1 < n and pat[i + 1] in "!^]" else pat.find("]", i + 1)
            if close < 0:
                raise GlobPatternError(f"unclosed character class '[' at offset {i} in {pattern!r}")
            i = close + 1
            continue
        if c == "{":
            depth += 1
            ...
            depth -= 1
            if depth < 0:
                raise GlobPatternError(f"stray '}}' at offset {i} in {pattern!r}")
        i += 1
    if depth != 0:
        raise GlobPatternError(f"unbalanced braces in {pattern!r}")


@lru_cache(maxsize=256)
def _glob_to_regex(pattern: str) -> "re.Pattern[str]":
    """Compile a glob to a PATH-AWARE regex:
      * ``**`` matches across path separators (recursive),
      * ``*`` / ``?`` match WITHIN a single segment (never ``/``),
      * ``{a,b}`` is alternation (nesting allowed), ``[...]`` a class.
    Raises GlobPatternError on an unparseable pattern.
    """
    ...
    while i < n:
        ...
        if c == "*":
            if i + 1 < n and pattern[i + 1] == "*":
                out.append(".*")
                i += 2
                if i < n and pattern[i] == "/":  # `**/` consumes the slash
                    i += 1
                continue
            out.append("[^/]*")
        elif c == "?":
            ...
            # A COMMA IS ALTERNATION ONLY INSIDE BRACES (#1042).
            #
            # This appended "|" unconditionally, so a comma ANYWHERE became an
            # alternation bar — and a comma is a legal character in a filename.
            # `report,final.md` compiled to `report|final\.md`, i.e. "matches
            # `report` OR `final.md`": it stopped matching the file that
            # actually exists and started matching two that do not. Both a
            # false negative and a false positive from one character.
            #
            # Depth-tracked rather than "is there a brace anywhere", so nesting
            # works: in `{a,{b,c}}` every comma is at depth >= 1 and alternates
            # at its own level, while the comma in `a,b/{x,y}.py` is at depth 0
            # before the brace opens and stays literal.
            out.append("|" if brace_depth > 0 else re.escape(","))
        elif c == "[":
            close = (
                pattern.find("]", i + 2)
                if i + 1 < n and pattern[i + 1] in "!^]"
                else pattern.find("]", i + 1)
            )
        ...
        i += 1
    flags = re.IGNORECASE if _PATHS_CASE_INSENSITIVE else 0
    return re.compile("(?s:" + "".join(out) + r")\Z", flags)


def glob_match_path(rel_path: str, pattern: str) -> bool:
    """Path-aware glob match for search scoping. A pattern WITHOUT a ``/``
    matches the basename (so ``*.py`` matches a file at any depth); a pattern
    WITH a ``/`` matches the full POSIX relative path. Unlike ``fnmatch``,
    ``*`` does not cross ``/`` and ``**`` is true recursive glob. Empty
    pattern matches everything. An unparseable pattern matches NOTHING —
    callers that want the reason use ``validate_glob`` first.
    """
    if not pattern:
        return True
    ...
    try:
        return _glob_to_regex(pat).match(target) is not None
    except GlobPatternError:
        return False

# Tier-0 sensitive-file patterns — kept in sync with access_gate's
# _SENSITIVE_PATTERNS. Duplicated here (not imported) to avoid the
# access_gate → query_gate → code_index_store circular-import risk.
# Anything matching these NEVER enters the code index: the indexer
# would otherwise surface .env lines via ai_text_search and expose
# them to ai_get_lines reads later. Defense in depth — access_gate
# still blocks direct read attempts, but de-indexing at write time
# is the clean fix.
_INDEX_SENSITIVE_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"(^|/)\.env(\.|$)", re.IGNORECASE),
    re.compile(r"\.(key|pem|pfx|p12|jks|keystore)$", re.IGNORECASE),
    re.compile(r"(^|/)id_(rsa|ed25519|ecdsa|dsa)(\.pub)?$", re.IGNORECASE),
...
# Parity with access_gate._SENSITIVE_WORD_PATTERNS — see the long note there.
# A bare word anywhere in the path also DE-INDEXED ordinary source that merely
# implements credential handling, which is worse here than at the read gate:
# an unindexed file is not refused, it is INVISIBLE. `ai_find` reported zero
# references in `gate_credential_renewal.py` and its tests, so an audit that
# trusted the index silently under-reported on exactly the files that handle
# credentials. Project source is indexed; its READ is still gated.
_INDEX_SENSITIVE_WORD_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"credential", re.IGNORECASE),
    re.compile(r"secret", re.IGNORECASE),
)

# Kept in sync with access_gate._CODE_SOURCE_SUFFIXES for the files the index
# actually accepts. Duplicated, like the patterns above, to avoid the
# access_gate -> query_gate -> code_index_store circular import.
_INDEX_SOURCE_SUFFIXES: tuple[str, ...] = (
    ".py", ".pyi", ".pyx",
    ...
    ".swift", ".m", ".mm",
    ".vue", ".svelte", ".dart", ".lua", ".sql",
)  # fmt: skip


# Bytes sniffed from a candidate file before it is allowed into the index.
# Mirrors access_gate._SECRET_SNIFF_BYTES: credential material lives at the top
# of key/token/env files, so a bounded head is cheap and catches what a
# NAME-only classifier misses.
_INDEX_SECRET_SNIFF_BYTES = 65536


def may_enter_index(rel_path: str, abs_path: str | Path | None) -> bool:
    """THE ONE ADMISSION AUTHORITY. Fail-CLOSED.

    Every path that puts a file into the code index asks THIS, and nothing else
    decides. Before it existed there were two ingestion paths and only one of
    them asked anything:

      * `_walk_source_files` — the full tree walk — applied the name blocklist
        and (after #1037) the content gate.
      * `_collect_scoped_inventory` — the #224 scoped/incremental fast path —
        "build inventory records for ONLY the scoped paths, no source-tree
        walk". It therefore skipped the walk AND EVERY CHECK INSIDE IT.

    That second path is not an edge case, it is the HOT path: the sync service's
    own comment notes "post-edit reindex paths call sync_code_files directly",
    and `sync_session_code` funnels into it too. So the ordinary
    edit-then-reindex cycle admitted files that the full walk would have
    refused — including on their NAME. A `.env` reached by a scoped sync was
    indexed; the same `.env` found by the walk was not. Two answers to one
    question, decided by which code path happened to run.

    FAIL-CLOSED IS THE POINT AND IT IS A DELIBERATE REVERSAL. The first cut of
    the content gate returned "not secret" when a file could not be read, so a
    transient IO error degraded to ADMIT. For a gate whose only job is to keep
    key material out of a searchable store, "I could not check" must mean NO.
    The cost is bounded and self-correcting — an unreadable file is one the
    indexer could not have parsed anyway, and the next sync re-asks.

    Returns True to ADMIT. Callers skip the file on False.
    """
    if _is_sensitive_for_index(rel_path):
        return False
    if abs_path is None:
        return False  # nothing to inspect -> cannot clear it -> refuse
    return _credential_scan(str(abs_path)) == "clean"


def _credential_scan(abs_path: str) -> str:
    """'clean' | 'credential' | 'unreadable'.

    Three answers rather than a bool, because "no credential found" and "could
    not look" are different facts and collapsing them is what made the first
    version fail open.

    WHY THE INDEXER NEEDS ITS OWN SNIFFER AT ALL. #1037 let indexed project
    source past the tier-0 credential/secret NAME patterns, arguing that
    "`_content_is_secret` still sniffs content". That sniffer lives in
    access_gate, on the READ path; THE INDEXING PATH HAD NO CONTENT CHECK OF
    ANY KIND. Only a path blocklist stood between a file and `ai_text_search`,
    and this module's own comment names the threat: "ai_text_search on a benign
    query could surface lines from .env files into agent context — and
    ai_get_lines would happily read back the indexed path later."

    Same detector and threshold as the read gate — `output_guard.scan_text`,
    blocking only on `credential:*` findings (PEM/OpenSSH private keys,
    GitHub/OpenAI/Anthropic/Stripe/Slack keys, JWTs, quoted api_key/password/
    secret assignments). The broad `sensitive:*` env/ssh heuristics stay
    excluded so Makefiles and .properties are not swept in; real `.env` files
    are name-blocked upstream anyway.
    """
    try:
        import os
        ...
        with open(abs_path, "rb") as fh:
            head = fh.read(_INDEX_SECRET_SNIFF_BYTES)
    except (OSError, ValueError):
        return "unreadable"
    if not head:
        return "clean"  # an empty file carries nothing
    text = head.decode("utf-8", errors="replace")
    try:
        # Imported lazily: output_guard is heavier than this module and the
        # access_gate -> query_gate -> code_index_store chain must not gain a
        # new import edge (the same reason the patterns above are duplicated).
        from .output_guard import scan_text

        result = scan_text(text, redact=False)
    except Exception:
        return "unreadable"  # the detector failed -> we did not clear it
    return (
        "credential"
        if any(f.category.startswith("credential:") for f in result.findings)
        else "clean"
    )


def _is_sensitive_for_index(rel_path: str) -> bool:
    """Refuse to index any file matching tier-0 sensitive patterns.
    rel_path is project-relative, forward-slash normalized.
    """
    if not rel_path:
        return False
    normalized = rel_path.replace("\\", "/")
    if any(p.search(normalized) for p in _INDEX_SENSITIVE_PATTERNS):
        return True
    if normalized.lower().endswith(_INDEX_SOURCE_SUFFIXES):
        return False
    return any(p.search(normalized) for p in _INDEX_SENSITIVE_WORD_PATTERNS)
...
def _scope_predicate(glob: str | None):
    """Path-scope predicate shared by search_text and count_files_in_scope
    (#478 secondary). Glob metacharacters keep full glob_match_path
    semantics; a PLAIN pattern (no * ? [) additionally scopes as a path
    PREFIX — "src" matches src/app.py but never src_other/x.py."""
    if not glob:
        return lambda _rel: True
    scope = str(glob)
    has_meta = any(c in scope for c in _GLOB_META)
    ...
    def _in_scope(rel: str) -> bool:
        ...
        if glob_match_path(rel_norm, scope):
            return True
        if has_meta:
            return False
        if _PATHS_CASE_INSENSITIVE:
            rel_norm = rel_norm.lower()
        return rel_norm == prefix or rel_norm.startswith(prefix + "/")
...
from .language_descriptors import (
    language_for_builtin_descriptor,
    language_for_custom_descriptor,
    layer_from_descriptor,
    load_index_config,
    module_hints_from_descriptors,
    role_from_descriptor,
)
from .session_store import SessionStore

#: Ordering for a trace match's `source` — the truth-kind #683 section B asks
#: every answer to declare. `schema` is the only EXACT evidence (the field was
#: resolved in the schema index); `symbol` is a name that merely CONTAINS the
#: needle; `text` is a hit in file content or a summary. Lower sorts first, so a
#: reader who stops at the first match gets the resolved field rather than a
#: filesystem helper that happens to share a word with it. Unknown/unstamped
#: sorts last rather than raising — a new source must degrade, not explode.
_TRACE_SOURCE_RANK = {"schema": 0, "symbol": 1, "text": 2}


class CodeIndexStore:
    """Derived SQLite index for repository code files and lightweight summaries."""

    INDEX_VERSION = "code-index-v8"
    ...
    def connect(self, project_root: Path) -> sqlite3.Connection:
        # RUNTIME (the default). This is the CODE INDEX: a projection of source
        # files that `ai_index_sync` rebuilds from the tree on demand, which is
        # the definition of re-derivable. Nothing it holds is a grant or a
        # consumption mark, so a power cut that loses the last commits costs a
        # re-sync, never an authority. It is also the single hottest database
        # in the project, so this is precisely where synchronous=NORMAL's 8-10x
        # (#754: 83ms vs 8-10ms over 20 commits) actually lands.
        #
        # The local _ClosingConnection factory this replaced already fixed the
        # #756 handle leak, and the canonical ClosingConnection does the same
        # thing (commit-then-close on __exit__, borrowed handles exempted), so
        # lifecycle behaviour is unchanged. What this store had NONE of is the
        # pragma set: no WAL, no synchronous, no busy_timeout, no foreign_keys.
        # foreign_keys=ON is inert here — this schema declares no FOREIGN KEY —
        # so turning it on cannot refuse a row that used to be accepted.
        #
        # mkdir stays HERE and is not pushed into the helper: the helper
        # deliberately never materialises a tree ("opening a store is not an
        # adoption"), and this caller is the one entitled to create it.
        db_path = self.db_path(project_root)
        db_path.parent.mkdir(parents=True, exist_ok=True)
        conn = _canonical_connect(db_path, durability=Durability.RUNTIME)
        _sample_connect("CodeIndexStore", db_path)
        return conn
        ...
        with self.connect(project_root) as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS code_references (
                    ...
                    PRIMARY KEY (path, line_number, token, kind)
                );

                CREATE INDEX IF NOT EXISTS idx_code_references_kind_token
                    ON code_references (kind, token);

                -- #1039: source files the admission authority REFUSED. A search
                -- that cannot see them must SAY so instead of returning a
                -- confident empty list (the #925 false-green).
                CREATE TABLE IF NOT EXISTS code_files_excluded (
                    path TEXT PRIMARY KEY,
            ...
            # The symbol a reference sits INSIDE. Added after the table shipped,
            # so old DBs upgrade here instead of breaking. Without it there is no
            # way to derive caller->callee: code_outlines stores no end-line, so
            # a reference's enclosing symbol cannot be recovered after the fact.
            self._ensure_column(conn, "code_references", "enclosing", "TEXT NOT NULL DEFAULT ''")
            # AFTER the column exists — an index on `enclosing` inside the
            # CREATE script above fails on any pre-column DB ("no such column").
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_code_references_enclosing "
                "ON code_references (path, enclosing)",
            )
            self._ensure_index_version(conn)

    _COMMISSION_KEY = "aidocs_commissioned"

    # ── #1039: refused-source ledger ───────────────────────────────────
    #
    # `ai_find(mode='references')` sweeps file CONTENT of indexed files, so a
    # file the admission authority refused is invisible to it — and its
    # "no references" answer looked exactly like a finding (measured: the
    # symbols of gate_credential_renewal.py and its tests; part of why #925
    # stayed open twelve days). The walk now records what it refused; every
    # content search reports "N source files in scope were NOT indexed"
    # alongside its result instead of a confident empty list.
    ...
    def record_excluded(
        ...
        *,
        ...
        """Persist the walk's refusals. ``replace=True`` (a full walk) makes
        the table exactly the refused set; ``replace=False`` (a scoped sync)
        upserts the refused rows and drops rows for paths now ``admitted``."""
        from datetime import datetime as _dt

        now = _dt.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
        with self.connect(project_root) as conn:
            if replace:
                conn.execute("DELETE FROM code_files_excluded")
            for rel, reason in refused:
                conn.execute(
                    "INSERT INTO code_files_excluded (path, reason, observed_at) VALUES (?, ?, ?) "
                    "ON CONFLICT(path) DO UPDATE SET reason=excluded.reason, "
                    "observed_at=excluded.observed_at",
                    (str(rel).replace("\\", "/"), str(reason), now),
                )
            for rel in admitted or []:
                conn.execute(
                    "DELETE FROM code_files_excluded WHERE path = ?",
                    (str(rel).replace("\\", "/"),),
                )

    def excluded_in_scope(
        self,
        project_root: Path,
        scope: str | None = None,
        *,
        ...
        """Source files under ``scope`` (same grammar as search_text) that the
        admission authority refused — {count, reasons, examples}."""
        try:
            self.init_db(project_root)
            with self.connect(project_root) as conn:
                rows = conn.execute(
                    "SELECT path, reason FROM code_files_excluded ORDER BY path",
                ).fetchall()
        ...
        for row in rows:
            ...
            if len(examples) < limit:
                examples.append({"path": path, "reason": reason})
        return {"count": count, "reasons": reasons, "examples": examples}

    def annotate_unindexed(
        self,
        project_root: Path,
        out: dict[str, object],
        scope: str | None = None,
    ) -> dict[str, object]:
        """Stamp a search result with the refused-source facts (#1039).

        ``files_not_indexed`` / ``not_indexed_reasons`` / ``not_indexed_examples``
        ride along on EVERY result with refusals in scope; when the result is
        EMPTY the next_action says it out loud, because that is the case where
        the silence would otherwise read as a finding."""
        try:
            facts = self.excluded_in_scope(project_root, scope)
        except Exception:
            return out
        count = int(facts.get("count") or 0)
        if not count:
            return out
        out["files_not_indexed"] = count
        out["not_indexed_reasons"] = facts.get("reasons") or {}
        out["not_indexed_examples"] = facts.get("examples") or []
        empty = not any(out.get(k) for k in ("matches", "results"))
        if empty:
            shown = ", ".join(str(e.get("path")) for e in (facts.get("examples") or []))
            note = (
                f" NOT A CLEAN EMPTY: {count} source file(s) in scope were refused by "
                f"the index admission gate ({', '.join(facts.get('reasons') or {})}) "
                f"and could not be searched — e.g. {shown}. Their content is invisible "
                f"to this sweep; read them directly if they could hold the answer."
            )
            out["next_action"] = str(out.get("next_action") or "").rstrip() + note
        return out

    def mark_commissioned(self, project_root: Path) -> None:
        """Stamp the index db as a DELIBERATELY-commissioned AIDOCS project.

        ONLY project_init calls this. is_aidocs_managed keys on this stamp — NOT
        on the db file's mere existence, which any store touch (list_sessions,
        memory ops, an incidental init_db) creates. Without a deliberate
        commission a project stays UNMANAGED even if a db file appears — otherwise
        a normal prompt on a non-AIDOCS project would force it managed.
        """
        from .mcp_server_runtime_helpers import stamp_commissioned
    ...
    # ── Index policy (downgrade guard) ──────────────────────────────────

    def _read_index_policy(self, project_root: Path) -> dict[str, str]:
        import sqlite3

        try:
            with self.connect(project_root) as conn:
                rows = conn.execute(
                    "SELECT key, value FROM index_meta WHERE key LIKE 'index_policy_%'",
                ).fetchall()
        ...
        *,
        ...
        with self.connect(project_root) as conn:
            conn.execute(
                ...
                ("index_policy_include_tests", str(include_tests).lower()),
            )
            conn.execute(
                "INSERT OR REPLACE INTO index_meta (key, value) VALUES (?, ?)",
                ("index_policy_label", effective_policy_label),
            )
            conn.execute(
                "INSERT OR REPLACE INTO index_meta (key, value) VALUES (?, ?)",
                ("index_policy_provenance", provenance),
            )
            conn.execute(
                "INSERT OR REPLACE INTO index_meta (key, value) VALUES (?, ?)",
                ("index_policy_writer_pid", str(os.getpid())),
            )
            conn.execute(
                "INSERT OR REPLACE INTO index_meta (key, value) VALUES (?, ?)",
                ("index_policy_writer_started_at", now),
            )
    ...
    def get_module_files(
        ...
        return self._modules.get_module_files(
            project_root,
            module_path,
            limit=limit,
    ...
    def find_untested_files(
        ...
        """Source files with no matching test file in the index.

        Bonus 2026-04-19. Heuristic test-coverage gap surface: a source
        file `src/foo.py` is considered "tested" if any indexed file
        with role='test' has `foo` in its basename. Conservative match —
        false negatives (real coverage missed) are fine; false positives
        (claiming covered when not) are what we avoid.

        Returns list of {path, language, line_count} sorted by line_count
        desc so the most-impactful gaps surface first.
        """
        ...
        with self.connect(project_root) as conn:
            ...
            test_rows = conn.execute("SELECT path FROM code_files WHERE role = 'test'").fetchall()
        # Index test basenames (without test_ prefix and without .ext).
        test_stems: set[str] = set()
        for row in test_rows:
            tp = str(row["path"])
            stem = tp.rsplit("/", 1)[-1].rsplit(".", 1)[0]
        ...
        untested: list[dict[str, object]] = []
        for row in source_rows:
            sp = str(row["path"])
            if glob and not glob_match_path(sp, glob):
                continue
            stem = sp.rsplit("/", 1)[-1].rsplit(".", 1)[0]
            if stem.lower() in test_stems:
                continue
            # Skip __init__ / conftest / very small wrappers — noise.
            if stem in ("__init__", "conftest", "__main__"):
                continue
            untested.append(
                {
                    "path": sp,
        ...
        untested.sort(key=lambda x: int(x.get("line_count", 0)), reverse=True)
        return untested[: int(limit)]
    ...
    def sync_code_files(
        ...
        return self._sync.sync_code_files(
            project_root, paths=paths, include_tests=include_tests, incremental=incremental
        )
    ...
    def code_index_db_status(self, project_root: Path) -> dict[str, object]:
        return self._sync.code_index_db_status(project_root)
    ...
    def search_code(
        ...
        mtime_filter = ""
        params: list[object] = []
        # #1039 polish: "migration sql" never matched tools/sql/migration.py
        # because the whole phrase was one LIKE. A multi-term query collects
        # per term and ranks by how many terms the PATH SEGMENTS + summary
        # match (AND-first), so file names are found by their parts.
        from .concept_rank import query_terms, terms_matched, text_tokens

        _terms = query_terms(needle) if needle else []
        if needle and len(_terms) >= 2:
            where = " OR ".join("(path LIKE ? OR summary LIKE ?)" for _ in _terms)
            for t in _terms:
        ...
        for row in rows:
            ...
            if len(_terms) >= 2:
                # AND-first: every distinct term found in the path segments
                # or summary outweighs any single whole-phrase hit.
                _n = terms_matched(_terms, text_tokens(str(row["path"]), str(row["summary"] or "")))
                score += 150 * _n
                if _n:
                    reasons.append(f"terms_matched:{_n}/{len(_terms)}")
        ...
        # `why` is an opt-in ranker trace. Emitting it on every result
        # row inflates routine search responses with debug noise that
        # crowds out the actual content (see agent DX complaint
        # 2026-04-24, mirrored in symbol search). High-stakes
        # ranking surfaces (find_hotspots, find_query_hotspots) emit
        # `why` unconditionally because the ranking decision IS the
        # answer there; routine search keeps the env-gate.
        import os as _os_rt
    ...
    def is_file_stale(self, project_root: Path, path: str) -> bool:
        ...
        with self.connect(project_root) as conn:
            row = conn.execute(
                "SELECT mtime_ns, size_bytes FROM code_files WHERE path = ?",
                (rel,),
            ).fetchone()
        if row is None:
        ...
        try:
            stat = abs_path.stat()
            return int(stat.st_mtime_ns) != int(row["mtime_ns"]) or int(stat.st_size) != int(
                row["size_bytes"],
            )
        except Exception:
            return False
    ...
    def search_text(
        ...
        *,
        ...
        ``only_tests`` inverts the role filter: search test/fixture-role files
        EXCLUSIVELY. It answers "find the test that covers X", which
        ``include_tests`` (search production AND tests) cannot express, and it
        wins over ``include_tests`` when both are set.
        """
        self.init_db(project_root)
        raw = text.strip()
        if not raw:
        ...
        if regex:
            try:
                # re.MULTILINE (#482, War HH): matching is per-LINE semantics
                # everywhere in this tool — the file-level prefilter runs the
                # pattern against whole-file content, so without MULTILINE an
                # anchored alternation branch ("^def a|^def b") silently
                # matched only at byte 0 and whole files were dropped even
                # though their lines matched.
                flags = _re.MULTILINE | (0 if case_sensitive else _re.IGNORECASE)
        ...
        with self.connect(project_root) as conn:
            ...
            params: list[object] = []
            if only_tests:
                query_sql += " WHERE role IN ('test', 'fixture')"
            elif not include_tests:
                query_sql += " WHERE (role IS NULL OR role NOT IN ('test', 'fixture'))"
            rows = conn.execute(query_sql, params).fetchall()

        # Scope filter — shared with count_files_in_scope (#478 secondary)
        # so "path not indexed" is judged by the exact same predicate that
        # scoped the search.
        _in_scope = _scope_predicate(glob)
        ...
        for row in rows:
            ...
            if regex:
                if not pattern.search(content if case_sensitive else search_content):
                    continue
            elif not any(n in search_content for n in needles):
                continue
            ...
            for i, line in enumerate(content.splitlines(), 1):
                ...
                if hit:
                    ...
                    if len(_stripped) > 300:
                        # Minified/generated line (a bundled dist .js is one
                        # ~15KB line): return a window CENTERED on the match, not
                        # the whole line, so one hit can't flood the result.
                        if regex:
                            _m = pattern.search(line if case_sensitive else check_line)
                            _pos = _m.start() if _m else 0
                        else:
                            _found = [check_line.find(n) for n in needles]
                            _pos = min([p for p in _found if p >= 0], default=0)
                        _start = max(0, min(_pos - 150, len(_stripped) - 300))
                        _end = _start + 300
                        _stripped = (
                            ("…" if _start > 0 else "")
                            + _stripped[_start:_end]
                            + ("…" if _end < len(_stripped) else "")
                        )
                        lines_matched.append(
                            {"line_number": i, "line": _stripped, "truncated": True},
                        )
                    else:
                        lines_matched.append({"line_number": i, "line": _stripped})
        ...
        # Text search is literal only. Fuzzy lives in symbol search.

        return matches

    def count_files_in_scope(
        self,
        project_root: Path,
        scope: str | None,
        *,
        ...
        """How many indexed files fall under a path/glob scope (#478
        secondary — path-scoped zero ambiguity). Uses the exact same
        predicate as search_text so the answer is honest:

          in_scope           — files visible under the current test filter
          in_scope_all_roles — files regardless of role filtering

        in_scope_all_roles == 0  → the path is NOT in the index at all
        (path_not_indexed); in_scope == 0 with in_scope_all_roles > 0 →
        only test/fixture files live there (pass include_tests=True).
        """
        self.init_db(project_root)
        with self.connect(project_root) as conn:
            rows = conn.execute("SELECT path, role FROM code_files").fetchall()
        pred = _scope_predicate(scope)
        total = 0
        non_test = 0
        for row in rows:
            if not pred(str(row["path"])):
                continue
            total += 1
            if str(row["role"] or "") not in ("test", "fixture"):
                non_test += 1
        return {
            "in_scope": total if include_tests else non_test,
            "in_scope_all_roles": total,
        }
    ...
    def _preview_deps_python(
        ...
        return self._analysis._preview_deps_python(
            path,
            full_text,
            block_text,
            outside_text,
    ...
    def _preview_deps_js(
        ...
        return self._analysis._preview_deps_js(
            path,
            full_text,
            block_text,
            outside_text,
    ...
    def find_symbol_range(
        ...
        return self._analysis.find_symbol_range(
            project_root,
            path,
            symbol,
            kind=kind,
    ...
    def suggest_extractions(
        ...
        return self._analysis.suggest_extractions(
            project_root,
            path,
            min_lines=min_lines,
            limit=limit,
    ...
    def find_stale_references(
        ...
        *,
        ...
        return self._analysis.find_stale_references(
            project_root,
            symbols,
    ...
    def search_symbols(
        ...
        return self._symbols.search_symbols(
            project_root,
            query,
            kind=kind,
            role=role,
    ...
    def search_symbols_tiered(
        ...
        """Tiered symbol search with the no-padding law (War AW)."""
        return self._symbols.search_symbols_tiered(
            project_root,
            query,
            kind=kind,
            role=role,
    ...
    def resolve_symbol(
        ...
        return self._symbols.resolve_symbol(project_root, symbol, path=path, kind=kind)

    def decorate_symbol_hits(
        self,
        project_root: Path,
        hits: list[dict[str, str | int | bool | None]],
    ) -> list[dict[str, str | int | bool | None]]:
        """Tool-layer compaction: line_end spans + member folding."""
        return self._symbols.decorate_hits(project_root, hits)
    ...
    def get_method_signature(
        ...
        return self._symbols.get_method_signature(
            project_root,
            method_name,
            container=container,
            limit=limit,
    ...
    def get_method_signatures(
        ...
        return self._symbols.get_method_signatures(
            project_root,
            methods,
            container=container,
    ...
    def get_enum_values(
        ...
        return self._symbols.get_enum_values(
            project_root,
            enum_name,
            limit=limit,
    ...
    def get_constructor_params(
        ...
        return self._symbols.get_constructor_params(
            project_root,
            type_name,
            limit=limit,
            include_related=include_related,
    ...
    def get_constructor_params_batch(
        ...
        return self._symbols.get_constructor_params_batch(
            project_root,
            types,
            include_related=include_related,
    ...
    def find_references(
        ...
        return self._symbols.find_references(
            project_root, symbol, limit=limit, budget_seconds=budget_seconds,
        )
    ...
    def trace_field_flow(
        ...
        try:
            ...
            # DO NOT FAIL OPEN INTO SILENCE. This is the only EXACT source in
            # the trace; if it dies, the remaining two are name and text
            # guesses. Returning [] quietly let a degraded trace look like a
            # confident one, so the failure now rides in the response.
            schema_fields = []
        ...
        for item in symbol_matches:
            ...
            entry = {
                ...
                # A NAME match, not a resolved field (#683 section B). This
                # symbol merely CONTAINS the needle: tracing `chain_seq` this
                # way returned filesystem-ancestor walkers and shell-chain
                # evaluators. Labelled rather than dropped — recall here is
                # genuinely good, and a field reached through a differently
                # named accessor lands in exactly this bucket.
                "source": "symbol",
            }
            ...
            merged.append(
                {
                    ...
                    # The needle appeared in the file's text or summary. Weakest
                    # of the three, and the bucket that carries indexed scratch
                    # dumps and markdown prose.
                    "source": "text",
                },
            )
        ...
        # PROVENANCE OUTRANKS LAYER (#683 section B). A reader who stops at the
        # first match must get the resolved schema field, not a symbol that
        # merely shares a word with it. Layer ordering still decides WITHIN a
        # provenance band, so the model->service->UI story survives intact.
        merged.sort(
            key=lambda item: (
                _TRACE_SOURCE_RANK.get(str(item.get("source") or ""), 9),
                self._layer_rank(str(item["layer"])),
                str(item["path"]),
        ...
        if schema_error:
            # Surfaced, not logged and forgotten: without the exact source this
            # trace is name and text guesses only, and the caller must know.
            result["schema_lookup_failed"] = schema_error
        return result

    _SETTING_KEY_SPLIT = re.compile(r"__|[:.]")

    def _setting_literal_rows(
        self,
        project_root: Path,
        needle: str,
        limit: int = 50,
    ) -> list[dict[str, object]]:
        """Content rows for the LITERAL setting key and its JSON default
        (#1039 A+ 6). Score 1000 for the key string itself (any spelling:
        ``A:B``, ``A__B``, ``A.B``), 900 for the JSON default (the last
        segment as a key in a .json file that also carries the root
        segment). Empty when the needle is a bare word with no literal hit."""
        rows: list[dict[str, object]] = []
        seen_lines: set[tuple[str, int]] = set()
        parts = [p for p in self._SETTING_KEY_SPLIT.split(needle) if p]
        if len(parts) < 2:
            # A bare word is not a config-key literal; the keyword tier is
            # the right answer for it (and a literal sweep for "days" would
            # be the very flood this fixes).
            return rows
        spellings = list(
            dict.fromkeys(
                s for s in (needle, ":".join(parts), "__".join(parts), ".".join(parts)) if s.strip()
            ),
        )

        def _add(path: str, line_number: int, line: str, score: int, kind: str) -> None:
        ...
        for spelling in spellings:
            try:
                # Case-insensitive: env spellings are CLEANUP__DAYS while the
                # .NET key is Cleanup:Days — one setting, several cases.
                hits = self.search_text(
                    project_root, spelling, case_sensitive=False, limit=limit, include_tests=True,
                )
            except Exception:
                hits = []
            for hit in hits:
                for ln in list(hit.get("lines") or []):
                    _add(str(hit["path"]), int(ln.get("line_number") or 0), str(ln.get("line") or ""), 1000, "setting_literal")
        if len(parts) >= 2:
            root_key, leaf_key = parts[0], parts[-1]
            try:
                json_hits = self.search_text(
                    project_root, f'"{leaf_key}"', glob="*.json", case_sensitive=True,
                    limit=limit, include_tests=True,
                )
            except Exception:
                json_hits = []
            for hit in json_hits:
                path = str(hit["path"])
                try:
                    body = (project_root / path).read_text(encoding="utf-8", errors="ignore")
                except OSError:
                    continue
                if f'"{root_key}"' not in body:
                    continue
                for ln in list(hit.get("lines") or []):
                    _add(path, int(ln.get("line_number") or 0), str(ln.get("line") or ""), 900, "setting_json_default")
        return rows[: max(limit, 1)]

    def trace_setting_usage(
        self,
        project_root: Path,
        setting_name: str,
        limit: int = 50,
        ...
        # #1039 A+ 6: THE LITERAL KEY FIRST. ai_trace(mode='setting',
        # query='Cleanup:Days') returned 50 keyword hits on "days" with the
        # real key buried mid-list, because the symbol/file probes score
        # NAMES. A setting is a STRING: the definition is
        # GetValue<T>("Cleanup:Days") / config["Cleanup:Days"] /
        # os.environ["CLEANUP__DAYS"], and its default is the JSON path
        # Cleanup -> Days. Sweep indexed content for the literal (and its
        # A__B / A.B spellings) and for the JSON default, and put those
        # rows ABOVE every keyword match. Nothing is removed — the keyword
        # tier follows, exactly as before.
        merged.extend(self._setting_literal_rows(project_root, needle, limit=limit))
        for row in merged:
            seen.add((str(row["path"]), None, int(row["line_number"] or 0)))
    ...
    def trace_model_usage(
        ...
        if not needle:
            return {"definitions": [], "schema": {}, "references": []}
        ...
        try:
            ...
            if not schema.get("entities") and not schema.get("fields"):
                for variant in variants:
                    schema = schema_store.trace_entity_flow(
                        project_root,
                        entity_name=variant,
                        limit=limit,
                    )
        ...
        match_count = (
            len(symbol_bundle.get("definitions", []))
            + len(symbol_bundle.get("references", []))
    ...
    def search_concept(
        ...
        """AND-first concept search (#1039 A+ 5) — see the symbol service."""
        return self._symbols.search_concept(project_root, query, limit=limit)
    ...
    def find_domain_clusters(
        ...
        result = self._hotspots.find_domain_clusters(project_root, concept, limit=limit)
        result.setdefault("concept", concept.strip())
        return result
    ...
    def find_duplicate_structures(
        ...
        return self._hotspots.find_duplicate_structures(
            project_root,
    ...
    def trace_component_usage(
        ...
        result = self._hotspots.trace_component_usage(project_root, component_name, limit=limit)
        result.setdefault("component", component_name.strip())
        return result
    ...
    def find_state_model_mismatch(
        ...
        result = self._hotspots.find_state_model_mismatch(project_root, concept, limit=limit)
        result.setdefault("concept", concept.strip())
        return result
    ...
    def find_ui_backend_touchpoints(
        ...
        result = self._route_queries.find_ui_backend_touchpoints(project_root, concept, limit=limit)
        result.setdefault("concept", concept.strip())
        return result
    ...
    def find_policy_surfaces(
        ...
        result = self._route_queries.find_policy_surfaces(project_root, concept, limit=limit)
        result.setdefault("concept", concept.strip())
        return result
    ...
    def find_factories(
        ...
        return self._hotspots.find_factories(
            project_root,
            query,
            include_tests=include_tests,
            limit=limit,
    ...
    def find_frontend_symbols(
        ...
        results = self._hotspots.find_frontend_symbols(
            project_root,
            query=query,
            kinds=kinds,
            limit=limit,
    ...
    def get_symbol_snippet(
        ...
        if row is None:
            # ONE-RESOLVER seam (War AW): the exact triple missed — resolve
            # through the SAME scorer ai_find uses, scoped to the SAME path
            # (no read-gate widening: only the requested file can answer).
            # Kills the resolver-disagreement class: a symbol findable by
            # ai_find is snippet-resolvable with the same query string.
            resolved = self._symbols.resolve_symbol(
                project_root,
                symbol,
                path=path,
                kind=kind,
                ...
                with self.connect(project_root) as conn:
                    row = conn.execute(
                        ...
                        (
                            str(resolved["path"]),
                            str(resolved["symbol"]),
                            int(resolved["line_number"]),  # type: ignore[arg-type]
                        ),
                    ).fetchone()
        if row is None:
    ...
    def get_symbol_bundle(
        ...
        return self._bundles.get_symbol_bundle(
            project_root,
            symbol,
            path=path,
            kind=kind,
    ...
    def get_subsystem_bundle(
        ...
        result = self._bundles.get_subsystem_bundle(project_root, concept, limit=limit)
        result.setdefault("concept", concept.strip())
        return result
    ...
    def investigate(
        ...
        return self._bundles.investigate(
            project_root,
            concept,
            limit=limit,
            depth=depth,
    ...
    def _enum_members_for_container(
        self,
        project_root: Path,
        path: str,
        container: str,
    ...
    def _extract_constructor_params(
        ...
        # Python: AST-based extraction — handles @dataclass, pydantic
        # BaseModel, NamedTuple, attrs, and plain __init__ classes.
        # Regex fallback below used the FIRST Name(...) call it found,
        # which returned garbage call-sites instead of defs for Python.
        if path.endswith(".py"):
            py = self._extract_python_constructor(text, type_name)
            if py is not None:
                return py
        ...
        # Non-Python ctor fallback: require the Name(...) match to sit
        # right after a class/struct/record keyword so call-sites don't
        # win over definitions.
        def_ctor = re.search(
            rf"\b(?:class|struct|record)\s+{re.escape(type_name)}\b[^{{(]*\(([^)]*)\)",
            text,
        )
        if def_ctor:
            raw = def_ctor.group(1).strip()
            params = [item.strip() for item in raw.split(",") if item.strip()]
            return {
                "kind": "constructor",
                "params": params,
    ...
    def _extract_python_constructor(
        self,
        text: str,
        type_name: str,
    ) -> dict[str, object] | None:
        """AST-based ctor extraction for Python types.

        Returns None if the class can't be found (caller falls through
        to regex path). Returns {} when class found but no fields —
        still better than a garbage call-site.

        Handles:
          - @dataclass / @dataclass(frozen=True) — fields from AnnAssigns
          - NamedTuple (class-style) — same
          - pydantic BaseModel — same
          - plain classes with __init__ — params from __init__ signature
        """
        import ast
        ...
        cls = self._find_classdef(tree, type_name)
        if cls is None:
            return None
        ...
        if init_fn is not None:
            ...
            for idx, a in enumerate(args.args):
                ...
                params.append(piece)
            # Keyword-only args (after `*` or `*args`). kw_defaults is
            # parallel to kwonlyargs; None = required, Expr = default.
            for a, d in zip(args.kwonlyargs, args.kw_defaults):
                piece = a.arg
                if a.annotation is not None:
                    piece += f": {ast.unparse(a.annotation)}"
                if d is not None:
                    piece += f" = {ast.unparse(d)}"
                params.append(piece)
            return {
                "kind": "constructor",
                "params": params,
        ...
        for child in cls.body:
            if isinstance(child, ast.AnnAssign) and isinstance(child.target, ast.Name):
                piece = child.target.id
                if child.annotation is not None:
                    piece += f": {ast.unparse(child.annotation)}"
                if child.value is not None:
                    piece += f" = {ast.unparse(child.value)}"
                field_params.append(piece)
        kind_label = "record_constructor" if self._is_record_class(cls) else "constructor"
        return {
            "kind": kind_label,
            "params": field_params,
            "signature": f"{type_name}({', '.join(field_params)})",
        }

    @staticmethod
    def _find_classdef(tree, type_name: str):
        """Walk the whole tree — nested classes count too."""
        import ast
    ...
    def _extract_class_properties_ast(
        self,
        project_root: Path,
        candidates: list[dict[str, object]],
    ) -> dict[str, object] | None:
        """AST-walk each candidate file until a ClassDef matches. Enumerate
        AnnAssigns + Assigns inside the class body. Returns None if
        nothing yielded properties — caller renders the "not found" note.
        """
        import ast
        ...
        for item in candidates:
            ...
            try:
                ...
                continue
            cls = self._find_classdef(tree, str(item.get("symbol") or ""))
            if cls is None:
                continue
            ...
            for child in cls.body:
                if isinstance(child, ast.AnnAssign) and isinstance(child.target, ast.Name):
                    ...
                    if child.annotation is not None:
                        entry["field_type"] = ast.unparse(child.annotation)
                    if child.value is not None:
            ...
            if properties:
                return {"properties": properties, "path": rel}
        return None

    @staticmethod
    def _is_record_class(cls) -> bool:
        """True when decorated with @dataclass or base is NamedTuple /
        pydantic BaseModel — anything that treats annotated fields as
        ...
        for base in cls.bases:
            ...
            if isinstance(base, ast.Name):
                ...
                name = base.attr
            if name in ("NamedTuple", "BaseModel"):
                return True
        return False

    def _extract_service_methods_from_declaring_files(
        self,
        project_root: Path,
        service_name: str,
        limit: int = 100,
                ...
                results.append(
                    {
                        ...
                        **(
                            {"namespace": namespace}
                            if (
                                namespace := self._namespace_for_path(
                                    project_root,
                                    rel,
                                    namespace_cache,
                                )
    ...
    def get_dependency_bundle(
        ...
        for edge in self.get_dependencies(project_root, path):
            resolved_paths = self._resolve_edge_to_paths(
                project_root,
                path,
                edge["target"],
                edge["kind"],
            ...
            dependencies.append(
                {
                    ...
                    "resolved_files": [
                        self._get_file_stub(project_root, item) for item in resolved_paths
                    ],
                },
            )
        ...
        if row["language"] == "csharp":
            ...
            for item in outline:
                if item.get("is_partial") and item["symbol"] not in seen:
                    ...
                    # Agent can use ai_get_lines if it needs the actual code.
    ...
    def get_component_bundle(
        ...
        frontend_items = [
            ...
            for item in outline
            if item.get("kind")
            in {"component", "context_provider", "hook", "initializer", "function"}
        ]

        imported_frontend_files: list[dict[str, object]] = []
        seen: set[str] = set()
        for dep in self.get_dependencies(project_root, path):
            for resolved in self._resolve_edge_to_paths(
                project_root,
                path,
                dep["target"],
                dep["kind"],
        ...
        imported_frontend_files.sort(
            key=lambda item: (
                -1
                if item["file"].get("role")
                in {"component", "context-provider", "hook-module", "page", "layout"}
                else 0,
                str(item["file"]["path"]),
            ),
        )
    ...
    def get_service_bundle(
        ...
        service_symbols = [
            item
            for item in outline
            if item.get("kind") in {"class", "record", "struct", "function", "method"}
        ]

        dependencies = self.get_dependency_bundle(
            project_root,
            path=path,
        ...
        local_related.sort(
            key=lambda item: (
                -1
                if item["file"].get("role") in {"service", "controller", "data-model", "policy"}
                else 0,
                str(item["file"]["path"]),
            ),
        )
    ...
    def get_query_bundle(self, project_root: Path, path: str, limit: int = 20) -> dict[str, object]:
        root = self.get_file_bundle(project_root, path)
        hotspot_matches = self.find_query_hotspots(
            project_root,
        ...
        try:
            ...
            schema_entities = [
                item for item in entities if str(item["entity_name"]).lower() in lower_text
            ][:limit]
            schema_fields = [
                item for item in fields if str(item["field_name"]).lower() in lower_text
            ][:limit]
        except Exception:
            pass
    ...
    def get_component_tree(
        ...
        while queue and len(nodes) < limit:
            ...
            if (
                bundle.get("role")
                not in {"component", "context-provider", "hook-module", "page", "layout", "unknown"}
                and current_depth > 0
            ):
                continue
            ...
            nodes.append(
                {
                    ...
                    "symbols": [
                        item["symbol"]
                        for item in bundle.get("outline", [])
                        if item.get("kind")
                        in {"component", "context_provider", "hook", "function", "initializer"}
                    ],
                    "depth": current_depth,
                },
            )
    ...
    def get_context_bundle(
        ...
        indexed_primary_paths = [
            path for path in primary_paths if self._is_indexed_file(project_root, path)
        ]
        ...
        if include_dependencies:
            ...
            for path in indexed_primary_paths:
                dep_bundle = self.get_dependency_bundle(
                    project_root,
                    path=path,
            ...
            dependency_items.sort(
                key=lambda item: (item["path"].count("/"), item["path"], item["kind"]),
            )
    ...
    def get_preset_bundle(
        ...
        if preset == "js-initializer":
            return {
                "preset": preset,
                "value": value,
                "bundle": self.find_initializers(
                    project_root,
                    path=value if value.strip() else None,
                    limit=limit,
                ),
            }
    ...
    @classmethod
    def _prune_dirs(cls, project_root: Path | None = None) -> set[str]:
        # Live-read index.extra_skip_dirs so dashboard changes take
        # effect on the next walk without MCP restart.
        ...
        return cls._PRUNE_DIRS_BASE | extra
    ...
    def _module_hint_dirs(cls, project_root: Path | None = None) -> set[str]:
        ...
        base = cls(None)._modules._MODULE_HINT_DIRS_BASE | extra
        if project_root is not None:
            base = base | module_hints_from_descriptors(project_root)
        return base

    @staticmethod
    def _max_json_size(project_root: Path | None = None) -> int:
        """Max JSON file size to index, in bytes. 0 = unlimited."""
        ...
        try:
            return int(v) if v is not None else 100_000
    ...
    def _walk_source_files(self, project_root: Path, include_tests: bool = False) -> list[Path]:
        ...
        results: list[Path] = []
        # #1039: the source files this walk REFUSED (path, reason) — read by
        # the sync service and persisted so searches can report them.
        self._walk_refused: list[tuple[str, str]] = []
        ...
        for dirpath, dirnames, filenames in os.walk(root_str):
            ...
            dirnames[:] = [
                d
                for d in dirnames
                if d.lower() not in self._prune_dirs(project_root)
                and d.lower() != "index_languages"
                and not d.startswith(".")
                # Build-output variants an exact-match set misses (dist-web,
                # dist-server, …): any `dist-*` directory is generated, never
                # source — its bundled .js is the noise the operator hit.
                and not d.lower().startswith("dist-")
            ...
            if any(
                segment in ("executed", "archived", "old", "bak", "fixtures", "sqlscripts")
            ...
            for fname in filenames:
                ...
                if lower_name in (
                    ...
                    continue
                # Tier-0 sensitive-file blocklist. Anything access_gate
                # would refuse at read time (.env, secrets, credentials,
                # .key/.pem/.pfx, global config DB) never lands in the
                # index. Without this, ai_text_search on a benign
                # query could surface lines from .env files into agent
                # context — and ai_get_lines would happily read back
                # the indexed path later.
                # dirpath is under root_str (os.walk invariant), so rel is just
                # the already-computed rel_dir plus the filename — no per-file
                # pathlib.relative_to. Matches fpath.relative_to(project_root)
                # .as_posix() exactly for files under root; the prior ValueError
                # fallback (path not under root) cannot occur here.
                rel_for_gate = fname if rel_dir == "." else f"{rel_dir}/{fname}"
                # ONE ADMISSION AUTHORITY (#1039). Name gate and content gate
                # both live in `may_enter_index`, which the scoped/incremental
                # fast path calls too — so this walk and that one can no longer
                # give different answers about the same file.
                if not may_enter_index(rel_for_gate, fpath):
                    # #1039: REMEMBER what was refused. A refused SOURCE file
                    # is a file a search will not see; the search must be able
                    # to say so. Non-source names (.env, .pem, secrets.json)
                    # are not "unsearched code" and are not recorded.
                    if lower_name.endswith(_INDEX_SOURCE_SUFFIXES):
                        self._walk_refused.append(
                            (
                                rel_for_gate,
                                "name_blocked"
                                if _is_sensitive_for_index(rel_for_gate)
                                else "content_refused",
                            ),
                        )
                    continue
                # Skip large JSON files (> index.max_json_size; 0 = unlimited)
                if lower_name.endswith(".json"):
                    try:
                        _mjs = self._max_json_size(project_root)
                        if _mjs > 0 and fpath.stat().st_size > _mjs:
                            continue
                    except OSError:
                        continue
    ...
    def _should_skip(self, project_root: Path, path: Path, include_tests: bool = False) -> bool:
        ...
        if any(
            ...
            return True
        # Build-output variants a plain segment match misses (dist-web,
        # dist-server, …): any `dist-*` directory is generated, never source.
        if any(segment.startswith("dist-") for segment in parts):
            return True
        ...
        if lower_name in (
            ...
            return True
        # Skip large JSON data files (> index.max_json_size; 0 = unlimited)
        _mjs = self._max_json_size(project_root)
        if lower_name.endswith(".json") and _mjs > 0 and path.stat().st_size > _mjs:
            return True
        if not include_tests:
    ...
    def _path_looks_like_test(path: str) -> bool:
        ...
        return any(
            part in {"tests", "test", "e2e", "__tests__", "__test__"} or part.endswith(".test")
            for part in parts
        )
    ...
    def _extract_references(
        ...
        """(token, line_number, kind, raw, enclosing) — see the outline service."""
        return self._outlines._extract_references(project_root, text, code_language)
    ...
    def find_broken_references(
        ...
        *,
        ...
        """Ref-integrity (Empire goal 2026-06-20): references whose token has no
        resolving definition. HEURISTIC (regex-extracted refs), truthfully
        labeled. Resolution per descriptor definition_source:
          {"symbol_kind": "<kind>"}                        vs code_outlines.symbol
          {"glob": "...", "pattern": "...", "capture": N}  vs an extracted token set
        Bounded by `limit` samples per kind.
        """
        from .language_descriptors import load_language_descriptors
        ...
        for desc in descriptors.values():
            ds = getattr(desc, "definition_source", None)
            if isinstance(ds, dict):
                for ref_kind, spec in ds.items():
                    if isinstance(spec, dict) and str(ref_kind) not in specs:
                        specs[str(ref_kind)] = spec
        report_kinds: list[dict[str, object]] = []
        total_broken = 0
        with self.connect(project_root) as conn:
            present = [
                str(r["kind"])
                for r in conn.execute(
                    "SELECT DISTINCT kind FROM code_references ORDER BY kind",
                )
            ]
            for kind in present:
                ref_rows = conn.execute(
                    "SELECT path, line_number, token FROM code_references WHERE kind = ?",
                    (kind,),
                ).fetchall()
                    ...
                    report_kinds.append(
                        {
                            "kind": kind,
                            "resolvable": False,
                            "reason": "no definition_source declared for this kind",
                            "reference_count": ref_count,
                        },
                    )
                    continue
                def_set = self._resolve_definition_set(project_root, conn, spec)
                broken = [
                    {"path": r["path"], "line": int(r["line_number"]), "token": r["token"]}
                    for r in ref_rows
                    if str(r["token"]) not in def_set
                ]
                ...
                report_kinds.append(
                    {
                        ...
                        "resolvable": True,
                        "reference_count": ref_count,
                        "definition_count": len(def_set),
                        "broken_count": len(broken),
                        "broken_sample": broken[:limit],
                        "truncated": len(broken) > limit,
                    },
                )
        return {
            "evidence": {
                ...
                "limitations": (
                    "regex heuristic, not AST; only kinds with a declared "
                    "definition_source are resolvable"
                ),
            },
    ...
    def _resolve_definition_set(
        ...
        symbol_kind = str(spec.get("symbol_kind") or "").strip()
        if symbol_kind:
            return {
                str(r["symbol"]).strip()
                for r in conn.execute(
                    "SELECT DISTINCT symbol FROM code_outlines WHERE kind = ?",
                    (symbol_kind,),
                )
            }
        glob = str(spec.get("glob") or "").strip()
        pattern = str(spec.get("pattern") or "").strip()
        if not glob or not pattern:
            return set()
        try:
            capture = int(spec.get("capture", 1))
        except (TypeError, ValueError):
            capture = 1
        try:
        ...
        tokens: set[str] = set()
        scanned = 0
        for file in sorted(project_root.glob(glob)):
            if scanned >= 2000:
                break
            if not file.is_file():
                continue
            ...
            for raw_line in content.splitlines():
                ...
                for match in regex.finditer(line):
                    try:
                        tok = match.group(capture) if capture else match.group(0)
    ...
    def _extract_component_semantics(
        ...
        self._outlines._extract_component_semantics(
            project_root,
            code_language,
            text,
            ast_outline,
    ...
    def _resolve_edge_to_paths(
        ...
        return self._edges._resolve_edge_to_paths(
            project_root,
            source_path,
            target,
            kind,
    ...
    def _existing_relative_candidates(
        ...
        return self._edges._existing_relative_candidates(
            project_root,
            base,
    ...
    def _infer_layer_from_path(self, path: str) -> str:
        ...
        if any(token in lower for token in ("service", "handler", "manager", "provider")):
            return "logic"
        if any(
            token in lower for token in ("component", "page", "view", "form", "dialog", "screen")
        ):
            return "ui"
        return "code"
    ...
    def _infer_code_role(
        ...
        if code_language in {"javascript", "typescript"}:
            ...
            if "core" in parts:
                return "core-module"
            if lower.endswith((".config.js", ".config.ts", ".config.mjs", ".config.cjs")) or any(
                token in parts for token in ("schemas", "schema")
            ):
                return "config-module"
            if lower.endswith((".d.ts", "-env.d.ts")) or name in {
                "next-env",
                "sst-env",
                "env",
                "sidebars",
        ...
        if code_language == "csharp":
            ...
            if name.endswith(("configuration", "config")) or any(
            ...
            if logical_name.endswith(("resources", "map", "ids")):
                return "utility"
            if name.endswith(("helper", "utilities", "utility", "extensions")) or any(
                token in parts for token in ("helpers", "utils", "extensions")
            ):
                return "utility"
            if name.endswith(("job", "worker", "task")) or any(
                token in parts for token in ("workers", "jobs", "background")
            ):
                return "worker"
    ...
    def _ensure_column(self, conn: sqlite3.Connection, table: str, column: str, ddl: str) -> None:
        ...
        if column not in existing:
            try:
                ...
                # The PRAGMA check alone is not enough: other agents read and
                # migrate this SAME index concurrently, so two processes can both
                # see the column missing and both issue the ADD. The loser used to
                # surface "duplicate column name" out of init_db and fail an
                # unrelated caller's read. Losing the race is success — the column
                # exists either way. Any OTHER OperationalError still raises.
