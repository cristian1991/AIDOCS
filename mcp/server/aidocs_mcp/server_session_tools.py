...
def register_session_tools(
    *,
    ...
    # session_read MCP tool REMOVED 2026-05-02 (king directive — paved-road
    ...
    # session_start RESTORED as a thin compat alias 2026-05-03 (king
    ...
    # auto-activates managed mode if it isn't already (matches king's
    ...
    def session_start(
        ...
        the most recent active session — matches the king's rule that
    ...
    def aidocs_mode_clear() -> dict[str, Any]:
        ...
        return hub.managed_mode.clear_mode(resolve_project_root())
    ...
    # Internal helper. Tool surface removed 2026-05-12 — ai_session(mode='claim_status').
    def session_claim_status(session_id: str, stale_after_minutes: int = 30) -> dict[str, Any]:
    ...
    # Internal helper. Tool surface removed 2026-05-12 — ai_session(mode='claim').
    def session_claim(
        session_id: str,
        agent_id: str,
        run_id: str,
    ...
    # Internal helper. Tool surface removed 2026-05-12 — ai_session(mode='release').
    def session_release(
        session_id: str,
        agent_id: str,
    ...
    def ai_lane_send(
        ...
        """Conductor-only: write a prompt to a parked lane worker's mailbox.

        When the worker next wakes (from its ScheduleWakeup poll), the
        claude_hook swaps its wake-prompt with this message so the agent
        acts on it immediately. Enables long-running lanes that alternate
        work ↔ park-for-conductor-instruction without respawning.
        ...
        Args:
            ...
            prompt: The instruction to inject. Becomes the next thing
                the worker agent "hears" on wake.
    ...
    def ai_lane_inbox(
        ...
        Read-only diagnostic surface. For dashboards and triage:
        'what did the conductor send this worker and when was it read?'
        """
        from .lane_mailbox_store import LaneMailboxStore

        project_root = resolve_project_root()
        rows = LaneMailboxStore().list_for_worker(
            project_root,
            worker_id=worker_id,
            limit=int(limit),
        )
        return {"worker_id": worker_id, "messages": rows, "count": len(rows)}
