...
def _detect_host_kind() -> str:
    ...
    Preserves the old return contract exactly ("unknown" when nothing is known).
    New code must call ``agent_memory_epoch.resolve_host_identity`` and handle
    the honest "".
    """
...
def _resolve_epoch(project_root: Path) -> str:
    ...
    (#525/#539). This entrypoint takes no identity arguments, which is itself the
    #539 defect on this surface: it can only ever use the ambient stamp, so a
    caller that HAS the host payload cannot hand it over. Threading host_kind /
    host_session_id through ``maybe_dnt_banner_for_read`` is the remaining work.
    """
    from .agent_memory_epoch import resolve_epoch

    return resolve_epoch(project_root)
    ...
    *,
    ...
    try:
        ...
        if rec is None or not rec.symbol:
            return ""
        # ONE resolution path for both branches (#525/#539): the explicit
        # identity is simply passed through to the same authority that resolves
        # the ambient one, so this surface can no longer key a different epoch
