...
_LAW_THRONE_LINE = (
    "empire-doctrine XII: law enters only through the throne; aidocs-doctrine "
    "IX: operator intent is present-tense, operator-authored and per-turn."
)


def lawbook_seal_confirm_phrase(skill_id: str, sha256: str) -> str:
    """The exact phase-2 phrase. Binds the id AND the scroll digest previewed."""
    return f"SEAL {skill_id} {sha256}"
...
def seal_lawbook(
    ...
    *,
    ...
    """THE lawbook seal door — ``ai_skill(mode='seal')`` (plan lawbook-seal-door).

    Every gate fails closed, in order:
      a. ``skill_id`` is a lawbook id (``load_lawbook_scroll``);
      b. a present-tense OPERATOR grant for (session, this id, OP_SEAL) minted
         this turn by the UserPromptSubmit pipeline (``empire_soul_gate``;
         origin-gated there, so worker/delegated/replay prompts mint nothing).
         Phase 1 PEEKS it; phase 2 CONSUMES it (single-use);
      c. the caller is not a spawned agent (``_xaacp_spawned_agent_refusal``);
      d. two-phase: phase 1 (no ``confirm_token``) previews and writes NOTHING;
         phase 2 must carry the exact phrase AND its digest must still equal
         the scroll on disk.
    The body written is the scroll's bytes; it is scanned as an ordinary skill
    (the scanner exempts kind='doctrine'). Every outcome is audited.
    """
    from . import conductor_comms as _cc
    from . import empire_soul_gate as _gate
    ...
    phase = "confirm" if token else "preview"
    seal: dict[str, str] = {"sha256": "", "source_path": "", "grant_id": "", "phase": phase}
    session_id = ""

    def _audit(outcome: str, reason: str = "", scan_status: str = "") -> None:
        record_skill_write_act(
            record_event,
            project_root,
    ...
    def _refuse(reason: str, detail: str = "", scan_status: str = "") -> dict[str, Any]:
        _audit("refused", reason, scan_status)
        return {
            "ok": False,
            "reason": reason,
            "skill_id": sid,
            "message": f"lawbook seal refused ({reason}){': ' + detail if detail else ''}. {_LAW_THRONE_LINE}",
        }

    # a. address
    scroll = load_lawbook_scroll(sid, project_root)
    if not scroll.get("ok") and scroll.get("reason") == "not_a_lawbook_id":
        return _refuse("not_a_lawbook_id")
    # b. the operator's present-tense grant, this turn, this id
    session_id, why = _gate.resolve_soul_session(project_root)
    if not session_id:
        return _refuse("session_unresolved", str(why or ""))
    grant = _gate.consume_grant_detail(
        project_root, session_id, sid, _gate.OP_SEAL, consume=False,
    )
    if not grant.get("ok"):
        return _refuse("no_operator_grant", str(grant.get("reason") or ""))
    seal["grant_id"] = str(grant.get("grant_id") or "")
    # c. never a spawned agent
    try:
        route = _cc.xaacp_resolve_caller_route(project_root)
    except Exception:
        return _refuse("caller_unresolved")
    if _cc._xaacp_spawned_agent_refusal(route, role="", verb="seal law for") or (
        str(_rt.current_calling_agent_id() or "").strip()
    ):
        return _refuse("spawned_agent", "a spawned agent never seals law")
    # scroll checks
    if not scroll.get("ok"):
        seal["source_path"] = str(scroll.get("source_path") or "")
        return _refuse(str(scroll.get("reason")), str(scroll.get("detail") or ""))
    seal["sha256"] = str(scroll["sha256"])
    seal["source_path"] = str(scroll["source_path"])
    body = str(scroll["body"])
    scan_refusal, scan_meta = scan_public_skill_write(sid, body, "skill")
    scan_status = str(scan_meta.get("scan_status") or "")
    if scan_refusal:
        return _refuse("skill_scan_refused", scan_refusal, scan_status)
    phrase = lawbook_seal_confirm_phrase(sid, seal["sha256"])
    # d. two-phase
    if not token:
        _audit("previewed", "", scan_status)
        return {
            "ok": True,
            "phase": "preview",
            "skill_id": sid,
            "sha256": seal["sha256"],
            "articles": scroll["articles"],
            "source_path": seal["source_path"],
            "confirm_phrase": phrase,
            "message": "Nothing written. Re-call with confirm_token set to confirm_phrase.",
        }
    parts = token.split(" ")
    if len(parts) != 3 or parts[0] != "SEAL" or parts[1] != sid:
        return _refuse("confirm_phrase_mismatch")
    if parts[2] != seal["sha256"]:
        return _refuse("scroll_changed_since_preview", "re-run the preview")
    consumed = _gate.consume_grant_detail(project_root, session_id, sid, _gate.OP_SEAL)
    if not consumed.get("ok"):
        return _refuse("no_operator_grant", str(consumed.get("reason") or ""))
    meta = scroll.get("meta") or {}
    try:
        skills.empire_skill_upsert(
            skill_id=sid,
            name=sid,
            content_text=body,
        ...
        return _refuse("store_write_failed", type(exc).__name__, scan_status)
    _audit("ok", "", scan_status)
    return {
        "ok": True,
        "phase": "sealed",
        "skill_id": sid,
        "sha256": seal["sha256"],
        "articles": scroll["articles"],
        "message": f"sealed {sid} sha256={seal['sha256'][:12]} {scroll['articles']} articles",
    }
...
def register_skill_tools(
    *,
    ...
    def ai_skill(
        ...
        transition; sovereign access lives in ai_soul per aidocs-doctrine §XI.
        """
        ...
        if m == "seal":
            # The ONLY lawbook write door. Upsert/delete below keep refusing
            # lawbook ids unconditionally (#615/#676) — this is a different
            # admission surface gated on the operator's own present-tense word.
            _auth_refusal = _refuse_unauthenticated_write("seal")
            if _auth_refusal:
            ...
            return seal_lawbook(
                ...
                record_event=getattr(getattr(hub, "execution", None), "record_event", None),
            )
        if m == "upsert":
            ...
            # attacker-selected. Empire-doctrine XII — law enters only through the
        ...
        return {
            "ok": False,
            "reason": "invalid_mode",
            "message": f"mode must be one of read|upsert|delete|list|seal, got {mode!r}",
        }
    ...
    # detector lands per §IX, this tool will additionally check
