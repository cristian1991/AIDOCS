...
class PolicyService:
    ...
    def preflight_action(
        ...
        inspection_allowed = (
            action_kind in {"inspect", "read_file", "read_error"} and len(explicit_targets) > 0
        )
        ...
        return {
            ...
            "reason": reason,
            "active_sessions": [self._summary(item) for item in active_sessions],
            "next_step": "run ai_session(mode='list') to choose, then ai_session(mode='connect'), or issue STOP",
        }
