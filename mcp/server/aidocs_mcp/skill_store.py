...
# EVERY seeding open (writes + lifecycle), so an operator-customized row would
# otherwise log the same skip on every one. Keyed (db path, skill_id) — tests
# with distinct tmp DBs never collide.
...
# Which lawbook scroll is PRIVATE (repo doctrine/) — every other lawbook id is
# bundled package data. The same split `_ensure_bundled_seed` reads.
_PRIVATE_LAWBOOK_ID = "aidocs-doctrine"


def lawbook_scroll_path(skill_id: str, project_root: Path | str | None = None) -> Path | None:
    """Canonical on-disk scroll for a lawbook id, from the seeder's own dirs.

    None when the id is not a lawbook id or its dir is unresolvable. Used by
    the operator seal door (``ai_skill(mode='seal')``); never takes a caller
    path.

    THE PRIVATE SCROLL FROM AN INSTALLED RUNTIME. ``_private_doctrine_dir``
    resolves only from a source checkout, so a server running from a runtime
    generation could never find ``aidocs-doctrine`` and the seal door refused
    ``scroll_absent`` forever. The fallback is the BOUND project's own
    ``doctrine/`` dir -- server-resolved, never caller text -- and the
    frontmatter-name check in ``load_lawbook_scroll`` still binds the file to
    the id.
    """
    sid = str(skill_id or "").strip()
    if sid not in LAWBOOK_SKILL_IDS:
        return None
    if sid == _PRIVATE_LAWBOOK_ID:
        d = SkillStore._private_doctrine_dir()
        if d is None and project_root:
            cand = Path(project_root) / "doctrine"
            d = cand if (cand / f"{sid}.md").is_file() else None
    else:
        d = SkillStore._bundled_data_dir()
    return (Path(d) / f"{sid}.md") if d is not None else None


_LAW_ARTICLE_RE = re.compile(r"^## [IVXLC]+\. ", re.MULTILINE)


def load_lawbook_scroll(skill_id: str, project_root: Path | str | None = None) -> dict[str, object]:
    """Read a lawbook scroll for sealing. ``{ok, reason, detail, ...}``.

    On ok: ``body`` (the file's bytes, strict UTF-8), ``sha256``,
    ``source_path``, ``articles``, ``meta``. Refuses a non-lawbook id, an
    absent/unreadable file, non-UTF-8 bytes and a frontmatter ``name`` that is
    not the id. Never takes caller text or a caller path.
    """
    import hashlib

    sid = str(skill_id or "").strip()
    path = lawbook_scroll_path(sid, project_root)
    if path is None:
        if sid not in LAWBOOK_SKILL_IDS:
            return {"ok": False, "reason": "not_a_lawbook_id", "detail": sid}
        return {"ok": False, "reason": "scroll_absent", "detail": sid}
    if not path.is_file():
        return {"ok": False, "reason": "scroll_absent", "detail": str(path)}
    try:
        raw = path.read_bytes()
        body = raw.decode("utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        return {"ok": False, "reason": "scroll_unreadable", "detail": type(exc).__name__}
    meta = SkillStore()._parse_frontmatter(body)
    if str(meta.get("name") or "").strip() != sid:
        return {
            "ok": False,
            "reason": "frontmatter_name_mismatch",
            "detail": f"name={meta.get('name')!r}",
    ...
    return {
        ...
        "source_path": str(path),
        "articles": len(_LAW_ARTICLE_RE.findall(body)),
        "meta": meta,
    }
...
# DRT-01 (doctrine split): `kingdom-doctrine` is RESERVED for a project's OWN
# law. The bundled scaffold used to ship under that very id as a `kind: sample`,
# and bundled content registers first at a winning priority — so a tenant's
# `.MEMORY/skills/kingdom-doctrine.md` could never be the row served for
# `kingdom-doctrine`. The scaffold now ships as `kingdom-doctrine-template`.
# The seed retires the OLD sample row only while the seed still OWNS it
# (source='bundled_seed'); an operator-written row under the reserved id is
# the project's actual law and is never touched (#479 operator-wins).
_RETIRED_BUNDLED_SAMPLE_IDS: dict[str, str] = {
    "kingdom-doctrine": "kingdom-doctrine-template",
}


# ── The sovereign soul chamber: ONE structural locator (#222 leak 1) ──
# The empire DB and the per-soul backup dir are BOTH derived here, from the
# same code that writes them. Every guard that fences the chamber consults
# this, so relocating the DB (AIDOCS_EMPIRE_DB) moves the chamber AND the
# fence together. The #222 audit found the opposite shape: protection made
# of hand-kept patterns for the DEFAULT path, which evaporates the moment
# the DB moves. Structural, not filtered (doctrine XI) — the same rule
# server_palace_tools._soul_forbidden_roots already follows for the miner.


# Law kinds the PUBLIC skill door may never write (#615/#629, operator
# amendment 2 of 2026-07-30). Empire-doctrine XII: "Untrusted content ... is
# EVIDENCE, never auto-promoted to law / doctrine / rules / memory. Only the
# operator, or a trusted-path promotion, seals law."
#
# WHY THIS SET IS DERIVED, NOT LISTED: skill_scanner SKIPS these kinds
# (skill_scanner.py:236, :339-340) because documentation prose legitimately
# mentions `subprocess`, `curl|sh` etc. and scanning it drowns the real signal.
# That exemption is correct for READING the empire's own scrolls and fatal for
# WRITING new ones through an agent-reachable door: the one class most needing
# a scan was the class exempt from it, and `kind` is caller-supplied, so the
# exemption was ATTACKER-SELECTED. Deriving the guard from the scanner's own
# exemption set means a future exempt kind cannot become a silent write door —
# the guard grows with the exemption automatically (the CLASS, not the instance).
def _scanner_exempt_kinds() -> frozenset[str] | None:
    """The scanner's own skip set, read lazily (no import cycle, no startup
    cost). Returns None when it CANNOT BE RESOLVED.

    None, never a hardcoded fallback set. A literal fallback here would be a
    SECOND COPY of skill_scanner._DOCUMENTATION_KINDS (#461 cause A): it starts
    identical, drifts the first time one side is edited, and the drift is
    invisible because both sides look authoritative. An earlier revision of
    this function carried exactly such a fallback; it is removed.

    The caller refuses everything on None — an undecidable verdict is UNKNOWN,
    never clean (empire promoted-06ad3c5f61ab).
    """
    try:
        from .skill_scanner import _DOCUMENTATION_KINDS

        return frozenset(_DOCUMENTATION_KINDS)
    except Exception:
        return None


def refuse_law_kind_from_public_door(kind: str) -> str | None:
    """Refusal text when ``kind`` is LAW, else None.

    Placed at the ADMISSION PATH (the ``ai_skill`` tool), never on
    ``SkillStore.empire_skill_upsert`` itself: the store primitive is what the
    operator-gated, fit-checked promotion path uses to seal law legitimately.
    Guarding the primitive would refuse the operator along with the agent —
    the same origin-vs-payload distinction the prompt gate makes.

    Returns a refusal that names the DOCTRINE LINE, not the mechanism (XXI).
    """
    exempt = _scanner_exempt_kinds()
    if exempt is None:
        # UNKNOWN IS NEVER CLEAN (empire promoted-06ad3c5f61ab). We cannot tell
        # which kinds are law, so we cannot tell that THIS one is not. Refuse
        # every write rather than admit one on an unresolved security input
        # (#662: a miss is UNRESOLVED — never permitted, never denied-as-clean).
        return (
            "refusing the write: the scanner's exemption set is UNRESOLVED, so "
            "the door cannot determine whether this kind is law. This is not a "
            "verdict about your content — it is a refusal to guess. Restore "
            "skill_scanner and retry; reads are unaffected."
        )
    normalized = str(kind or "").strip().lower()
    if normalized and normalized in exempt:
        return (
            f"refusing to write kind={normalized!r} through the public skill "
            "door: this is LAW, not a skill. empire-doctrine XII — untrusted "
            "content is EVIDENCE, never auto-promoted to law/doctrine; only "
            "the operator or a trusted-path promotion seals law. Route it "
            "through the operator-gated promotion path "
            "(ai_memory(mode='promote')), which is fit-checked and audited. "
            "Reads are unaffected: ai_skill(mode='read') still serves this "
            "scroll in full."
        )
    return None


def _lawbook_id_set() -> set[str]:
    """Every id that IS the lawbook, lowercased — current and legacy.

    ONE derivation, read by both the write-side refusal below and the #677
    read-side resolution order. The set was previously rebuilt inline in the
    refusal; a second copy elsewhere is exactly the #461-cause-A drift that
    lets a newly-named lawbook row be guarded on write and shadowable on read.
    """
    return {sid.lower() for sid in LAWBOOK_SKILL_IDS} | {
        sid.lower() for sid in _LEGACY_LAWBOOK_IDS
    }


def refuse_lawbook_write_from_public_door(skill_id: str) -> str | None:
    """Refusal text when ``skill_id`` IS the lawbook, else None.

    The kind guard above closes law injection by PAYLOAD. This closes it by
    ADDRESS, which the kind guard cannot see: ``empire_skill_upsert`` takes the
    UPDATE branch on a conflicting skill_id and ``ON CONFLICT(skill_id) DO
    UPDATE SET ... content_text = excluded.content_text``, so an upsert
    carrying a perfectly ordinary ``kind='skill'`` REWRITES THE BODY of
    `aidocs-doctrine` — the project's own law — and the kind guard never fires.
    Delete is refused for the same reason twice over: deleting the row destroys
    law, and delete-then-insert would otherwise walk around this very guard.

    DERIVED from LAWBOOK_SKILL_IDS + _LEGACY_LAWBOOK_IDS, never restated (#461
    cause A): a future lawbook row is guarded the moment it is named there, not
    the next time someone remembers this function exists.

    Placement is the ADMISSION PATH (``ai_skill``), never the store primitive —
    the seed and the operator-gated promotion path write the lawbook
    legitimately through ``empire_skill_upsert``, and guarding the primitive
    would refuse the operator along with the agent.

    Reads are untouched: ``ai_skill(mode='read')`` still serves the full scroll.
    """
    normalized = str(skill_id or "").strip().lower()
    if not normalized:
        return None
    if normalized not in _lawbook_id_set():
        return None
    return (
        f"refusing to write skill_id={normalized!r} through the public skill "
        "door: this row IS THE LAWBOOK, not a skill. empire-doctrine XII — "
        "untrusted content is EVIDENCE, never auto-promoted to law/doctrine; "
        "only the operator or a trusted-path promotion seals law. Amend the "
        "scroll at its canonical source and let the seed/promotion path carry "
        "it. Reads are unaffected: ai_skill(mode='read') still serves this "
        "scroll in full, and authoring an ordinary skill under any other "
        "skill_id still works."
    )

# Verdicts that REFUSE a write through the public skill door (#615 target 2).
# THRESHOLD, MEASURED not guessed (2026-08-01): every skill in the project's own
# 27-row catalog, forced through the ordinary-skill scan, lands at 26 safe /
# 1 medium / ZERO high-or-critical. So refusing high+critical refuses nothing
# AIDOCS itself ships — governed authorship, not forbidden authorship. Lower
# verdicts ride along on the receipt and in the audit row so an author (and the
# ledger) sees them without the door becoming unusable.
_SCAN_REFUSAL_LEVELS: frozenset[str] = frozenset({"high", "critical"})

# The verdict string for "the scanner could not reach a verdict". NOT "safe".
SCAN_STATUS_UNKNOWN = "unknown"


def scan_public_skill_write(
    skill_id: str,
    content_text: str,
    kind: str,
) -> tuple[str | None, dict[str, object]]:
    """Scan a skill body at the WRITE door. Returns ``(refusal_or_None, meta)``.

    #615 target 2 — THE CHECKER NEVER RAN ON WRITE. ``scan_skill`` was reached
    only from read/dashboard paths, so a skill carrying an instruction-override
    payload was stored verbatim and served to every future agent that loaded it.
    A skill is instruction text later agents follow AS IF IT WERE LAW; an
    unscanned skill is an unreviewed instruction to every future agent.

    A load-time check is not a substitute: load-time catches a poisoned row
    ALREADY in the store, write-time prevents it landing. This is the write half.

    UNKNOWN IS NOT A PASS. If the scanner cannot be imported or raises, this
    returns a REFUSAL, never a clean verdict — the same rule
    ``_scanner_exempt_kinds`` already follows for the exemption set. Refusing a
    write on an unresolved security input costs an author one retry; admitting
    it costs every future agent that loads the row.

    The caller-supplied ``kind`` reaches the scanner, but the door has already
    refused every scanner-exempt kind (``refuse_law_kind_from_public_door``), so
    the caller can no longer select its own exemption — that composition is why
    the exemption is not a bypass here.
    """
    body = str(content_text or "")
    try:
        from .skill_scanner import scan_skill

        result = scan_skill(str(skill_id or ""), body, kind=str(kind or ""))
        risk = str(result.risk_level or "")
        findings = [
            # Category/severity/description only — never `evidence`, which
            # quotes the body back and would carry content into a receipt.
            {"category": f.category, "severity": f.severity, "description": f.description}
            for f in result.findings[:10]
        ]
    except Exception as exc:
        meta: dict[str, object] = {
            "scan_status": SCAN_STATUS_UNKNOWN,
            "scan_error": f"{type(exc).__name__}: {exc}",
        }
        return (
            (
                "refusing the write: the skill scanner could not reach a verdict "
                f"({type(exc).__name__}), so this content is UNRESOLVED — not clean. "
                "A skill is instruction text later agents follow; an unchecked one "
                "is an unreviewed instruction. Restore skill_scanner and retry. "
                "Reads are unaffected."
            ),
            meta,
        )
    meta = {"scan_status": risk, "scan_findings": findings}
    if risk in _SCAN_REFUSAL_LEVELS:
        summary = "; ".join(
            f"{f['severity']}:{f['description']}" for f in findings
        ) or risk
        return (
            (
                f"refusing the write: the skill scanner rated this content {risk!r} — "
                f"{summary}. A skill is instruction text later agents follow AS IF IT "
                "WERE LAW, so a high-risk body is an unreviewed instruction to every "
                "future agent. Rewrite the skill, or route it through the "
                "operator-gated promotion path. Reads are unaffected."
            ),
            meta,
        )
    return None, meta
...
def record_skill_write_act(
    ...
    *,
    ...
    scan_status: str = "",
    reason: str = "",
    source_kind: str = "ai_skill",
    capability_name: str = "ai_skill",
    seal: dict[str, str] | None = None,
) -> bool:
    """Emit ONE act-audit event for a skill write — #615 target 4.

    ``source_kind`` / ``capability_name`` default to the ``ai_skill`` MCP door
    and are overridden by the OTHER admission surfaces (#676: the dashboard /
    desktop CLI doors) so the ledger records WHICH DOOR a skill act came
    through. One audit shape for every door; only the door label varies.

    Modeled on ``empire_soul_gate.record_soul_act`` deliberately rather than
    inventing a new shape: that is the in-project precedent for what "audited"
    means, and it already answers who / when / what / under what authority
    through the canonical execution audit path (``record_event`` stamps
    identity and time at insert). Before this, the door that WRITES LAW-SHAPED
    INSTRUCTION TEXT was less audited than the door that READS A SOUL.

    EVERY outcome is recorded — ok AND refused. The refusals are the
    interesting rows: an attempt to rewrite the lawbook must be the loudest
    thing in the ledger, not the quietest.

    PAYLOAD IS A FIXED IDENTIFIER WHITELIST. The body has no parameter here by
    construction — no content, no snippet, no digest of it. The act is
    auditable; the text is not (same floor as the soul ledger).

    ``seal`` is the ONE exception, used only by the lawbook seal door
    (`ai_skill(mode='seal')`): it carries the sha256 of the sealed scroll, its
    repo path, the operator grant id that authorised it and the phase
    (preview/confirm). The sealed body is the canonical scroll already
    committed at that path, so its digest identifies a law version rather than
    leaking private text; a grant id is an opaque audit handle, not a secret.
    Only the four named keys are ever copied.

    Best-effort: an audit failure never blocks or unblocks the write itself.
    """
    ...
    payload: dict[str, str] = {
        ...
        "kind": str(kind or ""),
        "scan_status": str(scan_status or ""),
        "outcome": str(outcome or ""),
        "reason": str(reason or ""),
    }
    for _key in ("sha256", "source_path", "grant_id", "phase"):
        if seal and _key in seal:
            payload[_key] = str(seal.get(_key) or "")
    try:
        record_event(
            Path(project_root),
            event_kind="skill_write",
            source_kind=str(source_kind or "ai_skill"),
            session_id=str(session_id or "") or None,
            capability_name=str(capability_name or "ai_skill"),
            action_kind=str(operation or ""),
            target_entity=str(skill_id or ""),
...
def empire_db_path() -> Path:
    """Resolved location of the empire sqlite ledger, honoring the
    ``AIDOCS_EMPIRE_DB`` override. Sole source of truth for WHERE the
    sovereign rows live."""
    import os
...
@lru_cache(maxsize=1)
def soul_chamber_paths() -> tuple[Path, ...]:
    """Every on-disk location that holds sovereign soul material in the
    clear: the empire DB itself and the per-soul backup directory.

    Returned RESOLVED so a symlinked or case-variant path cannot walk
    around a caller's containment check. Best-effort: an unresolvable
    path yields its unresolved form rather than an empty fence.

    Cached because the read gate consults it on every decision; the
    override is read once per process. Tests that relocate the DB call
    ``soul_chamber_paths.cache_clear()``.
    """
    db = empire_db_path()
    out: list[Path] = []
    for candidate in (db, db.parent / "soul-backups"):
        # BOTH forms. A caller may hand the gate either the literal path or
        # a resolved one; matching only the resolved form would let the
        # literal walk past the fence (and vice versa on a symlinked home).
        forms = [candidate]
        try:
            forms.append(candidate.resolve())
        except OSError:
            pass
        for form in forms:
            if form not in out:
...
class SkillStore:
        ...
        if existing is None or self._provider_priority(
            ...
            if content and len(content) > 10:
                try:
                    ...
                    # UNKNOWN IS NOT A PASS (#615). This was `except Exception:
                    # pass`, which stored the payload with NO scan_status —
                    # indistinguishable from a row that had been checked and
                    # come back clean. That is the fail-GREEN shape: "I could
                    # not check" silently became "it passed".
                    #
                    # The load path still STORES the row: refusing here would
                    # empty every agent's skill catalog on a scanner hiccup, and
                    # the write door (scan_public_skill_write) is where a bad
                    # body is actually stopped. But the failure is now VISIBLE
                    # on the row itself rather than swallowed — fail-quiet, not
                    # fail-green. Consumers that treat `scan_status` as a
                    # verdict must read "unknown" as unresolved, never as safe.
                    payload["scan_status"] = SCAN_STATUS_UNKNOWN
                    payload["scan_error"] = f"{type(exc).__name__}: {exc}"
                    _log.warning(
                        "skill scan could not reach a verdict for %s (%s) — "
                        "row tagged scan_status=unknown, NOT safe",
                        skill_id,
                        exc,
                    )
    ...
    def _empire_db(self) -> Path:
        ...
        Per empire-doctrine §XII + aidocs-doctrine §XI: empire content lives in SQL, not
        ...
        Delegates to the module-level ``empire_db_path`` so the WRITERS
        here and the soul-chamber GUARDS elsewhere can never disagree
        about where the sovereign rows live (#222).
        """
    ...
    _MODE_GATED_ROLE_SKILLS: frozenset[str] = frozenset(
        {
            "head-conductor",
            # #501: the co-conductor's doctrine ships as the bundled payload
            # 'co-conductor-doctrine' — THAT is the id the seat delivers, so
            # it must be mode-gated like the other seats. ('co-conductor' is
            # kept as the legacy operator-inscribed alias.)
    ...
    def read_role(self, skill_id: str) -> dict[str, object] | None:
        ...
        try:
            conn = self._empire_read_conn()  # pure read (#1095): no seed/restore
            if conn is None:
                return None
    ...
    def search_public_scrolls(
        ...
        """
        from aidocs_mcp._sqlite_connect import connect as _canonical_connect
        ...
        try:
            # #755: canonical connect — WAL, synchronous, busy_timeout and
            # foreign_keys=ON, none of which a raw sqlite3.connect() applies.
            # row_factory stays ROW (the helper's default) because this search
            # reads columns by name; the hand-set line it replaces is now
            # redundant.
    ...
    def _empire_conn(self):
        """The SEEDING opener -- writers and the bootstrap only (#1095).

        Opens the empire DB with the empire_skills schema ensured + a Row
        factory, then runs the bundled seed, the soul restore and the library
        restore. This is the ONE authority for seed/restore; it is reached by
        the write doors (upsert/delete/soul quills) and by ensure_empire_seed
        (the SessionStart lifecycle rail). READS never come here -- they use
        _empire_read_conn, which creates, seeds and restores nothing.
        """
        from aidocs_mcp._sqlite_connect import connect as _canonical_connect

        db = self._empire_db()
        db.parent.mkdir(parents=True, exist_ok=True)
        # #755: canonical connect (pragmas). row_factory stays ROW — the helper's
        # default — because callers of this handle read columns by name.
        #
        # BORROWED HANDLE, DELIBERATELY NOT WRAPPED. This function RETURNS the
        # connection, so the CALLER owns its lifetime. `_canonical_connect`
        # returns a ClosingConnection, which closes on `with`-exit — putting this
        # in a `with` block here would hand back an already-closed handle and turn
        # a pragma fix into a use-after-close. #756 swept the `with
        # sqlite3.connect(...)` leak everywhere; this site is the opposite shape
        # and was correct as it stood. Only the pragmas were missing.
        conn = _canonical_connect(db)
    ...
    def _empire_read_conn(self):
        """The PURE read opener (#1095): an EXISTING empire DB + table, or None.

        No mkdir, no schema ensure, no seed, no soul/library restore, no
        backup files. An absent DB file or an absent empire_skills table
        answers None, which every reader treats as "no empire rows". The
        caller owns (and closes) the returned handle.
        """
        from aidocs_mcp._sqlite_connect import connect as _canonical_connect
        ...
        try:
            present = conn.execute(
                "SELECT 1 FROM sqlite_master WHERE type='table' AND name='empire_skills'",
            ).fetchone()
    ...
    def ensure_empire_seed(self) -> None:
        """BOOTSTRAP entry: idempotently seed empire_skills from the package-data
        payloads and run the soul/library restores (#141). Called from the
        SessionStart lifecycle rail (hook_pipeline.on_session_start); every
        write door reaches the same _empire_conn. Reads never seed (#1095)."""
    ...
    def _ensure_bundled_seed(self, conn) -> None:
        ...
        try:
            ...
            # DRT-01: retire the OLD bundled sample row under a RESERVED id —
            # but only while the seed still owns it. A manual row under that
            # id is the project's own law and stays (operator-wins, #479).
            for retired, successor in _RETIRED_BUNDLED_SAMPLE_IDS.items():
                has_successor = (
                    conn.execute(
                        "SELECT 1 FROM empire_skills WHERE skill_id=?", (successor,)
                    ).fetchone()
                ...
                if has_successor:
                    conn.execute(
                        "DELETE FROM empire_skills "
                        "WHERE skill_id=? AND source='bundled_seed'",
                        (retired,),
                    )
            conn.commit()
    ...
    def empire_skill_read(
        ...
        *,
        ...
        per aidocs-doctrine §IX). Returns the row dict or None if missing.
        """
        import sqlite3
    ...
    def empire_skill_upsert(
        ...
        *,
        ...
        NLP grant per aidocs-doctrine §IX.
    ...
    def empire_soul_append(
        ...
        *,
        ...
        aidocs-doctrine §IX). Without authority, refuses on sovereign rows.
        ...
        try:
            ...
            if not sovereign_authority:
                raise PermissionError(
                    ...
                    f"grant per aidocs-doctrine §IX).",
                )
    ...
    # surface.
    #
    # Per aidocs-doctrine §IX grant grammar + operator-intent
    ...
    def empire_soul_read(
        ...
        *,
        ...
        doctrine §IX) is REQUIRED — without it the door is sealed and the
        ...
        if not sovereign_authority:
            ...
            return None
        try:
            conn = self._empire_read_conn()  # pure read (#1095): no soul restore
            if conn is None:
                return None
            conn.row_factory = sqlite3.Row
            try:
    ...
    def _load_empire_skills_from_sql(self) -> list[SkillRecord]:
        ...
        A PURE read (#1095): an existing DB + table only. An absent store or
        table yields no empire rows; nothing is created, seeded or restored
        (that is ensure_empire_seed's job, on the bootstrap/write paths).
        """
        import sqlite3

        try:
            conn = self._empire_read_conn()
            if conn is None:
                return []
            conn.row_factory = sqlite3.Row
            try:
                ...
                # surface.
                # #501: gate on the SAME frozenset the role door uses — a
                # hardcoded pair here is how the two seats drifted apart.
                _marks = ",".join("?" for _ in self._MODE_GATED_ROLE_SKILLS)
                rows = conn.execute(
                    "SELECT skill_id, name, description, kind, tags, "
                    "content_text, source FROM empire_skills "
                    "WHERE read_access = 'public' "
                    f"AND skill_id NOT IN ({_marks}) "
                    "ORDER BY skill_id",
                    tuple(sorted(self._MODE_GATED_ROLE_SKILLS)),
                ).fetchall()
            finally:
                conn.close()
        except sqlite3.Error:
        ...
        for r in rows:
            ...
            # #677 RESOLUTION ORDER — LAW IS NOT AN ORDINARY SKILL.
            #
            # A SEEDED lawbook row (source='bundled_seed') used to resolve to
            # `project_local`, the SAME priority a file in `.MEMORY/skills/`
            # gets, and the project-file loop in `list_skills` runs FIRST while
            # `_register_skill_payload` only overrides on a STRICTLY lower
            # priority. So `.MEMORY/skills/aidocs-doctrine.md` — a plain file,
            # written through no guarded door at all — SHADOWED the sealed law
            # in the catalog, and every later agent read the plant as law. That
            # is law substitution without a law write.
            #
            # Guarding the upload door stops NEW plants; it does not settle
            # which row wins when both exist (an older plant, a hand-copied
            # file, a synced repo). The lawbook must win BY ADDRESS.
            #
            # DERIVED from LAWBOOK_SKILL_IDS + _LEGACY_LAWBOOK_IDS, never
            # restated (#461 cause A) — the same set every other lawbook guard
            # reads, so a future lawbook row is covered the moment it is named.
            # ORDINARY skills are untouched: their precedence is unchanged, and
            # this is the only id-conditional branch on the read path.
            if skill_id.strip().lower() in _lawbook_id_set():
                provider_label = "empire_canonical"
            else:
    ...
    def _write_external_providers(
        ...
        # The store's init_db (legacy-JSON ingest + delete) runs HERE, on
        # the write path -- never on a read (#1095).
    ...
    def migrate_external_providers(self, project_root: Path) -> None:
        """LIFECYCLE / WRITE-PATH migration of the legacy skill-providers.json
        (#1095): schema + ingest + retire the file. Called by the SessionStart
        rail and at the head of every provider write, so the write folds the
        legacy registry in before it rewrites. Never a read."""
        from .skill_providers_store import SkillProvidersStore

        SkillProvidersStore().migrate_legacy(project_root)
    ...
    def _read_external_provider_payload(
        ...
        #
        # A PURE read (#1095): the established store only -- no init_db (it
        # ingests + DELETES the legacy file), and the legacy file is never read
        # back as authority. A missing table reads as no providers; the legacy
        # registry comes across on the SessionStart rail or the next write.
        from .skill_providers_store import SkillProvidersStore

        providers = SkillProvidersStore().read(project_root)
    ...
    def register_external_provider(
        ...
        *,
        ...
        # Write path: fold the legacy registry in BEFORE reading the list we
        # rewrite, or the pure read's [] would clobber it (#1095).
        self.migrate_external_providers(project_root)
        providers = [
            item
    ...
    def set_external_provider_override(
        ...
        # Write path: migrate first so the rewrite starts from every provider.
        self.migrate_external_providers(project_root)
    ...
    def list_skills(
        ...
        # rows filtered at the SQL layer per empire-doctrine §XII / aidocs-doctrine §XI.
        ...
        # #501: mode-gated ROLE/seat skills never appear in the public
        # catalog — including ones that arrive via the BUNDLED payload loop
        # (the SQL loader filters its own rows, the file loader did not).
        rows = [
            row
            for row in rows
            if str(row.get("skill_id") or "").strip() not in self._MODE_GATED_ROLE_SKILLS
        ]
        if not include_body:
    ...
    def get_selected_skills(self, project_root: Path, session_id: str) -> dict[str, object]:
        ...
        # A READ (#1095): no init_db (it ingests + DELETES legacy skills.json
        # for every session) and no self-heal rewrite. A row wins; a
        # not-yet-migrated legacy selection is honoured in place; migration
        # happens on the write path (set_selected_skills -> init_db).
        persisted = store.get_row(project_root, session_id)
        if persisted is None:
            persisted = store.read_legacy(project_root, session_id) or []
        raw_selected = [str(item) for item in persisted]
        ...
        # No self-heal write: stale ids stay persisted and keep being
        # reported in invalid_selected_skills until a write replaces them.
...
def run_skill_authority_lifecycle(
    project_root: Path,
    skills: SkillStore | None = None,
) -> dict[str, str]:
    """The ONE skill-authority LIFECYCLE step (#1095, r0b2 655e9231-9d9).

    Converges what the skill catalog read no longer converges: the empire
    library seed/restore and the legacy skill-providers.json migration. It
    calls the SAME existing authorities -- ensure_empire_seed (the seeding
    _empire_conn) and migrate_external_providers (SkillProvidersStore.
    migrate_legacy) -- and implements neither.

    Called ONLY from lifecycle rails: SessionStart (hook_pipeline), the local
    ai_session(mode='connect') success path (the rail hookless stdio / Codex /
    OpenCode hosts go through) and project_bootstrap_or_resume. NEVER from a
    list/read/status path.

    Fail-open and step-independent: a failing step is named in the receipt
    (its exception class) and never raises, so it can never fail a connect,
    a bootstrap or a session start.
    """
    store = skills if skills is not None else SkillStore()
    receipt = {"empire_seed": "ok", "providers": "ok"}
    try:
        store.ensure_empire_seed()
    except Exception as exc:  # noqa: BLE001 - lifecycle is fail-open
        receipt["empire_seed"] = type(exc).__name__
    try:
        store.migrate_external_providers(project_root)
    except Exception as exc:  # noqa: BLE001 - lifecycle is fail-open
        receipt["providers"] = type(exc).__name__
    return receipt
