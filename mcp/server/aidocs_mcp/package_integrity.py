...
# THE ONE EXCLUDE THE SIGNATURE AXIS USES — shared so the two sides cannot
# NOTHING IS EXCLUDED FROM THE SIGNATURE AXIS (#925d).
#
# There WAS an `FP_SIGNATURE_EXCLUDE = ("_vendor/",)` here for exactly one
# commit, and it was wrong in the most dangerous way available: it made the
# digest ignore EXECUTABLE CODE that ships inside the package.
# `aidocs_mcp/_vendor` carries the palace engine, and the reason #925 put it
# inside the package rather than beside it is stated in
# `aidocs_mcp/__init__._package_vendor_root`:
#
#     "Files under aidocs_mcp/ are covered by compute_package_fingerprint,
#      which IS the release signature. A tree staged beside the package would
#      be transported unsigned — the supply-chain hole #925 refuses."
#
# Excluding it re-opened that hole from the other side: the bytes still shipped
# and verification stopped looking at them, so a client would accept whatever
# `_vendor` an artefact happened to carry.
#
# The mismatch the exclude was reaching for was real, but its cause was the
# ORDER of the deploy, not the digest: `_vendor` was staged at SIGN time, after
# the custody tar was taken, so the promoted tree lacked a file the manifest
# digests. That is the same failure `_build_stamp.py` hit twice, and it has a
# settled remedy — materialize into the FROZEN SHIP-STAGE before the custody
# tar (Gate 2a), so custody, the seal and the signature describe ONE tree.
# Fixed there. Nothing is excluded here.
# (no constant here — the name is gone on purpose, so nothing can import it
#  back into the digest path by habit.)
...
def compute_package_fingerprint(
    ...
    *,
    ...
    for f in sorted(base.rglob("*"), key=lambda p: p.relative_to(base).as_posix()):
        ...
        # A trailing slash means a TREE, not a file (#925). The build stamp is
        # one path and needed nothing more; the packaged vendored engine is a
        # directory, and the freshness axis must omit all of it on BOTH sides or
        # a release that carries it reads as permanently STALE against a source
        # checkout that does not. Exact paths keep matching exactly, so every
        # already-signed manifest digests the same bytes as before.
        if rel in exclude or any(
            e.endswith("/") and rel.startswith(e) for e in exclude
        ):
            continue
        suf = f.suffix.lower()
        in_runtime_tree = any(
...
def prove_trust_chain(
    ...
    *,
    ...
    """Prove the selected-runtime trust chain end-to-end:
      1. every host adapter EXECUTES the selected governing runtime — .mcp.json
         and Codex by naming it directly, the Claude hook either directly or
         through the stable launcher + fail-closed shim (see below);
    ...
    LINK 1 USED TO BE A PATH EQUALITY, and #925k repealed that without telling
    this function (#1047). Once runtime generations exist the Claude hook entry
    must NOT name the governing runtime: a generation directory is removable by
    design, and a host entry pointing at a collected one cannot start at all —
    no shim can rescue a process that never runs. So the host names a stable,
    non-prunable launcher and the fail-closed shim reads the activation pointer
    and delegates INTO the verified generation.

    The consequence of leaving the old equality here was not cosmetic: a
    CORRECT generations install wrote the right stable entry, failed its own
    theorem about it, and `aidocs setup` exited 1.

    WHAT IS STILL FORBIDDEN IS UNCHANGED, and is the actual split-brain this
    link exists to catch: trust recorded for one governing interpreter while
    the host executes a DIFFERENT one that AIDOCS never verified. `delegated`
    is not that — it is the same governing runtime reached through one
    audited hop. The judgment lives in
    `claude_hooks_install.judge_hook_launcher`, shared with the installer, the
    passive repair and both health surfaces, so this proof cannot drift from
    what the writers actually write.
    """
    ...
    # THE CLAUDE ADAPTER IS JUDGED BY ROLE, NOT BY STRING EQUALITY (#1047).
    # It is the one adapter with something that can delegate for it, so
    # "different path" and "different governing runtime" stopped being the same
    # statement the moment generations existed. mcp and codex keep the plain
    # equality: nothing delegates for the module form, so naming anything else
    # there really does mean an unverified interpreter runs.
    from .claude_hooks_install import judge_hook_launcher

    claude_verdict = (
        judge_hook_launcher(claude_command, python_path, home=base)
        if claude_command
        else None
    )
    mismatches = {
        ...
        if k != "claude" and v != sel_norm
    }
    if claude_verdict is not None and not claude_verdict["governed"]:
        mismatches["claude"] = adapters["claude"]

    def _claude_status() -> str:
        if claude_verdict is None:
            return "absent"
        if not claude_verdict["governed"]:
            return "mismatched"
        # An entry that governs but cannot survive generation GC is NOT a trust
        # failure — the shim delegates to the verified generation today. It is a
        # LIFETIME defect, reported as itself so a reader is sent to the right
        # remedy (converge the entry) instead of the wrong one (reprovision the
        # runtime). The health surfaces refuse to call it healthy; this proof
        # refuses to call it a split brain.
        return "matched" if claude_verdict["durable"] else claude_verdict["role"]
    ...
    statuses["claude"] = _claude_status()
    path_eq = {
        "ok": (not mismatches),
        "proven": bool(present),
        "selected": sel_norm,
        ...
        **adapters,
        ...
        "claude_role": (claude_verdict or {}).get("role", "absent"),
        "claude_governed": bool((claude_verdict or {}).get("governed")),
        "claude_durable": bool((claude_verdict or {}).get("durable")),
    }
