...
# @vulture-class: fp
# ── execution_event_retention.PENDING_EVENT_KINDS ──
# INTENTIONALLY TEST-CONSUMED REGISTRY CONSTANT — not dead code, and NOT a
# precedent for allowlisting ordinary dead code (r0b ruling, #1061).
# Owner: #1061 (consul stream retention). The sanctioned pre-registration
# ledger (kind -> owner ticket) for event kinds classified AHEAD of their
# emitter. It is live BY CONTRACT, proven by:
#   test_execution_event_retention_registry.py::
#     TestEveryEmitterIsClassified::test_the_registry_has_no_dead_entries
#     (exempts exactly these kinds from the dead-entry check)
#   test_consul_stream_retention_1061.py::test_a_pending_kind_graduates_once_emitted
#     (fails once any pending kind gains an emitter — forced graduation)
#   test_consul_stream_retention_1061.py::test_every_pending_kind_is_classified_and_ticketed
#   test_consul_stop_park_1061.py (asserts the emitted Stop-park kind left it)
# All consumers live under mcp/tests/, outside vulture's ship-stage scan roots.
# Retire when the dict is empty (every pending kind has a live emitter).
PENDING_EVENT_KINDS  # noqa: F821

# @vulture-class: fp
# ── doctrine_global_law_seed.resolve_article / legacy_operator_rows ──
# Owner: stable law pointer ids (2eeb204d6). resolve_article is the reorder
# guard's oracle (a pointer binds to an article by TITLE, never by ordinal).
# legacy_operator_rows is the operator's report of operator-owned rows still
# under numeral ids — ensure_doctrine_global_law names it in its warning; the
# operator reads it, the seed never acts on those rows.
# CONSUMERS (outside vulture's ship-stage scan roots):
#   mcp/tests/memory/test_doctrine_global_law_stable_ids.py
# Retire when an operator-facing surface (ai_memory/law status) calls
# legacy_operator_rows, or when no numeral-id operator rows remain.
resolve_article  # noqa: F821
legacy_operator_rows  # noqa: F821

# @vulture-class: fp
# ── receive_lease_store.ReceiveLeaseStore.overdue — READ-ONLY PROBE ──
# Owner: #489 (60373419c moved the notification injector to the one-pass
# `claim_overdue_alarms` latch, leaving this non-mutating query as the
# test-side oracle for "which leases are past deadline" without latching).
# CONSUMERS (outside vulture's ship-stage scan roots):
#   mcp/tests/coordination/test_receive_lease_drt11.py (x3)
#   mcp/tests/host/test_stop_fault_containment_and_diagnosis_door.py (x1)
# Retire when those tests assert through claim_overdue_alarms and it is deleted.
overdue  # noqa: F821
...
# @vulture-class: fp
# ── spawn_authority policy GENERATORS — consumed by a script, not by runtime ──
# Owner: #1031 (spawn doctrine has ONE structural authority).
# `render_semgrep_rule` and `render_baseline_rows` are the whole point of that
# programme: the Semgrep spawn rule and the waiver registry are GENERATED from
# spawn_authority.SPAWN_SITES, so changing doctrine needs no second policy edit.
#
# Their consumer is `mcp/scripts/generate_spawn_policy.py:75` (--write emits the
# artifacts; --check exits 1 on drift and is what the tests run), plus three
# assertions in tests/security/test_spawn_authority_is_the_one_source_1031.py.
# Vulture's scan roots are `mcp/server/aidocs_mcp` + this file — neither
# `mcp/scripts/` nor `mcp/tests/` is in them, so a script-only entry point is
# INVISIBLE to it. That is the classic `fp` shape, not dead code.
#
# THE TEST IS THE PROOF, NOT THIS COMMENT: `generate_spawn_policy.py --check`
# runs in the deploy lane and in test_spawn_law_is_locally_enforced_1031, so if
# these functions ever stopped being called the gate would fail there — loudly —
# rather than leaving this entry to quietly grandfather real dead code.
render_semgrep_rule  # noqa: F821
render_baseline_rows  # noqa: F821

# @vulture-class: fp
# ── #925 packaged-engine staging — consumed by the RELEASE BUILDER, not runtime ──
# Owner: #925 (a signed release must carry a bootable mempalace).
# Same shape as the spawn generators above: vulture's scan roots are
# `mcp/server/aidocs_mcp` + the two suppression files, and `mcp/scripts/` is not
# among them, so a script-only entry point is INVISIBLE to it.
#
# The consumers are both in `mcp/scripts/build_signed_release.py`:
#   :318  _staged = stage_packaged_vendor(pkg, _engine)
#   and packaged_vendor_matches_claim gating the signature — on mismatch the
#   builder RETURNS 2 ("REFUSING TO SIGN") rather than warning, because this is
#   the one place it would otherwise ship bytes it cannot vouch for.
#
# THE TEST IS THE PROOF, NOT THIS COMMENT: eight assertions in
# tests/security/test_release_bootstrap_composition_925.py drive both functions
# against a REAL built release (fresh copy equivalent, tampered caught, STALE
# caught, staging provably precedes fingerprinting). If either stopped being
# called, those fail loudly rather than this entry quietly grandfathering dead
# code.
stage_packaged_vendor  # noqa: F821
packaged_vendor_matches_claim  # noqa: F821

# @vulture-class: fp
# ── blast-radius changed-symbol ledger: the per-edit delta reader ──
# Owner: #1075 (an edit's blast radius is the symbols it CHANGED, not the file).
# `blast_envelope.changed_symbols_for_edit(project_root, rel_path)` compares the
# file's current symbol set against the snapshot taken at the previous edit and
# returns the delta (None = no baseline yet). Its production consumers are the
# writers of that snapshot; the READER's contract — first call returns None,
# a rename shows as one added name, a file never edited returns None, a
# different file does not inherit another's baseline — is pinned entirely by
# the seam test, which lives outside vulture's ship-stage scan roots.
# CONSUMERS: mcp/tests/security/test_blast_changed_symbols_1075.py (13 calls,
#   one per behaviour above).
# Retire when a production surface reads the delta (the envelope renderer), or
# with the function if #1075 drops the ledger.
changed_symbols_for_edit  # noqa: F821

# @vulture-class: fp
# ── branch-transition barrier: the schema oracle and the three switch-lane probes ──
# Owner: the ai_git switch lane (branch_transition_barrier.py).
# Vulture's scan roots are `mcp/server/aidocs_mcp` + the two suppression files;
# mcp/tests/ is not among them, so a test-proved invariant reads as dead.
#
# `_required_columns()` -> {table: {column, ...}} is the barrier's DECLARED
# schema. It is deliberately data rather than code: the migration path builds
# its DDL from the live tables, so the only way to catch a column that silently
# stopped being created is to compare the real sqlite_master against a
# hand-maintained manifest. That comparison is the test's job, not the
# runtime's — a server that checked its own manifest against itself would prove
# nothing.
#   CONSUMER: tests/security/test_ai_git_switch_op.py:2898 (`btb._required_columns()`).
#
# `is_transport_created_web_actor(host_session_id)` is the STRUCTURAL gate-only
# test named as the authority by `_record_server_mediated_provenance`'s own
# docstring (branch_transition_barrier.py). The production path asks
# `caller_is_gate_only_surface()` — the server's registration — precisely
# because a host-session id must never be classified by spelling; this function
# is what the tests drive to prove the two agree and that an id alone says
# nothing about capability.
#   CONSUMERS: tests/security/test_ai_git_switch_op.py:1777, 2619, 2628, 2634.
#
# `egress_barrier_refusal(cwd, reachability)` returns '' or the refusal reason
# for a shell that would step outside the barrier. It is the pure predicate
# behind the refusal string; the egress path composes its message from the same
# facts, and the tests assert the empty-verdict cases directly so a widening of
# the predicate cannot pass unnoticed.
#   CONSUMERS: tests/security/test_ai_git_switch_op.py:684, 690.
# Retire each with its symbol, or when a production caller appears.
_required_columns  # noqa: F821
is_transport_created_web_actor  # noqa: F821
egress_barrier_refusal  # noqa: F821

# @vulture-class: fp
# ── OAuth surface-kind: the RETIRED legacy key and the DEMOTED projection reader ──
# Owner: #566 / r0b's ruling — a legacy config value may never create a
# positive `gate_only` record.
#
# `CHATGPT_SURFACE_KIND_CONFIG_KEY` ("oauth.chatgpt_surface_kind") HAS NO
# PRODUCTION READER AND MUST NOT ACQUIRE ONE — that absence IS the fix. It is
# the pre-#566 config key whose stored value used to decide a client's surface
# kind; the live path now answers `unknown` regardless (outer_gate_oauth.py
# names this key in the comment at the decision site). It survives so the tests
# can PLANT that legacy state and prove it is ignored. Delete the constant and
# the tests lose the ability to construct the adversary they exist to defeat —
# the regression would then be invisible, which is the worst outcome available.
#   CONSUMERS: tests/security/test_oauth_chatgpt_client.py:137-140 (plants each
#     legacy value); tests/security/test_ai_git_switch_op.py:2037-2072 (plants
#     SURFACE_GATE_ONLY and asserts no positive record follows), :2139-2150.
#
# `OAuthStore.client_surface_kind` was DELIBERATELY DEMOTED, not orphaned: its
# docstring now states it reads the REBUILDABLE PROJECTION and is "NOT THE
# AUTHORITY — NEVER WIRE THIS INTO A DECISION". Having no decision consumer is
# the property being kept, and outer_gate_transport.py:6814 points at it by
# name as the rebuildable projection. It stays as the reporting/reconciliation
# reader the tests interrogate to prove the projection's own rules (revocation
# does not resurrect a kind, a live grant does not leak one, a default client
# reads `unknown`).
#   CONSUMERS: tests/security/test_ai_git_switch_op.py — ~18 assertions between
#     :1824 and :2466.
# Retire the constant when no pre-#566 install can still hold that key; retire
# the reader if the projection is deleted. NEITHER may be retired by wiring it
# into a decision.
CHATGPT_SURFACE_KIND_CONFIG_KEY  # noqa: F821
_.client_surface_kind  # noqa: F821

# @vulture-class: fp
# ── tool_response_classifier: the two named evidence predicates ──
# Owner: the test-retry lane (rerun-after-change).
# `classify_tool_response` is the wired entry point; `is_dry_run(resp)` and
# `is_zero_delta(resp)` are the two evidence questions it is built from, and the
# module docstring (tool_response_classifier.py:19) documents is_dry_run as part
# of the surface. The retry tests assert them DIRECTLY rather than through the
# classifier because the classifier's verdict is a fold of both — a shape that
# flipped only is_dry_run would still classify identically today and silently
# stop doing so tomorrow.
# CONSUMERS (outside vulture's ship-stage scan roots):
#   tests/security/test_test_retry_rerun_after_change.py:495-498, 560-565 (every
#     dry-run evidence shape), :669-671 (zero delta).
# Retire when the tests assert only through classify_tool_response and the two
# predicates are inlined into it.
is_dry_run  # noqa: F821
is_zero_delta  # noqa: F821
