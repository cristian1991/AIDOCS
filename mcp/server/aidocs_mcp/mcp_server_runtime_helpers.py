...
_AIDOCS_MARKER = Path(".MEMORY") / ".aidocs" / "index.aidocs"
...
def _has_marker(path: Path) -> bool:
    return (path / _AIDOCS_MARKER).is_file()


def find_aidocs_project_root(candidate: Path) -> Path | None:
    """Walk up from ``candidate`` until an AIDOCS marker file appears.

    The marker is the presence of ``.MEMORY/.aidocs/index.aidocs``
    specifically — a bare directory at that path is insufficient because
    stray index-store mkdirs can create `.MEMORY/` with nothing in it.
    """
    try:
...
def is_aidocs_managed(root: Path) -> bool:
    ...
    Today: presence of the in-tree marker ``.MEMORY/.aidocs/index.aidocs``.
    Migration target (when ``.MEMORY`` is retired for the Postgres-backed
    webmcp): a managed-project REGISTRY lookup keyed by the git root. Routing
    all managed-detection through here makes that a one-function flip instead
    of an N-site hunt. Behaviour-preserving for now.
    """
...
def discover_project_root() -> Path:
    ...
    raise NoAidocsProjectError(
        "No AIDOCS project found. Expected marker "
        ".MEMORY/.aidocs/index.aidocs in the project root or an ancestor. "
        "Run `aidocs project init` in the target project, or set "
        "AIDOCS_PROJECT_ROOT.",
    )
...
# ── Deprecated aliases (one release; remove with the column drop) ──


def set_calling_conductor_cli_session_id(cli_session_id: str) -> None:
    """Deprecated: use set_calling_conductor_host_session_id."""
    set_calling_conductor_host_session_id(cli_session_id)


def current_calling_cli_session_id() -> str:
    """Deprecated: use current_calling_host_session_id."""
    return current_calling_host_session_id()
