...
def canonical_server_fields(
    ...
    """
    transport_str = str(transport or "stdio")
    if transport_str in ("http", "sse"):
        # Daemon/remote entries (#249): the command column carries the URL and
        # the projection renders the {type, url} shape Claude Code expects.
        return {"type": transport_str, "url": str(command or "")}
    block = {
        "type": transport_str,
