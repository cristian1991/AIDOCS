...
class PromptSubmitResult:
    ...
    # Audit attribution (empire §XII evidence-not-law, aidocs §IX grant grammar):
    # which sub-pipelines fired and WHY
    ...
    def to_claude_envelope(self) -> dict[str, Any] | None:
        if self.decision == "block":
            envelope = {"decision": "block", "reason": self.reason}
            if self.blocked_by:
                envelope["blocked_by"] = self.blocked_by
            return envelope
    ...
    def to_adapter_dict(
        self,
        *,
        rewrite_context_fallback: bool = False,
    ) -> dict[str, Any]:
        """Render the host-neutral prompt result.

        Hosts with a native ``additionalContext`` channel consume
        ``additional_context_blocks`` directly. Hosts that can only mutate the
        prompt may opt into a clearly fenced prepend. The fallback never
        changes the decision and consumes the separate blocks to avoid a host
        rendering the same advisory twice.
        """
        rewritten = self.rewritten_prompt
        context_blocks = [self.additional_context] if self.additional_context else []
        if rewrite_context_fallback and self.decision != "block" and self.additional_context:
            rewritten = (
                "<aidocs-additional-context>\n"
                + self.additional_context
                + "\n</aidocs-additional-context>\n\n"
                + rewritten
            )
...
class PromptSubmitService:
    def __init__(
        ...
        *,
        ...
        # #489: was a hard-coded 10.0 — the SAME number as the UPS round-trip
        # budget, so one contended acquire spent the operator's entire budget
        # waiting and the answer arrived after the client had already degraded
        # (MEASURED: 10.017s of a 13.329s evaluation). One deadline, one source.
        lock_timeout_seconds: float = _hook_budget.PROMPT_SUBMIT_LOCK_BUDGET_S,
    ...
    @staticmethod
    def _extract_result(raw: Any, prompt: str) -> tuple[str, str, str, str, str]:
        if raw is None:
            return "allow", "", "", prompt, ""
        ...
        blocked_by = str(raw.get("blocked_by") or "")
        ...
        return decision, reason, blocked_by, rewritten, context

    def _cross_turn_scrutiny_block(
        self,
        project_root: Path,
        prompt: str,
        *,
        ...
        """Observe this prompt for CROSS-TURN scrutiny (#651) and return an
        advisory context block, or "".

        WIRED HERE, and only here, because this is the ONE point both prompt
        rivers traverse: the turn-boundary hook (``claude_hook``
        ``_handle_user_prompt_submit`` -> ``hook_pipeline.run_user_prompt``) and
        the mid-turn INTERRUPT replay (``mid_flight_ups._default_judge`` ->
        the same ``run_user_prompt``). Hanging off a host hook would cover the
        first river only; interrupts come free from here.

        ADVISORY, NEVER A GATE. It cannot change ``decision``, cannot raise, and
        cannot skip: every prompt from every principal is observed, and a failure
        yields "" rather than a verdict (§IX drop-on-doubt).
        """
        try:
            from .cross_turn_scrutiny import observe_prompt, warnings_for_envelope

            state = observe_prompt(
                Path(project_root),
                prompt,
                host_kind=host_kind or None,
                host_session_id=host_session_id or None,
                # The ORIGIN GATE's verdict, never a caller's self-declaration.
                verified_operator_origin=bool(grant_eligible),
            )
            return "\n\n".join(warnings_for_envelope(state))
        except Exception:
            return ""
    ...
    def evaluate_submit(
        ...
        *,
        ...
        # #135 self-heal, BEFORE the participants are built (#672 fallout,
        # measured 2026-08-01). build_prompt_submit_participants resolves the
        # session per-conductor and yields "" for a host #672 refuses, so an
        # unbound host produced a transaction that snapshotted NOTHING: a
        # mid-mutation failure then "rolled back" while every authority write
        # of that turn survived. Binding identity first restores the
        # transaction's atomicity guarantee. Idempotent — a bound host returns
        # immediately.
        try:
        ...
        try:
            ...
            decision, reason, blocked_by, rewritten, context = self._extract_result(raw, prompt)
            # #651 cross-turn scrutiny — ADDITIVE and advisory. Appended after
            # the core's own context so it can never displace a refusal reason
            # or a surfaced doctrine block; empty for every prompt that does not
            # complete the credential + external-action composite.
            #
            # STAYS INLINE HERE ON PURPOSE. evaluate_submit is the ONE point
            # both prompt rivers traverse (turn boundary via
            # hook_pipeline.run_user_prompt, mid-turn via
            # mid_flight_ups._default_judge), and
            # test_cross_turn_scrutiny_651 pins that by reading this function's
            # source. Moving the call into the extracted tail below would keep
            # the runtime invariant (the tail is reachable only from here) while
            # silently voiding the pin that protects it — so the extraction
            # stops short of this call.
            scrutiny_block = self._cross_turn_scrutiny_block(
                project_root,
                prompt,
                host_kind=host_kind,
                host_session_id=host_session_id,
            ...
            if scrutiny_block:
                context = "\n\n".join(part for part in (context, scrutiny_block) if part)
            return self._finalize_successful_submit(
                project_root,
                decision=decision,
                reason=reason,
            ...
            self._abort_restore_and_release(txn, ledger)
            raise
        if txn.committed:
    ...
    def _finalize_successful_submit(
        self,
        project_root: Path,
        *,
        decision: str,
        ...
        """The SUCCESS exit's TAIL, extracted verbatim from ``evaluate_submit``.

        Extracted 2026-08-13 (#413 tranche D upkeep): ``evaluate_submit`` had
        crept back to CC 23 against its rank-C ceiling of 20, and every added
        branch in a gate path is a potential bypass — which is the whole
        premise of the pin. The four exits keep their ORDER and semantics; this
        is the tail of the first of them and nothing about it changed.

        The decision extraction and the #651 scrutiny append stay in the
        caller — see the note there — so this helper starts at the rollback.

        Ordering that must not be disturbed:
          1. roll back if the snapshot was never committed,
          2. release the lock, then publish the stage list,
          3. record the degraded event when snapshot infra was unavailable.
        """
        ...
        return PromptSubmitResult(
            ...
            state_ledger=ledger,
            host_integration=("explicit_hookless" if explicit_hookless else "native_hook"),
            automatic_ups=not explicit_hookless,
            why=tuple(why_tags),
        )

    @staticmethod
    def _abort_restore_and_release(
        txn: _SubmitTransaction,
        ledger: PromptSubmitStateLedger,
    ) -> None:
        """BaseException path: restore an uncommitted snapshot, always release.

        Extracted verbatim alongside ``_finalize_successful_submit``. The
        caller still re-raises — this helper must never swallow the exception,
        which is why it returns None and catches only the ROLLBACK's own
        failure. ``release_lock`` stays in a ``finally`` so a failed restore
    ...
    def _run_submit_core(
        ...
        *,
        ...
        # Both brackets are opened INSIDE the try whose finally resets them:
        # a raise between the stamp and the try would otherwise leak this
        # request's identity (and its mode memo) into the next request in the
        # same process -- the transport-scope defect in a narrower window.
        identity_token = None
        memo_token = None
        audit_session_id = ""
        try:
            ...
            # #587-A: THE authoritative moment. This is the one place that
            # receives the host's OWN statement of what it is (the UPS
            # payload's host_kind) together with its session id. Until now that
            # statement lived exactly as long as the ContextVar above and was
            # then lost, so the next request had to guess from the MCP server's
            # environment -- the "six answers, three disagreeing" defect.
            # Recording it here makes the answer OUTLIVE the request, keyed by
            # the host's own session id.
            try:
                from .agent_memory_epoch import record_host_identity
                ...
                # Identity capture must never fail a prompt submission.
                pass
            ...
            from .managed_mode_service import begin_request_mode_memo
            ...
            # Guarded: either bracket may never have opened (a raise above),
            # and the memo's reset is imported here so a failed import of the
            # module cannot turn the reset itself into a NameError.
            if memo_token is not None:
                from .managed_mode_service import reset_request_mode_memo
    ...
    def _submit_transaction_failed_result(
        ...
        *,
        ...
        """Pre-commit stage failure: roll back, audit, flip SEC-005 degraded
        state — then BLOCK for a MUTATION stage, or DEGRADE for an advisory one.
        (Extracted from evaluate_submit, #413 tranche D.)

        THE SPLIT IS #748 (2026-08-03). This handler used to block on ANY stage
        failure, which silently contradicted the doctrine stated at the top of
        this module: intent_dispatch / route_classification / operator_intent
        "are READ/advisory consumers ... they still run so the operator's prompt
        degrades to advisory context INSTEAD OF BEING BLOCKED on an infra hiccup
        (infra errors fail OPEN for read/advisory surfaces only)."

        That was implemented for a MISSING SNAPSHOT (PromptSubmitMutationUnavailable
        skips mutation stages) but never for an exception RAISED INSIDE an advisory
        stage — those took this path and blocked. Observed on the live project:
        `prompt_mutation_failed` with failed_stage=intent_dispatch and
        =route_classification, error "database is locked", decision=block. A
        transient sqlite contention in a READ-ONLY stage stopped the operator's
        prompt outright.

        Failing open here is safe by construction, not by optimism: an advisory
        stage is excluded from MUTATION_STAGES precisely because it changes no
        authority, so there is nothing a block would be protecting. Rollback,
        audit and the SEC-005 degraded flag still happen either way — the prompt
        proceeds with reduced context, and says so, rather than being refused.
        """
        rollback_failure = ""
        if txn.snapshot is not None and not txn.committed:
            try:
        ...
        # ADVISORY STAGE -> FAIL OPEN (#748). Membership in MUTATION_STAGES is the
        # authority: a stage outside it committed nothing, so blocking protects
        # nothing and costs the operator the whole turn. The prompt proceeds with
        # reduced context and the SEC-005 degraded flag set above already records
        # that it did. A mutation stage still blocks, unchanged.
        if txn.current_stage and txn.current_stage not in MUTATION_STAGES:
            return PromptSubmitResult(
                decision="allow",
                reason="",
                rewritten_prompt=prompt,
