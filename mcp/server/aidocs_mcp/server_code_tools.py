...
def register_code_tools(
    *,
    ...
    def code_search(query: str = "", modified_since: str | None = None, limit: int = 10, root: str = "") -> list[dict[str, Any]]:
    ...
    def code_text_search(
        ...
        """Full-text search across all indexed files. Replaces Grep — returns matches with line numbers. Use | or OR for multi-term. Set regex=true for patterns."""
        root = resolve_project_root(root)
    ...
    def code_get_symbol_info(
        ...
        Examples:
          code_get_symbol_info("my_method", kind="signature")
          code_get_symbol_info("MyClass", kind="constructor")
          code_get_symbol_info("MyClass", kind="api")
          code_get_symbol_info("Status", kind="enum")
          code_get_symbol_info("UserDTO", kind="properties")
          code_get_symbol_info("", kind="signatures", symbols=["method_a", "method_b"])
          code_get_symbol_info("", kind="constructors", symbols=["ClassA", "ClassB"])
        """
        root = resolve_project_root(root)
        k = kind.strip().lower()

        if k == "signature":
            return hub.code.get_method_signature(root, method_name=symbol, container=container, limit=limit)
        elif k == "signatures":
            methods = symbols or [symbol]
            return hub.code.get_method_signatures(root, methods=methods, container=container, limit_per_method=limit)
        elif k == "constructor":
            return hub.code.get_constructor_params(root, type_name=symbol, limit=limit, include_related=include_related)
        elif k == "constructors":
            types = symbols or [symbol]
            return hub.code.get_constructor_params_batch(root, types=types, include_related=include_related, limit_per_type=limit)
        elif k == "enum":
            return hub.code.get_enum_values(root, enum_name=symbol, limit=limit, include_related=include_related)
        elif k == "properties":
            return hub.code.get_entity_properties(root, entity_name=symbol)
        elif k == "api":
            return hub.code.get_service_api(root, service_name=symbol, limit=limit)
        else:
            return {"error": f"Unknown kind: {kind}. Use: signature, signatures, constructor, constructors, enum, properties, api."}

    # ── Legacy granular tools (kept for backward compat, deferred) ──
    ...
    def code_get_method_signature(
        ...
        """Get exact method signature without reading the file. Use BEFORE editing or calling a method — confirms params, return type, decorators in ~30 tokens instead of reading 100+ lines."""
        root = resolve_project_root(root)
    ...
    def code_get_method_signatures(
        ...
        """Get exact signatures for multiple methods in one call. Batch version — pass a list of method names, get all signatures back. Saves N file reads."""
        root = resolve_project_root(root)
    ...
    def code_get_enum_values(
        ...
        """Get all enum members and values without reading the file. Use when you need to know valid enum states (e.g., LaneState values, status codes)."""
        root = resolve_project_root(root)
    ...
    def code_get_constructor_params(
        ...
        """Get constructor/record params without reading the file. Use BEFORE instantiating a class — confirms required args, types, defaults."""
        root = resolve_project_root(root)
    ...
    def code_get_constructor_params_batch(
        ...
        """Get constructor params for multiple types in one call. Batch version — pass a list of type names."""
        root = resolve_project_root(root)
    ...
    def code_get_service_api(
        ...
        """Get the full API surface of any class in one call — all method names, signatures, and containers. Use BEFORE editing a service to understand its interface. Replaces 5+ file reads."""
        root = resolve_project_root(root)
    ...
    def code_get_entity_properties(
        ...
        """Get all properties/fields of a data model, DTO, or entity class. Use for data-layer work — confirms field names and types without reading the file."""
        root = resolve_project_root(root)
        ...
        if m == "symbols":
            ...
            return _grant(
                hub.code.search_symbols(
                    root, query=query, kind=kind, role=role, limit=limit, modified_since_ns=mtime_ns
                )
            )
        if m == "references":
