...
Consumers — and the DELIBERATE split (this is NOT one law everywhere):
  ...
  * The DESTRUCTIVE FLOOR (``evaluate_destructive_floor``) additionally
  ...
  The taxonomy also exports one canonical shape-BLIND locked-family set
  (``SHELL_LOCKED_COMMAND_FAMILIES``). Both the destructive policy floor and
  the user-intent grant filter import that same object. Internal un-gated egress
  (``code_runner._run_process`` behind ``git_ops``) therefore preserves the
  stricter rule that any locked family is refused, while gated surfaces may still
  apply the richer shape taxonomy where policy permits it.
...
# One home for command families that can never be minted by an intent grant.
# Policy evaluation and grant detection import this exact object so the
# unbypassable floor cannot drift between consumers.
SHELL_LOCKED_COMMAND_FAMILIES: frozenset[str] = frozenset(
    {
        "rm",
        "sudo",
        "doas",
