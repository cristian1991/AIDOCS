...
#: Inverse of TERMINAL_STATE_FOR_SEAL_REASON for the boundary sealer: a turn
#: already sitting in a terminal state must be sealed with a reason whose
#: terminal state MATCHES, or the frozen machine rejects the transition.
_SEAL_REASON_FOR_TERMINAL_STATE: dict[TurnState, SealReason] = {
    TurnState.COMPLETED: SealReason.NORMAL_COMPLETION,
    TurnState.INTERRUPTED: SealReason.USER_INTERRUPT,
    TurnState.ABANDONED: SealReason.SERVER_RECOVERY,
}
...
class CausalTurnStore(SQLiteIndexStoreBase):
    ...
    # ── natural turn end (the prompt boundary, #444) ──

    def close_superseded_turn(
        self,
        project_root: Path,
        prior_turn_id: str,
        *,
        ...
        """Seal the turn a NEW accepted operator instruction just ended.

        This is the causal-turn's natural terminal seam (#444): the prior
        turn is provably over the instant the operator issues the next
        instruction, so it is sealed HERE rather than late by a sweeper —
        the recovery pass stays the backstop for turns no boundary can
        reach, not the only sealer (invariant 10).

        The seal reason is derived from what actually happened, never
        flattened to "recovery":

        * a durable ATTEMPT still in flight (a ``tool_call_started`` row
          with no outcome) means the turn died MID-EXECUTION — the new
          instruction is recorded as a first-class OperatorInterrupt
          governance event (advancing the instruction revision, never a
          silent edit) and the turn seals as USER_INTERRUPT / INTERRUPTED;
        * otherwise every attempt was resolved and the turn seals as
          NORMAL_COMPLETION / COMPLETED;
        * a turn already driven to a terminal state (e.g. ABANDONED by a
          prior mint whose seal failed) seals with the reason matching that
          state, so no illegal transition is ever attempted.

        Orphaned attempts are classified INDETERMINATE — recovery may not
        invent proof and never auto-retries a non-idempotent attempt.

        Raises on failure: the caller decides the fail-direction, and an
        unsealed turn must never become invisible.
        """
        tid = str(prior_turn_id or "").strip()
        if not tid:
            return {"sealed": False, "reason": "no_prior_turn"}
        turn = self.get_turn(project_root, tid)
        if turn is None:
            return {"sealed": False, "reason": "unknown_turn"}
        state = TurnState(turn["state"])
        if state is TurnState.SEALED:
            return {"sealed": False, "reason": "already_sealed", "turn_id": tid}

        orphans = [
            o
            for o in self.list_orphan_attempts(project_root, turn_id=tid)
            if not o.get("resolution")
        ]
        pre_terminal = state in (
            TurnState.OPEN,
            TurnState.EXECUTING,
        ...
        if superseded_by:
            with self.session(project_root) as conn:
                conn.execute(
                    "UPDATE causal_turns SET superseded_by = ? WHERE turn_id = ?",
                    (str(superseded_by), tid),
                )
        interrupt: dict[str, Any] | None = None
        if pre_terminal and orphans:
            interrupt = self.record_interrupt(
                project_root,
                tid,
                kind=InstructionKind.OPERATOR_INTERRUPT,
                actor_id=actor_id or self._resolve_actor(project_root),
                actor_role=actor_role,
                reason=(
                    "superseded by a new operator instruction while a durable "
                    "tool attempt was still in flight"
                ),
                origin_channel=origin_channel,
                affected_attempt_ids=[o["attempt_event_id"] for o in orphans],
            )
            reason = SealReason.USER_INTERRUPT
        elif pre_terminal:
            reason = SealReason.NORMAL_COMPLETION
        else:
            reason = _SEAL_REASON_FOR_TERMINAL_STATE[state]
        for orphan in orphans:
            self.resolve_orphan(
                project_root,
                orphan["attempt_event_id"],
                ...
                evidence=(
                    "turn boundary: durable attempt with no outcome when the "
                    "next operator instruction superseded the turn"
                ),
                resolved_by="turn_boundary",
                turn_id=tid,
                session_id=session_id or str(turn["session_id"] or ""),
            )
        seal = self.seal_turn(project_root, tid, reason)
        return {
            "sealed": True,
            "turn_id": tid,
            "seal_reason": reason.value,
    ...
    def recover_open_turns(
        ...
        *,
        ...
        ``stale_before`` (#444) is the middle ground the backstop needs: an
        ISO-8601 cutoff (inclusive) that makes a session's CURRENT turn
        eligible ONCE IT HAS GONE IDLE past that instant. Without it the
        LAST turn of every session would stay `current` — and therefore
        un-sealed — forever, since no later operator prompt ever arrives to
        supersede it. With it, a session nobody came back to gets its final
        turn sealed by the scheduled sweep while genuinely live work (whose
        ``updated_at`` is newer than the cutoff) is never touched.
        ...
        with self.session(project_root) as conn:
            sql = (
                "SELECT turn_id, session_id, updated_at FROM causal_turns "
                "WHERE state != ?"
            )
        ...
        for cand in candidates:
            ...
            sid = str(cand["session_id"])
            if tid == current_by_session.get(sid, "") and not include_current:
                # Live work: only an IDLE current turn (updated_at at or
                # before the cutoff) may be sealed by the backstop.
                updated_at = str(cand.get("updated_at") or "")
                if not (stale_before and updated_at and updated_at <= stale_before):
