...
def register_run_tools(*, server: Any, hub: Any, runtime: Any) -> None:
    ...
    async def ai_test(
        ...
        run_env = build_test_env(fw, os.environ)
        ...
        if state == "running":
            ...
            inline_budget = float(min(governed_timeout, CLIENT_IDLE_GUARD_SECONDS))
        ...
        if state == "running":
            ...
            return {
                ...
                "args": list(argv[1:]),
                "reason": (
                    f"Suite still running after the {int(CLIENT_IDLE_GUARD_SECONDS)}s "
        ...
        if state == "running":
            ...
            try:
                ...
                if spawn_attr.get("session_id") or spawn_attr.get("agent_context_id"):
                    ...
                    managed = _mm_rp.ManagedModeService().get_mode(
                        root,
