...
MCP_TIER_OVERRIDES: dict[str, tuple[str, str, str, str]] = {
    ...
    # King 2026-06-21: per-handler security audit (NOT blanket decorator-trust) of the
    ...
    "schema_query": (TIER_R, "read_verified", "server_code_tools.py:2389 hub.schema find/get/trace pure SELECT", "read"),
    "memory_search": (TIER_R, "read_verified", "server_memory_index_tools.py:196 hub.index.search_memory SELECT memory_index", "read"),
    ...
    "ai_palace_search": (
        TIER_R,
        "read_verified",
        "server_palace_tools.py:45 hub.palace.search hybrid BM25+vector query; only "
        "side-effect _mint_audit_event (palace_hub_extension.py:162 audit-event row)",
        "read",
    ),
    "ai_palace_status": (
        TIER_R,
        "read_verified",
        "server_palace_tools.py:82 hub.palace.status health read; only side-effect "
        "_mint_audit_event (palace_hub_extension.py:162 audit-event row)",
        "read",
    ),
    ...
    "memory_read": (
        TIER_R,
        "read_verified",
        "server_memory_index_tools.py:121 gated_memory_read → hub.memory.read_memory "
        "SELECT; audit-event write only on the refused-inactive / degraded edge path "
        "(record_event status=refused), no domain mutation",
        "read",
    ),
