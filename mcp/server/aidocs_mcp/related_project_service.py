...
# #814/#755: every connect in this module goes through the canonical helper.
# `sqlite3` stays imported for OperationalError and the Connection/Row types.
...
class RelatedProjectService:
    ...
    def _init_db(self, project_root: Path) -> None:
        ...
        # #814/#755: canonical connect — journal_mode=WAL (file header),
        # synchronous, busy_timeout, foreign_keys=ON. `with` on a plain sqlite3
        # connection is a TRANSACTION manager that never closes; the helper's
        # ClosingConnection commits and then closes (#756).
    ...
    def _connect(self, project_root: Path) -> sqlite3.Connection:
        ...
        # row_factory=sqlite3.Row is the helper's default, so it is not set again here.
        return _canonical_connect(self._db_path(project_root))
    ...
    def _import_legacy_markdown_once(self, project_root: Path) -> None:
        ...
        if not entries:
            return
        now = self._now_iso()
        with _canonical_connect(self._db_path(project_root)) as conn:
        ...
        if not (target / ".MEMORY").is_dir():
            return {
                ...
                "error": (
                    ...
                    # `aidocs bootstrap` is not a command — cli.COMMANDS has
                    # `init` (law 311bf3e6: a named remedy must exist). The
                    # commissioning door is the RIGHT door here: this branch
                    # really did test "not commissioned", and registering a
                    # related project is an explicit request to manage it.
                    f"Run `aidocs init` there first."
                ),
            }
