...
# Flags whose VALUE is inert, non-path data for every command that takes them
# (templates, titles, prose bodies, HTTP header/verb/UA). Only LONG spellings:
# short flags mean different things per command (`patch -d DIR`, `cp -t DIR`,
# `python -c CODE`), so none is provably inert. Deliberately ABSENT (r0b,
# blast-radius-ax): execution-bearing `-c`/`-e` (inline scripts ARE code),
# path-bearing `-F`/`--file`/`--body-file`, and curl's `--data*` family
# (`@path` reads a file). git `-m`/`--message` is handled by the shared
# git-segment-scoped projection, not by this list (`python -m` is execution).
# Even for these flags only a PROVEN-LITERAL value word is masked: a
# double-quoted/unquoted chunk carrying `$`/backtick expands before the
# command sees it and stays visible.
_DATA_FLAGS_BY_COMMAND: dict[str, tuple[str, ...]] = {
    # SUBCOMMAND-SCOPED (r0b, blast-radius-ax round 3). `git`, `gh` and `glab`
    # are EXTENSIBLE: they dispatch external subcommands, aliases and
    # extensions that hand argv to another program, so `git <ext> --format` /
    # `gh extension exec … --body` carry whatever that program says — possibly
    # a real path. Scoping to the BASE COMMAND was therefore never a security
    # proof. Each key below names a PROVEN BUILTIN subcommand; an unknown /
    # alias / extension subcommand matches no key and masks NOTHING.
    #
    # git: `--pretty` / `--format` are output TEMPLATES on these builtins.
    "git log": ("--pretty", "--format"),
    "git show": ("--pretty", "--format"),
    "git for-each-ref": ("--format",),
    "git branch": ("--format",),
    "git tag": ("--format",),
    # gh / glab: prose fields of the create/edit builtins. `--body-file` is
    # deliberately absent — it names a path.
    "gh pr create": ("--body", "--title"),
    "gh pr edit": ("--body", "--title"),
    "gh issue create": ("--body", "--title"),
    "gh issue edit": ("--body", "--title"),
    "gh release create": ("--notes", "--title"),
    "gh repo create": ("--description",),
    "glab mr create": ("--body", "--title", "--description"),
    "glab mr update": ("--body", "--title", "--description"),
    "glab issue create": ("--body", "--title", "--description"),
    "glab issue update": ("--body", "--title", "--description"),
    # curl / wget stay BINARY-GLOBAL: they dispatch no subcommands, so the
    # HTTP verb and the UA string mean the same thing for every invocation.
    # `--header` is GONE (r0b): curl documents `-H/--header <header/@file>`
    # and `@filename` reads the headers FROM A FILE, so the flag is
    # path-bearing. The `--data*` family is absent for the same reason.
    "curl": ("--request", "--user-agent"),
    "wget": ("--user-agent",),
}


def _strip_bash_quoted_regions(cmd: str) -> str:
    """The INFRASTRUCTURE-PROTECTION projection of a shell command.

    Thin delegate to the shared shell semantic projection
    (`shell_data_windows.mask_infrastructure_data_windows`) — there is ONE
    heredoc / data-window authority, not a second bespoke parser here. It
    blanks (same length) only PROVEN data windows: literal git message
    values, complete quote-delimited heredoc bodies fed to a proven inert
    reader (plus the Beat-7 ``git commit -m "$(cat <<'EOF' …)"`` form), and
    literal values of the SUBCOMMAND-SCOPED inert flags above (a flag mask
    applies only inside the proven builtin subcommand that parses it — an
    unknown git/gh/glab alias or extension masks nothing). Interpreter heredocs,
    ``-c``/
    ``-e`` scripts, path-valued flags and expansion-bearing values stay
    visible. Any parse uncertainty or error returns the RAW command: a
    masking parser may fail stricter, never looser.
    """
    try:
        from .shell_data_windows import mask_infrastructure_data_windows

        return mask_infrastructure_data_windows(cmd, _DATA_FLAGS_BY_COMMAND)
    except Exception:
        return cmd
...
_TEST_RETRY_EDIT_TOOLS: frozenset[str] = frozenset(
    {
        ...
        "ai_create_file",
        "ai_slop",
        # #1039 S4: the UNIFIED edit surfaces. `ai_replace` (Empire doctrine
        # 2026-05-01) is THE primary edit tool and was never in this list, so
        # an agent that fixed a fixture through it was told its third run
        # "repeated without resolution" — the gate had not seen the edit.
        "ai_replace",
        "ai_delete",
        "ai_deslop_apply",
        "ai_file",
...
def record_test_retry_unlock(
    hub: Any,
    project_root: Path,
    session_id: str,
    *,
    reason: str = "ai_failures:fixed",
    lane_id: str = "",
) -> None:
    """Unlock ONE rerun window for the session's test-retry counter (#1039 S4).

    The counter resets on an edit; this is the OTHER door — the agent's
    explicit ``ai_failures(mode='fixed'|'autoclear')`` claim that the failure
    is resolved. A fix made outside the edit tools (a generated fixture, a
    data file, a dependency) still opens the door. Same marker the edit path
    writes (``test_retry_reset``, scoped to session + lane, payload shape
    identical so ``_count_test_invocations_since_reset`` matches it); the
    claim's provenance rides on ``target_entity``. It grants the same two-run
    budget an edit grants — never a standing bypass.
    """
    ...
    try:
        _log_record(
            ...
            target_entity=reason,
        )
    except Exception:
        pass


def authenticated_actor_lane(project_root: Path | str) -> str:
    """The LANE axis of every test-retry identity: the retry counter, the reset
    marker, and a native/MCP edit witness (r0b on 6f90729b7).

    Sourced from the caller's AUTHENTICATED identity
    (``task_actor_identity.resolve_task_actor``: spawn env stamp
    AIDOCS_EXPERT_LANE_ID / subagent principal / #217 registry / #457
    derivation), NEVER from ``query_gate.current_lane_id`` — that column is
    SESSION-SHARED routing state which a sibling lane's session_connect clobbers
    between one invocation's separate hook moments (PreToolUse, allow->deny
    abort, PostToolUse[Failure]). Each hook subprocess of one invocation
    inherits the same worker env, so this answer is stable across them.
    Non-worker/conductor -> "". Unresolvable -> "" (never a guess)."""
    try:
        from .task_actor_identity import resolve_task_actor

        _actor, lane_id, is_worker = resolve_task_actor(project_root)
        return str(lane_id or "").strip() if is_worker else ""
    except Exception:
        return ""


def _bare_tool_name(tool_name: object) -> str:
    """'mcp__aidocs__ai_replace' -> 'ai_replace'; unprefixed names unchanged."""
...
def _test_output_passed(output: str) -> bool:
    """True only when the LAST pytest summary line reports passes and no
    failure/error. Absent a summary line the outcome is unknown -> False."""
    import re as _re

    summary = _re.compile(r"\b\d+ (passed|failed|errors?|xfailed|xpassed|skipped|deselected)\b")
...
def record_test_run_outcome(
    ...
    *,
    ...
    """PostToolUse door: a verification that PASSED resolved its signal, so the
    next run of it is a fresh verification, not a blind retry. Writes the same
    reset marker an edit writes. Returns True when the unlock was recorded."""
    if not _is_test_runner_invocation(command or ""):
        return False
    if not _test_output_passed(output or ""):
        return False
    record_test_retry_unlock(
        hub, project_root, session_id, reason="test_run:passed", lane_id=lane_id
    )
    return True


def _edit_response_succeeded(tool_response: object, tool_name: str = "") -> bool:
    """Response-side precondition: a proven success that is neither a dry run
    nor an explicit zero-delta, per the ONE shared classifier. Not sufficient to
    unlock on its own - see edit_mutation_witnessed."""
    from .tool_response_classifier import edit_unlocks

    return edit_unlocks(tool_response, tool_name)


# ── Mutation witness (r0b on d90e803db, 43e75aa62) ───────────────────────────
# A successful edit is not a change: str_replace(old == new), an identical
# anchor/lines/symbol body, an all-unchanged batch all report ok:true and write
# the same bytes. The retry budget may only reset when THIS invocation's target
# bytes changed.
#
# INVOCATION-CORRELATED. PreToolUse snapshots the sha256 of every path the call
# names as `edit_preimage` rows carrying the host's tool_use_id and the lane.
# PostToolUse (record_edit_outcome) looks up exactly that invocation's rows -
# never "latest for the path" - hashes the paths again, records an
# `edit_postimage` row, and credits the call only when:
#   * pre != post for some path, AND
#   * no OTHER invocation (any session/lane) COMPLETED (edit_postimage) on that
#     path inside this call's window (between its preimage and postimage rows).
#     If one did, the bytes may be that call's work, not ours -> fail closed.
# No tool_use_id, no preimage for it, or equal hashes -> no unlock.
# Accepted residual: a mutation by a process AIDOCS does not observe (an editor,
# a script) inside the window is indistinguishable from this call's write.
_EDIT_PATH_KEYS = (
    "path",
    "file_path",
    "filePath",
...
def _current_invocation_id() -> str:
    """The barrier's id for the tool call this code is running inside, or ''."""
    try:
        from .branch_transition_barrier import current_tool_invocation_id

        return str(current_tool_invocation_id() or "")
    except Exception:
        return ""


def _restore_destination(hub: Any, project_root: Path, trash_id: str) -> str:
    """ai_delete/ai_file restore without an explicit path: the destination the
    restore itself will use - the audited `file_deleted` event for trash_id."""
    try:
        from .file_delete_service import FileDeleteService

        # ONE authority: the same lookup the restore itself uses.
        return str(
            FileDeleteService(hub)._find_delete_event_by_trash_id(
                Path(project_root).resolve(), trash_id
            )
            or ""
        )
    except Exception:
...
def _edit_target_paths(tool_input: object, hub: Any = None, project_root: Path | None = None) -> list[str]:
    if not isinstance(tool_input, dict):
        return []
    found: list[str] = []
    items: list[dict] = [tool_input]
    edits = tool_input.get("edits")
    if isinstance(edits, list):
        items.extend(e for e in edits if isinstance(e, dict))
    for item in items:
        for k in _EDIT_PATH_KEYS:
            v = item.get(k)
            if isinstance(v, str) and v.strip() and v.strip() not in found:
                found.append(v.strip())
    trash_id = tool_input.get("trash_id")
    if not found and isinstance(trash_id, str) and trash_id.strip() and project_root is not None:
        dest = _restore_destination(hub, project_root, trash_id.strip())
        if dest:
            found.append(dest)
    return found


def _edit_real_path(project_root: Path, raw: str) -> str:
    """Absolute path with its REAL case preserved (it is what gets hashed)."""
    import os

    p = Path(raw)
    full = p if p.is_absolute() else Path(project_root) / p
    return os.path.abspath(str(full))


def _edit_path_key(project_root: Path, raw: str) -> str:
    """Comparison identity. os.path.normcase folds case ONLY where the platform
    filesystem is case-insensitive (Windows); on POSIX it is the identity, so
    CaseFile.py and casefile.py stay different paths (r0b 385c21a5)."""
    import os

    return os.path.normcase(_edit_real_path(project_root, raw)).replace("\\", "/")


def _file_sha(real_path: str) -> str:
    import hashlib

    try:
        return hashlib.sha256(Path(real_path).read_bytes()).hexdigest()
...
def record_edit_preimage(
    ...
    *,
    ...
    """PreToolUse: snapshot the byte hash of every path this invocation names.
    Requires the host tool_use_id (no id -> nothing recorded, no unlock later)."""
    if not session_id or not tool_use_id or _bare_tool_name(tool_name) not in _TEST_RETRY_EDIT_TOOLS:
        return 0
    n = 0
    for raw in _edit_target_paths(tool_input, hub, project_root):
        real = _edit_real_path(project_root, raw)
        sha = _file_sha(real)
        if not sha:
            continue
        try:
            hub.execution.record_event(
                project_root,
                event_kind="edit_preimage",
                source_kind="pre_tool_use",
                session_id=session_id,
                capability_name=_bare_tool_name(tool_name),
                action_kind="edit",
                target_entity=_edit_path_key(project_root, raw),
                status="observed",
                payload={
                    ...
                    "real_path": real,
                    "sha": sha,
                    # Needed to ask the lease authority about THIS invocation:
                    # the native lease id is keyed by (host_session_id,
                    # tool_use_id) and the tool name decides which lease it is.
                    "host_session_id": host_session_id or "",
                    "tool_name": str(tool_name or ""),
                    # The barrier's tool-invocation id when this mint happens
                    # INSIDE a tool call (mcp_tool:/mcp_worker: leases carry the
                    # same id). '' from a host hook process or the outer-gate
                    # dispatch, where no tool_call_lease is in scope — those
...
# ── EXACT invocation identity (r0b on 2901ee338) ─────────────────────────────
# A witness is named by (session, lane, tool_use_id, host_session_id). A NATIVE
# tool_use_id is host/session-scoped: two host sessions bound to the SAME managed
# session and lane may legitimately reuse it, so (session, lane, tool_use_id)
# names nothing for a native call. An IN-PROCESS witness ("mcp:" id minted by
# open_mcp_edit_witness from a fresh uuid) carries no host axis — its id is
# globally unique by construction, so its host_session_id is legitimately ''.
#
# FAIL CLOSED: a NATIVE witness without host_session_id is identity-incomplete.
# It is still minted (it must keep blocking others: its bytes are real), but
# nothing may ever consume, credit, close or dispose it by the partial tuple -
# edit_invocation_mutated and abort_edit_witness refuse it, and
# _invocation_may_still_write keeps it blocking. Rows written before this change
# (native preimage with no host, postimage/disposal with no pre_rowid) fall
# under the same rule: never matched, never waived.
#
# Every postimage and every disposition persists the EXACT pre_rowid it closes,
# so closure and delayed-bookkeeping lookups follow that link and never
# "recover the latest matching preimage".


def _is_in_process_id(tool_use_id: str) -> bool:
    return str(tool_use_id or "").startswith("mcp:")


def _identity_complete(tool_use_id: str, host_session_id: str) -> bool:
    if not tool_use_id:
        return False
    if _is_in_process_id(tool_use_id):
        return True
    return bool(host_session_id)


def abort_edit_witness(
    hub: Any,
    project_root: Path,
    session_id: str,
    tool_use_id: str,
    ...
    *,
    ...
    """Close this exact (session, lane, tool_use_id, host_session_id) witness
    because the call will NOT run — a pre-execution authority after the gate's
    allow refused it.

    Recorded as an explicit disposition (never a silent delete), same row shape
    as an expiry. Returns how many preimages were disposed of. Identity-
    incomplete native call (no host_session_id) -> disposes NOTHING (0): the
    partial tuple could name another host's live witness (r0b on 2901ee338).
    """
    # EXPLICIT SYNCHRONOUS DISPOSAL, not terminal evidence (r0b on c3dbec75a):
    # allow->deny means the tool never ran, so the exact preimages are disposed
    # here and now. No synthetic host-native receipt is minted.
    if not session_id or not _identity_complete(tool_use_id, host_session_id):
        return 0
    n = 0
    for pre_rowid, key, _real, _sha in _invocation_preimages(
        hub, project_root, session_id, tool_use_id, lane_id, host_session_id=host_session_id
    ):
        _dispose_stale_preimage(
            hub,
            project_root,
            key=key,
            session_id=session_id,
...
def _invocation_preimages(
    ...
    *,
    ...
    """(rowid, key, real_path, sha) for exactly this invocation's preimages.
    FULL identity incl. host_session_id; identity-incomplete native -> []."""
    import json as _json

    if not _identity_complete(tool_use_id, host_session_id):
        return []
    ...
    try:
        ...
        with hub.execution.connect(project_root) as conn:
            rows = conn.execute(
                "SELECT rowid, target_entity, payload_json FROM execution_events "
                "WHERE event_kind = 'edit_preimage' AND session_id = ? "
                "AND json_extract(payload_json, '$.tool_use_id') = ? "
                "AND COALESCE(json_extract(payload_json, '$.lane_id'), '') = ? "
                "AND COALESCE(json_extract(payload_json, '$.host_session_id'), '') = ? "
                "ORDER BY rowid ASC",
                (session_id, tool_use_id, lane_id or "", host_session_id or ""),
            ).fetchall()
        seen: set[str] = set()
        for r in rows:
            key = str(r[1] or "")
            if not key or key in seen:
                continue
            seen.add(key)
            p = _json.loads(r[2] or "{}") or {}
            out.append((int(r[0]), key, str(p.get("real_path") or ""), str(p.get("sha") or "")))
    except Exception:
        return []
    return out
...
def _record_postimage(
    ...
    *,
    ...
    """Persist this invocation's postimage, linked to the EXACT preimage row it
    closes (pre_rowid). Returns the new row's rowid, 0 on failure / no link."""
    if not pre_rowid:
        return 0
    try:
        hub.execution.record_event(
            project_root,
            event_kind="edit_postimage",
            ...
            payload={
                ...
                "lane_id": lane_id or "",
                "host_session_id": host_session_id or "",
                "pre_rowid": int(pre_rowid),
                "sha": sha,
            },
        )
        with hub.execution.connect(project_root) as conn:
            # EXACT link (r0b on 2901ee338): the postimage names the preimage
            # row it closes; no (session, lane, tool_use_id) recovery.
            row = conn.execute(
                "SELECT MAX(rowid) FROM execution_events WHERE event_kind = 'edit_postimage' "
                "AND target_entity = ? "
                "AND json_extract(payload_json, '$.pre_rowid') = ?",
                (key, int(pre_rowid)),
            ).fetchone()
...
# Recorded as ``ttl_seconds`` on an explicit ``edit_preimage_expired`` row for
# the IN-PROCESS witness path only. It is NOT a quiescence rule: age proves
# nothing about whether an invocation can still write (r0b on 1444c6ec7). A
# HOST-NATIVE witness is retired ONLY by positive terminal evidence for its
# exact native invocation — an ordinary PostToolUse (success OR failure), a
# PostToolUseFailure receipt, or an explicit allow->deny abort of its exact
# pre_rowid. A native witness whose completion was lost KEEPS BLOCKING that
# path; no clock and no lease disappearance ever releases it.
_EDIT_PREIMAGE_TTL_SECONDS = 15 * 60


def _event_age_seconds(hub: Any, observed_at: str) -> float | None:
    """Age measured on the STORE's clock (the same one that stamped the row),
    so the answer stays consistent wherever that clock comes from."""
    from datetime import datetime, timezone

    def _parse(text: object) -> Any:
        try:
            return datetime.strptime(str(text), "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        except Exception:
            return None

    then = _parse(observed_at)
    if then is None:
        return None
    now = None
    try:
        stamp = getattr(hub.execution, "_timestamp", None)
        if callable(stamp):
            now = _parse(stamp())
    except Exception:
        now = None
...
def _dispose_stale_preimage(
    ...
    *,
    ...
    """Record an explicit disposition for an abandoned preimage.

    Evidence is never deleted: the preimage row stays and an
    ``edit_preimage_expired`` row is written beside it naming the identity, the
    reason and the age. The overlap check treats a disposed preimage as closed.
    """
    try:
        with hub.execution.connect(project_root) as conn:
            seen = conn.execute(
                "SELECT 1 FROM execution_events WHERE event_kind = 'edit_preimage_expired' "
                "AND target_entity = ? "
                "AND COALESCE(json_extract(payload_json, '$.pre_rowid'), -1) = ? LIMIT 1",
                (key, int(pre_rowid)),
            ).fetchone()
        if seen is not None:
            return
        hub.execution.record_event(
            project_root,
            event_kind="edit_preimage_expired",
            source_kind="test_retry_witness",
            session_id=session_id or None,
            action_kind="edit",
            target_entity=key,
            status="expired",
            payload={
                ...
                "ttl_seconds": _EDIT_PREIMAGE_TTL_SECONDS,
            },
        )
    except Exception:
        pass
...
# NO LIVENESS RUNG (r0b on da2458265). The first cut inferred "that lane is
# finished" from query_gate.current_lane_id — but that column is SESSION-SHARED
# ROUTING STATE, not ownership: concurrent lane workers share a session and each
# session_connect(lane_id=...) clobbers it (tests/security/
# test_worker_lane_env_isolation.py, test_war5_identity_consumers.py), so a
# still-running worker would have been declared closed and its overlap dropped.
#
# AGE IS NOT QUIESCENCE EITHER (r0b on 1444c6ec7): a live invocation stalled
# past any wall-clock bound can still write. So an open preimage BLOCKS only
# while the project's own mutation-lease authority says that invocation could
# still write, and is disposed of only when that authority says it could not:
#
#   * branch_transition_barrier.live_mutation_leases() is the authority. Those
#     rows are heartbeat-RENEWED while a mutation runs (mutation_lease,
#     MUTATION_TTL_S) and reaped by pid liveness (_is_stale -> holder_state), so
#     a long-running edit keeps its lease and keeps blocking.
#   * A host-native edit (Edit/Write/...) holds a lease keyed deterministically
#     by (host_session_id, tool_use_id) — native_lease_id — registered before
#     PreToolUse returns allow and released on PostToolUse/PostToolUseFailure.
#     Present -> may still write. Gone -> PROVES NOTHING: a native lease has
#     no pid and no heartbeat, so absence/TTL is not quiescence. A native
#     witness is retired ONLY by a terminal receipt for its exact invocation
#     (PostToolUse of any status, PostToolUseFailure, or an allow->deny abort
#     of its exact pre_rowid); without one it keeps blocking.
#   * An in-process AIDOCS tool call holds tool_call_lease for the whole call
#     ("mcp_tool:" owner). Any such live lease keeps in-process witnesses
#     blocking, because one of them may be this invocation.
#   * A just-minted witness gets a short grace: the mint happens microseconds
#     before the lease is registered, and that race must not read as "never ran".
#   * If the lease store cannot answer, NOTHING is disposed — the path stays
#     blocked. Unknown is never permission.
#
# A NATIVE invocation minted but never executed (a later pre-execution
# authority refused it after the gate's allow) is closed by that refusal's
# explicit abort of its exact pre_rowid — not by the absence of a lease. The
# abort's identity (session, lane, tool_use_id, host session) must be sourced
# exactly as the mint's, lane from the caller's authenticated identity
# (LifecycleService._witness_actor_lane), or it disposes nothing and the
# witness blocks indefinitely.
_WITNESS_LEASE_GRACE_SECONDS = 10.0


class _LeaseView:
    """What the mutation-lease authority currently says."""

    __slots__ = ("lease_ids", "invocation_ids", "in_process_live", "answered")

    def __init__(self, lease_ids=(), invocation_ids=(), in_process_live=False, answered=True):
        self.lease_ids = set(lease_ids)
        self.invocation_ids = set(invocation_ids)
        self.in_process_live = bool(in_process_live)
        self.answered = bool(answered)


def _live_lease_view(project_root: Path) -> _LeaseView:
    try:
        from .branch_transition_barrier import lease_invocation_id, live_mutation_leases

        rows = live_mutation_leases(project_root) or []
    except Exception:
        return _LeaseView(answered=False)
    view = _LeaseView()
    for r in rows:
        try:
            owner = str(r.get("owner") or "")
            view.lease_ids.add(str(r.get("lease_id") or ""))
            inv = lease_invocation_id(owner)
            if inv:
                view.invocation_ids.add(inv)
            if "mcp_tool:" in owner:
                view.in_process_live = True
        except Exception:
            continue
    return view


def native_invocation_key(host_session_id: str, tool_use_id: str, tool_name: str) -> str:
    """The canonical native-invocation identity: the same key the native lease
    uses. ``native_lease_id``'s basis is the tool_use_id when present (input is
    only folded in when the host gives no id), so a hook-path witness — which
    always has one — needs no tool_input."""
    try:
        from .branch_transition_barrier import native_lease_id

        if not (host_session_id and tool_use_id):
            return ""
        return native_lease_id(host_session_id, tool_use_id, tool_name, None)
    except Exception:
        return ""


def record_edit_terminal(
    hub: Any,
    project_root: Path,
    session_id: str,
    tool_use_id: str,
    ...
    *,
    host_session_id: str = "",
    tool_name: str = "",
    reason: str = "post_tool_use",
) -> bool:
    """POSITIVE terminal evidence that this invocation is over (r0b on
    7744a5583): a matched PostToolUse / PostToolUseFailure. (An allow->deny
    abort is NOT recorded here - abort_edit_witness disposes the exact
    preimages directly.) A receipt proves only "cannot write after this row";
    readers must compare its rowid against their window. A native witness is retired ONLY on such a receipt - a
    native lease that merely vanished by TTL proves nothing, because those rows
    carry no pid and no heartbeat.

    Returns True ONLY when a receipt keyed by the EXACT native invocation
    identity was persisted. Identity-incomplete (no host_session_id or no
    tool_use_id) -> writes NOTHING, returns False: a receipt without the host
    axis could only be matched by the bare tool_use_id, which is
    host/session-scoped and so identifies nothing (r0b on 161f1e621). Any store
    exception -> False. Never raises."""
    native_key = native_invocation_key(host_session_id, tool_use_id, tool_name)
    if not native_key:
        return False
    try:
        hub.execution.record_event(
            project_root,
            event_kind="edit_invocation_terminal",
            source_kind="test_retry_witness",
            session_id=session_id or None,
            action_kind="edit",
            target_entity=native_key,
            status="terminal",
            payload={
                "tool_use_id": tool_use_id,
                "lane_id": lane_id or "",
...
def _terminal_receipt_rowid(
    hub: Any, project_root: Path, *, native_key: str, other_pre_rowid: int
) -> int | None:
    """WHEN did EXACTLY this native invocation become terminal (r0b on c3dbec75a)?

    Returns the rowid of the EARLIEST terminal receipt keyed by ``native_key``
    that lands strictly AFTER this witness's own preimage (``other_pre_rowid``);
    0 when there is none; None when the store cannot answer.

    A receipt proves only "cannot write AFTER this row", so identity alone is
    not enough - the caller compares this rowid against its own window.

    Matches on ``target_entity = native_key`` ONLY - no ``(tool_use_id,
    session_id)`` fallback: tool_use_id is host/session-scoped and a receipt for
    host session B must never retire host session A's witness (r0b on 161f1e621).

    The ``rowid > other_pre_rowid`` bound: a receipt at or before the witness's
    own preimage belongs to an older (or reused) identity and says nothing about
    this invocation. EARLIEST, not latest: the first receipt after the preimage
    is the moment this invocation stopped being able to write. Any later receipt
    for the same key can only come from a reused id; taking MAX would push the
    termination point later than the truth (spurious overlap), and nothing can
    make it earlier than the first real receipt. MIN is the tightest provable
    termination point.

    Empty native_key (identity-incomplete witness) -> 0: no receipt can prove
    such an invocation over, so it keeps blocking."""
    if not native_key:
        return 0
    try:
        with hub.execution.connect(project_root) as conn:
            row = conn.execute(
                ...
                "WHERE event_kind = 'edit_invocation_terminal' "
                "AND target_entity = ? AND rowid > ?",
                (native_key, int(other_pre_rowid)),
            ).fetchone()
        return int(row[0]) if row and row[0] is not None else 0
    except Exception:
        return None
...
def _invocation_may_still_write(
    project_root: Path,
    *,
    host_session_id: str,
    tool_use_id: str,
    ...
    terminal_rowid: int | None = None,
    window_lo_rowid: int = 0,
) -> bool:
    """Can this open invocation still write? Unknown -> True (fail closed).

    EXACT where the evidence allows it: a witness stamped with the barrier's
    tool-invocation id (branch_transition_barrier.current_tool_invocation_id,
    delivered 4738dc5b7) matches only the live lease carrying that same id, so
    two concurrent in-process invocations no longer block each other.
    CONSERVATIVE only where the id is genuinely absent — a witness minted before
    that field existed, or one minted outside a tool call (a host PreToolUse
    hook runs in its own process, and the outer-gate dispatch does not enter
    tool_call_lease) — where any live in-process lease still blocks.
    """
    if not lease_view.answered:
        return True
    if age_seconds is not None and age_seconds < _WITNESS_LEASE_GRACE_SECONDS:
        return True  # mint -> lease registration race
    if invocation_id:
        # In-process (gate dispatch / MCP boundary): the lease carrying the same
        # id IS the invocation. Both leases of one invocation (mcp_tool: and
        # mcp_worker: / gate_dispatch) carry it, and those rows are pid-backed
        # and heartbeat-renewed, so their absence is real evidence.
        return invocation_id in lease_view.invocation_ids
    if tool_use_id and not _is_in_process_id(tool_use_id) and not host_session_id:
        # Identity-incomplete NATIVE witness (r0b on 2901ee338): no exact
        # receipt can ever prove it over, and the in-process lease view says
        # nothing about a host call -> it keeps blocking.
        return True
    if host_session_id and tool_use_id and not tool_use_id.startswith("mcp:"):
        # HOST-NATIVE identity. Native lease rows carry no pid and no heartbeat,
        # so "the lease is gone" may mean released OR merely expired. Only a
        # POSITIVE terminal receipt keyed by this same native identity retires
        # the witness; unknown keeps blocking (r0b on 7744a5583).
        # CHRONOLOGY (r0b on c3dbec75a): a receipt proves only "cannot write
        # after this row". It retires the witness relative to the asking window
        # ONLY when other_pre_rowid < terminal_rowid <= window_lo_rowid (the
        # lower bound is enforced by _terminal_receipt_rowid's SQL). A receipt
        # landing after the window opened means the invocation was live inside
        # it -> may have written -> overlap. None/0 -> keep blocking.
        if not terminal_rowid:
            return True
        return int(terminal_rowid) > int(window_lo_rowid)
    return lease_view.in_process_live


def _terminal_proven_before_window(
    hub: Any,
    project_root: Path,
    conn: Any,
    key: str,
    ...
    """True ONLY when the exact native terminal receipt of the invocation that
    wrote postimage ``post_rowid`` proves ``other_pre < terminal <= lo_rowid``
    (r0b on d078ec740: postimage row time is not completion time once the
    receipt is written first).

    EXACT LINK (r0b on 2901ee338): the postimage names the preimage row it
    closes (``pre_rowid``); that row - and only that row - supplies the native
    identity. No "latest matching preimage" recovery, so a same-(S,L,X)
    preimage from ANOTHER host session can never lend its identity. Anything
    unproven -> False (overlap stands): no/legacy link, link to a row that is
    not this path's preimage, host/tool id disagreeing between the two rows,
    in-process invocation (invocation_id / "mcp:" id), identity-incomplete
    native key, no receipt, store unknown (None), or a receipt after our
    window opened."""
    import json as _json
    ...
    try:
        post = conn.execute(
            ...
            "WHERE rowid = ? AND event_kind = 'edit_postimage' AND target_entity = ?",
            (int(post_rowid), key),
        ).fetchone()
        if post is None:
            return False
        post_payload = _json.loads(post[0] or "{}") or {}
        other_pre = int(post_payload.get("pre_rowid") or 0)
        if other_pre <= 0 or other_pre >= int(post_rowid):
            return False
        row = conn.execute(
            "SELECT payload_json FROM execution_events "
            "WHERE rowid = ? AND event_kind = 'edit_preimage' AND target_entity = ?",
            (other_pre, key),
        ).fetchone()
        if row is None:
            return False
        payload = _json.loads(row[0] or "{}") or {}
        if str(payload.get("invocation_id") or ""):
            return False
        tuid = str(payload.get("tool_use_id") or "")
        host = str(payload.get("host_session_id") or "")
        if not tuid or _is_in_process_id(tuid) or not host:
            return False
        if str(post_payload.get("tool_use_id") or "") != tuid or str(
            post_payload.get("host_session_id") or ""
        ) != host:
            return False
        native_key = native_invocation_key(host, tuid, str(payload.get("tool_name") or ""))
        if not native_key:
            return False
        terminal = _terminal_receipt_rowid(
            hub, project_root, native_key=native_key, other_pre_rowid=other_pre
        )
        if not terminal:  # 0 (none) or None (unknown)
            return False
        return other_pre < int(terminal) <= int(lo_rowid)
    except Exception:
        return False


def _other_completion_in_window(
    hub: Any,
    project_root: Path,
    key: str,
    tool_use_id: str,
    ...
    *,
    ...
    """True when ANOTHER invocation on this path overlapped our witness window.

    An invocation is (session_id, lane_id, tool_use_id, host_session_id) — never
    the bare host id, which two sessions or lanes may reuse (r0b on 1a8c69281),
    and never without the host axis, which two host sessions bound to one
    managed session+lane may reuse (r0b on 2901ee338). Our own rows are the
    preimage ``lo_rowid`` and postimages linked to it; closure of another
    preimage is its postimage linked by ``pre_rowid``.

    Counting only COMPLETED others miscredits on reverse completion order (B
    opens, A opens, A writes, B's no-op completes FIRST and sees A's bytes), so
    IN-FLIGHT work counts too: another invocation whose preimage opened before
    our postimage and which had not closed before our preimage is concurrent
    with us and the bytes cannot be attributed. Concurrent same-path edits
    therefore never unlock — fail closed, on both sides.

    An open preimage is only concurrent while it could still be RUNNING: one
    the lease authority proves cannot write is disposed of explicitly (an
    ``edit_preimage_expired`` row) and stops blocking.
    """
    import json as _json

    if not _identity_complete(tool_use_id, host_session_id):
        return True  # identity-incomplete self: cannot tell self from other
    me = (session_id or "", lane_id or "", tool_use_id, host_session_id or "")
    lease_view: _LeaseView | None = None
    try:
        with hub.execution.connect(project_root) as conn:
            # Another invocation COMPLETED inside our window.
            done = conn.execute(
                "SELECT COALESCE(session_id, ''), "
                "COALESCE(json_extract(payload_json, '$.lane_id'), ''), "
                "COALESCE(json_extract(payload_json, '$.tool_use_id'), ''), "
                "COALESCE(json_extract(payload_json, '$.host_session_id'), ''), "
                "COALESCE(json_extract(payload_json, '$.pre_rowid'), 0), rowid "
                "FROM execution_events WHERE event_kind = 'edit_postimage' "
                "AND target_entity = ? AND rowid > ? AND rowid < ?",
                (key, lo_rowid, hi_rowid),
            ).fetchall()
            for r in done:
                other_done = tuple(str(c) for c in r[:4])
                if other_done == me and int(r[4] or 0) == int(lo_rowid):
                    continue  # our own postimage, linked to OUR preimage
                # DELAYED BOOKKEEPING (r0b on d078ec740): an ordinary
                # PostToolUse records the terminal receipt FIRST and the
                # postimage after, so a postimage row inside our window is not
                # proof the invocation wrote inside it. Waived ONLY when the
                # exact native receipt proves other_pre < terminal <= lo.
                if _terminal_proven_before_window(
                    hub, project_root, conn, key, int(r[5]), int(lo_rowid)
                ):
                    continue
                return True
            # Another invocation IN FLIGHT across our window.
            rows = conn.execute(
                "SELECT rowid, observed_at, COALESCE(session_id, ''), payload_json "
                "FROM execution_events WHERE event_kind = 'edit_preimage' "
                "AND target_entity = ? AND rowid < ?",
                (key, hi_rowid),
            ).fetchall()
            expired = {
                ...
                for r in conn.execute(
                    "SELECT COALESCE(json_extract(payload_json, '$.pre_rowid'), -1) "
                    "FROM execution_events WHERE event_kind = 'edit_preimage_expired' "
                    "AND target_entity = ?",
                    (key,),
                ).fetchall()
            }
            stale: list[tuple[int, str, str, str, str, float | None]] = []
            overlap = False
            for pre_rowid, observed_at, other_session, payload_json in rows:
                payload = _json.loads(payload_json or "{}") or {}
                other = (
                    str(other_session or ""),
                    str(payload.get("lane_id") or ""),
                    str(payload.get("tool_use_id") or ""),
                    str(payload.get("host_session_id") or ""),
                )
                if int(pre_rowid) == int(lo_rowid) or int(pre_rowid) in expired:
                    continue  # our own preimage (exact row), or explicitly disposed
                # Any OTHER preimage row - even one carrying our full identity
                # (a reused id) - is only harmless if its linked postimage
                # closed it before our window opened.
                closed = conn.execute(
                    "SELECT MIN(rowid) FROM execution_events "
                    "WHERE event_kind = 'edit_postimage' AND target_entity = ? "
                    "AND json_extract(payload_json, '$.pre_rowid') = ? "
                    "AND rowid > ?",
                    (key, int(pre_rowid), int(pre_rowid)),
                ).fetchone()
                closed_rowid = closed[0] if closed and closed[0] is not None else None
                if closed_rowid is not None and int(closed_rowid) <= lo_rowid:
                    continue  # closed before our window opened
                if closed_rowid is not None and _terminal_proven_before_window(
                    hub, project_root, conn, key, int(closed_rowid), int(lo_rowid)
                ):
                    continue  # terminal before our window; postimage is late bookkeeping
                if closed_rowid is None:
                    # Still open. The mutation-lease authority — not the clock —
                    # decides whether it could still write.
                    age = _event_age_seconds(hub, observed_at)
                    if lease_view is None:
                        lease_view = _live_lease_view(project_root)
                    if not _invocation_may_still_write(
                        project_root,
                        host_session_id=other[3],
                        tool_use_id=other[2],
                        tool_name=str(payload.get("tool_name") or ""),
                        invocation_id=str(payload.get("invocation_id") or ""),
                        age_seconds=age,
                        lease_view=lease_view,
                        terminal_rowid=_terminal_receipt_rowid(
                            hub,
                            project_root,
                            native_key=native_invocation_key(
                                other[3], other[2], str(payload.get("tool_name") or "")
                            ),
                            other_pre_rowid=int(pre_rowid),
                        ),
                        window_lo_rowid=int(lo_rowid),
                    ):
                        stale.append((int(pre_rowid), *other, age))
                        continue
                overlap = True  # noqa: E501 - unknown is not quiescence
                break
    except Exception:
        return True  # cannot prove exclusivity -> fail closed
    for pre_rowid, s_id, l_id, t_id, h_id, age in stale:
        _dispose_stale_preimage(
            hub,
            project_root,
            key=key,
            ...
            lane_id=l_id,
            tool_use_id=t_id,
            host_session_id=h_id,
            pre_rowid=pre_rowid,
            reason="no_live_write_lease",
            age_seconds=age,
        )
    return overlap


def edit_invocation_mutated(
    hub: Any,
    project_root: Path,
    session_id: str,
    tool_use_id: str,
    ...
    *,
    ...
    """True only when THIS invocation's own window shows a byte change on a path
    it named that no other invocation can account for. Identity-incomplete
    NATIVE call (no host_session_id) -> False, touching nothing: it must never
    consume or close a witness by the partial tuple (r0b on 2901ee338)."""
    if not session_id or not _identity_complete(tool_use_id, host_session_id):
        return False
    credited = False
    for pre_rowid, key, real, pre_sha in _invocation_preimages(
        hub, project_root, session_id, tool_use_id, lane_id, host_session_id=host_session_id
    ):
        post_sha = _file_sha(real) if real else ""
        post_rowid = _record_postimage(
            hub,
            project_root,
            session_id,
            tool_use_id,
        ...
        if credited or not post_sha or not pre_sha or post_sha == pre_sha or not post_rowid:
            continue
        if _other_completion_in_window(
            hub,
            project_root,
            key,
            tool_use_id,
...
# ── MCP-owned invocation witness (r0b finding 1 on 4cb62aa1b) ────────────────
# The direct MCP / outer-gate / WebMCP path has no host PreToolUse or
# PostToolUse callback, so it carries no host tool_use_id and could never mint
# the witness: a real successful edit there left the burned retry budget shut.
# The MCP tool boundary therefore mints its OWN canonical invocation id and
# takes the same pre/post byte hashes, feeding the SAME unlock authority. The
# NOTE for future adapters: only hook_pipeline.decide_tool_use (allow branch)
# and this function mint a witness. host_adapter_cli._handle_pretool and
# openai_agents_adapter.on_tool_start call ToolGate.evaluate_tool directly and
# mint nothing, so a refusal there has nothing to dispose. If either ever routes
# through decide_tool_use, its refusal branch needs the same
# hook_pipeline.abort_edit_witness_after_allow(...) call claude_hook makes.
#
# The host-hook path keeps using the host's tool_use_id; these ids are namespaced
# ("mcp:") so the two can never be confused, and the open/close pair is
# per-call state, so a hook-driven call that also passes here cannot be
# credited twice (the reset marker is idempotent per session+lane anyway).
import contextvars as _cv_edit

_mcp_edit_invocation: _cv_edit.ContextVar[dict | None] = _cv_edit.ContextVar(
    "aidocs_mcp_edit_invocation", default=None
)


def open_mcp_edit_witness(hub: Any, project_root: Path, tool_name: str, tool_input: object) -> str:
    """Server-side PreToolUse equivalent: mint an invocation id and snapshot the
    named paths' bytes. Returns the id, or "" when this is not a witnessable
    edit call. Best-effort; never raises into the tool."""
    import uuid as _uuid

    try:
        if _bare_tool_name(tool_name) not in _TEST_RETRY_EDIT_TOOLS:
            return ""
        from .managed_mode_service import resolve_managed_session

        # A request that NAMES its calling host resolves THAT conductor's
        # binding; only a genuinely identity-less (legacy/local) call may use
        # the singleton (r0b). Named-but-unbound -> "" -> no witness.
        from . import mcp_server_runtime_helpers as _rt

        calling_host = str(_rt.current_calling_host_session_id() or "").strip()
        if not calling_host and _rt._request_identity_scoped.get():
            # Identity-SCOPED request that knows no host: not identity-less, so
            # the singleton would be another actor's session. Fail closed.
            return ""
        session_id = str(
            resolve_managed_session(hub.managed_mode, project_root, host_session_id=calling_host) or ""
        ).strip()
        if not session_id:
            return ""
        # Lane from the caller's authenticated identity, never session-shared
        # query_gate routing state (r0b on 6f90729b7).
        lane_id = authenticated_actor_lane(project_root)
        inv = f"mcp:{_uuid.uuid4().hex}"
        if not record_edit_preimage(
            hub, project_root, session_id, tool_name, tool_input, tool_use_id=inv, lane_id=lane_id
        ):
            return ""
        _mcp_edit_invocation.set(
            {
                "id": inv,
                "session_id": session_id,
                "lane_id": lane_id,
...
def close_mcp_edit_witness(hub: Any, tool_response: object) -> bool:
    """Server-side PostToolUse equivalent: hash again, decide, and reset the
    retry budget when THIS invocation changed bytes. Returns True on unlock."""
    try:
        state = _mcp_edit_invocation.get()
        if not state:
            return False
        _mcp_edit_invocation.set(None)
        return record_edit_outcome(
            hub,
...
def record_edit_outcome(
    ...
    *,
    ...
    """PostToolUse door: an edit invocation that EXECUTED, SUCCEEDED and itself
    CHANGED BYTES resets the retry counter for (session, lane). A refused,
    failed, dry-run, zero-delta, uncorrelated or unwitnessed edit never does."""
    if _bare_tool_name(tool_name) not in _TEST_RETRY_EDIT_TOOLS:
        return False
    if not tool_use_id:
        return False
    # THE terminal receipt for this invocation: PostToolUse ran, so whatever it
    # was going to write, it wrote. Recorded first, so the decision below (and
    # every later overlap check) sees this invocation as over.
    record_edit_terminal(
        hub,
        project_root,
        session_id,
        tool_use_id,
    ...
    if not _edit_response_succeeded(tool_response, tool_name):
        # Still close the invocation window so it can disqualify overlaps.
        edit_invocation_mutated(
            hub, project_root, session_id, tool_use_id, lane_id, host_session_id=host_session_id
        )
        return False
    if not edit_invocation_mutated(
        hub, project_root, session_id, tool_use_id, lane_id, host_session_id=host_session_id
    ):
        return False
    record_test_retry_unlock(
        ...
        project_root,
        session_id,
        reason=f"edit:{_bare_tool_name(tool_name)}",
        lane_id=lane_id,
    )
    return True


def _named_test_paths(command: str, project_root: Path) -> list[Path]:
    """Existing filesystem paths named by the runner segment, resolved against
    a leading `cd <dir>` (git-bash /d/... spelling accepted) and project_root."""
    import re as _re
    import shlex
    ...
    bases: list[Path] = []
    for seg in _split_chained(command or ""):
        toks = seg.strip().split(None, 1)
        if len(toks) == 2 and toks[0] == "cd":
            d = toks[1].strip().strip("'\"")
            m = _re.match(r"^/([a-zA-Z])/(.*)$", d)
            if m:
                d = f"{m.group(1)}:/{m.group(2)}"
            p = Path(d)
            bases.append(p if p.is_absolute() else Path(project_root) / p)
    bases.append(Path(project_root))
    segment = _find_runner_segment(command) or ""
    try:
    ...
    for tok in tokens[1:]:
        ...
        if not ("/" in tok or "\\" in tok or tok.endswith(".py")):
            continue
        cand = Path(tok)
        for base in [Path("")] if cand.is_absolute() else bases:
            full = cand if cand.is_absolute() else base / cand
            try:
                if full.exists():
                    out.append(full)
                    break
...
def _newest_mtime(paths: list[Path], cap: int = 5000) -> float:
    newest = 0.0
    seen = 0
    for p in paths:
        try:
...
class AgentOrchestrator:
    ...
    def check_tool(
        ...
        # #253 empire §XIV: over the WebMCP gate, resolve managed mode STRICTLY per the
                ...
                if zone == PathTrustZone.UNKNOWN_EXTERNAL:
                    ...
                    if self._normalized_tool(tool_name) == "read":
                        try:
                            ...
                            _read_artifact_ok = False
                    # ai_read_jsonl exists to read Claude Code transcripts
                    # (~/.claude/projects/<slug>/*.jsonl). Least-privilege
                    # carve-out: ONLY the read-only TRANSCRIPT_READ_TOOLS, ONLY
                    # a <owned-host-uuid>.jsonl directly under THIS bound
                    # project's slug dir, canonical paths compared (no
                    # traversal/link escape). The slug is lossy, so identity is
                    # the OWNED host-uuid chain computed HERE and passed in (no
                    # usable uuid -> fail closed). Other projects, ~/.claude
                    # credentials/settings and every other tool stay refused.
                    if not _read_artifact_ok:
                        try:
                            from .host_transcript import (
                                TRANSCRIPT_READ_TOOLS,
                                is_own_project_transcript,
                            )

                            if self._normalized_tool(tool_name) in TRANSCRIPT_READ_TOOLS:
                                _read_artifact_ok = is_own_project_transcript(
                                    raw_target,
                                    project_root=project_root,
                                    owned_session_ids=self._host_session_ids(
                                        project_root,
                                        session_id_for_event,
                                    ),
                                )
        ...
        # Test-retry mutation witness preimages are NOT taken here: check_tool has
        # no host tool_use_id, and an uncorrelated snapshot can be consumed by the
        # wrong invocation (r0b 903db95a). LifecycleService.on_pre_tool_use_edit_
        # preimage records them from the PreToolUse payload instead.
        ...
        # Test-retry reset is NOT written here. check_tool runs BEFORE the
        # tool executes, so an edit attempt that is refused, fails or never
        # runs would otherwise unlock reruns (r0b, 5459d39a8). Resets come
        # only from post-execution evidence: tool_edit_completed/applied
        # (conductor, _count_test_invocations_since_reset) and
        # record_edit_outcome on PostToolUse (lane-scoped).
        # Both doors require a byte-level mutation witness; the preimage is
        # snapshotted right after session resolution above.
        ...
        if not user_granted:
            ...
            if not raw.allowed:
                ...
                # #489: forensic-only (readers: denial_tier_stats dashboards /
                # health score, worker-result summary). The strike below is its
                # own synchronous event kind and is NOT deferred.
                self._record_event_behind(
                    project_root,
                    "raw_tool_block",
                    tool_name,
                    "blocked",
        ...
        if tool_name.lower() in ("bash", "ai_run"):
            ...
            if _is_test_runner_invocation(raw_cmd):
                ...
                # The retry counter's lane axis is the CALLER's authenticated
                # lane — the same authority every reset marker and edit witness
                # is keyed by (r0b on 6f90729b7). `current_lane_id` above is
                # session-shared routing state and stays routing only.
                retry_lane = authenticated_actor_lane(project_root)
                ...
                # A named test file/dir modified AFTER the last run means the
                # rerun can produce a new answer — not a blind retry. Open a
                # fresh budget the same way an edit does.
                if prior >= 2 and self._named_tests_changed_since_last_run(
                    project_root, session_id, key, raw_cmd, lane_id=retry_lane
                ):
                    self._record_test_retry_reset(project_root, session_id, retry_lane)
                    _sec012_trace.add("test_retry_gate", "reset", reason="named_tests_modified", key=key)
                    prior = 0
                ...
                _pending_test_invocation.append((key, retry_lane))
        ...
        # Canonical shell law (#319 / doctrine VI): ai_run and every
        ...
        if judge.should_block:
            ...
            # Security-floor uncertainty: an inspection that did not finish.
            # Decided HERE, from the unfiltered set, BEFORE override thinning
            # and A/B/C — otherwise it lands in Class C, where an override
            # can thin it and operator intent can turn it into an ask. The
            # canonical taxonomy decides, so a real malicious verdict in the
            # same set still wins (and strikes).
            if any(
                v.rule_id == "INLINE_PROTECTED_PATH_INSPECTION_FAILED"
                for v in _gating_verdicts
            ):
                from . import judge_taxonomy as _jt

                _floor = _jt.evaluate_verdicts(
                    [v.rule_id for v in _gating_verdicts],
                    operator_destructive_intent=False,
                )
                if _floor.decision == _jt.DECISION_BLOCK_STRIKE:
                    _sv = self._security_strike(
                        project_root,
                        session_id,
                        "judge_malicious_forbidden",
                        tool_name,
                        _floor.triggering_rule_id,
                    )
                    return _sec012_finalize(
                        ToolDecision(
                            allowed=False,
                            reason=self._augment_reason(
                                f"Risk assessment: {_floor.reason}",
                                _sv,
                            ),
                            blocked_by="judge_malicious_forbidden",
                        ),
                    )
                # Anything else with this rule present is a security block:
                # no confirm, no strike, intent irrelevant.
                self._record_event(
                    project_root,
                    "judge_block",
                    tool_name,
                ...
                return _sec012_finalize(
                    ToolDecision(
                        ...
                        reason=(
                            "Security inspection did not complete — the command "
                            "cannot be shown safe, so it does not run. Not "
                            "confirmable; no strike recorded. Rewrite it without "
                            "the construct the inspector could not read."
                        ),
                        blocked_by="judge_security_inspection_failure",
                    ),
                )
    ...
    def _record_event_behind(
        ...
        **extra: object,
        ...
        """#489: ``_record_event`` for FORENSIC-ONLY rows no decision reads.

        Queued behind the verdict inside the warm broker (label = event_kind,
        counted); inline elsewhere. Never for strikes, freezes or grants.
        """
        from . import hook_write_behind as _hwb

        from .tool_call_log import record as _log_record

        def _write() -> None:
            # NOT via _record_event: that swallows, and a failed deferred row
            # must be COUNTED as failed by the write-behind, not as written.
            _log_record(
                self.hub,
                project_root,
                phase=event_kind,
    ...
    def _count_test_invocations_since_reset(
        ...
        try:
            ...
            with self.hub.execution.connect(project_root) as conn:
                ...
                # A governed edit the MCP edit path WITNESSED (tool_edit_completed
                # / applied) is a reset in its own right, independent of whether
                # the pre-tool hook saw the edit's tool name. Conductor scope only:
                # the row carries no lane, and one lane's edit must never reset
                # another lane's counter.
                # The tool_edit_completed/applied door is RETIRED (r0b 903db95a):
                # that row carries no tool_use_id and no pre/post bytes, so it can
                # not prove THIS invocation changed anything. Edit resets come only
                # from record_edit_outcome's invocation-correlated witness.
                row = conn.execute(
    ...
    def _named_tests_changed_since_last_run(
        self,
        project_root: Path,
        session_id: str,
        key: str,
        ...
        """True when a test path named by the command was modified after the
        most recent recorded run of this (session, lane, key). Fail-closed to
        False: no recorded run, no named path, or any error keeps the limit."""
        try:
            import json as _json
            from datetime import datetime, timezone

            payload = _json.dumps({"key": key, "lane_id": lane_id or ""}, sort_keys=True, default=str)
            self.hub.execution.init_db(project_root)
            with self.hub.execution.connect(project_root) as conn:
                row = conn.execute(
                    "SELECT observed_at FROM execution_events "
                    "WHERE event_kind = 'test_runner_invocation' "
                    "AND COALESCE(session_id, '') = COALESCE(?, '') "
                    "AND payload_json = ? ORDER BY observed_at DESC LIMIT 1",
                    (session_id or None, payload),
                ).fetchone()
            if not row:
                return False
            ts = str(row["observed_at"]).replace("Z", "+00:00")
            last_run = datetime.fromisoformat(ts)
            if last_run.tzinfo is None:
                last_run = last_run.replace(tzinfo=timezone.utc)
            paths = _named_test_paths(raw_cmd, project_root)
            if not paths:
                return False
            # observed_at is second-resolution: require the change to land in a
            # LATER second than the run, so a same-second touch never counts.
            return _newest_mtime(paths) >= last_run.timestamp() + 1.0
        except Exception:
            return False
    ...
    def _check_infrastructure(
        ...
        *,
        ...
        if is_shell:
            ...
            # Mask PROVEN data windows only (literal commit prose, inert
            # quote-delimited heredoc bodies, literal inert flag values) so
            # tools talking ABOUT a protected path don't false-positive.
            # Code (`python -c`, interpreter heredocs) and anything the
            # shared projection cannot prove stays visible (fail closed).
