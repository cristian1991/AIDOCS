...
def register_session_tools(
    *,
    ...
    def session_start(
        ...
        # stamp_host_identity import REMOVED 2026-08-21 (#859): both call sites
        # here stamped a value RECOVERED from the work-session slot, which is the
        # laundering edge this change cuts. Nothing in this module mints an
        # identity any more — it only ever passes through an identity the request
        # actually carried.
        from .mcp_server_runtime_helpers import current_calling_host_session_id

        _hsid = (current_calling_host_session_id() or "").strip()
        sid_active, _mm_reason = explain_managed_session(
            hub.managed_mode, project_root, host_session_id=_hsid
        )
        if not sid_active and _mm_reason.startswith("stale_bind:"):
            # The binding DOES name a session. This site heals-or-refuses it
            # below (ensure_member_or_heal), which is stricter than the door's
            # report-only stale verdict, so keep that path reachable.
            sid_active = _mm_reason.split(":", 1)[1]
        if sid_active:
            ...
            if sid_active:
                ...
                if not SessionMembershipStore().ensure_member_or_heal(project_root, sid_active):
                    return {
                        ...
                        "error": (
                            ...
                            f"cleared from trust. Run `ai_session(mode='connect')` with an "
                ...
                try:
                    ...
                    # RECOVERY REMOVED 2026-08-21 (#859). This recovered
                    # query_gate.get_last_host_session_id when the request
                    # carried no identity and stamped it. ai_session(connect)
                    # REFUSES that same value -- "binding a stranger is worse
                    # than binding nobody" -- and this twin bound it instead.
                    # The recovered value is keyed by the WORK SESSION, so it
                    # names whoever wrote there LAST; measured 2026-08-21 it had
                    # been frozen for three days at another window's uuid.
                    # Empty is already supported on this path (see the
                    # restamp_singleton=(not _h) below), so "" is the honest
                    # answer, not a degradation.
                    _h = _hsid
                    # #786: the session_start TWIN of ai_session(connect). It
                    # had the identical shape -- set_mode without
                    # authenticate_host, then report boot_token_restamped: true
                    # regardless of what landed. The operator's trapped agent
                    # tried exactly this alias and got a green
                    # boot_token_restamped followed by the same refusals.
                    # Fixing connect and leaving this would be the twins
                    # mistake a fifth time (#522/#774/#779/#781/#782).
                    _started = hub.managed_mode.set_mode(
                        project_root,
                        session_id=sid_active,
                    ...
                    # Only claim a restamp when a per-conductor row actually
                    # landed. `_h` may legitimately be empty here (no host
                    # identity available at all), and that case still restamps
                    # the singleton as before -- so the honesty check applies
                    # ONLY when an identity was supplied and then not bound.
                    restamped = not _h or str(
                        (_started or {}).get("resolved_via") or "",
                    ) == "per_conductor"
                except Exception:
                    pass
            return {
        ...
        if not sid:
            return {
                "ok": False,
                "active": False,
                # NAME A DOOR THAT EXISTS (law 311bf3e6). This text used to send
                # the operator to `/aidocs` — a COMMISSIONING command, the wrong
                # door entirely on a project that is already AIDOCS-managed —
                # and to `session_connect`, which stopped being an exposed tool
                # when #157 folded it into ai_session(mode=...). Both named
                # remedies were dead ends on 2026-08-16 while every gated tool
                # refused. Name only tools that are exposed AND bootstrap-exempt,
                # so the caller can actually reach them while unbound.
                "error": (
                    "No active session to auto-bind: this project has no "
                    "session with status 'active'. Use "
                    "`ai_session(mode='list')` to see them, then "
                    "`ai_session(mode='connect', session_id='<id>')` to bind "
                    "one (both work while unbound). "
                    "`ai_session(mode='create', title='<title>')` starts a new "
                    "one. If a bind keeps being refused after connect "
                    "succeeds, `admin_clear_reconnect` clears the reconnect "
                    "flags. /aidocs only COMMISSIONS a project that is not "
                    "AIDOCS-managed yet — it is not a rebind."
                ),
            }
        try:
            ...
            # RECOVERY REMOVED 2026-08-21 (#859) — and THIS is the branch that
            # mattered most. The comment below already says it: a locked-out
            # caller has no per-conductor row, so control comes STRAIGHT PAST
            # the restamp branch into this one, and Claude Code fires
            # session_start on EVERY /mcp reconnect, so it ran UNATTENDED.
            # Recovering here meant an unidentified caller was silently bound
            # under whatever window wrote the work-session slot last — measured
            # 2026-08-21 as an id frozen three days earlier on a DIFFERENT
            # window. #786 made this branch honest about whether the bind
            # LANDED; it did not stop it binding a stranger. Now it cannot:
            # "" is preserved and the post-check below only fires when a real
            # identity was supplied and failed to bind.
            _h = _hsid
            # #786: THE BRANCH A TRAPPED CALLER ACTUALLY REACHES. session_start
            # has TWO binding branches and the restamp one above was fixed
            # first; this one was left behind -- the SIXTH instance of the twin
            # pattern in this codebase (#522/#774/#779/#781/#782, and
            # ai_session(connect) vs ManagedModeService.connect in this very
            # item), and the one I introduced myself while fixing the others.
            #
            # It matters more than its sibling: a locked-out caller has no
            # per-conductor row, so _resolve_mode returns
            # unresolvable_host_session (managed_mode_service.py:288-300),
            # active=False, and control goes STRAIGHT PAST the restamp branch
            # into this one. Claude Code also fires session_start on every /mcp
            # reconnect, so it runs unattended.
            #
            # MEASURED through the live tool before this fix:
            #   receipt: {"ok":true,"active":true,"source":"session_start_auto_activated"}
            #   gate:    deny why=('managed_mode_not_active',) -- same host session
            #   per-conductor row: None
            # That is #786's exact signature surviving on the sibling surface.
            _activated = hub.managed_mode.set_mode(
                project_root,
                session_id=sid,
        ...
        # Report the bind that ACTUALLY landed. `_h` may legitimately be empty
        # (no host identity available at all), which still restamps the
        # singleton exactly as before -- so this only refuses to claim success
        # when an identity WAS supplied and then failed to bind.
        if _h and str((_activated or {}).get("resolved_via") or "") != "per_conductor":
            return {
                "ok": False,
                "active": False,
                "session_id": sid,
                ...
                "error": (
                    f"session_start auto-activate wrote no per-conductor binding "
                    f"for host session '{_h}' on '{sid}', so every gated tool "
                    f"will refuse managed_mode_not_active. Answering active=True "
                    f"here is what left an agent with no symptom to act on. If "
                    f"this window reaches the daemon without the stdio shim "
    ...
    def aidocs_mode_set(session_id: str, source: str = "/aidocs") -> dict[str, Any]:
        ...
        # #816: bind the CALLER, not just the deprecated singleton. This is the
        # exact twin of aidocs_mode_clear directly below, which has resolved the
        # calling host since #438 ("a disable that skips the unbind is a no-op
        # against authority") — the enable half was left writing a row no gate
        # reads, so an agent that called it was told mode was set and then
        # refused managed_mode_not_active.
        from .mcp_server_runtime_helpers import current_calling_host_session_id
    ...
    _ti_c20.register_impl("bump_agent_memory_epoch", bump_agent_memory_epoch)
    ...
    def ai_lane_grant_scope(
        ...
        DRT-16 / Empire 2026-09-10: "a lane must not be able to grant ANY tools
        to itself". `conductor_comms.lane_grant_scope` has NO authority check of
        its own and its `granted_by` DEFAULTS to the literal string "conductor"
        (conductor_comms.py:2286) which is written straight into the audit row
        (:2362) — so before this guard a lane could widen its own scope and have
        the record attribute it to the conductor. Forged provenance on top of the
        widening.

        The verdict comes from `authority_rank.require_descent_grant`, which
        DERIVES the caller's rank from its XAACP actor identity: self-grant and
        peer-grant refuse, only a strict ancestor may widen a descendant, and an
        unidentifiable or merely unstamped caller is REFUSED rather than read as
        the conductor. `granted_by` is then stamped from the DERIVED grantor rank,
        never from a caller-supplied string.
        """
        from .authority_rank import RankRefused, require_descent_grant
        from .conductor_comms import lane_grant_scope

        root = resolve_project_root()
        try:
            verdict = require_descent_grant(
                root,
                grantee_lane_id=lane_id,
                capability=f"lane_scope:{kind}",
            )
        except RankRefused as exc:
            return {
                "granted": False,
                "blocked_by": "authority_rank",
                "reason": exc.reason,
                "detail": exc.detail,
                "lane_id": lane_id,
            }
