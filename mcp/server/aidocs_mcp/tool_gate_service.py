...
# agent-dispatch brief gate. ONE HOME (Article VI): imported from
...
class ToolGate:
    ...
    @staticmethod
    def _reconstruct_edited_file_pair(
        tool_input: Any,
        path: str,
        project_root: Path,
    ) -> tuple[str, str]:
        """(BEFORE, AFTER) for a fragment edit, both from ONE read.

        AFTER is the FULL file content a fragment edit would produce, so
        syntax validation runs on a whole program — never on a bare
        fragment. BEFORE is the very text it was built from, so the Ruff
        edit gate compares against the same snapshot rather than a second
        disk read. ("", "") (→ skip validation) when the file can't be read
        or the edit can't be applied; the downstream file_ops writer
        ...
        return text, text.replace(old_string, new_string)
    ...
    def validate_edit_syntax(
        ...
        *,
        ...
        path = ""
        content = ""
        # BEFORE snapshot for the Ruff edit gate: for a reconstructed fragment
        # edit it is the SAME read the AFTER was built from. For a full-file
        # write it stays None and host_policy_service reads the file on disk
        # (an earlier defense; file_ops re-judges its exact snapshot).
        ...
        if isinstance(tool_input, dict):
            ...
            if _is_fragment_edit:
                before, content = self._reconstruct_edited_file_pair(
                    tool_input,
                    path,
                    project_root,
                )
        ...
        try:
            ...
            # The Ruff edit gate inside RETURNS its refusals (never raises), so
            # a broken ruff is a deny below, not this advisory continue.
            result = _vs(path, content, project_root=project_root, before_content=before)
        except Exception:
        ...
        if isinstance(result, dict) and not result.get("valid", True):
            return ToolGateResult.deny(
                ...
                + ((str(result["reason"]),) if result.get("reason") else ()),
            )
    ...
    def session_freeze_pretool(
        ...
        *,
        ...
        """
        # BRANCH-TRANSITION BARRIER (r0b send-back on 7cb98b4d9, blocker 2).
        # A project-scoped freeze held by ai_git op=switch from before its
        # liveness/clean samples through its HEAD receipt. Every host adapter
        # reaches this method (evaluate_tool, and the native-shell structural
        # pass with tool_name=""), so this is the pretool chokepoint. Reads and
        # freeze-remedy/report tools pass; everything else is refused.
        try:
        ...
        if _bt_why:
            return ToolGateResult.deny(
                reason=_bt_why,
                why=("session_freeze_pretool", "branch_transition_barrier"),
            )
    ...
    def record_pretool_audit(
        ...
        *,
        ...
        # #489: FORENSIC-ONLY row (readers: dashboards, metrics, prompt skill
        # suggestions — no gate reads it within this or the next hook). Inside
        # the warm broker it goes behind the verdict (counted; drops/failures
        # surfaced); everywhere else it stays inline, exactly as before.
        from . import hook_write_behind as _hwb

        def _write_pretool_audit() -> None:
        ...
        try:
            if not _hwb.submit("pretool_native_tool_use", _write_pretool_audit):
                return ToolGateResult.cont(why=("pretool_audit_dropped",))
        except Exception:
