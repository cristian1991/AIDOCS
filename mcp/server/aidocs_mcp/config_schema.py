...
SETTINGS_CATALOG: dict[str, SettingMetadata] = {
    ...
    "tools.native_shell_execution_pilot": _setting(
        type="boolean",
        default=False,
        description="[T0 DASHBOARD-ONLY] [DEPRECATED 2026-06-05] Former Batch 2.0-B0 pilot gate-opening drill. DEMOTED to a no-op for authorization: native execution is now the normal governed shell surface gated by the canonical ai_run law (see tools.native_shell_readonly_enabled / shell_adapter), not a pilot allowlist. Retained only for back-compat; toggling it no longer widens or narrows native authorization.",
        security_sensitive=True,
        dashboard_only=True,
        service_managed="governed_bash",
        scope=["global", "project", "session"],
    ...
    # `dev.dev_mode` removed (2026-06-12): self-edit / dev authority is now
    # derived from the DEV distribution flavour (distribution.flavor=dev on
    # the canonical AIDOCS source repo), not a per-project config toggle.
    ...
    "dev.kill_switch": _setting(
        type="boolean",
        default=False,
        description=(
            "[T0 DASHBOARD-ONLY — DEV FAILSAFE] Total bypass kill switch. When True "
            "AND distribution.flavor='dev', every AIDOCS enforcement "
            "surface returns a no-op: the PreToolUse hook exits at "
            "the dispatch boundary (handle() returns None before any "
            "gate runs), the pre-tool gate cascade short-circuits to "
            "allow, the reconnect gate is ignored, the force-wakeup "
            "gate is ignored, RBAC checks short-circuit to allow. If "
            "AIDOCS itself crashes or misbehaves, the operator can "
            "flip this flag and keep working while we debug. INSTALL-"
            "PATH-LOCKED: only honored when distribution.flavor="
            "'dev'. Corpo and solo installs refuse this setting — "
            "config_set returns an error and the hook/gate code "
            "treats it as False regardless of stored value. Dev "
            "builds emit an enforcement_disabled_bypass audit event "
            "on every bypass so the audit chain still records "
            "activity during the exemption window. Production "
            "releases will strip this setting entirely. Renamed "
            "from dev.kill_switch 2026-04-22 so the "
            "dashboard groups it under SECURITY·DEV with the red "
            "treatment other dev-security keys get. "
            "Per-session scope added 2026-05-07 (Phoenix): operator "
            "can disable enforcement WITHIN a single session (e.g. "
            "while the Phoenix wields wider authority for structural "
            "fixes) without releasing chaos across the kingdom. "
            "Cascade: session > project > global. Setting the switch "
            "at session scope leaves other sessions fully gated."
        ),
        security_sensitive=True,
        dashboard_only=True,
        scope=["global", "project", "session"],
    ...
    "security.allow_raw_edits": _setting(
        ...
        description=(
            ...
            "ai_str_replace / ai_create_file. Breaks the AIDOCS index "
    ...
    # Superseded by [bash] / [raw_bash] declarative tables. Operators
    ...
    "bash": _setting(
        ...
        description=(
            ...
            "Raw bash is blocked agent-side; ai_run is the only path "
            "to shell-out, gated by this policy. Read as a tree by "
    ...
    "security.judge_override": _setting(
        ...
        default=[],
        description=(
            "[T0 DASHBOARD-ONLY] Judge rule IDs to suppress for this "
            "project (e.g. GIT_FORCE_PUSH for sync repos). Per §6 "
            "override taxonomy this is dashboard-only — bypassing "
            "specific judge rule_ids is a posture decision and never "
            "agent-grantable."
        ),
        security_sensitive=True,
        dashboard_only=True,
    ...
    "security.require_login": _setting(
        type="boolean",
        default=False,
        description=(
            "[T0 DASHBOARD-ONLY] Require an authenticated login for EVERY install "
            "flavor. When true, the dev/solo local-admin passthrough is suppressed "
            "and RBAC/auth applies as in corpo (the physically-present operator is "
            "no longer auto-super_admin; an authenticated account = the operator "
            "user_id). Default false preserves single-operator boxes during the "
            "universal-login rollout; flip on once desktop login is wired."
        ),
        security_sensitive=True,
        dashboard_only=True,
        scope=["global"],
    ...
    "security.freeze_all_sessions_on_malicious_intent": _setting(
        ...
        description=(
            ...
            "Requires an authenticated operator (security.require_login); with no "
            "login the user_id is empty and the freeze stays per-session regardless."
        ),
        security_sensitive=True,
        dashboard_only=True,
        scope=["global"],
    ...
    "conductor.lane_allowed_tools": _setting(
        ...
        default=[
            ...
            "context_*",
            "edit_history_*",
            # Phoenix 2026-05-12 (king directive): msg_* universal
    ...
    "memory.capture_gate.auto_merge": _setting(
        ...
        description=(
            ...
            "Default False (king doctrine 2026-05-17): gate refuses "
    ...
    "edit.str_replace_max_old_chars": _setting(
        ...
        default=1000,
        description=(
            "Maximum characters allowed in `old_str` for ai_str_replace / "
            "ai_replace(mode='string') calls. History: tightened "
            "2026-05-01 from 2000 to 500 (king doctrine #113), "
    ...
    "edit.line_edit_relock_free_span": _setting(
        ...
        default=10,
        description=(
            "Only one line-based edit (ai_edit_lines / ai_insert_lines) per file per "
            ...
            "with fresh line numbers; ai_str_replace / ai_batch_edit always bypass. "
    ...
    "audit.dev_mode_bypass": _setting(
        type="boolean",
        default=False,
        description=(
            "[T0 DASHBOARD-ONLY] When True (combined with "
            "distribution.flavor='dev'), audit-trail enforcement "
            "softens — failures to record events become non-fatal "
            "warnings instead of refusals. Sister flag to "
            "rbac.dev_mode_bypass. Read in mcp_server_runtime_helpers."
        ),
        security_sensitive=True,
        dashboard_only=True,
        scope=["global"],
    ),
    "rbac.dev_mode_bypass": _setting(
        type="boolean",
        default=False,
        description=(
            "[T0 DASHBOARD-ONLY] When True (combined with "
            "distribution.flavor='dev'), RBAC permission checks "
            "short-circuit to allow. Pre-RBAC bootstrap path also "
            "uses this. Sister flag to audit.dev_mode_bypass. Read "
            "in rbac_store and service_hub.require_permission."
        ),
        security_sensitive=True,
        dashboard_only=True,
        scope=["global"],
