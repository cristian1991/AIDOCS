...
- ``line_end`` is computed via SQL LEAD() over the same path —
  the next outline's line - 1, or the file's total line_count.
...
class CodeUnitVendor:
    ...
    def iter_units(self) -> Iterator[Unit]:
        ...
        Uses SQL LEAD() to compute line_end as the next outline's
        line_number - 1 within the same file, or the file's line_count
        as a fallback when no next outline exists.
        """
        index_version = self.get_index_version()

        with sqlite3.connect(str(self.index_db_path)) as conn:
            # SQLite >= 3.25 supports window functions. LEAD over
            # outlines ordered by (path, line_number) gives the next
            # symbol's start line; line_end becomes (next - 1) or
            # (file.line_count) if no next.
            rows = conn.execute(
                """
                SELECT
                    co.path,
                    ...
                    LEAD(co.line_number) OVER (
                        PARTITION BY co.path ORDER BY co.line_number
                    ) AS next_line
