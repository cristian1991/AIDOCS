...
def _collect_notification_blocks(
    ...
    # not depend on one.
    blocks.extend(_collect_worker_mailbox_blocks(project_root))
    if not session_id:
        return blocks
    ...
    try:
        ...
        pending_xaacp = routed.get("messages", []) if isinstance(routed, dict) and routed.get("ok") else []
        if pending_xaacp:
            blocks.append(_cc.xaacp_format_block(pending_xaacp))
    except Exception:
        pass
...
def _inject_into_return(raw: Any) -> Any:
    ...
    try:
        ...
        if _is_worker_caller():
            ...
            blocks = _collect_notification_blocks(project_root, "", actor_id, lane_id)
            return _augment_return(raw, blocks) if blocks else raw
        if not session_id:
            return raw
