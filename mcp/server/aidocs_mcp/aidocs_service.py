...
# #1061: backoff for re-attempting a broker start that was never adopted.
_BROKER_START_RETRY_MIN_S = 60.0
_BROKER_START_RETRY_MAX_S = 900.0
...
def replay_stop_duties(*, roots=None) -> dict:
    """Replay the IDEMPOTENT Stop duties that dead dispatches never proved.

    THE SUPERVISOR IS THE ONLY ACTOR THAT CAN DISCHARGE THIS DEBT. A Stop
    dispatch that died mid-flight leaves owed duties behind; the process that
    owed them is gone, and the agent whose turn it was has ended. The accepted
    Stop theorem's fault-containment half is therefore not composed at all
    unless something OUTSIDE the dispatch replays it -- which is what
    ``stop_duty_ledger.replay_idempotent`` was written for and never wired to.
    Until this call existed the ledger recorded debt that nobody ever paid.

    Same shape as ``recover_causal_turns`` on purpose: the watchdog is the one
    process that outlives daemon restarts, ``roots`` defaults to the
    install-wide registry, and it is FAIL-SOFT PER PROJECT -- one locked or
    corrupt database is logged and counted, never allowed to abort the others.

    Only idempotent duties are re-run. ``replay_idempotent`` returns every
    non-replayable duty in ``outstanding`` rather than marking it discharged,
    and that count is logged: a duty no one can safely re-run stays visible
    debt instead of being laundered into a clean sweep.
    """
    if roots is None:
        try:
            from .known_projects_store import KnownProjectsStore
            ...
            _log(f"stop-duty replay: project registry unreadable: {exc!r}")
            return {"projects": 0, "replayed": 0, "outstanding": 0, "errors": 1}
    from . import stop_duty_ledger as _sdl

    projects = replayed = outstanding = errors = 0
    for root in roots:
        projects += 1
        try:
            out = _sdl.replay_idempotent(Path(root))
            replayed += len(out.get("replayed") or ())
            outstanding += len(out.get("outstanding") or ())
        except Exception as exc:  # noqa: BLE001 — per-project isolation
            errors += 1
            _log(f"stop-duty replay FAILED for {root}: {exc!r}")
    if replayed or outstanding or errors:
        _log(
            f"stop-duty replay: projects={projects} replayed={replayed} "
            f"outstanding={outstanding} errors={errors}"
        )
    return {
        "projects": projects,
        "replayed": replayed,
...
def _activated_generation() -> str:
    """Which runtime generation the MACHINE has activated, right now (#1030).

    The "what SHOULD serve" side of the supervisor's staleness question, read
    through the same resolver the spawn uses so parent and child cannot end up
    describing different decisions. Empty string means no generation is
    activated — a legacy single-tree install, a real and stable state.
    """
    py, gen = child_python()
    if not py:
        # AN UNSERVEABLE POINTER IS UNKNOWN, NOT AN ID. `child_python` answers
        # ("", <reason>) when a generation is activated but cannot be served,
        # and that reason is a NON-EMPTY string — so returning it verbatim
        # would hand the supervisor a "generation id" no child can ever
        # attest. Adoption would be refused (safe), but the trigger would fire
        # on EVERY tick, spawning into a target that cannot start. Raising puts
        # it on the unknown path, where nothing adopts and nothing churns.
        raise RuntimeError(f"the activated generation cannot be served: {gen}")
    return gen or ""
...
class BrokerChild:
    ...
    def __init__(
        ...
        *,
        ...
        # WHICH GENERATION THE MACHINE HAS ACTIVATED, injectable for the same
        # reason `disk_identity` is: the two axes are the two worlds this
        # supervisor can find itself in, and a test has to be able to say which
        # one it is describing. Returning None/"" means "legacy in-place
        # install", where content identity is the only axis that exists.
        self._activated_generation = activated_generation or _activated_generation
        self._clock = clock
        self._sleep = sleep
        ...
        # WHY THE LAST REPLACEMENT FAILED TO PUBLISH, carried across the one
        # boundary that used to destroy it. `_await_publication` returns a bare
        # None for two very different facts (died with an rc / alive but
        # silent), and `_proven_fresh` cannot re-derive which from None. Holding
        # the reason here lets the refusal be reported ONCE, by whichever of the
        # two actually knows it, instead of being replaced downstream by a
        # generic sentence about a comparison that never ran.
        self._publish_failure: str | None = None
        # #1061: the last self-heal skip reason, so each is said ONCE per
        # transition rather than every poll (or, before, never).
        self._last_skip: str | None = None
        self._next_start_retry: float = 0.0
        self._start_backoff: float = 0.0
    ...
    def _await_publication(self, proc) -> dict | None:
        ...
        # CLEARED PER ATTEMPT. A reason left over from an earlier replacement
        # would suppress the fallback message for a LATER silent refusal, which
        # is the same bug in a new costume: a refusal with no reason, this time
        # because a stale one stood in for it.
        self._publish_failure = None
        ...
        while True:
            ...
            if state is not None and int(state.get("pid") or -1) != superseded:
                return state
            if proc.poll() is not None:
                # DIED BEFORE IT COULD SAY ANYTHING -- and the rc is RIGHT HERE.
                # This branch used to `return None` bare. `_proven_fresh` then
                # turned that into its ONE silent `False` (`if not state`), and
                # `start()` reported "could not prove it loaded the code on
                # disk" -- a sentence describing a COMPARISON THAT NEVER
                # HAPPENED, while the actual cause was discarded at the only
                # boundary that still held it.
                #
                # MEASURED COST, 2026-09-12: the warm broker died at every cold
                # boot for ~18h. Every hook fell back to the cold
                # fresh-interpreter path, so EVERY operator prompt arrived
                # `UPS DEGRADED ... timed_out` after ~10s, `hook_broker.json`
                # stopped being written, and the log could only offer a sentence
                # that was false. Nobody could say why, because the why had been
                # thrown away here.
                #
                # `poll()` is ALREADY called, so naming the rc costs no new I/O
                # and cannot block the supervisor -- which matters, because a
                # supervisor that hangs while explaining itself is strictly
                # worse than one that explains nothing (`describe_self`, same
                # file, says exactly that).
                self._publish_failure = f"died rc={proc.returncode}"
                self._emit(
                    "hook broker: the replacement DIED before publishing its "
                    f"rendezvous (rc={proc.returncode}) — there was nothing to "
                    "prove against; this is NOT an identity mismatch"
                )
                return None
            if self._clock() >= deadline:
                # STILL ALIVE, JUST SILENT. A DIFFERENT FACT from death, and
                # collapsing the two is what made them indistinguishable in the
                # log. Preserved as first-class UNKNOWN, with its timing, so the
                # next reader can tell "it crashed" from "it was too slow".
                self._publish_failure = f"silent past {self._publish_timeout}s"
                self._emit(
                    "hook broker: the replacement did not publish its "
                    f"rendezvous within {self._publish_timeout}s and is STILL "
                    "ALIVE — UNKNOWN, not an identity mismatch"
                )
                return None
            self._sleep(_BROKER_PUBLISH_POLL_S)

    def _intended_generation(self) -> str | None:
        """The generation a replacement spawned NOW is meant to load.

        THREE ANSWERS, and collapsing any two of them is a bug:
          "<gid>"  a generations box, and this is the target to attest against
          ""       a LEGACY in-place install — no generation exists on either
                   side, so content identity is the only axis there is
          None     UNKNOWN: the resolver could not answer at all

        Unknown must not read as legacy. If it did, a transient failure to read
        the pointer would drop a generations box onto the content axis, where
        the watchdog's frozen tree never matches the child's identity — and the
        supervisor would respawn on every tick forever.
        """
        try:
            return self._activated_generation()
        except Exception:  # noqa: BLE001 — a broken resolver is UNKNOWN
            return None

    def _proven_fresh(self, state: dict | None, intended: str | None = None) -> bool:
        """Did the replacement prove it loaded what we meant it to load?

        TWO AXES, AND ON A GENERATIONS BOX ONLY ONE OF THEM CAN ANSWER (#1030).

        content identity — "does this child's tree match the tree on disk". The
            only axis that exists on a LEGACY in-place install, and preserved
            unchanged for it.
        generation      — "did this child load the generation the machine has
            activated". The only axis that can answer on a generations box,
            because `self._disk_identity()` reads the WATCHDOG'S OWN
            `__file__`, which lives inside an immutable generation directory.
            That tree is frozen for the life of the process, so a child that
            correctly loaded B would be compared against A and refused. The
            trigger could not fire; had it fired, adoption could not succeed.

        THE WATCHDOG'S GENERATION IS NEVER B'S FRESHNESS AUTHORITY. The
        comparison is intended-vs-attested: what we resolved from the pointer at
        spawn, against what the child says it loaded FROM ITS IMPORT PATH. The
        watchdog's own generation appears nowhere in it.

        UNKNOWN IS NOT A PASS (#589), on either axis: the published fields are
        JSON null rather than "" precisely so an absent answer cannot be read as
        an answer.
        """
        if not state:
            # NO RENDEZVOUS AT ALL -- so NO COMPARISON WAS EVEN ATTEMPTED. This
            # was the one refusal path in this method with no reason attached,
            # and it is the one that actually fired for ~18h: `start()` then
            # printed its generic "could not prove it loaded the code on disk",
            # describing a comparison that never happened and sending every
            # reader to the identity axis, which was innocent.
            #
            # `_await_publication` knows the real cause (died with an rc, or
            # alive and silent past the deadline) and now records it. Deferring
            # to it keeps the specific fact and states it ONCE; the fallback
            # exists because a caller that did not come through that path must
            # still not refuse in silence.
            if not self._publish_failure:
                self._emit(
                    "hook broker: no rendezvous was published, so NOTHING was "
                    "compared — this is UNKNOWN, not an identity mismatch"
                )
            return False
        if intended is None:
            # UNKNOWN WORLD, so nothing can be proven. Adopting on the content
            # axis here would let a generations child be judged against the
            # watchdog's frozen tree — the very substitution this guards.
            self._emit(
                "hook broker: cannot resolve the activated generation, so a "
                "replacement cannot prove what it loaded — not adopted"
            )
            return False
        if intended:
            attested = state.get("runtime_generation")
            if not attested:
                self._emit(
                    "hook broker: the replacement published no runtime "
                    "generation — cannot prove it loaded the activated one, "
                    "so it is not adopted"
                )
                return False
            if attested != intended:
                self._emit(
                    "hook broker: the replacement loaded generation "
                    f"{str(attested)[:24]} but the machine activated "
                    f"{str(intended)[:24]} — refusing to adopt a substitute"
                )
                return False
            return True
        ...
        #
        # The generation axis above names every one of its refusals; this axis
        # used to refuse in silence, which is the same defect one branch over.
        # WHICH SIDE is unknown is the whole diagnosis — a child that published
        # nothing is a child problem, an unreadable tree is a machine problem,
        # and they are fixed in different places — so say which.
        if not loaded or not on_disk:
            self._emit(
                "hook broker: cannot compare code identity because "
                + (
                    "the replacement published none"
                    if not loaded
                    else "the tree on disk yielded none"
                )
                + " — UNKNOWN, so not adopted (this is NOT a mismatch)"
            )
            return False
    ...
    def attested_generation(self) -> str | None:
        """The generation the ADOPTED child attested it loaded, or None.

        The child's own word (its rendezvous), never the watchdog's bookkeeping
        about the daemon. The supervisor reads it only to AVOID a gratuitous
        re-home; adoption still goes through ``_proven_fresh``.
        """
        if self._proc is None or not self._state:
            return None
        return self._state.get("runtime_generation") or None

    def start(self) -> BrokerChild:
        self._clear_state()
        # ONE POINTER READ PER REPLACEMENT, taken BEFORE the spawn and reused as
        # the proof target. Re-reading it after the child publishes would ask
        # about the POINTER rather than about the child, and a flip landing in
        # that window would reject a perfectly good replacement (or, worse,
        # accept one that loaded something else).
        intended = self._intended_generation()
        proc = self._spawn()
        state = self._await_publication(proc)
        if not self._proven_fresh(state, intended):
            self._terminate(proc)
            self._restore_state()
            # THE CONSEQUENCE, NOT A CAUSE. This line used to assert "could not
            # prove it loaded the code on disk", which names the CONTENT AXIS
            # specifically -- and was a lie on every other refusal path,
            # including the one that actually fired for ~18h (no rendezvous at
            # all, so no axis was consulted). `_proven_fresh` and
            # `_await_publication` each emit the real reason immediately above;
            # this says only what was DONE about it, which is the one thing this
            # frame actually knows.
            self._emit(
                "hook broker host was not adopted (reason above) — killed; "
                "hooks stay on their local (fresh-interpreter) path"
            )
            return self
    ...
    def maybe_refresh_on_code_change(self) -> bool:
        ...
        if self._proc is None or self._state is None:
            # #1061 NOTHING ADOPTED. This used to `return False` in silence on
            # the theory that start() owns this path — but start() runs ONCE,
            # at watchdog boot. A boot-time failure therefore left the warm
            # path down for the watchdog's whole life with no further line.
            # Retry, backed off so a child that always dies is not a storm.
            now = self._clock()
            if now < self._next_start_retry:
                self._skip(
                    "hook broker: no broker is adopted — next start attempt "
                    f"in {self._next_start_retry - now:.0f}s"
                )
                return False
            self._emit(
                "hook broker: no broker is adopted (the last start was not "
                "adopted, reason logged then) — retrying start"
            )
            self._last_skip = None
            self.start()
            if self._proc is None or self._state is None:
                self._start_backoff = min(
                    max(self._start_backoff * 2, _BROKER_START_RETRY_MIN_S),
                    _BROKER_START_RETRY_MAX_S,
                )
                self._next_start_retry = self._clock() + self._start_backoff
                return False
            self._start_backoff = 0.0
            return True

        # ── generations box: the pointer is the authority ────────────────
        #
        # WHY THE CONTENT AXIS CANNOT ANSWER HERE, which is why this self-heal
        # sat inert on every generations box since #1030 landed. `_disk_identity`
        # hashes `Path(__file__).parent` — the WATCHDOG's own tree, inside an
        # immutable generation directory. A new build does not overwrite it; it
        # writes a NEW directory and moves the pointer. So `loaded == on_disk`
        # compared A against A forever, returned False every time, and the
        # supervisor written to stop a child going stale waited to be asked.
        # Measured: five consecutive unattended updates, each leaving the warm
        # path down until a human ran `aidocs service restart`.
        #
        # ADOPTED vs ACTIVATED is the comparison that can actually move: what
        # the child we adopted says it LOADED, against what the machine has
        # since ACTIVATED. Neither side is the watchdog's own generation.
        intended = self._intended_generation()
        if intended is None:
            # CANNOT TELL WHICH WORLD WE ARE IN. Falling through to the content
            # axis here would compare a generations child against the
            # watchdog's frozen tree, never match, and respawn on every tick.
            self._skip(
                "hook broker: cannot resolve the activated generation — "
                "self-heal skipped (UNKNOWN is not a change); broker kept"
            )
            return False
        if intended:
            adopted = self._state.get("runtime_generation")
            # UNKNOWN IS NOT A CHANGE. A child that published no generation
            # tells us nothing, and respawning on "nothing" is a churn loop
            # against a broker that may be serving perfectly well.
            if not adopted:
                self._skip(
                    "hook broker: the adopted broker published no runtime "
                    "generation — self-heal skipped (UNKNOWN is not a change)"
                )
                return False
            if adopted == intended:
                self._skip(
                    f"hook broker: current — serving the activated generation "
                    f"{str(intended)[:24]}"
                )
                return False
            self._last_skip = None
            self._emit(
                f"hook broker: the machine activated {str(intended)[:24]} while "
                f"this broker serves {str(adopted)[:24]} — respawning onto the "
                "activated generation"
            )
            return self.refresh_for_deploy()

        # ── legacy in-place install: content identity, unchanged ─────────
        ...
        if not loaded or not on_disk:
            self._skip(
                "hook broker: code identity UNKNOWN ("
                + ("broker published none" if not loaded else "tree yielded none")
                + ") — self-heal skipped"
            )
            return False
        if loaded == on_disk:
            self._skip(f"hook broker: current — loaded code {str(loaded)[:12]}")
            return False
        self._last_skip = None
        self._emit(
...
# LANE B: where a spawned daemon child carries the runtime generation it was
# spawned under (from the SAME `child_python()` snapshot that chose its argv).
# Absent means UNKNOWN (a pre-generation or adopted daemon) — never guessed.
_LOADED_GEN_ATTR = "aidocs_runtime_generation"


def _loaded_generation_of(proc) -> str | None:
    gen = getattr(proc, _LOADED_GEN_ATTR, None)
    return gen if isinstance(gen, str) else None


def _judge_runtime_target(desired: str | None, loaded: str | None, error: str | None) -> dict:
    """The health-facing verdict of ACTIVE vs LOADED. ``loaded`` is reported as
    it is — None stays None (unknown), never the intended generation."""
    if error is not None:
        return {"status": "unservable", "reason": error, "loaded": loaded}
    if desired and loaded is None:
        status = "unknown"
    elif desired and loaded != desired:
        status = "drift"
    else:
        status = "current"
    return {"status": status, "desired": desired, "loaded": loaded}
...
def run_watchdog(
    ...
    *,
    ...
    _win_no_window = getattr(subprocess, "CREATE_NO_WINDOW", 0) if os.name == "nt" else 0
    spawn_is_default = spawn is None
    if spawn is None:
        ...
        def spawn(backend_port):
            ...
            # LANE B: the generation this child was spawned under travels WITH
            # the child — the same snapshot that chose its interpreter. The
            # supervisor never re-reads the pointer to guess it later.
            try:
                setattr(_child, _LOADED_GEN_ATTR, _gen or "")
            except Exception:  # noqa: BLE001 — untaggable child reads as UNKNOWN
                pass
            return _child

    # LANE B — SWAP AUTHORITY IS GENERATION, NOT "A REFRESH RAN". The desired
    # side is the canonical resolver. An injected spawn with no injected
    # resolver has no generation to attest (a test double of the legacy
    # world), so the generation axis is silent there rather than guessed.
    if desired_generation is None:
        desired_generation = _activated_generation if spawn_is_default else (lambda: "")
    ...
    # LANE B: the last judgement of the ACTIVE target against the daemon's
    # LOADED generation, re-attached to every health write like last_refresh.
    runtime_target: dict | None = None
    # The desired generation an UNKNOWN (untagged) daemon was already swapped
    # toward. Stops a persistently untagged child from being re-swapped for the
    # SAME target; a new desired generation earns a new swap.
    unknown_swapped_for: str | None = None

    def _health(**kw) -> None:
        if runtime_target is not None:
            kw.setdefault("runtime_target", dict(runtime_target))
        if last_refresh:
            kw.setdefault(
                "runtime",
    ...
    # the deploy channel uses. One update path for both channels (doctrine VI);
    ...
    def _sweep_stop_duties() -> None:
        """Pay the owed, replayable Stop duties of dispatches that died.

        Rides the SAME tick as the causal-turn sweep and is gated by the SAME
        seam: both are "finish what a dead turn left unfinished", and splitting
        them into two intervals would only create a second thing to forget to
        turn on. FAIL-SOFT: a replay failure is logged, never fatal.
        """
        if sweep is None:
            return
        try:
            ...
            _log(f"stop-duty replay sweep failed: {exc!r}")
    ...
    def _consume_refresh_request() -> dict | None:
        ...
        # LANE B: OBSERVABILITY ONLY. This used to be `restart_daemon = code == 0`
        # and it was swap authority — so a no-op "already current" refresh
        # (code 0, verdict fresh) overlap-restarted the daemon and re-homed the
        # broker every cycle. Whether the daemon swaps is decided by generation
        # drift on the supervision tick. `runtime_changed` is consulted only on a
        # LEGACY in-place install, where no generation exists to compare.
        result["runtime_changed"] = result.get("code") == 0 and result.get("verdict") == "refreshed"
        # restart_daemon=False is RETIRED, not honoured and not silently
        # ignored: honouring it would let a caller leave the daemon on a stale
        # generation, which is the exact state this supervisor exists to end.
        # The request body is never swap permission, legacy path included.
        if body.get("restart_daemon") is False:
            _log(
                "refresh request carried restart_daemon=False — that flag is "
                "retired: daemon swaps are decided by generation drift (or, on a "
                "legacy install, by an actual reinstall), never by the request"
            )
        return result
    ...
    try:
        while True:
            ...
            spawned_at = clock()
            proc = spawn(backend)
            # ONE FACT with (proc, backend): what THIS child was spawned under.
            loaded_gen = _loaded_generation_of(proc)
            ...
            while proc.poll() is None:
                ...
                # #569 / LANE B: a requested refresh still RUNS here, but it no
                # longer decides the swap. A no-op "already current" refresh used
                # to overlap-restart the daemon and re-home the broker every
                # cycle. What decides is the daemon's LOADED generation against
                # the ACTIVE one — evaluated on EVERY tick, request or not, so an
                # out-of-band activation swaps too. The proven overlap path below
                # is reused: new daemon ready FIRST, proxy flips, old one drains.
                refreshed = _consume_refresh_request()
                if refreshed:
                    last_refresh = refreshed
                try:
                    desired_gen = desired_generation()
                    target_error = None
                except Exception as exc:  # noqa: BLE001 — unservable is UNKNOWN
                    # A POINTER ERROR IS NEVER A GENERATION ID. Unservable,
                    # malformed or unsealed: nothing is spawned, nothing is
                    # substituted; the live old daemon keeps serving.
                    desired_gen = None
                    target_error = str(exc) or type(exc).__name__
                # Known generation on a generations box, and the child is on a
                # DIFFERENT one: drift. "" is a LEGACY in-place install, where no
                # generation exists on either side. An UNKNOWN child gets ONE
                # conservative swap per desired generation — never promoted to
                # the intended one (intent is not evidence), never looped on.
                generation_drift = bool(desired_gen) and (
                    loaded_gen != desired_gen
                    if loaded_gen is not None
                    else unknown_swapped_for != desired_gen
                )
                # LEGACY ONLY: an in-place reinstall moves no generation, so the
                # install that actually CHANGED the runtime is the only fact.
                # The request body is not consulted (restart_daemon is retired).
                legacy_reinstalled = bool(
                    desired_gen == "" and refreshed and refreshed.get("runtime_changed")
                )
                _prev_target = runtime_target
                runtime_target = _judge_runtime_target(desired_gen, loaded_gen, target_error)
                if target_error is not None and (
                    not _prev_target or _prev_target.get("reason") != target_error
                ):
                    _log(
                        f"active runtime target is UNSERVABLE ({target_error}) — "
                        f"no replacement will be spawned; the running daemon "
                        f"(loaded={loaded_gen or 'UNKNOWN'}) keeps serving. "
                        "Repair with: aidocs runtime --fix"
                    )
                ...
                _want_swap = target_error is None and (
                    generation_drift or _marker_moved or swap_pending or legacy_reinstalled
                )
                if _want_swap and clock() >= swap_retry_at:
                    # CAUSES STAY DISTINCT. Deliberately never says "failed
                    # readiness": #726's guard requires every line carrying that
                    # phrase to quote the child's last words.
                    _causes = []
                    if generation_drift:
                        _causes.append(
                            f"generation drift (loaded={loaded_gen or 'UNKNOWN'} "
                            f"active={desired_gen}) — overlap-restart onto the "
                            "active generation"
                        )
                    if _marker_moved:
                        _causes.append("release marker changed — overlap-restart onto new code")
                    if legacy_reinstalled:
                        _causes.append(
                            "runtime refreshed (legacy in-place install) — "
                            "overlap-restart so the daemon runs the new code too"
                        )
                    if swap_pending and not _causes:
                        _causes.append(
                    ...
                    _log("; ".join(_causes))
                    # The target this swap aims at — used ONLY to remember that an
                    # untagged child was already swapped toward it. It is never
                    # recorded as what a child loaded.
                    swap_target = desired_gen
                    ...
                    # LANE B: do not re-home a broker that ALREADY attests the
                    # active generation. Its attestation is the CHILD's own word,
                    # never the daemon's bookkeeping; a stale or unknown one still
                    # goes through refresh_for_deploy, which adopts only a child
                    # that proves it loaded the intended generation.
                    _broker_gen = None
                    _attested = getattr(broker, "attested_generation", None)
                    if _attested is not None:
                        try:
                            _broker_gen = _attested()
                        except Exception:  # noqa: BLE001 — unknown attestation
                            _broker_gen = None
                    if _refresh is not None and desired_gen and _broker_gen == desired_gen:
                        _log(
                            f"hook broker already attests the active generation "
                            f"{str(desired_gen)[:24]} — not re-homed"
                        )
                        _refresh = None
                    ...
                    if ready_wait(new_backend, new_proc):
                        ...
                        if refreshed:
                            last_refresh = refreshed
                        # LANE B: the new child's facts land BEFORE the health
                        # write, so health never shows the new daemon as drifted
                        # on the old generation. loaded_gen is ONLY the child's
                        # own tag (set by the spawn from the same child_python()
                        # snapshot that chose its argv) — never re-read, never
                        # the watchdog's desired_gen. Untagged stays UNKNOWN.
                        loaded_gen = _loaded_generation_of(new_proc)
                        unknown_swapped_for = swap_target if loaded_gen is None else None
                        # HEALTH-ONLY RE-READ of the active target. The pointer
                        # may have moved since the swap decision; describing the
                        # transition against that pre-spawn read would be stale
                        # at exactly the moment it describes. NOT spawn
                        # authority: it never touches loaded_gen, never feeds
                        # this tick's swap decision, and a raise never undoes the
                        # already-ready adoption.
                        try:
                            _desired_now, _target_err_now = desired_generation(), None
                        except Exception as exc:  # noqa: BLE001 — unservable is UNKNOWN
                            _desired_now = None
                            _target_err_now = str(exc) or type(exc).__name__
                        runtime_target = _judge_runtime_target(
                            _desired_now, loaded_gen, _target_err_now
                        )
                        _health(
                            port=port,
...
def request_runtime_refresh(*, check_only: bool = False, restart_daemon: bool = True) -> Path:
    ...
    The reinstall alone makes the HOOKS current IMMEDIATELY — every hook entry in
    ~/.claude/settings.json is a FRESH SUBPROCESS per event. The long-lived daemon
    picks new code up only through the watchdog's overlap-restart.

    ``restart_daemon`` IS RETIRED (lane B). It is still accepted and written for
    compatibility, but it no longer suppresses anything: the watchdog decides a
    daemon swap by GENERATION DRIFT (loaded vs active) on every tick — or, on a
    legacy in-place install with no generations, by a refresh that actually
    reinstalled — and never by the request body, on either path. Honouring
    ``False`` would let a caller leave the daemon on a stale generation. The
    watchdog logs that the flag is retired whenever a request carries ``False``.
    """
    payload = {
...
# law: no pip, no artifact fetch (aidocs-doctrine §XX — installs are signed/
...
def act_on_update_state(
    ...
    *,
    ...
    ONE UPDATE PATH FOR BOTH CHANNELS (doctrine VI). This installs nothing. It
    ...
    if state.get("update_available"):
        # The RELEASE axis has something to say, and it outranks the source axis
        # here: a published release is the artefact this function was written to
        # consume, and its verdicts are load-bearing for public installs.
        if chosen != "auto":
            return "notify"
        try:
            ...
            return "unverified"
        return "requested"
    # ── THE SOURCE AXIS, WHICH NOTHING USED TO CONSULT (#569's other half) ────
    #
    # REACHED ONLY WHERE THIS USED TO `return "current"`, and that placement is
    # the fix to a regression caught by tests/host/test_update_policy_868.py: a
    # first cut consulted the source axis BEFORE the two fail-closed guards
    # above, so on any box with a real checkout an ERRORED or MISSING release
    # check bought an install — breaking "a failed check never triggers an
    # install", which is law. The integrity guards on the input come first; the
    # source axis only ever replaces the "nothing to do" answer.
    #
    # WHY A SECOND AXIS AT ALL. This function used to read the RELEASE axis only,
    # and on a source machine that axis CANNOT MOVE: it compares the running build
    # against the
    # release channel, so local source edits never change it (measured
    # 2026-09-10: running 278, deployed 278, released 278). Both gates below were
    # therefore closed permanently, `request_runtime_refresh` had no reachable
    # production caller on this box, and the operator had to type
    # `aidocs service restart` by hand — which the operator has ruled IS A BUG.
    #
    # #569's decoupling is preserved exactly: this DETECTS and ASKS, it never
    # installs, and under the fail-closed 'notify' default it only reports. The
    # goal is that the estate can OBSERVE drift and request a refresh, not that
    # it silently reinstalls behind the operator.
    #
    # ORDER MATTERS. This sits AFTER the policy resolution and after the `pinned`
    # check, because a pin a new heuristic can override is not a pin.
    try:
        from . import runtime_refresh
        from .runtime_refresh import AXIS_SOURCE

        report = runtime_refresh.freshness_report()
        if isinstance(report, dict) and report.get("axis") == AXIS_SOURCE:
            fresh = report.get("fresh")
            if fresh is False:
                # NOTIFY ONLY, UNDER EVERY POLICY (operator 2026-09-22, r0b lane
                # A1). Asking the refresher here was NON-CONVERGENT: this axis
                # compares the CHECKOUT to the active generation, while the
                # refresher (given no reference) pulls the SIGNED AUTHORITY build
                # and compares that — so a checkout ahead of the deployed build was
                # "stale" here and "already current" there, re-requested every
                # supervision cycle forever. Standing law now: this box updates
                # only from what a deploy published, never from unshipped local
                # source. The 2026-09-10 "restart by hand is a bug" ruling above is
                # superseded by that; real updates still arrive via the build and
                # release axes checked before this point.
                return "notify:source_drift"
            if fresh is None:
                # THE QUESTION WAS ASKED AND NOT ANSWERED — a checkout resolved
                # but the fingerprints could not be compared. Falling through to
                # the release axis would report "current", which is precisely how
                # the original blindness got laundered into a pass. Unknown is
                # not a pass, and the note is carried so the cause survives.
                note = str(report.get("note") or "").strip()
                return f"unverified:source_unknown:{note}" if note else "unverified:source_unknown"
        # axis == AXIS_RELEASE means this box has NO checkout, so the source axis
        # is NOT APPLICABLE rather than unknown — fall through untouched.
    except Exception:  # noqa: BLE001 — this runs on the supervision tick
        # A broken probe must not stop supervision: a missed drift notice is not
        # an outage, a watchdog that stopped ticking is.
        pass
    return "current"
