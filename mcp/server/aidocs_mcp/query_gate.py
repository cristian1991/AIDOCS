...
class QueryGateStore:
    ...
    # stamp_autowake / get_last_autowake are gone; underlying columns
    # kept in storage for migration safety. stamp_compaction stays —
    # it's used by other features (session-freeze recovery grace).
    ...
    def stamp_compaction(
        ...
        *,
        ...
        self._init(project_root)
        return self._store.stamp_compaction(project_root, session_id, now=now)

    def get_last_compaction(
        self,
        project_root: Path,
        session_id: str,
    ) -> int:
        self._init(project_root)
        return self._store.get_last_compaction(project_root, session_id)
    ...
    def get_last_cli_session_id(
        ...
        """Return last_cli_session_id stamped for this session, or "".

        Public wrapper around SessionQueryGateStore.get_last_cli_session_id.
        Used by session_connect / conductor_mode_enter / _check_session_bind_match
        to recover host identity when MCP-server global is empty.
        """
        self._init(project_root)
        return self._store.get_last_cli_session_id(project_root, session_id)
