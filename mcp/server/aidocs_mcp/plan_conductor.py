...
class PlanConductor:
    ...
    def graph(self) -> dict[str, object]:
        lanes = [
            PlanConductorLane(
                ...
                # #161 (Phoenix 2026-05-10): surface plan-parsed
                # per-lane allowed_tools / verification so the
                # conductor_status response confirms the parser
                # honored the plan's `Allowed tools:` / `Verification:`
                # lines.
                allowed_tools=list(getattr(lane, "allowed_tools", []) or []),
                verification=str(getattr(lane, "verification", "") or ""),
            )
        ...
        graph = PlanConductorGraph(
            ...
            lanes=lanes,
            dependencies={
                lane.lane_id: list(lane.depends_on) for lane in self.plan.lanes if lane.depends_on
            },
    ...
    def runnable_lanes(self) -> dict[str, object]:
        ...
        # Phase semantics (Empire directive 2026-05-12, dental bug report):
        # `Phase:` is a labeling/grouping construct for readability and
        # display — it does NOT serialize lanes. Cross-phase ordering
        # must be declared explicitly via `depends_on:` on each lane.
        # The earlier behavior (one phase active at a time, all other
        # phases blocked with "waiting-on-phase:") forced operators to
        # write strictly-sequential plans even when phases were intended
        # as semantic grouping. Now every incomplete lane is a candidate
        # and `depends_on` is the sole ordering mechanism.
        if all(self._lane_is_completed(lane) for lane in self.plan.lanes):
            return self._with_interaction_state(asdict(result))

        candidate_lanes = [lane for lane in self.plan.lanes if lane.lane_id not in completed_lanes]
        ...
        for lane in candidate_lanes:
            unresolved_dependencies = [
                ...
                and not self._dependency_is_satisfied_by_contract_ready(dependency, lane)
            ]
        ...
        overlap_eligible = [
            lane for lane in candidate_lanes if lane.lane_id not in dependency_blocked_ids
        ]
        ...
        for lane_id, reasons in overlap_reasons.items():
            for reason in reasons:
                ...
                if len(parts) >= 3:
                    ...
                    overlap_groups.setdefault(group_key, []).append((lane_id, other_lane))
        ...
        for lane in candidate_lanes:
            ...
            if unresolved_dependencies:
                ...
                reasons.extend(f"waiting-on:{dependency}" for dependency in unresolved_dependencies)
            ...
            if lane.lane_id in overlap_reasons and lane.lane_id not in allowed_from_overlap:
                pass
            ...
            # Lane state machine check — READY and REOPENED lanes are
            # both runnable. REOPENED is the conductor's explicit
            # "re-run this" signal (plan_conductor_reopen_lane);
            # plan_dispatch_next transitions REOPENED → RUNNING on
            # dispatch via the existing LaneState.can_transition path
            # (types.py allows REOPENED → RUNNING). Without this
            # exemption the reopen flow is a half-brick: the conductor
            # can mark a lane reopened but never re-dispatch it
            # without an out-of-band state poke. Phoenix 2026-05-12.
            lane_state = self.lane_states.get(lane.lane_id)
            if lane_state is not None and lane_state not in (
                LaneState.READY,
                LaneState.REOPENED,
            ):
    ...
    def _with_interaction_state(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        payload["lane_signals"] = {
            lane_id: signals for lane_id, signals in self.lane_signals.items() if signals
        }
        return payload
    ...
    def _lane_is_completed(self, lane: PlanConductorLane) -> bool:
        ...
        if lane.lane_id in self.reopened_lane_ids:
            return False
        matching_lane = next(
            (plan_lane for plan_lane in self.plan.lanes if plan_lane.lane_id == lane.lane_id),
            None,
        )
    ...
    def _dependency_is_satisfied_by_contract_ready(
        self,
        dependency: str,
        lane: PlanConductorLane,
    ) -> bool:
        if dependency not in self.contract_ready_lane_ids or not self._lane_is_contract_like(lane):
            return False
        dependency_lane = next(
            (plan_lane for plan_lane in self.plan.lanes if plan_lane.lane_id == dependency),
            None,
        )
    ...
    def _candidate_overlap_reasons(self, lanes: list[PlanConductorLane]) -> dict[str, list[str]]:
            ...
            for lane_id, display in collisions:
                ...
                for other_lane_id, _ in collisions:
                    if other_lane_id == lane_id:
                        continue
                    lane_reasons.append(f"shared-file-overlap:{display}:{other_lane_id}")
        return reasons

    def _canonical_file_identity(self, file_path: str) -> str:
        # Doctrine 2026-05-29 (Empire triage, clean-VPS Gate 2b): the
        # previous implementation used os.path.normcase() to canon-
        # icalize for hashing. On Windows normcase lowercases; on
        # POSIX it's a no-op. Plans authored on Windows with mixed-
        # case paths (e.g. lane-a: src/foo.py, lane-b: src\FOO.py)
        # then RAN on the VPS would NOT detect the overlap — the
        # lanes hashed as distinct keys, _candidate_overlap_reasons
        # returned no collisions, and both lanes were marked
        # runnable. test_conductor_canonicalizes_file_paths_before_
        # overlap_checks pins the cross-platform contract.
        # Fix: replace os.path.normcase with explicit .lower() so
        # the canonical identity is platform-independent (matches
        # _display_file_identity's normalization). Backslash-to-
        # forward conversion is preserved so Windows-authored paths
        # collide with their POSIX twins.
