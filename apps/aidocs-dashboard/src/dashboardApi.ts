...
export async function conductorOutput(since?: number): Promise<{ running: boolean; lines: ConductorOutputLine[]; total_buffered: number }> {
  return invoke("conductor_output", { since: since ?? null });
}
