...
_OUTLINE_FAMILIES: dict[str, list[tuple[str, str]]] = {
    ...
    "java_basic": [
        (
            r"^\s*(?:public|private|protected|static|abstract|final|\s)*class\s+([A-Za-z_][A-Za-z0-9_]*)",
            "class",
        ),
        (
            r"^\s*(?:public|private|protected|static|abstract|final|\s)*interface\s+([A-Za-z_][A-Za-z0-9_]*)",
            "interface",
        ),
        (
            r"^\s*(?:public|private|protected|static|abstract|final|\s)*enum\s+([A-Za-z_][A-Za-z0-9_]*)",
            "enum",
        ),
        (
            r"^\s*(?:public|private|protected|static|abstract|final|synchronized|\s)+[A-Za-z_<>,\[\]?]+\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(",
            "method",
        ),
    ],
    "kotlin_basic": [
        (
            r"^\s*(?:data\s+|sealed\s+|abstract\s+|open\s+)?class\s+([A-Za-z_][A-Za-z0-9_]*)",
            "class",
        ),
    ...
    "php_basic": [
        ...
        (
            r"^\s*(?:public|private|protected|static|\s)*function\s+([A-Za-z_][A-Za-z0-9_]*)",
            "function",
        ),
    ],
    "elixir_basic": [
    ...
    "sql_ddl_basic": [
        (
            r"(?i)^\s*CREATE\s+(?:OR\s+REPLACE\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:\")?([A-Za-z_][A-Za-z0-9_.]*)(?:\")?",
            "table",
        ),
        (
            r"(?i)^\s*CREATE\s+(?:OR\s+REPLACE\s+)?(?:UNIQUE\s+)?INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:\")?([A-Za-z_][A-Za-z0-9_.]*)(?:\")?",
            "index",
        ),
        (
            r"(?i)^\s*CREATE\s+(?:OR\s+REPLACE\s+)?VIEW\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:\")?([A-Za-z_][A-Za-z0-9_.]*)(?:\")?",
            "view",
        ),
        (
