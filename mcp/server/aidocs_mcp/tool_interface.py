...
def _delegate(_tool_name: str, **kwargs: Any) -> Any:
    ...
    def _invoke_direct(direct: Any) -> Any:
        if inspect.iscoroutinefunction(direct):
            ...
            try:
                ...
                # CARRY THE REQUEST CONTEXT ACROSS THE HOP (8d84 phase 2). A
                # plain Thread starts with an EMPTY context, so the impl lost
                # the request's host session id and every per-request
                # ContextVar with it -- its project bind was never looked up
                # and resolve_project_root() fell to cwd discovery. Measured
                # 2026-09-22. anyio.to_thread copies the context; this did not.
                _ctx = contextvars.copy_context()
                t = threading.Thread(target=lambda: _ctx.run(_worker), daemon=True)
                t.start()
                t.join()
    ...
    try:
        ...
        # Same hop, same fix as the direct-dispatch path above: carry the
        # request context into the thread (8d84 phase 2).
        _ctx = contextvars.copy_context()
        t = threading.Thread(target=lambda: _ctx.run(_worker), daemon=True)
        t.start()
        t.join()
...
def ai_lane(
    ...
    standalones fold in here; resume + kill resolve the worker BY LANE):
      ...
      action="activity"   — audit-ledger-backed lane observability:
                            per-worker recent tool events, counts,
...
@tool(
    ...
    modes={
        ...
        "xaacp_send": {"required": ["session_id", "target_actor_id", "lane_id", "message_kind", "body"], "optional": ["correlation_id", "in_reply_to", "metadata", "wake", "ttl_seconds", "confirm_token", "target_project_id"], "desc": "actor-routed XAACP send: deliver a typed message to an exact session/lane actor. target_project_id (a canonical project id, never a path) sends into another project's session; omitted = your own project"},
        "xaacp_inbox": {"required": ["session_id", "lane_id"], "optional": ["unread_only", "pending_only", "mark_read", "limit", "confirm_token"], "desc": "list XAACP entries routed to the calling actor on a session/lane, oldest first. unread_only=True: entries this actor has not read (a message is read once it was surfaced to the actor in a tool result, marked read, or decided by the actor). pending_only=True: incoming with no terminal decision yet. The two compose. limit takes the front of the queue; the reply carries truncated and outstanding_total so a capped read is not mistaken for the whole"},
        ...
        "xaacp_ack": {"required": ["message_id", "session_id"], "optional": ["body", "confirm_token"], "desc": "acknowledge RECEIPT and ownership of a pending XAACP message WITHOUT deciding it; non-terminal, fires once, and the terminal decision still lands later"},
        "xaacp_wait": {"required": ["message_id", "session_id"], "optional": ["timeout_seconds", "confirm_token"], "desc": "block until an XAACP message reaches a terminal decision (or timeout); the reply carries acked_at/acked_by so a timeout is distinguishable from silence"},
        ...
        "xaacp_attach": {"required": ["session_id"], "optional": [], "desc": "make the calling actor addressable in the session it is already bound to; idempotent; binds, selects and grants nothing, and takes no identity arguments (the actor is derived from the request)"},
        "wait_next": {"required": ["session_id"], "optional": ["lane_id", "after_cursor", "timeout_seconds", "confirm_token"], "desc": "block for the next XAACP deliveries to the calling actor, oldest undelivered first; after_cursor is deprecated, ignored for delivery, and accepted for compatibility until removed; the server hands an entry out at most once (not lossless: a response lost in transport is not re-sent); the returned cursor is the last handed-out seq, or on timeout the server watermark; returning does not mark read or decide it"},
    },
)
def ai_msg(
    mode: str,
    ...
    8d84 phase 3: mode='xaacp_send' accepts target_project_id -- the CANONICAL
    project id of another project (never a root path, repo key or slug). The
    send then goes into that project's session_id, and only if the canonical
    cross-project authority allows it: an approved relation, plus the
    xaacp.message_session permission on your side and on the target project
    and target session. Any refusal looks exactly like an unknown target.
    Omitted, or your own project's id, means your own project. No other mode
    takes it: directory, inbox, wait, ack, reply-decision and cancel act on
    your own project, and you follow a cross-project message you sent with
    xaacp_wait / xaacp_cancel on its message_id as usual.
    ...
    require an exact session/lane route. wait_next blocks for the next
    deliveries, oldest undelivered first. after_cursor is deprecated, ignored
    for delivery, and accepted for compatibility until removed. The server
    hands an entry out at most once; this is not lossless (an entry is stamped
    delivered before the response is known to arrive). The returned cursor is
    the last handed-out seq, or on timeout the server watermark; it never
    passes an undelivered entry. Returning does not mark
    read or decide the message.
    Decision status is separate from transport
    truth: a rejected/blocked/canceled decision still returns ok=True.

    DRT-12: mode='xaacp_ack' is the NON-TERMINAL receipt. Every xaacp_reply
    decision (accepted/rejected/completed/blocked) is TERMINAL, so 'accepted'
    cannot mean "I have this and I own the answer" -- the message could then
    never reach 'completed' or 'blocked'. xaacp_ack says exactly that and
    nothing more: it records acked_at/acked_by (+ optional `body`) WITHOUT
    deciding, and the terminal decision still lands on the same message
    afterwards. Three facts, three answers:

        delivered/read  !=  acknowledged responsibility  !=  eventual outcome

    The acking actor is DERIVED from your own route -- there is no parameter for
    it. Only the actor the message is addressed TO may ack. An ACK fires ONCE:
    a second one returns the standing receipt with ack_announced=False. Acking a
    message that already reached a terminal decision is REFUSED
    (status='already_terminal'), and acking never ends your duty to re-arm
    wait_next.

    A lane that is not a seat CAN still report upward. For
    ...
    mode='xaacp_directory' is the DISCOVERY surface — the session-scoped
...
def ai_task(
    ...
    list accepts format='table'|'csv' (opt-in; default structured).
    """
    return _delegate(
        "ai_task",
        mode=mode,
...
@tool(
    ...
    modes={
        ...
        "connect": {"required": [], "optional": ["session_id", "confirm_token"], "desc": "bind this agent to a session (default: most recent active). Two-phase: a call without confirm_token changes nothing and returns the phrase; a call that echoes it applies the bind"},
        ...
        "delete": {"required": ["session_id"], "optional": ["confirm_token"], "desc": "PERMANENTLY delete a session workspace. Requires admin.manage_sessions on the project. Two-phase: a call without confirm_token changes nothing and returns the phrase; a call that echoes it performs the delete"},
...
def ai_project(
    ...
    Connect points the current context at a project that ALREADY exists.
    Enrolling a new project is a different tool: related_project_register
    (make it findable by name) or project_register_from_github_url (clone and
    enroll). Connecting to a project that was never enrolled fails.

    Modes:
      connect  — point this context at an existing project. Local: pass
                 project_root; this host session is re-rooted to that tree
                 for later calls. Gate: pass project_id (a registered id or
                 name); this user's selection moves to it. Two-phase.
      status   — the project this caller is on; with project_id, the trust
                 and index status of that registered project.
      unbind   — local only: drop the connection and return to the
                 directory's own project. Two-phase. Refused on the gate,
                 where a selection is replaced, not cleared.
      sessions — local only: list the sessions of a named project tree
                 (needs an approved cross-project relation and permission).
                 Refused on the gate; use ai_session(mode='list') there.
      list     — list known AIDOCS projects, marking the connected one.

    A local connection is kept per host session and expires after an idle
    period (30 minutes by default); activity refreshes it.

    Args:
        mode: connect | status | unbind | sessions | list.
        project_root: local target, a filesystem tree. Refused on the gate.
        project_id: gate target, a registered project id or name in your
            tenant. Refused locally.
        confirm_token: omit on the first call of connect or unbind. That
            call returns status confirm_required with a token; repeating the
            same call with that token applies it. The token is issued per
            request and is not listed in this schema.
    """
    return _delegate(
        "ai_project",
        mode=mode,
...
def ai_whoami() -> dict:
    ...
    instrument: the X-Aidocs-Host-Session header the shim
...
@tool(
    ...
    modes={
        ...
        "co-enter": {"required": [], "optional": ["session_id", "confirm_token"], "desc": "take the CO-CONDUCTOR seat: dumps the co-conductor role scroll and maps this host session to the 'co_conductor' comms role; does NOT rebind managed mode"},
        "succeed": {"required": ["role"], "optional": ["session_id", "reason"], "desc": "DRT-10 SUCCESSION (not eviction): inherit a seat whose holder is gone — granted only when the incumbent is NOT provably live and its own heartbeat is stale past a grace period the caller cannot shorten; a provably live seat may be transferred only on the order of r0 or above (never by peers), and silent-but-not-yet-stale refuses because unknown is not death. The successor is derived from the caller's XAACP route, never named, and a spawned agent never inherits a seat"},
        "exit": {"required": [], "optional": ["session_id", "role", "reason"], "desc": "leave the seat: clear the inline-conductor marker AND hand back the caller's OWN XAACP seat row (conductor or co_conductor), so the next actor can simply claim it instead of waiting out a succession grace. Releases only the caller's own seat — the holder is derived from the caller's XAACP route and can never be named"},
...
def ai_seat(
    ...
    """Agent seat lifecycle — mode='enter'|'co-enter'|'succeed'|'exit'|'status'|
    'overview' (the impl's real accepted set). enter: become the session
    conductor (verbose=True adds SESSION.md body + journal tail); co-enter: take
    the CO-CONDUCTOR seat; succeed: inherit a seat whose holder is gone (DRT-10,
    requires role); exit: clear the inline-conductor marker; status: is the
    ...
    SUCCESSION IS THE ONLY WAY A HELD SEAT CHANGES HANDS, AND IT IS NOT EVICTION.
    'succeed' exists because a terminal Web-ChatGPT co-conductor conversation
    used to strand `co_conductor` forever — the replacement conversation is a
    different actor and got `occupied` with no remedy. It is granted only when
    the incumbent is NOT provably live AND its own heartbeat is stale past a
    grace period the caller cannot shorten. A PROVABLY LIVE seat is a different
    act: transferring it is an authority transfer only r0 or above may order —
    peers (r0a and r0b are peers) can never authorize it. Silent but not yet
    stale refuses too, because unknown is not death.
    """
    return _delegate(
        "ai_seat", mode=mode, session_id=session_id, verbose=verbose,
        confirm_token=confirm_token, role=role, reason=reason,
    )
...
@tool(
    ...
    modes={
        ...
        "seal": {"required": ["skill_id"], "optional": ["confirm_token"], "desc": "seal a lawbook row from its canonical scroll (operator present-tense grant this turn; two-phase confirm)"},
    },
)
def ai_skill(
    mode: str,
    ...
    """Public empire-skill door — mode='read'|'upsert'|'delete'|'list'|'seal' (the impl's
...
@tool(
    ...
    modes={
        ...
        # #1056: the DIAGNOSIS door. The enforcement surface named a signature
        # and no way to learn what it was; this is the way.
        "diagnose": {"required": ["signature"], "optional": ["session_id"]},
...
def ai_failures(
    ...
    """Failure-stewardship DIAGNOSIS + disposition surface — agent-callable
    CONSUMER of the failure ledger (the Stop hook is the producer). Without
    this an untriaged blocker the agent already fixed could wedge the
    turn-seal — and without mode='diagnose' the agent could not find out
    what the blocker IS.

    mode='diagnose' — ONE failure, by signature or short prefix: nodeid,
    full error type AND message, assertion line, duty + causal origin,
    first/last seen, history, reproducing command. The Stop block
    advertises this call with the signature already substituted.
    mode='list' (default) — failures THIS session owns, BOUNDED: terse rows,
    limit/offset, signature/nodeid filter. The full ledger ships only with
    include_all=True.
...
@tool(
    ...
    modes={
        ...
        "update": {"required": ["id"], "optional": ["content", "title", "append", "status", "priority", "kind", "difficulty", "tags", "reason", "allow_clear"], "desc": "patch a backlog item by id. UNPASSED FIELDS ARE UNTOUCHED — but content is NOT protected: content=<text> REPLACES the whole body and RE-DERIVES the title from its first line, with no history and no undo. To keep the existing reasoning and add to it, pass append=True (adds at the end, title unchanged) — that is the safe primitive for the 'add a dated section' workflow. TO FIX A STALE HEADLINE, pass title=<text>: it patches the title ALONE, leaves the body untouched, survives later appends, and BEATS content's derivation when both are passed. Before it existed the only route to a title was the destructive content= replace, so nobody took it and headlines rotted while the bodies underneath stayed accurate — if you correct an item's body, correct its title in the same call. allow_clear=True is only for content='' . A status change KEEPS merged_into; only status='open' on a legacy status='merged' row un-folds (receipt: unmerged=True) — otherwise use mode='unmerge'"},
        "remove": {"required": ["id"], "optional": ["reason"], "desc": "soft-remove a backlog item by id"},
        "merge": {"required": ["ids"], "optional": ["umbrella_id"], "desc": "merge >=2 items (ANY status) into one umbrella; absorbed rows get merged_into and KEEP their status (only an 'open' item takes status='merged'), reversible via unmerge"},
        "unmerge": {"required": ["id"], "optional": [], "desc": "clear merged_into on a folded item and LEAVE STATUS ALONE; a never-merged id is an honest no-op"},
        "approve": {"required": ["approval_id", "decision", "reason"], "optional": [], "desc": "BETA's DISCHARGE DOOR: rule on ONE backlog-admission obligation. decision=accepted|rejected|blocked — XAACP's own vocabulary, not a rival enum; 'completed' is an XAACP decision and NOT an approval verdict, because done is not approved. reason= is the freetext rationale and is REQUIRED: a bare yes/no deletes the critique the approving seat exists to produce, so a bodiless verdict is malformed and leaves the item UNAPPROVED. IDENTITY IS DERIVED — there is no parameter naming the decider, only the addressed BETA/co_conductor seat may rule, it can never be the actor that asked, and a verdict is ONE-SHOT and is never overturned by re-ruling. An obligation belonging to another domain (a seat succession, say) is REFUSED here: an approval is purpose-scoped"},
        "approval_status": {"required": [], "optional": ["approval_id", "content", "priority", "kind", "difficulty", "tags"], "desc": "THE DIAGNOSIS DOOR: the state of ONE admission obligation, by approval_id (from the refusal) or by the IDENTICAL add payload whose digest keys it. Bounded by construction — it never lists the queue. A paraphrased payload is a DIFFERENT subject and will read as unknown, which is the payload binding working, not a bug"},
        "cutover_status": {"required": [], "optional": [], "desc": "READ-ONLY readiness for the server-authority flip: reports `ready`, the outbox depth, the unclassified-conflict count and their queue_ids, and a `blockers` list naming what to do about each. An outbox depth that could not be DETERMINED blocks too — unknown is not empty, and a flip must not proceed over a queue nobody counted"},
...
def ai_backlog(
    ...
    append: bool = False,
    format: str = "",
    # #1055 BETA lane. No parameter here names the requester or the decider:
    # both identities are DERIVED from the XAACP route, never claimed.
    approval_id: str = "",
    decision: str = "",
) -> dict:
    """Project-owned durable backlog CRUD — mode='add'|'list'|'get'|'update'|
    'remove'|'merge'. Recording (`add`, `update`) and reading (`list`, `get`)
    need no active task; `remove`, `merge` and `unmerge` require one and are
    audited. `merge` folds items into an umbrella (reversible with `unmerge`).
    Same multi-mode read+write consolidator pattern as ai_task's todo modes.
    list accepts format='table'|'csv' (opt-in; default structured JSON).
    kind = what KIND of monster, orthogonal to priority: known-fix |
...
def ai_read_jsonl(
    ...
    since/until: filter rows by their ISO-8601 `timestamp` (ISO, or relative
    '30h'/'2d' back from now); untimed rows are then skipped and counted."""
    return _delegate(
        "ai_read_jsonl",
        path=path,
...
def ai_memory(
    ...
    promote: promote a kingdom entry into the EMPIRE tier (must already exist
      in kingdom memory; portable; non-empty audited reason; admin authority).
        ...
        row's text verbatim. candidates mode lists pending
...
def ai_delete(
    ...
    .TRASH/ recovery area, or restore a trashed file. Two-phase
...
@tool(
    ...
    modes={
        ...
        "reingest_canonical": {"required": [], "optional": ["force", "operator_token"], "desc": "bulk-project every active memory_index row into the palace AND rebuild its KG triples (graph backfill; admin-gated, no dry_run)"},
...
def ai_palace_maintenance(
    ...
    canonical->palace projection plus the KG triple rebuild — no
...
def ai_git(
    ...
    log, diff, add, commit, push, pull, branch, switch, stash}.
    ...
    op=push pushes an explicit `remote` + `refspec` (source:destination);
    pass both or neither (derived from the tracked upstream, reported as
    push_plan). Force/delete/detached/no-upstream shapes are refused. On the
    gate, push also requires the `git.push` capability.
    op=switch REQUIRES `branch`; create=True runs `git switch -c`. Returns a
    receipt (head_branch, head_sha); ok=False unless HEAD is that branch.
    `branch`/`create` on any other op are refused, never ignored.
    op=fetch / op=pull / op=branch (lists) / op=stash.
...
def session_select(session_id: str, confirm_token: str = "") -> dict:
    ...
    call writes/reads against. Two-phase: a first call without
    confirm_token changes nothing and returns confirm_required with a
    summary of the change and a token; the change is applied only by a
    second call that echoes that token back.
    """
...
@tool(
    ...
    tier=R,
    scope="catalog",
    # phrase= is the confirm INTENT folded into the handshake's bind hash
    # (canonical_invocation.gate_confirm_handshake); it is NOT a token a client
    # may echo. Since 2026-09-17 the transport issues a single-use token bound to
    # target project + kind + content digest + target_hint (r0b e54b73a2-675).
    confirm=TWO_PHASE,
...
def dashboard_memory_capture(
    ...
    sqlite-canonical). Two-phase confirm: the first call returns
    confirm_required with a single-use confirm_token bound to this exact write
    (target project, kind, content, target_hint); echo that token verbatim,
    never construct one. Optional project_id targets the VIEWED project
    (OWNER/ADMIN in that project's org). Intent audited before mutation.
    Agents use memory_capture under tier_m_edit instead — this surface never
