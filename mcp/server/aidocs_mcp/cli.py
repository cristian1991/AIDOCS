...
def cmd_runtime(args: list[str]) -> int:
    ...
    if "--reconcile-hook-shim" in args:
        ...
        from .claude_hooks_install import (
            converge_host_hook_commands,
            ensure_hook_shim,
            shim_path,
            shim_source,
        )
        ...
        try:
            ...
            fresh = False
        # TWO AXES, BOTH OF THEM RECOVERY (#925k). Refreshing the shim's BYTES
        # while leaving the host COMMAND pinned to the generation that just got
        # superseded fixes the half that still runs and ignores the half that
        # is about to stop running at all. This subcommand is invoked in a
        # fresh process right after a pointer flip — the exact moment the
        # installer is not running and no hook has fired yet, so it is the only
        # thing standing between a collected generation and a dead gate.
        host = converge_host_hook_commands()
        rep = {
            "ok": fresh and host["ok"],
        ...
        if as_json:
            ...
            if host["changed"]:
                print(
                    "host hook command converged onto the stable launcher: "
                    f"{host['path']}",
                )
            elif not host["ok"]:
                print(
                    f"host hook command NOT converged: {host['reason']} — an "
                    "entry naming a collected generation cannot start at all.",
                )
        else:
...
def cmd_service(args: list[str]) -> int:
    ...
    def _spawn_watchdog() -> int:
        ...
        # The advice printed here until 2026-09-24 was "/mcp reconnect in EVERY
        # open window" (#833 layer 3: the shim captured the conversation id at
        # spawn). The window spine (#880) retired that: identity is the stable
        # window key stamped by the SessionStart/SubagentStart hooks, and the
        # shim reaches the daemon on its stable port.
        print(
            "open windows keep working: no `/mcp reconnect` is needed after a "
            "service restart (identity is the stable window key, #880). "
            "Reconnect only after the stdio shim itself is updated."
        )
        return 0
...
def cmd_setup(args: list[str]) -> int:
    ...
    # TWO INTERPRETERS ONCE GENERATIONS EXIST (#925k). `python_path` above is
    # the runtime setup VERIFIED and recorded trust for — the code that
    # governs. The host entry names a launcher that outlives generation GC, and
    # the shim rejoins them by reading the activation pointer at every call.
    # Printing only one of the two is how the split went unnoticed.
    _launcher = _hook_res.get("launcher") or ""
    if _hook_res.get("ok") and _launcher and _launcher != str(python_path).replace("\\", "/"):
        print(
            f"  \033[2mhook launcher: {_launcher} (stable); governing runtime: "
            f"{python_path} — the shim follows the activation pointer\033[0m",
        )
    if _hook_res.get("ok") and not _hook_res.get("stable_entry", True):
        print(
            "  \033[33m!\033[0m the hook entry names a runtime GENERATION; "
            "garbage-collecting it will stop hooks from launching at all. "
            "Provision the base runtime (`aidocs runtime --fix`).",
        )
    ...
    _labels = {
        # NOT an equality any more (#1047): the Claude entry names a stable
        # launcher that outlives generation GC and reaches the governing runtime
        # through the fail-closed shim. Saying "==" here taught every reader the
        # repealed invariant — including the proof itself, which kept enforcing
        # it after #925k made it false.
        "path_equality": "every adapter executes the governing runtime",
...
def main() -> None:
    _force_utf8_stdout()
    # #1095: install-wide startup steps (legacy known-projects registry merge)
    # run at process entry -- the CLI and the watchdog it hosts read the
    # registry, and no read migrates it. Never raises.
