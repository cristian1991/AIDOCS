...
def _invalidate_action_token_cache() -> None:
    """Clear the action token cache so next load re-reads from disk."""
    global _action_token_cache
    _action_token_cache = None
...
def classify_action(text: str) -> dict[str, str]:
    ...
    tokens = _load_action_token_lists()
    for action_kind, token_list in tokens.items():
        if action_kind.startswith("__"):
            continue
        if any(token in lower for token in token_list):
...
def _load_action_token_lists(
    ...
    global _action_token_cache
    # Only use cache when loading defaults with all languages
    if _action_token_cache is not None and directory is None and enabled_languages == "all":
    ...
    for toml_file in sorted(root.glob("*.toml")):
        if enabled_set is not None and toml_file.stem not in enabled_set:
            continue
    ...
    for yaml_file in sorted(root.glob("*.yaml")):
        ...
        try:
            ...
            continue
    if directory is None and enabled_languages == "all":
