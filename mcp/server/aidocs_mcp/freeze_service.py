...
def get_existing_freeze(
    ...
    Thin wrapper over SessionFreezeStore. Centralized here so callers
    don't need to import the storage class. Failure swallowed — return
    None on any storage error so the caller can decide whether to
    fall through to other gates.
    """
    try:
        from .session_freeze_store import SessionFreezeStore

        # Per-agent: resolve the CALLING agent's host when not supplied, so the
        # check matches that agent's freeze (get_active_freeze also matches the
        # session-wide '' freeze, which still blocks everyone).
        ...
        if not host:
            try:
                ...
                host = ""
        # Operator-wide freeze: when enabled, resolve the calling operator so a
        # malicious-intent freeze minted in ANY of their sessions blocks this one
        # too. Empty (no login) -> per-session, today's behavior.
        operator_user_id = ""
        try:
...
def _render_strike_trail(
    ...
    *,
    ...
    """Render the recent security-violation strikes for a session as a
    short audit trail to append to the freeze deny envelope. Returns
    "" on any lookup error (best-effort — the freeze envelope must
    NEVER fail to render because of a strike-list issue).

    LEARNING SURFACE (2026-05-27 step 2): every tool call against a
    frozen session shows the agent which verb-and-target attempts
    landed it here.
    """
    ...
    try:
        ...
        with ex.connect(project_root) as conn:
            # ORDER BY rowid DESC for deterministic recency — see SVS
            # get_recent_strikes for the rationale.
            rows = conn.execute(
    ...
    for r in rows:
        try:
            pl: dict[str, Any] = _json.loads(r["payload_json"] or "{}")
        except Exception:
            pl = {}
        name = str(pl.get("tool_name") or "")
...
def build_existing_freeze_response(
    ...
    *something* did.
    ...
    if freeze.kind in ("repeated_security_violation", "hostile_operator_prompt"):
        ...
        trail = _render_strike_trail(project_root, session_id)
        if trail:
            rendered = rendered + "\n" + trail
        return {
