...
export function useDashboardData() {
  ...
  // Change-detection refresh (replaces the old blind 2s full-snapshot poll that
  // spawned cli-dashboard every tick — the process storm + refresh stutter).
  // Every 2s read a CHEAP sqlite-derived live cursor (Rust, NO python spawn) and
  // pull the full snapshot ONLY when it changes; plus a slow safety refresh every
  // ~30s so static sections (repo summary, config) still catch up. Visible-only;
  // skips while a fetch is in flight.
  useEffect(() => {
    let lastCursor = "";
    let sinceFullMs = 0;
    let stopped = false;
