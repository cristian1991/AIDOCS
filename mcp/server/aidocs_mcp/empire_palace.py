...
The palace remains a REBUILDABLE PROJECTION (doctrine XXII): canonical
...
def _public_empire_skill_rows() -> list[dict]:
    ...
    Soul exclusion by query (§XI): the WHERE clause admits ONLY
    ...
    try:
        ...
        # which is what "Read-only; never creates the empire DB" above asks
        # for; row_factory=sqlite3.Row is its default.
        conn = _canonical_connect(db, read_only=True)
        try:
