...
# #755/#756: the ONE canonical connect. The single site was
...
# Symbol lookup is pure SELECT over the code index, so read_only=True.
        ...
        *,
        ...
        try:
            ...
            with _canonical_connect(str(db), read_only=True, row_factory=False) as conn:
                # #481: no LEAD() here — "next outline row - 1" truncates a
                # function's span at its first nested def/closure, and a naive
                # name-membership containment test breaks members named after
                # their container. The span is computed below against the
                # path's full outline using the shared scope-chain law.
                base = (
                ...
                for p in sorted({str(r[0]) for r in rows}):
                    outlines[p] = [
                        (int(o[0]), str(o[1] or ""))
                        for o in conn.execute(
                            "SELECT line_number, COALESCE(container, '') "
                            "FROM code_outlines WHERE path = ? ORDER BY line_number",
                            (p,),
                        )
                    ]
                for path, symbol, raw_kind, container, line_start, line_count in rows:
                    ...
                    for ln, cont in outlines.get(str(path), []):
                        if ln <= line_start:
                            continue
                        if row_is_inside_scope(cont, container, str(symbol)):
                            continue  # still inside this symbol's body (#481)
                        line_end = max(line_start, ln - 1)
                        break
                    out.append(
                        {
