...
class RuntimeService:
    ...
    def _task_actor_identity(self, project_root: Path) -> tuple[str, str, bool]:
        ...
        """
        from .task_actor_identity import UnprovenActorIdentity, resolve_slot_actor

        actor, lane, is_worker = resolve_slot_actor(project_root)
        # THE LIFECYCLE MUST NOT CREATE ACTOR-OWNED STATE FOR A REFUSED CALLER
        # (#1043). This triple is what task_begin/update/complete write rows
        # UNDER — the task slot, todo ownership, lane binding and event
        # attribution all key on it. A caller we have POSITIVE evidence is a
        # subagent, whose exact axis is absent, has no id to own anything with:
        # writing a row under the refusal value would put unattributable work
        # into the very tables this seam exists to attribute, and a later
        # correctly-stamped subagent would find state it never created.
        #
        # Raised rather than returned because a 3-tuple of strings has no value
        # that means "no identity": "" is read as legacy consent and anything
        # else is read as an actor. See UnprovenActorIdentity.
        if UnprovenActorIdentity.is_refused(actor):
            raise UnprovenActorIdentity(
                "this call carries worker/subagent evidence but no agent "
                "identity, so no task, todo or lane row can be attributed to it "
                "[rule_id=unproven_subagent_identity]. Re-spawn the subagent so "
                "the spawn path stamps its agent id."
            )
        return actor, lane, is_worker
    ...
    def task_status(
        ...
        """
        # THE ONE LIFECYCLE OPERATION THE REFUSED STATE MAY STILL REACH
        # (#1043b). task_begin/update/complete CREATE or MUTATE actor-owned
        # state and must raise. `task_status` creates nothing — and a caller
        # that cannot be identified still has to be able to find out WHY it is
        # being refused, or the state is unrecoverable from inside (the #726
        # self-sealing shape).
        #
        # It answers from the REFUSAL ITSELF, not from any store: falling
        # through with a blanked actor would read the SESSION sections, i.e.
        # report the conductor's task to a subagent as though it were its own —
        # the same borrow by a quieter route. Diagnosis must not be a read
        # channel onto somebody else's state.
        from .task_actor_identity import UnprovenActorIdentity

        try:
            actor_id, lane_id, is_worker = self._task_actor_identity(project_root)
        except UnprovenActorIdentity as exc:
            return {
                "ok": False,
                "session_id": session_id,
                "error": "unproven_subagent_identity",
                "detail": str(exc),
                "task": None,
                "remedy": (
                    "Re-spawn this subagent so the spawn path stamps its agent "
                    "id. Until then it owns no task and can begin none."
                ),
            }
        if is_worker:
    ...
    def auto_task_begin(
        ...
        *,
        ...
        # REFUSE BEFORE OPENING ANYTHING (#1043b). The actor-slot write below
        # is wrapped in a best-effort `except Exception`, so the refusal raised
        # by `_task_actor_identity` is swallowed there — and by then the
        # SESSION-LEVEL task already exists. That leaves precisely the unowned
        # task #599 was written to abolish, opened on behalf of a caller we
        # have positive evidence is a subagent and cannot name.
        #
        # Checked here, where it can still decline to act at all. Returning None
        # is this method's established "no auto-task" answer, so the UPS path
        # degrades exactly as it does when a task is already active.
        try:
            self._task_actor_identity(project_root)
        except UnprovenActorIdentity:
            return None
    ...
    def task_complete(
        ...
        if actor_id:
            try:
                ...
                if _actor_row and str(_actor_row.get("status") or "") == "active":
                    ...
                    _actor_slot_goal = str(_actor_row.get("goal") or "")
            except Exception:
                _actor_slot_task = ""
                _actor_slot_lane = ""
                _actor_slot_goal = ""
        ...
        if actor_id and _actor_slot_task:
            try:
                self._worker_task_store().set(
                    ...
                    # #1039: keep the goal on the closed row — the next
                    # no_active_task refusal prefills task_begin with it.
                    state={"goal": _actor_slot_goal, "result_summary": result_summary},
        ...
        try:
            ...
            pass
        # ── #1046: CLOSE THE BLAST ENVELOPE AT A LIFECYCLE BOUNDARY ──────────
        #
        # Phase 1 shipped `close_envelope` with NO production caller, so an
        # envelope accumulated for the life of a lineage and the carry-forward
        # path existed only in tests. An envelope that never closes is a ledger
        # that grows without bound and a late trivial edit inheriting
        # obligations from an early runtime change — #1046 amendment 2 names the
        # boundary as an AIDOCS lifecycle boundary, and THIS is the one that
        # means "a unit of work finished".
        #
        # Closing CARRIES unresolved obligations into the next envelope rather
        # than dropping them (amendment 4): completing a task settles the
        # accounting, it does not absolve what was never accounted for.
        #
        # Fail-quiet and after the task is already closed: a ledger problem must
        # never be able to fail a completion that has otherwise succeeded.
        try:
            from . import blast_envelope as _blast

            _blast.close_envelope(project_root)
        except Exception:
            pass
        session = self.hub.sessions.read_session(project_root, session_id)
