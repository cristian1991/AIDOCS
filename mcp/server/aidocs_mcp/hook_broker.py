...
ONE LOGIC, ONE HOME (Article VI): ``evaluate_hook_event`` wraps
...
def _record_timing(
    ...
    *,
    ...
    ``sqlite`` (#489 phase 1) is the evaluation's chokepoint ledger: time in
    SQLite calls per database file, stall time (single calls >= STALL_MS, the
    lock-wait shape), busy-timeout ``locked`` failures, and the first stalls.
    Absent means NOT MEASURED (a drop row, or a pre-instrument broker), never 0.
    ...
    try:
        ...
        if isinstance(sqlite, dict):
            extra = {
                "sqlite_ms": round(float(sqlite.get("sqlite_ms") or 0.0), 1),
                "lock_wait_ms": round(float(sqlite.get("stall_ms") or 0.0), 1),
                "locked": int(sqlite.get("locked") or 0),
                "lock_waits": {
                    db: {k: (round(v, 1) if isinstance(v, float) else v) for k, v in slot.items()}
                    for db, slot in (sqlite.get("by_db") or {}).items()
                    if slot.get("stall_ms") or slot.get("locked")
                },
                "stalls": list(sqlite.get("stalls") or [])[:8],
            }
...
class HookBroker:
    ...
    def _write_state_file(self) -> None:
        ...
        payload = {
            ...
            # #1030: WHICH GENERATION these bytes came from, read off the IMPORT
            # PATH at construction — never off the pointer, which can move
            # between spawn and first breath (see `_generation_loaded`).
            #
            # This is the child's ATTESTATION, and it is the only thing that can
            # prove a replacement actually loaded the generation its parent
            # intended. The parent cannot answer this for it: the watchdog lives
            # in an IMMUTABLE generation directory of its own, so every question
            # it asks about its own tree returns the same answer forever. A
            # supervisor that judges a child's freshness by its own unchanging
            # __file__ can never adopt anything new.
            #
            # JSON null when the load path names no generation — a legacy
            # single-tree install, a real and stable state. Null is UNKNOWN to a
            # reader and must never compare equal to an intended id.
            "runtime_generation": (
                self._generation if isinstance(self._generation, str) else None
            ),
        }
    ...
    def _process(self, line: bytes) -> dict:
        ...
        if kind == TIMINGS_KIND:
            ...
            try:
                from . import hook_write_behind as _hwb  # noqa: PLC0415

                _write_behind = _hwb.stats()
            except Exception as exc:  # noqa: BLE001 — unknown is not zero
                _write_behind = {"unavailable": f"{type(exc).__name__}: {exc}"}
            return {
                "v": PROTOCOL_VERSION,
                "ok": True,
                "summary": summarize_timings(rows),
                # #489: the non-authoritative telemetry written behind hook
                # evaluations. dropped/failed are the numbers to read.
                "write_behind": _write_behind,
        ...
        with self._eval_gate.hold(_key, exclusive=_exclusive):
            ...
            from . import hook_write_behind as _hwb  # noqa: PLC0415
            from ._sqlite_connect import sqlite_wait_ledger  # noqa: PLC0415

            # broker_evaluation(): this long-lived process may write telemetry
            # behind the verdict; a cold per-event interpreter never may.
            with sqlite_wait_ledger() as _sqlite_ledger, _hwb.broker_evaluation():
