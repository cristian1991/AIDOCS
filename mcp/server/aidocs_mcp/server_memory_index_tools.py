...
def _diagnose_unresolvable(target: str) -> dict[str, str] | None:
    """Name WHY a target could not be dereferenced, and what would work —
    or ``None`` when silence is the DELIBERATE answer.

    Session law (operator 2026-07-29): "nothing surfaces without a handle
    the surfacing tool itself can resolve; a failed dereference is an
    ERROR, never a silent drop." ai_memory(search) emits three identifier
    shapes this tool cannot resolve, so each gets a named reason instead
    of an absent key.

    THE LINE (and why it is drawn here): loud for targets in the WRONG ID
    SPACE — a palace drawer_id, an empire-tier id, an absolute filesystem
    path. Those can NEVER resolve through this tool, so silence is a bug.
    SILENT for a well-formed project-relative path: its absence is the
    retired-memory suppression channel, and the security gate deliberately
    does not disclose whether such a path is superseded, removed, or simply
    misspelled (pinned by tests/host/test_memory_read_inactive_gate.py —
    ``out == {}``). Disclosing "this path exists but is retired" would
    weaken that gate, and a diagnostic is never worth a gate.
    """
    t = str(target or "").strip()
    if t.startswith("drawer_") or t.startswith("memdrawer:"):
        return {
            "target": t,
            "reason": "drawer_id_not_addressable",
            "hint": (
                "palace drawer ids are internal: RFC-4 §8.4 keeps drawer "
                "bodies off the tool surface and no agent tool accepts one. "
                "Read the canonical memory path from the row's `path` field "
                "instead (project-relative, under .MEMORY/)."
            ),
        }
    if t.startswith("empire:"):
        return {
            "target": t,
            "reason": "empire_tier_id_not_addressable",
            "hint": (
                "empire-tier law/skill ids live in the machine-global empire "
                "store, not this project's memory_index. ai_memory(read) "
                "resolves kingdom .MEMORY/ paths only."
            ),
        }
    if t.startswith("/") or t.startswith("\\") or (len(t) > 2 and t[1] == ":"):
        return {
            "target": t,
            "reason": "absolute_path_not_canonical",
            "hint": (
                "pass the project-relative canonical memory path (e.g. "
                "'rules/communication.md'), not an absolute filesystem path."
            ),
        }
    return None  # well-formed path: suppression is the gate's to disclose


def annotate_unresolved_targets(
    result: dict[str, Any],
    targets: list[str],
) -> dict[str, Any]:
    """Add an explicit ``_unresolved`` entry for every requested target the
    store did not return. Resolvable targets pass through UNTOUCHED, and a
    fully-resolvable read is returned byte-identically (no diagnostic noise
    on the happy path).
    """
    try:
        ...
        if not missing:
            return result
        return {**result, _UNRESOLVED_KEY: missing}
...
def gated_memory_read(
    ...
    *,
    ...
    Path keys map to body strings; the reserved ``_unresolved`` key (present
    only when a wrong-id-space target was requested) maps to a list of
    diagnosis dicts — hence ``dict[str, Any]`` rather than ``dict[str, str]``.
    ...
    # ONE RESOLVER (DRT-06): the receipt binds a hash of the body handed over,
    # and the edit gate re-derives the CURRENT body to detect amendment. If
    # this door and that verifier resolved bodies differently, every receipt
    # would read as stale forever. The logic moved to memory_read_receipts so
    # there is a single place for it to be right.
    from .memory_read_receipts import resolve_palace_reader

    palace_reader = resolve_palace_reader(hub, project_root)
    result = hub.memory.read_memory(
        project_root,
        targets,
        include_inactive=effective_inactive,
        palace=palace_reader,
    )
    # DRT-06: MINT THE READ RECEIPT HERE, and nowhere else.
    #
    # This is the single seam where a memory BODY is handed to an agent, so it
    # is the only place that can honestly say "this actor has now seen this
    # content". The edit gate used to accept a retyped route_id as proof of
    # reading; it now requires one of these rows. Minted only for targets the
    # store actually RETURNED -- a suppressed (retired) target mints nothing,
    # because nothing was read. Never raises: see mint_memory_read_receipts.
    try:
        from .memory_read_receipts import mint_memory_read_receipts

        mint_memory_read_receipts(
            hub,
            project_root,
            {
                k: v
                for k, v in (result or {}).items()
                if k != _UNRESOLVED_KEY and isinstance(v, str)
            },
            session_id=session_id,
            source_tool="memory_read",
        )
    except Exception:  # noqa: BLE001 - a failed receipt write must not fail the read
        pass
    # LOUD DEREFERENCE (session law, operator 2026-07-29): a target the
    # store could not resolve becomes an explicit, reason-bearing entry.
    # Previously it was simply an absent key, so an agent handed a
    # drawer_id / empire: id / absolute path by ai_memory(search) got a
    # thinner dict with no signal that anything had failed.
    return annotate_unresolved_targets(result, targets)
...
def merged_memory_search(
    ...
    §XVIII drain precondition ('verify discovery from the new home before
    ...
    *,
    ...
    def memory_read(targets: list[str], include_inactive: bool = False) -> Any:
        ...
        try:
            ...
            session_id = resolve_managed_session(hub.managed_mode, root) or None
        except Exception:
            session_id = None
    ...
    _ti_c20_memory.register_impl("memory_read", memory_read)
    _ti_c20_memory.register_impl("memory_search", memory_search)
    ...
    def ai_memory(
        ...
        """Unified project memory — capture/read/search/promote plus laddered global-law retirement.
        ...
        retire_request: stage retirement while the law remains active.
        retire_decide: a different super-admin approves or denies it.
        """
    ...
    def ai_index_sync(include_tests: bool = False, timeout: int | None = None) -> dict[str, Any]:
        ...
        # #1080 (diagnosability): every raising stage below used to propagate to the
        # MCP tool boundary, which logs a bare "Error calling tool 'ai_index_sync'"
        # with no exception type and no traceback — neither the operator nor a
        # reviewer could diagnose a production failure. Each stage now returns the
        # SAME result["error"] contract the postcondition failures already use, so
        # every existing caller that checks for "error" keeps working, plus
        # structured fields naming the stage and the exception type. We emit only
        # the exception TYPE and str() of the exception — never environment,
        # never locals, never arbitrary state. Two separate controls on str(exc):
        #   * REDACTION: the message is passed through the project's credential
        #     guard (output_guard.scrub_persisted_text), which masks
        #     credential-SHAPED spans. It is pattern-based: a sensitive value
        #     that does not look like a credential is NOT caught.
        #   * _MSG_CAP is a BLAST/SIZE bound only — it limits how much text one
        #     failure can emit; it is not sanitization of any kind.
        _MSG_CAP = 500
        ...
        def _safe_msg(exc: BaseException) -> str:
            ...
            try:
                ...
                text = "<unprintable exception message>"
            text = scrub_persisted_text(text)
            if len(text) > _MSG_CAP:
                text = text[:_MSG_CAP] + f"...<truncated, {len(text)} chars>"
            return text

        def _stage_failure(stage: str, exc: BaseException, db: str | None) -> dict[str, Any]:
            exc_type = type(exc).__name__
            msg = _safe_msg(exc)
            result["error"] = (
                f"index_sync_failed: stage={stage} {exc_type}: {msg}"
                + (f" (db {db})" if db else "")
            )
            result["error_key"] = "index_sync_failed"
        ...
        try:
            ...
            return _stage_failure("db_path", exc, None)

        try:
            validation = validate_language_descriptors(project_root)
        except Exception as exc:
            return _stage_failure("validate_language_descriptors", exc, db_path)
        descriptor_issues = validation.get("issues", [])

        def _durable_code_files_count() -> int:
            try:
                # read_only=True: this is the POSTCONDITION read that decides
                # whether sync_code_files told the truth, so it must never
                # write and must never MATERIALISE a missing database — a read
                # that creates an empty file would report 0 rows as a fact
                # instead of surfacing the absent index. Durability is moot
                # for a reader; the three per-connection pragmas it had none
                # of (synchronous, busy_timeout, foreign_keys) are the gain.
                c = _canonical_connect(db_path, read_only=True, row_factory=False)
        ...
        try:
            ...
            return _stage_failure("sync_code_files", exc, db_path)
        ...
        try:
            ...
            return _stage_failure("sync_modules", exc, db_path)
        ...
        try:
            ...
            result["dnt_registry"] = {
                ...
                "error_message": _safe_msg(exc),
                "error": f"dnt sync failed: {type(exc).__name__}: {_safe_msg(exc)}",
            }
        ...
        try:
            ...
            # #1080: still best-effort (never fails the sync), but no longer
            # SILENT — a swallowed stamp failure used to leave no trace at all.
            result["reconcile_stamp"] = {
                "ok": False,
                "stage": "reconcile_stamp",
                "error_type": type(exc).__name__,
                "error_message": _safe_msg(exc),
            }
        return result
