...
implements the CLIENT side on the EXISTING ``SyncTransport`` seam (§VI
...
server-side (§VI). Per the war brief, the server half ships as this spec:
...
def reconcile_outbox(
    ...
    summary = {
        ...
        "accepted": 0,
        "rejected": 0,
        # WHY a rejection happened, not just how many. r0b ruling 2026-09-17
        # ("a dying rail must fail truthfully during retirement"): the server
        # returns a per-event reason and this summary used to fold every one of
        # them into the count above and drop it. Measured that day: 230
        # submitted, 230 rejected, and the operator's only signal was the number
        # 230 — on a rail that had NEVER accepted an event, for two reasons
        # (empty project_id, then unauthorized_actor) that the server had been
        # naming correctly the whole time. Hours went into a question the
        ...
        for stream in STREAMS:
            ...
            for rec in transport.submit_events(events):
                if rec.accepted:
                    ...
                    summary["rejected"] += 1
                    _why = str(getattr(rec, "reason", "") or "rejected")
                    summary["rejected_reasons"][_why] = (
                        summary["rejected_reasons"].get(_why, 0) + 1
                    )
