...
PROJECT_TOOL_SPECS: dict[str, dict] = {
    ...
    "org_select": {
        "desc": (
            ...
            "one. Two-phase: a first call without confirm_token changes nothing and "
            "returns confirm_required with a summary of the change and a token; the "
            "change is applied only by a second call that echoes that token back."
        ),
        "schema": {
            "type": "object",
            "properties": {
    ...
    "project_select": {
        "desc": (
            ...
            "Two-phase: a first call without confirm_token changes "
            "nothing and returns confirm_required with a summary of "
            "the change and a token; the change is applied only by a "
    ...
    "project_grant_member": {
        "desc": (
            ...
            "must belong to THIS org (cross-org grants are refused). Pass "
            "capability='git.push' to grant that member an explicit push grant on "
            "the project (they must already have project access)."
        ),
        "schema": {
            "type": "object",
            "properties": {
                ...
                "project_id": {"type": "string"},
                "capability": {"type": "string", "enum": ["git.push"]},
            },
    ...
    "project_revoke_member": {
        "desc": (
            ...
            "from their allowlist). Pass capability='git.push' to revoke only the "
            "member's explicit push grant."
        ),
        "schema": {
            "type": "object",
            "properties": {
    ...
    "dashboard_memory_capture": {
        "desc": (
            ...
            "confirm_required with a single-use confirm_token bound to this exact "
            "write (target project, kind, content, target_hint); echo that token "
            "verbatim, never construct one. Optional project_id targets the VIEWED "
            "project (OWNER/ADMIN in that project's org). Intent audited before "
            "mutation. Agents use memory_capture under tier_m_edit instead — this "
            "surface never widens that boundary."
        ),
        "schema": {
            "type": "object",
            "properties": {
    ...
    "dashboard_operator_message": {
        "desc": (
            "Operator dashboard only: send an operator message to the bound session's "
            "conductor/co-conductor seat as ONE origin='operator' XAACP stream entry. "
            "Principal/session are server-derived; the caller must be the session's "
            "bound operator; the UPS safety head runs first; idempotent on "
            "client_message_id."
        ),
        "schema": {
            "type": "object",
            "properties": {
                "to_roles": {"type": "array", "items": {"type": "string"}},
                "body": {"type": "string"},
                "client_message_id": {"type": "string"},
            },
            "required": ["to_roles", "body", "client_message_id"],
        },
        "cls": CLASS_SELECTOR,
    },
    "freeze_list": {
    ...
    "session_select": {
        "desc": (
            ...
            "against. Two-phase: a first call without confirm_token "
            "changes nothing and returns confirm_required with a "
            "summary of the change and a token; the change is applied "
            "only by a second call that echoes that token back."
        ),
        "schema": {
            "type": "object",
            "properties": {
...
ORG_ADMIN_DASHBOARD_TOOLS: frozenset[str] = frozenset(
    {
        ...
        "freeze_list",
        "escalation_list",
        # #1061: dashboard operator message -> XAACP stream (bound operator only).
        "dashboard_operator_message",
        "tauri_backlog_list",
        "tauri_todo_list",
        "dashboard_snapshot",
        "dashboard_view",
        "config_view",
        # #1060 v2: the project ACL door (grant incl. capability=git.push).
        # Hidden from agents; routable ONLY for org OWNER/ADMIN. The handler
        # (_ogt_pt_project_acl) re-checks is_org_admin itself.
        "project_grant_member",
        "project_revoke_member",
        "project_acl_list",
    }
...
# required to administer the org's own panels). Empire-doctrine §XIV: the
