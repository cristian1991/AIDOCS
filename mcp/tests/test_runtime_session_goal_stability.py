...
def _make_runtime_with_session(tmp_path: Path, goal: str = "Ship host hooks") -> tuple[RuntimeService, Path, str]:
    ...
    hub.sessions.create_session(project_root, session_id, "Runtime stability", "Agent", goal)
...
def test_task_begin_does_not_overwrite_session_goal_when_working_subtask_starts(tmp_path: Path) -> None:
    runtime, project_root, session_id = _make_runtime_with_session(tmp_path, goal="Ship host hooks")
    ...
    assert any("Fix one failing test" in line for line in started["session"]["sections"]["State"])
    assert any("Fix one failing test" in line for line in started["plan"]["sections"]["Current State"])
...
def test_task_update_preserves_original_session_purpose(tmp_path: Path) -> None:
    ...
    updated = runtime.task_update(project_root, session_id, state=["Investigating query gate"])

    assert updated["plan"]["sections"]["Purpose"][0] == "- Ship host hooks"
    assert updated["plan"]["sections"]["Current State"][0] == "- Investigating query gate"
...
def test_task_complete_updates_execution_state_without_rewriting_high_level_goal(tmp_path: Path) -> None:
    ...
    completed = runtime.task_complete(project_root, session_id, result_summary="Fixed query gate")

    assert completed["session"]["sections"]["Goal"][0] == "- Ship host hooks"
    assert completed["plan"]["sections"]["Purpose"][0] == "- Ship host hooks"
    assert any("Fixed query gate" in line for line in completed["session"]["sections"]["State"])
    assert any("Completion result: Fixed query gate" in line for line in completed["plan"]["sections"]["Validation"])
