...
class ManagedModeService:
    ...
    def get_mode(
        ...
        *,
        ...
        strict=True (WebMCP tool-authorization, #253 empire §XIV): resolve ONLY the
    ...
    def _resolve_mode(
        ...
        *,
        ...
        # #1095: STRUCTURAL SCHEMA ONLY on this read. init_db also ingests +
        # DELETES the legacy aidocs-managed.json; that migration belongs to
        # the write path (set_mode / clear / unbind) and the SessionStart
        # lifecycle rail (migrate_legacy). SQLite is the sole authority: the
        # legacy file is never read back, so no row reads as inactive.
        try:
            self._store.ensure_schema(project_root)
        ...
        # Gate-strict (#253 empire §XIV): WebMCP tool-authorization resolves ONLY
