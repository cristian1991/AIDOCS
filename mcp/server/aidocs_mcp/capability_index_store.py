...
class CapabilityIndexStore(SQLiteIndexStoreBase):
    ...
    _EXPLICIT_METADATA: dict[str, dict[str, object]] = {
        ...
        # bootstrap probe; project_bootstrap_or_resume covers the
        # first-prompt flow; ai_session(mode='connect') handles binds.
        "project_bootstrap_or_resume": {
            "aliases": ["bootstrap_project", "resume_project"],
            "family": "bootstrap",
        },
        "workflow_actions_compile": {
            "aliases": ["compile_workflow", "compile_workflow_actions"],
            "family": "workflow",
        },
        "workflow_actions_get": {
            "aliases": ["read_workflow_actions"],
            "family": "workflow",
        },
        "action_surface_compare": {
            "aliases": ["should_can_did", "compare_action_surface"],
            "family": "analysis",
        },
    }
    ...
    def _serialize_tool(self, tool: Any) -> dict[str, Any]:
        ...
        timeout_seconds = (
            float(timeout.total_seconds()) if hasattr(timeout, "total_seconds") else None
        )
        payload = {
        ...
        return {
            ...
            "meta_json": json.dumps(payload["meta"], sort_keys=True, default=str)
