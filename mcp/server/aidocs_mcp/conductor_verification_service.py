...
class ConductorVerificationService:
    ...
    def verify_lane(
        ...
        if not commands:
            ...
            self.runtime._conductor_state.transition_lane(
                project_root, session_id, lane_id, LaneState.COMPLETED
            )
            self.runtime._conductor_dispatch.store_lane_result(
                project_root, session_id, lane_id, {"success": True}
            )
            return {
                "passed": True,
                "commands_run": [],
        ...
        if passed:
            ...
            self.runtime._conductor_dispatch.store_lane_result(
                project_root, session_id, lane_id, {"success": True, "commands_run": [r.get("command", "") for r in results]}
            )
        else:
            self.runtime._conductor_state.transition_lane(
                project_root, session_id, lane_id, LaneState.REOPENED
            )
    ...
    def verify_full_suite(
        ...
        conductor = PlanConductor(
            self.hub, project_root, session_id,
    ...
    def validate_agent_output(
        ...
        if require_tests:
            test_evidence = packet_result.get("test_evidence") or packet_result.get("verification_results")
            if not test_evidence:
                ...
                for result_text in command_results:
                    if isinstance(result_text, str) and any(kw in result_text.lower() for kw in ("failed", "error", "failure")):
                        issues.append({
    ...
    def _require_agent_tests(self, project_root: Path) -> bool:
        ...
        try:
            ...
            return bool(get_setting("conductor.require_agent_tests", project_root=project_root, default=False))
        except Exception:
            return False
    ...
    def _run_command(
        self, project_root: Path, command: str, timeout: int = 120
    ) -> dict[str, object]:
