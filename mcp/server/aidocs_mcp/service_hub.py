...
class AidocsServiceHub:
    ...
    def __init__(self, templates_root: Path | None, script_root: Path | None = None) -> None:
        # templates_root is Optional since #628: the canonical resolver returns
        # None rather than fabricating a path when no real template tree exists
        # (installed runtime with no checkout beside it).
        from .host_services.path_resolver_service import (
            resolve_script_root as _canonical_script_root,
        )
        ...
        self.updater = UpdaterService(
            # #628: templates_root may be None (no real tree resolvable), so the
            # derived script root falls back to the canonical script resolver
            # instead of dereferencing None.
            script_root=script_root
            or (
                templates_root.parents[2] / "scripts"
                if templates_root is not None
                else _canonical_script_root()
            ),
        )
    ...
    def require_permission(
        ...
        *,
        ...
        # #576: roll the permission CATALOG forward before resolving.
        # A permission added to permission_catalog.ALL_PERMISSIONS must
        # become reachable on an existing install with no manual step —
        # otherwise `memory.set_global_law` (and every future entry)
        # stays refused for a proven super_admin because the row simply
        # was never written. Roll-forward ONLY: a virgin store is left
        # virgin, so the pre-RBAC bootstrap bypass below is unchanged.
        # Memoised per process per identity DB; never raises; never an
        # input to the verdict.
        try:
            from .permission_catalog import ensure_permission_catalog_current

            ensure_permission_catalog_current(project_root, store=self.rbac)
        except Exception:
            pass
        ...
        #
        # ATTRIBUTION IS AUTHORITY (#1027): an audit row filed against a
        # session that does not exist is worse than one filed against none,
        # so this asks the resolver rather than trusting `active`.
        from .managed_mode_service import resolve_managed_session

        session_id = resolve_managed_session(self.managed_mode, project_root) or None
