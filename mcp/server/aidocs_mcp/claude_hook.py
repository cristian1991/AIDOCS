...
class ClaudeHookHandler:
            ...
            if root_candidate is not None and _outer_cli_sid:
                try:
                    _managed = self.runtime.hub.managed_mode.get_mode(
                        root_candidate,
                        host_session_id=_outer_cli_sid,
                    )
    ...
    def _handle_user_prompt_submit(
        ...
        if not route.get("managed_mode"):
            ...
            if not auto_bound:
                # bug #234-2: NEVER hard-block. If auto_bind could not activate
                # (project not AIDOCS-managed, or a managed project whose bootstrap
                # failed), NO-OP so the agent runs the prompt normally. Managed
                # enforcement applies only once a session is actually bound —
                # forcing "/aidocs first" onto every prompt was the reported loop.
