...
# CONCEPTUAL-WORD PATTERNS, held apart from the tuple above because they are a
# DIFFERENT KIND OF TEST and were behaving like a different kind of rule.
#
# Every pattern above matches a SHAPE — an extension, a path anchor, a known
# store's exact name. These two match a WORD anywhere in the path, and that
# breadth is deliberate: a credential store can be called anything, and
# `prod-credential-dump.txt` must not read just because nobody enumerated it.
#
# WHAT WENT WRONG (measured 2026-09-07). Applied to EVERY path, the bare word
# swept in ordinary source that merely IMPLEMENTS credential handling:
# `gate_credential_renewal.py` and its two test files became unreadable and
# uneditable by any tool, and — via the twin patterns in code_index_store —
# UNINDEXED, so `ai_find` could not even see them. A seam moved underneath one
# of those tests and it could not be repaired; the test then monkeypatched a
# function no longer on its path and silently became non-deterministic.
#
# That is the rule inverted. It cannot stop a secret leaking from a test module
# (there is none), and it does stop the credential-handling code from being
# reviewed, tested or fixed. This file already states the principle thirty
# lines up — patterns are "scoped tightly ... so ordinary source like
# `service_account_manager.py` is NOT swept in — those reach the indexed gate
# instead". These two were the exception that never got the treatment.
#
# So: INDEXED SOURCE INSIDE THE PROJECT falls through to the indexed-read gate,
# which is still a gate — discovery first, no free-form reads. Everything else
# keeps the broad word test. `credentials.json`, `secrets.yaml`,
# `.aws/credentials`, `my-credential.txt` and any EXTERNAL path are unaffected,
# and `_content_is_secret` still sniffs the head of anything allowed through,
# so a .py that actually holds key material is caught on its CONTENT — which is
# the honest test for source, and the one a name could never make.
_SENSITIVE_WORD_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"credential", re.IGNORECASE),
    re.compile(r"secret", re.IGNORECASE),
)


def _is_project_source(normalized: str) -> bool:
    """True for an indexed source file inside the project tree.

    Deliberately NOT true for external paths: an absolute or home-relative
    path is outside this project's review discipline, so the broad word test
    stays in force there.
    """
    if not normalized or _is_external_path(normalized):
        return False
    return normalized.lower().endswith(_CODE_SOURCE_SUFFIXES)
...
def _is_sensitive(path: str) -> bool:
    ...
    if any(p.search(normalized) for p in _SENSITIVE_PATTERNS):
        return True
    # The conceptual-word test, exempting indexed project source — see the
    # comment on _SENSITIVE_WORD_PATTERNS for why the exemption is narrow.
    if not _is_project_source(normalized) and any(
        p.search(normalized) for p in _SENSITIVE_WORD_PATTERNS
    ):
        return True
