...
# ── Sanctioned git switch (operator ruling: switching is allowed when SAFE) ──
# The CFG_GIT_SWITCH rule keeps blocking every shell `git switch`. The ONE
# exception is ai_git op=switch, which holds the project's branch-transition
# barrier and checks its own safety preconditions (clean tree, no in-progress
# op, no other live actor) BEFORE it builds this request.
#
# PROVENANCE IS STRUCTURAL, NOT NAMING-BASED. SANCTIONED_GIT_SWITCH is a
# public, importable module attribute -- it is NOT private, and nothing here
# relies on it being hard to name. What an agent cannot do is place THIS
# Python object into ``ShellEgressRequest.metadata``: every agent-reachable
# door receives JSON (strings, numbers, dicts), and the waiver compares by
# IDENTITY (``is``), so a string, a dict or a re-created instance never
# matches. Only in-process code that already runs with server authority can
# pass it, and the production reference is ai_git's switch door alone.
# It is honoured only for the exact argv ``git_switch.switch_argv`` builds
# with a re-validated branch name. Floor, barrier, lifecycle preflight, output
# guard and audit still run; only CFG_GIT_SWITCH is waived, and only if it is
# the sole rule that fired.
class _SanctionedGitSwitch:
    __slots__ = ()

    def __repr__(self) -> str:
        return "<SANCTIONED_GIT_SWITCH>"


SANCTIONED_GIT_SWITCH = _SanctionedGitSwitch()
SANCTIONED_GIT_SWITCH_METADATA_KEY = "sanctioned_git_switch"
_GIT_SWITCH_WAIVABLE_RULES = frozenset({"CFG_GIT_SWITCH"})


def sanctioned_git_switch_waiver(argv: Any, metadata: Any) -> "frozenset[str]":
    """The waivable rule ids for this request, or an empty set."""
    try:
        if (metadata or {}).get(SANCTIONED_GIT_SWITCH_METADATA_KEY) is not SANCTIONED_GIT_SWITCH:
            return frozenset()
        a = tuple(argv or ())
        if len(a) != 5 or a[0] != "git":
            return frozenset()
        from .git_switch import branch_name_refusal, switch_argv

        name = a[4]
        if branch_name_refusal(name):
            return frozenset()
        if list(a[1:]) not in (switch_argv(name, True), switch_argv(name, False)):
            return frozenset()
        return _GIT_SWITCH_WAIVABLE_RULES
    except Exception:
        return frozenset()


def _barrier_refusal_result(why: str) -> "ShellEgressResult":
    return ShellEgressResult(
        ...
        stderr=why,
        duration_s=0.0,
        refused_reason="branch_transition_barrier",
    )


def _git_worktree_of(cwd: str) -> "str | None":
...
def _under_project_mutation_lease(fn):
    """SHARED mutation lease for a governed subprocess (r0b send-back on
    1b60a1275, hole 2). A subprocess whose cwd is inside a git work tree holds a
    ``branch_transition_barrier.mutation_lease`` from before its barrier check
    until it has finished, so a branch switch either refuses it (barrier taken
    first) or waits for it to drain (lease taken first) -- never samples the
    tree while it runs. A shell command cannot be proven read-only, so every
    subprocess participates. Outside any work tree there is nothing to switch."""
    import functools
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            a = bound.arguments
            req = a.get("request")
            cwd = str((req.cwd if req is not None else a.get("cwd")) or "")
            reach = str((req.reachability if req is not None else a.get("reachability")) or "agent_reachable")
        except Exception:  # noqa: BLE001 - malformed call: let the body refuse it
            return fn(*args, **kwargs)
        tree = _git_worktree_of(cwd)
        if tree is None:
            return fn(*args, **kwargs)
        try:
            from .branch_transition_barrier import BarrierHeld, describe, mutation_lease

            lease = mutation_lease(tree, f"shell_egress pid={os.getpid()}")
            lease.__enter__()
        except BarrierHeld as exc:
            return _barrier_refusal_result(
                f"AIDOCS branch-transition barrier: {describe(exc.holder)}; governed "
                "subprocesses in this project are refused until it finishes."
            )
        except Exception as exc:  # noqa: BLE001
            if reach != "agent_reachable":
                return fn(*args, **kwargs)
            return _barrier_refusal_result(
                f"branch-transition mutation lease unavailable ({exc!r}); refusing closed"
            )
        try:
            return fn(*args, **kwargs)
        finally:
...
def _consult_judge(
    ...
    ``waivable_rule_ids`` is non-empty ONLY for a request carrying the
    SANCTIONED_GIT_SWITCH allowance whose argv is exactly the sanctioned
    shape (see ``sanctioned_git_switch_waiver``); the raw shell-string
    door (``execute_shell`` / code_runner.ai_run) never passes it.
    ...
    if hasattr(verdict, "decide"):
        ...
        if decision == "allow":
            return None
        # Narrow structured waiver (ai_git op=switch): ONLY when the caller
        # proved the sanctioned shape AND every rule that fired is in the
        # waivable set. Any other rule firing alongside keeps the refusal.
        if waivable_rule_ids:
            _fired = {str(getattr(v, "rule_id", "")) for v in (getattr(verdict, "verdicts", None) or [])}
            if _fired and _fired <= set(waivable_rule_ids):
                return None
...
class ShellEgressService:
    ...
    def execute(self, request: ShellEgressRequest) -> ShellEgressResult:
        ...
        _maybe_refused = _consult_judge(
            ...
            sanctioned_git_switch_waiver(request.argv, request.metadata),
        )
