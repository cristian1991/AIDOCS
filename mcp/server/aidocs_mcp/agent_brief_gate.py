...
# #433 FP tuning (king decision 2026-07-17, tighten-shaped — the research
# class is NEVER loosened): a brief that trips a research keyword is
# downgraded block→pass ONLY when it is demonstrably an implementation
# order, i.e. BOTH concrete-implementation markers are present:
#   1. an explicit repo file path (mcp/|core/|apps/ prefix + a file
#      extension — a bare directory does NOT count), anywhere in the brief;
#   2. an imperative implementation verb in the FIRST 400 characters.
# Research-shaped briefs without file-path grounding refuse exactly as
# before; the operator 'delegate research' escape is untouched.
_IMPLEMENTATION_VERB_RE = re.compile(r"\b(?:implement|fix|wire|add|extend)\b")
_REPO_FILE_PATH_RE = re.compile(r"\b(?:mcp|core|apps)/[\w.\-/\\]*\.\w{1,10}\b")
_IMPL_VERB_WINDOW_CHARS = 400


def _implementation_grounded(text: str) -> bool:
    """True iff the lowercased brief carries BOTH concrete markers."""
    return bool(
        _REPO_FILE_PATH_RE.search(text)
        and _IMPLEMENTATION_VERB_RE.search(text[:_IMPL_VERB_WINDOW_CHARS])
    )
...
def evaluate_agent_brief(brief: str, override_phrase_present: bool) -> dict[str, Any]:
    ...
    for pattern in _RESEARCH_VERB_PATTERNS:
        ...
        if m:
            if _implementation_grounded(text):
                # #433: keyword tripped, but the brief is a concrete
                # implementation order (repo file path + imperative
                # implementation verb up front) — pass, don't block.
                return {
                    "allowed": True,
                    "reason": (
                        "Brief contains research language (matched: "
                        f"'{m.group(0)}') but is implementation-grounded "
                        "(explicit repo file path + imperative "
                        "implementation verb) — dispatch permitted."
                    ),
                    "matched_pattern": "",
                }
            return {
