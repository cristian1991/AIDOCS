...
reads JSON from stdin, writes JSON result to stdout. Event kinds:
    ...
    posttool        → LifecycleService.on_post_tool_use_audit +
      ...
      prompt_mutate   → PromptSubmitService.evaluate_submit rendered into
                        the adapter-neutral prompt result shape.
...
Prompt-submit support (War 3 canonicalization)
----------------------------------------------
``prompt_mutate`` and ``oc_chat_message`` are thin renderers over the same
``PromptSubmitService`` used by Claude hooks and explicit WebMCP handling.
Authority-bearing operator intent runs in that one service and remains
origin-gated; adapters neither reimplement nor downgrade it. Host differences
are limited to envelope rendering and truthful hook capability labels. WebMCP
is explicit-only and never claims automatic UPS.
"""
    ...
    *,
    ...
    Principal attribution:
      ...
      - else: ``strict=True`` (AUTHORITY-BEARING grant/mutation path) →
        ...
        principal to human for authority). ``strict=False`` is retained only
    for legacy non-authority adapter callers.
    """
    import os
...
def _handle_pretool(payload: dict) -> dict:
    ...
    # Host-agnostic native-tool lease (r0b 3e084f64): OpenCode executes its own
    # native tools, so the lease is registered here, BEFORE the allow is
    # returned, and released in _handle_posttool.
    _why = _native_lease_pre(project_root, payload, denied=str(result.verdict or "") == "deny")
    if _why:
        from .tool_gate_service import ToolGateResult

        return _result_to_dict(
            ToolGateResult.deny(reason=_why, why=("branch_transition_barrier",))
        )

    return _result_to_dict(result)


def _native_lease_pre(project_root: Path, payload: dict, *, denied: bool) -> str:
    from . import hook_pipeline as _hp

    inner = payload.get("payload") or {}
    return _hp.native_tool_pre(
        project_root,
        host_kind=_host_kind(payload),
        host_session_id=str(payload.get("host_session_id") or ""),
        tool_use_id=str(inner.get("tool_use_id") or payload.get("tool_use_id") or ""),
        tool_name=str(payload.get("tool_name") or ""),
        tool_input=payload.get("tool_input") or {},
        denied=denied,
    )
...
def _host_kind(payload: dict) -> str:
    """The host kind this CLI invocation PROVES. The caller passes host_kind
    explicitly; an absent one is UNKNOWN, never an attested host name."""
    from . import hook_pipeline as _hp

    marker = str(payload.get("host_kind") or "").strip()
    if not marker:
        return _hp.UNKNOWN_HOST_KIND
    from . import branch_transition_barrier as _btb

    return _btb.canonical_host_kind(marker) or _hp.UNKNOWN_HOST_KIND


def _native_lease_post(payload: dict) -> None:
    from . import hook_pipeline as _hp
...
def _handle_posttool(payload: dict) -> dict:
    # FIRST: release the native-tool lease, whatever the rest of this handler
    # does (a failure downstream must not strand the lease).
    _native_lease_post(payload)
...
def _handle_prompt_mutate(payload: dict) -> dict:
    ...
    result = PromptSubmitService(runtime).evaluate_submit(
        project_root,
        inner_payload,
        host_kind=str(payload.get("host_kind") or "unknown"),
        audit_source="host_adapter_cli.prompt_mutate",
        grant_eligible=grant_eligible,
    )
    host_kind = str(payload.get("host_kind") or "unknown").strip().lower()
    supports_additional_context = bool(payload.get("supports_additional_context"))
    return result.to_adapter_dict(
        rewrite_context_fallback=(
            host_kind != "claude_code" and not supports_additional_context
        )
    )
...
def _handle_compact(payload: dict) -> dict:
    ...
    try:
        ...
        managed_sid = resolve_managed_session(runtime.hub.managed_mode, project_root)
    except Exception:
        managed_sid = ""
...
def _handle_oc_chat_message(payload: dict) -> dict:
    ...
    # Run one canonical prompt-submit transaction for the OpenCode host.
    # The service owns doctrine, rollback, and degradation semantics; this
    # adapter only renders its host-neutral result into OpenCode's shape.
    submit_result = PromptSubmitService(runtime).evaluate_submit(
        project_root,
        {
            "prompt": prompt_text,
        ...
        host_kind="opencode",
        audit_source="host_adapter_cli.oc_chat_message",
        grant_eligible=_grant_eligible_for_payload(
            "oc_chat_message",
            payload,
        ),
    )
    submit_out = submit_result.to_adapter_dict()
    ctx_blocks = list(submit_out.pop("additional_context_blocks", ()))
    out.update(submit_out)

    # Pull actor-scoped host_state for managed-mode/lifecycle/skills.
    try:
        host_state = runtime.host_state(
            project_root,
            host_session_id=host_session_id,
    ...
    managed = bool(session_state.get("managed", False))
    session_id_resolved = str(session_state.get("session_id") or "")
    startup_state = str(session_state.get("state") or "")
    ...
    # Operator-intent resolution is already included in the canonical service result.
...
_ACTION_TOOL_DIRECTIVES: dict = {
    "edit": (
        "`task_begin` → `ai_get_lines` (read) → `ai_replace` (mode=string/anchor/symbol/lines) or "
