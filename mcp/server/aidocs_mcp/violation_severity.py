...
# ── Substrate-unavailable behaviour (#590) ─────────────────────────
#
# #590: an agent in ANOTHER project was refused a raw ``Read``, told to use
# ``ai_find`` / ``ai_bundle``, and those tools were unreachable because this
# machine's daemon was restarting for unrelated work. The refusal itself SAID
# the named tools were unreachable — and held anyway. The agent stopped, which
# is the behaviour we want, and was rewarded with a dead end. That is an
# IMPOSSIBLE OBLIGATION: the only moves left are stall or disobey.
#
# THE POINT NOBODY HAD SPENT: this module ALREADY knows the distinction #590
# asks for. ``indexed_file_gate`` and ``read_gate`` are ``SEVERITY_FRICTION``
# — "block + redirect, there's no security risk being averted." The
# classification existed; substrate-unavailable behaviour simply never
# consulted it, because a single global fail-closed default covered both
# classes. Security controls and workflow nudges need OPPOSITE behaviour when
# the enforcement substrate is gone:
#
#   security control, substrate down -> REFUSE (a gate that cannot check
#                                      cannot permit)
#   workflow nudge,   substrate down -> ALLOW, LOUDLY (the nudge's whole
#                                      value was advisory)
#
# WHY THIS IS NOT SIMPLY "ALL FRICTION FAMILIES". FRICTION also covers
# ``raw_shell_t0`` (raw Bash instead of ai_run) and ``tier0_edit_redirect``
# (raw Edit/Write instead of ai_replace). Those blocks route an EXECUTION or a
# WRITE, and that same chokepoint is where the judge cascade and the audit
# ledger attach. Degrading them would hand back the raw shell and the raw
# writer during an outage — trading #590's deadlock for a far worse hole.
#
# The line is READ versus MUTATE, and it is a property of the act, not a list
# of artefacts: re-reading a file the agent already holds a path to permits
# nothing that was not already permitted, so routing it is pure workflow
# advice. Executing or writing is not.
#
# raw_shell_t0 / tier0_edit_redirect therefore KEEP REFUSING during an outage,
# and their refusal must then say so HONESTLY rather than naming a tool it
# knows is down (see daemon_reachability.decorate_refusal).
_SUBSTRATE_DEGRADABLE_LEVELS: frozenset[str] = frozenset(
    {
        "indexed_file_gate",  # "file not discovered yet → ai_find/ai_bundle first"
        "read_gate",  # "reach the file via ai_find/... first; raw Read is gated"
    },
)


def degrades_when_substrate_down(level: str) -> bool:
    """True iff this gate level must DEGRADE (allow + warn) rather than refuse
    when the AIDOCS tool surface is unreachable.

    Two conditions, both required, and the first is derived rather than
    listed: the level must be FRICTION severity (a routing nudge that averts
    no security risk), AND it must route a READ rather than an execution or a
    write. A level that is not FRICTION can never degrade here, so a future
    re-classification of a family to STRIKE automatically withdraws its
    degradation without anyone remembering to edit this function.
    """
    if not level:
        return False
    if level not in _SUBSTRATE_DEGRADABLE_LEVELS:
        return False
    return _FAMILY_SEVERITY.get(level) == SEVERITY_FRICTION
