  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden">
      ...
      <main className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 border-b border-castle-line bg-castle-panel/40 px-4 py-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Filter className="h-3.5 w-3.5 text-castle-mute" />
            {(
              [
                ["modified", "Modified"],
                ["security", "Security"],
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={saveAll}
              disabled={!hasDirty || !!savingSetting || !rowsLoaded}
              title={
                !rowsLoaded
          ...
          {quarantinedDirty.length > 0 && (
            <div className="mb-4 rounded-2xl border border-castle-deny/40 bg-castle-deny/[0.06] p-3">
              ...
              <div className="mb-2 text-[11px] text-castle-mute">
                These keys are service-managed, deprecated, dashboard-only,
                T0, or hidden-owned. Normal Save All cannot persist them —
                use the Advanced Raw expert edit (or the owning profile).
              </div>
              <div className="flex flex-wrap gap-1.5">
                {quarantinedDirty.map((e) => (
                  <button
                    ...
                    title="Open the Advanced Raw expert editor for this key"
                  >
                    {e.path}
                  </button>
                ))}
            ...
            <div className="mt-6 overflow-hidden rounded-2xl border border-castle-deny/30 bg-castle-deny/[0.04]">
              <div className="flex items-center gap-2 border-b border-castle-deny/20 px-4 py-2.5">
                ...
                <span className="text-[11px] text-castle-mute">
                  Guardrail keys — owned by a profile, deprecated, or
                  T0/dashboard-only. Not editable as normal rows. Use the
                  owning profile action; T0 keys need exact confirmation.
                </span>
              </div>
              <div>
                {advancedRows.map((row) => {
                  const key = rowField(row, "key");
                  const owner = rowField(row, "owning_profile");
                  const route = saveRouteFor({
                    key,
                  ...
                  const msg =
                    route === "blocked"
                      ? `Deprecated — ${rowField(row, "deprecated")}`
                      : route === "profile"
                      ? `Managed by the '${owner}' profile — use its action.`
                      : "T0 / dashboard-only — expert edit requires exact confirmation.";
                  return (
                    <div
                      key={key}
                    ...
                    >
                      ...
                      {route === "expert" ? (
                        ...
                        <span className="px-2.5 py-1 text-[11px] text-castle-mute italic">
                          read-only
                        </span>
                      )}
                    </div>
              ...
              {expertKey && (
                <div className="border-t border-castle-deny/20 bg-black/20 p-4">
                  <div className="mb-2 text-[11px] font-bold text-slate-200">
                    Expert edit: <code className="font-mono">{expertKey}</code>{" "}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      {expertBusy ? "Applying..." : "Apply (expert)"}
                    </button>
                    <button
                      type="button"
