    ...
    *,
    ...
    def ai_slop(
        ...
        if _result is not None:
            ...
            return _attach_slop_evidence(_canon, _result)
        return {
            "success": False,
    ...
    def memory_capture(
        ...
        if _palace is not None and target_rel:
            # TIMEBOXED (2026-06-13): mempalace is a 3rd-party engine with no
            # internal timeout; a blocking ingest here would wedge the whole
            # memory_capture tool (and the MCP connection). The canonical
            # sqlite row is already durable, so we abandon a hung ingest and
            # fall through to the memory_palace_lag event below.
            ...
            _ingest_res, palace_timed_out = _run_timeboxed(
                _do_palace_ingest, PALACE_INGEST_TIMEOUT_S
            )
            palace_ok = bool(_ingest_res)
            if not palace_ok:
                try:
                    hub.execution.record_event(
                        project_root,
                        ...
                        payload={
                            "path": target_rel,
                            "kind": kind,
                            "timed_out": palace_timed_out,
                            "note": (
                                "memory_index row is canonical; palace "
                                "drawer projection "
                                + (
                                    f"timed out (>{PALACE_INGEST_TIMEOUT_S:g}s) and was abandoned"
                                    if palace_timed_out
                                    else "failed"
                                )
                                + " so palace search/bootstrap-context may "
                                "miss this row until the next full ingest."
                            ),
                        },
                    )
                except Exception:
