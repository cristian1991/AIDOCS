...
class RuntimeService:
    ...
    def verification_gate(
        ...
        if not commands_run:
            return {
                ...
                "reason": self._missing_evidence_reason(required_commands),
                "required_commands": required_commands,
                "commands_run": commands_run,
                "command_results": command_results,
            }
    ...
    @staticmethod
    def _missing_evidence_reason(required_commands: list[str]) -> str:
    ...
    def _worker_task_update(
        ...
        **updates: object,
        ...
        if not current or not str(current.get("task_id") or ""):
            return {
                "ok": False,
                "blocked": True,
                "status": "no_open_task",
        ...
        try:
            ...
            return {
                ...
                "error": (
                    ...
                    f"Check the session store (.MEMORY/.index) and retry task_begin."
                ),
            }
        if str(readback or "") != task_id:
            return {
                ...
                "error": (
                    ...
                    f"(silent no-op guard, #474). Retry task_begin; if it repeats, "
    ...
    def mint_session_scaffold_grant(
        ...
        granting_session = str(mode.get("session_id") or "").strip()
        is_conductor = bool(
            mode.get("active") and granting_session and mode.get("membership_valid"),
        )
        if not is_conductor:
            return {
                "ok": False,
    ...
    def task_complete(
        ...
        try:
            ...
            _session_slot_task = ""
        _open_task = _actor_slot_task or _session_slot_task
        if not _open_task:
            return {
                "session_id": session_id,
                "blocked": True,
