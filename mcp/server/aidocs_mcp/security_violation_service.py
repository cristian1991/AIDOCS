...
class SecurityViolationService:
    ...
    def peak_strike_count(
        ...
        """(current strike count for the ACTING agent, threshold). Identity-spine
        fix 2026-07-07: when host_session_id is given, scope to that agent's
        derived agent_context_id (the axis the FREEZE uses) so a worker's strikes
        never inflate the conductor's surfaced count. Empty = legacy session-wide
        peak. Returns (0, threshold) when clean.
        """
        if not session_id:
            return (0, 0)
        ...
        try:
            ...
            with self.hub.execution.connect(project_root) as conn:
                rows = conn.execute(
                    "SELECT target_entity te, COUNT(*) c FROM execution_events "
                    "WHERE event_kind = ? "
                ...
                # Per-agent peak: only scopes ending in the acting agent's
                # context id (scope_key = "actor|lane|family|acid"). acid=''
                # (no host id) keeps the legacy session-wide peak across scopes.
                peak = 0
                for te, c in rows:
                    if acid and not str(te or "").endswith("|" + acid):
                        continue
                    peak = max(peak, int(c))
        except Exception:
            peak = 0
