...
def _nonminting_core_law(hub: Any, runtime: Any):
    ...
    def _delegate(envelope: ShellCommandEnvelope) -> Any:
        ...
        if getattr(decision, "needs_confirmation", False):
            # #571 three-way routing at the delegate, BEFORE the confirmable
            # signal is emitted. This is the site's only chance: downstream,
            # ShellPolicy keys off "confirm" appearing in blocked_by and
            # ShellEnforcement._mint_freeze then mints unconditionally, so a
            # rung-3 verdict that reaches there can no longer avoid a freeze.
            ...
            _sh_outcome, _sh_class = outcome_for(
            ...
            if _sh_outcome == OUTCOME_ALLOW:
                return _Decision(None)
            if _sh_outcome == OUTCOME_BLOCK:
                from .freeze_service import build_workflow_block_response

                _blk = build_workflow_block_response(
                    tool_name="ai_run",
                    tool_input={"command": ""},
                    reason=str(getattr(decision, "reason", "") or "workflow block"),
                    verdict_class=_sh_class,
                )
                # NOTE the absence of "confirm" in blocked_by: that is what
                # keeps ShellPolicy from treating this as confirmable and so
                # what keeps _mint_freeze from ever running for a rung 3.
                return _Decision(
                    {
...
def native_route(
    ...
    *,
    ...
    """
    # Capability floor applies to BOTH native identity paths (provider-attested
    # AND #319 local-solo machine-presence): native-safe host/provider +
    # host-replaceable output. A capability gap always defers to ai_run.
    ...
    if not native_enabled:
        # #319: provider attestation not enrolled → the ONLY native path is
        # local-solo machine-presence on the proven claude_code+bash adapter
        # (machine presence IS the authority). Everything else → ai_run.
        from .governed_shell_attest import machine_presence_native_ok

        mp_ok, _mp_reason = machine_presence_native_ok(host, provider, project_root)
        return ROUTE_NATIVE if mp_ok else ROUTE_AI_RUN
