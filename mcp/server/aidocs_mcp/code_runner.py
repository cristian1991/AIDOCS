...
class RunResult:
    ...
    """Did the COMMAND succeed, independently of whether its output could be
    shown (#608)? ``None`` when it never ran. ``success`` above is the
    conservative summary that also folds in output certification, so the two
    DISAGREE exactly when the guard withheld a payload from a command that
    exited 0 — the case that made ai_git report a landed commit as failed."""

    output_status: str = "not_executed"
    """The output_guard's verdict — see ``shell_egress_service.OUTPUT_STATUSES``
    (clean / redacted / withheld_finding / withheld_scan_error /
    not_executed)."""
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        # Only stated when it CHANGES the reading: a clean echo needs no note,
        # a notice standing in for the real output absolutely does (#608).
        if self.output_status not in ("clean", "not_executed"):
            result["output_status"] = self.output_status
            result["action_success"] = self.action_success
...
def _run_process(
    ...
    popen_kwargs: dict[str, object] = dict(
        ...
        text=True,
        # #684: pin the decode. text=True alone decodes with the system ANSI
        # codepage, mojibaking any UTF-8 the command emits. errors='replace'
        # (never 'strict') so genuinely non-UTF-8 output degrades visibly
        # instead of raising and taking the run down.
        encoding="utf-8",
        errors="replace",
    )
...
def ai_run(
    ...
    *,
    ...
    return RunResult(
        ...
        # #608: carry the ACTION's outcome and the guard's verdict separately,
        # so a caller is never forced to read a withheld echo as a failed run.
        action_success=res.action_ok,
