...
class RuntimeConductorStateService:
    ...
    def plan_conductor_graph(
        ...
        """Return the plan graph with its canonical work-store projection."""
        plan_graph = self._plan_conductor_snapshot(project_root, session_id)["graph"]
        from .cross_agent_coordination import work_graph_snapshot

        return {
            **plan_graph,
            "work_graph": work_graph_snapshot(
                project_root,
