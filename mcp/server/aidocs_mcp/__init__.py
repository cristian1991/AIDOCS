...
def _package_vendor_root(pkg_dir: Path) -> Path | None:
    """A vendored engine carried INSIDE this package (``aidocs_mcp/_vendor``).

    THE BOOTSTRAP POSTURE, and it exists to solve a chicken-and-egg (#925).
    The fix that lets a release install its own engine ships INSIDE that
    release, so the client that must install it is by definition running the
    OLD code that cannot. A pre-#925 watchdog pulls the artefact, pip-installs
    the project, installs no engine, and its `verify_interpreter` refuses the
    candidate with `mempalace_unresolvable` — forever, every fifteen minutes.

    WHAT MAKES THIS WORK WITHOUT CHANGING THE OLD CLIENT: that verification
    runs a snippet INSIDE the candidate, and the snippet does

        import aidocs_mcp as m          <- THIS module, from the NEW release
        ...
        'mempalace': u.find_spec('mempalace') is not None

    in that order. So the new package's own import-time wiring runs BEFORE the
    question is asked, and can answer it. The old client needs no change
    because the thing it interrogates is the new code.

    WHY INSIDE THE PACKAGE RATHER THAN BESIDE IT. Files under ``aidocs_mcp/``
    are covered by ``compute_package_fingerprint``, which IS the release
    signature. A tree staged beside the package would be transported unsigned —
    the supply-chain hole #925 refuses — and the vendored CLAIM cannot be
    repointed to cover it without breaking every already-signed release
    (SIG_MISMATCH, after which a client needs the very update it is refusing).
    Here the engine rides inside the signed envelope, the legacy claim is
    untouched, and nothing new needs signing.

    Checked FIRST, ahead of the ancestor walk, because in a source checkout
    both may exist and the package-internal copy is the one that travels with
    an install.
    """
    vendor = pkg_dir / "_vendor"
...
def _wire_vendored_mempalace() -> str | None:
    ...
    packaged = _package_vendor_root(here.parent)
    if packaged is not None:
        p = str(packaged)
        if p not in sys.path:
            sys.path.insert(0, p)
        return f"vendored-packaged:{p}"
    vendor = _locate_vendor_root(here)
...
#     them AFTER promoting (doctrine XIII), so a deploy structurally ends with
...
def _source_drift(repo_root: Path | None, deployed_commit: str) -> dict:
    ...
    then commits its own reports + session journal as X+1 (doctrine XIII —
    ...
    if repo_root is None or not deployed_commit or not (repo_root / ".git").exists():
        ...
        return out
    try:
        # Reuse the ONE audited git helper (Article VI): it is already
