...
class SkillProvidersStore(SQLiteIndexStoreBase):
    ...
    def migrate_legacy(self, project_root: Path) -> None:
        """LIFECYCLE migration of the legacy skill-providers.json (#1095):
        schema + ingest + retire the file. The same work init_db does on the
        write path, named for the SessionStart / write-path caller. Never a
        read."""
        self.init_db(project_root)

    def read(self, project_root: Path) -> list[dict[str, Any]]:
        """PURE read (#1095): the established store only. Never creates the
        table, never ingests or deletes the legacy JSON, never reads the legacy
        file back as authority. A missing table reads as no providers; any other
        error raises."""
        try:
