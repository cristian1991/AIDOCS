...
def _gate_handoff_permission(target: str, operation: str) -> dict[str, Any] | None:
    """BOTH-ENDS consent for a CROSS-PROJECT handoff (#500).

    Layered ON TOP of ``_gate_cross_project_session``: that ladder proves the
    caller may reach the target at all (commissioning + approved relation +
    target session membership/RBAC). This one asks each PROJECT whether it
    consents to participate in a handoff at all — ``project.create_handoff``
    must hold in the SOURCE project's own RBAC store AND the TARGET's own
    store. Operator ruling: "if one project doesn't allow, no handoff."

    Returns a refusal dict naming the denying side, or None when allowed.
    Fail-closed: any gate error refuses (``handoff_gate_error``).
    """
    try:
        from .mcp_server_runtime_helpers import current_calling_host_session_id
        from .project_authority import require_create_handoff

        d = require_create_handoff(
            resolve_project_root(),
            Path(target),
            operation=operation,
            host_session_id=current_calling_host_session_id(),
        )
    except Exception as exc:  # fail closed — a broken gate never permits egress
        return {
            "ok": False,
            "error": f"handoff permission gate error: {exc}",
            "blocked_by": "handoff_gate_error",
        }
    if d.get("ok"):
        return None
    return {
        "ok": False,
        "error": d.get("reason", "handoff refused"),
        "blocked_by": d.get("blocked_by"),
        "stage": d.get("stage"),
...
def apply_list_grant(current: list, grant_value: dict) -> list:
    """Resolve a list-mutation sentinel against the CURRENT list value.

    The operator names ONE element ("add X to <key>"); every other entry is
    carried over from ``current`` HERE rather than being retyped into the
    prompt. That is the property that makes the grant safe: a truncated or
    mis-parsed phrase cannot drop entries it never carried (2026-08-21).

    ``add`` is idempotent — re-adding an existing element must not put a
    duplicate row in a security allowlist. ``remove`` of an absent element
    is a no-op rather than an error.
    """
    elem = grant_value.get("element")
    if grant_value.get("__list_op__") == "add":
        return list(current) if elem in current else [*current, elem]
    return [x for x in current if x != elem]
...
def _is_policy_mutation_key(key: str) -> bool:
    """#100 FIX1: True when `key` mutates a shell policy table — bash.allow.*,
    bash.deny.*, or any future *.allow.* / *.deny.* / *.allow / *.deny. Matches
    an `allow`/`deny` segment that is NOT the leading namespace segment (so
    `security.allow_config_edit` is NOT a policy key). These edits gate the shell
    execution surface and require the dedicated allowlist-edit authority."""
    parts = (key or "").split(".")
    return len(parts) >= 2 and any(seg in ("allow", "deny") for seg in parts[1:])


def _allowlist_edit_blocked(
    key: str, *, allow_config_edit: bool, allow_allowlist_edit: bool
) -> bool:
    """#100 FIX1: True when a policy-table mutation must be BLOCKED for lack of
    the dedicated authority. A shell allow/deny edit requires BOTH
    security.allow_config_edit AND security.allow_allowlist_edit — either off
    refuses. Non-policy keys are never blocked here (the ordinary config-edit
    gate governs them upstream)."""
    if not _is_policy_mutation_key(key):
        return False
    return not (allow_config_edit and allow_allowlist_edit)
...
def register_project_admin_tools(
    *,
    ...
    @server.tool(
        ...
        annotations=dict(CONFIG_SET_ANNOTATIONS),
    )
    def config_set(
        setting_path: str,
        value: Any,
        ...
        # #100 FIX1 — T0 allowlist-mutation gate. Editing a shell policy table
        # (bash.allow.* / bash.deny.* / *.allow.* / *.deny.*) is the gateway to
        # shell execution, so it needs a DEDICATED authority beyond "can edit
        # config": security.allow_allowlist_edit (dashboard-only, default FALSE).
        # Both toggles must be ON — a single coarse config-edit switch must not
        # also mean "the agent can self-grant new shell commands."
        _allow_allowlist = bool(
            get_setting(
                "security.allow_allowlist_edit",
                project_root=root,
                default=False,
            ),
        )
        if _allowlist_edit_blocked(
            k, allow_config_edit=_allow, allow_allowlist_edit=_allow_allowlist
        ):
            return {
                "ok": False,
                "error": (
                    f"config_set refused: '{k}' mutates a shell allow/deny "
                    "policy table (the gateway to shell execution). This "
                    "requires the dedicated dashboard authority "
                    "`security.allow_allowlist_edit=true` IN ADDITION to "
                    "`security.allow_config_edit`. Enable the separate "
                    "allowlist-edit toggle in the dashboard (DANGER: it lets "
                    "the agent modify shell allow/deny rules) before retrying."
                ),
                "blocked_by": "allowlist_edit_gate",
            }
        ...
        #
        # #866 ORDER: the categorical verdicts above (deprecated /
        # service-managed / dashboard-only) are answered BEFORE the
        # missing-grant refusal. They depend only on the key, and they are
        # the actionable truth — measured 2026-08-21, an operator asking
        # for the dashboard-only 'security.egress_allowlist' was first told
        # to obtain a user-intent grant that could never help, and logged
        # in twice chasing it. A remedy that cannot work is not a remedy
        # (law 311bf3e6).
        ...
        # Resolve a list-mutation sentinel against the CURRENT list, the same
        # way "__toggle__" resolves against the current bool. The operator
        # named ONE element ("add X to <key>"); every other entry is carried
        # over HERE rather than being retyped into the prompt — which is what
        # makes the whole list impossible to lose to a truncated grant.
        if isinstance(grant_value, dict) and "__list_op__" in grant_value:
            current = get_setting(
                k,
                project_root=root,
                session_id=session_id or None,
            ...
            if not isinstance(current, list):
                return {
                    "ok": False,
                    "error": (
                        f"config_set refused: '{k}' is not list-valued "
                        f"(current type: {type(current).__name__}), so "
                        f"'add/remove ... {k}' does not apply. Use "
                        f"'set {k} to <value>' instead."
                    ),
                    "blocked_by": "list_op_on_non_list",
                    "setting": k,
                }
            grant_value = apply_list_grant(current, grant_value)
    ...
    from . import tool_interface as _ti_c20_config

    _ti_c20_config.register_impl("config_set", config_set)
    ...
    def related_project_register(
        ...
        permission (authenticated RBAC — every flavor, #404) — registering an
    ...
    def handoff_create(
        ...
        """CROSS-PROJECT: write a handoff into ANOTHER project's session without switching binding. NOT a same-project 'save my session' / session-resume tool — use the session/journal tools for that. At least one section field must be provided.
        ...
        caller must hold permission (authenticated RBAC — every flavor, #404).

        BOTH-ENDS CONSENT (#500): a handoff is an EGRESS of one project's
        context into another tree, so `project.create_handoff` must hold in
        the SOURCE project's own RBAC store AND in the TARGET project's own
        store. If either side lacks it there is no handoff — the refusal
        names which side denied.
        """
        ...
        if _refusal is not None:
            return _refusal
        # #500 both-ends handoff consent. MUST precede every write path below
        # (including the content_path branch, which writes HANDOFF.md
        # directly) — otherwise the egress is ungated.
        _refusal = _gate_handoff_permission(target_project_root, "handoff_create")
        if _refusal is not None:
            return _refusal
    ...
    def related_project_code_search(name: str, query: str, limit: int = 10) -> dict[str, Any]:
        ...
        matches = hub.code.search_code(related_root, query=query, limit=limit)
        # #648 (P2 provenance stamp): this result comes from ANOTHER
        # project root — label its origin (mirrors
        # related_project_compare_concept's "related_project" dict).
        # Additive: the match list is preserved verbatim under
        # "results"; no matches dropped, no new refusals.
        return {
            "results": matches,
            "related_project": {
                "name": name,
    ...
    def related_project_symbol_bundle(
        ...
        related_root = resolve_related_root(root, name)
        hub.code.sync_code_manifest(related_root, include_tests=False)
        bundle = hub.code.get_symbol_bundle(
            related_root,
            symbol=symbol,
            path=path,
            kind=kind,
        ...
        # #648 (P2 provenance stamp): label the origin (mirrors
        # related_project_compare_concept's "related_project" dict).
        # Additive only — no keys removed, no new refusals.
        if isinstance(bundle, dict):
            bundle["related_project"] = {"name": name, "path": str(related_root)}
            bundle["provenance"] = "related_project"
        return bundle
    ...
    def related_project_subsystem_bundle(
        ...
        bundle = hub.code.get_subsystem_bundle(related_root, concept=concept, limit=limit)
        # #648 (P2 provenance stamp): label the origin (mirrors
        # related_project_compare_concept's "related_project" dict).
        # Additive only — no keys removed, no new refusals.
        if isinstance(bundle, dict):
