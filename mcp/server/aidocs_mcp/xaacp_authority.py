...
class RemoteMcpXaacpAuthority:
    ...
    @property
    def pinned_project_id(self) -> str:
        """The canonical project this authority was RESOLVED for (#1095, r0b2
        68842fc6-547). An owed attach records this, never a re-classification
        made after the attempt."""
    ...
    def dispatch(self, project_root: Path, **payload: Any) -> dict:
        ...
        if not bound_session:
            return _error(
                # #1095 step 4: the same actionable status the gate returns.
                "needs_binding",
        ...
        is_send = str(payload.get("mode") or "").strip().lower() == "xaacp_send"
        if bound_session != requested_session and not is_send:
            return _error(
                "forbidden",
        ...
        # A SEND may name another session (8d84 phases 1 and 3): the gate's
        # own dispatcher judges that target's authority exactly as the local
        # one does. Every other mode stays the caller's own session (above).

        meta = {
            META_AIDOCS_HOST_SESSION: hsid,
            META_AIDOCS_HOST_KIND: hkind,
            META_AIDOCS_PROJECT_ID: self._project_id,
            # THE CALLER'S SESSION, which is its proven managed binding --
            # never the requested TARGET. This used to send `requested_session`
            # and was only right because the two were forced equal; a send to
            # another session would otherwise tell the gate the caller was
            # bound somewhere it is not.
            META_AIDOCS_SESSION_ID: bound_session,
        }
        ...
        # 8d84 phase 3 (r0b2 Q3): a fixed argument set DROPS what it does not
        # name, and a dropped target_project_id would deliver a cross-project
        # send as a same-project one. Carried whenever supplied.
        target_project_id = str(payload.get("target_project_id") or "").strip()
        if target_project_id:
            args["target_project_id"] = target_project_id
        # #1095 step 4 (r0b2 804329fc-66d): NO AUTO-CONNECT, NO AUTO-REGISTER.
        # This used to detect "no binding" in the gate's answer and call the
        # gate's ai_session(connect) itself -- a bind performed as a side effect
        # of an ordinary XAACP call. The gate's answer is now returned verbatim:
        # needs_binding (remedy: an explicit ai_session(connect), which attaches
        # via connect_and_attach) or needs_attach (remedy: an explicit
        # ai_msg(mode='xaacp_attach')). An ordinary call never binds anyone.
        return self._tool_call("ai_msg", args, meta)

    @staticmethod
    def _identity_meta(
        *, host_session_id: str, host_kind: str, host_agent_id: str, project_id: str, session_id: str
    ) -> dict[str, str]:
        from .webmcp_identity import (
            META_AIDOCS_HOST_AGENT,
            META_AIDOCS_HOST_KIND,
        ...
        meta = {
            META_AIDOCS_HOST_SESSION: host_session_id,
            META_AIDOCS_HOST_KIND: host_kind,
            META_AIDOCS_PROJECT_ID: project_id,
            META_AIDOCS_SESSION_ID: session_id,
        }
        if host_agent_id:
            meta[META_AIDOCS_HOST_AGENT] = host_agent_id
        return meta
    ...
    def attach_lifecycle(
        ...
        *,
        ...
        """INTERNAL lifecycle attach (#1095 step 4; r0b2 804329fc-66d item 4).

        Called by SubagentStart / SessionStart with the TRUSTED, host-measured
        hook identity -- a hook is not an MCP tools/call, so the request
        ContextVars ``dispatch`` reads do not exist there. These identity
        fields are never on the public xaacp_attach schema and never come from
        a model. The parent host session must already hold a local managed
        binding to ``session_id``; the gate then derives the actor (the
        subagent axis from host_agent_id) exactly as for any forwarded call.
        Bounded by ``timeout``; the caller owes a failure to the outbox.
        """
        hsid = str(host_session_id or "").strip()
        hkind = str(host_kind or "").strip()
        sid = str(session_id or "").strip()
        if not hsid or not hkind or hkind == "unknown" or not sid:
            return _error("forbidden", "lifecycle attach requires a host-measured identity")
        from .managed_mode_service import ManagedModeService, resolve_managed_session

        try:
            bound = resolve_managed_session(ManagedModeService(), project_root, host_session_id=hsid)
        except Exception as exc:  # noqa: BLE001
            return _error("forbidden", f"managed binding unreadable: {type(exc).__name__}")
        if bound != sid:
            return _error(
                "needs_binding",
                "the parent host session holds no managed binding to this session",
                remedy=f"ai_session(mode='connect', session_id='{sid}')",
            )
        meta = self._identity_meta(
            host_session_id=hsid,
            host_kind=hkind,
            host_agent_id=str(host_agent_id or "").strip(),
            project_id=self._project_id,
            session_id=sid,
        )
        previous = self._timeout
        self._timeout = min(float(timeout), previous) if timeout else previous
        try:
            pstat = self._tool_call("ai_project", {"mode": "status"}, meta)
            if isinstance(pstat, dict) and pstat.get("ok") is False:
                return dict(pstat)
            selected = (
                str((pstat.get("project") or {}).get("project_id") or "")
                if isinstance(pstat.get("project"), dict)
                else ""
            )
            if not selected or selected != self._project_id:
                return _error(
                    "forbidden",
                    "XAACP gate project selection does not match this local project's binding",
                    bound_project_id=self._project_id,
            ...
            return self._tool_call("ai_msg", {"mode": "xaacp_attach", "session_id": sid}, meta)
        finally:
            self._timeout = previous

    def connect_and_attach(
        self, project_root: Path, *, session_id: str, confirm_token: str = ""
    ) -> dict:
        """Attach the CALLER on the gate as part of its explicit local connect.

        First an explicit xaacp_attach (request identity, same path as every
        forwarded call). If the gate says the caller has no gate-side binding,
        THIS explicit connect performs the gate's two-phase ai_session(connect)
        and relays its confirmation; the operator re-invokes the local connect
        with that confirm_token. Never reached from an ordinary XAACP call.
        """
        sid = str(session_id or "").strip()
        first = self.dispatch(project_root, mode="xaacp_attach", session_id=sid)
        if str((first or {}).get("status") or "") != "needs_binding":
            return first
        from .mcp_server_runtime_helpers import (
            current_calling_agent_id,
            current_calling_host_kind,
        ...
        connect_args = {"mode": "connect", "session_id": sid}
        if confirm_token:
            connect_args["confirm_token"] = str(confirm_token)
        bound = self._tool_call("ai_session", connect_args, meta)
        if bound.get("confirmed") is True and str(bound.get("session_id") or "") == sid:
            return self.dispatch(project_root, mode="xaacp_attach", session_id=sid)
        if str(bound.get("_error") or "") == "confirm_required" and bound.get("confirm_token"):
            return {
                "ok": False,
                "status": "confirm_required",
                "error": (
                    "this caller has no binding on the gate yet; binding it there "
                    "is part of this connect and needs confirmation"
                ),
                "action": "xaacp_gate_connect",
                "session_id": sid,
                "confirm_token": bound.get("confirm_token"),
                "summary": bound.get("summary", ""),
                "remedy": (
                    f"ai_session(mode='connect', session_id='{sid}', "
                    "confirm_token=<the token above>)"
                ),
            }
        return {
            "ok": False,
            "status": "forbidden",
            "error": "gate-side session bind failed",
            "bind_result": bound,
        }
    ...
    def claim_seat(
        ...
        *,
        ...
        sid = str(session_id or "").strip()
        # #1095 step 4: seat entry is an explicit actor lifecycle event, so it
        # ATTACHES the candidate explicitly. It used to probe with a directory
        # READ whose failure triggered the bridge's auto-connect -- a read used
        # as a hidden registration door. A missing gate binding now surfaces as
        # needs_binding (remedy: explicit ai_session(connect)).
        ready = self.dispatch(project_root, mode="xaacp_attach", session_id=sid)
        if not ready.get("ok"):
            return ready
    ...
    def _seat_lifecycle_call(
        ...
        *,
        ...
        sid = str(session_id or "").strip()
        # #1095 step 4: succession is an occupancy lifecycle event and attaches
        # the successor explicitly; RELEASE never mints a missing actor merely
        # to release (r0b2 804329fc-66d item 2), so it skips the attach.
        if attach:
            ready = self.dispatch(project_root, mode="xaacp_attach", session_id=sid)
            if not ready.get("ok"):
                return ready
...
class UnavailableXaacpAuthority:
    ...
    def __init__(self, reason: str, *, project_id: str = "") -> None:
        self.reason = reason
        # BOUND with an unusable credential: the exact project it was resolved
        # for. UNRESOLVABLE: blank -- nothing replayable may be minted from it.
        self.pinned_project_id = str(project_id or "").strip()
    ...
    # #1095 step 4: an unreachable authority can attach nothing; the lifecycle
    # caller owes the attach to the outbox, and connect reports it honestly.
    def attach_lifecycle(self, project_root: Path, **_kwargs: Any) -> dict:
        return _error("unavailable", self.reason)

    def connect_and_attach(self, project_root: Path, **_kwargs: Any) -> dict:
        return _error("unavailable", self.reason)
...
def remote_authority_for(project_root: Path):
    """Return remote authority for a cloud-bound project, ``None`` iff PROVEN local.
    ...
    The same rule now holds one step earlier (#1095, r0b2 dddda188-dad): an
    UNRESOLVABLE project identity is also UNAVAILABLE, never "local".
    """
    root = Path(project_root)
    ident = classify_xaacp_project(root)
    if ident.state == XAACP_PROJECT_LOCAL:
        # PROVEN local (or gate-side execution): the local-only floor is
        # untouched.
        return None
    if ident.state == XAACP_PROJECT_UNRESOLVABLE:
        return UnavailableXaacpAuthority(
            f"this project's canonical identity could not be read ({ident.reason}); "
            "refusing to fork XAACP into local SQLite on an unknown answer"
        )
    return _remote_authority_with_credential(root, ident.project_id)


#: The three answers to "which XAACP registry owns this project". There is no
#: fourth: an unreadable identity is UNRESOLVABLE, never folded into LOCAL.
XAACP_PROJECT_BOUND = "bound"
XAACP_PROJECT_LOCAL = "local"
XAACP_PROJECT_UNRESOLVABLE = "unresolvable"


@dataclass(frozen=True)
class XaacpProjectIdentity:
    state: str
    project_id: str = ""
    reason: str = ""


def classify_xaacp_project(project_root: Path) -> XaacpProjectIdentity:
    """WHICH PROJECT AM I, for XAACP -- typed, credential-free (#1095 step 4).

      BOUND(project_id)  cloud-bound: the gate owns the registry; no local
                         xaacp row may be written or refreshed.
      LOCAL              PROVEN local-unregistered (the registration store is
                         absent or has no row for this root), or gate-side
                         execution, which IS the canonical copy.
      UNRESOLVABLE       the question could not be answered (the config or the
                         registration store is unreadable). Treated as neither
                         local nor bound by every caller: no local write, no
                         fallback authority, owed debt stays owed.

    ``remote_authority_for``, the liveness touch and the attach-outbox scope
    proof all read THIS, so "cloud-bound" and "local" have one definition.
    """
    ...
    if current_gate_principal():
        return XaacpProjectIdentity(XAACP_PROJECT_LOCAL, reason="gate_canonical_copy")
    ...
    try:
        ...
        project_id = str(
            get_setting("sync.vps_hub_project_id", project_root=root, default="") or ""
        ).strip()
    except Exception as exc:  # noqa: BLE001 -- an unreadable config is not "unset"
        return XaacpProjectIdentity(
            XAACP_PROJECT_UNRESOLVABLE, reason=f"config_unreadable: {type(exc).__name__}"
        )
    if project_id:
        return XaacpProjectIdentity(XAACP_PROJECT_BOUND, project_id=project_id)
    # CANONICAL IDENTITY, NOT THE BACKLOG'S CONVERGENCE VERDICT (#981). The
    # backlog helper answers "converged enough to exchange rows" (needs an
    # org); XAACP asks "which project am I" -- borrowing the stricter verdict
    # once turned a missing org into a missing project and forked XAACP.
    try:
        from . import project_binding_resolver as pbr

        binding = pbr.resolve(root)
    except Exception as exc:  # noqa: BLE001 -- an unanswerable identity is NOT local
        return XaacpProjectIdentity(
            XAACP_PROJECT_UNRESOLVABLE, reason=f"resolver_raised: {type(exc).__name__}"
        )
    project_id = str(binding.project_id or "").strip()
    if project_id:
        return XaacpProjectIdentity(XAACP_PROJECT_BOUND, project_id=project_id)
    reason = str(getattr(binding, "reason", "") or "")
    if reason == pbr.REASON_LOCAL_UNREGISTERED:
        return XaacpProjectIdentity(XAACP_PROJECT_LOCAL, reason=reason)
    # REASON_LOCAL_REGISTRY_ERROR (or any other non-answer) is UNKNOWN.
    return XaacpProjectIdentity(XAACP_PROJECT_UNRESOLVABLE, reason=reason or "no_answer")


def _remote_authority_with_credential(root: Path, project_id: str):
    """The credential half of ``remote_authority_for`` for a project that
    ``canonical_project_id_for`` already proved cloud-bound."""
