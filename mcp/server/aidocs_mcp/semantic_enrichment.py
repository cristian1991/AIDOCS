...
def judge_semantic_verdicts(
    ...
    *,
    ...
    sub-millisecond; §XXVII keeps the guest off the hot path).
    """
    args = tool_input or {}
...
# ── Consumer C rail: per-INVOCATION edit receipts (#1075) ────────────
#
# _post_edit_reindex_and_grant records what THIS edit touched; the shared
# edit_result renderer reads back ONLY the receipts of the SAME tool
# invocation.
#
# WHY NOT THE OLD STASH. This used to be a process-global single slot
# (_PENDING_RADIUS) drained by "whatever edit_result runs next". The stash only
# wrote when the radius had dependents or anchored memories, so an edit of a
# dependency-free leaf did not overwrite it — and that edit's ack then drained
# ANOTHER edit's note. On the shared local daemon (conductor, lanes, subagents
# in one process) actor B's edit acknowledgement could carry actor A's dependent
# file paths and anchored memories. A failed edit did not drain either, so the
# next successful edit of a DIFFERENT file was acknowledged with the previous
# file's radius. r0b ruling 59b231f8-a13: no drain semantics anywhere.
#
# THE ATTRIBUTION RULE, by construction rather than by care:
#   1. receipts live in a ContextVar, never module state. Concurrent requests run
#      in copied contexts, so one request cannot see another's receipts. Values
#      are IMMUTABLE tuples: a copied context shares the object, and an in-place
#      append would leak across exactly the boundary this exists to hold.
#   2. a receipt renders only for the SAME project, and only if it was recorded
#      at or after the reading invocation's own started_at — a receipt left over
#      from any earlier call is older than started_at and is rejected, even if
#      two calls happen to run sequentially in one context.
#   3. every edit_result clears the receipts, success or failure.
#   4. EVERY edit records a receipt, including a dependency-free one, so there is
#      never a "nothing written, read someone else's" gap.

import contextvars as _contextvars

_EDIT_RECEIPTS: _contextvars.ContextVar[tuple[dict[str, object], ...]] = _contextvars.ContextVar(
    "aidocs_edit_receipts", default=()
)
# Fallback window ONLY when a caller supplies no started_at: a receipt must be
# this fresh to be attributed to the current invocation.
# (no wall-clock fallback window: see receipts_for_invocation, r0b B2)


def _norm_root(project_root: object) -> str:
    try:
        return str(Path(str(project_root)).resolve())
    except Exception:  # noqa: BLE001 — unresolvable root compares as its string form
        return str(project_root)


def record_edit_receipt(
    project_root: object,
    target: str,
    radius: dict[str, object] | None,
) -> None:
    """Record what ONE governed edit touched, for THIS invocation's ack only."""
    ...
    _EDIT_RECEIPTS.set(tuple(_EDIT_RECEIPTS.get()) + (receipt,))


def receipts_for_invocation(
    project_root: object,
    started_at: float | None,
) -> list[dict[str, object]]:
    """The receipts recorded by the invocation that started at ``started_at``.

    Never returns another project's receipt, and never one recorded before this
    invocation began. Read-only; ``clear_edit_receipts`` ends the invocation.
    """
    if project_root is None:
        return []
    root = _norm_root(project_root)
    out: list[dict[str, object]] = []
    for r in _EDIT_RECEIPTS.get():
        if r.get("project_root") != root:
            continue
        # Without started_at there is no time filter (r0b B2: a wall-clock
        # window dropped the early paths of a slow batch). What bounds it is
        # the ContextVar itself plus edit_result clearing on EVERY ack.
        if started_at is not None and float(r.get("at") or 0.0) < float(started_at):
            continue
        out.append(r)
    return out


def clear_edit_receipts() -> None:
    """End of an edit invocation: no receipt survives into the next call."""
    _EDIT_RECEIPTS.set(())
