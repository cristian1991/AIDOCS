...
# scope_daemon_url REMOVED with #787. It existed to bind a shared-daemon URL to
# one project by appending a validated ?root=<abs>, because the bare
# {"type": "http", "url": ...} entry had no other way to carry the root -- a URL
# inherits no environment. The daemon entry now launches the stdio shim, which
# receives AIDOCS_PROJECT_ROOT in its own env and appends ?root= itself at
# connect time (stdio_shim.py:102-118), skipping it when already present. Doing
# both would encode one fact in two places that can disagree, so the scoping
# moved to the shim and this function had no remaining caller.
# The gate proved it: vulture flagged it at 60% confidence and HARD-FAILED the
# deploy rather than let an orphan linger. Deleted rather than allowlisted --
# the allowlist is for functions with an owner ticket and a reason to survive,
# and this one has neither now that its only call site is gone.


def _derived_daemon_url() -> str:
    """The local daemon's endpoint, derived from its OWN health record.

    #812 step 2. The endpoint was already a fact this system owned in four
    places -- stdio_shim.DEFAULT_ENDPOINT (:60), the service default port, the
    CLI/server defaults, and write_daemon_health, which records the ACTUALLY
    RUNNING port. Asking an operator to retype it into mcp.local_daemon_url was
    one fact stored twice, and the copy that decided the transport DEFAULTED TO
    EMPTY -- silently selecting direct-stdio, the one shape that cannot carry
    host identity (CLAUDE_CODE_SESSION_ID is read only in stdio_shim.py:83).
    An operator who configured nothing landed on the degraded path and had no
    way to know. Measured cost: a project that never opted in spent a session
    unable to bind its own window.

    RETURNS "" ON ANY DOUBT, and that direction is deliberate. "" means
    direct-stdio, which is the long-standing behaviour, so a box this cannot
    read keeps working exactly as before. Pointing a shim at a daemon that is
    not there would trade one outage for another.

    NO HEALTH FILE IS NOT AN OUTAGE -- it is a legitimate no-daemon install
    (single project, locked-down box, CI). That case is why the presence check
    reads the record itself rather than trusting unreachable_status() alone:
    that function answers None for "reachable" AND for "never commissioned",
    and only one of those should get a shim.
    """
    try:
        from .aidocs_service import read_daemon_health
        from .daemon_reachability import unreachable_status

        health = read_daemon_health()
        if not isinstance(health, dict):
            return ""
        if unreachable_status() is not None:
            return ""
        port = int(health.get("port") or 0)
        if port <= 0:
            return ""
        return f"http://127.0.0.1:{port}/mcp"
    except Exception:  # noqa: BLE001 -- config generation must never wedge on a probe
        return ""
...
class RuntimeProjectSupportService:
    ...
    def ensure_claude_mcp_config(
        ...
        try:
            ...
            daemon_url = ""
        # #812 step 2: the setting is an OVERRIDE, not the switch. When it is
        # unset, derive the daemon's endpoint from its own health record rather
        # than defaulting to the transport that cannot carry identity. The
        # override is checked FIRST and wins outright, so an operator pointing
        # at a non-default port or a non-local daemon is still obeyed.
        if not daemon_url:
            daemon_url = _derived_daemon_url()
        # NOT scope_daemon_url'd here. The bare endpoint goes to the shim, which
        # appends ?root= from AIDOCS_PROJECT_ROOT at connect time
        # (stdio_shim.py:110-117) and skips it when already present. Baking the
        # root into the URL as well would encode the same fact twice, in two
        # places that can disagree -- and the env copy is the better source,
        # because it is the value the shim's own per-window process holds.
        ...
        # ── #787: DAEMON MODE GOES THROUGH THE SHIM, NOT STRAIGHT AT THE URL ──
        # #249 replaced the per-project stdio spawn with a bare
        # {"type": "http", "url": ...} entry. That killed per-window identity: a
        # stdio-spawned process inherits CLAUDE_CODE_SESSION_ID, and a URL
        # inherits nothing. #758 then BUILT stdio_shim.py to restore it --
        #     window --stdio--> shim (inherits the env)
        #                          --HTTP + X-Aidocs-Host-Session--> daemon
        # -- and THIS GENERATOR WAS NEVER UPDATED, so the shim has never been in
        # any project's path. The fix shipped and nothing pointed at it.
        #
        # Cost, measured 2026-08-17: with no host header the request carries
        # only a transport token that ROTATES between consecutive calls of one
        # agent (project_scope.py:140-145), so correlate_host_session becomes
        # the sole identity route -- and it refuses on >=2 live bindings, which
        # two host windows guarantee permanently. Downstream: #783, #785 cause
        # 2, #786.
        #
        # The shim reads its endpoint from AIDOCS_MCP_ENDPOINT and appends
        # ?root= from AIDOCS_PROJECT_ROOT (stdio_shim.py:102-118), so the daemon
        # is still the destination and the existing ?root= convention is
        # preserved rather than duplicated.
        #
        # ONE WRITER: this shares the interpreter resolution, env construction,
        # idempotence check and upsert below with the direct-stdio mode. The old
        # daemon branch had its OWN copy of all four and returned early -- which
        # is precisely how it drifted out of agreement with the shim in the
        # first place, and the twin shape that has cost this codebase six bugs
        # (#522/#774/#779/#781/#782/#786).
        if daemon_url:
            env["AIDOCS_MCP_ENDPOINT"] = daemon_url
            args = ["-m", "aidocs_mcp.stdio_shim"]
        else:
