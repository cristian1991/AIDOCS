...
Per aidocs-doctrine §X memory injection triggers, empire-doctrine §XII evidence
...
def _detect_host_kind() -> str:
    ...
    This name is the de-facto host_kind resolver for four other modules
    (``doctrine_resurface``, ``execution_index_store``,
    ``host_session_context_store``, ``sticky_grants_store`` all import it), so it
    keeps its exact old return contract — "unknown" rather than "" when nothing
    is known — while the actual resolution moves to
    ``agent_memory_epoch.resolve_host_identity``. New code must call that
    directly and handle the honest "".
    """
...
def _resolve_surfacing_epoch(
    ...
    *,
    ...
    """Current agent_memory_epoch, or "" when a link in the identity chain is
    unresolvable (caller then surfaces unconditionally — no dedup possible).
...
def _resolve_managed_session_id(
    ...
    try:
        ...
        return resolve_managed_session(
            ManagedModeService(), project_root, host_session_id=sid
        )
    except Exception:
        pass
    return ""
...
# aidocs-doctrine §X "memory injection triggers" / once-per-
