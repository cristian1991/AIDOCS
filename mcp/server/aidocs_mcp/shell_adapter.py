...
# ── #851/#771 (law 311bf3e6): audience-scoped continuations ─────────────
# ai_run is surface=GATE_ONLY by deliberate operator ruling (2026-07-26,
# ONE RUNNER PER SURFACE, tool_families/execution.py): harness agents hold
# governed native Bash; ai_run exists for WebMCP / no-shell callers. The
# Claude Code stdio surface therefore CANNOT load ai_run (measured three
# ways in #771; reproduced live in #851 and again during this fix), so a
# refusal naming it to that caller promises a path that cannot be walked.
# An unreachable remedy does not merely fail to help — it teaches working
# around the gate. Continuations are host-aware: the proven Claude Code
# adapter gets remedies it can actually take; gate-capable callers keep
# the exact ai_run call. Pinned by
# tests/security/test_named_remedy_is_reachable_311bf3e6.py (axis 5).
_HOSTS_WITHOUT_AI_RUN: frozenset[str] = frozenset({"claude_code"})


def _host_can_call_ai_run(host: str) -> bool:
    return (host or "").strip().lower() not in _HOSTS_WITHOUT_AI_RUN
...
def _native_continuation(command: str, *, same_command: bool = True, host: str = "") -> str:
    """The EXACT, usable continuation for a denied native call.

    Governed Bash is never a dead end: every denied native path tells the
    caller how to continue the governed way — and the named door must
    exist for THAT caller (law 311bf3e6). Gate-capable callers get the
    ai_run call (the AIDOCS-managed shell: full law — future-sight x-ray,
    receipt, output-guard, audit). A Claude Code agent, whose surface has
    no ai_run, gets the remedies it can actually take.
    """
    cmd = (command or "").strip()
    if not _host_can_call_ai_run(host):
        return (
            "no agent-side dispatcher on this surface (ai_run is gate-only): "
            "re-issue the command in a form the governed native policy "
            "allows, or ask the operator to run or allow it"
        )
...
def _continuation_text(command: str, *, same_command: bool = True, host: str = "") -> str:
    if not _host_can_call_ai_run(host):
        return (
            " ↪ Continuation on this surface: ai_run is GATE_ONLY (ONE "
            "RUNNER PER SURFACE, 2026-07-26) and is not exposed to Claude "
            "Code agents, so it is NOT the remedy here. Either re-shape "
            "the command so the governed native policy can pass it, or "
            "hand the step back and ask the operator to run or allow it. "
            "Do not hunt for an unblocked spelling — the refusal, not the "
            "matcher, is the verdict. No raw unreceipted Bash is unlocked."
        )
    cont = _native_continuation(command, same_command=same_command, host=host)
...
def fail_closed_deny_envelope(detail: str = "", host: str = "") -> dict[str, Any]:
    ...
    """
    reason = "native shell enforcement failed; action remains blocked."
    if _host_can_call_ai_run(host):
        reason += _AI_RUN_GUIDANCE
    else:
        # #851: no ai_run on this surface — name the door that exists.
        reason += " Ask the operator to retry or repair the enforcement seam."
    if detail:
        reason += f" ({detail})"
...
def _render_enforcement(res: Any, command: str = "", *, host: str = "") -> dict[str, Any]:
    action = res.action
    ...
    if action == _enf.ACTION_DENY:
        # Hard deny still offers a continuation — never a dead end — and
        # the continuation is audience-scoped (#851, law 311bf3e6).
        same = getattr(res, "fallback_same_command", True)
        return _deny_env(
            res.reason + _continuation_text(command, same_command=same, host=host),
            res.blocked_by or "denied",
            continuation=_native_continuation(command, same_command=same, host=host),
        )
    if action == _enf.ACTION_EXECUTE_NATIVE:
        return _deny_env(
            "native shell execution is disabled (Batch 2.0-A renders "
            "verdicts only; no native process runs)."
            + _continuation_text(command, host=host),
            "native_execution_disabled_2a",
            continuation=_native_continuation(command, host=host),
        )
    # ACTION_FALLBACK (or any residual allow): native blocked; continuation
    # is audience-scoped (#851).
    reason = res.reason or "native shell blocked"
    same = getattr(res, "fallback_same_command", False)
    return _deny_env(
        reason + _continuation_text(command, same_command=same, host=host),
        res.blocked_by or "fallback_to_ai_run",
        continuation=_native_continuation(command, same_command=same, host=host),
    )


#: #489: waiting budget for the ONE synchronous native_allow receipt write on
#: PreToolUse. Warm PreToolUse is 0.3-0.5s; the client hook budget is 10s. 750ms
#: (single attempt, no retry) keeps the worst case near ~1.3s while absorbing
#: the common sub-second lock holds; longer holds fail closed to ai_run.
NATIVE_ALLOW_RECEIPT_BUDGET_MS = 750
...
def _record(
    ...
    *,
    ...
    strict_budget_ms: int | None = None,
    **route_extra: Any,
) -> None:
    """Record the shell verdict row. ``strict_budget_ms`` (#489): a bounded,
    single-attempt write whose failure RAISES — used only for the native_allow
    receipt, which the caller must not return ALLOW without."""
    try:
        from .execution_index_store import ExecutionIndexStore

        _payload_extra = {k: v for k, v in route_extra.items() if v is not None}
        _budget_kw = {} if strict_budget_ms is None else {"write_budget_ms": int(strict_budget_ms)}
        ...
        if strict_budget_ms is not None:
            raise
...
def _native_capability_safe(command: str) -> tuple[bool, str]:
    ...
    # THE FLAG MUST BELONG TO THE TAIL (measured 2026-09-05).
    #
    # This read `_re.search(r"\btail\b", c) and _re.search(<follow>, c)` -- two
    # searches over the WHOLE command string, independently. Any -f/-F anywhere
    # satisfied the second half, including one belonging to another command.
    #
    # MEASURED: `git commit -F <msgfile> | tail -20` was refused as a
    # follow/stream. The -F is git's "read the message from this file"; the
    # tail is bounded at 20 lines; there is no follow in the command at all.
    # `echo x -F | tail -2` reproduces it with nothing but an echo.
    #
    # A REFUSAL THAT MISNAMES ITS CAUSE IS WORSE THAN A MISSING ONE. The remedy
    # it implies -- drop the follow -- cannot be performed, because there is no
    # follow to drop. Dropping the innocent `| tail -20` DID clear it, which
    # then framed the redirect as the culprit and hid the real rule. Every
    # other refusal in this codebase names its own cause precisely (window_key
    # carries eight distinct reasons rather than one "unavailable"); this one
    # sent the reader after a stream that never existed.
    #
    # Each `tail` is now matched WITH ITS OWN ARGUMENT RUN -- up to the next
    # pipe or command separator -- so the flag is read from the invocation it
    # actually modifies. Still refused: `tail -f`, `tail -F`, `tail --follow`,
    # `tail --follow=name`, `tail file -f`. No longer refused: a neighbour's -F
    # standing beside a bounded tail.
    for _tail in _re.finditer(r"\btail\b([^|;&\n]*)", c):
        if _re.search(r"(^|\s)(-[A-Za-z]*[fF]\b|--follow\b)", _tail.group(1)):
    ...
    try:
        ...
        if res.action == _enf.ACTION_EXECUTE_NATIVE:
            ...
            if receipted and _identity_ok and native_capable:
                # #489: the receipt shell_receipt._has_native_allow correlates
                # on PostToolUse. DURABLE BEFORE ALLOW, never write-behind; its
                # wait is BOUNDED, and a receipt that cannot land in time
                # degrades speed before authority: governed ai_run route.
                try:
                    _record(
                        project_root,
                        host,
                        ...
                        strict_budget_ms=NATIVE_ALLOW_RECEIPT_BUDGET_MS,
                    )
                except Exception as _receipt_exc:
                    _same = _fallback_same_command_for(env)
                    return _deny_env(
                        "native execution refused — the native_allow receipt could "
                        f"not be made durable within {NATIVE_ALLOW_RECEIPT_BUDGET_MS}ms "
                        f"({type(_receipt_exc).__name__}: execution index busy). No "
                        "native shell runs without a durable receipt."
                        + _continuation_text(env.command, same_command=_same, host=env.host),
                        "native_receipt_not_durable",
                        continuation=_native_continuation(
                            env.command, same_command=_same, host=env.host
                        ),
                        family=fam.family,
                    )
                return _allow_env(
            ...
            if not live["ok"] and not _mp_ok:
                ...
                return _deny_env(
                    ...
                    f"{live['reason']}. Repair: {live['repair']}."
                    + _continuation_text(env.command, same_command=_same, host=env.host),
            ...
            if not native_capable:
                ...
                return _deny_env(
                    f"native execution refused — {_cap_reason}."
                    + _continuation_text(env.command, same_command=_same, host=env.host),
                    "native_capability_unsafe",
                    continuation=_native_continuation(
                        env.command, same_command=_same, host=env.host
