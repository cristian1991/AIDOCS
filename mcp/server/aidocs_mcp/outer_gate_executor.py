...
applied via the SHARED `with_gate_execution_scope` seam (which sets the
`with_target_project_root` ContextVar, and the gate principal where one exists) so
`resolve_project_root()` inside the canonical tools resolves to the bound project
deterministically. Every gate-side dispatch site uses that one seam — see #714.
...
class ReadOnlyTierRExecutor:
    ...
    def _ensure_server(self):
        if self._server is None:
            if self._server_factory is not None:
                ...
                self._server = create_server(tools_profile="read_only", expose_deploy=True, expose_run=True)
        return self._server

    def _schema(self, name: str) -> dict:
        if name not in self._schemas:
            srv = self._ensure_server()  # also populates C.20 direct bindings
            from . import tool_interface as _ti

            spec = _ti.get(name)
            direct = _ti.direct_impls().get(name)
            if spec is not None and (
                spec.cls == _ti.RUN
                or (spec.cls == _ti.READ and direct is not None)
            ):
                self._schemas[name] = _ti.schema_for(spec)
            else:
    ...
    def __call__(self, entry: dict, request) -> Any:
        ...
        # Same identity boundary as `run()` — the READ_EXEC path dropped the
        # principal too. `request` carries it; passing it here is what lets an
        # impl authorize on the caller the gate actually authenticated.
        return self._run(name, args, root, principal=getattr(request, "principal", None))
    ...
    def run(
        ...
        *,
        ...
        # WHICH CALLER, AND NOTHING STANDING IN FOR IT.
        #
        # This was `host_session_id or sid` -- so a request with no caller
        # silently borrowed the SELECTED SESSION as its identity. That is the
        # third substitution in this one line's history (the token supplied the
        # value, the session supplied the fallback), and both are forbidden:
        # WHO, the token and the session are separate facts from WHICH CALLER.
        #
        # An absent caller is "". `set_request_host_identity("")` still marks
        # the request SCOPED, which #672 distinguishes from "never scoped" --
        # so downstream reads get an honest empty instead of falling through to
        # the shared process global and inheriting the stdio caller's identity,
        # which on a multi-tenant daemon is A DIFFERENT ACTOR.
        hid = str(host_session_id or "").strip()
        from .mcp_server_runtime_helpers import (
            reset_request_host_identity,
            set_request_host_identity,
            with_gate_execution_scope,
        )
        ...
        with self._lock:
            ...
            try:
                # THE PRINCIPAL MUST CROSS THIS BOUNDARY (2026-08-30).
                #
                # MEASURED on the live gate: ai_whoami reported authenticated
                # SUPER_ADMIN while ai_run refused with every rung of the auth
                # ladder absent — "gate_principal=absent, token=absent,
                # binding=absent, machine=absent". The last three are LOCAL by
                # design and no remote caller can satisfy them, so rung 0 is the
                # only one a WebMCP principal can pass, and it reads a ContextVar
                # that NOTHING ON THIS PATH EVER SET.
                #
                # `OuterGate.run` HAS the principal — it authenticates with it,
                # resolves the user from it, and checks project_run scope with it
                # — and then handed off to a runner whose signature could not
                # accept one. The gate authenticated an identity and dropped it
                # at this boundary. The transport's sibling dispatch site passes
                # `principal=ctx.principal` through the SAME shared seam and its
                # comment says the override "is visible inside the impl"; this
                # site used the seam without the argument.
                #
                # DEFAULT None IS DELIBERATE AND FAIL-CLOSED. An explicit None
                # CLEARS any inherited principal (the seam's documented
                # semantics) rather than preserving it. An executor invocation
                # that states no principal must not silently borrow the previous
                # request's — that is the last-binder-wins cross-tenant leak the
                # scope helper exists to make impossible, and it is worse on a
                # thread-reusing gate than a refusal is.
                with with_gate_execution_scope(root, principal=principal):
    ...
    def _run(self, name: str, args: dict, root: Path, principal: Any = None) -> Any:
        from .mcp_server_runtime_helpers import with_gate_execution_scope

        with self._lock:
            srv = self._ensure_server()  # populates direct bindings
            from . import tool_interface as _ti
