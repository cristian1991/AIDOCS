...
It COMPOSES existing law (it does not reimplement it):
  ...
  2. provider dialect detector            (shell_provider_dialect)  ← PS/cmd
...
class ShellPolicy:
    ...
    def evaluate(
        ...
        *,
        ...
        # 2. Provider dialect (PowerShell / cmd). A critical finding is a
        #    hard deny regardless of enable flag — falling back to ai_run
        #    would just run the dangerous command another way.
        findings = _dialect.evaluate_provider(provider, envelope.command)
        ...
        # Native (host_native / monitor): capability + enable gate.
