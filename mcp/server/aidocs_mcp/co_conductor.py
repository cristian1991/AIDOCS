"""Co-conductor — second-tier sovereign aid (Empire doctrine 2026-05-01).

Castle hierarchy (from #105):
- Empire          : the operator. Rules by judgment.
- Conductor     : crowned cerberus head. Strikes at Empire's will.
- Co-Conductor  : black wolf head. Focus + alert. Backs the conductor when
                  ...
                  requests). Helps rule the kingdom in Empire's absence.
- Lane Experts  : white wolf head. No will of their own; serve the strike.

Status: DEFERRED. The Empire has chosen to ship Worker/Expert split first;
...
# TODO(co-conductor-deferred, Empire 2026-05-01): wire the review path for
...
class CoConductor:
    ...
    def review_conductor_tool_request(
        ...
        *,
        ...
        up. Conductor → co-co → (rarely) Empire.
    ...
    def relieve_conductor(self, *, reason: str = "") -> dict[str, Any]:
        """Take over conducting in the Empire's absence.
