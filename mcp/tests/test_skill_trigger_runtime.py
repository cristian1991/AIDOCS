...
def _register_superpowers_provider(
    ...
    *,
    ...
    skill_definitions = skills or {
        ...
        "verification-before-completion": ("Imported verification skill.", "external, provider"),
    }
...
def _make_runtime_with_incompatible_superpowers(tmp_path: Path) -> tuple[RuntimeService, Path, str]:
    runtime, project_root = _make_runtime(tmp_path)
    session_id = "session-a"
...
def test_runtime_selects_external_brainstorming_for_creative_task(tmp_path: Path) -> None:
    ...
    result = runtime.skill_trigger_state(project_root, session_id, intent="brainstorming")
...
def test_incompatible_provider_is_not_auto_activated_without_override(tmp_path: Path) -> None:
    runtime, project_root, session_id = _make_runtime_with_incompatible_superpowers(tmp_path)
...
def test_runtime_logs_why_external_skill_was_triggered(tmp_path: Path) -> None:
    ...
    result = runtime.skill_trigger_state(project_root, session_id, intent="verification")
...
def test_selected_skills_rank_ahead_of_compatible_unselected_auto_triggered_skills(tmp_path: Path) -> None:
    runtime, project_root = _make_runtime(tmp_path)
    session_id = "session-a"
    _register_superpowers_provider(
        ...
        skills={
            "brainstorming": ("Imported brainstorming skill.", "external, provider"),
            "creative-helper": ("Imported creative helper.", "external, provider, brainstorming"),
        },
    )
    runtime.hub.skills.set_selected_skills(project_root, session_id, ["superpowers_external/brainstorming"])

    result = runtime.skill_trigger_state(project_root, session_id, intent="brainstorming")

    assert result["active_skills"][:2] == [
        "superpowers_external/brainstorming",
        "superpowers_external/creative-helper",
    ]


def test_compatible_unselected_imported_skill_can_auto_trigger_from_intent(tmp_path: Path) -> None:
    runtime, project_root = _make_runtime(tmp_path)
    session_id = "session-a"
    _register_superpowers_provider(
        runtime,
...
def test_bundled_writing_plans_still_resolves_to_aidocs_native_override(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result["triggered"][0]["provider"] == "aidocs"
    assert result["triggered"][0]["runtime_provider"] == "aidocs"
    assert result["triggered"][0]["override_mode"] == "aidocs_native_override"
    assert result["imported_skill_state"]["mode_metadata"]["active_skill_modes"] == {
        "writing-plans": "aidocs_native_override"
    }
    assert annotated["override_modes"] == {"writing-plans": "aidocs_native_override"}


def test_bundled_brainstorming_still_uses_provider_content_runtime_control(tmp_path: Path, monkeypatch) -> None:
    _patch_bundled_superpowers(
        monkeypatch,
        tmp_path,
        skills={
    ...
    assert annotated["override_modes"] == {"brainstorming": "provider_content_aidocs_runtime"}


def test_bundled_systematic_debugging_remains_provider_native(tmp_path: Path, monkeypatch) -> None:
    _patch_bundled_superpowers(
        monkeypatch,
        tmp_path,
        skills={
    ...
    runtime, project_root = _make_runtime(tmp_path)
    session_id = "session-a"
    runtime.hub.skills.set_selected_skills(project_root, session_id, ["systematic-debugging"])
...
def test_bundled_skill_mode_metadata_prefers_canonical_selected_skill_over_provider_leaf_collision(
    ...
    _register_external_provider(
        ...
        version="5.1.0",
        skills={
            "brainstorming": ("Other provider brainstorming skill.", "external, provider"),
        },
    )
    runtime.hub.skills.set_selected_skills(
        project_root,
    ...
    bundled_decision = next(item for item in result["triggered"] if item["skill_id"] == "brainstorming")
    ...
    assert annotated["imported_skill_state"]["mode_metadata"]["selected_skill_modes"] == {
        "brainstorming": "provider_content_aidocs_runtime",
        "other_external/brainstorming": "provider_native",
    }
