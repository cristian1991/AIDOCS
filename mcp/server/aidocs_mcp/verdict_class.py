...
#: A SECURITY refusal of ONE invocation that is nobody's offence. The security
#: floor could not establish that the action is safe — protected-path
#: inspection did not complete, or the judge produced a decision no gate knows
#: how to route — so the call is refused, but the CALLER did nothing wrong: the
#: defect is in our own judge. Hence: OUTCOME_BLOCK (nothing latched), NO
#: strike, NO freeze, and — unlike rung 3 — NOT agent-cancellable and NOT
#: dissolvable by detected user-intent, because there is no pending action to
#: approve and intent cannot turn "unknown" into authority (r0b 3943b325-5be).
CLASS_SECURITY_BLOCK = "security_block"
...
VALID_VERDICT_CLASSES: frozenset[str] = frozenset(
    {
        ...
        CLASS_NO_OFFENCE,
        CLASS_SECURITY_BLOCK,
    },
)

#: The STRIKE-BEARING rungs. This set used to be named SECURITY_CLASSES and was
#: read directly by issues_strike, so "is security-relevant" and "accrues a
#: strike" were one fact. They are not — CLASS_SECURITY_BLOCK is
#: security-relevant and must never strike. Security relevance is answered by
#: is_security_class below (fail-closed: everything except the two explicitly
#: non-security classes), so no second enumerated set exists to drift from it.
STRIKE_CLASSES: frozenset[str] = frozenset(
...
def issues_strike(verdict_class: str | None) -> bool:
    """Does this class accrue a strike? Only the two strike-bearing rungs do.

    Reads STRIKE_CLASSES, not is_security_class: CLASS_SECURITY_BLOCK is
    security-relevant but is our own judge's defect, never the caller's offence.
    """
    return (verdict_class or "").strip().lower() in STRIKE_CLASSES
...
def freezes_agent(verdict_class: str | None) -> bool:
    ...
    if value in (
        ...
        CLASS_NO_OFFENCE,
        CLASS_SECURITY_STRIKE,
        # An invocation-only refusal: nothing is latched, so nothing freezes.
        CLASS_SECURITY_BLOCK,
    ):
        return False
    return True
...
#: Security-floor UNCERTAINTY, not an offence (r0b 3943b325-5be). Protected-path
#: inspection did not complete, or the judge returned a decision no gate knows
#: how to route. Classified BEFORE the intent override: detected user-intent
#: must never turn "the judge could not establish this is safe" into
#: CLASS_NO_OFFENCE.
JUDGE_SECURITY_INSPECTION_FAILURE_BLOCKED_BY = "judge_security_inspection_failure"
JUDGE_TAXONOMY_UNKNOWN_DECISION_BLOCKED_BY = "judge_taxonomy_unknown_decision"
#: #1097: the judge RAISED before producing any decision. Same tier as the two
#: above -- uncertainty, not offence -- and for the same reason it sits before
#: the intent override: measured 2026-09-22, an unregistered blocked_by fell to
#: CLASS_NO_OFFENCE the moment user intent was detected, so intent laundered
#: "the judge crashed" into "nothing happened".
JUDGE_PRECHECK_UNAVAILABLE_BLOCKED_BY = "judge_precheck_unavailable"
_SECURITY_BLOCK_BLOCKED_BY: frozenset[str] = frozenset(
    {
        JUDGE_SECURITY_INSPECTION_FAILURE_BLOCKED_BY,
        JUDGE_TAXONOMY_UNKNOWN_DECISION_BLOCKED_BY,
        JUDGE_PRECHECK_UNAVAILABLE_BLOCKED_BY,
    },
)
...
def classify(
    *,
    ...
    # SECURITY-FLOOR UNCERTAINTY — before the intent override, deliberately.
    # Intent removes an OFFENCE; this is not one, it is the judge failing to
    # finish, and intent cannot supply the missing inspection.
    if bb in _SECURITY_BLOCK_BLOCKED_BY:
        return CLASS_SECURITY_BLOCK
...
def outcome_for(
    *,
    ...
    if resolved == CLASS_SECURITY_BLOCK:
        return OUTCOME_BLOCK, resolved
...
def gate_permission_for(verdict_class: str | None) -> str:
    ...
    ``CLASS_SECURITY_BLOCK`` files under NO permission: nothing may lift an
    unfinished security inspection, so it must never reach the
    ``run_destructive`` approval chain.
    """
    if normalize(verdict_class) == CLASS_SECURITY_BLOCK:
        return ""
