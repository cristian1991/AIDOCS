...
LOCAL_ONLY = frozenset(
    {
        ...
        # ── Host-local config / UX (would mutate the deployed gate host) ──
        # `config_get` (read-only) stays gate-exposed; `config_set` writes to
        # the GATE host's config and would affect every tenant — never remote.
        "config_set",
        "notifications_clear",  # operator UX on the host
        "ai_backlog",  # dev surface; surfaces internal TODOs
    },
)
        ...
        "schema": {
            ...
            "properties": {
                ...
                "confirm_token": {
                    "type": "string",
                    "description": (
                        "Required to apply. Format: 'confirm-org-select <org_id>'. "
                        "Omit on the first call to receive it in the confirm_required "
                        "response."
                    ),
                },
            },
        ...
        "schema": {
            ...
            "properties": {
                ...
                "confirm_token": {
                    "type": "string",
                    "description": (
                        "Required to actually apply. Format: "
                        "'confirm-project-select <project_id>'. "
                        "Omit on the first call to receive the "
        ...
        "schema": {
            ...
            "properties": {
                ...
                "confirm_token": {
                    ...
                    "description": (
                        "Required to actually apply. Format: "
                        "'confirm-session-select <session_id>'. "
                        "Omit on the first call to receive the "
                        "token in the confirm_required response."
                    ),
                },
...
PROJECT_READ_TOOLS = frozenset(
    n for n, s in PROJECT_TOOL_SPECS.items() if s["cls"] == CLASS_SELECTOR
)
PROJECT_EDIT_TOOLS = frozenset(n for n, s in PROJECT_TOOL_SPECS.items() if s["cls"] == CLASS_IMPORT)
...
def resolve(
    ...
    *,
    ...
    """
    from .outer_gate_projects import resolve_execution_grant
    ...
    if cls == CLASS_RUN:
        ...
        from .outer_gate_projects import resolve_execution_grant as _g
