...
class RuntimeBootstrapService:
    ...
    def _seed_factory_memory_into_index(self, root) -> dict:
        ...
        if not seed_db.is_file():
            return empty
        try:
            # read_only=True, and it is the strongest ruling available here:
            # this is SHIPPED PACKAGE DATA, built at build time and never
            # written by the runtime — the docstring above already says the
            # bootstrap "only OPENS it (read-only SELECT)". Enforcing that in
            # sqlite means a future writer gets a refusal instead of quietly
            # mutating a build artefact, and journal_mode (the one pragma a
            # reader cannot set) is correctly skipped rather than trying to
            # rewrite the header of a file we do not own.
            #
            # This file was NOT on the ratchet's debt list: `_sqlite3.connect`
            # was invisible to the AST census, which is the alias blindness
            # fixed alongside this migration (2026-08-26).
            seed_conn = _canonical_connect(
                str(seed_db), read_only=True, row_factory=False,
            )
    ...
    def project_init(
        ...
        # #628: templates_root is Optional — the canonical resolver returns None
        # rather than a fabricated path when no real template tree exists. Fall
        # back to a fresh resolve (which also probes THIS project), then account
        # for the miss visibly instead of dereferencing None.
        if templates_root is None:
            from .host_services.path_resolver_service import find_templates_root

            templates_root = find_templates_root(project_root=root)
        aidocs_bundle_root = templates_root.parent if templates_root is not None else None
        ...
        # RESILIENT: a locked/unwritable index db (dashboard holding the WAL,
        # AV, another MCP instance) must NOT kill the whole init half-way —
        # the legacy marker + registry row below still commission the project,
        # and heal_legacy_commission stamps it forward at the next bootstrap.
        # Visible accounting, never a silent skip.
        try:
            ...
            skipped.append(
                f"commission stamp FAILED (index db unavailable: {exc}); "
                "will heal-forward at next bootstrap",
            )
        ...
        if aidocs_bundle_root is None:
            # Visible accounting — never a silent skip (#628): an installed
            # runtime with no source checkout beside it carries no bundle tree.
            skipped.append(
                ".MEMORY/.aidocs bundle payload: no AIDOCS template tree resolvable "
                "(set AIDOCS_PATH to a source checkout, or deploy a runtime that "
                "ships core/.MEMORY/.aidocs/)",
            )
        else:
        ...
        for tmpl_name in ["AGENTS.md", "CLAUDE.md"]:
            ...
            if dest.exists():
                ...
                if end_idx >= 0:
                    ...
                    managed_content = managed_content.rstrip() + "\n\n" + existing
                # Always backup when overwrite changes content (Empire directive 2026-05-12):
                ...
                try:
                    ...
                    try:
                        with _tf.NamedTemporaryFile(mode="w", suffix=".gh.out", delete=False) as _f:
                            _gh_out = _f.name
                        # #345: routed through audited_run (ledger row per
                        # spawn); kwargs pass through UNCHANGED.
                        ...
                        with open(_gh_out, "w", encoding="utf-8") as _fh:
                            result = audited_run(
                                ...
                                fingerprint=("runtime_bootstrap_service.py", "project_init", "subprocess.run"),
                                reason="gh-repo-create",
