...
# A token bound (RFC 8707) to a resource this route does not serve. Route-
# scoped, never a credential refusal: the same token is valid on /v1/mcp.
REASON_RESOURCE_MISMATCH = "resource_mismatch"
...
# #972: the drain could not durably record a wire-mapper state (blocked,
# released, local-only) on a queue row. It then sends NOTHING that cycle: a row
# whose state did not land is still `pending`, and a sibling's applied verdict
# would consume it — or a released row would go out and never be settled.
REASON_QUEUE_STATE = "queue_state_transition_failed"
...
_NON_CREDENTIAL_403: dict[str, str] = {
    ...
    "resource_mismatch": REASON_RESOURCE_MISMATCH,
}
...
REASON_REMEDY: dict[str, str] = {
    ...
    REASON_QUEUE_STATE: (
        "a queue row's state could not be written locally, so nothing was sent "
        "this cycle — the local backlog database is locked or failing; see "
        "`state_failures` for which rows, and repair the store before draining"
    ),
    REASON_AUTH_REFUSED: (
    ...
    REASON_RESOURCE_MISMATCH: (
        ...
        "than /v1/backlog — a gate that predates the desktop backlog-route binding "
        "refuses it; the credential is NOT revoked, was not latched, and stays "
        "valid for XAACP"
    ),
...
def hub_route(project_root: Path | None = None) -> HubRoute:
    ...
    from . import gate_credential_renewal as _renewal
    from .operator_token_resolution import GATE_CRED_ABSENT, GATE_CRED_OK

    # ONE CREDENTIAL DOOR (#1063, r0b ruling 2026-09-14). This read
    # `cached_gate_credential()` directly while XAACP, from the SAME box and
    # the SAME cache, went through `ensure_gate_credential` -- the custody and
    # renewal door. Measured: XAACP kept reaching the gate while 214 backlog
    # writes sat behind `gate_credential_unusable`, because an expired-but-
    # renewable bearer renewed for one caller and was terminal for the other.
    # Two doors onto one credential is a rival custody; backlog now uses
    # XAACP's. The door still spends nothing on a live bearer, refuses a
    # latched (#992) one without renewing it, and self-limits renewal to one
    # attempt per window on disk -- so this cannot become the 401 loop.
    cred = _renewal.ensure_gate_credential(project_root=project_root).credential
...
# ── queue → wire (#972) ──────────────────────────────────────────────────────
#
# The queue keeps the STORE's canonical snake_case field dict; the authority's
# route speaks camelCase. Nothing translated between them, so every multi-word
# field (completedAt, mergedInto, linkedTaskId, createdInSessionId, the creation
# time, ...) was silently dropped at the authority for the outbox's whole life.
#
# CLOSED WORLD (r0b 58337626 / 8d674504): every key the store can emit is
# classified below — pinned by an AST census of every `_emit_backlog` call site
# (test_backlog_wire_mapper_972) — and anything else BLOCKS its whole intent
# before the network. "Drop the unknown key and submit the rest" is refused:
# the server would apply a half, and the verdict fold would then consume the
# queue row, losing the dropped field for good.

WIRE_SUBMIT = "submit"
WIRE_BLOCKED = "blocked"
WIRE_LOCAL_ONLY = "local_only"
WIRE_UNMAPPED = "unmapped"

BLOCK_UNMAPPED = "unmapped_fields"
BLOCK_SERVER_CREATE_UNSUPPORTED = "server_create_unsupported"
BLOCK_MERGE_TARGET_UNPROVEN = "merge_target_unproven"
BLOCK_HELD_BEHIND = "held_behind_blocked"
#: The durable intent's `fields` no longer parse. An UNKNOWN write, not an
#: empty one: blocked with its bytes kept, never retired as "nothing to send".
BLOCK_MALFORMED_FIELDS = "malformed_fields"

#: Single-word keys the route accepts under the same name.
_WIRE_PASS = frozenset({"content", "title", "status", "priority", "kind", "difficulty", "tags"})

#: The explicit snake → camel table. Explicit on purpose: an "already camel"
#: key is NOT passed through merely for looking like a wire key.
_WIRE_RENAME: dict[str, str] = {
    "completed_at": "completedAt",
    "removed_at": "removedAt",
    "removed_reason": "removedReason",
    "linked_task_id": "linkedTaskId",
    "source_task_id": "sourceTaskId",
    "promoted_from_todo_id": "promotedFromTodoId",
    "created_in_session_id": "createdInSessionId",
    # The merge relation travels ONLY as the identity captured at merge time.
    "merged_into_global_id": "mergedInto",
}

#: Fields the route's CREATE path does not store yet. An add carrying one would
#: land a done/removed item with its status but not its date — a half-applied
#: state — so it BLOCKS until the route supports it (then empty this set).
SERVER_CREATE_UNSUPPORTED: frozenset[str] = frozenset(
    {"completed_at", "removed_at", "removed_reason"}
)

_CLS_PASS = "pass"
_CLS_RENAME = "rename"
_CLS_LIFT_CREATED_AT = "lift_created_at"
_CLS_SERVER_OWNED = "server_owned"
_CLS_AUDIT_ONLY = "audit_only"
_CLS_LOCAL_DISPLAY = "local_display"
_CLS_CREATE_UNSUPPORTED = "create_unsupported"


def classify_wire_field(op: str, key: str) -> str:
    """How ONE emitted key crosses to the authority for ``op``; WIRE_UNMAPPED
    when no rule covers it (which blocks its intent)."""
    if key in _WIRE_PASS:
        return _CLS_PASS
    if key == "created_at":
        # The route reads the creation time from the intent's TOP level, and
        # only at create. On any other op it is a shape no emitter produces.
        return _CLS_LIFT_CREATED_AT if op == "add" else WIRE_UNMAPPED
    if op == "add" and key in SERVER_CREATE_UNSUPPORTED:
        return _CLS_CREATE_UNSUPPORTED
    if key in _WIRE_RENAME:
        return _CLS_RENAME
    if key == "updated_at":
        return _CLS_SERVER_OWNED  # the authority stamps its own updatedAt
    if key == "reason":
        # The evidence trail for a state change (#710). The server row has no
        # home for it; the local journal keeps it. Reported, never silent.
        return _CLS_AUDIT_ONLY
    if key == "merged_into":
        return _CLS_LOCAL_DISPLAY  # the local #N pointer; never an identity
    return WIRE_UNMAPPED


def to_wire_intent(intent: dict) -> dict:
    """Map ONE queued intent to the authority's wire shape. Pure: the queued
    intent (and so the durable queue row) is never modified.

    Returns ``disposition`` — WIRE_SUBMIT (with ``wire``), WIRE_BLOCKED (with
    ``reason`` and ``unmapped_fields``; never a sendable body) or
    WIRE_LOCAL_ONLY (nothing in it is server item state) — plus
    ``local_audit_fields``: the keys deliberately kept local-only."""
    op = str(intent.get("op") or "")
    fields = dict(intent.get("fields") or {})
    classes = {k: classify_wire_field(op, k) for k in fields}

    def _blocked(reason: str, keys: list[str]) -> dict:
        return {
            "disposition": WIRE_BLOCKED,
            "reason": reason,
            "unmapped_fields": keys,
            "wire": None,
            "local_audit_fields": [],
        }

    unmapped = sorted(k for k, c in classes.items() if c == WIRE_UNMAPPED)
    if unmapped:
        return _blocked(BLOCK_UNMAPPED, unmapped)
    unsupported = sorted(k for k, c in classes.items() if c == _CLS_CREATE_UNSUPPORTED)
    if unsupported:
        return _blocked(BLOCK_SERVER_CREATE_UNSUPPORTED, unsupported)
    if (
        "merged_into" in fields
        and "merged_into_global_id" not in fields
        and fields["merged_into"] is not None
    ):
        # A display #N with no identity captured beside it (a merge queued
        # before identity capture, or a seeded row). #N renumbers, so resolving
        # it now could name a DIFFERENT umbrella: migration debt, never a guess.
        return _blocked(BLOCK_MERGE_TARGET_UNPROVEN, ["merged_into"])

    wire_fields: dict[str, Any] = {}
    created_at = None
    for key, cls in classes.items():
        if cls == _CLS_PASS:
            wire_fields[key] = fields[key]
        elif cls == _CLS_RENAME:
            wire_fields[_WIRE_RENAME[key]] = fields[key]
        elif cls == _CLS_LIFT_CREATED_AT:
            created_at = fields[key]
        elif cls == _CLS_LOCAL_DISPLAY and "merged_into_global_id" not in fields:
            # Only reachable with a None pointer (a legacy unmerge): clearing
            # the relation needs no identity, and leaving it would keep the
            # authority's copy merged.
            wire_fields["mergedInto"] = None
    audit = sorted(k for k, c in classes.items() if c == _CLS_AUDIT_ONLY)

    if not wire_fields and created_at is None:
        return {
            "disposition": WIRE_LOCAL_ONLY,
            "reason": "local_audit_only" if audit else "no_server_state",
            "unmapped_fields": [],
            "wire": None,
            "local_audit_fields": audit,
        }
    wire: dict[str, Any] = {
        "queue_id": intent.get("queue_id"),
        "globalId": intent.get("globalId"),
        "op": op,
        "fields": wire_fields,
        "baseUpdatedAt": intent.get("baseUpdatedAt"),
    }
    if created_at is not None:
        wire["createdAt"] = created_at
    return {
        "disposition": WIRE_SUBMIT,
        "reason": "",
        "unmapped_fields": [],
        "wire": wire,
        "local_audit_fields": audit,
...
def drain_queue(project_root: Path, updated_by: str = "") -> dict:
    ...
    if not org_id:
        return out
    out["bound"] = True
    # NO DRAIN LEASE, deliberately (#972, r0b c6d8ba92). A time-bounded lock is
    # not mutual exclusion — a paused holder outlives its lease and a second
    # drainer re-sends — and no local lock can close "the authority committed,
    # our receipt was lost". Both windows close at the AUTHORITY instead:
    # per-intent idempotency keys, with the receipt stored in the same
    # transaction as the write. Duplicate sends are then harmless, not unsafe.
    try:
        from . import backlog_write_queue as _q

        items = _q.candidates(Path(project_root), project_id)
        if not items:
            return out
        # #972 WIRE MAPPING, per intent and IN QUEUE ORDER. Three outcomes:
        #   blocked     not sent; the row moves to `blocked` (safe from the
        #               verdict fold) and every LATER intent for the same item
        #               is held behind it, so writes are never reordered.
        #   local_only  fully known but no server item state (a reason-only
        #               annotation): retired locally, ZERO network calls, the
        #               row and its reason kept as the receipt.
        #   submit      the mapped copy is sent; the queue row keeps the
        #               store's canonical bytes.
        send: list[dict] = []
        blocked: list[dict] = []
        local_only: list[dict] = []
        not_represented: list[dict] = []
        blocked_at: dict[str, int] = {}
        state_failures: list[dict] = []
        sent_gids: set[str] = set()
        for it in items:
            qid, gid = int(it["queue_id"]), str(it["globalId"])
            if gid in sent_gids:
                # #972 ONE INTENT PER ITEM PER CYCLE. The route answers each
                # intent keyed by globalId alone, so two intents for one item in
                # one batch could not be told apart in the verdicts. The later
                # ones stay pending, untouched, and go next cycle — in order.
                continue
            if gid in blocked_at:
                held = f"{BLOCK_HELD_BEHIND}:{blocked_at[gid]}"
                if not _q.set_state(Path(project_root), qid, _q.STATE_BLOCKED, held):
                    state_failures.append({"queue_id": qid, "globalId": gid, "to": _q.STATE_BLOCKED})
                blocked.append(
                    {"queue_id": qid, "globalId": gid, "reason": held, "unmapped_fields": []}
                )
                continue
            if it.get("fields_malformed"):
                # Checked BEFORE mapping: parsed as {}, the mapper would read
                # "no server state" and retire the row for good (r0b 9dbd2629).
                d = {"disposition": WIRE_BLOCKED, "reason": BLOCK_MALFORMED_FIELDS, "unmapped_fields": []}
            else:
                d = to_wire_intent(it)
            if d["disposition"] == WIRE_BLOCKED:
                why = f"{d['reason']}:{','.join(d['unmapped_fields'])}"
                if not _q.set_state(Path(project_root), qid, _q.STATE_BLOCKED, why):
                    state_failures.append({"queue_id": qid, "globalId": gid, "to": _q.STATE_BLOCKED})
                blocked_at[gid] = qid
                blocked.append(
                    {
                        ...
                        "globalId": gid,
                        "reason": d["reason"],
                        "unmapped_fields": list(d["unmapped_fields"]),
                    }
                )
                continue
            if d["disposition"] == WIRE_LOCAL_ONLY:
                if not _q.set_state(Path(project_root), qid, _q.STATE_LOCAL_ONLY, d["reason"]):
                    state_failures.append({"queue_id": qid, "globalId": gid, "to": _q.STATE_LOCAL_ONLY})
                local_only.append(
                    {"queue_id": qid, "globalId": gid, "fields": list(d["local_audit_fields"])}
                )
                continue
            if it.get("state") == _q.STATE_BLOCKED and not _q.set_state(
                Path(project_root), qid, "pending", ""
            ):
                # The release must land BEFORE the row is eligible to send: a
                # row the server applied while it still reads `blocked` is never
                # settled by the (pending-only) verdict fold, and goes again.
                state_failures.append({"queue_id": qid, "globalId": gid, "to": "pending"})
                continue
            send.append(d["wire"])
            sent_gids.add(gid)
            if d["local_audit_fields"]:
                not_represented.append(
                    {"queue_id": qid, "globalId": gid, "fields": list(d["local_audit_fields"])}
                )
        out["blocked"] = blocked
        out["local_audit_only"] = local_only
        out["not_represented"] = not_represented
        if state_failures:
            # FAIL CLOSED, WHOLE CYCLE. A block that did not land leaves its row
            # `pending`; if a sibling write for the same item were sent and
            # applied, the verdict fold would consume that row with it. Nothing
            # goes out until every state this cycle computed is durable.
            out["state_failures"] = state_failures
            out["error"] = REASON_QUEUE_STATE
            out["remedy"] = REASON_REMEDY.get(REASON_QUEUE_STATE, "")
            _record_drain_outcome(Path(project_root), project_id, out)
            return out
        if not send:
            _record_drain_outcome(Path(project_root), project_id, out)
            return out
        out["submitted"] = len(send)
        uid = updated_by or _resolve_uid(Path(project_root))
        outcome: dict[str, Any] = {}
        verdicts = submit_intents(
            org_id, project_id, uid, send, outcome, project_root=Path(project_root)
        )
        ...
        if verdicts is None:
            ...
            _record_drain_outcome(Path(project_root), project_id, out)
            return out
        # The fold settles EXACTLY the rows this batch carried (#972): it is told
        # what was sent, and touches nothing else.
        out.update(
            _q.apply_verdicts(
                Path(project_root),
                verdicts,
                submitted=[{"queue_id": w["queue_id"], "globalId": w["globalId"]} for w in send],
            )
        )
        _record_drain_outcome(Path(project_root), project_id, out)
...
def _ensure_drain_meta_table(conn: sqlite3.Connection) -> None:
    ...
    # #1063: WHICH credential a refusal was about (non-reversible fingerprint),
    # so a later reader can tell a refusal of THIS credential from a stale one
    # about a credential the shared door has since replaced. Additive column.
    try:
        conn.execute("ALTER TABLE backlog_drain_meta ADD COLUMN last_credential_fp TEXT")
    except sqlite3.OperationalError:
        pass


def credential_identity() -> dict:
    """The shared gate credential's CURRENT identity, read-only (#1063).

    reason       cached_gate_credential's verdict
    fingerprint  sha256[:16] of the bearer the door would present, "" if none
    verified_at  when ANY authenticated gate call (XAACP included) last got a
                 200 for it -- the live proof `record_gate_answer` stamps

    No network, no renewal, never raises, never returns the token.
    """
    try:
        from . import operator_token_resolution as _otr

        cred = _otr.cached_gate_credential()
        return {
            "reason": str(cred.reason or ""),
            "fingerprint": _otr._token_fingerprint(cred.token) if cred.token else "",
            "verified_at": str(cred.verified_at or ""),
        }
    except Exception:  # noqa: BLE001
        return {"reason": "", "fingerprint": "", "verified_at": ""}


def refusal_superseded(last: dict) -> bool:
    """Is a recorded CREDENTIAL refusal stale, by live proof? (#1063, r0a rule)

    True only when the refusal was credential-class AND the shared door now
    holds a usable credential AND either
      * its fingerprint differs from the refused one (a newer generation:
        renewal or sign-in), or
      * an authenticated gate call over it succeeded AFTER the refusal
        (verified_at > last_attempt_at) -- XAACP's 200s count.
    Never true by merely re-presenting the refused token: nothing here sends.
    """
    reason = str((last or {}).get("reason") or "")
    if reason not in (REASON_GATE_CREDENTIAL, REASON_AUTH_REFUSED, REASON_UNCONFIGURED):
        return False
    from .operator_token_resolution import GATE_CRED_OK

    now = credential_identity()
    if now["reason"] != GATE_CRED_OK or not now["fingerprint"]:
        return False
    # An EMPTY refused fingerprint means no usable bearer existed at refusal
    # time (latched / expired / absent -- the latch is fingerprint-scoped, so a
    # latched token can never read back OK). A usable one now IS a newer
    # generation. A matching fingerprint needs the live-success proof below.
    refused_fp = str((last or {}).get("credential_fingerprint") or "")
    if refused_fp != now["fingerprint"]:
        return True
    attempted = str((last or {}).get("last_attempt_at") or "")
    return bool(now["verified_at"] and attempted and now["verified_at"] > attempted)
        ...
        with _canonical_connect(str(db), row_factory=False) as conn:
            ...
            if reason:
                conn.execute(
                    ...
                    " last_credential, last_submitted, last_attempt_at, last_credential_fp)"
                    " VALUES (?,?,?,?,?,?,?) ON CONFLICT(project_id) DO UPDATE SET"
                    " last_reason = excluded.last_reason,"
                    ...
                    " last_attempt_at = excluded.last_attempt_at,"
                    " last_credential_fp = excluded.last_credential_fp",
                    (
                        str(project_id),
                        reason,
                        ...
                        stamp,
                        str(out.get("credential_fingerprint") or credential_identity()["fingerprint"]),
                    ),
                )
            else:
                conn.execute(
...
def last_drain_outcome(project_root: Path, project_id: str) -> dict:
    ...
    try:
        ...
        with _canonical_connect(str(db), row_factory=False) as conn:
            ...
            row = conn.execute(
                ...
                " last_attempt_at, last_success_at, last_credential_fp FROM backlog_drain_meta"
                " WHERE project_id = ?",
                (str(project_id),),
            ).fetchone()
    ...
    out.update(
        ...
        credential_fingerprint=str(row[6] or ""),
    )
