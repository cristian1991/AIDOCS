...
class CodeIndexInferenceService:
    ...
    #: Trace provenance, strongest first — see code_index_store._TRACE_SOURCE_RANK.
    _EVIDENCE_KINDS = ("schema", "symbol", "text")

    def _trace_confidence(self, matches: list[dict[str, object]]) -> str:
        """Grade the KIND of evidence found, not how much of it there is.

        THE BUG THIS REPLACES (measured 2026-08-24, #683): confidence was
        `len(matches)` with three labels, so four unrelated word-matches graded
        "high". Tracing the real column `chain_seq` returned filesystem-ancestor
        walkers and shell-command-chain evaluators and called that high
        confidence — the tool advertised certainty in proportion to its own
        fuzziness, and `why` was a histogram of the same count, so nothing in
        the response could contradict it.

        Now: an EXACT hit (the field resolved in the schema index) earns "high";
        a symbol NAME match earns "medium"; text-only earns "low". The grade can
        therefore be wrong, which is the point — a claim that cannot fail is not
        a claim.

        LEGACY FALLBACK, deliberately. Trace modes that do not yet stamp
        `source` keep the old count behaviour: this change fixes field_flow and
        must not silently regrade the modes it has not migrated.
        """
        found = {
            str(match.get("source") or "")
            for match in matches
            if isinstance(match, dict)
        } & set(self._EVIDENCE_KINDS)
        if found:
            for kind, grade in zip(self._EVIDENCE_KINDS, ("high", "medium", "low")):
                if kind in found:
                    return grade
        if len(matches) >= 4:
            return "high"
