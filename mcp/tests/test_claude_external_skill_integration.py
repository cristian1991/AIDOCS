...
def _make_runtime_with_missing_provider(tmp_path: Path) -> tuple[RuntimeService, Path, str]:
...
def _make_runtime_with_selected_bundled_skill(tmp_path: Path, name: str, skill_id: str) -> tuple[RuntimeService, Path, str]:
    ...
    runtime.project_init(project_root, init_git=False, create_remote=False)
    session = runtime.hub.sessions.create_session(project_root, f"session-{name}", "A", "Agent", "Goal A")
    runtime.hub.managed_mode.set_mode(project_root, session.session_id)
...
def test_claude_uses_bundled_skills_without_external_plugin(tmp_path: Path) -> None:
    ...
    assert host_state["skill_state"]["session_snapshot"]["selected_skills"] == ["deep-retrieval"]
    assert host_state["skill_state"]["session_snapshot"]["active_skills"] == ["deep-retrieval"]
...
def test_host_views_show_canonical_bundled_skill_names(tmp_path: Path) -> None:
    async def run() -> tuple[dict[str, object], dict[str, object]]:
        ...
        runtime.project_init(project_root, init_git=False, create_remote=False)
        session = hub.sessions.create_session(project_root, "session-host-view-bundled", "A", "Agent", "Goal A")
        ...
        payload = tool_result[0].text if isinstance(tool_result, list) else tool_result.content[0].text
        return host_state, json.loads(payload)

    host_state, session_start = asyncio.run(run())

    assert session_start["active_skills"] == ["deep-retrieval", "test-driven-validation"]
    assert session_start["imported_skill_state"]["selected_skills"] == ["deep-retrieval", "test-driven-validation"]
    assert host_state["skill_state"]["session_snapshot"]["selected_skills"] == ["deep-retrieval", "test-driven-validation"]
    assert host_state["skill_state"]["session_snapshot"]["active_skills"] == ["deep-retrieval", "test-driven-validation"]
    assert "aidocs_bundled_superpowers/deep-retrieval" not in json.dumps(host_state)
    assert "aidocs_bundled_superpowers/test-driven-validation" not in json.dumps(host_state)
    assert "aidocs_bundled_superpowers/deep-retrieval" not in json.dumps(session_start)
    assert "aidocs_bundled_superpowers/test-driven-validation" not in json.dumps(session_start)
...
def test_claude_startup_context_uses_override_resolved_active_skills(tmp_path: Path) -> None:
    templates = tmp_path / "templates-override"
    _write_templates(templates)
    runtime = RuntimeService(AidocsServiceHub(templates_root=templates))
    project_root = tmp_path / "project-override"
    runtime.project_init(project_root, init_git=False, create_remote=False)
    session = runtime.hub.sessions.create_session(project_root, "session-override", "A", "Agent", "Goal A")
    runtime.hub.managed_mode.set_mode(project_root, session.session_id)
    _register_superpowers_provider(runtime, project_root, tmp_path / "superpowers-external-override")
    ...
    assert result["active_skills"] == ["writing-plans"]
    assert result["imported_skill_state"]["active_skills"] == ["writing-plans"]
...
def test_claude_runtime_state_can_include_override_mode_metadata(tmp_path: Path) -> None:
    async def run() -> tuple[list[str], dict[str, object]]:
        ...
        runtime.project_init(project_root, init_git=False, create_remote=False)
        session = hub.sessions.create_session(project_root, "session-override-metadata", "A", "Agent", "Goal A")
        hub.managed_mode.set_mode(project_root, session.session_id)
        _register_superpowers_provider(runtime, project_root, tmp_path / "superpowers-external-override-metadata")
        hub.skills.set_selected_skills(project_root, session.session_id, ["superpowers_external/writing-plans"])
        runtime.project_bootstrap_or_resume(project_root, session_id=session.session_id, include_code_bundle=False, include_tests=False)
...
def test_claude_hook_and_opencode_share_same_prompt_level_skill_decision_source(tmp_path: Path) -> None:
    ...
    assert "Action: `edit`." in context or "Action: `understand`." in context or "Action: `task_begin`." in context or "Action: `planning`." in context
    assert "superpowers_external/brainstorming" in context
...
def test_mcp_session_start_state_tool_can_include_imported_skill_state(tmp_path: Path) -> None:
    async def run() -> tuple[list[str], dict[str, object]]:
        server = create_server()
        tool_names = [tool.name for tool in await server.list_tools()]
        hub = server._aidocs_test_hub
        ...
        runtime.project_init(project_root, init_git=False, create_remote=False)
        session = hub.sessions.create_session(project_root, "session-tool", "A", "Agent", "Goal A")
        hub.managed_mode.set_mode(project_root, session.session_id)
        _register_superpowers_provider(runtime, project_root, tmp_path / "superpowers-external-tool")
