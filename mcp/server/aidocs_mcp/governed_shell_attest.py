...
Empire re-seal 2026-05-30: replace the multi-toggle / manual-allowlist
...
# Content-hash-keyed cache for the Authenticode probe (perf 2026-07-16).
# _publisher_ok spawns PowerShell (~0.8s) to read a file's signature. The
# Authenticode Status + signer subject are a DETERMINISTIC function of the
# file's bytes (the signature covers the file's hash), so the verdict for a
# given (resolved_path, sha256) never changes. Keying on the CURRENT sha256
# makes this provably safe: a swapped binary has different bytes → a different
# key → a cache MISS → a fresh probe. The cache can never serve a stale verdict
# for changed content. A short TTL additionally bounds signature
# revocation/expiry staleness for the long-lived daemon. Only DEFINITIVE
# verdicts (True/False) are cached — a transient probe error (None,
# 'unverifiable') is never cached so retries still work.
_PUBLISHER_CACHE: dict[tuple[str, str], tuple[float, tuple[bool | None, str]]] = {}
_PUBLISHER_CACHE_LOCK = threading.Lock()
_PUBLISHER_CACHE_TTL_S = 300.0


def _on_windows() -> bool:
    """Windows-host predicate — a testable seam for the Authenticode paths.

    Tests must NOT monkeypatch ``os.name`` to force the nt branch: in
    Python 3.13 ``Path(...)`` picks WindowsPath vs PosixPath from ``os.name``
    AT CONSTRUCTION, so patching it globally makes ``Path(p).resolve()`` build
    a WindowsPath on a Linux CI host — which raises, silently NULLing the
    publisher-cache key (probe re-runs every call). Patch THIS instead so the
    branch flips while pathlib stays honest to the real platform.
    """
    return os.name == "nt"


def _reset_publisher_cache() -> None:
    """Test/admin helper — drop the Authenticode probe cache."""
    with _PUBLISHER_CACHE_LOCK:
        _PUBLISHER_CACHE.clear()


def _publisher_ok(path: Path) -> tuple[bool | None, str]:
    """Windows Authenticode validity + expected-publisher check (cached).

    Content-hash-keyed cache (see the note above ``_PUBLISHER_CACHE``): the
    expensive PowerShell probe runs once per (resolved_path, sha256);
    identical content within the TTL is served from cache, changed bytes
    re-probe. Delegates to ``_publisher_ok_uncached`` on a miss.
    """
    if not _on_windows():
        return (None, "non-windows: no Authenticode publisher to check")
    # Key on the CURRENT content hash so changed bytes can never hit a stale
    # verdict. If hashing fails, skip the cache entirely (probe every time).
    cache_key: tuple[str, str] | None = None
    try:
        cache_key = (str(Path(path).resolve()), _gbs._sha256(Path(path)))
    except Exception:
        cache_key = None
    if cache_key is not None:
        now = time.monotonic()
        with _PUBLISHER_CACHE_LOCK:
            hit = _PUBLISHER_CACHE.get(cache_key)
        if hit is not None and (now - hit[0]) < _PUBLISHER_CACHE_TTL_S:
            return hit[1]

    result = _publisher_ok_uncached(path)

    # Cache ONLY definitive, content-derived verdicts. A None result is a
    # transient 'unverifiable' (probe unavailable) — caching it would wrongly
    # suppress the retry that might later succeed.
    if cache_key is not None and result[0] is not None:
        with _PUBLISHER_CACHE_LOCK:
            _PUBLISHER_CACHE[cache_key] = (time.monotonic(), result)
    return result


def _publisher_ok_uncached(path: Path) -> tuple[bool | None, str]:
    ...
    try:
        ...
        cp = audited_run(
            ...
            fingerprint=("governed_shell_attest.py", "_publisher_ok_uncached", "subprocess.run"),
            reason="authenticode-publisher-probe",
            ...
            check=False,
            env=env,
            # #333 Phase 2: daemon runs console-less (pythonw); without this
            # flag every spawn allocates a NEW visible console window.
...
def identity_binding_ok(
    ...
    *,
    ...
    """THE host-native ALLOW identity binding (Empire re-seal 2026-05-30).
...
def machine_presence_native_ok(
    host: str,
    provider: str,
    project_root: Path | None,
) -> tuple[bool, str]:
    """LOCAL-SOLO machine-presence native authority (#319, operator decision
    2026-07-12 after the wiring-map artifact f7ed1534).

    The DECOUPLED execution floor: when Governed Bash provider attestation is
    NOT enrolled (``tools.native_shell_provider_enabled`` off), a raw host-
    native command that the LAW already ALLOWED (full ShellPolicy + bash_policy
    + judge + anti-coup + lifecycle ran) and that is capability-safe (native-
    safe host + host-replaceable receipt + safe command shape) may STILL run
    native — but ONLY when ALL hold:

      * a bound project execution context exists (project_root), AND
      * host == claude_code (the one PROVEN local native-shell adapter), AND
      * provider == bash, AND
      * operator_auth_service._auth_status_flavor() in {dev, solo} — a LOCAL
        single-tenant install where machine presence IS the authority (the SAME
        predicate that exempts local solo/dev from corpo live-auth in
        agent_orchestrator._bash_unauthenticated_refusal).

    Corpo / remote / unknown / opencode NEVER qualify provider-less — they
    route to ai_run, where AIDOCS owns the spawn. This is NOT bare-config
    fabrication of a provider identity: it is the operator-blessed local-solo
    trust boundary (machine presence), scoped to the proven adapter, standing
    in for the corpo/remote SHA-pinned-provider hardening. Fails CLOSED (treats
    any read error as corpo) so an indeterminate state routes to ai_run.

    Returns (ok, reason). This is the SINGLE definition consumed by all three
    native gates (ShellPolicy enable-gate, native_route, the shell_adapter
    final gate) so there is one law, one home (empire-doctrine Article XXII).
    """
    if project_root is None:
        return (False, "no bound project context for machine-presence authority")
    try:
        from .shell_capability_matrix import HOST_CLAUDE_CODE, PROVIDER_BASH
    except Exception:
        return (False, "capability-matrix constants unavailable — fail closed")
    if (host or "") != HOST_CLAUDE_CODE or (provider or "") != PROVIDER_BASH:
        return (
            False,
            "machine-presence native is limited to the proven claude_code+bash "
            "adapter; other hosts/providers route to ai_run",
        )
    try:
        from .operator_auth_service import _auth_status_flavor

        flavor = str(_auth_status_flavor() or "").strip().lower()
    except Exception:
        flavor = "corpo"  # fail closed to the auth-required direction
    if flavor not in ("dev", "solo"):
        return (
            False,
            "machine-presence native requires a local single-tenant install "
            f"(auth flavor={flavor!r}); corpo/remote route to ai_run",
        )
    return (
        True,
        "local single-tenant: machine presence is the authority "
        "(no provider enrolled; proven claude_code+bash adapter)",
    )
