...
def _ups_login_block(project_root, payload):
    ...
    AIDOCS. An unauthenticated prompt is blocked HERE — before any
    UPS mutation (audit record, grants, SEC-001/002 staging) can run
    on behalf of an anonymous caller — mirroring the pre-flight block
    shape. login_required_block is fail-open internally, and an
    authenticated caller (valid token / approved host binding) passes.

    Extraction pin (#413 T2): verbatim from _run_user_prompt_core; runs
    FIRST, before the UPS audit record."""
    try:
...
def _ups_advise_rails(
    ...
    notice, (3) #448 Consumer B intent code-target enrichment
    ...
    try:
        _ui_session = ""
        try:
            _mm = runtime.hub.managed_mode.get_mode(project_root)
            if _mm.get("active"):
                _ui_session = str(_mm.get("session_id") or "").strip()
        except Exception:
            _ui_session = ""
        if _ui_session:
    ...
    try:
        from .semantic_enrichment import prompt_code_mention_block

        _sem_block = prompt_code_mention_block(
            project_root, _operator_text or prompt
        )
        if _sem_block:
            additional_context = (
                (additional_context or "")
...
def _run_user_prompt_core(
    ...
    *,
    ...
    additional_context = _ups_auto_task_leg(
        runtime,
        project_root,
        session_id,
        prompt,
...
def _post_update_intent_satisfier(runtime, project_root, payload, tool_name_raw):
    ...
    try:
        _mm_post = runtime.hub.managed_mode.get_mode(project_root)
        if _mm_post.get("active"):
            _ui_session_post = str(_mm_post.get("session_id") or "").strip()
            if _ui_session_post:
...
def on_session_start(runtime, project_root, payload=None, *, host_kind="claude_code"):
    """SessionStart core (S6 rip, #251) — verbatim relocation."""
