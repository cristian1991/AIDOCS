...
def run_owner_mismatch(
    ...
    *,
    ...
    same_session = bool(expected_session) and expected_session == (session_id or "").strip()
    if expected_session and not same_session:
        return "cross_session_run_id"
    if same_session:
        # Clause 2 (operator ruling): ownership is (actor OR same-session).
        # A confirmed same-session match overrides a differing actor id.
        return ""
...
def spawn_detached(
    ...
    *,
    ...
    try:
        ...
        host_sid = _ccsid()
        _early_session_id = _mm_attr.resolve_managed_session(
            _mm_attr.ManagedModeService(),
            project_root,
...
def _spawn_and_track(
    *,
    ...
    try:
        ...
        # #757: a DETACHED run is exactly the shape that produced the orphans —
        # it outlives the caller by design, so it must be born with a lifetime
        # or nothing will ever own it. BOUNDED by the run's own timeout,
        # clamped to the 30-minute ceiling; a timeout of 0/None means the
        # caller asked for "no deadline", which the ceiling now supplies rather
        # than granting immortality. Kills the whole TREE (job object), which
        # is the part that killing the parent never did.
        from .process_lifetime import Lifetime as _Lifetime
        ...
        proc = audited_popen(
            ...
            lifetime=_Lifetime.bounded_for(
                float(governed_timeout or 0) or None, "detached-run"
            ),
    ...
    try:
        ...
        spawn_session_id = _mm_attr.resolve_managed_session(
            _mm_attr.ManagedModeService(),
            project_root,
            host_session_id=host_sid,
        )
...
def _monitor_run(live: _LiveRun, log_file) -> None:
    ...
    try:
        ...
        # #757: A RUN IS NOT FINISHED WHILE ITS DESCENDANTS ARE STILL RUNNING.
        # This `finally` fires when the PARENT exits, and used to record the run
        # as finished right there -- which is exactly how six pytest workers ran
        # for 90 MINUTES under a run the ledger already called done. The workers
        # are children of the runner, not of us, so the parent's exit says
        # nothing about them.
        #
        # Drain first, then close the grant. The brief wait is for the ordinary
        # case (children exiting microseconds after their parent); anything
        # still alive after it is an orphan by definition -- the thing that
        # owned it is gone -- so closing the job collects the tree via
        # KILL_ON_JOB_CLOSE. Best-effort throughout: a monitor that raises here
        # would leave the run with no terminal record at all.
        try:
            from . import process_lifetime as _pl

            if getattr(live.proc, "_aidocs_job", None) is not None:
                _pl.await_tree_exit(live.proc, timeout=5.0)
                _pl.release(live.proc)
        except Exception:  # noqa: BLE001 -- never break the monitor
            pass
