...
class AIDOCSRunHooks:
    ...
    def _resolve_templates_root(self) -> Path | None:
        """Find the AIDOCS templates directory.

        CONVERGED 2026-07-30 (#628): the fourth rival — it probed
        ``$AIDOCS_PATH`` and ``parents[3|2]`` for the ``.MEMORY/.aidocs``
        BUNDLE dir and handed that to AidocsServiceHub as if it were the
        templates dir (one level too high: no ``context.md`` lives there),
        falling back to the package dir itself. Now delegates to the
        canonical resolver, which honors the same ``$AIDOCS_PATH`` lever and
        returns a dir that provably holds ``context.md``, else ``None``.
        """
        from .host_services.path_resolver_service import find_templates_root

        return find_templates_root(project_root=self.project_root)
    ...
    async def on_tool_start(self, context: Any, agent: Any, tool: Any) -> None:
        ...
        # Host presence (r0b 3e084f64). openai_agents is NOT an attested native
        # lifecycle host: on_tool_end carries neither a call id nor the tool
        # input, so a release cannot be matched to its registration. Presence is
        # still recorded, and ai_git switch REFUSES while this host is attached.
        # FAIL CLOSED, never best-effort: the presence row is the ONLY evidence
        # that an unattested host has a native tool running. If it cannot be
        # written, the tool must not run — a lost row would let a later switch
        # proceed over a live native mutation.
        from . import hook_pipeline as _hp

        _why = _hp.native_tool_pre(
            self.project_root,
        ...
        if _why:
            raise RuntimeError(f"AIDOCS blocked tool '{tool_name}': {_why}")
