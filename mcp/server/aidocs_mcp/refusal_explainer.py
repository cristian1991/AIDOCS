...
def _ladder_match(blocked_by: str, matched_rule: str, risk_class: str, intent: bool) -> str:
    ...
    if bb in vc._SECURITY_BLOCK_BLOCKED_BY:
        return "blocked_by in _SECURITY_BLOCK_BLOCKED_BY (security block; intent cannot lift it)"
    if intent:
...
def explain_refusal(
    *,
    ...
    elif resolved == vc.CLASS_SECURITY_BLOCK:
        out["latches_next_calls"] = False
        out["writes"] = "nothing: no freeze row, no escalation request, no strike"
        out["summary"] = (
            "SECURITY BLOCK — this invocation is refused; not confirmable, not "
            "liftable by intent or approval; no strike, nothing latched"
        )
...
def _judge_branch(rule_ids: list[str], *, intent: bool, surface: str) -> dict[str, Any]:
    ...
    if d == jt.DECISION_BLOCK_FREEZE_NO_CONFIRM:
        ...
        branch["consequence"] = explain_refusal(blocked_by=bb, needs_confirmation=False)
        return branch
    if d == jt.DECISION_BLOCK_SECURITY_FAILURE:
        bb = "judge_security_inspection_failure"
        branch["blocked_by"] = bb
        branch["gate_tier"] = (
            "hard refusal of this invocation: a security inspection did not "
            "complete; NO confirm path (intent ignored), NO strike, NO freeze"
        )
        branch["consequence"] = explain_refusal(blocked_by=bb, needs_confirmation=False)
        return branch
    if d == jt.DECISION_ASK_CONFIRM:
        ...
        return branch
    # Unrecognised decision: the gate fails closed, so the explainer must not
    # describe it as confirmable.
    bb = "judge_taxonomy_unknown_decision"
    branch["blocked_by"] = bb
    branch["gate_tier"] = (
        "hard refusal: unrecognised taxonomy decision fails closed; "
        "NO confirm path, NO strike, NO freeze"
    )
    branch["consequence"] = explain_refusal(blocked_by=bb, needs_confirmation=False)
    return branch
...
def refusal_matrix(project_root: Path | None = None) -> dict[str, Any]:
    ...
    class_cost = {
        ...
        jt.CLASS_SECURITY_INSPECTION_FAILURE: (
            "hard refusal of the invocation (judge_security_inspection_failure); "
            "operator intent ignored, no confirm path, no strike, no freeze"
        ),
    }
