...
# #755/#756: the ONE canonical connect. Both sites here were
...
# single pragma (foreign_keys OFF, no busy_timeout, synchronous=FULL).
# DURABILITY: RUNTIME. pos/centroid_blob are a DERIVED enrichment of rows
# the store already holds, and this module is an idempotent re-runnable
# script: a lost UPDATE is simply re-enriched on the next run. The first
# site is a pure SELECT, so it says read_only=True.
