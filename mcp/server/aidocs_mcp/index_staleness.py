...
def memory_staleness_keys(project_root: Path) -> dict[str, Any]:
    ...
    try:
        conn = _canonical_connect(db, read_only=True, row_factory=False)
        try:
            row = conn.execute(
...
def axes_for(tool_name: str, tool_args: Any = None) -> frozenset[str] | None:
    """Resolve the index axes for a tool call.

    Most tools map statically via INDEX_BACKED_TOOLS. ai_memory is mode-
    selective (fix/768 consolidated memory_capture/read/search/promote into it):
    only mode='search' is memory-index-backed — capture/read/promote are not —
    so it maps to the memory_search axes rather than a static table entry (adding
    ai_memory to INDEX_BACKED_TOOLS unconditionally would mislabel the write/read
    modes, same reason ai_slop is kept out and stamps itself)."""
    axes = INDEX_BACKED_TOOLS.get(tool_name)
    if axes:
        return axes
    if tool_name == "ai_memory" and isinstance(tool_args, dict):
        if str(tool_args.get("mode", "")).strip().lower() == "search":
            return INDEX_BACKED_TOOLS.get("memory_search")
    return None


def stamp_tool_result(result: Any, tool_name: str, project_root: Path,
                      tool_args: Any = None) -> Any:
    ...
    try:
        axes = axes_for(tool_name, tool_args)
        if not axes:
            return result
