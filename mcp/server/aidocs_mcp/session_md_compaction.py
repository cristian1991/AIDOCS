...
# DURABILITY: AUDIT, deliberately -- i.e. the FULL this file already had.
# The stamp row is not runtime state, it is the dated record that a
# DESTRUCTIVE rewrite of the operator's SESSION.md files happened, which
# is this module's own stated reason for writing it ("a dated, auditable
# act rather than an ambient behavior change"). A row missing after a
# crash would itself be the finding -- the exact test Durability.AUDIT
# exists for. It costs nothing: this is a DELIBERATE-RUN conductor sweep,
# three commits per run, never on the hook path.
