...
def cutover_readiness(project_root: Path) -> dict:
    ...
    try:
        ...
        # #1063: the SAME receipt every ai_backlog add/update/get carries.
        out["sync_health"] = sync_receipt(project_root)
        ...
        # STRICT readers at the cutover boundary (#972, r0b 9dbd2629). The
        # best-effort `pending()` answered ANY read failure with [] — read here
        # as depth 0, a green over a queue nobody counted. The strict reader
        # raises, so the failure lands in the `pending is None` branch below.
        try:
            pending = len(_q.pending_strict(Path(project_root), project_id) or [])
        except Exception:  # noqa: BLE001
            pending = None
        out["pending"] = pending
        ...
        # Same law, independently: this query selects columns the others do
        # not, so it can fail while they answer. Unknown is None, never 0.
        try:
            unclassified = list(
                _q.unclassified_conflicts_strict(Path(project_root), project_id) or []
            )
            out["unclassified_conflicts"] = len(unclassified)
        except Exception:  # noqa: BLE001
            unclassified = []
            out["unclassified_conflicts"] = None
        ...
        # #972 BLOCKED WRITES. The wire mapper holds an intent off the network
        # when the authority cannot represent it yet (an unmapped field, a
        # create the route does not support, a merge with no captured
        # identity). Such a row is deliberately NOT `pending` — so without this
        # count a stuck write would vanish from the depth above and the flip
        # would read an empty outbox over writes the authority never saw.
        try:
            held = list(_q.blocked(Path(project_root), project_id) or [])
        except Exception:  # noqa: BLE001
            held = None
        out["blocked"] = None if held is None else len(held)
        out["blocked_writes"] = held or []
        if held is None:
            blockers.append(
                "the blocked-write count could not be determined — a flip must "
                "not proceed over writes nobody could count"
            )
        elif held:
            reasons = sorted({str(r.get("reason") or "") for r in held})
            blockers.append(
                f"{len(held)} write(s) are blocked from the wire and never reached "
                f"the authority ({'; '.join(reasons)}) — see blocked_writes; they "
                "release on their own once the mapper or route can carry them"
            )
        if out.get("unclassified_conflicts") is None:
            blockers.append(
                "the unclassified-conflict count could not be determined — "
                "'unknown' is not 'none', and a flip must not proceed over "
                "conflicts nobody could read"
            )
        if unclassified:
            blockers.append(
...
#: A drain reason that means THE CREDENTIAL (or its absence) stopped the queue,
#: as opposed to a network blip. Loud, never merely stored (#1063 ruling 5).
_CREDENTIAL_REASONS = frozenset(
    {
        "gate_credential_unusable",
        "auth_refused",
        "insufficient_scope",
        "tenant_mismatch",
        "hub_unconfigured",
...
#: How old the last successful drain may be, with writes pending, before the
#: sync is called STALE. Config: observability.backlog_sync_stale_seconds.
DEFAULT_STALE_SECONDS = 900


def _stale_seconds(project_root: Path) -> int:
    try:
        from .config import get_setting

        return int(
            get_setting(
                "observability.backlog_sync_stale_seconds",
...
def _age_seconds(stamp: str) -> float | None:
    ...
    try:
        then = datetime.strptime(str(stamp), "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=UTC)
    except (TypeError, ValueError):
        return None
    return (datetime.now(UTC) - then).total_seconds()


def sync_receipt(project_root: Path, *, wrote: bool = False) -> dict:
    """The sync facts EVERY ai_backlog receipt carries (#1063 ruling 4).

      outbox               "local" or "gate" -- WHICH queue this is; the gate
                           has its own and the two were being confused
      pending              queued writes not yet accepted by the server
                           (None = unbound, or could not be counted)
      sitter_state         "running" | "stopped" in THIS process
      sitter_mode          "full" | "drain_only" | ""
      last_success_at      last drain the server answered, "" = never
      last_attempt_at      last drain attempt, "" = never
      last_attempt_reason  why the last attempt failed, "" = it did not
      credential           the credential state named by that attempt
      consecutive_failures / backoff_seconds   the sitter's retry state
      write_status         (write receipts only) "queued_offline" -- landed
                           locally, NO server verdict yet, so NOT globally
                           durable -- or "local_only" for an unbound project

    Never raises; a receipt must not fail the write it describes.
    """
    out: dict[str, Any] = {
        "outbox": "local",
        "pending": None,
        "sitter_state": "stopped",
        "sitter_mode": "",
        "last_success_at": "",
        "last_attempt_at": "",
        "last_attempt_reason": "",
    ...
    try:
        ...
        try:
            if _hub._on_gate_surface():
                out["outbox"] = "gate"
        except Exception:  # noqa: BLE001
            pass
        st = _sitter.backlog_sync_status(Path(project_root))
        if isinstance(st, dict):
            out["sitter_state"] = "running" if st.get("running") else "stopped"
            out["sitter_mode"] = str(st.get("mode") or "")
            out["consecutive_failures"] = int(st.get("consecutive_failures") or 0)
            out["backoff_seconds"] = st.get("backoff_seconds")
        _org, pid = _hub.binding(Path(project_root))
        bound = bool(pid)
        if pid:
            try:
                out["pending"] = len(_q.pending(Path(project_root), pid) or [])
            except Exception:  # noqa: BLE001 -- unknown stays None, never 0
                out["pending"] = None
            last = _hub.last_drain_outcome(Path(project_root), pid)
            out["last_success_at"] = str(last.get("last_success_at") or "")
            out["last_attempt_at"] = str(last.get("last_attempt_at") or "")
            out["last_attempt_reason"] = str(last.get("reason") or "")
            out["credential"] = str(last.get("credential") or "")
            # r0a: a refusal recorded against a credential the shared door has
            # since replaced -- or that has since been ACCEPTED by the gate
            # (XAACP's 200s stamp it) -- is stale, and must not read as live.
            out["refusal_superseded"] = bool(_hub.refusal_superseded(last))
        if isinstance(st, dict) and st.get("held_reason"):
            # A gate drainer HOLDING rows it cannot attribute says so by name.
            out["last_attempt_reason"] = str(st.get("held_reason"))
            out["held"] = st.get("held_count")
            if out["pending"] is None and isinstance(st.get("held_count"), int):
                out["pending"] = st.get("held_count")
    except Exception as exc:  # noqa: BLE001
        out["error"] = type(exc).__name__
    if wrote:
        out["write_status"] = "queued_offline" if bound else "local_only"
    return out


def sync_alarm(project_root: Path) -> tuple[str, str] | None:
    """(dedup_key, text) when backlog sync needs a human, else None (#1063 ruling 5).

    Fires only with writes PENDING, and then when ANY of: no sitter in this
    process, the last attempt was refused for a credential reason, or the last
    success is older than the stale bound (never succeeded counts as stale once
    an attempt exists). The key names the conditions and reason, so the rail
    announces once per TRANSITION and again when the cause changes.
    """
    r = sync_receipt(project_root)
    pending = r.get("pending")
    if not isinstance(pending, int) or pending <= 0:
        return None
    causes: list[str] = []
    if r.get("sitter_state") != "running":
        causes.append("sitter_stopped")
    reason = str(r.get("last_attempt_reason") or "")
    if reason in _CREDENTIAL_REASONS and not r.get("refusal_superseded"):
        causes.append("credential_refused")
    if reason == "held_unattributed_gate_writes":
        causes.append("held")
    if r.get("last_attempt_at") or r.get("last_success_at"):
        age = _age_seconds(str(r.get("last_success_at") or ""))
        if age is None or age > _stale_seconds(Path(project_root)):
            causes.append("stale")
    if not causes:
        return None
    key = f"backlog_sync_alarm:{r.get('outbox')}:{'+'.join(causes)}:{reason}"
    parts = [
        (
            f"⛔ BACKLOG SYNC ALARM ({r.get('outbox')} outbox): {pending} backlog write(s) "
            "have NOT reached the server backlog authority."
        ),
        f"sitter={r.get('sitter_state')}",
        f"last_success_at={r.get('last_success_at') or 'never'}",
    ]
    if reason:
        parts.append(f"last_attempt_reason={reason}" + (f" [{r.get('credential')}]" if r.get("credential") else ""))
    text = " · ".join(parts)
    if "credential_refused" in causes:
        try:
            from . import backlog_hub_client as _hub
            ...
            remedy = ""
        text += f". ACT: {remedy or 'sign in to CODENEXUS again from the Dashboard'}."
    if "sitter_stopped" in causes:
        text += " No sitter is draining in this process; ai_session(mode='connect') ensures one."
    text += (
        " The writes stay queued in order and are never dropped; "
        "ai_backlog(mode='cutover_status') shows the detail."
    )
    return key, text
