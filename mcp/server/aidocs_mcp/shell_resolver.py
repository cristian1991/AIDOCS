...
class ResolvedShell:
    ...
    dialect
        The GRAMMAR the resolved binary parses in — one of
        ``shell_provider_dialect.KNOWN_DIALECTS`` (#561 phase 2). Naming
        the executable was never enough on its own: phase 1's lesson is
        that a resolution nobody can read back as "and therefore the law
        must reason in THIS grammar" is how a bash-grammar law came to
        vet strings another shell would run. Always ``"unknown"`` unless
        the verdict is ``"usable"`` — a refusal must not carry the tag
        that would let a caller proceed.
    """
    ...
    dialect: str = _dialect.DIALECT_UNKNOWN
...
def _build_audit_payload(
    *,
    ...
    command / argv / prompt content — ``dialect`` is a grammar NAME, not
...
def _make_unavailable(
    *,
    ...
    return ResolvedShell(
        ...
        verdict="unavailable",
        rejection_reason=rejection_reason,
        dialect=_dialect.DIALECT_UNKNOWN,
...
def _make_usable_bash(
    *,
    ...
    return ResolvedShell(
        ...
        verdict="usable",
        rejection_reason="",
        # Read off the RESOLVED BINARY, never assumed from the backend
        # label — the tag has to describe what will actually parse the
        # command, or it is the same species of claim phase 1 removed.
        dialect=_dialect.dialect_for_executable(path),
        audit_payload=_build_audit_payload(
            backend=backend,
            path=path,
            version=version,
...
def _make_rejected_powershell(
    *,
    ...
    return ResolvedShell(
        ...
        verdict="rejected",
        rejection_reason=reason,
        # Rejected, so no dispatch can follow; the tag records WHICH
        # grammar was refused rather than pretending none was seen.
        dialect=_dialect.DIALECT_POWERSHELL,
        audit_payload=_build_audit_payload(
            backend="powershell_superadmin",
            path=path,
            version="",
...
def _read_flavor(project_root: Path) -> str:
    ...
    path-locks the `dev` claim. This reader gates a real dev override, so
    the raw read it used to do was the most load-bearing of the three.
    The 'solo' fail-closed value is unchanged and passed explicitly.
    """
    from .distribution_flavor import SOLO, effective_flavor

    return effective_flavor(project_root, on_error=SOLO)
