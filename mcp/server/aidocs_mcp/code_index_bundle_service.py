...
class CodeIndexBundleService:
    ...
    def investigate(
        ...
        evidence lanes consult:
          ...
          * LSP guest-oracle MATERIALIZED reference sites (code_edges
            semantic_ref rows, §XXVII — an absent guest costs zero).
        ...
        # 0. CONCEPT (#1039 A+ 5): the AND-first cluster ranker runs FIRST and
        # its full-coverage rows are folded into the top of the symbol tier.
        # "draft cleanup" used to rank Document (one term) above
        # StaleDraftCleanupJob (both terms, AND its summary says "removes
        # abandoned empty draft documents") because the symbol probe scored
        # container NAMES only. Prose is signal; AND beats OR.
        concept_top: list[dict[str, object]] = []
        concept_terms: list[str] = []
        try:
            from .concept_rank import query_terms

            concept_terms = query_terms(needle)
            if len(concept_terms) >= 2:
                concept = self.store.search_concept(project_root, needle, limit=symbol_limit * 2)
                rows = list(concept.get("results") or [])
                best = max((int(r.get("terms_matched") or 0) for r in rows), default=0)
                # Full coverage first; otherwise the best partial coverage,
                # but never single-term rows (that is the OR-flood).
                floor = best if best >= 2 else 0
                concept_top = [r for r in rows if floor and int(r.get("terms_matched") or 0) >= floor]
        except Exception:
            concept_top = []
        if concept_top:
            findings.append(
                {
                    "area": "concept",
                    "source": "concept_ranker",
                    "terms": concept_terms,
                    "count": len(concept_top),
                    "top": [
                        {
                            k: r.get(k)
                            for k in ("symbol", "kind", "path", "facet", "terms_matched", "terms_total")
                        }
                        for r in concept_top[:6]
                    ],
                },
            )
        ...
        if concept_top:
            # Fold the concept ranker's symbol rows in FIRST (deduped by
            # path+symbol); the name-only tier keeps its order after them.
            _seen = {(str(s.get("path")), str(s.get("symbol"))) for s in symbols}
            folded: list[dict[str, object]] = []
            for r in concept_top:
                if r.get("facet") != "symbol":
                    continue
                key = (str(r.get("path")), str(r.get("symbol")))
                if key in _seen:
                    symbols = [s for s in symbols if (str(s.get("path")), str(s.get("symbol"))) != key]
                _seen.add(key)
                folded.append(
                    {
                        "symbol": r.get("symbol"),
                        ...
                        "tier": "concept",
                        "terms_matched": r.get("terms_matched"),
                    },
                )
            symbols = folded + list(symbols)
        if symbols:
        ...
        # (§XXVII: the guest's durable trace; an absent guest costs zero).
        try:
    ...
    def _lsp_oracle_lane(
        ...
        (§XXVII fail-open). Live on-demand materialization for investigate's
