...
PERMISSIONS = {
    ...
    "config.security": "Change security settings (dev_mode, gate.enforce)",
    # Memory operations
...
BUILTIN_ROLES: dict[str, dict[str, object]] = {
    ...
    "operator": {
        ...
        "permissions": [
            "code.read", "code.edit", "code.create",
            "session.read", "session.create", "session.edit", "session.claim",
            "config.read", "config.edit",
            "memory.read", "memory.write",
            "conductor.view", "conductor.control",
            "tools.bash", "tools.agent",
        ],
    },
    "viewer": {
...
TOOL_PERMISSIONS: dict[str, str] = {
    # Code read
    "code_get_lines": "code.read",
    "code_find": "code.read",
    "code_trace": "code.read",
    "code_bundle": "code.read",
    "code_search": "code.read",
    "code_text_search": "code.read",
    "code_investigate": "code.read",
    "code_get_symbol_snippet": "code.read",
    "code_get_dependencies": "code.read",
    "code_get_modules": "code.read",
    "code_get_module_files": "code.read",
    # Code edit
    "code_edit_lines": "code.edit",
    "code_batch_edit": "code.edit",
    "code_str_replace": "code.edit",
    "code_batch_str_replace": "code.edit",
    "code_insert_lines": "code.edit",
    "code_create_file": "code.create",
    # Sessions
    "session_list": "session.read",
    "session_read": "session.read",
    "session_select": "session.read",
    "session_create": "session.create",
    "session_update": "session.edit",
    "session_claim": "session.claim",
    "session_release": "session.claim",
    "task_begin": "session.edit",
    "task_update": "session.edit",
    "task_complete": "session.edit",
    # Config
    ...
    # Conductor
    "plan_conductor_status": "conductor.view",
    "plan_conductor_graph": "conductor.view",
    "execution_loop_next": "conductor.control",
    "plan_dispatch_next": "conductor.control",
    "plan_dispatch_report": "conductor.control",
    # Tools
    "bash": "tools.bash",
}
...
class RBACStore:
    ...
    def create_user(
        self, project_root: Path, username: str, role: str = "operator",
    ) -> User:
        if role not in self._all_role_ids(project_root):
            raise ValueError(f"Unknown role: {role}. Available: {list(self._all_role_ids(project_root))}")
    ...
    def get_user(self, project_root: Path, username: str) -> User | None:
        with self._lock:
            ...
            try:
                row = conn.execute(
                    "SELECT * FROM rbac_users WHERE username = ?", (username,)
                ).fetchone()
                if row is None:
                    return None
                return User(
    ...
    def deactivate_user(self, project_root: Path, username: str) -> bool:
        with self._lock:
            ...
            try:
                cursor = conn.execute(
                    "UPDATE rbac_users SET active = 0 WHERE username = ?", (username,),
                )
                conn.commit()
                return cursor.rowcount > 0
            finally:
    ...
    def get_role_permissions(self, project_root: Path, role: str) -> list[str]:
        ...
        try:
            row = conn.execute(
                "SELECT permissions_json FROM rbac_custom_roles WHERE role_id = ?", (role,),
            ).fetchone()
            if row is None:
                return []
    ...
    def create_custom_role(
        self, project_root: Path, role_id: str, description: str, permissions: list[str],
    ) -> dict[str, object]:
    ...
    def check_permission(
        self, project_root: Path, username: str | None, permission: str,
    ) -> PermissionCheck:
        ...
        if not users:
            return PermissionCheck(allowed=True, reason="No RBAC users configured — all operations allowed.")
    ...
    def check_tool_permission(
        self, project_root: Path, username: str | None, tool_name: str,
    ) -> PermissionCheck:
        ...
        if permission is None:
            ...
            return PermissionCheck(allowed=True, reason=f"Tool '{name}' has no permission mapping — allowed by default.")
