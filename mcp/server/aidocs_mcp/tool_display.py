...
def _tc_both(text: str) -> TextContent:
    """TextContent visible to both user and LLM."""
    return _tc(text, audience=["user", "assistant"])
...
#
# Why (b) (king bubble-truth re-seal 2026-05-30): the overwhelming
    ...
    for k, v in structured.items():
        if v is None or isinstance(v, (bool, int, float)):
            ...
            try:
                ...
                size = len(str(v))
            # TYPE-PRESERVING (#61, king 2026-06-20): a big list elides to [] — NOT a
...
def render_schema(result: Any, *, query: str = "", mode: str = "") -> list[TextContent]:
    """Render schema_query — entity/field results as bullet rows."""
    blocks: list[TextContent] = []
    if isinstance(result, list):
    ...
    if tail:
        ...
        try:
            ...
            if kind.startswith("test-"):
                ...
                # Probe / unknown: show body verbatim, not as a bullet.
...
def renders_as(kind: str, title: str | None = None):
    ...
    def decorator(fn):
        ...
        def _render(raw: Any, kwargs: dict[str, Any], call_start: float) -> Any:
            ...
            if isinstance(raw, dict):
                try:
                    from .index_staleness import INDEX_BACKED_TOOLS, apply_axes
                    from .mcp_server_runtime_helpers import resolve_project_root

                    _axes = INDEX_BACKED_TOOLS.get(tool_name)
                    if _axes:
...
def _append_pending_lane_reviews(
    blocks: list[TextContent],
    project_root: Path | None,
) -> None:
    """Surface pending lane_completion_reviews for this conductor's
    session as a 📋 block. Per emperor §VIII the LANE blocks waiting
    for verdict; the CONDUCTOR is never blocked — they see pending
    requests on every tool call and decide via
    ai_review. Lane workers are fenced out
    via AIDOCS_EXPERT_LANE_ID env (worker should never see another
    lane's pending review). Phoenix, 2026-05-07.
    """
    try:
        import os as _os

        if _os.environ.get("AIDOCS_EXPERT_LANE_ID", "").strip():
            return
        ...
        from .mcp_server_runtime_helpers import (
            resolve_project_root as _rpr,
        )

        root = project_root if project_root is not None else _rpr()
        session_id = ""
        try:
            ...
            session_id = ""
        if not session_id and not host_sid:
            return
        pending = _lcr.pending_for_session(
            root,
            session_id=session_id,
            host_session_id=host_sid,
        )
        if pending:
            blocks.append(_tc(_lcr.format_pending_block(pending)))
    except Exception:
        # Never break a tool response over a notification surface.
        pass
...
def _append_pending_notifications(
    ...
    """Drain the project's run_notifications queue and append a 📣 block
    listing any runs that finished while the agent was doing other
    work. Silent no-op when the queue is empty — invisible by default.

    Called from every tool response builder so the agent never has to
    poll a run to learn it finished; the notification arrives on the
    NEXT tool call after completion, whatever that call is. Paired
    with the hard-block on ai_run_output(wait_seconds>0) this
    eliminates polling as a pattern.

    #50 L3 (canonical 2026-04-26):
    - Lane workers (AIDOCS_EXPERT_LANE_ID env set) NEVER drain.
      Drain is a conductor/session-owner primitive. Subagents receive
      run results via the lane plan/handoff path, not via the conductor's
      drain queue. Without this gate, a worker would silently steal its
      own conductor's notifications.
    - Conductor callers resolve THEIR session_id via managed_mode
      (per-conductor mapping per #58) and call drain(session_id=...)
      so cross-conductor / cross-session notifications stay isolated.
    """
    try:
        import os as _os

        # Lane-caller fence: workers don't drain. Conductor primitive only.
        if _os.environ.get("AIDOCS_EXPERT_LANE_ID", "").strip():
            return
        ...
        # Resolve calling conductor's session via the per-conductor
        # mapping (#58). Empty session_id falls through to unfiltered
        # drain — back-compat for hosts without managed_mode active or
        # without conductor identity available.
        session_id = ""
        try:
            from . import managed_mode_service as _mm
            from .service_hub import AidocsServiceHub  # noqa: F401

            host_sid = current_calling_host_session_id()
            svc = _mm.ManagedModeService()
            managed = svc.get_mode(root, host_session_id=host_sid)
            if managed.get("active"):
                session_id = str(managed.get("session_id") or "").strip()
        except Exception:
            session_id = ""

        # Phoenix 2026-05-07: peek (no remove) so notifications
        # persist across every tool surface until dismissed by
        # ai_run_output reading the run. Empty session_id falls
        # back to unfiltered peek for legacy back-compat (host
        # without managed_mode active or unresolvable identity) —
        # the test pin in test_run_notifications_drain_layer_
        # isolation.py asserts this fallback.
        if session_id:
            pending = _rn.peek_for_session(root, session_id=session_id)
        else:
            pending = _rn.peek(root)
        if pending:
            blocks.append(_tc(_rn.format_block(pending)))
    except Exception:
        # Tool responses must never fail because of a notification
        # drain error. Silent fallback.
        pass
    # Phoenix 2026-05-07: surface pending lane completion reviews
    # alongside run-done notifications. Same shape, same call sites.
    _append_pending_lane_reviews(blocks, project_root)
