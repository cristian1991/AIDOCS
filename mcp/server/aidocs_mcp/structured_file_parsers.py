...
def _sqlite_connect_readonly(path: Path) -> sqlite3.Connection:
    ...
    # driver level — belt-and-braces with the statement regex above. The URI
    # is now built by the canonical connect (#755) rather than here, which
    # matters more for THIS reader than for a store: `path` is an ARBITRARY
    # operator-named file, so a '?' or '#' in it used to be parsed as URI
    # syntax and silently open a different database (or none).
    return _canonical_connect(path, read_only=True, row_factory=False)
...
_RELATIVE_SPAN_RE = re.compile(r"^\s*(\d+(?:\.\d+)?)\s*([smhdw])\s*$", re.IGNORECASE)
_SPAN_SECONDS = {"s": 1, "m": 60, "h": 3600, "d": 86_400, "w": 604_800}


def _parse_iso_utc(value: Any) -> datetime | None:
    """ISO-8601 string -> aware UTC datetime; None when absent/unparseable.
    A naive timestamp is taken as UTC (Claude Code writes ``...Z``)."""
...
def _parse_time_bound(name: str, value: str | None, now: datetime) -> datetime | None:
    """``since``/``until`` value -> aware UTC datetime. Accepts ISO-8601 or a
    relative span back from ``now`` (``30m``, ``30h``, ``2d``, ``1w``). ``now``
    is the caller's ONE frozen effective_now so both bounds share an instant.
    Malformed or overflowing spans raise ValueError (never OverflowError)."""
    if value is None or not str(value).strip():
        return None
    m = _RELATIVE_SPAN_RE.match(str(value))
    if m:
        try:
            seconds = float(m.group(1)) * _SPAN_SECONDS[m.group(2).lower()]
            if not math.isfinite(seconds):
                raise OverflowError("non-finite span")
            return now - timedelta(seconds=seconds)
        except OverflowError as exc:
            raise ValueError(f"{name} relative span out of range: {value!r}") from exc
    dt = _parse_iso_utc(str(value))
    if dt is None:
        raise ValueError(
            f"{name} must be ISO-8601 (e.g. 2026-09-23T00:00:00Z) or a relative "
            f"span like '30h'/'2d' (got {value!r})",
        )
    return dt
...
def read_jsonl(
    ...
    `since` / `until` bound the row's top-level ISO-8601 `timestamp`
    (inclusive; ISO or relative '30h'/'2d' back from `now`). With either set,
    rows with no/unparseable `timestamp` are excluded and counted in
    `untimed_skipped`; without them behaviour is unchanged.
    ...
    invalid = 0
    matched = 0
    # ONE frozen UTC instant for BOTH relative bounds (two clock reads would
    # make equal spans describe two different instants).
    effective_now = datetime.now(UTC) if now is None else now
    if effective_now.tzinfo is None:
        effective_now = effective_now.replace(tzinfo=UTC)
    effective_now = effective_now.astimezone(UTC)
    since_dt = _parse_time_bound("since", since, effective_now)
    until_dt = _parse_time_bound("until", until, effective_now)
    if since_dt is not None and until_dt is not None and since_dt > until_dt:
        raise ValueError(
            f"invalid time range: since {since_dt.isoformat()} is after until "
            f"{until_dt.isoformat()} (since={since!r}, until={until!r})",
        )
    ...
    with p.open("r", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if time_filtered:
                ts = _parse_iso_utc(obj.get("timestamp") if isinstance(obj, dict) else None)
                if ts is None:
                    untimed_skipped += 1
                    continue
    ...
    if time_filtered:
        out["untimed_skipped"] = untimed_skipped
        out["since"] = since_dt.isoformat() if since_dt is not None else None
        out["until"] = until_dt.isoformat() if until_dt is not None else None
    return out
