...
# The judge could not finish a security inspection. Not an offence (no
# strike — the defect is ours) and not confirmable (intent cannot supply the
# inspection that never ran). Hard refusal of this invocation only.
CLASS_SECURITY_INSPECTION_FAILURE = "security_inspection_failure"

ALL_CLASSES: frozenset[str] = frozenset(
    {
        CLASS_SAFE_ADVISORY,
        CLASS_CONFIRMABLE_DESTRUCTIVE,
        CLASS_MALICIOUS_FORBIDDEN,
        CLASS_SECURITY_INSPECTION_FAILURE,
    },
)
...
# BLOCK_SECURITY_FAILURE: a security inspection did not complete. Hard
# refuse, NO strike, NO confirm prompt, operator intent ignored.
DECISION_BLOCK_SECURITY_FAILURE = "block_security_failure"
...
_SECURITY_INSPECTION_FAILURE: set[str] = {
    # ── Protected-path inspection could not complete ───────────────
    # The protected-path family used to wrap its whole analysis in
    # `except Exception: pass`, so any parser or classifier exception
    # became "no verdicts" — a proven bypass (an oversized numeric literal
    # made the fold throw and every protected-path rule vanished for that
    # body). An inspection that did not finish is UNKNOWN, not clean.
    # NOT malicious_forbidden: that records a strike, and a strike for our
    # own parser defect punishes the operator for a bug in the judge.
    # NOT confirmable: operator intent would turn it into an ask, and a
    # confirm cannot supply the inspection that never ran.
...
for _rid in _SECURITY_INSPECTION_FAILURE:
    RULE_CLASS[_rid] = CLASS_SECURITY_INSPECTION_FAILURE
...
# ── Declared command-shape coverage (#624) ─────────────────────────
#
# WHY THIS TABLE EXISTS. The module docstring above declares, in
# PROSE, which command shapes each class covers — "service stop", "db
# drop", "kill", "git rollback/reset/restore". Prose cannot be
# executed, so nothing ever checked that a shape the taxonomy CLAIMS
# to cover is a shape some pattern in heuristic_judge can actually
# MATCH. #624 is what that gap costs: the docstring claimed "service
# stop" was confirmable_destructive and rule BASH_SERVICE_STOP
# shipped, while the implementing pattern required *service <word>
# stop* — so `aidocs service stop`, the one stop that DISABLES THE
# GOVERNANCE DAEMON, was the single shape the rule could not see. Two
# layers each claiming coverage, neither having it.
#
# The disease was never the regex; it was the absence of a parity
# check between DECLARED coverage and IMPLEMENTING patterns. This
# table is that declaration made machine-readable, and
# tests/security/test_taxonomy_pattern_parity_624.py is the parity
# gate: every shape below MUST draw its declared rule_id out of the
# real judge, and MUST classify into its declared class. A regex that
# stops matching a declared shape now fails the suite instead of
# silently un-covering it.
#
# MAINTENANCE: adding a rule_id to a bucket above does not require an
# entry here, but adding one is how the rule's coverage stops being a
# claim. The table may only GROW — a shrinking corpus is coverage
# being withdrawn, and _DECLARED_SHAPE_FLOOR below pins the floor.
#
# DELIBERATE OMISSION: filesystem-destruction shapes (BASH_RM_RF_*,
# BASH_FIND_DELETE, BASH_WIN_DELETE, INLINE_FS_DESTROY) are NOT
# declared here. Their literal text is itself matched by the host bash
# policy when this file is read as evidence (#626 froze four lanes on
# exactly that), so they are pinned by rule_id in the judge's own
# suite instead. This omission is the reason the floor below is a
# floor and not a total.
DECLARED_COMMAND_SHAPES: dict[str, tuple[str, ...]] = {
    # ── Service / daemon / process stop ────────────────────────────
    # The last three are AIDOCS's OWN daemon lifecycle verbs. They are
    # the #624 regression vectors: the governance daemon's stop must be
    # at least as visible to the judge as a stranger's nginx.
    "BASH_SERVICE_STOP": (
        ...
        "sc stop MyService",
        "aidocs service stop",
        "aidocs service restart",
        "python -m aidocs_mcp.cli service stop",
    ),
    "BASH_KILL_PROCESS": (
        "kill -9 12345",
        "pkill -f uvicorn",
    ...
    # ── DB drop ────────────────────────────────────────────────────
    # SQL-STATEMENT shape AND the two CLI-wrapper shapes (#660): the
    # parity gate found `dropdb`/`mysqladmin drop` uncovered the first
    # time it ran; heuristic_judge's BASH_DB_DROP pattern now matches
    # all four and KNOWN_UNCOVERED_SHAPES below is empty.
    "BASH_DB_DROP": (
        "psql -c 'drop table users'",
        "mysql -e 'drop database appdb'",
        "dropdb production",
        "mysqladmin drop appdb",
    ),
    # ── Git rollback / restore / reset / force ─────────────────────
    "GIT_RESET_HARD": ("git reset --hard origin/main",),
    "GIT_RESTORE_DISCARD": ("git restore --staged --worktree src/",),
    "GIT_CLEAN": ("git clean -fdx",),
    "GIT_FORCE_PUSH": ("git push --force origin main",),
    "GIT_BRANCH_DELETE": ("git branch -D feature/old",),
    "GIT_STASH_DROP": ("git stash drop",),
    # ── Sudo / privileged invocation ───────────────────────────────
    "BASH_SUDO": ("sudo apt-get install -y curl",),
}

# Coverage ratchet. The corpus above is SEEDED, not complete — most
# rule_ids still carry only a prose claim. This floor makes the gap
# COUNTABLE and one-way: the parity test refuses a corpus that shrank.
# Raising it is how coverage debt gets paid down; lowering it is
# withdrawing a guarantee and must be an explicit, reviewed act.
_DECLARED_SHAPE_FLOOR: int = 10


# ── Known-uncovered shapes: the debt, pinned (#624) ────────────────
#
# Shapes the taxonomy's PROSE claims but no pattern matches. The parity
# gate found each of these the first time it ran, which is the whole
# argument for having it: the docstring says "db drop" is
# confirmable_destructive, and the two most common ways an operator
# actually drops a database are invisible to the rule.
#
# These are pinned ASSERTING THE CURRENT WRONG BEHAVIOUR — the test
# requires them NOT to match. That is deliberate: an unfixed gap that
# fails loudly is a bug report; an unfixed gap that was quietly deleted
# from the corpus is the #624 disease again. When a pattern is widened
# to cover one, its test flips and the entry MOVES UP into
# DECLARED_COMMAND_SHAPES. The table may only SHRINK.
#
# #660 fixed `dropdb`/`mysqladmin drop` — both entries moved up into
# DECLARED_COMMAND_SHAPES and the ceiling below dropped to 0. Empty,
# not deleted: the table stays wired for the next gap the gate finds.
KNOWN_UNCOVERED_SHAPES: dict[str, tuple[str, ...]] = {}

_UNCOVERED_SHAPE_CEILING: int = 0


def known_uncovered_shapes() -> tuple[tuple[str, str], ...]:
    """Flatten KNOWN_UNCOVERED_SHAPES into (rule_id, command) pairs."""
    return tuple(
        (rule_id, command)
        for rule_id, commands in KNOWN_UNCOVERED_SHAPES.items()
        for command in commands
    )


def declared_shapes() -> tuple[tuple[str, str], ...]:
    """Flatten DECLARED_COMMAND_SHAPES into (rule_id, command) pairs.

    The parity gate's input. Kept as a function so the corpus is read
    through ONE door — a caller cannot accidentally test a subset.
    """
    return tuple(
        (rule_id, command)
        for rule_id, commands in DECLARED_COMMAND_SHAPES.items()
        for command in commands
    )
...
def evaluate_verdicts(
    ...
    *,
    ...
    Precedence (strict — DOES NOT depend on heuristic risk levels):
      ...
      1b. Otherwise, any security_inspection_failure hit →
         DECISION_BLOCK_SECURITY_FAILURE (no confirm, no strike,
         intent ignored).
    ...
    # Pass 2: a security inspection that did not finish. Below malicious
    # (a real offence still strikes) and above confirmable (intent must
    # not turn an unfinished inspection into an ask). Intent is ignored.
    for rid in ids:
        if classify(rid) == CLASS_SECURITY_INSPECTION_FAILURE:
            return Decision(
                decision=DECISION_BLOCK_SECURITY_FAILURE,
                triggering_rule_id=rid,
                triggering_class=CLASS_SECURITY_INSPECTION_FAILURE,
                reason=(
                    f"rule {rid}: security inspection did not complete — "
                    "hard refused, not confirmable, no strike"
                ),
                strike=False,
            )
    # Pass 3: any confirmable_destructive? Split (2026-05-26):
    ...
    # Pass 4: only safe_advisory or empty → allow.
