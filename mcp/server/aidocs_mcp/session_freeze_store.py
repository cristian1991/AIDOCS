...
class SessionFreezeStore:
    ...
    def init_db(self, project_root: Path) -> None:
        ...
        with sqlite3.connect(str(path)) as conn:
            ...
            if "agent_context_id" not in cols:
                host_expr = "host_session_id" if "host_session_id" in cols else "''"
                user_expr = "user_id" if "user_id" in cols else "''"
                legacy = conn.execute(
                    f"SELECT session_id, {host_expr} AS hsid, request_id, "
                    f"fingerprint_phrase, kind, frozen_at, expires_at, "
                    f"{user_expr} AS uid FROM session_freeze",
                ).fetchall()
                conn.executescript("""
        ...
        with sqlite3.connect(str(self.db_path(project_root))) as conn:
            ...
            if operator_user_id:
                ...
                row = conn.execute(
                    ...
                    "ORDER BY (session_id = ?) DESC, (agent_context_id = '') LIMIT 1",
                    (
                        session_id,
                        acid,
                        operator_user_id,
                ...
                row = conn.execute(
                    _cols
                    + "WHERE session_id = ? AND agent_context_id IN ('', ?) "
                    "ORDER BY (agent_context_id = '') LIMIT 1",
                    (session_id, acid),
                ).fetchone()
        if row is None:
            return None
