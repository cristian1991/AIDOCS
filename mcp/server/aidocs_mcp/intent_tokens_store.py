...
Per empire-doctrine §XI + king-directive 2026-05-05: NLP intent vocabulary
...
def group_tokens_by_parent(
    lang: str,
    kind: str,
) -> dict[str, list[str]]:
    """Convenience: returns {parent_key: [token, ...]} for the
    bare-list consumers (action_token, intent_guard, plan_vague_pattern).
    For kinds with no parent_key (anything where the bare alias IS the
    parent), use kind=='action_token' with parent_key set per-row.
    """
    out: dict[str, list[str]] = {}
    for r in get_rows_by_kind(lang, kind):
        out.setdefault(r["parent_key"] or "", []).append(r["token"])
    return out
...
def add_alias(
    ...
    *,
    ...
    """Append a single tool alias. Idempotent (INSERT OR IGNORE).
    ...
    with sqlite3.connect(str(empire_db_path())) as conn:
        conn.execute(
            "SELECT changes()",
        ).fetchone()  # init sentinel
        added = _insert_tokens(
            conn,
            (lang or "en").lower(),
            "tool_alias",
            tool_name,
...
def remove_alias(
    ...
    *,
    ...
    """Drop a single tool alias. Returns True if a row was deleted."""
    init_db()
    with sqlite3.connect(str(empire_db_path())) as conn:
        cur = conn.execute(
            """DELETE FROM intent_lemma_sets
               WHERE lang=? AND kind='tool_alias'
                 AND parent_key=? AND parent_mode=? AND token=?""",
            (
                (lang or "en").lower(),
                tool_name,
                mode,
