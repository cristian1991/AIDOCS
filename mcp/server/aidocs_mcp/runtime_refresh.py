...
refresh retries once against the settled tree before returning this.
7 = UPDATE NEEDED and 8 = UPDATE REFUSED (#925h): the authority names a build NEWER than
the one installed here and this cycle did not install it — 8 when the pull itself was
refused or unreachable, 7 when nothing on this path even attempted it. Split because the
remedies differ (investigate the refusal vs. run a refresh), and separate from 5 because
"we never got the bytes" is not "we installed and it did not take".
Non-zero means the local runtime does NOT match its reference (or could not be proven to),
so a deploy must not be reported successful over it.

THE STRONGEST ESTABLISHED FACT WINS, and it is established FIRST. Once the build axis says
deployed > current, no later step may report freshness: not a refused pull, not a release
axis that cannot compare, not a missing comparison, not even a successful provision from
some OTHER reference. Each of those answers a weaker question, and a weaker answer must
never overwrite a stronger one — that inversion is exactly how a box sat a build behind
while the log read "already current — nothing to do, verdict=fresh code=0".
"""
...
def _owned_runtime_has_mempalace() -> bool:
    ...
    try:
        ...
        if not installed:
            return False
        # BOTH SHAPES COUNT, and missing the second one SEALED THIS MACHINE
        # (#925f, measured live 2026-09-09T03:49:40).
        #
        # Until #925d the engine could only be a sibling of the package —
        # `site-packages/mempalace` — so that was the only thing this looked
        # for. #925d ships it INSIDE the package at `aidocs_mcp/_vendor`, where
        # the release signature covers it, and a runtime provisioned from such
        # an artefact has NO sibling copy at all.
        #
        # So this returned False on a runtime that demonstrably HAS the engine,
        # and the pre-pull guard above it then refused every subsequent update:
        # the first update landed, and the machine sealed itself against all the
        # rest. Observed exactly once and exactly here — build 265 refusing 266,
        # which carried the fix for the refusal.
        pkg = Path(installed)
        return (pkg.parent / "mempalace").is_dir() or (pkg / "_vendor" / "mempalace").is_dir()
...
def _outcome(outcome: dict | None, state: str, detail: str = "") -> None:
    """Record WHY the authority path ended as it did (#925h).

    `_authority_reference` returns None for four different things — the operator
    disabled the authority, the artefact was refused, the fetch failed, the
    build carried no package — and the caller could not tell them apart. It then
    fell through to a weaker axis and reported "already current", turning a
    REFUSAL into a claim of freshness.

    Optional and side-effect-only, so every existing caller and test double that
...
def _authority_reference(emit, outcome: dict | None = None) -> str | None:
    ...
    try:
        ...
        # THE GUARD MOVED BELOW THE PULL (#925d), because up here it could only
        # ask the WRONG QUESTION. It refused whenever the OWNED RUNTIME lacked
        # mempalace — and on a machine that never had it, that is a refusal
        # nothing can ever lift: the artefact carrying the cure is the artefact
        # being refused. Self-sealing, exactly the shape #925 exists to break,
        # and invisible here because this box's legacy venv happened to have the
        # engine already.
        #
        # What actually decides whether the install is safe is whether the
        # ARTEFACT can boot, and since #925d it carries the engine INSIDE the
        # signed package (aidocs_mcp/_vendor). That is knowable only after the
        # pull, so the question is asked there, against the pulled bytes.
        ...
        _outcome(outcome, "refused", str(exc))
        return None
        ...
        _outcome(outcome, "unreachable", repr(exc))
        return None
    ...
    if pkg is None:
        ...
        _outcome(outcome, "refused", "no package dir in the pulled build")
        return None
    # CAN THIS ARTEFACT BOOT? (#925d — the guard that used to sit before the
    # pull, asking about the wrong tree.)
    #
    # aidocs_mcp declares mempalace's runtime DEPS but not the engine itself, so
    # installing it over a runtime that lacks mempalace yields a venv that
    # imports aidocs_mcp and CANNOT START THE SERVER — an install where every
    # check passes and the server never comes up, which is the worst failure
    # available on this path.
    #
    # Two ways to be safe, and EITHER suffices:
    #   * the artefact carries the engine inside its signed package (#925d), so
    #     the install brings its own; or
    #   * the owned runtime already has it, the pre-#925 situation.
    # Only when NEITHER holds is the install genuinely unsafe — and then this
    # refuses, exactly as before.
    if not (pkg / "_vendor" / "mempalace").is_dir() and not _owned_runtime_has_mempalace():
        emit(
            "[runtime-refresh] REFUSING the authority's build: it carries no "
            "packaged palace engine and this runtime does not already have one, "
            "so installing it would produce a runtime that imports aidocs_mcp "
            "and cannot start. Provision from a full source tree first; this "
            "path UPDATES, it does not provision."
        )
        return None
    # ASCII ARROW ON PURPOSE (#925f). `emit` is a seam: the watchdog writes to a
    # UTF-8 log, but `aidocs runtime --fix` writes to the OPERATOR'S CONSOLE,
    # which on Windows is cp1252 — and U+2192 has no cp1252 mapping. Measured:
    # this exact line raised UnicodeEncodeError mid-refresh, AFTER the pull and
    # BEFORE the provision, turning a working update into a traceback. A
    # progress message must never be able to fail the operation it describes.
    _outcome(outcome, "ok")
    emit(f"[runtime-refresh] pulled the authority's SIGNED build -> {project}")
    return str(project), str(pkg)


#: Verdicts that mean "a newer build is KNOWN and this cycle did not install it".
#: Distinct codes because the three states need distinct remedies, and a single
#: "not green" would hide which one happened.
CODE_UPDATE_NEEDED = 7      # authority is ahead; nothing here installed it
CODE_UPDATE_REFUSED = 8     # authority is ahead; the pull itself was refused


def _known_newer_build(emit) -> tuple[bool, dict]:
    """Does the AUTHORITY name a build newer than the one installed here? (#925h)

    THE STRONGEST FACT AVAILABLE ON A CLIENT, and the one this module kept
    throwing away. `check_build_axis` compares ONE INTEGER against ONE INTEGER
    and is answerable with no checkout, no git and no ship stage — which is
    exactly the machine where every tree-diffing axis returns "cannot tell".

    Established BEFORE the pull so it survives whatever happens next: a refusal,
    an unreachable release axis, or a comparison that cannot run. Nothing later
    in the cycle may downgrade it, because none of those outcomes is evidence
    that this box is current.

    FAIL-SOFT: an errored or unanswerable axis returns False — it establishes
    nothing, so it constrains nothing. Only a POSITIVE, error-free "you are
    behind" is carried forward.
    """
    try:
        from .aidocs_service import check_build_axis

        axis = check_build_axis() or {}
    except Exception as exc:  # noqa: BLE001 — the fact is optional, never fatal
        return False, {"error": f"{type(exc).__name__}: {exc}"}
    if axis.get("error"):
        return False, axis
    known = bool(axis.get("update_available"))
    if known:
        emit(
            "[runtime-refresh] the authority names build "
            f"{axis.get('latest_build')} and this runtime is "
            f"{axis.get('current_build')} — a newer build is KNOWN to exist, so "
            "nothing below may report this runtime as current."
        )
    return known, axis
...
def refresh_runtime(
    *,
    ...
    live and unpiped — §XV), the watchdog wants it in watchdog.log. Returns
    ...
    # THE STRONGEST FACT IS ESTABLISHED FIRST AND SURVIVES THE CYCLE (#925h).
    #
    # Measured on the operator's box: the build axis said `current=265
    # deployed=266 update=True`, the pull was REFUSED, the release axis could not
    # compare, and this function reported `already current — nothing to do,
    # verdict=fresh code=0`. Every later step answered a WEAKER question, and
    # each weaker answer overwrote the strong one — so a machine that was known
    # to be a build behind reported success, and `restart_daemon` (gated on
    # code == 0) duly restarted it onto the same stale code.
    #
    # None of "the pull was refused", "the release axis cannot compare" and "no
    # comparison is available" is evidence that this runtime is current. They are
    # all reasons we FAILED to establish it. So the fact is taken once, up front,
    # and nothing below is allowed to downgrade it.
    build_axis: dict = {}
    known_newer = False
    authority: dict = {}
    # DERIVED FROM THE ARTEFACT, NOT FROM THE REPORTING CHANNEL. `authority` says
    # WHY a pull ended as it did — useful for naming the failure, and the wrong
    # thing to gate green on: anything that returned the artefact without
    # annotating the dict would read as "did not install", turning a SUCCESSFUL
    # update into a permanent non-green and stalling every future one. The fact
    # is simply whether the authority's build came back.
    installed_the_named_build = False
    if package_spec is None and reference_pkg is None:
        known_newer, build_axis = _known_newer_build(emit)
        pulled = _authority_reference(emit, outcome=authority)
        installed_the_named_build = bool(pulled)
        if pulled:
    ...
    def _behind(axis: str, before_detail: dict, after_detail: dict | None) -> dict:
        """The verdict for 'a newer build is known and this cycle did not install it'.

        Three states, three codes, because they need three different remedies:
        the pull was REFUSED (investigate the refusal), the authority could not
        be reached (transport), or nothing here even tried (needs a refresh).
        """
        state = authority.get("state") or "not-attempted"
        refused = state in ("refused", "unreachable")
        verdict = "update_refused" if refused else "update_needed"
        code = CODE_UPDATE_REFUSED if refused else CODE_UPDATE_NEEDED
        emit(
            f"[runtime-refresh] NOT CURRENT: the authority names build "
            f"{build_axis.get('latest_build')} and this runtime is "
            f"{build_axis.get('current_build')}; the update did not land "
            f"(authority={state}). Refusing to report success on a runtime that "
            "is KNOWN to be behind, whatever the weaker axes say."
        )
        return {
            "code": code,
            "verdict": verdict,
    ...
    if report.get("fresh") is None:
        # A KNOWN "you are behind" outranks "cannot tell". Both are non-zero, so
        # neither was ever a pass — but reporting `unknown` here would discard a
        # fact we hold, and the operator reading the log would be told the weaker
        # of two answers we actually have.
        if known_newer and not installed_the_named_build:
            return _behind(report["axis"], before, None)
        emit(
    ...
    if report.get("fresh") is True:
        # THE EXACT COLLAPSE THIS GOAL EXISTS TO CLOSE. "fresh" here means the
        # runtime matches whatever reference this axis could reach — which, when
        # the pull was refused, is the OLD build it already had. Matching the
        # thing you failed to replace is not currency.
        if known_newer and not installed_the_named_build:
            return _behind(report["axis"], before, before)
    ...
    if check_only:
        ...
        return {
            ...
            "before": before,
            "after": None,
            # DRIFT IS ALREADY NON-GREEN, so it tells no lie — but the build-axis
            # fact must not evaporate just because a weaker axis also had
            # something to say. A reader who sees only "drift" cannot tell
            # "differs from its reference" from "and a newer build exists that
            # we failed to fetch". Carried, not collapsed.
            "build_axis": build_axis,
            "authority": dict(authority),
        }
    ...
    for _attempt in (1, 2):
        ...
        if after_report.get("fresh") is True:
            # THE SAME GATE ON THE WAY OUT. A provision can succeed against the
            # LOCAL CHECKOUT while the authority's build was never pulled — the
            # after-report then reads fresh, and this would have returned code 0
            # for a runtime still a build behind. Green requires that the build
            # we know exists is the one that landed.
            if known_newer and not installed_the_named_build:
                return _behind(after_report["axis"], before, after)
...
def main(argv: list[str] | None = None) -> int:
    # #1095: install-wide startup steps at process entry. Never raises.
    ...
    # wants it on stdout (its verdicts must stream live and unpiped — §XV),
