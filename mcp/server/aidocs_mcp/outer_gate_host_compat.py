...
def _patch_catalog(cat: ModuleType) -> None:
    ...
    # Mirror the static catalog construction EXACTLY — including the
    # _NOT_AGENT_ADVERTISED exclusion. Omitting it here let host/desktop +
    # operator-authority tools (tauri_*, escalation, grant/revoke/ACL) reappear in
    # PROJECT_TOOLS at serve time (advertised AND routable to handle_project_tool),
    # which the static catalog correctly drops. King 2026-06-21 regression.
    _hidden = cat._NOT_AGENT_ADVERTISED
    cat.PROJECT_READ_TOOLS = frozenset(
        n
        for n, s in cat.PROJECT_TOOL_SPECS.items()
        if s["cls"] == cat.CLASS_SELECTOR and n not in _hidden
    )
    cat.PROJECT_EDIT_TOOLS = frozenset(
        n
        for n, s in cat.PROJECT_TOOL_SPECS.items()
        if s["cls"] == cat.CLASS_IMPORT and n not in _hidden
    )
...
def _selected_exec_root(*, gate, home, default_exec_root, token_id, principal=None) -> str | None:
    try:
        ...
        # M3 stale-bound parity: resolve the bound project ONLY if visible to the
        # principal. A gated member whose project was de-allowlisted after the bind
        # gets NO exec root from it (returns None → falls back to the default exec
        # root, never the stale project) — the tools/call exec backstop must not run
        # against a project the principal can no longer see.
        _status, cur = store.current_if_visible(home, token_id or "", principal)
        if _status == "project_not_allowlisted":
            return None
