    ...
    def build_enforced_context(
        ...
        *,
        ...
        if project_root is not None:
            ...
            # Seat re-surface (#225): an occupied seat reloads its ROLE BODY
            # plus law/soul POINTERS once per epoch (re-fires after compaction).
            #
            # DRT-05: occupancy used to be hardcoded True here — "the managed
            # UPS path is the seat-holder's prompt" — so EVERY managed prompt,
            # a lane worker's included, was told it held the head-conductor
            # seat. The seat is now RESOLVED from the durable seat map for this
            # host session; unmapped is not a seat and re-surfaces nothing.
            from .conductor_comms import msg_role_for_host
            from .seat_resurface import maybe_seat_resurface_blocks

            try:
                seat_role = msg_role_for_host(
                    project_root,
