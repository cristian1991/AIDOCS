...
REFUSE_LINK_EXPIRED = "sign_link_expired"
...
def verify_sign_request(
    *,
    ...
    when the deploy is awaiting_2fa, the one-time link credential matches + is unconsumed + unexpired,
    and the TOTP code is valid; otherwise ``{"ok": False, "refusal": <stable code>}``. `expires_at`
    time-limits the one-time link (None = no expiry configured; a real enqueued request always carries
    it). Pure — the caller persists the consumed flag (one-time) and advances the state on ok."""
    ...
    if expires_at is not None and now > float(expires_at):
        return {"ok": False, "refusal": REFUSE_LINK_EXPIRED}
