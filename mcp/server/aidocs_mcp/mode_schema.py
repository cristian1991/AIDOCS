...
Phoenix 2026-05-12 (king directive). Anthropic API rejects top-level
...
def modes(**specs):
    """Declare per-mode param contracts for a dispatcher tool.

    Each spec may carry:
      required: [param, ...]
      optional: [param, ...]
      desc: one-line agent-facing description — WHAT the mode does and
            what the polymorphic primary param (query/target/...) means
            in that mode. Rendered into the mode enum description so
            agents never have to guess (operator directive 2026-06-11:
            "easy to understand so agents don't have to guess what they
            have to do with what").
    """

    def wrap(func):
        func._mode_specs = specs
        return func
...
def enrich_flat_schema(base_schema, mode_specs):
    ...
    for pname, info in usage.items():
        ...
        # 'Ignored in mode=...' annotation removed 2026-05-12 (king
...
def apply_mode_schemas(server):
    ...
    for key, comp in list(components.items()):
        ...
        if fn is None:
            continue
        specs = getattr(fn, "_mode_specs", None)
        if not specs:
            continue
        try:
