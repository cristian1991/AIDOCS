...
Same verdict on raw Bash and ai_run (one shared core, doctrine VI).
"""
...
def _ws_existing_file(tok: str, ws: Path, *, suffix: str = "") -> tuple[bool, str]:
    """An EXISTING file under the workspace, named either way.

    Shares one bound with shell_local_script (#582): a relative path and the
    absolute path of the SAME file are the same file, so both spell it. The
    workspace containment check is the boundary; `_ws_token_ok` above already
    graded plain argv tokens exactly this way, and this helper was the odd one
    out that refused the spelling instead of the target.
    """
    from .shell_local_script import workspace_bounded_path

    if suffix and not tok.replace("\\", "/").lower().endswith(suffix):
        return False, f"'{tok}' is not a {suffix} file"
    rp, why = workspace_bounded_path(tok, ws)
    if rp is None:
        return False, why
    if not rp.is_file():
