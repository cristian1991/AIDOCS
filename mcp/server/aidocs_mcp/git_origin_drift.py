    ...
    *,
    ...
    if attempt_fetch:
        ...
        try:
            ...
            elif len(ran) > 2 and isinstance(ran[2], dict):
                # A runner may report WHY (category + already-scrubbed reason),
                # so unreachable is never a bare, unexplained label.
                failure = {
                    "category": str(ran[2].get("category") or "other"),
                    "reason": str(ran[2].get("reason") or ""),
                }
        except Exception as exc:
            # Offline / process failure: stay UNREACHABLE (fail-safe), never crash.
            fetched = False
            origin_check = ORIGIN_CHECK_UNREACHABLE
            failure = {"category": "other", "reason": type(exc).__name__}
    ...
    if origin_check == ORIGIN_CHECK_UNREACHABLE and failure:
        result["origin_check_category"] = failure["category"]
        result["origin_check_reason"] = failure["reason"]
    return result
