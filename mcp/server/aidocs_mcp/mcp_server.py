...
def create_server() -> Any:
    ...
    @server.tool()
    async def conductor_start(root: str = "", session_id: str = "", backend: str = "claude") -> str:
        """Start a persistent conductor agent. Backend: 'claude' or 'codex'."""
        import shutil
        import subprocess
        ...
        cli_name = "claude" if backend == "claude" else "codex"
        cli_path = shutil.which(cli_name)
        if not cli_path:
            return json.dumps({"started": False, "reason": f"{cli_name} CLI not found."})

        prompt = (
            f"You are the AIDOCS conductor for session '{sid}'. "
            f"Use AIDOCS MCP tools: plan_conductor_status, execution_loop_next, "
            f"plan_dispatch_next, plan_dispatch_report, agent_spawn_worker. "
            f"Drive lanes to completion. Wait for user commands between dispatches."
        )
        ...
        try:
            ...
            return json.dumps({"started": True, "backend": backend, "session_id": sid, "pid": child.pid})
        except Exception as exc:
