...
<td align="center" width="200">
  ...
  <sub>Hooks: <code>UserPromptSubmit</code>, <code>PreToolUse</code>, <code>PostCompact</code></sub><br>
...
- **Dashboard** — Tauri desktop app for monitoring, conductor, settings, skills, and MCP registry
- **Host adapters** — Claude hooks, OpenCode plugin, OpenAI Agents SDK adapter, and generic MCP-client support
...
mcp/                        # Optional MCP runtime + CLI layer
  ...
  tests/                    # 1178 tests (private repo only)
```
...
|------|--------|----------|
| `aidocs.toml` | TOML | Static definitions — journal limits, index skip dirs, module hints, language filtering |
...
- heuristic judge with argument-level risk assessment and config-driven dangerous patterns
- output guard — post-execution credential/injection scanning with auto-redaction
- circuit breakers with exponential backoff for flaky MCP servers
...
| Version | Highlights |
|---------|------------|
| **2.1.0b** | Agent-agnostic orchestration, heuristic judge, output guard, metrics, circuit breakers, edit history/rollback, SQLite config store, RBAC, code runner, tool policies, MCP registry, skill scanner, context compaction, deferred tool loading (50 eager / 70 deferred), PostCompact hook, conductor MCP tools, OpenAI Agents SDK adapter, dashboard monitoring/conductor/skills/MCP pages, 1178 tests |
