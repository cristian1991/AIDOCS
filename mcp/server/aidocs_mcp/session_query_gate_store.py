...
class SessionQueryGateStore(SQLiteIndexStoreBase):
    ...
    # Schema-ensure-once, per db path, per process (#489). MEASURED: init_db ran
    # 24 TIMES for one UserPromptSubmit, each run issuing a CREATE TABLE, a
    # table_info probe, ~22 ALTER statements that are EXPECTED to fail on an
    # already-migrated DB (and are swallowed), plus a filesystem walk of
    # .MEMORY/sessions in _ingest_all_legacy_json. The migration is idempotent
    # and correct — it is simply not news after the first run in a process.
    #
    # Keyed by DB PATH, never a global flag: a second project root in the same
    # process must still be migrated, and getting that wrong would leave a
    # project silently unmigrated. The file's existence is re-checked so a
    # deleted/reset DB is re-migrated rather than trusted.
    _schema_ready: set[str] = set()

    @classmethod
    def schema_ready_clear(cls) -> None:
        """Force the next init_db to re-run the migration (tests, recovery)."""
        cls._schema_ready.clear()

    def init_db(self, project_root: Path) -> None:
        db_key = str(self.db_path(project_root))
        if db_key in self._schema_ready and self.db_path(project_root).is_file():
            # The memo may NOT skip a migration. Re-checking only the file's
            # EXISTENCE was not enough: a DB that still exists but whose SHAPE
            # changed under a live process was trusted and never migrated, so a
            # read of a renamed column died with "no such column:
            # last_host_session_id". That is not hypothetical — a deploy
            # hot-swap changes the schema beneath running processes, and the
            # #151 rename is exactly such a change.
            #
            # So the fast path verifies the SHAPE, not just the file. One
            # PRAGMA table_info on a pooled connection is far cheaper than what
            # the memo exists to avoid (CREATE TABLE + ~22 expected-to-fail
            # ALTERs + the .MEMORY/sessions filesystem walk in
            # _ingest_all_legacy_json, x24 per UserPromptSubmit), so the #489
            # win is kept while the migration becomes reliable.
            try:
                with self.session(project_root) as conn:
                    have = {
                        str(r[1])
                        for r in conn.execute("PRAGMA table_info(session_query_gate)")
                    }
                # `last_host_session_id` is checked EXPLICITLY: it is the #151
                # rename's TARGET and is deliberately absent from
                # _TABLE_COLUMNS (that tuple is the SEC-001 write-surface
                # allowlist, not the physical schema). Omitting it made this
                # probe pass on a DB whose column was still named
                # last_cli_session_id — the exact state the migration exists to
                # repair — so the fast path returned and the rename never ran.
                if (
                    have.issuperset(self._TABLE_COLUMNS)
                    and "last_host_session_id" in have
                    # Privilege column read by the tool path: a DB migrated
                    # before it existed must take the full ensure below.
                    and "user_intent_actions" in have
                ):
                    return
            except sqlite3.Error:
                pass  # unreadable → fall through and let the full ensure decide
            self._schema_ready.discard(db_key)
        with self.session(project_root) as conn:
            conn.executescript(
                """
            ...
            for column_def in (
                ...
                # #599 (2026-07-31): the OPEN-TASK HOLDER SET. current_task_id
                # above is ONE cell, and a session is worked by MORE THAN ONE
                # actor whenever co-hosted agents share it (their
                # agent_context_id collapses — see #650). Actor B's task_begin
                # therefore overwrote actor A's live task id, and B's
                # task_complete then legitimately drained the cell, revoking
                # A's still-open task ("no active task" on A's very next
                # write). JSON array of every task id currently OPEN on this
                # session, in begin order. current_task_id stays the
                # single-valued read (last element) so audit stamping is
                # unchanged; closing a task removes ONLY its own id and falls
                # back to the most recent remaining holder.
                "ALTER TABLE session_query_gate ADD COLUMN open_task_ids TEXT NOT NULL DEFAULT '[]'",
                ...
                # Per-turn user-intent ACTION grants (bleed B, 2026-09-24).
                # JSON list of canonical action tuples the operator's OWN
                # UserPromptSubmit text asked for -- {tool, mode, target,
                # turn_id, origin, ...}. The generic sibling of
                # user_intent_tools: that column grants a TOOL for a turn;
                # this one grants ONE named ACT (ai_session delete X) for a
                # turn, consumed single-use by consume_user_intent_action.
                # Local control-plane acts (ai_session create/delete today,
                # ai_project later) read their consent here instead of from
                # a phrase the model can echo. PRIVILEGE STATE: in
                # _PRIVILEGE_COLUMNS, replaced at every operator UPS.
                "ALTER TABLE session_query_gate ADD COLUMN user_intent_actions TEXT NOT NULL DEFAULT '[]'",
            ):
                try:
        ...
        # Marked ready only HERE — after the schema work and the legacy ingest
        # have both completed. An exception on the way leaves the memo unset so
        # the next call retries rather than inheriting a half-migrated DB.
        self._schema_ready.add(db_key)
    ...
    def record_host_session_ids(
        ...
        with self.session(project_root) as conn:
            ...
            if not changed:
                return False
            # EVICTION MUST BE VISIBLE (#859, 2026-08-21). This truncation is
            # silent, and what it drops is an ATTESTED id — the evidence
            # chain_attested_heal uses to recognise a window. When a live
            # window's id falls off the front, that window stops being
            # recognised and locks out, and NOTHING anywhere says why; the heal
            # simply returns None, which is indistinguishable from "never owned
            # it". MEASURED the same day: phoenix held 9 of 16 entries and
            # gained FOUR in a single evening (Claude Code rotates the host id
            # on update, window change and /mcp reconnect), so the cap is a few
            # sessions away, not a theoretical bound.
            #
            # The cap itself is deliberately NOT raised here: a longer chain is
            # a WIDER authority set (every entry is a window permitted to act),
            # so that is an operator decision, not a cleanup. This only ensures
            # the decision is informed by a signal instead of a lockout.
            if len(chain) > self._HOST_ID_CHAIN_CAP:
                _evicted = chain[: -self._HOST_ID_CHAIN_CAP]
                chain = chain[-self._HOST_ID_CHAIN_CAP :]
                _logger.warning(
                    "host-id chain full for session %s: evicted %d attested id(s) "
                    "%s (cap=%d). The chain is no longer an authority predicate "
                    "(#1001 retired chain_attested_heal); a window with no "
                    "per-conductor binding refuses managed_mode_not_active and is "
                    "told to run ai_session(mode='connect').",
                    session_id,
                    len(_evicted),
                    _evicted,
                    self._HOST_ID_CHAIN_CAP,
                )
            conn.execute(
    ...
    def set_current_task_id(
        ...
        Pass empty string to CLEAR THE WHOLE SESSION — the wholesale reset.
        Cheap writer — hot-path on every task_begin / task_complete and a
        once-per-session cost for every other gate write.

        #599: a non-empty write also registers the id in the OPEN-TASK HOLDER
        SET (``open_task_ids``), so a later :meth:`close_session_task` can fall
        back to a peer's still-open task instead of blanking the session. The
        empty-string write keeps its historical meaning — it purges every
        holder, which is what keeps a stale/abandoned begin RECLAIMABLE rather
        ...
        with self.session(project_root) as conn:
            ...
            holders: list[str] = []
            if tid:
                holders = [h for h in self._read_open_task_ids(conn, session_id) if h != tid]
                holders.append(tid)
            conn.execute(
                "UPDATE session_query_gate SET current_task_id = ?, "
                "open_task_ids = ? WHERE session_id = ?",
                (tid, json.dumps(holders), session_id),
            )

    def _read_open_task_ids(self, conn: Any, session_id: str) -> list[str]:
        """The open-task holder set for a session, in begin order (#599)."""
        try:
            row = conn.execute(
                "SELECT open_task_ids FROM session_query_gate WHERE session_id = ?",
                (session_id,),
            ).fetchone()
        except Exception:
            return []
        if row is None:
            return []
        try:
            parsed = json.loads(str(row["open_task_ids"] or "[]"))
        except Exception:
            return []
    ...
    def close_session_task(
        self,
        project_root: Path,
        session_id: str,
        task_id: str,
    ) -> str:
        """Close ONE task on this session and return the new current id (#599).

        The session-level slot is shared by every co-hosted actor (their
        agent_context_id collapses — #650), so "clear the slot" is the wrong
        primitive: it revokes a peer's still-open task. This removes only
        ``task_id`` from the holder set and re-points ``current_task_id`` at
        the most recent REMAINING holder — empty only when this was the last
        open task, so a genuinely finished task still closes.

        Closing an id this session never opened is an honest no-op: it does
        not disturb the holders and does not drain anyone else's slot.
        """
        tid = str(task_id or "").strip()
        with self.session(project_root) as conn:
            self._ensure_row(conn, session_id)
            holders = self._read_open_task_ids(conn, session_id)
            row = conn.execute(
                "SELECT current_task_id FROM session_query_gate WHERE session_id = ?",
                (session_id,),
            ).fetchone()
            current = str((row["current_task_id"] if row else "") or "")
            if not tid or tid not in holders:
                # Legacy rows (written before open_task_ids existed) have no
                # holders at all; falling back to the pre-#599 behaviour there
                # keeps the single-actor contract exact.
                if tid and not holders and current == tid:
                    conn.execute(
                        "UPDATE session_query_gate SET current_task_id = '' "
                        "WHERE session_id = ?",
                        (session_id,),
            ...
            if current == tid:
                current = holders[-1] if holders else ""
            conn.execute(
                "UPDATE session_query_gate SET current_task_id = ?, "
                "open_task_ids = ? WHERE session_id = ?",
                (current, json.dumps(holders), session_id),
            )
        return current
    ...
    def rotate_current_turn_id(
        ...
        *,
        ...
        prior_turn_id = self.get_current_turn_id(project_root, session_id)
        # #444 TERMINAL HALF — seal the turn this instruction just ended.
        # This is the causal-turn's NATURAL end (the prior turn is provably
        # over the moment a new operator instruction is accepted), so the
        # seal lands here and the watchdog recovery sweep stays the backstop
        # rather than the only sealer (spec invariant 10). Runs BEFORE
        # open_turn so the prior turn is still pre-terminal and a mid-flight
        # attempt can be recorded as a first-class OperatorInterrupt.
        if prior_turn_id and prior_turn_id != new_turn_id:
            try:
                from .causal_turn_store import CausalTurnStore

                CausalTurnStore().close_superseded_turn(
                    project_root,
                    prior_turn_id,
                    superseded_by=new_turn_id,
                    session_id=session_id,
                    actor_id=actor_id,
                ...
                # A failure to seal must never be INVISIBLE (that would swap
                # an unenforced invariant for a lying one): it is logged AND
                # recorded as its own audit event, and the un-sealed turn
                # stays visible to the recovery sweep, which retries it.
                self._report_seal_failure(
                    project_root, session_id, prior_turn_id, exc
                )
        with self.session(project_root) as conn:
            self._ensure_row(conn, session_id)
            conn.execute(
    ...
    @staticmethod
    def _report_seal_failure(
        project_root: Path,
        session_id: str,
        turn_id: str,
        exc: BaseException,
    ) -> None:
        """Make a failed turn seal LOUD, never silent.

        The prompt boundary is best-effort by mint doctrine, but silence
        would be worse than the original defect: an unenforced invariant
        replaced by a lying one. So the failure is logged at WARNING and
        appended to the hash-bound audit chain as its own
        ``turn_seal_failed`` event; the turn itself stays un-sealed and
        therefore still visible to the recovery sweep, which retries it.
        """
        _logger.warning(
            "causal turn seal FAILED session=%s turn=%s: %r "
            "(turn stays open; recovery sweep will retry)",
            session_id,
            turn_id,
            exc,
        )
        try:
            ...
            ExecutionIndexStore().record_event(
                project_root,
                event_kind="turn_seal_failed",
                source_kind="session_query_gate_store",
                session_id=session_id or None,
                action_kind="seal",
                target_entity=turn_id,
                status="error",
                payload={"turn_id": turn_id, "error": repr(exc)},
            )
        except Exception:
            # The logger line above is the floor; an audit-store that is
            # itself broken cannot be reported through the audit store.
            pass
    ...
    _PRIVILEGE_COLUMNS: tuple[str, ...] = (
        ...
        # Bleed B (2026-09-24): per-turn operator ACTION grants. Each
        # entry is consent for one named control-plane act; a blocked
        # or unmanaged prompt MUST leave it byte-identical.
        "user_intent_actions",
    )
    ...
    #
    # Generic, per-turn, single-use consent for ONE named control-plane act,
    # minted only from the operator's own UserPromptSubmit text (the UPS
    # stage passes the provenance-floored operator slice, origin-gated).
    # Entry shape (canonical action tuple, never raw regex captures):
    #   {tool, mode, target, turn_id, origin, ...extra bind/provenance}
    # ``tool``/``mode``/``target`` are the bind axes; ``turn_id`` is the
    # operator turn the instruction came from, and a grant is consumable only
    # while that turn is still the session's current turn.

    _ACTION_BIND_KEYS: tuple[str, ...] = ("tool", "mode", "target")
    _ACTION_ORIGIN = "user_prompt_submit"
    _ACTION_PROVENANCE = "operator_text"

    @classmethod
    def _normalize_actions(cls, actions: Any) -> list[dict[str, str]]:
        ...
        for entry in actions:
            ...
            if not all(str(clean.get(k) or "").strip() for k in cls._ACTION_BIND_KEYS):
                continue
            clean["tool"] = clean["tool"].strip().lower()
            clean["mode"] = clean["mode"].strip().lower()
            clean["target"] = clean["target"].strip()
            out.append(clean)
        return out

    def set_user_intent_actions(
        self,
        project_root: Path,
        session_id: str,
        actions: list[dict[str, Any]],
    ) -> None:
        """Replace the session's per-turn action grants wholesale. Called
        from the UPS per-turn stage with THIS prompt's grants (or [] to
        clear the previous turn's unused ones)."""
        clean = self._normalize_actions(actions)
        with self.session(project_root) as conn:
            self._ensure_row(conn, session_id)
            conn.execute(
                "UPDATE session_query_gate SET user_intent_actions = ? WHERE session_id = ?",
                (json.dumps(clean), session_id),
            )

    def get_user_intent_actions(
        self,
        project_root: Path,
        session_id: str,
    ) -> list[dict[str, str]]:
        with self.session(project_root) as conn:
            row = conn.execute(
                "SELECT user_intent_actions FROM session_query_gate WHERE session_id = ?",
                (session_id,),
            ).fetchone()
        if row is None:
            return []
        try:
            return self._normalize_actions(json.loads(row["user_intent_actions"] or "[]"))
        except (ValueError, TypeError):
            return []

    def consume_user_intent_action(
        self,
        project_root: Path,
        session_id: str,
        *,
        ...
        """Atomically remove and return ONE grant matching (tool, mode,
        target) whose turn_id is the session's CURRENT turn; None when there
        is no such grant.

        Single-use by construction: the read, the match and the write happen
        inside one ``BEGIN IMMEDIATE`` transaction (the write lock is taken
        BEFORE the read), and the UPDATE is additionally a compare-and-swap
        on the exact prior value. Two concurrent callers therefore can never
        both consume the same operator instruction.
        """
        want = (str(tool or "").strip().lower(), str(mode or "").strip().lower(), str(target or "").strip())
        if not all(want):
            return None
        consumed: dict[str, str] | None = None
        with self.session(project_root) as conn:
            if not conn.in_transaction:
                conn.execute("BEGIN IMMEDIATE")
            row = conn.execute(
                "SELECT user_intent_actions, current_turn_id FROM session_query_gate "
                "WHERE session_id = ?",
                (session_id,),
            ).fetchone()
            if row is None:
                return None
            raw = row["user_intent_actions"] or "[]"
            turn = str(row["current_turn_id"] or "")
            try:
                entries = self._normalize_actions(json.loads(raw))
            except (ValueError, TypeError):
                return None
            if not turn:
                return None
            for index, entry in enumerate(entries):
                if (
                    (entry["tool"], entry["mode"], entry["target"]) == want
                    and entry.get("turn_id") == turn
                    # PROVENANCE AT THE SINK (r0b2 blocker 3): the writer
                    # census is not the boundary. Only a grant minted from
                    # operator text at UserPromptSubmit is consent; a
                    # structurally valid entry with any other (or no)
                    # origin/provenance is refused and stays unspent.
                    and entry.get("origin") == self._ACTION_ORIGIN
                    and entry.get("provenance") == self._ACTION_PROVENANCE
                ):
                    consumed = entry
                    remaining = entries[:index] + entries[index + 1 :]
                    cur = conn.execute(
                        "UPDATE session_query_gate SET user_intent_actions = ? "
                        "WHERE session_id = ? AND user_intent_actions = ?",
                        (json.dumps(remaining), session_id, raw),
                    )
                    if cur.rowcount != 1:
                        consumed = None
                    break
        if consumed is not None:
            self._audit_state_change(
                project_root,
                session_id,
                event_kind="session.user_intent_action_consumed",
                payload={
                    "tool": consumed["tool"],
                    "mode": consumed["mode"],
                    "target": consumed["target"],
                    "turn_id": consumed.get("turn_id", ""),
                },
            )
        return consumed
