...
def _check_bash_rules(
    ...
    *,
    ...
    # ── Shell-mediated file writes (king directive 2026-05-17) ──
    ...
    if re.search(
        r"(?:^|[\s'\"=><|])"
        r"(?:[A-Za-z]:[/\\])?"
        r"mcp[/\\]server[/\\]aidocs_mcp[/\\][^\s'\"<>|]+",
        command,
    ) and re.search(
        r">>?\s+[^\s|&;<>()$]+|\btee\s+|\bcp\s+|\bmv\s+"
        r"|\bopen\s*\([^)]*['\"][^'\"]*\.py['\"][^)]*['\"][wax+]"
    ...
    if re.search(
        r"\b(systemctl\s+(stop|disable|mask)|service\s+\w+\s+stop|sc\s+(stop|delete))\b",
        lower,
    ):
        verdicts.append(
            RuleVerdict(
...
def _check_egress_allowlist(
    ...
    *,
    ...
    try:
        ...
        raw = []
    allowlist = [str(x) for x in (raw or []) if str(x).strip()]
...
def _check_dangerous_rules(
    ...
    *,
    ...
    for rule in rules:
        ...
        elif cmd_match in cmd_lower and (not args_match or args_match in cmd_lower):
            verdicts.append(
                RuleVerdict(
