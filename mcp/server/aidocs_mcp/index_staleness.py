...
#   - schema_query: its own SchemaIndexStore (schema_entities) -> SCHEMA axis only.
...
def code_staleness_keys(project_root: Path) -> dict[str, Any]:
    ...
    if fw.get("state") == "unknown":
        return {
            ...
            "_index_hint": (
                "Index freshness not yet established (the sitter has not "
                "reconciled this project). Call ai_index_sync if you need a "
