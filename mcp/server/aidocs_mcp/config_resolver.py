...
def _read_layer_rows(
    ...
    if not db_path.is_file():
        return []
    try:
        # read_only=True (#755): the canonical connect, and the READ property
        # this function already documents is now ENFORCED BY SQLITE rather
        # than merely intended -- a reader cannot create the file, the table
        # or the WAL header. row_factory=False keeps plain tuples, which is
        # what this function's `list[tuple]` contract and its callers' index
        # access expect.
        conn = _canonical_connect(db_path, read_only=True, row_factory=False)
        try:
            return conn.execute(
...
class LayeredConfigResolver:
    ...
    def resolve(
        ...
        *,
        ...
        # Global layer — install-wide DB. `project_root` is threaded in so a
        # tenant project resolves the ORG's global store even with no request
        # bound (#497): otherwise every non-request read (index sitter,
        # watchdogs) silently read the host daemon's store instead, and a
        # dashboard toggle never reached the code that applies it.
        ...
        global_value, global_origin, global_replace = self._db_layer(
            ...
            scope_filter="global",
            scope_key="",
            cached_rows=_cached(global_db, "global", ""),
        )
    ...
    def _factory_layer(
        ...
        1. `_DEFAULT_CONFIG` walked via dotted path. Captures nested
           namespaces (e.g. the canonical `bash` policy).
        ...
        *,
        ...
        if cached_rows is not _ROWS_UNSET:
            ...
            try:
                ...
                # read_only=True (#755) makes that paragraph a GUARANTEE instead
                # of a promise: sqlite itself now refuses the create. row_factory
                # =False keeps the plain tuples the unpacking below reads.
                conn = _canonical_connect(db_path, read_only=True, row_factory=False)
                try:
                    rows = conn.execute(
    ...
    def _merge_layer(
        ...
        Per Empire's decree: dicts deep-merge, lists REPLACE per leaf.
        """
    ...
    def _deep_merge(base: dict, overlay: dict) -> dict:
        """Recursive dict merge. Lists at leaves REPLACE per Empire's decree.
