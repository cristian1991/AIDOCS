...
_NOT_PROJECT_TOOLS_ROUTABLE = _NOT_AGENT_ADVERTISED - _ORDINARY_DASHBOARD_READS_KEPT_ROUTABLE
...
def _catalog_row(
    *,
    ...
    # Staged binding law: out-of-stage tools are HIDDEN (not merely
    # scope-blocked) on every listing surface AND not executable — the
    # visibility law and the invocation law read the same stage_gate().
    if not stage_ok:
        advertise = False
        visible = False
        _in_catalog = False
...
def _resolve_run_row(name, out, *, principal, project, scope) -> dict:
    """CLASS_RUN branch of resolve() (#413 extraction — behavior unchanged).
    ...
    NO registry relay here — Empire ruling 2026-06-20 (tool_interface.py
    ai_run carve-out + tool_surface_waivers permanent entries): the
    shell trio is structurally sealed via RUN_ALLOWLIST, which never
    unions the registry. A registry row for a RUN tool must stay inert
    (classify()'s RUN_ALLOWLIST floor guarantees it can't re-route the
    class either) — pinned by test_ssot_selector_run_registry_relay.
    """
