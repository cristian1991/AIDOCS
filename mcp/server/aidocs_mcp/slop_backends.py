...
def _pip_hint(package: str, extra: str = "") -> str:
    """An install hint that names the interpreter that will be ASKED.

    A bare "pip install vulture" is an unreachable remedy here (law
    311bf3e6). Detection runs via importlib INSIDE THE DAEMON, and the
    daemon does not run from the project venv -- ai_version reports
    source_root C:\\Users\\User\\.aidocs\\runtime\\venv\\Lib. Measured
    2026-08-26: vulture and semgrep are both importable in mcp/.venv and
    both report available=false here, because those are two different
    environments. Following the bare hint installs into the wrong one,
    changes nothing, and looks like the tool lying.

    Naming sys.executable makes the hint land where the check reads.
    """
    suffix = f"  (or {extra})" if extra else ""
    return f"{sys.executable} -m pip install {package}{suffix}"
...
def backend_status() -> dict[str, Any]:
    ...
    return {
        ...
        "vulture": {
            ...
            "source": "vulture",
            "needs": _pip_hint("vulture", "aidocs-mcp[slop]"),
        ...
        "semgrep": {
            ...
            "source": "semgrep",
            "needs": _pip_hint("semgrep", "CI/Linux lane"),
...
def _is_trivial_body(fn: ast.AST) -> bool:
    """True when the function body (docstring excluded) is a single statement.

    One-statement bodies (path-property accessors, SSOT delegate stubs) match
    each other structurally by construction; reporting them as clones is a
    false positive class, not a refactor candidate.
    """
    ...
    *,
    ...
    for rel, text in files:
        ...
        for fn in ast.walk(tree):
            ...
            if _is_trivial_body(fn):
                # single-statement bodies (property accessors, delegate stubs)
                # share their AST shape by construction — boilerplate, not a
                # copy-paste clone. Suppressed per the no-blanket-allow rule:
                # there is nothing extractable in a one-statement body.
                continue
    ...
    return {
        ...
        "evidence": {
            ...
            f"ignores blocks < {min_nodes} AST nodes and single-"
            "statement bodies (accessor/delegate boilerplate); Python only",
            "confidence_basis": "block size x distinct files",
        },
    }


# Decorators that REGISTER a function with a framework (MCP tool decls, web
# routes, CLI commands): the decoration IS the reference, so flagging the
# function as unused is a structural false positive in any project.
...
def run_vulture(
    ...
    *,
    ...
    ignore_decorators: tuple[str, ...] = REGISTRATION_DECORATORS,
    runner=None,
) -> dict[str, Any]:
    ...
    if importlib.util.find_spec("vulture") is None:
        return {
            ...
            "source": "vulture",
            "findings": [],
            "install": _pip_hint("vulture", "aidocs-mcp[slop]"),
            "evidence": {
                "kind": "unavailable",
                "source": "vulture",
    ...
    if runner is not None:
        ...
        if ignore_decorators:
            argv += ["--ignore-decorators", ",".join(ignore_decorators)]
        try:
        ...
        try:
            ...
            v = _vulture.Vulture(verbose=False, ignore_decorators=list(ignore_decorators))
...
def _default_runner(argv: list[str]) -> tuple[int, str, str]:
    ...
    p = audited_run(
        argv,
        fingerprint=("slop_backends.py", "_default_runner", "subprocess.run"),
        reason="slop-backend-runner",
