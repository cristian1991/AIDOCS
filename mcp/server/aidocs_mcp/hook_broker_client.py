...
def _read_actionable_backlog_counts(
    ...
    # A row folded into an umbrella is NOT separately actionable -- the umbrella
    # carries it. Only `status='open'` rows are rewritten to 'merged' by the
    # merge path, so every merged `in_progress`/`blocked` row kept inflating
    # this banner forever: the operator merged ~150 items and the count never
    # moved, which is how a ledger stops being believed. Mirrors the canonical
    # predicate in `project_backlog_store.list_backlog`
    # (`merged_into IS NULL AND status != 'merged'`), guarded on column
    # presence because this reader is deliberately tolerant of older schemas.
    merged_clause = " AND merged_into IS NULL" if "merged_into" in columns else ""
    try:
        rows = conn.execute(
            "SELECT priority, COUNT(*) FROM project_backlog "
            f"WHERE status IN ({placeholders}){merged_clause} GROUP BY priority",
