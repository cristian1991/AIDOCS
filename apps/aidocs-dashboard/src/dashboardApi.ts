...
async function loadDashboardFromGate(projectId?: string): Promise<DashboardSnapshot> {
  ...
  // BOUND project (it takes no project arg), so without this the snapshot is empty and
  // the dashboard renders blank. project_select is two-phase-confirm; the dropdown
  // selection IS the user's explicit confirmation, so we pass the deterministic phrase
  // directly. Best-effort: a bind failure still attempts the snapshot.
  if (projectId) {
    const confirmPhrase = "confirm-project-" + "select " + projectId;
    try {
      await tauriFetch(WEBMCP_GATE_MCP, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          params: {
            name: "project_select",
            arguments: { project_id: projectId, confirm_token: confirmPhrase },
          },
        }),
      });
    } catch {
      /* fall through to the snapshot; surfaces as the empty/again-blank state */
    }
  }
...
// ── Ref-integrity (broken references) — king goal 2026-06-20 ──
