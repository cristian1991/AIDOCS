...
class ConductorVerificationService:
    ...
    def _run_command(
        ...
        try:
            ...
            # (including shell=False) pass through UNCHANGED.
            from .shell_egress_service import audited_run

            proc = audited_run(
                [resolved.path, "-c", command],
                fingerprint=("conductor_verification_service.py", "_run_command", "subprocess.run"),
                reason="conductor-verification-exec",
                run=lambda *a, **kw: subprocess.run(*a, **kw),
                shell=False,
                cwd=str(project_root),
                capture_output=True,
                text=True,
                encoding="utf-8",  # #684: else the ANSI codepage mojibakes UTF-8
                errors="replace",
                timeout=timeout,
