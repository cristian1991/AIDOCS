...
REASON_RECEIVE_ONLY = "freeze_receive_only"
...
#:   xaacp_ack           — DRT-12's non-terminal receipt. It is `must_act`
#:                         ENFORCED-WORKFLOW traffic, not a report: it transfers
#:                         ownership of an obligation and fires exactly once, so
#:                         it belongs with xaacp_reply on the gated side rather
#:                         than mixed into informational notification traffic.
#:                         A frozen lane still has send / xaacp_send / ai_issues
#:                         to say "received".
...
#:   wait_next           — NOT a report, so it is not in THIS table. It is
#:                         freeze-reachable through ``_RECEIVE_ONLY_TOOLS``
#:                         below (#1061), a separate, narrower class.
#:
...
def freeze_remedy_reachable(tool_name: str, tool_input: object = None) -> str:
    ...
    granted = report_mode_grant(tool_name, tool_input)
    if granted:
        return granted
    return receive_only_grant(tool_name, tool_input)


#: #1061 — RECEIVE-ONLY reachability (operator ruling 2026-09-14; r0b pinned
#: "frozen actors stay receive-reachable while work stays frozen").
#:
#: A seated Consul's Stop now REFUSES THE SEAL until the actor parks in
#: ``wait_next``. Under a freeze that order was unperformable — the same
#: deadlock shape as the 2026-07-17 stewardship incident — and an unparked
#: frozen actor cannot hear the operator who must thaw it.
#:
#: WHY wait_next QUALIFIES. It is a blocking READ of the caller's OWN stream:
#: it claims delivery of events addressed to the caller (a CAS on its own
#: cursor), arms/settles the caller's OWN receive lease, and returns. It does
#: not decide, acknowledge, cancel, send or mark read; it drives nobody else's
#: control flow. Identity is still the caller's canonical route, and the
#: middleware still audits the call.
#:
#: WHY IT IS NOT IN ``_REPORT_MODE_TOOLS``. That table is also consulted by the
#: reconnect gate; parking a reconnect-flagged caller is a different question
#: and is deliberately left unchanged. This table feeds ONLY the freeze gate.
#:
#: STILL FROZEN (pinned by tests/security/test_ai_msg_block_reporting_640.py):
#: xaacp_reply, xaacp_ack, xaacp_cancel, xaacp_wait — and every work tool.
_RECEIVE_ONLY_TOOLS: dict[str, frozenset[str]] = {
    "ai_msg": frozenset({"wait_next"}),
}


def receive_only_grant(tool_name: str, tool_input: object = None) -> str:
    """#1061 — ``REASON_RECEIVE_ONLY`` for a legible receive-only mode, else "".

    Same fail-closed shape as ``report_mode_grant``: tool NAME first, a dict
    payload with a listed mode, nothing else.
    """
    bare = _bare(tool_name)
    modes = _RECEIVE_ONLY_TOOLS.get(bare) if bare else None
    if not modes or not isinstance(tool_input, dict):
        return ""
    mode = str(tool_input.get("mode") or "").strip().lower()
    return REASON_RECEIVE_ONLY if mode in modes else ""
