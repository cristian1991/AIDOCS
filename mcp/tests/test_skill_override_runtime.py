...
def test_runtime_uses_aidocs_native_override_for_writing_plans(tmp_path: Path) -> None:
    ...
    runtime.hub.skills.set_selected_skills(project_root, session_id, ["superpowers_external/writing-plans"])

    result = runtime.skill_trigger_state(project_root, session_id, intent="planning")

    assert result["active_skills"][0] == "writing-plans"
...
def test_runtime_leaves_systematic_debugging_provider_native(tmp_path: Path) -> None:
    runtime, project_root, session_id = _make_runtime(tmp_path)
    _register_superpowers_provider(runtime, project_root, tmp_path)
    runtime.hub.skills.set_selected_skills(project_root, session_id, ["superpowers_external/systematic-debugging"])
