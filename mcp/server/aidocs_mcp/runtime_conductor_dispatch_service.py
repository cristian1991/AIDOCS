...
def build_adhoc_task_packet(
    ...
    """Build a packet for a lane its SPAWNER created, with no plan entry (#779).

    OPERATOR RULING 2026-08-17: "lanes are subagents, the rank-more-than-0
    agents, and they should be plan-able or via direct instruction from it's
    spawner". A subagent is something an agent spawns AND instructs; requiring a
    plan entry first put the plan document, not the spawner, in charge of who may
    exist.

    Deliberately a MODULE-LEVEL PURE FUNCTION, not a method. The plan path needs
    the session (context sections, predecessor lanes, the code index); an ad-hoc
    lane is defined entirely by its call, so it needs none of that. Keeping it
    dependency-free is what makes the scope rule below testable in isolation
    rather than only through a live dispatch.

    SCOPE DEFAULTS CLOSED, and this is the load-bearing line:
    `allowed_files` comes ONLY from the caller's argument. There is no fallback
    to the plan's scope, no inheritance from the spawner, and NOTHING is read out
    of `brief`. #774 settled that prose never widens scope -- a brief that could
    grant files would be privilege escalation by prose -- and an ad-hoc lane is
    where that would be easiest to slip in, since no plan constrains it. So an
    unscoped ad-hoc lane can touch nothing at all, which is the honest default
    for a boundary: a throwaway probe must not silently inherit the write access
    of real work.
    """
    files = [str(item).replace("\\", "/") for item in (allowed_files or []) if str(item).strip()]
    tools = [str(item).strip() for item in (allowed_tools or []) if str(item).strip()]
    verification_command = str(verification or "").strip()
    scope_constraints = [
        "Stay within allowed_files unless the conductor explicitly expands scope.",
    ]
    if not files:
        # Say it out loud. A worker that does not know it has no write scope
        # will argue with the edit gate instead of working inside the boundary,
        # which is how a correctly-fenced lane still burns its whole run.
        scope_constraints.append(
            "This lane has NO file scope: it must not create, edit or delete any "
            "file. If the work genuinely needs one, report that instead of "
            "attempting it -- the spawner grants scope, never the brief.",
        )
    packet = SubagentTaskPacket(
        ...
        goal=goal,
        allowed_files=files,
        required_reads=list(files),
        required_symbols=[],
        constraints=[
            *(constraints or []),
            *scope_constraints,
            (
                "Do NOT use raw bash/shell commands. Use AIDOCS MCP tools instead: "
                "ai_run (tests/builds/shell; detached, completion notify is universal "
                "-- read via ai_run_output), ai_get_lines, ai_replace."
            ),
        ],
        verification_commands=[verification_command] if verification_command else [],
        lane_allowed_tools=tools,
        done_definition=[goal] if str(goal or "").strip() else [lane_id],
        must_not=[
        ...
        output_schema={
            ...
            "notes": (
                "Return structured execution evidence so the conductor can "
                "reconcile lane progress deterministically."
            ),
        },
        predecessor_context=[],
    )
    data = packet.to_dict()
    # MARKED, not inferred. Downstream (roster, audit, completion review) must be
    # able to tell a spawner-owned lane from a plan lane without guessing from
    # the absence of a plan entry.
    data["adhoc"] = True
    # Mirrors _apply_operator_brief (#774): instructions ride on the packet, and
    # only instructions. Note what is NOT here -- the brief is never consulted
    # for files or tools.
    brief_text = str(brief or "").strip()
    if brief_text:
        data["operator_brief"] = brief_text
    return data
