...
class CodeIndexHotspotService:
    ...
    def find_domain_clusters(
        ...
        # #1039 A+ 5: a multi-term concept collects PER TERM and ranks
        # AND-first — "draft cleanup" must put StaleDraftCleanupJob (both
        # terms, and its summary says "removes abandoned empty draft
        # documents") above Document (one term). Single-term queries keep
        # the historical whole-needle path untouched.
        from .concept_rank import and_rank, query_terms

        terms = query_terms(needle)
        if len(terms) >= 2:
            try:
                concept_rows = self.store.search_concept(project_root, needle, limit=limit * 2)
            except Exception:
                concept_rows = {"results": []}
            for item in list(concept_rows.get("results") or []):
                key = (str(item.get("path")), str(item.get("symbol")) if item.get("symbol") else None)
                if key in seen:
                    continue
                seen.add(key)
                cluster.append(
                    {
                        "source": "symbol" if item.get("facet") == "symbol" else "file",
                        "path": item.get("path"),
                        "layer": self.store._infer_layer_from_path(str(item.get("path"))),
            ...
            try:
                with self.store.connect(project_root) as conn:
                    for row in conn.execute("SELECT path, summary FROM code_files"):
                        summaries[str(row["path"])] = str(row["summary"] or "")
            except Exception:
                summaries = {}
            cluster = and_rank(
                cluster,
                ...
                lambda r: (
                    ...
                    str(r.get("path") or ""),
                    summaries.get(str(r.get("path") or ""), ""),
                ),
                drop_unmatched=False,
            )
            cluster.sort(
                key=lambda item: (
                    -int(item.get("terms_matched") or 0),
                    # the row whose own NAME carries the terms first
                    -int(item.get("identity_terms") or 0),
                    # a NAMED thing before the file that merely holds it
                    0 if item.get("symbol") else 1,
                    self.store._layer_rank(str(item["layer"])),
                    str(item["path"]),
                    str(item["symbol"] or ""),
                ),
            )
            return {"cluster": cluster[:limit], "terms": terms}
