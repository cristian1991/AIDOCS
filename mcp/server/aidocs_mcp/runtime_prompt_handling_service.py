...
class RuntimePromptHandlingService:
    ...
    def _surface_for_prompt(
        ...
        UserPromptSubmit hook would have injected via additionalContext. The hook was
        the ONLY caller of surface_on_prompt, so without this the whole surfacing layer
        is dark off-hook. Best-effort; never fails prompt handling.
        """
        try:
    ...
    def aidocs_handle_prompt(
        ...
        # #58/#69: tool-driven surfacing so a hookless context gets the NLP tool +
        # memory hints the UserPromptSubmit hook would have surfaced.
        surfaced = self._surface_for_prompt(project_root, user_request, action_kind)
