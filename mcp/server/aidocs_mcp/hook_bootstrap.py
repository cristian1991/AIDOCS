...
rail gate_health rides), this module verifies the host hook wiring and:
  ...
  * HEALS it when broken — via `claude_hooks_install.ensure_claude_hooks`,
    the SAME canonical installer `aidocs setup` uses (Article VI: one
...
def _verify(path: Path) -> list[str]:
    """Full verify: read + parse + judge every canonical event. Returns the
    list of DEFECTS ([] == healthy). Unreadable/unparseable counts as
    everything missing — absence of a check must never read as a pass.

    PRESENCE WAS NEVER THE WHOLE QUESTION (#925k). An entry whose launcher sits
    in `runtime/generations/<id>` passes an events-only probe and is completely
    dead the moment GC collects that generation: the host cannot start the
    process, so nothing runs to notice. The launcher's LIFETIME is therefore
    part of health, and a generation-pinned event is reported as a defect so
    the same heal path that repairs a missing event converges it.
    """
    ...
    try:
        ...
        try:
            ...
            return [ev for ev, *_ in CANONICAL_HOOKS]
    from .claude_hooks_install import host_entry_defects

    defects = missing_hook_events(cfg)
    # ONE LAW, not a second opinion (#1047): the same judgment
    # `claude_hooks_status` reports and the passive repair acts on, so a box
    # cannot be healthy on one surface and defective on another.
    defects.extend(
        f"{d['event']} ({d['role']})"
        for d in host_entry_defects(cfg, home=path.parent.parent)
        if d["event"] not in defects
    )
    return defects
...
def _healed_notice(missing: list[str], result: dict) -> str:
    return (
        "🔧 HOST HOOK BOOTSTRAP: the AIDOCS enforcement hooks were broken "
        f"in {result.get('path', '~/.claude/settings.json')} "
        f"(defects: {', '.join(missing)}) — re-installed them now "
