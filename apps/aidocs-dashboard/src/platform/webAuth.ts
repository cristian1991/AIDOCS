...
export async function beginLogin(): Promise<void> {
  if (!isWebBuild()) {
    ...
    // TODO(#92, tranche 2): silent-renewal seam — when loadGateConnection()
    // finds a near-expired token WITH a refreshToken, run a
    // grant_type=refresh_token exchange here instead of a fresh browser OAuth.
    return;
  }
  const v = rand();
  const st = rand();
