...
def _global_db_path() -> Path:
    ...
    Precedence (fail-safe, request-scoped first):
      ...
      2. `AIDOCS_GLOBAL_CONFIG_DB` env var — test isolation from the real home;
      3. `~/.aidocs/config.sqlite3` — the single-tenant / local default.
    """
    import os
...
class ConfigStore:
    ...
    def project_db_path(self, project_root: Path) -> Path:
        return _project_db_path(project_root)
    ...
    def _connect_for_scope(self, project_root: Path, scope: str) -> sqlite3.Connection:
        if scope in _GLOBAL_SCOPES:
            ...
            return self._connect(_global_db_path())
    ...
    def _scan_immediate_children(
        self,
        project_root: Path,
        parent_path: str,
        session_id: str | None,
    ) -> dict[str, Any]:
        """Return all descendant sub-key rows under ``parent_path`` as
        a nested dict. Recursive: ``parent.x.y`` becomes ``{x: {y: …}}``
        so a query at ``parent`` sees the full sub-tree built from
        sibling rows at any depth. Required for cases where the
        operator queries a top-level namespace (e.g. ``bash``) but
        sub-key writes target deeper paths (``bash.allow.opencode``).

        Cascade: global < project < session.

        Conflict policy: shallower writes processed first so leaves at
        outer levels are never overwritten by deeper materialization.
        When a deeper write would descend through a non-dict leaf
        already in the tree, the deeper write is silently dropped —
        the leaf wins.
        """
        if not parent_path:
            return {}
        prefix = parent_path + "."
        like = prefix + "%"
        scoped: dict[str, list[tuple[str, Any]]] = {
            "global": [],
            "project": [],
            "session": [],
        }
        proj_conn = self._connect(_project_db_path(project_root))
        try:
            rows = proj_conn.execute(
                "SELECT setting_path, scope, scope_key, value "
                "FROM config_settings WHERE setting_path LIKE ?",
                (like,),
            ).fetchall()
            for row in rows:
                sp = row["setting_path"]
                ...
                sc = row["scope"]
                if sc == "session":
                    if not session_id or row["scope_key"] != session_id:
                        continue
                    scoped["session"].append((sub, json.loads(row["value"])))
                elif sc == "project":
                    scoped["project"].append((sub, json.loads(row["value"])))
        finally:
            proj_conn.close()
        global_db = _global_db_path()
        if global_db.is_file():
            glob_conn = self._connect(global_db)
            try:
                rows = glob_conn.execute(
                    "SELECT setting_path, value FROM config_settings "
                    "WHERE setting_path LIKE ? AND scope = 'global' "
                    "AND scope_key = ''",
                    (like,),
                ).fetchall()
                for row in rows:
                    ...
                    if not sub:
                        continue
                    scoped["global"].append((sub, json.loads(row["value"])))
            finally:
                glob_conn.close()
        merged: dict[str, Any] = {}
        for scope_name in ("global", "project", "session"):
            for sub, value in sorted(scoped[scope_name], key=lambda pair: pair[0].count(".")):
                self._set_nested(merged, sub.split("."), value)
        return merged

    @staticmethod
    def _set_nested(target: dict[str, Any], parts: list[str], value: Any) -> None:
        """Walk ``parts`` into ``target``, creating intermediate dicts
        as needed, and set the leaf. Dropped silently when an
        intermediate path collides with a non-dict leaf — leaves win
        over deeper writes against them.
        """
        cursor = target
        for part in parts[:-1]:
    ...
    def effective_config(
        ...
        *,
        ...
        with self._lock:
            ...
            # Global rows come from the install-wide DB
            global_db = _global_db_path()
            if global_db.is_file():
