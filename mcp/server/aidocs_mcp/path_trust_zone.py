...
def _classify_resolved(
    ...
    *,
    ...
    """
    # 0. HOST CACHE + agent scratchpad — readable/indexable by AIDOCS tools, never
    # 0. Agent scratchpad — the agent's OWN session scratch space (the OS temp
    # dir's "claude/" subtree the harness MANDATES for temp files). On Windows it
    # sits under AppData\Local\Temp, which would otherwise trip the "appdata"
    # sensitive-subdir rule and freeze on the agent's own scratchpad (#266). It is
    # the agent's workspace (freely read/write), NOT host files — approve it.
    # Host cache (<home>/.claude, <home>/.codex) is deliberately NOT approved
    # here: host files must be INDEXED and read only THROUGH governed AIDOCS tools
    # (redaction + gate), never open to raw Read/Grep. That policy lives in the
    # host-cache read path, not this trust-zone classifier.
    try:
        import tempfile

        if _path_contains(Path(tempfile.gettempdir()).absolute() / "claude", resolved):
