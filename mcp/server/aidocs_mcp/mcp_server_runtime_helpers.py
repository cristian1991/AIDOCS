...
#: The CLEARED surface kind of the authenticated client making this request
#: (outer_gate_oauth.OAuthStore.cleared_surface_kind, a rendering of the
#: is_cleared_gate_only predicate). NOT the rebuildable
#: oauth_clients.surface_kind projection: that row is NOT the authority and must
#: never be wired into a decision — the canonical capability record is, and a
#: client with no canonical record is unknown however the projection reads.
#: Written ONLY by the gate transport, from server-side state — never from
#: anything the client sends or omits. Empty outside a gate request, which
#: readers must treat as "unknown", never as a grant.
_request_surface_kind: contextvars.ContextVar[str] = contextvars.ContextVar(
    "_request_surface_kind",
    default="",
)


#: The AUTHENTICATED client identity behind this request, and the auth home its
#: registration lives in. Carried so a durable gate-only clearance can be
#: RE-VALIDATED later against the live registration: a downgrade or a disable
#: must invalidate at once, never at TTL. Written only by the gate transport.
_request_client_binding: contextvars.ContextVar[tuple[str, str]] = contextvars.ContextVar(
    "_request_client_binding",
    default=("", ""),
)


def set_request_surface_kind(kind: str, client_id: str = "", auth_home: str = ""):
    binding_token = _request_client_binding.set((str(client_id or ""), str(auth_home or "")))
    return (_request_surface_kind.set(str(kind or "")), binding_token)


def current_request_client_binding() -> "tuple[str, str]":
    """(client_id, auth_home) for this request, or ('', '') outside one."""
    return _request_client_binding.get()


def reset_request_surface_kind(token) -> None:
    # NOTE: this module imports names FROM contextlib (ExitStack,
    # contextmanager), not the module itself — a bare `contextlib.suppress`
    # here raised NameError in the FINALLY of every web transport call.
    kind_token, binding_token = token if isinstance(token, tuple) else (token, None)
    for var, tok in ((_request_surface_kind, kind_token), (_request_client_binding, binding_token)):
        if tok is None:
            continue
        try:
            var.reset(tok)
        except Exception:  # noqa: BLE001 - a reset failure must not break the call
            pass


def current_request_surface_kind() -> str:
    """The authenticated client's registered surface kind, or '' when unknown."""
    return _request_surface_kind.get()
...
def with_gate_execution_scope(root: Path, principal: Any = _KEEP_PRINCIPAL):
    ...
    with ExitStack() as stack:
        ...
        # BRANCH-TRANSITION SHARED LEASE + INVOCATION ID for the gate's dispatch
        # sites. _registry_invoke_edit calls spec.fn directly and OuterGate.edit
        # calls outer_gate_edit.apply directly, so neither passes through
        # mcp_server's instrumented call_tool where the MCP boundary lease is
        # taken. This seam is the one all gate dispatch sites share.
        from .branch_transition_barrier import gate_dispatch_lease

        stack.enter_context(gate_dispatch_lease(Path(root)))
        yield
...
class ProjectBindUnresolvable(RuntimeError):
    """A host session's project bind may exist but cannot be honoured.

    8d84 phase 2, operator 2026-09-22: "an unresolvable binding means a
    refused send, never a fallback to the caller's own project." Raised by
    ``resolve_project_root()`` when the bind store cannot be read, or the
    bound root no longer exists, or it is no longer a commissioned AIDOCS
    project. Before this, all three silently fell through to cwd discovery,
    which in a checkout of AIDOCS is AIDOCS itself -- so a session bound to
    project X wrote into AIDOCS while ``ai_project`` still said bound=true.

    Deliberately NOT a NoAidocsProjectError: callers that catch that one to
    fall back to discovery would re-open exactly the hole this closes.
    """

    def __init__(self, host_session_id: str, bound_root: str, reason: str) -> None:
        self.host_session_id = host_session_id
        self.bound_root = bound_root
        self.reason = reason
        where = f" bound to {bound_root!r}" if bound_root else ""
        super().__init__(
            f"project bind for host session {host_session_id!r}{where} cannot be "
            f"resolved: {reason}. Refusing rather than falling back to another "
            "project. Remedy: ai_project(mode='unbind') to drop the bind, or "
            "ai_project(mode='connect', ...) to re-bind to a live project."
        )
...
def resolve_conductor_key(principal: dict | None = None) -> tuple[str, str]:
    ...
    this (Doctrine VI: one logic, one home).
...
def _resolve_session_project_bind() -> Path | None:
    ...
    THREE STATES, FAIL CLOSED (8d84 phase 2, operator 2026-09-22). This used
    to be "read-only + fail-open": any error returned None and resolution fell
    through to cwd discovery. Now:
      * UNBOUND -- no host session in the call chain, no row, or a row expired
        by idle TTL (expiry IS the documented un-bind): None, and discovery
        runs exactly as before. A session with no bind sees zero change.
      * BOUND -- a live row naming a directory that is itself a commissioned
        AIDOCS project: that root. Checked with ``_has_marker`` on the root
        ITSELF, not ``find_aidocs_project_root``, which walks upward and
        would answer a decommissioned folder nested in another project with
        the parent -- the wrong-project case this exists to stop.
      * UNRESOLVABLE -- the store could not be read, or the bound root is gone
        or no longer a project: ``ProjectBindUnresolvable``. Never a fallback.
    READ-ONLY (#1095, r0b2 130b63b1-aaa). This used to read through
    `resolve(refresh=True)`, which lazily DELETED an expired bind and UPDATEd a
    keepalive -- semantic writes riding every tool call, including every tool
    advertised readOnlyHint=true. It now reads through `evaluate()`, the one
    pure bind-state judgement: an expired bind is logically UNBOUND here and
    its row is left for an explicit path (connect / unbind / resolve) to clean
    up. The keepalive moved to the host lifecycle:
    `keep_project_bind_alive_for_call` (non-read-only call boundary) and the
    PreToolUse hook.
    """
    sid = current_calling_host_session_id()
    if not sid:
        return None
    try:
        from .session_project_bind_store import BIND_BOUND, SessionProjectBindStore

        state = SessionProjectBindStore().evaluate(sid)
    except Exception as exc:  # noqa: BLE001 -- ANY store failure is "cannot tell"
        raise ProjectBindUnresolvable(
            sid, "", f"the bind store could not be read ({type(exc).__name__}: {exc})"
        ) from exc
    if state.status != BIND_BOUND:
        return None
    bound = state.project_root
    root = Path(bound)
    if not root.is_dir():
        raise ProjectBindUnresolvable(sid, str(bound), "the bound project root no longer exists")
    if not _has_marker(root):
        raise ProjectBindUnresolvable(
            sid, str(bound), "the bound root is no longer a commissioned AIDOCS project"
        )
    return root


def _tool_is_advertised_read_only(tool_name: str) -> bool:
    """THE call-boundary read-only test (#1095): the tool's registry spec
    advertises readOnlyHint=true. A tool with no spec is NOT read-only -- the
    conservative reading for the call-boundary writes gated on it, which only
    ever skip work for a read."""
    from . import tool_interface as _ti

    spec = _ti.REGISTRY.get(str(tool_name or ""))
    return spec is not None and (spec.annotations or {}).get("readOnlyHint") is True


def keep_project_bind_alive_for_call(tool_name: str) -> None:
    """Keepalive of the caller's LIVE project bind at the call boundary of a
    tool that is NOT advertised read-only (#1095). Never raises.

    The idle-TTL keepalive used to ride root resolution, so every read tool
    extended a routing lease. A read-only tool must not own that mutation. For
    hosts WITH hooks, the PreToolUse lifecycle event refreshes it for every
    call; this covers hosts without hooks for the calls that change something.
    """
    try:
        if _tool_is_advertised_read_only(tool_name):
            return
        sid = current_calling_host_session_id()
        if not sid:
            return
    ...
    except Exception:  # noqa: BLE001 -- a heartbeat must never fail a tool call
        return


def keep_xaacp_actor_alive_for_call(tool_name: str) -> None:
    """Refresh the caller's EXISTING XAACP actor liveness at the call boundary
    of a tool NOT advertised read-only (#1095 step 4a). Never raises, never
    registers.

    Seat occupancy reads the actor's `updated_at` as "last spoke". It used to
    ride every XAACP call (the dispatcher's UPSERT, reads included); it now
    rides the host lifecycle -- the PreToolUse hook for hosts with hooks, and
    THIS boundary for hosts without them (the gate runs registry tools through
    the same wrapper). Same rule and same classifier as the project-bind
    keepalive: a read-only tool never owns a liveness refresh.
    """
    try:
        if _tool_is_advertised_read_only(tool_name):
            return
        ...
        route = _cc.xaacp_resolve_caller_route(Path(root))
        actor = str(route.get("actor_id") or "").strip()
        if not actor or str(route.get("actor_kind") or "") == "worker":
            return
        _cc.xaacp_touch_actor_liveness(Path(root), actor)
    except Exception:  # noqa: BLE001 -- a heartbeat must never fail a tool call
        return


def record_project_for_call(
    tool_name: str,
    project_root: Path,
    *,
    managed_session_id: str | None,
) -> None:
    """Register the project a NON-read-only tool call acted on (#1095 step 2).
    Never raises.

    The known-projects registry is discovery state: the cross-project
    locator's local inventory, project_list, the doctor and the CLI read it,
    and `last_seen_at` orders the listing. Recording it on every call made
    every read tool a registration write. A tool advertised read-only now
    records nothing; the per-project `project_info` mirror follows the same
    gate.
    """
    try:
        if _tool_is_advertised_read_only(tool_name):
            return
    except Exception:  # noqa: BLE001 -- cannot tell: skip the write
        return
    try:
        from .project_registry_service import ProjectRegistryService

        ProjectRegistryService().record_project(
            project_root,
            managed_session_id=managed_session_id,
            title=project_root.name,
        )
    except Exception:  # noqa: BLE001 -- registration must never fail a tool call
        pass
    try:
        from .project_info_store import ProjectInfoStore

        info = ProjectInfoStore()
        info.init_db(project_root)
        info.record(project_root, title=project_root.name)
    except Exception:  # noqa: BLE001 -- project_info is observability
        pass
...
class _UnprovenSlotOwner(BaseException):
    """The slot lookup resolved to the #1043 refusal, not to an actor.

    A BASEEXCEPTION ON PURPOSE. The slot-resolution block it is raised in is
    wrapped in `except Exception: _own_slot_closed = False` — a best-effort
    handler whose "safe" default is to fall through to the SHARED SESSION SLOT.
    Raised as an Exception this refusal would be caught by that handler and
    silently converted into the conductor's task: the #1042b defect, rebuilt out
    of error handling instead of out of a return value.

    Not an error condition and never escapes the function — it is a control
    signal carrying the one meaning a `(str, str, bool)` triple cannot express:
    no own slot AND no fall-through.
    """
...
def require_active_task(
    ...
    *,
    ...
    if env in ("1", "true", "yes", "on"):
        return None
    # THE REFUSED IDENTITY STATE IS A HARD STOP (#1043).
    #
    # Worker/subagent evidence is PROVEN and the exact actor axis is ABSENT, so
    # we know this caller is a subagent and cannot say WHICH. Isolating it into
    # its own slot (which the distinguished actor id already achieves) stops it
    # touching the conductor's task, but on its own it would still let it
    # task_begin and mutate under an identity nobody can attribute — and
    # attribution is what this gate exists for.
    #
    # Placed AFTER the exemptions and DEV_MODE so lifecycle/bootstrap tools and
    # the harness keep working, and BEFORE the session read so it cannot be
    # reached by the permissive "no session → allow" path below.
    try:
        from .task_actor_identity import identity_is_unproven, stable_actor_id

        if identity_is_unproven(stable_actor_id(project_root)):
            _raise_tool_error(
                "this call carries worker/subagent evidence but no agent identity, "
                "so its work cannot be attributed to any actor "
                "[rule_id=unproven_subagent_identity]. The spawn path stamps the "
                "agent id; a subagent that lost it must be re-spawned rather than "
                "run unattributed. (A caller with NO worker evidence is unaffected "
                "and keeps the legacy session-slot contract.)"
            )
    except ImportError:  # pragma: no cover — identity module always present
        pass
    ...
    try:
        ...
        try:
            from .task_actor_identity import identity_is_unproven, resolve_slot_actor
            from .todo_state_store import ActorTaskStateStore

            actor_id, lane_id, _is_worker = resolve_slot_actor(project_root)
            # THE REFUSAL IS NOT A SLOT KEY (#1043b). `if actor_id` is a
            # TRUTHINESS test, and the refused state is a non-empty string — so
            # without this it reads as an actor and the store is queried under
            # it. Every unstamped subagent resolves to the SAME value, so that
            # row is a shared slot: one unattributable subagent would see
            # another's open task and could close it.
            #
            # The gate above already refuses this state before we get here, so
            # today this is unreachable. That is an ordering accident, not a
            # property: this block is also entered from the exemption paths, and
            # the next person to move a `return None` earlier re-opens it. The
            # value must be refused where it is USED, not only where it is made.
            # NOT a fall-through. Blanking the id alone would drop this caller
            # into the `not actor_id` legacy path, which reads the SHARED
            # SESSION SLOT — i.e. the conductor's live task. That is the #1042b
            # defect exactly. Refusing the fall-through (no own slot, no session
            # slot, hence no task) is the only answer that borrows nothing.
            if identity_is_unproven(actor_id):
                raise _UnprovenSlotOwner
            if actor_id:
        ...
        except _UnprovenSlotOwner:
            # No own slot AND no session fall-through: this caller resolves to
            # no task at all, which is what the refusal means. Deliberately
            # ahead of the broad handler below — a BaseException so a stray
            # `except Exception` can never turn this refusal back into consent.
            _own_slot_closed = True
        except Exception:
            _own_slot_closed = False
        task_id = (
    ...
    try:
        ...
        _expiry_hint = ""
    # #1039 S1 (secondary): a fresh no_active_task refusal should hand the
    # caller the remedy READY TO SEND — task_begin with the caller's OWN
    # previous goal prefilled — instead of a bare "call task_begin". The
    # goal lives on the caller's actor row (closed or not); the session
    # lifecycle snapshot carries only task_id/status. Best-effort.
    _goal_hint = ""
    try:
        from .task_actor_identity import identity_is_unproven, resolve_slot_actor
        from .todo_state_store import ActorTaskStateStore

        _g_actor, _g_lane, _ = resolve_slot_actor(project_root)
        _g_row = None
        # #1043b: the refusal must not key this read either. The hint is
        # cosmetic, but the row it reads is not — every unstamped subagent
        # resolves to the SAME value, so querying under it would quote one
        # unattributable caller's private goal text back to a different one.
        # A leak does not stop being a leak because it arrived in an error
        # message.
        if _g_actor and identity_is_unproven(_g_actor):
            _g_actor = ""
        if _g_actor:
            _g_row = ActorTaskStateStore().get(project_root, session_id, _g_actor, _g_lane)
            if _g_row is None and _g_lane:
                _g_row = ActorTaskStateStore().get(project_root, session_id, _g_actor, "")
        _g_goal = str((_g_row or {}).get("goal") or "").strip().replace("'", "\\'")
        if _g_goal:
            _goal_hint = (
                f" Retry with: ai_task(mode='begin', session_id='{session_id}', "
                f"goal='{_g_goal}') — your previous goal, prefilled; then re-send "
                f"the refused {tool_name} call."
            )
    except Exception:
        _goal_hint = ""
    _raise_tool_error(
        f"no active task (call task_begin before {tool_name}) "
        f"[rule_id=no_active_task]{_expiry_hint}{_goal_hint}"
    )
