...
gates are not yet wired; wiring lands after Empire resolutions on:

    Q1: RESOLVED by #404 (2026-07-16): there is no emergency key —
        the kill-switch gate is excised from the pipeline entirely.
...
# Lane 2 phase 1 (2026-05-04): the controller is bridged to the legacy
# facade. Flip False once the controller's own gates are wired so the
# skeleton Decision(...) fallback below becomes reachable again.
_LEGACY_FACADE_ACTIVE = True
...
class EnforcementController:
    ...
    def enforce(self, request: Request) -> Decision:
        ...
        if _LEGACY_FACADE_ACTIVE:
            return enforce_via_legacy(request)
        # Skeleton fallback, kept until full migration prunes it:
        return Decision(
