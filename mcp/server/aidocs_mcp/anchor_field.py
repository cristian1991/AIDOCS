...
# foreign_keys=ON, none of which a raw sqlite3.connect() gets. row_factory
# is False at every site here: these rows are read positionally (r[0]) and
# the helper defaults True, which would hand back sqlite3.Row instead.
...
def _resolve_domains(
    ...
    counts: dict[str, int] = {}
    try:
        conn = _canonical_connect(str(empire_db_path), row_factory=False)
        try:
            for anchor in anchors:
                rows = conn.execute(
