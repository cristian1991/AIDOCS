...
const WEBMCP_GATE_MCP = "https://mcp.codenexus.cloud/v1/mcp";
...
export type DashboardManagedProject = {
  ...
  current: boolean;
  source?: string; // WebMCP scope only: gate project source (no session counts)
};
...
// WebMCP (cloud) scope: the dashboard snapshot is served by the gate's
...
async function loadDashboardFromGate(): Promise<DashboardSnapshot> {
  const conn = loadGateConnection();
  if (!conn?.accessToken) {
    throw new Error("Not connected to WebMCP — sign in again from the top bar.");
  }
  ...
  if (!res.ok) {
    throw new Error("WebMCP dashboard request failed (HTTP " + res.status + ")");
  }
  ...
  if (rpc.error) {
    throw new Error("WebMCP: " + (rpc.error.message || "tool error"));
  }
  const text = rpc.result?.content?.[0]?.text;
  if (!text) {
    throw new Error("WebMCP returned an empty dashboard snapshot.");
  }
  ...
  if (parsed._error) {
    throw new Error("WebMCP: " + parsed._error);
  }
  if (!parsed.snapshot) {
    throw new Error("WebMCP response had no snapshot.");
  }
...
export async function loadManagedProjects(
  ...
  if (getScope() === "web") {
    // WebMCP scope: the project list is the set the GATE serves, captured by
...
// Phoenix 2026-05-12: renamed from cerberus_* per king directive
...
// ── ai_backlog / ai_todo (Phoenix backlog-todo dashboard) ──
...
): Promise<{ ok: boolean; items: BacklogItem[] }> {
...
export async function backlogGet(
  backlogId: number,
  projectRoot?: string,
): Promise<{ ok: boolean; item: BacklogItem | null }> {
...
export async function backlogAdd(
  content: string,
  projectRoot?: string,
  opts?: { priority?: string; status?: string; tags?: string[]; sessionId?: string },
...
export async function backlogUpdate(
  backlogId: number,
  patch: { status?: string; priority?: string; content?: string; tags?: string[] },
  projectRoot?: string,
): Promise<{ ok: boolean; result: Record<string, unknown> }> {
...
export async function todoUpdate(
  todoId: number,
  patch: { status?: string; content?: string; tags?: string[] },
  projectRoot?: string,
): Promise<{ ok: boolean; result: Record<string, unknown> }> {
...
export async function deleteSession(
  ...
  return invoke("delete_session", { projectRoot, sessionId });
}
