...
def _read_dev_session_override(
    ...
    """Read dev_ai_run_bash_path from SessionQueryGateStore. Returns
    "" when unset, when distribution.flavor != "dev", when no
    session_id is provided, or on any read failure.

    Delegates to SessionQueryGateStore.get_dev_ai_run_bash_path which
    enforces the read-side flavor gate as defense in depth — even if
    the column was somehow populated on a non-dev install, the getter
    returns "" on non-dev flavors. We also short-circuit here when
    session_id is missing because the resolver is reachable from
    code paths that don't have a session bound.
    """
    if not session_id:
        return ""
    try:
        ...
        return SessionQueryGateStore().get_dev_ai_run_bash_path(
            project_root,
            session_id,
        )
    except Exception:
