...
6. If the target project is missing AIDOCS structure (the `/.MEMORY/.aidocs/index.aidocs` commissioning marker and the sqlite index under `/.MEMORY/.index/`, plus the managed `AGENTS.md` or `CLAUDE.md` root markers), bootstrap it. Durable memory is canonical in the sqlite `memory_index`; bootstrap seeds factory doctrine into it from the shipped project-memory seed DB and never scaffolds `.MEMORY/**/*.md` memory files.
