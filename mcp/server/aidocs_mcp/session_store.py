...
class SessionStore:
    ...
    def create_session(
        ...
        # Create session-scoped aidocs.toml for per-session config overrides
        session_config = base / "aidocs.toml"
        if not session_config.is_file():
            session_config.write_text("# Session-scoped AIDOCS config overrides\n# Settings here override project-level aidocs.toml for this session only.\n", encoding="utf-8")
