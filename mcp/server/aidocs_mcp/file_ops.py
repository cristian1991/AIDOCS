...
from .branch_transition_barrier import governed_project_mutation
...
# #1010 -- the literal head of the definition above; its presence identifies
# the signature-definition file regardless of which copy (venv or checkout).
_MOJIBAKE_DEFINITION_MARKER = "_MOJIBAKE_SIGNATURES: tuple[str, ...] = ("
...
def _detect_mojibake(text: str, path: str) -> str | None:
    ...
    # file is the one place these byte sequences are legitimate. Path fast-path:
    # the exact file on disk that is running.
    try:
        if Path(path).resolve() == Path(__file__).resolve():
            return None
    except (OSError, ValueError):
        pass
    # #1010 -- the daemon runs the INSTALLED artefact (venv site-packages), so
    # __file__ is the venv copy while edits target the CHECKOUT copy: the paths
    # never match and every edit to the checkout file_ops.py was refused on its
    # own doc comment. Recognise the definition file by CONTENT as well -- but
    # ONLY together with a path guard: basename file_ops.py directly under a
    # directory named aidocs_mcp (the checkout copy and the venv copy both
    # qualify, nothing else does). Content alone was a hole: any file merely
    # quoting the marker line (a comment, a test) skipped the whole screen.
    # Exempt files skip both the signature loop and the #830 round-trip check.
    if _MOJIBAKE_DEFINITION_MARKER in text and _is_mojibake_definition_path(path):
        return None
    ...
    # #830 -- THE LIST ABOVE IS AN ALLOWLIST OF WESTERN EUROPEAN LOWERCASE.
    #
    # It enumerates the mojibake of e-acute, e-grave, a-grave, a-circumflex,
    # o-circumflex, u-circumflex, c-cedilla, i-circumflex, i-diaeresis,
    # e-diaeresis, u-diaeresis, n-tilde -- and nothing else. So i-circumflex
    # and a-circumflex are REFUSED while a-breve (U+0103) and capital E-grave
    # (U+00C8) pass SILENTLY: no entry begins with the U+00C4 lead byte, and
    # only the LOWERCASE Latin-1 forms are listed. Every Latin Extended-A
    # diacritic (Romanian, Polish, Czech, Hungarian, Turkish) is invisible BY
    # CONSTRUCTION, while French is protected.
    #
    # MEASURED 2026-08-18 on shipped product text (DentalClinic-WebApp), where
    # two locale files were corrupted and only one alphabet tripped the guard.
    # The reporter's summary of why that pairing is the dangerous one: "it
    # fails silently for a-breve / E-grave and only trips the validator for
    # i-circumflex / a-circumflex".
    #
    # THE COMPLETE TEST ALREADY EXISTED 40 LINES BELOW, inside fix_mojibake:
    # encode cp1252, decode utf-8, and treat a clean round-trip that CHANGES
    # the text as proof. It needs no per-language list and is already trusted
    # enough to REPAIR with -- while the detector beside it used a partial
    # enumeration to REFUSE with. One question, two implementations, one of
    # them incomplete: the shape this codebase keeps paying for (#800 had its
    # two copies 130 lines apart inside a single function).
    #
    # PER LINE, not per file, and that is deliberate: encode("cp1252") raises
    # on the first character cp1252 cannot hold, so ONE emoji or CJK glyph
    # anywhere in a file would otherwise disable the whole check for that file
    # -- re-creating the same silent-for-some-alphabets hole one level up.
    for _n, _raw in enumerate(text.split("\n"), 1):
        _repaired = _round_trip_repair(_raw)
        if _repaired is None:
            continue
        return (
            f"Mojibake (double-encoded UTF-8) in {path} at line {_n}: "
            f"{_raw.strip()[:60]!r} decodes as {_repaired.strip()[:60]!r}. "
            f"UTF-8 bytes were read through cp1252 somewhere. Re-emit the "
            f"content as clean UTF-8 and verify BY CODEPOINT -- a Windows "
            f"console is cp1252 and renders both forms plausibly (#830)."
        )
    return None


def _is_mojibake_definition_path(path: str) -> bool:
    """True only for .../aidocs_mcp/file_ops.py (either separator style)."""
    try:
        p = PurePath(str(path).replace("\\", "/"))
    except (TypeError, ValueError):
        return False
    return p.name == "file_ops.py" and p.parent.name == "aidocs_mcp"


def _round_trip_repair(line: str) -> str | None:
    """The repaired text if `line` is double-encoded UTF-8, else None.

    Language-agnostic by construction: it asks whether these characters, read
    back as cp1252 bytes, form VALID UTF-8 that says something DIFFERENT.
    Legitimate text fails one of those two tests --
      * pure ASCII round-trips to itself (no change -> None);
      * a real e-acute encodes to one byte 0xE9, which is not valid standalone
        UTF-8, so the decode raises -> None;
      * emoji and CJK are not cp1252-encodable at all, so the encode raises
        -> None.
    Only genuinely double-encoded text both survives the round-trip AND
    changes, which is why this needs no alphabet list to keep up to date.
    """
    if line.isascii():
        return None
    try:
        repaired = line.encode("cp1252", errors="strict").decode("utf-8", errors="strict")
...
def fix_mojibake(text: str) -> str:
    """Repair double-encoded UTF-8 (the exact corruption _detect_mojibake flags)
    by reversing it: re-encode as cp1252, decode as UTF-8. SAFE for TEXT content
    (skills, docs) and embedded there — deliberately NOT wired into the general
    edit gate, which REFUSES rather than repairs (a silent byte-rewrite could
    corrupt security code). Guaranteed non-destructive: it repairs only when the
    round-trip removes ALL mojibake signatures without a failed decode; clean
    text and mixed content that can't round-trip cleanly pass through untouched,
    so it can only ever REMOVE mojibake, never introduce it.
    """
    if not text or not any(sig in text for sig in _MOJIBAKE_SIGNATURES):
        return text
    try:
        repaired = text.encode("cp1252", errors="strict").decode("utf-8", errors="strict")
    except (UnicodeEncodeError, UnicodeDecodeError):
        return text  # mixed/partial: the clean unicode isn't cp1252-encodable
    if "�" not in repaired and _detect_mojibake(repaired, "") is None:
        return repaired
    return text
...
# Documented general ceiling for get_lines. #481 (War KK): raised from the
# old silent ~200 cap; requests up to this are honored, larger requests are
# clamped WITH an in-band "showing N of M" budget sentinel (no-silent-caps
# law) carried via result["requested"] + result["truncated"].
MAX_LINE_COUNT = 600
# Raised ceiling for known single-symbol spans (ai_get_lines symbol= /
# whole-symbol reads): one symbol should be readable in one call even when
# it is a 1,300-line function (EE's outer_gate_transport dispatch war).
MAX_SYMBOL_SPAN_LINE_COUNT = 2000
...
def _is_blocked_system_dir(lower_str: str) -> bool:
    ...
    Carve-out (Empire 2026-06-20, WebMCP M-tier fix): a registered TENANT project root
    ...
    # A registered tenant project is the operator's EDITABLE workspace, exempt from
    # the broad /opt block. TWO layouts are recognized: the org-scoped
    #   .../gate-root/tenants/<org>/projects/<project>   (per-org isolated), and
    #   .../gate-root/projects/<project>                 (the flat layout the WebMCP
    # project_register_from_github_url handler ACTUALLY writes: projects_base =
    # Path(home)/"projects"; #253 2026-07-07). Both REQUIRE a <project> segment
    # directly under a gate-root projects dir, so gate-root itself, the tenants/
    # <org>/projects PARENT dirs, the custody tree (/opt/aidocs/custody/...), and
    # bare gate internals stay blocked. The whole check is /opt-scoped below, so a
    # spoofed /etc/gate-root/projects/... stays blocked.
    is_tenant = bool(
        re.search(r"/gate-root/tenants/[^/]+/projects/[^/]+", norm)
        or re.search(r"/gate-root/projects/[^/]+", norm)
    )
...
# Non-RUNTIME trees inside the AIDOCS source repo: tests, docs, and scratch.
# Writes here can never alter the running gate, so the whole-repo self-edit
# fence (dev-build-only) does not apply to them. Covered for BOTH bindings
# (outer …/AIDOCS root and the mcp/ subdir). Lowercase, '/'-normalized.
_SELF_EDIT_NON_RUNTIME_PREFIXES: tuple[str, ...] = (
    "mcp/tests/",
    "tests/",
    "mcp/docs/",
    "docs/",
...
def _is_aidocs_source_repo(project_root: Path) -> bool:
    ...
    self-edit authority (2026-06-13). On a released install dev_mode_authorized
...
def _validate_project_root(project_root: Path, *, write: bool = False) -> None:
    ...
    """
    # Doctrine 2026-05-29 (Empire triage, clean-VPS Gate 2b): detect
...
def _resolve_path(
    ...
    *,
    ...
    # Doctrine 2026-05-29 (Empire triage, clean-VPS Gate 2b): also
    ...
    if os.path.isabs(relative_path) or _is_win_abs:
        # Gate-authorized governed-external READ (2026-07-10): ai_read_raw already
        # passed the zoned read gate (sensitive-dir hard-block + content-secret
        # refusal + `..` traversal check), so an absolute external path is
        # authorized. `and not write` is defense-in-depth — writes NEVER get
        # external access, so the project-confinement stays absolute for mutations.
        if allow_external and not write:
    ...
    if write:
        ...
        # source repo unless dev_mode is on. Empire directive 2026-05-03:
        ...
        # Non-runtime carve-out (2026-07-17): tests/, docs/, and scratch/
        # in the AIDOCS source repo are NOT runtime code — the fence exists
        # to stop agents editing the tool they are RUNNING ON, and a worker
        # writing a new test/doc/scratch file cannot alter that runtime.
        # The running package (_SELF_DIR, first two clauses below) and every
        # other source path (apps/, core/, scripts/, ...) stay fail-closed.
        is_non_runtime_path = rel_lower_self.startswith(_SELF_EDIT_NON_RUNTIME_PREFIXES)
        protected_self = (
            ...
            or (
                in_aidocs_source
                and in_live_install
                and not is_memory_path
                and not is_non_runtime_path
            )
        )
        if protected_self:
            # Self-edit authority is ORDINARY authenticated ADMIN authority
            # (#404 excised the dev-flavour term; #434 fixed the plumbing):
            # editing AIDOCS source is permitted ONLY when project_root IS the
            # canonical AIDOCS source repo AND the caller holds authenticated
            # admin authority (operator token OR an approved host-session
            # binding, resolved through project_authority — fail-closed). A
            # bound admin (conductor or an authorized worker) may edit; an
            # unauthenticated caller cannot. On a non-source install, nobody.
            ...
            if not dev_mode_authorized(project_root):
                ...
                raise ValueError(
                    f"AIDOCS self-edit blocked: {relative_path}. Path is in "
                    f"{scope_msg}; editing AIDOCS source requires an authenticated "
                    f"ADMIN operator (dashboard login or an approved host-session "
                    f"binding). Sign in / bind this session, then retry.",
                )
...
def _strip_razor_comments_and_strings(text: str) -> str:
    ...
    # String + char literals last (after script/style bodies are wiped).
    # A regex can't model C# string forms — verbatim @"..." (where "" is an
    # escaped quote and \ is LITERAL) and raw """...""" (arbitrary quote
    # fence) — so a hand-written scanner owns the string boundaries. Getting
    # this wrong leaks a brace out of a string and corrupts the @{ } block
    # boundary the caller depth-matches on.
    _wipe_csharp_literals_in_place(rebuild, _wipe_range)

    return "".join(chars)


def _wipe_csharp_literals_in_place(text: str, wipe) -> None:  # type: ignore[no-untyped-def]
    r"""Blank C# string/char-literal spans in ``text`` via ``wipe(start, end)``.

    Handles regular ``"..."`` (``\`` escapes), verbatim ``@"..."`` (``""``
    doubles, ``\`` literal), raw fenced strings (3+ quotes), and ``'...'``
    char literals. Interpolated ``$"..."`` / ``$@"..."`` are treated by their
    quoting rules (the ``$`` only adds holes, which get blanked with the rest
    of the string). ``wipe`` preserves newlines, so line numbers survive.
    """
    n = len(text)
    i = 0
    while i < n:
        ...
        if ch == '"':
            ...
            if run >= 3:
                fence = '"' * run
                end = text.find(fence, k)
                stop = n if end == -1 else end + run
                wipe(i, stop)
                i = stop
                continue
            if run == 2:
                ...
                continue
            # run == 1
            verbatim = i - 1 >= 0 and text[i - 1] == "@"
            if not verbatim and i - 1 >= 0 and text[i - 1] == "$":
                verbatim = i - 2 >= 0 and text[i - 2] == "@"
            j = i + 1
            if verbatim:
                while j < n:
                    if text[j] == '"':
...
def _razor_brace_balance_walk(text: str) -> str | None:
    ...
    """
    return _walk_razor(text, 0, len(text), in_code_block=False)
...
_RAZOR_MARKUP_LINE_RE = re.compile(r"(?m)^[ \t]*<[ \t]*[A-Za-z/!]")
_RAZOR_LITERAL_LINE_RE = re.compile(r"(?m)^[ \t]*@:")
_RAZOR_TEXT_TAG_RE = re.compile(r"<[ \t]*/?[ \t]*text[ \t]*>", re.IGNORECASE)


def _razor_fragment_has_markup_transition(fragment: str) -> bool:
    """Does this @{ } body transition back into markup? (#811)

    A Razor code block is not pure C#. These are all legal inside one
    and are all rejected by every C# parser:

        @{ if (x) { <p>hi</p> } }     markup element on its own line
        @{ <text>literal</text> }     the Razor <text> escape
        @{ @: literal line }          the Razor @: escape

    When any of them is present the fragment is mixed Razor, so the C#
    parser is not entitled to a verdict on it and Layer 3 skips it.

    Deliberately anchored to LINE STARTS for the element form. A bare
    `<` is not enough — `a < b` and `List<string>` are ordinary C# and
    must keep being validated. Markup transitions inside a code block
    begin a line in every real-world Razor file; generic type arguments
    and comparisons do not. Erring toward "this is C#" keeps genuine
    syntax errors caught; the narrow miss (markup opened mid-line after
    C# code) degrades to the old behaviour for that one fragment only.
    """
    if _RAZOR_TEXT_TAG_RE.search(fragment):
        return True
    if _RAZOR_LITERAL_LINE_RE.search(fragment):
        return True
    return bool(_RAZOR_MARKUP_LINE_RE.search(fragment))
...
def _check_razor_syntax(text: str) -> str | None:
    ...
    Three layers:
      ...
      L2: whole-file verdict from the Razor-aware backend, reached via
          the extension hint (#811). Today that backend answers
          "razor validate not supported; outline-only" — ok, fail
          open. It is wired anyway so a real Razor verdict starts
          flowing the moment the backend can give one.
      L3: extract @{ ... } code blocks and validate each as a C#
          method body via csharp_edit_validator. Catches missing
          semicolons, bad expressions, etc. inside Razor code blocks.

    L3 IS NOT ALLOWED TO JUDGE MARKUP (#811). A Razor code block may
    legally transition back into markup — ``<p>x</p>``, ``<text>``,
    ``@:`` — and every C# parser rejects all three. Roslyn refusing
    ``@section Styles { <link /> }`` with CS1014 was not Roslyn being
    wrong; it was Roslyn being asked the wrong question, and the
    resulting refusal was whole-file and atomic, so ONE such construct
    made an entire file uneditable through every governed write path.
    Fragments containing a markup transition are therefore SKIPPED
    rather than refused: fail open on what this parser was never
    entitled to decide, keep judging what it was.
    ...
    # Layer 2 — whole-file verdict from the Razor-aware backend, via the
    # extension hint (#811). Reuses the same bridge the outline path
    # uses; no rival Razor detector. Today this is a documented no-op
    # (ok=True, parser names itself as outline-only), so it costs one
    # cheap round-trip and buys the wiring for the day it isn't.
    whole = validate_csharp_content(text, ext=".cshtml")
    if not whole.get("ok"):
        errs = whole.get("error_nodes") or []
        parser = whole.get("parser") or "unknown parser"
        if errs:
            first = errs[0]
            return (
                f"Edit would produce invalid Razor — {parser} reports "
                f"line {first.get('line', '?')}, col {first.get('column', '?')}: "
                f"{(first.get('text') or '').strip()[:120]}"
            )
        return f"Edit would produce invalid Razor — refused by {parser}"

    # Layer 3 — validate each @{ ... } block as a C# method body.
    blocks = _extract_razor_code_blocks(text)
    if not blocks:
        return None
    for line_num, fragment in blocks:
        # #811: a Razor code block may transition back into markup, and
        # a C# parser is not entitled to judge that. Skip — fail open —
        # rather than refuse the whole file for a construct that was
        # never C# in the first place.
        if _razor_fragment_has_markup_transition(fragment):
            continue
        ...
        result = validate_csharp_content(wrapped, ext=".cs")
        if not result.get("ok"):
            errs = result.get("error_nodes") or []
            parser = result.get("parser") or "unknown parser"
            if errs:
                first = errs[0]
            ...
            return (
                f"Edit would produce invalid Razor — C# error in @{{ }} block "
                f"(judged by {parser})"
            )
...
def _check_syntax(abs_path: Path, text: str) -> str | None:
    ...
    if suffix in {".js", ".cjs", ".mjs"}:
        ...
        try:
            ...
            proc = audited_run(
                ...
                fingerprint=("file_ops.py", "_check_syntax", "subprocess.run"),
                reason="js-syntax-check",
    ...
    # error-tolerant by spec and has no strict parse verdict to enforce.
        ...
        # #811: the language hint is explicit here. .cs IS C#, so this
        # path is unchanged in behaviour — a genuinely broken .cs is
        # still refused. The hint exists so the RAZOR branch below can
        # say something different, not so this one can say less.
        result = validate_csharp_content(text, ext=".cs")
        if not result.get("ok"):
            errs = result.get("error_nodes") or []
            parser = result.get("parser") or "unknown parser"
            if errs:
            ...
            return f"Edit would produce invalid C# (judged by {parser})"
...
def _validate_write(
    ...
    *,
    ...
    """The write-path validation contract: syntax, then the Ruff edit gate.

    ``before`` is the SAME snapshot ``after`` was built from ("" for a create),
    never a second disk read, so the Ruff gate compares exactly what this
    writer changed. Every writer routes through here; ``_check_syntax`` keeps
    its two-argument shape (tests and the switch barrier patch it by name).
    """
    syntax_err = _check_syntax(abs_path, after)
    if syntax_err:
        return syntax_err
    from .ruff_edit_gate import check_edit

    result = check_edit(project_root=project_root, target=abs_path, before=before, after=after)
    if result.refused:
        return result.refusal_message()
    return None


def _split_new_content_lines(new_content: str) -> list[str]:
    """Split replacement text into lines, byte-for-byte (#476b, War T/W).

    ``"foo\\n".split("\\n")`` yields ``["foo", ""]`` — the trailing newline
    became a spurious blank line on every line-mode edit whose content ended
    with ``\\n`` (the natural way agents ship line blocks). One trailing
    newline is the LINE TERMINATOR, not an extra empty line; strip exactly
    one before splitting. Empty string still means "delete the range".
    """
    if new_content == "":
        return []
    body = new_content[:-1] if new_content.endswith("\n") else new_content
    return body.split("\n")
...
def _suggest_near_miss_path(project_root: Path, clean_rel: str) -> str | None:
    """Given a project-relative path that does NOT exist, look for a unique
    near-miss: the same relative path one directory level down (monorepo
    prefix drop, e.g. 'server/x.py' -> 'mcp/server/x.py'). Returns the
    suggested project-relative path only when EXACTLY one top-level
    directory matches; ambiguous or zero matches return None. Suggestion
    only — callers must still error, never auto-open. (#314 fix 3)"""
    if not clean_rel:
        return None
    candidates: list[str] = []
    try:
    ...
    for d in entries:
        try:
            if (d / clean_rel).is_file():
                candidates.append(f"{d.name}/{clean_rel}")
        except OSError:
            continue
    if len(candidates) == 1:
        return candidates[0]
...
def get_lines(
    ...
    *,
    ...
    Args:
        ...
        count: Number of lines to read. Clamped to the ceiling (see max_count).
        show_line_numbers: If True, prefix each line with `N<TAB>` — everything
            after the FIRST tab is the source line byte-for-byte (#481 fidelity
            pin; the old two-space separator merged into space-indentation).
        max_count: Optional ceiling override for known single-symbol spans;
            clamped to MAX_SYMBOL_SPAN_LINE_COUNT. Default: MAX_LINE_COUNT.
    ...
    Returns:
        {
            ...
            "requested": int,        # Only when truncated: the count asked for
        }
    ...
    for _prefix in READ_DENYLIST_PREFIXES:
        if _clean_rel.startswith(_prefix):
            raise ValueError(
                ...
                f"Use dedicated tools (ai_git, mode_get, dashboard) instead.",
            )
    abs_path = _resolve_path(project_root, path)
    if not abs_path.is_file():
        # #314(3): near-miss hint. In a monorepo agents often drop the
        # top-level prefix ("server/x.py" for "mcp/server/x.py"). When
        # exactly ONE top-level dir contains the given relative path,
        # SUGGEST it — never auto-open a different file.
        clean_rel = path.replace("\\", "/").lstrip("/")
        hint = _suggest_near_miss_path(project_root, clean_rel)
        if hint:
            raise FileNotFoundError(
                f"File not found: {abs_path}. Did you mean {hint!r}? "
                f"That project path exists — retry with it "
                f"(not auto-opened).",
            )
    ...
    ceiling = MAX_LINE_COUNT
    if max_count is not None:
        ceiling = max(1, min(int(max_count), MAX_SYMBOL_SPAN_LINE_COUNT))
    count = max(1, min(count, ceiling))
    ...
    if show_line_numbers:
        # TAB separator (cat -n convention, #481 fidelity pin): the source
        # line is everything after the FIRST tab, byte-for-byte. The old
        # "NNN  line" form was ambiguous — the right-aligned digits + two
        # spaces merged into space-indentation and put tabs mid-prefix, so
        # prefix-stripping lost leading whitespace (GG/EE edit-exactness war).
        width = len(str(end))
...
def read_raw(
    ...
    *,
    ...
    """
    _validate_project_root(project_root)
    abs_path = _resolve_path(project_root, path, allow_external=allow_external)
    if not abs_path.is_file():
        return {
            "path": path,
            "total_size_bytes": 0,
...
@governed_project_mutation("file_ops.create_file")
def create_file(
    project_root: Path,
    path: str,
    content: str,
    *,
    ...
    # Ruff edit gate: a create has an EMPTY before, so any finding is new.
    syntax_err = _validate_write(project_root, abs_path, before="", after=content)
    if syntax_err:
        return {
            "success": False,
            "path": path,
...
@governed_project_mutation("file_ops.edit_lines")
def edit_lines(
    project_root: Path,
    path: str,
    start_line: int,
    ...
    *,
    ...
    old_content = "\n".join(old_lines)
    new_lines = _split_new_content_lines(new_content)
    ...
    updated_text = _render_lines(result_lines, final_newline=final_newline)
    # BEFORE = the original lines, rendered with the ORIGINAL final-newline state.
    syntax_err = _validate_write(
        project_root,
        abs_path,
        before=_render_lines(all_lines, final_newline=original_final_newline),
        after=updated_text,
    )
    if syntax_err:
...
# #390: the ONLY fields a line-mode batch edit accepts. Anything else is a
# typo or a wrong-format payload — reject rather than silently coerce.
_ALLOWED_BATCH_EDIT_FIELDS = frozenset(
    {"path", "start_line", "end_line", "new_content", "expect", "mode"}
)


def _validate_batch_edit_fields(edit: object) -> str | None:
    """Return an error string if `edit` is a malformed line-edit payload, else None.

    Guards the silent-deleter failure mode: a misspelled `new_content` (e.g.
    `content`) or an omitted `new_content` must NOT default to "" and delete
    the target range. Unknown fields are rejected; new_content and start_line
    must be present. An intentional delete passes new_content="" explicitly.
    """
    if not isinstance(edit, dict):
        return f"edit must be an object, got {type(edit).__name__}"
    keys = set(edit.keys())
    unknown = keys - _ALLOWED_BATCH_EDIT_FIELDS
    if unknown:
        hint = ""
        if "content" in unknown:
            hint = " — did you mean 'new_content'?"
        elif "edits" in unknown:
            hint = (
                " — the nested {path, edits:[...]} shape is not supported here; "
                "pass flat edits: {path, start_line, end_line, new_content}"
            )
        return (
            f"unknown edit field(s) {sorted(unknown)}{hint}. "
            f"Allowed: {sorted(_ALLOWED_BATCH_EDIT_FIELDS)}"
        )
    if "start_line" not in keys:
        return "missing required field 'start_line'"
    if "new_content" not in keys:
        return (
            "missing required field 'new_content' — to delete a line range, "
            "pass new_content='' explicitly"
        )
    return None


@governed_project_mutation("file_ops.batch_edit")
def batch_edit(
    project_root: Path,
    ...
    *,
    ...
    for edit in edits:
        path = str(edit.get("path", "")) if isinstance(edit, dict) else ""

        # #390: destructive-edit inputs are strict. A one-word typo
        # (`content` for `new_content`) must NOT drop the value and delete the
        # line while returning ok:true. Reject unknown/misspelled fields and
        # require new_content/start_line to be PRESENT — an intentional delete
        # passes new_content="" explicitly; an omitted field is an error.
        field_err = _validate_batch_edit_fields(edit)
        if field_err is not None:
            validations.append(
                _edit_error(
                    path,
                    int(edit.get("start_line", 0) or 0) if isinstance(edit, dict) else 0,
                    int(edit.get("end_line", 0) or 0) if isinstance(edit, dict) else 0,
                    field_err,
                ),
            )
            continue
            ...
            validations.append(
                {
                    ...
                    "end": end,
                    "removed": len(old_lines),
                    "added": len(_split_new_content_lines(new_content)),
                    **(
    ...
    for canonical_path, file_edits in edits_by_file.items():
        ...
        # BEFORE = this file's original Phase-1 snapshot (file_cache is never
        # mutated in place; all_lines above is a rebuilt copy).
        syntax_err = _validate_write(
            project_root,
            abs_path,
            before=_render_lines(
                file_cache[canonical_path],
                final_newline=file_final_newlines[canonical_path],
            ),
            after=final_text,
        )
...
def str_replace_old_chars_cap(project_root: Path | None) -> int:
    """The configured old_string length cap for string-mode replaces
    (`edit.str_replace_max_old_chars`, default 1000; 0 = unlimited).
    Single source of truth — error messages and tool suggestions must
    quote THIS value, never a hardcoded number."""
    try:
        _moc = get_setting(
        ...
        return int(_moc) if _moc is not None else 1000
    except (TypeError, ValueError):
        return 1000


@governed_project_mutation("file_ops.str_replace")
def str_replace(
    project_root: Path,
    path: str,
    old_str: str,
    ...
    *,
    ...
    """
    # Empire doctrine 2026-05-01 (bumped to 500 on 2026-05-02): str_replace
    ...
    # happier populace.
    max_old_chars = str_replace_old_chars_cap(project_root)
    # 0 = unlimited (no cap on the matched old_str length).
    #
    # #1039 A+ 8: the cap discourages shipping big old/new pairs, but it
    # refused BEFORE looking at the file — so a long block that matches
    # EXACTLY ONCE (the only case where a big old_string is unambiguous)
    # was sent to a line-range workaround mid-edit. The cap is now a
    # uniqueness-conditional: over the cap and UNIQUE in the file -> allowed
    # up to a hard ceiling of 8x the cap; over the cap and ambiguous (or
    # absent, or beyond the ceiling) -> the original refusal.
    _over_cap = max_old_chars > 0 and len(old_str) > max_old_chars
    _hard_ceiling = max_old_chars * 8 if max_old_chars > 0 else 0
    if _over_cap and _hard_ceiling and len(old_str) > _hard_ceiling:
        return {
            "success": False,
            "path": path,
            "lines_changed": 0,
            "replacements": 0,
            "error": (
                f"old_string too long ({len(old_str)} chars, hard limit {_hard_ceiling} = 8x "
                f"edit.str_replace_max_old_chars). Use ai_replace(mode='anchor') for big "
                f"spans or mode='symbol' for whole bodies."
            ),
        }
    ...
    if _over_cap and count != 1:
        # Over the cap AND not unique: the original refusal — with the
        # measured count, so the caller knows which door is open.
        return {
            "success": False,
            "path": canonical_path,
            "lines_changed": 0,
            "replacements": 0,
            "error": (
                f"old_string too long ({len(old_str)} chars, limit {max_old_chars}); matches "
                f"{count} time(s), cap lifts only for a UNIQUE match. Use "
    ...
    if count > 1 and not replace_all:
        # Name WHERE the matches are (operator 2026-07-16): without the line
        # numbers the agent bloats old_str blind — and a blind retry is how a
        # wrong-twin edit lands on the OTHER identical site. Cap the listing
        # so a pathological match count stays readable.
        _match_lines: list[int] = []
        _pos = content.find(old_str)
        while _pos != -1 and len(_match_lines) < 8:
            _match_lines.append(content.count("\n", 0, _pos) + 1)
            _pos = content.find(old_str, _pos + 1)
        _lines_desc = "lines " + ", ".join(str(n) for n in _match_lines)
        if count > len(_match_lines):
            _lines_desc += f", … {count - len(_match_lines)} more"
        return {
            "success": False,
            "path": canonical_path,
            "lines_changed": 0,
            ...
            "error": (
                f"Found {count} matches for old_str in {canonical_path} ({_lines_desc}). "
                "Use replace_all=True, add context to make old_str unique, or switch to "
                "mode='anchor' around the intended line."
            ),
        }
    ...
    # Syntax validation — validate full final file after edit (BEFORE = the
    # content already read above, the snapshot new_content was built from)
    err = _validate_write(project_root, abs_path, before=content, after=new_content)
    if err:
        return {
            "success": False,
            "path": canonical_path,
...
@governed_project_mutation("file_ops.anchor_replace")
def anchor_replace(
    project_root: Path,
    path: str,
    *,
    ...
    inclusive=True (#1008): the replaced span runs from the START of
    start_anchor through the END of end_anchor -- both anchors are
    consumed and `replacement` takes their place. Adjacent anchors form
    a valid span; identical or overlapping anchors are refused. The
    default (False) keeps BETWEEN semantics byte-for-byte.
    ...
    Refuses:
      ...
      - empty span (start_anchor immediately followed by end_anchor —
        use ai_replace(mode='string') for inserts)
    ...
    # inclusive (#1008): both anchors are consumed, so they must name two
    # distinct regions. Identical anchors can never form a span.
    if inclusive and start_anchor == end_anchor:
        return {
            "success": False,
            "path": canonical_path,
            "error": (
                "inclusive=True: start_anchor and end_anchor are identical; "
                "they must be two distinct, non-overlapping anchors"
            ),
        }
    ...
    if inclusive:
        # Overlapping anchors (the sole end_anchor occurrence starts inside
        # start_anchor) cannot be consumed as one span - refuse clearly.
        _e_any = content.find(end_anchor)
        if _e_any < s + len(start_anchor) and _e_any + len(end_anchor) > s:
            return {
                "success": False,
                "path": canonical_path,
                "error": (
                    "inclusive=True: start_anchor and end_anchor overlap; "
                    "choose anchors that do not share text"
                ),
            }
    ...
    # inclusive (#1008): consume the anchors themselves. Adjacent anchors
    # are a valid (non-empty) span here, and the BETWEEN-mode foot-gun
    # guards do not apply - the replacement legitimately restates or
    # rewrites the anchor text.
    if inclusive:
        span_left = s
        span_right = e + len(end_anchor)
    else:
        span_left = s + len(start_anchor)
        span_right = e
    if not inclusive and span_left == span_right:
        return {
            "success": False,
            "path": canonical_path,
            "error": ("empty span between anchors — use ai_replace(mode='string') to insert content"),
        }

    # Foot-gun guard (2026-05-03): anchors PERSIST; replacement only
    # fills the content BETWEEN them.
    if not inclusive and replacement.startswith(start_anchor):
        return {
            "success": False,
            "path": canonical_path,
            "error": (
                ...
                "or use ai_replace(mode='string') for full-region replace."
            ),
        }
    if not inclusive and replacement.endswith(end_anchor):
        return {
            "success": False,
            "path": canonical_path,
            "error": (
                ...
                "use ai_replace(mode='string') for full-region replace."
            ),
        }

    # Line-frame the replacement (Empire 2026-06-20): in default (line-aligned) mode
    ...
    if allow_partial_anchors:
        framed = replacement
    elif inclusive and replacement == "":
        # Block DELETION: nothing to frame -- injecting a "\n" here left a stray
        # blank line. If the anchors did not carry their own line terminator the
        # span ends just before one; consume it so the deletion removes whole
        # lines (mirrors what anchors WITH a trailing "\n" already achieve).
        framed = ""
        if span_right < len(content) and content[span_right] == "\n":
            span_right += 1
    else:
        ...
        # included it (Empire 2026-06-20 regression; test_edit_checkpoint).
        framed = replacement
        ...
        # File edges count as boundaries (reachable only with inclusive=True,
        # where the span may start at offset 0 or run to EOF).
        left_has_nl = span_left == 0 or content[span_left - 1] == "\n"
        right_has_nl = span_right >= len(content) or content[span_right] == "\n"
    ...
    if err:
        return {
            ...
            "error": (
                f"Syntax err: {err} "
                + (
                    "(inclusive=True: both anchors were consumed and replaced — "
                    if inclusive
                    else "(anchors persist and replacement fills only the BETWEEN — "
                )
...
@governed_project_mutation("file_ops.batch_str_replace")
def batch_str_replace(
    ...
    *,
    ...
    # Per-file ORIGINAL snapshot (file_cache is mutated as edits apply): the
    # Ruff edit gate's BEFORE, from the same read the edits were built on.
    ...
    for edit_index, edit in enumerate(edits):
        ...
        try:
            ...
            if canonical_path not in file_cache:
                ...
                file_originals[canonical_path] = file_cache[canonical_path]
                file_paths[canonical_path] = abs_path
    ...
    for canonical_path, content in file_cache.items():
        ...
        syntax_err = _validate_write(
            project_root,
            abs_path,
            before=file_originals[canonical_path],
            after=content,
        )
        if syntax_err:
            return {
...
@governed_project_mutation("file_ops.extract_block")
def extract_block(
    project_root: Path,
    source_path: str,
    start_line: int,
