...
# #755/#756: the ONE canonical connect. All six sites were
...
# single pragma, so this store ran with foreign_keys OFF (its own
# route_id FK inert), no busy_timeout and the default synchronous=FULL.
# DURABILITY: AUDIT for the two writes. An anchor is what makes
# edit_memory_gate BLOCK an edit until the operator acknowledges the
# doctrine pinned to that symbol, so an anchor a power cut un-writes
# hands back the right to edit unacknowledged. The four SELECT paths say
# read_only=True, which is the truthful mode and costs nothing.
...
class AnchorStore:
    ...
    def __init__(
        ...
        *,
        ...
        # Schema is migrated once per resolved db FILE. With the
        # lazy-resolver pattern, the underlying path can change between
        # calls (tests, project switching), and each new path needs the
        # palace-anchor columns added. Identity-validated (2026-07-09,
        # schema_memo.py): a recycled path with a fresh db re-migrates
        # instead of skipping into 'no such column/table'.
        from .schema_memo import SchemaMemo

        self._migrated_paths = SchemaMemo()
    ...
    def ensure_schema(self) -> dict:
        ...
        with _canonical_connect(
            str(path), durability=_Durability.AUDIT, row_factory=False
        ) as conn:
            report = migrate_memory_symbol_anchors(conn)
        try:
            self._migrated_paths.mark(path.resolve())
        except OSError:
            self._migrated_paths.mark(path)
        return report
    ...
    def _ensure_schema_for_current_path(self) -> None:
        ...
        try:
            ...
            key = self.db_path
        if self._migrated_paths.is_current(key):
            return
        self.ensure_schema()
    ...
    def register_anchor(
        ...
        *,
        ...
        with _canonical_connect(
            str(self.db_path), durability=_Durability.AUDIT, row_factory=False
        ) as conn:
    ...
    def list_anchors_for_unit(
        ...
        *,
        ...
        with _canonical_connect(
            str(self.db_path), read_only=True, row_factory=False
        ) as conn:
