...
# `peek_for_session` USED TO LIVE HERE, and its presence in production was a
# DEFECT rather than a convenience. `prompt_mutator.notifications_drain` called
# it: a NON-CONSUMING read that announced a finished background run and left
# the record pending, so the MCP rail announced it again -- one of the causes
# of the triple-announce. The fix LANDED (prompt_mutator now delegates to
# `notification_injector.compose_notices`), and
# `tests/runtime/test_notification_rail_one_authority.py` asserts that
# `peek_for_session` appears nowhere in that drain, so a production consumer
# for it must never exist again. Removed 2026-09-12; the non-consuming
# inspection the security tests need now lives in
# `tests/notification_rail_support.py`, built from `peek` + `_owned_by`, which
# are the same two production parts it was made of.
#
# `surface_for_session` below is the live, CONSUMING read.
