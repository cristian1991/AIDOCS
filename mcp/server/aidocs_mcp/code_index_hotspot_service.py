...
class CodeIndexHotspotService:
    ...
    def find_hotspots(self, project_root: Path, query: str | None = None, limit: int = 30) -> dict[str, object]:
        ...
        for row in rows:
            ...
            if needle:
                score += self.store._score_text_match(needle, path, exact=60, prefix=35, contains=20, reasons=reasons, label="path")
                score += self.store._score_text_match(needle, str(row["summary"] or ""), exact=25, prefix=15, contains=8, reasons=reasons, label="summary")
    ...
    def find_query_hotspots(self, project_root: Path, query: str | None = None, limit: int = 30) -> dict[str, object]:
        ...
        for row in rows:
            ...
            reasons: list[str] = []
            if needle:
                score += self.store._score_text_match(needle, path, exact=50, prefix=30, contains=20, reasons=reasons, label="path")
                score += self.store._score_text_match(needle, str(row["summary"] or ""), exact=20, prefix=10, contains=5, reasons=reasons, label="summary")
    ...
    def find_domain_clusters(self, project_root: Path, concept: str, limit: int = 50) -> dict[str, object]:
        ...
        cluster.sort(key=lambda item: (self.store._layer_rank(str(item["layer"])), str(item["path"]), str(item["symbol"] or "")))
        return {
            "concept": concept,
    ...
    def find_transition_points(self, project_root: Path, concept: str | None = None, limit: int = 50) -> dict[str, object]:
        ...
        symbol_matches = self.store.search_symbols(project_root, seed_query, limit=max(limit * 3, 100))
    ...
    def find_frontend_symbols(
        ...
        for row in rows:
            ...
            if needle_text:
                score += self.store._score_text_match(needle_text, row["symbol"], exact=140, prefix=100, contains=70, reasons=reasons, label="symbol")
                score += self.store._score_text_match(needle_text, row["container"] or "", exact=35, prefix=20, contains=10, reasons=reasons, label="container")
