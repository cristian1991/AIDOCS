...
from .language_descriptors import descriptor_for_language, extractor_family_for_language, line_patterns_for_language, outline_family_for_language, outline_patterns_for_language
...
class CodeIndexOutlineService:
    ...
    def _extract_outline(self, project_root: Path, text: str, code_language: str) -> list[tuple[str, str, int, str | None, bool]]:
        ...
        if code_language not in {"python", "javascript", "typescript", "jsx", "tsx", "csharp", "go", "rust"}:
            try:
                from .tree_sitter_service import extract_outline as ts_outline
                ext_map = {"java": ".java", "dart": ".dart", "html": ".html", "css": ".css"}
        ...
        if extractor_family == "python_ast":
            ...
            if ast_outline is not None:
                for symbol, kind, line_number, container, is_partial in ast_outline:
                    ...
                    elif symbol[:1].isupper() and kind == "function":
                        js_kind = "component"
                ...
                self._extract_component_semantics(project_root, code_language, text, ast_outline, outlines)
            else:
                patterns = [
