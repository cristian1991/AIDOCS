...
HOOK_CMD="python -m aidocs_mcp.claude_hook"
...
def remove_aidocs_groups(groups):
    ...
    for group in normalize_groups(groups):
        ...
        for hook in group_hooks:
            ...
            if ("claude-hook.ps1" in command or "claude-hook.sh" in command or "aidocs_mcp.claude_hook" in command or status.startswith("AIDOCS ")):
                is_aidocs = True
                break
        if not is_aidocs:
            result.append(group)
...
hook_command = "python -m aidocs_mcp.claude_hook"
session_start_group = {
    "hooks": [
        {
            "type": "command",
...
hooks["PostCompact"] = remove_aidocs_groups(hooks.get("PostCompact")) + [post_compact_group]
