...
# ── #492: the ONE governed documentation manifest inside the protected
# config dir ───────────────────────────────────────────────────────────
#
# GATE SELF-CONTRADICTION (specimen 2026-07-19): `.MEMORY/.aidocs/
# STORE_INVENTORY.md` is operator DOCUMENTATION whose currency
# `mcp/tests/host/test_store_inventory_contract.py` REQUIRES — it fails
# CI when a new sqlite table or `.MEMORY/**` artifact is unlisted. But
# the path matches `.memory/.aidocs`, so aidocs_config_mutation denied
# BOTH the native Edit path and the governed ai_replace path: the rule
# forbade its own sanctioned alternative, leaving no lawful way to
# satisfy the project's own census test.
#
# Narrowing (not deletion — the _URL_SINK/foreign-host precedent): the
# GOVERNED, gate-routed, audit-stamped AIDOCS editors may write EXACTLY
# this one markdown manifest. Everything else is unchanged:
#   * native/ungoverned editors (Edit/Write/MultiEdit/NotebookEdit, …)
#     are still DENIED on it — they bypass the AIDOCS audit envelope;
#   * shell writes (redirect/tee/sed -i/…) are still DENIED — the
#     command classifier never consults this allowance;
#   * every other path in `.MEMORY/.aidocs/` and `.MEMORY/.index/` —
#     real config/security state — is still DENIED on every surface.
# The match is an exact normalized SUFFIX, so a nested or lookalike
# path (`.aidocs/nested/STORE_INVENTORY.md`, `STORE_INVENTORY.md.py`)
# inherits nothing.
_GOVERNED_DOC_MANIFESTS = (".memory/.aidocs/store_inventory.md",)


def _is_governed_doc_manifest(path: str) -> bool:
    """True iff ``path`` is an allowlisted documentation manifest that the
    governed AIDOCS editors are permitted to maintain in-place.
    """
    low = _norm_path(path)
    if ".." in low:
        return False  # never resolve traversal here — fail closed
    return any(low == m or low.endswith("/" + m) for m in _GOVERNED_DOC_MANIFESTS)
...
def classify_tool(tool_name: str, tool_input: dict | None) -> CoupFinding | None:
    ...
    try:
        ...
        if path is None:
            return None
        # #492: the governed (gate-routed, audit-stamped) AIDOCS editors
        # may maintain the allowlisted documentation manifest. Native /
        # ungoverned editors fall through and stay denied.
        if nt.startswith("ai_") and _is_governed_doc_manifest(path):
            return None
