...
/** True when a login was EVER completed on this install — a stored gate
 *  connection record exists, even if its token has expired. Empire ruling
 *  2026-07-19 (#404 + #92): the FIRST login is a wall; expiry is not — an
 *  expired-but-stored identity renews, it never re-walls. Sign-out
 *  (saveGateConnection(null)) removes the record and restores the wall. */
...
/** Does the login WALL (App.tsx) let the shell render?
 *
 *  Web build: requires a LIVE gate connection (the browser has no other identity).
 *  Desktop: the shell is already gated on the operator token (dashboard_auth_status);
 *  here a STORED login satisfies the wall in EVERY mode (#92 renew, never re-wall).
 *  Previously desktop cloud modes demanded a live ~1h token while renewal ran only
 *  at boot, so clicking "WebAgent" after the hour re-drew sign-in for a signed-in
 *  operator. An expired gate session is reported by WebmcpModePanel's "reconnect"
 *  control, and gate calls still refuse without a live bearer. */
export function loginWallSatisfied(): boolean {
  if (isWebBuild()) return loadGateConnection() !== null;
  return hasStoredLogin();
}

/** The stored connection INCLUDING an expired one.
 *
 *  `loadGateConnection()` drops expired records, which is correct for "am I
 *  connected?" and fatal for "can I renew?" — the refresh token lives inside the
 *  record it discards. Renewal (#92) is the only caller; everything else must
 *  keep using loadGateConnection() so an expired token can never be treated as
 *  live. */
export function loadGateConnectionIgnoringExpiry(): GateConnection | null {
  try {
    const raw = localStorage.getItem(GATE_KEY);
    return raw ? (JSON.parse(raw) as GateConnection) : null;
  } catch {
    return null;
  }
}
