...
function runPythonJson(projectRoot, args) {
  ...
  const pythonPath = process.env.AIDOCS_MCP_PATH || ""
  try {
...
function classifyPromptAction(text, projectRoot) {
  ...
  // Try local tokens first (fast, works when action_tokens/ is accessible)
...
async function AIDOCSPlugin(input) {
  ...
  return {
    ...
    "experimental.session.compacting": async ({ sessionID }, output) => {
      ...
      // Reset token counters on compaction (new context window)
        if (state.sessionID) {
          runPythonJson(projectRoot, [
            "-c",
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (state.sessionID) {
        try {
          ...
          const commsResult = runPythonJson(projectRoot, [
            "-c",
            `import json; from pathlib import Path; from aidocs_mcp.conductor_comms import check_lane_and_messages; print(json.dumps(check_lane_and_messages(Path(${JSON.stringify(String(projectRoot))}), ${JSON.stringify(String("LANE_PLACEHOLDER"))})))`.replace("LANE_PLACEHOLDER", laneID),
          ])
          if (commsResult) {
            // Lane paused → block
            if (commsResult.lane_state === "paused") {
              throw new Error(`Lane '${laneID}' is paused: ${commsResult.lane_reason || "paused by conductor"}. Wait for conductor to resume.`)
            }
            if (commsResult.lane_state === "canceled") {
              throw new Error(`Lane '${laneID}' has been canceled. Stop working.`)
            }
            // Pending conductor messages → inject into system context
          ...
          // If it's our own thrown error (lane paused/canceled), re-throw
          if (String(commsErr).includes("Lane '")) {
            throw commsErr
          }
          // Otherwise ignore — conductor comms unavailable
        }
      }
