"""THE ONE CANONICAL SQLITE CONNECT (#755, empire-doctrine VI).
...
rots back exactly the way #746 did: absence is a bug, not a skip (doctrine XXVI).
"""
...
# ── #489 PHASE 1: where does a hook evaluation's SQLite time go? ─────────────
#
# The broker ring said eval_dominant with a 14s p95 and daemon.out carried
# "database is locked" lines with no timestamps. Nothing tied the two together,
# so this ledger does, AT THE CHOKEPOINT: while a ledger is armed on the current
# context, every execute/executemany/executescript/commit on a connection opened
# through connect() is timed and attributed to its database FILE NAME.
#
# HONEST LIMITS, stated so the numbers are not over-read:
#   * Python's sqlite3 exposes no busy-handler hook, so a lock WAIT cannot be
#     separated from work inside one call. A single call taking >= STALL_MS is
#     recorded as a ``stall`` (lock-wait-SHAPED: these statements take
#     single-digit ms uncontended); ``locked`` counts calls that ran out the
#     busy timeout and raised. Both are facts; "it was the lock" is an inference.
#   * Coverage is connect() only. Stores still on raw sqlite3.connect (the
#     chokepoint allowlist) are invisible, so ``sqlite_ms`` is a FLOOR.
#   * Context-local: a worker thread spawned by the evaluation is not counted.
# Disarmed cost: one ContextVar.get per call.
STALL_MS = 50.0
_MAX_STALL_ROWS = 16
_ledger: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "aidocs_sqlite_wait_ledger", default=None
)


@contextlib.contextmanager
def sqlite_wait_ledger():
    """Arm a per-evaluation SQLite timing ledger; yields the dict it fills."""
    ledger: dict = {"sqlite_ms": 0.0, "stall_ms": 0.0, "locked": 0, "by_db": {}, "stalls": []}
...
_PLUMBING = frozenset({"_sqlite_connect.py", "_sqlite_index_store_base.py", "contextlib.py", "functools.py"})


def _stall_site() -> str:
    """Innermost non-plumbing caller chain (3 frames). Stall rows only, so rare."""
    ...
    while frame is not None and len(sites) < 6:
        ...
        if name not in _PLUMBING:
            sites.append(f"{name}:{frame.f_lineno} {frame.f_code.co_name}")
        frame = frame.f_back
    return " < ".join(sites)


def _account(conn: object, what: str, sql: object, started: float, locked: bool) -> None:
    try:
        ledger = _ledger.get()
        if ledger is None:
            return
        ms = (time.perf_counter() - started) * 1000.0
        db = getattr(conn, "_aidocs_db", "?")
        slot = ledger["by_db"].setdefault(db, {"ms": 0.0, "calls": 0, "stall_ms": 0.0, "locked": 0})
        slot["ms"] += ms
        slot["calls"] += 1
        ledger["sqlite_ms"] += ms
        if locked:
            slot["locked"] += 1
            ledger["locked"] += 1
        if ms >= STALL_MS or locked:
            slot["stall_ms"] += ms
            ledger["stall_ms"] += ms
            if len(ledger["stalls"]) < _MAX_STALL_ROWS:
                head = " ".join(str(sql or what).split())[:80]
                ledger["stalls"].append(
                    {"db": db, "ms": round(ms, 1), "sql": head, "locked": locked, "site": _stall_site()}
                )
    except Exception:  # noqa: BLE001,S110 -- an instrument must never break the caller
        pass
...
class ClosingConnection(sqlite3.Connection):
    ...
    at the one canonical connect (#755, empire-doctrine VI), because the
    ...
    def _timed(self, what: str, sql: object, call, *args):  # noqa: ANN001,ANN202
        ...
        try:
            ...
            _account(self, what, sql, started, "locked" in str(exc).lower())
            raise
        _account(self, what, sql, started, False)
        return result

    def execute(self, *args):  # noqa: ANN002,ANN201
        return self._timed("execute", args[0] if args else "", super().execute, *args)

    def executemany(self, *args):  # noqa: ANN002,ANN201
        return self._timed("executemany", args[0] if args else "", super().executemany, *args)

    def executescript(self, *args):  # noqa: ANN002,ANN201
        return self._timed("executescript", "executescript", super().executescript, *args)

    def commit(self):  # noqa: ANN201
        return self._timed("commit", "COMMIT", super().commit)
...
def connect(
    ...
    *,
    ...
    with contextlib.suppress(Exception):
        conn._aidocs_db = Path(memo_key.split("?", 1)[0]).name  # noqa: SLF001
    if row_factory:
