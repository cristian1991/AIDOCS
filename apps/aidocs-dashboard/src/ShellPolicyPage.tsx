...
import { JudgeOverridePanel } from "./JudgeOverridePanel";
import { CastlePageHeader } from "./CastleShell";
import { configEditingAvailable } from "./entitlements";
import { getMode } from "./webmcpScope";
import type {
  BashCommandTriState,
  BashPolicySnapshot,
  JudgeOverridesSnapshot,
} from "./dashboardApi";
...
export type ShellPolicyPageProps = {
  ...
  /** Backlog #19/#22 — judge-rule override editor data + save. */
  judgeOverrides?: JudgeOverridesSnapshot | null;
  onSaveJudgeOverrideFamily?: (family: string, ruleIds: string[]) => void;
};
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
        {/* Governed Bash is a local/desktop-only switch — over WebAgent it only
            renders a dead "not available" card that eats half the page. Show it
            only in local mode so the policy table gets the room. */}
        {getMode() === "local" ? <GovernedBashPanel projectRoot={projectRoot} /> : null}
        {policy ? (
          <BashPolicyPanel
            policy={policy}
        ...
        {judgeOverrides && onSaveJudgeOverrideFamily ? (
          <JudgeOverridePanel
            data={judgeOverrides}
            activeLayer={activeLayer}
            canEdit={configEditingAvailable()}
            saving={!!saving && saving.includes("judge_override")}
            onSaveFamily={onSaveJudgeOverrideFamily}
          />
        ) : null}
      </div>
    </div>
