...
class SessionQueryGateStore(SQLiteIndexStoreBase):
    ...
    def init_db(self, project_root: Path) -> None:
        with self.session(project_root) as conn:
            ...
            for column_def in (
                ...
                "ALTER TABLE session_query_gate ADD COLUMN last_cli_session_id TEXT NOT NULL DEFAULT ''",
    ...
    def get_last_cli_session_id(
        self,
        project_root: Path,
        session_id: str,
    ) -> str:
        """Return the last CLI session id stamped for this session, or "".
        ...
        with self.session(project_root) as conn:
            row = conn.execute(
                "SELECT last_cli_session_id FROM session_query_gate WHERE session_id = ?",
                (session_id,),
            ).fetchone()
        if row is None:
            return ""
    ...
    def set(
        ...
        *,
        ...
        with self.session(project_root) as conn:
            ...
            def resolve_scalar(val: Any, key: str) -> Any:
                if val is self._KEEP:
    ...
    def set_dev_ai_run_bash_path(
        ...
        *,
        ...
        """
        from . import shell_resolver as _sr
        ...
        if normalized:
            ...
            if flavor != "dev":
                ok = False
                rejection_reason = (
                    "dev_ai_run_bash_path is dev-flavor only; "
                    f"current distribution.flavor={flavor!r}"
                )
                validation_result = "rejected_flavor"

            # Path validation reuses the resolver's universal rules.
            if ok:
                # PowerShell flag — read-side check; tier-2 PowerShell
                # path rejection lives in the resolver, but we mirror
                # it here so the setter rejects pwsh paths even when
                # the flag is true (Batch C will revisit).
                try:
                    powershell_flag = bool(
                        get_setting(
                ...
                ok_validate, reason_validate = _sr._validate_path(
                    normalized,
                    project_root,
                ...
                if not ok_validate:
                    ok = False
                    rejection_reason = f"path validation failed: {reason_validate}"
                    validation_result = "rejected_validation"

            # Probe.
            if ok:
                ok_probe, _version, reason_probe = _sr._probe(normalized)
                if not ok_probe:
                    ok = False
                    rejection_reason = f"probe failed: {reason_probe}"
                    validation_result = "rejected_probe"
    ...
    def get_dev_ai_run_bash_path(
        ...
        """Read the dev-flavor session-scoped override.

        Returns "" when:
          - session row doesn't exist
          - column is empty
          - distribution.flavor != "dev" (read-side flavor gate
            enforced as defense in depth — even if the column was
            somehow populated on a non-dev install, this getter
            returns "")
        """
        from .config import get_setting
        ...
        with self.session(project_root) as conn:
            row = conn.execute(
                "SELECT dev_ai_run_bash_path FROM session_query_gate WHERE session_id = ?",
                (session_id,),
            ).fetchone()
        if row is None:
            return ""
        return str(row["dev_ai_run_bash_path"] or "").strip()
    ...
    def stamp_compaction(
        ...
        *,
        ...
        """Record that a context_compact event just fired for this session.

        The force-wakeup guard extends its deadline by 120s when this is
        within 120s of now, so the agent gets a grace window to re-emerge
        after compaction without being locked out.
        """
        import time
        ...
        with self.session(project_root) as conn:
            self._ensure_row(conn, session_id)
            conn.execute(
                "UPDATE session_query_gate SET last_compaction_at = ? WHERE session_id = ?",
                (ts, session_id),
            )
        return ts

    def get_last_compaction(self, project_root: Path, session_id: str) -> int:
        with self.session(project_root) as conn:
            try:
                row = conn.execute(
                    "SELECT last_compaction_at FROM session_query_gate WHERE session_id = ?",
                    (session_id,),
                ).fetchone()
            except Exception:
                return 0
        return int(row["last_compaction_at"]) if row else 0
    ...
    def check_and_update_host_session_id(
        ...
        the canonical name. Column `last_cli_session_id` keeps its
        legacy name this round; column rename ships in a separate
        migration PR with data-copy.
        """
        ...
        with self.session(project_root) as conn:
            self._ensure_row(conn, session_id)
            row = conn.execute(
                "SELECT last_cli_session_id, current_lane_id "
                "FROM session_query_gate WHERE session_id = ?",
                (session_id,),
            ).fetchone()
            previous = str(row["last_cli_session_id"]) if row else ""
    ...
    def check_and_update_cli_session_id(
        self,
        project_root: Path,
        session_id: str,
        cli_session_id: str,
    ) -> bool:
        """Deprecated: use check_and_update_host_session_id."""
        return self.check_and_update_host_session_id(
            project_root,
            session_id,
            cli_session_id,
        )
    ...
    # and last_cli_session_id are NOT restored because they're
