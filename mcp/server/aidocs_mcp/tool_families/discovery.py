...
@tool(
    ...
    # Dedupe (2026-06-29): callers/references/usages are OWNED by ai_find — its
    # references mode is the real impl; ai_trace's references mode is a thin
    # delegate to it. So surface ai_trace on FLOW keywords only; caller keywords
    # route to ai_find. ("trace" already hits the name part.)
    aliases=("lineage", "dataflow", "tracing"),
)
def ai_trace(
    query: str,
    mode: str = "field_flow",
    ...
    """Unified FLOW tracer — follow a field/value/service/component/route through
    the codebase across hops: field_flow (DB/struct field lineage), service,
    model, component, css_class, api_to_ui, query_shape, setting. For plain
    callers/usages of a symbol use ai_find(mode='references') — the canonical
    owner; ai_trace's references mode is a thin delegate to it."""
    return _delegate(
