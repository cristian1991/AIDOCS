...
class PromptSubmitService:
    ...
    def _run_submit_core(
        ...
        *,
        ...
        # #587-A: THE authoritative moment. This is the one place that receives
        # the host's OWN statement of what it is (the UPS payload's host_kind)
        # together with its session id. Until now that statement lived exactly as
        # long as the ContextVar above and was then lost, so the next request had
        # to guess from the MCP server's environment — the "six answers, three
        # disagreeing" defect. Recording it here makes the answer OUTLIVE the
        # request, keyed by the host's own session id.
        try:
