...
class CodeIndexEdgeService:
    ...
    def _resolve_edge_to_paths(
        ...
        if kind in {"import", "require", "dynamic_import"} and target.startswith("."):
            ...
            # #1039 A+ 7: a `using DentalApp.Domain.Enums;` used to expand to
            # EVERY file in the namespace (20 enum files for one using). Resolve
            # by the symbols the source file ACTUALLY references: keep only the
            # namespace's files whose top-level types appear as tokens in the
            # source text. Fall back to the whole namespace only when nothing
            # resolves (an unparsed source, or a namespace used purely for
            # extension methods), so the answer is never emptier than before.
            with self.store.connect(project_root) as conn:
                rows = conn.execute(
                    "SELECT path, symbol FROM code_outlines WHERE container = ? ORDER BY path",
                    (target,),
                ).fetchall()
            by_path: dict[str, set[str]] = {}
            for row in rows:
                by_path.setdefault(str(row["path"]), set()).add(str(row["symbol"]))
            referenced: list[str] = []
            try:
                source_text = source_abs.read_text(encoding="utf-8", errors="ignore")
            except OSError:
                source_text = ""
            if source_text:
                import re as _re

                tokens = set(_re.findall(r"[A-Za-z_][A-Za-z0-9_]*", source_text))
            ...
            if referenced:
                candidates.extend(referenced[:limit])
            else:
                candidates.extend([p for p in by_path if p != source_path][:limit])
