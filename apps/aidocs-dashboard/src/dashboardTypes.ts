...
export type SessionsPageProps = {
  ...
  // Project root for the session-detail popup's copyable CLI command. Optional:
  // with no root the command is still shown, annotated that the path must be
  // supplied — never emitted as a silently-broken command.
