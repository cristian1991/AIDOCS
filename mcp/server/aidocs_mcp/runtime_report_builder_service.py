...
class RuntimeReportBuilderService:
    ...
    def _build_session_start_report(self, response: dict[str, object]) -> dict[str, object]:
        if response.get("requires_session_selection"):
            sessions = (
                response.get("sessions") if isinstance(response.get("sessions"), list) else []
            )
            repo_summary = (
            ...
            extra = (
                repo_summary.get("bullets") if isinstance(repo_summary.get("bullets"), list) else []
            )
            return {
                ...
                + self._environment_bullets(response),
        ...
        handoff_steps = (
            response.get("handoff_steps") if isinstance(response.get("handoff_steps"), list) else []
        )
        ...
        compliance = (
            response.get("compliance") if isinstance(response.get("compliance"), dict) else {}
        )
        journal_coverage = (
        ...
        bullets.extend(self._environment_bullets(response))
        return {
    ...
    @staticmethod
    def _environment_bullets(response: dict[str, object]) -> list[str]:
        """Surface a #769-class venv-drift warning at session start when the
        environment preflight found a below-floor/missing declared dependency
        or a pip dependency conflict. Silent when the environment is clean."""
        env = response.get("environment") if isinstance(response.get("environment"), dict) else {}
        if not env or env.get("ok"):
            return []
        summary = str(env.get("summary") or "").strip()
        return [f"⚠ {summary}"] if summary else ["⚠ Environment drift detected — rebuild the venv from the manifest."]
    ...
    def _build_bootstrap_report(self, result: dict[str, object]) -> dict[str, object]:
        ...
        capabilities = (
            sync.get("capabilities") if isinstance(sync.get("capabilities"), dict) else {}
        )
    ...
    def _build_readiness_summary(
        ...
        *,
        ...
        code_manifest = (
            sync.get("code_manifest") if isinstance(sync.get("code_manifest"), dict) else {}
        )
