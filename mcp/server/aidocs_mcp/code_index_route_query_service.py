...
class CodeIndexRouteQueryService:
    ...
    def find_ui_backend_touchpoints(
        ...
        for item in code_matches:
            ...
            if score == 0:
                # Same rule as the symbol branch: a file with no relevance to
                # the query is noise, not a touchpoint (search_code now
                # collects per term, so it can return partial-term rows).
                continue
            seen.add(key)
            merged.append(
                {
        ...
        if not merged:
            # #1039 polish: touchpoints returned EMPTY for "document type
            # settings" on every query. An empty answer is the worst one a
            # concept tool can give; degrade to the AND-first cluster engine
            # and SAY so, rather than return nothing.
            try:
                clusters = self.store.find_domain_clusters(project_root, needle, limit=limit)
            except Exception:
                clusters = {"cluster": []}
            rows = list(clusters.get("cluster") or [])
            if rows:
                return {
                    "matches": [],
                    "degraded_to": "clusters",
                    "cluster": rows,
                    "empty_reason": "no_touchpoint_match",
                    "next_action": (
                        "No UI<->backend touchpoint matched this concept by name; "
                        f"the {len(rows)} cluster row(s) above are the AND-ranked "
                        "symbols/files for it — start from those, or call "
                        "ai_find(mode='clusters') directly."
                    ),
                }
            return {
                "matches": [],
                ...
                "next_action": (
                    "No symbol, file or cluster matched this concept — try fewer / "
                    "different terms, or ai_find(mode='symbols')."
                ),
            }
        return {"matches": merged[:limit]}
