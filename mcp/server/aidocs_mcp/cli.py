...
def dashboard_worker_serve(lines, emit, snapshot_fn) -> None:
    ...
    pure core (tested in tests/runtime/test_dashboard_worker.py):
      ...
      request  {"id": N, "root": "...", "session_id": "..."|null}
    ...
    for raw in lines:
        ...
        if not root:
            ...
            continue
        try:
            snapshot = snapshot_fn(root, req.get("session_id") or None)
...
def cmd_dashboard_worker(args: list[str]) -> int:
    ...
    def _snapshot(root: str, session_id: str | None):
        return runtime.dashboard_snapshot(Path(root), session_id=session_id)
...
def _cmd_admin_clear_freeze(args: list[str]) -> int:
    ...
    # an operator's by any admin. Skipped under kill-switch / dev-flavor
    # break-glass (no resolved approver identity). Fail-closed tiers.
    if not kill_switch_on and not dev_flavor:
