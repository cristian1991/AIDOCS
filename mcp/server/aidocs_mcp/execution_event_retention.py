...
from .xaacp_stream import (
    AUDIT_STREAM_ACK,
    AUDIT_STREAM_DECIDE,
    AUDIT_STREAM_DELIVER,
    AUDIT_STREAM_READ,
    AUDIT_STREAM_SEND,
)
...
EVENT_KIND_RETENTION: dict[str, RetentionClass] = {
    ...
    # The test-retry edit witness WINDOW. These three rows exist only to
    # describe one in-flight edit invocation, and their useful life is
    # _EDIT_PREIMAGE_TTL_SECONDS = 15 MINUTES (agent_orchestrator.py): a
    # preimage is closed by its postimage, disposed on abort, or retired as
    # expired. MECHANICAL's 1-day horizon is already ~96x that life, and the
    # volume is one row PER NAMED PATH PER EDIT INVOCATION on the hottest path
    # in the tree -- exactly the shape that filled the table in 2026-08-23.
    # They share a class deliberately: a closure row must not outlive, nor be
    # outlived by, the row it closes.
    #
    # NOTE the asymmetry with `edit_invocation_terminal` below, which is
    # DECISION. These three going early is harmless (the window is over);
    # the terminal receipt going early manufactures a false "still open".
    "edit_preimage": _M,
    "edit_postimage": _M,
    "edit_preimage_expired": _M,
    # ── OPERATIONAL ─────────────────────────────────────────────────
    ...
    # #1061 stream transport (r0b: ordinary traffic must not bloat the ledger).
    # Same tier as conductor_comms: the message body lives in the stream store;
    # these rows are delivery bookkeeping. The DECISION on an item is stream_decide.
    AUDIT_STREAM_SEND: _O,  # one row per message -- high volume
    AUDIT_STREAM_DELIVER: _O,  # delivery receipt, duplicates the send
    AUDIT_STREAM_READ: _O,  # read receipt, high volume
    # Receipt/ownership acknowledgement, NOT a decision (the terminal decision
    # lands later as stream_decide, which is DECISION). One per message.
    AUDIT_STREAM_ACK: _O,
    # ── DECISION ────────────────────────────────────────────────────
    ...
    # Non-strike security hard blocks (inspection could not finish / the
    # taxonomy returned a decision the gate does not know). Same retention as
    # their judge siblings: a refusal decision, kept as one.
    "judge_security_inspection_failure_block": _D,
    "judge_taxonomy_unknown_decision_block": _D,
    # #1097: the judge PRECHECK raised, so the call is refused closed with no
    # strike -- the same class as the inspection-failure block above
    # (gate_tool.py, "Same class as the inspection-failure precedent").
    "judge_precheck_unavailable_block": _D,
    ...
    # #1063: a recorded credential refusal judged stale (credential changed or a
    # later gate call succeeded) and backoff reset -- an authority-relevant
    # decision to resume syncing past a refusal.
    "backlog_sync_refusal_superseded": _D,
    # #1061 operator bridge: operator-origin input admitted to / refused from
    # the governed stream (payload: ids + content_sha256, never text).
    "operator_prompt_bridged": _D,
    "operator_prompt_bridge_refused": _D,
    "skill_write": _D,
    "memory_capture": _D,
    # DRT-06 server-minted read receipts. DECISION, not FORENSIC, and the
    # count cap is the axis that decides it:
    #   - NOT count-capped (so not OPERATIONAL). This row is the EVIDENCE
    #     `edit_memory_gate` consults. If a burst of chatter could evict it,
    #     a memory that WAS read becomes "you never read it" -- the refusal
    #     the DECISION class exists to protect, arriving as a false positive.
    #   - Age-pruned (so not FORENSIC), because a receipt's value EXPIRES by
    #     construction: it dies when the body revision changes or the memory
    #     epoch rotates on compaction, and the verifier only ever scans the
    #     newest `_MAX_RECEIPT_SCAN` rows. One row per memory body per read
    #     on a hot path, kept forever, with no reader past its epoch, is the
    #     2026-08-23 saturation trap in a new costume.
    "memory_read_receipt": _D,
    "restore_facade": _D,
    ...
    "turn_seal_failed": _D,
    "operator_intent": _D,
    # 8dffd6552 (bleed B): one operator-prompt consent for a local
    # ai_session create/delete was consumed (atomically, single use). A grant
    # being spent -- kept as the decision record, like app_confirmation_consumed.
    "session.user_intent_action_consumed": _D,
    ...
    # #1061 Consul governance semantics (r0b ruling: long-horizon DECISION,
    # not FORENSIC -- FORENSIC is for anomalies/credential proof). Each is the
    # durable record behind a MECHANICAL host-hook row. PRE-REGISTERED: see
    # PENDING_EVENT_KINDS.
    "consul_turn_mirror": _D,  # what a seated Consul's turn said/did
    "consul_park_enter": _D,  # seat parked: a governance state transition
    "consul_park_return": _D,  # seat resumed: closes the park interval
    "consul_peer_verdict": _D,  # peer verification outcome someone must justify
    "consul_stop_outcome": _D,  # the retained turn-end record (raw `stop` is 1-day)
    "consul_peer_absent": _D,  # verification did NOT happen -- must outlive chatter
    AUDIT_STREAM_DECIDE: _D,  # a decision on a stream item; never count-capped
    # #1060 r0b: explicit member git.push grant/revoke (and refused attempts) --
    # authority mutations on the shared org push credential. DECISION per r0b.
    "git_push_grant": _D,
    "git_push_revoke": _D,
    # A branch switch performed through ai_git. NOT forensic like `git_push`:
    # no shared credential is spent and nothing leaves the machine, so the
    # never-pruned tier would be over-claiming. It is still a state transition
    # someone may have to justify -- it moves the working tree out from under
    # every other agent on this checkout, which is why it runs behind the
    # liveness blockers and the branch-transition barrier (mcp_server.py
    # ai_git.switch). DECISION, so a burst of chatter can never count-cap away
    # the answer to "who moved the branch, and when".
    "git_switch": _D,
    # POSITIVE terminal evidence that a native edit invocation ended, keyed by
    # the full native identity (host_session_id + tool_use_id + tool_name).
    # DECISION rather than MECHANICAL/OPERATIONAL beside the witness rows
    # above, and the COUNT CAP is what decides it:
    #   - r0b ruled that lease absence / TTL is NOT quiescence, so this receipt
    #     is the ONLY thing that may retire a native witness. If a burst of
    #     chatter could evict it, an invocation that demonstrably ENDED reads
    #     as still open and the retry gate refuses a rerun that should unlock
    #     -- a refusal invented by retention, which is precisely what the
    #     uncapped DECISION tier exists to prevent (cf. `memory_read_receipt`).
    #   - Age-pruned (so not FORENSIC): readers compare its rowid against their
    #     own window, and no window outlives the 15-minute preimage TTL, so the
    #     row has no reader past its horizon. Keeping one row per completed
    #     edit invocation forever is the 2026-08-23 trap in a new costume.
    "edit_invocation_terminal": _D,
    # RFC 0003 §16.1 application-boundary events (#1096), pinned ahead of their
    # emitters (see PENDING_EVENT_KINDS). The RFC states the class for exactly
    # these: "Mutation apply/refuse/outcome and consequential confirmation
    # events are DECISION retention and are not count-capped." A refused
    # call -- mutation or read -- is ONE kind, `app_call_refused` (r0b2).
    # `app_mutation_proposed`: the proposal opens the consequential pending
    # approval / pins preview state. The four kinds the RFC leaves unstated
    # are classified by the r0b2 ruling on #1096: an invalid backend response
    # and an identity link are decisions someone may have to justify; a
    # completed read and an entitlement refresh are ordinary lifecycle.
    "app_call_refused": _D,
    "app_mutation_proposed": _D,
    "app_confirmation_consumed": _D,
    "app_mutation_applied": _D,
    "app_mutation_outcome_unknown": _D,
    "app_backend_invalid_response": _D,
    "app_identity_linked": _D,
    "app_read_completed": _O,
    "app_entitlement_refreshed": _O,
    # ── FORENSIC ────────────────────────────────────────────────────
    ...
    # #1060: every gate-surface push through the org credential — who pushed,
    # which refspec, what result. The commit author does not prove the pusher
    # under a shared token; this row does, so it is never pruned.
    "git_push": _F,
    ...
    # Agent-to-agent approval authority (r0b ruling, 2026-09-12). ONE kind for
    # all five lifecycle writes — request, decide, expire, consume, rebind — the
    # verb is carried by `action_kind`, and a second kind is only worth minting
    # when RETENTION differs, which it does not here.
    #
    # It was emitted as `conductor_comms` (OPERATIONAL: 7 days, count-capped),
    # so conductor chatter could evict the audit record of an approval decision.
    # Placed beside `escalation_requested/approved/denied/consumed` above
    # deliberately: those are the same fact in the escalation domain and are all
    # FORENSIC. DECISION's 90 days would be wrong for the same reason it is
    # wrong for `escalation_approved` — an authorisation someone may have to
    # justify does not stop needing justification on day 91.
    #
    # NOT rescued by `_FORENSIC_PREFIXES`, so it is pinned by name in
    # `_REGISTRY_ONLY_FORENSIC` (tests/audit/...registry.py). No `approval_`
    # prefix was added: a prefix would silently forensic-ify any future
    # `approval_*` kind, which is the catch-all the prefix-net test forbids.
    "approval_authority": _F,
...
#: Kinds classified AHEAD of their emitter, each mapped to its owner ticket.
#: The ONLY sanctioned exemption from the registry's dead-entry check: it lets
#: a deploy carry the classification before the emitter lands (so the VPS
#: "unclassified event_kind" gate never trips), and
#: ``test_a_pending_kind_graduates_once_emitted`` fails as soon as an emitter
#: exists, so an entry cannot linger here once real.
PENDING_EVENT_KINDS: dict[str, str] = {
    # consul_turn_mirror / consul_park_return / consul_stop_outcome /
    # consul_peer_absent GRADUATED: emitted by receive_lease_stop (#1061);
    # consul_park_enter GRADUATED: emitted by conductor_comms.xaacp_wait_next.
    "consul_peer_verdict": "#1061 lane-consul-stop (emitted when the peer rules on a turn_end mirror)",
    "app_call_refused": "#1096 RFC 0003 §16.1 (emitter lands with the app_invoke path)",
    "app_mutation_proposed": "#1096 RFC 0003 §16.1 (emitter lands with two-step confirmation)",
    "app_confirmation_consumed": "#1096 RFC 0003 §16.1 (emitter lands with two-step confirmation)",
    "app_mutation_applied": "#1096 RFC 0003 §16.1 (emitter lands with the app_invoke path)",
    "app_mutation_outcome_unknown": "#1096 RFC 0003 §16.1 (emitter lands with the app_invoke path)",
    "app_backend_invalid_response": "#1096 RFC 0003 §16.1 (emitter lands with the app_invoke path)",
    "app_read_completed": "#1096 RFC 0003 §16.1 (emitter lands with the app_invoke path)",
    "app_entitlement_refreshed": "#1096 RFC 0003 §16.1 (emitter lands with delegated entitlement)",
    "app_identity_linked": "#1096 RFC 0003 §16.1 (emitter lands with identity linking)",
}
