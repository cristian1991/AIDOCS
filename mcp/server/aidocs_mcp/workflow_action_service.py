...
class WorkflowActionService:
    ...
    def _compile_rule(
        ...
        match = re.match(r"^(?:before|after|when)\s+(.+?),\s*(.+)$", normalized_rule, flags=re.IGNORECASE)
        if match is None:
            return [], None, [{"rule": rule_text, "reason": "expected format: 'Before/After <trigger>, <action>'."}]
        ...
        # Prefix with before_ if the rule says "Before"
    ...
    def _compile_trigger(self, trigger_phrase: str) -> str | None:
        ...
        if any(token in text for token in ("each push", "every push", "git push", "after push", "a push", "pushing")) or compact in {"push", "pushing to git"}:
            return "git_push"
        if any(token in text for token in ("each commit", "every commit", "git commit", "committing", "commiting")) or compact in {"commit", "committing to git", "commiting to git"}:
            return "git_commit"
        if any(token in text for token in ("committing/pushing", "commiting/pushing", "commit/push", "commit and push", "committing or pushing")):
    ...
    def is_action_satisfied(self, project_root: Path, action_id: str) -> bool:
        ...
        conn = self._satisfaction_db(project_root)
        try:
            row = conn.execute("SELECT 1 FROM satisfied_actions WHERE action_id = ?", (action_id,)).fetchone()
