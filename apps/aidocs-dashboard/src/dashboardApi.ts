...
export type OperatorLoginResult = {
  ...
  expires_at?: string;
  cached?: boolean;
  // On refusal the CLI returns ok:false with a reason/message (never a token).
  reason?: string;
  message?: string;
};

/** Password sign-in (email+password). The bearer token never crosses back to
 *  the frontend — the Rust side seeds its in-process + machine cache. */
export async function operatorLogin(
  email: string,
  password: string,
  projectRoot?: string,
): Promise<OperatorLoginResult> {
  return invoke<OperatorLoginResult>("operator_login", { projectRoot, email, password });
}
