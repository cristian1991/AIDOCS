...
def evaluate_command_read_policy(
    ...
    def _reason(path: str, shape: str, decision: GateDecision) -> str:
        return (
            ...
            f"[{decision.level}]. "
            + (decision.reason or "")
            + " Reading file content by running a command is "
            "subject to the same policy as the Read tool — use "
            "ai_find / ai_bundle / ai_get_lines for indexed "
            "source, and don't print secrets to stdout."
        ).strip()
