...
class SkillStore:
    ...
    def _built_in_dir(self) -> Path:
        """Resolve the bundled-skills root across layout variants.

        Doctrine (2026-05-29): the original `parents[3] / core / .skills`
        rule worked on the AIDOCS dev layout
          (AIDOCS/mcp/server/aidocs_mcp/skill_store.py
           → parents[3] = AIDOCS/
           → AIDOCS/core/.skills/  ✓)
        but FAILED everywhere else:
          - VPS test-run layout (parents[3] = /home/  → /home/core/.skills ✗)
          - Deployed gate layout (parents[3] = /home/ → same miss)
        The deploy script ships core/.skills/ to /home/app/core/ on the
        VPS — so we walk candidate paths and return the first that
        contains the expected files. AIDOCS_BUILT_IN_SKILLS_DIR env
        var overrides everything for operators with custom layouts.
        Why: the prior single-path rule meant the bundled-provider
        tests would never have passed in any deployment scenario;
        they only worked in local-dev test runs. The deploy gate's
        Gate 2b correctly surfaced the gap on 2026-05-29.
        Apply: callers don't change; only the resolution strategy
        does. First found wins; if none match, the original
        parents[3] path is returned as the legacy default (matches
        the old behavior for callers that build SkillRecord rows
        with a path they later resolve themselves).
        """
        import os
        ...
        candidates = [
            here.parents[3] / "core" / ".skills",  # legacy local dev
            here.parents[2] / "core" / ".skills",  # alt test layout
            here.parents[1] / "core" / ".skills",  # tight layout
            Path("/home/app/core/.skills"),  # production VPS path
            Path.home() / ".aidocs" / "skills",  # ~/.aidocs install
        ]
        for cand in candidates:
            try:
                ...
                continue
        # No candidate hit — return the legacy default so callers
        # that walk it for diagnostics get the original path. Empty
        # bundled provider is the natural consequence (load_bundled_
        # provider_skills handles the not-a-dir case gracefully).
        return here.parents[3] / "core" / ".skills"
    ...
    def read_role(self, skill_id: str) -> dict[str, object] | None:
        ...
        return {
            ...
            "name": str(row["name"]),
            "kind": str(row["kind"]),
            "content_text": self._normalize_role_text(str(row["content_text"] or "")),
        }
    ...
    def empire_skill_read(
        ...
        *,
        ...
        unless ``sovereign_authority=True`` (caller has the king-NLP grant
        per king-doctrine §X). Returns the row dict or None if missing.
        """
        import sqlite3
        ...
        # (king directive 2026-05-22): the public surface only serves
    ...
    def empire_skill_upsert(
        ...
        *,
        ...
        require ``sovereign_authority=True`` — minted only via the king's
        NLP grant per king-doctrine §X.
    ...
    def empire_soul_append(
        ...
        *,
        ...
        ``sovereign_authority`` required (king's NLP grant per
        king-doctrine §X). Without authority, refuses on sovereign rows.
        ...
        try:
            ...
            if not sovereign_authority:
                raise PermissionError(
                    ...
                    f"requires sovereign_authority=True (king's NLP "
                    f"grant per king-doctrine §X).",
                )
    ...
    # surface.
    #
    # Per king-doctrine §X grant grammar + §XVII operator-intent
    ...
    def list_skills(self, project_root: Path) -> list[dict[str, object]]:
        ...
        # priority 1) wins for empire-only skills (king-doctrine, scribe,
