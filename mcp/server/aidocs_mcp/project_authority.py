...
def _login_required() -> bool:
    """When security.require_login is set, EVERY flavor must authenticate — the
    dev/solo local-admin passthrough is suppressed and RBAC/auth applies as in
    corpo. Default False (today's behavior); the universal-login rollout flips it
    on. GLOBAL-only. Fails to the default (False) on read error so a config glitch
    never silently strands a local operator mid-rollout."""
    try:
        from .config import get_setting

        return bool(get_setting("security.require_login", default=False))
    except Exception:
        return False


def _passthrough(flavor: str) -> bool:
    """The dev/solo local-admin passthrough is active ONLY when the flavor is a
    local-admin flavor AND login is not required (security.require_login off)."""
    return flavor in LOCAL_ADMIN_FLAVORS and not _login_required()


def is_local_admin_flavor() -> bool:
    return _passthrough(_flavor())
...
def require_cross_project(
    ...
    *,
    ...
    if not _passthrough(flavor) and not _has_perm(
        conductor_root,
        permission,
        host_session_id,
    ):
    ...
    audit(
        ...
        target=str(target_root),
        status="allowed",
        reason=("local_admin_flavor" if _passthrough(flavor) else "rbac_grant"),
    )
    return Decision(
        ok=True,
        flavor=flavor,
