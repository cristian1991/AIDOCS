...
# XAACP cross-session messaging (r0b ruling 8d84fdea-4e9, phase 1). Sending a
# message INTO a session other than the caller's bound one, within the same
# project, needs a grant reaching THAT session -- through the same two-stage
# membership + session-RBAC ladder require_session runs, but with THIS
# permission, not admin.manage_sessions: that one means create / archive /
# transfer sessions and is overpowered for chat. Owning a session grants
# nothing here; reaching another session is its own decision.
PERM_XAACP_MESSAGE_SESSION = "xaacp.message_session"
...
ALL_PERMISSIONS: tuple[Permission, ...] = (
    ...
    Permission(
        PERM_XAACP_MESSAGE_SESSION,
        "Send XAACP messages into another session of the same project "
        "(session-scoped grant on the TARGET session, or project-scoped)",
    ),
...
_ADMIN_PERMS = frozenset(
    {
        ...
        # 8d84fdea-4e9: admin may message across sessions of a project. NOT
        # widened to session_owner -- owning one session is not a licence to
        # reach every other; that is exactly the grant this permission names.
        PERM_XAACP_MESSAGE_SESSION,
