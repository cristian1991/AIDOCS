...
def _delegate(name: str, **kwargs: Any) -> Any:
    ...
    coro = srv.call_tool(name, impl_kwargs)
    try:
...
def _check_confirm(spec: ToolSpec, kwargs: dict) -> dict | None:
    ...
    if spec.confirm != TWO_PHASE:
        return None
    try:
        expected = spec.phrase.format(**kwargs)
    except KeyError:
        expected = spec.phrase
    if str(kwargs.get("confirm_token") or "") != expected:
...
def _json_entry_for(ann) -> dict:
    ...
    # Optional[T] / Union[T, None] — unwrap to the single non-None arm.
...
def ai_lane(
    ...
    return {
        ...
        "_detail": (
            ...
            "(expected: exit, state, summary, control, grant, send, inbox, "
...
@tool(
    ...
    scope="",
    confirm=TWO_PHASE,
    phrase="confirm-clear-freeze {freeze_id}",
    annotations={
...
def admin_clear_freeze(
    ...
    Args:
        ...
        confirm_token: 'confirm-clear-freeze <freeze_id>' (or
            'confirm-clear-freeze session:<session_id>' when only
            session_id is supplied). Omit on the first call to receive
            the token in the confirm_required response.
...
def ai_msg(
    ...
    """Inter-agent message channel — already multi-mode at the impl
    layer (mode='send'|'list'|'read'|'mark_read'|etc.). The registry
    entry forwards verbatim.
    """
    return _delegate(
        "ai_msg",
        mode=mode,
...
def ai_session(
    ...
    """Session lifecycle — create / get / patch / list / connect /
    archive. Multi-mode at the impl layer; registry forwards verbatim.
    """
    return _delegate(
        "ai_session",
        mode=mode,
...
@tool(
    ...
    scope="tier_m_edit",
    confirm=TWO_PHASE,
    phrase="confirm-project-bind {project_root}",
    annotations={
        "readOnlyHint": False,
        "destructiveHint": True,
        "idempotentHint": True,
...
def ai_project(
    ...
    Args:
        ...
        confirm_token: 'confirm-project-bind <project_root>' — omit on the
            first bind call to receive it in the confirm_required response.
...
def ai_seat(
    ...
    """Agent seat lifecycle — claim / release / inspect. Conductor-
    side primitive for binding which agent is at which seat in a
    session.
    """
...
def ai_soul(
    ...
    """Agent soul/personality CRUD — sovereign-only writes; reads
    respect privacy floor (souls are agent-personal, not operator-
    visible by default per RFC 003).
    """
    return _delegate(
        "ai_soul",
        skill_id=skill_id,
...
def ai_todo(
    ...
    """Todo / backlog CRUD — mode='add'|'list'|'update'|'remove'|
    'done'. `scope='task'` for in-session, `scope='session'` for
    durable session-level todos.
    """
    return _delegate(
        "ai_todo",
        mode=mode,
...
def ai_qa(
    ...
    """Unified Q&A channel — mode='ask'|'answer'|'list'|'cancel'|
    'history'. Asking with wait=True blocks until answered or timeout.
    """
    return _delegate(
        "ai_qa",
        mode=mode,
...
def config_set(key: str = "", value: str = "") -> dict:
    """Mutate gate-host config. A remote tenant setting host config
    affects every other tenant on the same gate. `config_get` stays
    gate-exposed (read-only); `config_set` does NOT.
    """
    return _delegate("config_set", key=key, value=value)
...
@tool(surface=LOCAL_ONLY, cls="internal", tier=L, scope="", annotations=_LOCAL_INTERNAL_ANN)
def ai_backlog() -> dict:
    """Local dev surface — surfaces the project's internal TODO list.
    Not an agent-facing primitive.
    """
    return _delegate("ai_backlog")
...
def semantic_search(query: str, limit: int = 10) -> dict:
    ...
    sentence-transformers. Run `semantic_index_sync` first.
    """
...
def related_project_code_search(
    ...
    """Code-text search inside a registered related project."""
...
def related_project_symbol_bundle(name: str, symbol: str) -> dict:
    """Read a symbol bundle from a registered related project."""
...
def related_project_subsystem_bundle(name: str, subsystem: str) -> dict:
    """Read a subsystem bundle from a registered related project."""
...
def related_project_compare_concept(
    ...
    """Compare a concept across a registered related project's
    indexed surfaces. Read-only — never mutates.
    """
...
def ai_insert_lines(
    ...
    Clearer than ai_edit_lines insert mode.
    """
    return _delegate(
        "ai_insert_lines",
        path=path,
...
def ai_replace(
    ...
    'symbol' (index-resolved symbol body rewrite).
    """
    return _delegate(
        "ai_replace",
        mode=mode,
...
def ai_protect(
    ...
    same user (or admin+) can remove it. mode ∈ {protect, unprotect,
    list, status}.
    """
    return _delegate(
        "ai_protect",
...
@tool(
    ...
    scope="tier_m_edit",
    confirm=TWO_PHASE,
    phrase="confirm-palace-maintenance {mode}",
    annotations=_EDIT_ANN_DESTRUCTIVE,
)
