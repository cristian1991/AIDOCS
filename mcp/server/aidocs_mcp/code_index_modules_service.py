...
class CodeIndexModulesService:
    ...
    def _MODULE_MANIFESTS(self) -> dict[str, str]:
        ...
        return dict(manifests) if manifests and isinstance(manifests, dict) else self.store._MODULE_MANIFESTS_DEFAULT
    ...
    def _detect_informal_modules(
        ...
        for child in sorted(project_root.iterdir()):
            ...
            if name_lower in self.store._module_hint_dirs(project_root):
                source_count = sum(1 for _ in child.rglob("*") if _.is_file() and self.store._language_for(_, project_root=project_root) is not None)
                if source_count > 0:
            ...
            for f in child.rglob("*"):
                ...
                if f.is_file() and self.store._language_for(f, project_root=project_root) is not None:
    ...
    def _find_entry_point(self, directory: Path) -> str | None:
        patterns = self._get_entry_point_patterns(getattr(self.store, '_last_project_root', None) or directory)
    ...
    def _stack_from_entry(self, entry: str) -> str | None:
        patterns = self._get_entry_point_patterns(getattr(self.store, '_last_project_root', None) or Path('.'))
    ...
    def get_module_files(self, project_root: Path, module_path: str, limit: int = 200, modified_since_ns: int | None = None) -> list[dict[str, object]]:
        ...
        self.store.init_db(project_root)
        where = "module = ?"
        params: list[object] = [module_path]
