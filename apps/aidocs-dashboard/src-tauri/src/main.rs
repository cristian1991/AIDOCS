...
/// ── THE TRANSPORT SEAM (#180 AIDOCS Remote) ──────────────────────────────
///
/// EVERY AIDOCS operation the dashboard performs should reach the CLI through
/// HERE. That matters for more than tidiness: #180 requires a web or local
/// dashboard to drive an AIDOCS project on ANOTHER of the operator's machines,
/// with execution staying on that device and the gate acting as a relay. When
/// that lands, the local-subprocess call below becomes ONE branch of a dispatch
/// (local subprocess | gate relay to a named device) — and because 62 of the 79
/// blocking commands funnel through this function, that is a change to ONE
/// function rather than 79 call sites.
///
/// MEASURED 2026-07-25 — the 17 commands that BYPASS this seam, and why it
/// matters which is which:
///
///   DEVICE-AFFINE — must NEVER be remoted (they act on the machine in front of
///   the operator; relaying them would be a bug, not a feature):
///       open_url · select_folder · select_skill_file · setup_detect ·
///       setup_install · setup_configure · setup_install_python
///
///   SHOULD BE REMOTEABLE but currently spawn raw Command::new, so each would
///   need its own remote handling — this is the #180 work list:
///       bindings_list · binding_approve · upload_skill ·
///       set_skill_provider_override · tauri_todo_remove · opencode_models ·
///       conductor_start · conductor_send · conductor_stop · conductor_output
///
/// Route those ten through this seam BEFORE building the relay. Retrofitting a
/// second command set afterwards is the expensive path, and the operator's note
/// (2026-07-25) is explicit that remote is coming.
///
/// PREREQUISITE ALREADY MET: all 79 commands now run via
/// tauri::async_runtime::spawn_blocking instead of on the UI thread (#474
/// defect 2). A remote call adds network latency to every invocation, so the old
/// synchronous model — which froze the whole window for ~10s on open — could
/// never have carried a remote transport.
fn run_json_cli(project_root: &Path, args: &[String]) -> Result<Value, String> {
    ...
    // #471: fast-fail while the ladder is in failure backoff — a broken
    // python must not spawn a fresh set of corpses on every UI poll.
    if let Some(at) = *CLI_LADDER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) {
        if at.elapsed() < CLI_FAILURE_BACKOFF {
            return Err(
                "Dashboard bridge is backing off after failed interpreter resolution \
                 (retrying shortly)."
                    .into(),
            );
        }
    }
    let cached = CLI_GOOD_INTERPRETER
        .lock()
        .unwrap_or_else(|p| p.into_inner())
        .clone();
    let from_cache = cached.is_some();
    let mut command_sets: Vec<(String, Vec<String>)> = Vec::new();
    if let Some((program, uses_mcp_args)) = cached {
        // Known-good interpreter from an earlier call: no ladder walk.
        let set_args = if uses_mcp_args { mcp_args.clone() } else { args.to_vec() };
        command_sets.push((program, set_args));
    } else {
        if let Some(py) = resolve_mcp_python() {
            command_sets.push((py, mcp_args.clone()));
        }
        command_sets.push(("python".to_string(), mcp_args.clone()));
        command_sets.push(("py".to_string(), mcp_args.clone()));
    ...
    let mut cached_spawn_err = false;
    for (program, program_args) in command_sets {
        ...
        // #471: hard timebox — a wedged python is killed instead of holding
        // SPAWN_GATE (and thus every later invoke) hostage forever.
        let output = match output_with_timeout(cmd, CLI_HARD_TIMEOUT) {
            Ok(output) => output,
            Err(err) => {
                if err.kind() != std::io::ErrorKind::TimedOut {
                    // Spawn-level failure: the interpreter itself is broken.
                    cached_spawn_err = from_cache;
                }
        ...
        if !output.status.success() {
            // #570: a non-zero exit is how the CLI says NO, not how it says
            // "I failed to start". If stdout parses as a JSON object
            // carrying an "ok" key, the CLI ANSWERED — render it exactly
            // like a successful invocation: cache the interpreter as good
            // and clear the failure backoff, same as the success path
            // below. A typed refusal (ok:false) is a WORKING ladder rung,
            // not a ladder failure — only empty/unparseable stdout is a
            // genuine spawn/bridge failure and falls through to the error
            // path (and the next interpreter) unchanged.
            let stdout_for_parse = String::from_utf8_lossy(&output.stdout);
            if let Some(parsed) = serde_json::from_str::<Value>(&stdout_for_parse)
                .ok()
                .filter(|v| v.get("ok").is_some())
            {
                *CLI_GOOD_INTERPRETER.lock().unwrap_or_else(|p| p.into_inner()) =
                    Some((program.clone(), program != "aidocs"));
                *CLI_LADDER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) = None;
                return Ok(parsed);
            }
        ...
        // Remember the working interpreter; clear any failure backoff.
        *CLI_GOOD_INTERPRETER.lock().unwrap_or_else(|p| p.into_inner()) =
            Some((program.clone(), program != "aidocs"));
        *CLI_LADDER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) = None;
    ...
    if from_cache {
        if cached_spawn_err {
            // The cached interpreter vanished/broke — forget it so the next
            // call re-resolves via the full ladder.
            *CLI_GOOD_INTERPRETER.lock().unwrap_or_else(|p| p.into_inner()) = None;
        }
    } else {
        // Full ladder walked and nothing worked: open the backoff window.
        *CLI_LADDER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) =
...
/// #471 tranche 1 (process churn a): remember the first interpreter that
/// SUCCESSFULLY ran the CLI so later calls go straight to it instead of
/// re-walking the 4-entry fallback ladder — each dead rung is a
/// spawned-and-died python, i.e. visible process churn. The bool records
/// the arg shape: `true` = `-m aidocs_mcp.cli` args, `false` = the bare
/// `aidocs` binary args.
static CLI_GOOD_INTERPRETER: std::sync::Mutex<Option<(String, bool)>> =
    std::sync::Mutex::new(None);

/// #471 tranche 1 (process churn a): when the WHOLE ladder failed, back off
/// briefly before walking it again so a broken python doesn't spawn several
/// corpses per invoke while the UI polls.
static CLI_LADDER_FAILED_AT: std::sync::Mutex<Option<std::time::Instant>> =
    std::sync::Mutex::new(None);
const CLI_FAILURE_BACKOFF: std::time::Duration = std::time::Duration::from_secs(5);

/// #471 tranche 1 (process churn b): hard timebox for one-shot CLI calls.
/// `cmd.output()` used to block with NO timeout while holding SPAWN_GATE —
/// one wedged python froze every later invoke behind the mutex.
const CLI_HARD_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(30);

/// Run a prepared command with a hard timeout, killing the child on expiry.
/// Pipes are drained on threads so a chatty child can't deadlock on a full
/// pipe buffer while we poll `try_wait`.
...
/// Path of the per-machine operator-token cache written by
/// `aidocs operator-login` / `aidocs dashboard-login`
/// (aidocs_mcp/operator_token_resolution.py). Env-overridable via
/// AIDOCS_TOKEN_CACHE; default `~/.aidocs/operator_token.json`. This is the
/// SHARED credential store — reading it here is what lets the dashboard,
/// CLI and hooks ride ONE login (Empire directive 2026-07-17).
fn machine_cache_path() -> Option<PathBuf> {
    if let Some(p) = env::var_os("AIDOCS_TOKEN_CACHE") {
    ...
    let home = env::var_os("HOME")
        ...
        .map(PathBuf::from)?;
    Some(home.join(".aidocs").join("operator_token.json"))
}

/// Read the bearer token from the machine cache file (no expiry math here —
/// use `machine_cached_token_unexpired` when the token's own 30-day life is
/// the question). Returns None when the file is absent/corrupt or holds no
/// token.
fn read_machine_cached_token() -> Option<String> {
    read_machine_cached_row().map(|(token, _expires_at)| token)
}

/// The cached row as `(token, expires_at)`. `expires_at` is the ISO-8601 UTC
/// instant the LOGIN minted (30 days out, #509) and may be empty on a legacy
/// row.
fn read_machine_cached_row() -> Option<(String, String)> {
    let path = machine_cache_path()?;
...
/// Unix seconds for an ISO-8601 UTC `%Y-%m-%dT%H:%M:%SZ` stamp, or None when it
/// is empty/malformed. Hand-rolled (no chrono dependency for one parse): the
/// days-from-civil algorithm, proleptic Gregorian, which is exact for every
/// date this cache can hold.
fn iso_utc_to_unix(stamp: &str) -> Option<f64> {
    let s = stamp.trim().trim_end_matches('Z');
    ...
    Some((days * 86_400 + t[0] * 3600 + t[1] * 60 + t[2]) as f64)
}

/// ISO-8601 UTC `%Y-%m-%dT%H:%M:%SZ` for a unix timestamp — the inverse of
/// `iso_utc_to_unix`, civil-from-days, same proleptic Gregorian algorithm and
/// the same reason for being hand-rolled (no chrono for one format).
fn unix_to_iso_utc(secs: f64) -> String {
...
/// The cached token IF its own 30-day life has not run out (#509).
///
/// This is the FAIL-SOFT answer: it consults only the credential the operator
/// already holds, with no process spawn and no network, so it still answers
/// when the CLI bridge is broken or the machine is offline. A legacy row with
/// no `expires_at` is treated as unexpired — the server-side gate remains the
/// one that refuses a genuinely dead token.
fn machine_cached_token_unexpired() -> Option<String> {
    let (token, expires_at) = read_machine_cached_row()?;
    if expires_at.is_empty() {
        return Some(token);
    }
    match iso_utc_to_unix(&expires_at) {
        Some(exp) if exp <= now_seconds() => None,
        _ => Some(token),
    }
}

/// What an authority said about a token — THREE states, on purpose.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TokenVerdict {
    /// The CLI confirmed an authenticated principal.
    Valid,
    /// The CLI answered, and the answer was NO (expired / revoked / unknown row).
    Revoked,
    /// NOBODY ANSWERED. The bridge could not run the CLI, the interpreter
    /// ladder is in backoff, the process was killed — no evidence either way.
    Unknown,
}

/// Ask the CLI whether this token resolves to an authenticated principal.
///
/// #508 / #509 — WHY THIS IS TRI-STATE AND WHY `Unknown` IS NOT `Revoked`.
///
/// This used to return `bool`, with `Err(_) => false`: any failure to REACH the
/// validator was recorded as "the token is invalid". That is how the operator
/// was locked out of their own machine. Their bridge could not start python
/// ("Dashboard bridge could not run the local AIDOCS CLI … (exit 1)"), so:
///
///   run_json_cli -> Err  ->  "invalid"  ->  resolve_operator_token returns None
///     ->  binding_approve forwards NO token  ->  the CLI refuses
///         `unauthenticated`  ->  "Bind to me", the ONLY in-app recovery route
///         out of an unbound session, could not succeed. Total deadlock.
///
/// Nothing had expired and nothing had been revoked. Invalidation is a POSITIVE
/// act by the authority (removed from project / banned / permissions changed)
/// and must NEVER be inferred by the client from absent or unreachable
/// evidence. So a transport failure is `Unknown`, and callers fall back to the
/// cached token's own 30-day life instead of reporting a signed-out operator.
///
/// The security boundary is untouched: the CLI still authenticates every
/// forwarded token server-side and still fails CLOSED on a genuinely dead one.
/// This function only decides whether we are entitled to WITHHOLD a credential
/// the operator legitimately holds — and a failure to ask is not entitlement.
fn token_verdict(project_root: &Path, token: &str) -> TokenVerdict {
    // --project-root explicitly: same cwd-dependence bug as dashboard_auth_status
    // (2026-07-23). Identity itself is machine-global (#488), so the root no
    // longer selects the ledger; it is still stated rather than inferred.
    let args = vec![
    ...
    match run_json_cli(project_root, &args) {
        Ok(v) => match v.get("authenticated").and_then(|a| a.as_bool()) {
            Some(true) => TokenVerdict::Valid,
            // An affirmative negative from the authority — the one thing that
            // signs the operator out.
            Some(false) => TokenVerdict::Revoked,
            // Answered, but not with a verdict (schema drift / truncated
            // output): that is not an affirmative revocation.
            None => TokenVerdict::Unknown,
        },
        // Could not ask. NOT a verdict. See the doc comment above; do not
        // "simplify" this arm back to `false`.
        Err(_) => TokenVerdict::Unknown,
    }
}

/// Resolve a valid operator token WITHOUT ever minting one (Empire directive
/// 2026-07-17: 1 dashboard = 1 user = bind; #404: local tokens are never
/// auto-minted). Order:
///   1. in-process cache (this dashboard process) — if it still validates;
///   2. the SHARED machine cache file (`~/.aidocs/operator_token.json`),
///      written by `dashboard-login` / `operator-login` — if it validates,
///      seed the in-process cache from it;
///   3. FAIL-SOFT (#508/#509): if no authority could be REACHED, hand back the
///      cached token while its own 30-day life lasts. Offline must not mean
///      signed out, and a broken bridge must not deadlock the recovery route.
///   4. None => the operator is genuinely signed out (an authority said so, or
///      there is no credential at all) and the frontend MUST show LOGIN
///      (no fake-connected state).
fn resolve_operator_token(project_root: &Path) -> Option<String> {
    let mut unreachable = false;
    // 1. in-process cache.
    {
        if let Ok(guard) = OPERATOR_TOKEN_CACHE.lock() {
            if let Some(tok) = guard.as_ref() {
                match token_verdict(project_root, tok) {
                    TokenVerdict::Valid => return Some(tok.clone()),
                    TokenVerdict::Unknown => unreachable = true,
                    TokenVerdict::Revoked => {}
                }
            }
        }
    }
    // 2. shared machine cache file (the once-login door).
    if let Some(tok) = read_machine_cached_token() {
        match token_verdict(project_root, &tok) {
            TokenVerdict::Valid => {
    ...
    // 3. Nobody could be asked: ride the operator's own unexpired credential.
    if unreachable {
        if let Some(tok) = machine_cached_token_unexpired() {
            return Some(tok);
        }
    }
    // 4. an authority said no, or there is no credential => signed out.
    None
}
...
fn revoke_cached_operator_token(project_root: &Path) {
    ...
    // Revoke whichever token we hold: prefer the in-process one, else the
    // shared machine-cache token (so a logout from a fresh process still
    // revokes the row minted by an earlier CLI/dashboard login).
    let tok = token.or_else(read_machine_cached_token);
    if let Some(tok) = tok {
        let args = vec![
    ...
    // Clear the SHARED machine cache file so no later surface (CLI / hooks /
    // a fresh dashboard) rides a revoked login. Best-effort.
    if let Some(path) = machine_cache_path() {
        let _ = std::fs::remove_file(path);
    }
}

/// Auto-update checker (Empire directive 2026-07-06). CHECK-ONLY: compares the
/// running version against the release channel; never installs anything
/// (aidocs-doctrine §XXIV — installs are signed/verified operator paths).
/// Read-only bridge (run_python_json_command, no CLI mutation) — placed
/// directly before the next #[tauri::command] so the auth-boundary audit's
/// block slice stays tight.
// ── #474 defect 2: the UI-thread freeze ───────────────────────────────────
// MEASURED live 2026-07-25 (process watcher + operator report "froze the whole
// window, couldn't drag, couldn't close"). Startup issued four BLOCKING CLI calls
// in sequence — dashboard-worker, a python -c, then `bindings --json` — spaced
// 1.8s / 2.3s / 5.8s apart. In Tauri a SYNCHRONOUS #[tauri::command] runs ON THE
// MAIN THREAD, so the entire window (not just the page) is dead for the duration:
// ~10s of unresponsive UI on every open.
//
// The app had 93 sync commands and only 2 async ones, and 81 of the sync ones do
// blocking subprocess work — so this is the shape of EVERY command, not one bug.
// These five are the startup path and therefore the freeze the operator sees.
//
// spawn_blocking (not a bare `async fn`): the bodies are genuinely blocking, so
// they belong on the blocking pool. Marking them `async` alone would merely move
// the stall onto an async-runtime worker instead of the UI thread, which hides the
// problem rather than fixing it.
#[tauri::command]
async fn update_check() -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || update_check_blocking())
        .await
        .map_err(|e| format!("update_check worker join failed: {e}"))?
}

fn update_check_blocking() -> Result<Value, String> {
    let script = r#"
import json
from aidocs_mcp.aidocs_service import check_for_update
print(json.dumps({"ok": True, "update": check_for_update()}))
"#;
    let command_sets = vec![
        ("python", vec!["-c".to_string(), script.to_string()]),
        ("py", vec!["-c".to_string(), script.to_string()]),
    ];
    run_python_json_command(command_sets, "Update check")
}

/// Local service watchdog health (health.json + liveness probe + last
/// persisted update-check result), for the dashboard Updates page.
#[tauri::command]
async fn update_status() -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || update_status_blocking())
        .await
        .map_err(|e| format!("update_status worker join failed: {e}"))?
}

fn update_status_blocking() -> Result<Value, String> {
    let script = r#"
import json
from aidocs_mcp.aidocs_service import service_status
print(json.dumps({"ok": True, "service": service_status()}))
"#;
    let command_sets = vec![
        ("python", vec!["-c".to_string(), script.to_string()]),
        ("py", vec!["-c".to_string(), script.to_string()]),
    ];
    run_python_json_command(command_sets, "Update status")
}

#[tauri::command]
async fn dashboard_auth_status(project_root: Option<String>) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || dashboard_auth_status_blocking(project_root))
        .await
        .map_err(|e| format!("dashboard_auth_status worker join failed: {e}"))?
}

fn dashboard_auth_status_blocking(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    // Reflect ACTUAL token validity — NEVER mint (Empire directive 2026-07-17:
    // no fake-connected). This is a READ: it resolves a token INLINE (in-process
    // cache, else the shared machine cache) rather than through the authenticated
    // mutation-resolver, so the auth-boundary seal does not (correctly) treat it
    // as an authenticated mutation. A token minted by `aidocs operator-login` /
    // `dashboard-login` in another surface is honored here.
    let resolved = OPERATOR_TOKEN_CACHE
        .lock()
        .ok()
        .and_then(|g| g.clone())
        .or_else(read_machine_cached_token);
    match resolved {
        Some(tok) => {
            // --project-root EXPLICITLY (fixed 2026-07-23). Without it the CLI
            // resolves the root from its own cwd, so the SAME valid token read
            // as authenticated from the project directory and NOT authenticated
            // from anywhere else — the identity DB it looked in depended on
            // where the app happened to be launched from. The operator signed
            // in successfully and was still shown the login form.
            let args = vec![
                "dashboard-auth-status".to_string(),
                "--operator-token".to_string(),
                tok,
...
#[tauri::command]
async fn dashboard_logout(project_root: Option<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || dashboard_logout_blocking(project_root))
        .await
        .map_err(|e| format!("dashboard_logout worker join failed: {e}"))?
}

fn dashboard_logout_blocking(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    revoke_cached_operator_token(&root);
    Ok(serde_json::json!({"ok": true, "logged_out": true}))
}

// `operator_login` REMOVED 2026-08-31 (operator ruling).
//
// It was the app's password door: `dashboard-login --email --password`, which
// authenticates against the LOCAL bcrypt store, making this machine an identity
// authority — the second source of truth #207 forbids and #516 removed at real
// cost. Operator ruling 2026-08-31: "there is no 'locally created user' — users
// are stored on codenexus.cloud, not logged in no access to aidocs"; "having a
// local operator makes the whole process swiss cheese".
//
// It also had no caller: the desktop login became CodeNexus-only at #507
// (DesktopLoginView), and the last UI that reached it — WebmcpModePanel's
// "WebAgent access" modal — was the very defect this change removes. A live
// authenticated entry point with no user is pure attack surface.
//
// THE CLI's `--method password` on-ramp STAYS (cli.py::_login_core). It is the
// PRODUCER of the machine-cached device session for CI, headless installs and
// dashboard binding approval; retiring it broke TestBindingApprovalDeadlock, a
// real lockout guard, and was reverted. Retiring a CONSUMER needs no
// substitute; retiring a PRODUCER does. This was the consumer.

/// Complete the OAuth sign-in IN THE KERNEL, then stamp the local operator token.
///
/// WHY THE EXCHANGE MOVED HERE (2026-07-22): the desktop app demanded TWO
/// logins. The CodeNexus sign-in wrote only a webview `localStorage` record, so
/// `dashboard_auth_status` — which reads the operator-token cache — stayed false
/// and bounced the operator to a local password form that their cloud identity
/// could never satisfy. #207 §3 names a codenexus-authenticated session a valid
/// principal, and its absorbed #420/#421 require that such a login STAMP a local
/// token. To do that safely the principal must be attested by the AUTHORITY, so
/// the code→token exchange happens here, over TLS, and the email is read out of
/// the gate's own response. It never passes through the webview, which is why
/// the mint can trust it (#404: no token without a verified principal).
///
/// The local stamp is BEST-EFFORT and never fails the cloud sign-in: losing a
/// good gate session because a local mint hiccuped is the exact
/// discard-a-valid-login bug this whole change exists to end. The outcome is
/// reported instead, so the UI can say which half succeeded.
#[tauri::command]
async fn webmcp_oauth_complete(
    ...
    let form = vec![
        ...
        // Cloned, not moved: the renewal path (#1000) must cache the client
        // this credential was issued to, and the gate fails a rotation closed
        // when the presented client drifts from it.
    ...
    let res = tauri_plugin_http::reqwest::Client::new()
        .post(format!("{}/oauth/token", gate.trim_end_matches('/')))
        .form(&form)
        .send()
        .await
        .map_err(|e| format!("token exchange failed: {e}"))?;
    let status = res.status();
    // `text()` + serde_json, not `json()`: the re-exported reqwest is built
    // without its json feature, and pulling in a second HTTP stack for one
    // parse would cost more than parsing it here.
    let raw = res
        .text()
        .await
        .map_err(|e| format!("could not read token response ({status}): {e}"))?;
    ...
    if !status.is_success() {
        ...
        return Err(format!("token exchange failed ({status}): {desc}"));
    }
    let access = body
        .get("access_token")
        .and_then(|v| v.as_str())
        ...
        .to_string();
    if access.is_empty() {
        return Err("no credential in token response".to_string());
    }
    let email = body
        .get("email")
        .and_then(|v| v.as_str())
    ...
    // THE GATE TOKEN'S OWN LIFETIME, carried instead of discarded.
    //
    // `expires_in` has always been in this response and was never passed on,
    // so the shared cache had no clock for the bearer and had to fall back on
    // the LOCAL token's — which is 30 days, against a gate token measured at
    // about an HOUR (a 2026-08-22 sign-in on the operator's machine cached at
    // 09:14:50 and expired at 10:14:50). That presented a dead bearer as fresh
    // for a month until e5741ad74 gave it a separate field; this fills that
    // field in with the truth rather than leaving it unproven.
    //
    // ABSENT STAYS ABSENT (#627): no `expires_in` means no stamp, never a
    // guessed default. The Python side reads an unrecorded expiry as UNPROVEN
    // and lets the live call decide, which is the honest answer when the
    // authority did not say.
    let gate_expires_at = body
        .get("expires_in")
        .and_then(|v| v.as_f64())
        .filter(|s| *s > 0.0)
        .map(|s| unix_to_iso_utc(now_seconds() + s))
        .unwrap_or_default();

    // THE UNATTENDED WAY BACK (#1000), carried instead of handed only to the
    // webview. The gate has returned a refresh credential from this exchange
    // since #92 and the line below has always read it out — but it went ONLY
    // into the response this command returns, i.e. into `webAuth`, which has
    // been silently renewing the BROWSER's session with it for months. The
    // machine cache the daemon reads got nothing, so the operator's Dashboard
    // tab stayed signed in while the sitter, the lane workers and every
    // background reconcile died on the hour with no way back but a human at
    // this same browser flow. Measured 2026-09-04: four lapses in one session.
    //
    // The scope rides along because it is the BASELINE the hourly permission
    // recheck compares its fresh answer against ("on refresh it needs to check
    // if any perms changed"), and the client id because the gate fails the
    // rotation closed when the presented client drifts from the one the
    // refresh credential was issued to.
    let gate_refresh = body
        .get("refresh_token")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string();
    ...
    // Stamp the LOCAL operator token from this gate-attested principal.
    let (stamped, reason) = if email.is_empty() {
        // The gate did not name the principal, so there is nothing to attest to.
        // Fail LOUDLY in the payload rather than minting against a guess.
        (false, "gate_returned_no_email".to_string())
    } else {
        let root2 = root.clone();
        let access2 = access.clone();
        let email2 = email.clone();
        let expires2 = gate_expires_at.clone();
        let refresh2 = gate_refresh.clone();
        let scope2 = gate_scope.clone();
        let client2 = client_id.clone();
        let stamp = tauri::async_runtime::spawn_blocking(move || {
            let mut args = vec![
                "dashboard-login-oauth".to_string(),
                ...
                "--gate-url".to_string(),
                gate.trim_end_matches('/').to_string(),
                "--emit-token".to_string(),
                "--json".to_string(),
            ];
            // Only when the authority actually said so — an omitted flag is
            // the absence, and the CLI treats it as such.
            if !expires2.is_empty() {
                args.push("--gate-token-expires-at".to_string());
                args.push(expires2);
            }
            // ABSENT STAYS ABSENT (#627): an omitted flag is the absence, and
            // the CLI treats it as such — the renewal path then reports
            // "nothing to renew from" rather than guessing at a credential.
            if !refresh2.is_empty() {
                args.push("--gate-refresh-token".to_string());
                args.push(refresh2);
                if !client2.is_empty() {
                    args.push("--gate-client-id".to_string());
                    args.push(client2);
                }
                if !scope2.is_empty() {
                    args.push("--gate-scope".to_string());
                    args.push(scope2);
                }
            }
            run_json_cli_checked(&root2, &args)
        })
        .await
        .map_err(|e| format!("local stamp task failed: {e}"))?;
        match stamp {
            Ok(v) => {
    ...
    // The bearer goes back to the webview (it drives gate tool calls); the LOCAL
    // operator token never does — it stays in the kernel + machine cache.
    Ok(serde_json::json!({
        "access_token": access,
        "refresh_token": gate_refresh,
...
/// Sign out: revoke the token row + clear BOTH the in-process and the shared
/// machine cache. Alias-grade sibling of dashboard_logout with an explicit
/// operator-facing name for the login view.
#[tauri::command]
async fn operator_logout(project_root: Option<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || operator_logout_blocking(project_root))
        .await
        .map_err(|e| format!("operator_logout worker join failed: {e}"))?
}

fn operator_logout_blocking(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    revoke_cached_operator_token(&root);
    Ok(serde_json::json!({"ok": true, "logged_out": true}))
}
...
/// The ledger contains host-session and operator identifiers, so this read is
/// authenticated just like binding approval; login/status remain the on-ramp.
#[tauri::command]
async fn bindings_list(project_root: Option<String>) -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(move || bindings_list_blocking(project_root))
        .await
        .map_err(|e| format!("bindings_list worker join failed: {e}"))?
}

fn bindings_list_blocking(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let mut args = vec!["bindings".to_string(), "--json".to_string()];
    if let Some(tok) = resolve_operator_token(&root) {
...
/// Approve a pending binding ("Bind to me"): rides the resolved operator token
/// through the SAME audited approve path the CLI uses. Fail-closed — with no
/// valid token the CLI refuses (unauthenticated) and we surface that.
#[tauri::command]
async fn binding_approve(
    project_root: Option<String>,
    binding_id: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || binding_approve_blocking(project_root, binding_id))
        .await
        .map_err(|e| format!("binding_approve worker join failed: {e}"))?
}

fn binding_approve_blocking(
    project_root: Option<String>,
    binding_id: String,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let mut args = vec![
        "dashboard-binding-approve".to_string(),
    ...
    // Authenticated mutation → checked runner: an unauthenticated / refused
    // approve ({ok:false, blocked_by:operator_auth}) surfaces as an Err the
    // Bindings panel renders, never a false success (auth-boundary seal).
    run_json_cli_checked(&root, &args)
}
...
async fn load_dashboard(
    ...
    tauri::async_runtime::spawn_blocking(move || load_dashboard_blocking(project_root, session_id, live_only))
        .await
        .map_err(|e| format!("load_dashboard worker join failed: {e}"))?
}

fn load_dashboard_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    live_only: Option<bool>,
) -> Result<Value, String> {
    ...
    let session = session_id
        ...
        .map(str::trim)
        .filter(|s| !s.is_empty());
    // #210 slice-loads: on-change refreshes request ONLY the live sections.
    ...
    if let Ok(v) = dash_worker_snapshot(&root, session, live) {
        return Ok(v);
    }
    // One-shot fallback has no slice mode — a FULL snapshot is a superset,
    // so the frontend's merge path simply replaces instead.
    let mut args = vec![
        "dashboard".to_string(),
        root.to_string_lossy().to_string(),
        "--json".to_string(),
...
/// #471 tranche 1 (process churn c): when the worker died/was killed, don't
/// respawn again within this window — a boot-crashing worker previously
/// turned every snapshot call into a spawn→die loop.
static DASH_WORKER_FAILED_AT: std::sync::Mutex<Option<std::time::Instant>> =
    std::sync::Mutex::new(None);
const DASH_WORKER_BACKOFF: std::time::Duration = std::time::Duration::from_secs(5);

/// Last good worker snapshot, served (flagged degraded) during the backoff
/// window so the UI keeps rendering instead of erroring or re-spawning.
static DASH_LAST_GOOD_SNAPSHOT: std::sync::Mutex<Option<Value>> = std::sync::Mutex::new(None);
...
fn kill_dash_worker(slot: &mut Option<DashWorker>) {
    if let Some(mut w) = slot.take() {
        let _ = w.child.kill();
        let _ = w.child.wait();
        // #471: every kill marks the failure instant so dash_worker_snapshot
        // backs off instead of respawning immediately. (Also set on the clean
        // exit path — harmless, the process is going away.)
        *DASH_WORKER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) =
            Some(std::time::Instant::now());
    }
}

fn dash_worker_snapshot(
    root: &Path,
    session_id: Option<&str>,
    live_only: bool,
) -> Result<Value, String> {
    ...
    if guard.is_none() {
        // #471: failure backoff — if the worker died within the window, serve
        // the last good snapshot (flagged) instead of respawning into the same
        // crash. No cache yet (boot crash) → Err → caller's one-shot fallback.
        let recent_failure = DASH_WORKER_FAILED_AT
            .lock()
            .unwrap_or_else(|p| p.into_inner())
            .map(|at| at.elapsed() < DASH_WORKER_BACKOFF)
            .unwrap_or(false);
        if recent_failure {
            let cached = DASH_LAST_GOOD_SNAPSHOT
                .lock()
                .unwrap_or_else(|p| p.into_inner())
                .clone();
            if let Some(mut snapshot) = cached {
                if let Some(obj) = snapshot.as_object_mut() {
                    obj.insert("worker_degraded".into(), Value::Bool(true));
                }
                return Ok(snapshot);
            }
            return Err("dashboard worker in failure backoff (no cached snapshot)".into());
        }
        match spawn_dash_worker() {
            Ok(w) => *guard = Some(w),
            Err(e) => {
                *DASH_WORKER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) =
                    Some(std::time::Instant::now());
        ...
        match recv {
            Ok(line) => {
                ...
                if v.get("ok") == Some(&Value::Bool(true)) {
                    ...
                    // #471: remember the last good FULL snapshot (served during
                    // backoff) and clear the failure marker — the worker is
                    // demonstrably healthy. live_only slices are subsets and
                    // must not shadow a full snapshot in the cache.
                    if !live_only {
                        *DASH_LAST_GOOD_SNAPSHOT.lock().unwrap_or_else(|p| p.into_inner()) =
                            Some(result.clone());
                    }
                    *DASH_WORKER_FAILED_AT.lock().unwrap_or_else(|p| p.into_inner()) = None;
                    return Ok(result);
                }
                return Err(v
                    .get("error")
...
async fn approve_escalation(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || approve_escalation_blocking(project_root, request_id, approver_email, reason))
        .await
        .map_err(|e| format!("approve_escalation worker join failed: {e}"))?
}

fn approve_escalation_blocking(
    project_root: Option<String>,
    request_id: String,
    approver_email: String,
    reason: Option<String>,
...
async fn deny_escalation(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || deny_escalation_blocking(project_root, request_id, approver_email, reason))
        .await
        .map_err(|e| format!("deny_escalation worker join failed: {e}"))?
}

fn deny_escalation_blocking(
    project_root: Option<String>,
    request_id: String,
    approver_email: String,
    reason: Option<String>,
...
// ── War 0 (2026-07-13): freeze surfacing + recovery — desktop twins of the
// gate ops `freeze_list` / `escalation_list` / `clear_freeze` (anti-drift seam
// law: same command names, same response shapes as
// outer_gate_dashboard_reads.freeze_list/escalation_list and
// outer_gate_freeze_recovery.clear_freeze). Reads are pure rusqlite over the
// project's identity DB (no python spawn, same pattern as memory_kg_list);
// the clear shells the canonical CLI (`aidocs admin clear-freeze`), which
// clears through the ONE audited ClearFreezeService — no second clearing
// implementation.

fn identity_db(project_root: &Path) -> Result<rusqlite::Connection, String> {
    let db = project_root
        .join(".MEMORY")
        .join(".index")
        .join("aidocs_identity.sqlite3");
    if !db.is_file() {
        return Err("no identity db".into());
    }
...
#[tauri::command]
fn freeze_list(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let conn = match identity_db(&root) {
        Ok(c) => c,
        Err(_) => return Ok(serde_json::json!({ "ok": true, "items": [] })),
    };
    let mut stmt = match conn.prepare(
        "SELECT request_id, session_id, kind, fingerprint_phrase, frozen_at,
                COALESCE(expires_at,'') AS expires_at,
                COALESCE(user_id,'') AS user_id,
                COALESCE(agent_context_id,'') AS agent_context_id
         FROM session_freeze",
    ) {
        Ok(s) => s,
        Err(_) => return Ok(serde_json::json!({ "ok": true, "items": [] })),
    ...
    let rows = stmt
        .query_map([], |r| {
            let expires: String = r.get(5)?;
            Ok(serde_json::json!({
                "freeze_id": r.get::<_, String>(0)?,
                "request_id": r.get::<_, String>(0)?,
                "session_id": r.get::<_, String>(1)?,
                "kind": r.get::<_, String>(2)?,
                "fingerprint_phrase": r.get::<_, String>(3)?,
                "frozen_at": r.get::<_, String>(4)?,
                "expires_at": if expires.is_empty() { Value::Null } else { Value::String(expires) },
                "user_id": r.get::<_, String>(6)?,
                "agent_context_id": r.get::<_, String>(7)?,
            }))
        })
        .map_err(|e| e.to_string())?
...
#[tauri::command]
fn escalation_list(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let conn = match identity_db(&root) {
        Ok(c) => c,
        Err(_) => return Ok(serde_json::json!({ "ok": true, "items": [] })),
    ...
    let mut stmt = match conn.prepare(
        "SELECT request_id, COALESCE(requester_label,'') AS requester_label,
                COALESCE(requester_user_id,'') AS requester_user_id,
                COALESCE(session_id,'') AS session_id,
                COALESCE(task_id,'') AS task_id,
                COALESCE(plan_path,'') AS plan_path,
                COALESCE(gate_permission,'') AS gate_permission,
                COALESCE(gate_phrase,'') AS gate_phrase,
                COALESCE(sticky,0) AS sticky,
                COALESCE(created_at,'') AS created_at,
                COALESCE(expires_at,'') AS expires_at
         FROM rbac_escalations WHERE status = 'pending'",
    ) {
        Ok(s) => s,
        Err(_) => return Ok(serde_json::json!({ "ok": true, "items": [] })),
    };
    let rows = stmt
        .query_map([], |r| {
            Ok(serde_json::json!({
                "request_id": r.get::<_, String>(0)?,
                "requester_label": r.get::<_, String>(1)?,
                "requester_user_id": r.get::<_, String>(2)?,
                "session_id": r.get::<_, String>(3)?,
                "task_id": r.get::<_, String>(4)?,
                "plan_path": r.get::<_, String>(5)?,
                "gate_permission": r.get::<_, String>(6)?,
                "gate_phrase": r.get::<_, String>(7)?,
                "sticky": r.get::<_, i64>(8)? != 0,
                "created_at": r.get::<_, String>(9)?,
                "expires_at": r.get::<_, String>(10)?,
            }))
        })
        .map_err(|e| e.to_string())?
        .filter_map(Result::ok)
...
async fn clear_freeze(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || clear_freeze_blocking(project_root, freeze_id, reason, approver_email))
        .await
        .map_err(|e| format!("clear_freeze worker join failed: {e}"))?
}

fn clear_freeze_blocking(
    project_root: Option<String>,
    freeze_id: String,
    reason: String,
    approver_email: Option<String>,
    ...
    let root = resolve_project_root(project_root)?;
    let trimmed_id = freeze_id.trim();
    if trimmed_id.is_empty() {
        return Err("freeze_id is required".to_string());
    }
    let trimmed_reason = reason.trim();
    if trimmed_reason.is_empty() {
    ...
    if let Some(email) = approver_email {
        let trimmed_email = email.trim();
        if !trimmed_email.is_empty() {
            args.push("--approver-email".to_string());
            args.push(trimmed_email.to_string());
        }
    }
    // Same contract as approve_escalation: attach the operator token so the
    // admin-only CLI authorizes the bridge call; a refused clear surfaces as
    // a UI error instead of a silent no-op.
    if let Some(token) = resolve_operator_token(&root) {
        args.push("--operator-token".to_string());
        args.push(token);
    }
...
#[tauri::command]
async fn list_managed_projects(project_root: Option<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || list_managed_projects_blocking(project_root))
        .await
        .map_err(|e| format!("list_managed_projects worker join failed: {e}"))?
}

fn list_managed_projects_blocking(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    Ok(serde_json::json!({
        "ok": true,
...
async fn save_config_setting(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || save_config_setting_blocking(project_root, setting_path, value, scope, session_id, reason))
        .await
        .map_err(|e| format!("save_config_setting worker join failed: {e}"))?
}

fn save_config_setting_blocking(
    project_root: Option<String>,
    setting_path: String,
    value: Value,
    scope: Option<String>,
...
#[tauri::command]
async fn batch_config_settings(project_root: Option<String>, operations: Value) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || batch_config_settings_blocking(project_root, operations))
        .await
        .map_err(|e| format!("batch_config_settings worker join failed: {e}"))?
}

fn batch_config_settings_blocking(project_root: Option<String>, operations: Value) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
#[tauri::command]
async fn delete_config_setting(
    project_root: Option<String>,
    setting_path: String,
    scope: Option<String>,
    session_id: Option<String>,
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || delete_config_setting_blocking(project_root, setting_path, scope, session_id))
        .await
        .map_err(|e| format!("delete_config_setting worker join failed: {e}"))?
}

fn delete_config_setting_blocking(
    project_root: Option<String>,
    setting_path: String,
    scope: Option<String>,
    session_id: Option<String>,
...
#[tauri::command]
async fn load_toml_documents(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || load_toml_documents_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("load_toml_documents worker join failed: {e}"))?
}

fn load_toml_documents_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
async fn save_toml_document(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || save_toml_document_blocking(project_root, session_id, relative_path, content))
        .await
        .map_err(|e| format!("save_toml_document worker join failed: {e}"))?
}

fn save_toml_document_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    relative_path: String,
    content: String,
...
#[tauri::command]
async fn skill_scan_results(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || skill_scan_results_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("skill_scan_results worker join failed: {e}"))?
}

fn skill_scan_results_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
async fn toggle_skill(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || toggle_skill_blocking(project_root, session_id, skill_id, enabled))
        .await
        .map_err(|e| format!("toggle_skill worker join failed: {e}"))?
}

fn toggle_skill_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    skill_id: String,
    enabled: bool,
...
async fn delete_skill(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || delete_skill_blocking(project_root, session_id, skill_id))
        .await
        .map_err(|e| format!("delete_skill worker join failed: {e}"))?
}

fn delete_skill_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    skill_id: String,
) -> Result<Value, String> {
...
#[tauri::command]
async fn set_skill_provider_override(
    project_root: Option<String>,
    provider_id: String,
    choice: Option<String>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || set_skill_provider_override_blocking(project_root, provider_id, choice))
        .await
        .map_err(|e| format!("set_skill_provider_override worker join failed: {e}"))?
}

fn set_skill_provider_override_blocking(
    project_root: Option<String>,
    provider_id: String,
    choice: Option<String>,
) -> Result<Value, String> {
...
async fn upload_skill(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || upload_skill_blocking(project_root, file_name, content))
        .await
        .map_err(|e| format!("upload_skill worker join failed: {e}"))?
}

fn upload_skill_blocking(
    project_root: Option<String>,
    file_name: String,
    content: String,
) -> Result<Value, String> {
    // #677: this used to be a bare `fs::write` of caller-supplied CONTENT under
    // a caller-supplied FILENAME into `.MEMORY/skills/` — no admin wall, no
    // doctrine scan, no audit, no lawbook check, no sanitization. A skill is
    // instruction text every later agent follows as if it were law, and this
    // door never touched the CLI, so #615 and #676 could not see it.
    //
    // Now it is a SHELL over the guarded CLI command, exactly like
    // delete_skill_blocking (#676 precedent). Every guard lives in one place;
    // a guard reimplemented in Rust is a second guard that drifts. The name is
    // basenamed here too, but that is belt-and-braces — the CLI refuses any
    // non-basename outright.
    let root = resolve_project_root(project_root)?;
    let safe_name = Path::new(&file_name)
        .file_name()
        .map(|value| value.to_string_lossy().to_string())
        .unwrap_or_default();
    // The body goes over a temp FILE, not argv: a skill body is arbitrarily
    // long and a command line is not.
    let staged = std::env::temp_dir().join(format!(
        "aidocs-skill-upload-{}-{}.md",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_nanos())
    ...
    fs::write(&staged, &content)
        .map_err(|err| format!("Failed to stage skill content: {}", err))?;
    let mut args = vec![
        "dashboard-upload-skill".to_string(),
        root.to_string_lossy().to_string(),
        "--json".to_string(),
        "--file".to_string(),
        safe_name,
...
#[tauri::command]
async fn select_skill_file() -> Result<Option<String>, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || select_skill_file_blocking())
        .await
        .map_err(|e| format!("select_skill_file worker join failed: {e}"))?
}

fn select_skill_file_blocking() -> Result<Option<String>, String> {
    #[cfg(target_os = "windows")]
    {
...
#[tauri::command]
async fn import_skill_file(project_root: Option<String>, file_path: String) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || import_skill_file_blocking(project_root, file_path))
        .await
        .map_err(|e| format!("import_skill_file worker join failed: {e}"))?
}

fn import_skill_file_blocking(project_root: Option<String>, file_path: String) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let source = std::path::Path::new(&file_path);
    if !source.is_file() {
        return Err(format!("File not found: {}", file_path));
    }
    // #677: this is the door the UI ACTUALLY uses (App.tsx `onUploadSkill`
    // calls `import_skill_file`, not `upload_skill`), and it was the same hole
    // wearing a different verb — `fs::copy` of an operator-chosen file into
    // `.MEMORY/skills/` with no admin wall, no doctrine scan, no audit and no
    // lawbook check. `fs::copy` is not `fs::write`, so the auth-boundary seal's
    // write detector never even saw it.
    //
    // Now it is the SAME shell over the SAME guarded CLI command as
    // `upload_skill`: the chosen file becomes `--content-file`, its basename
    // becomes `--file`, and every guard runs in one place.
...
#[tauri::command]
async fn list_mcp_servers(project_root: Option<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || list_mcp_servers_blocking(project_root))
        .await
        .map_err(|e| format!("list_mcp_servers worker join failed: {e}"))?
}

fn list_mcp_servers_blocking(project_root: Option<String>) -> Result<Value, String> {
...
async fn install_mcp_server(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || install_mcp_server_blocking(project_root, name, command, args, transport))
        .await
        .map_err(|e| format!("install_mcp_server worker join failed: {e}"))?
}

fn install_mcp_server_blocking(
    project_root: Option<String>,
    name: String,
    command: String,
    args: Vec<String>,
...
async fn delete_mcp_server(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || delete_mcp_server_blocking(project_root, name))
        .await
        .map_err(|e| format!("delete_mcp_server worker join failed: {e}"))?
}
...
#[tauri::command]
async fn context_budget_check(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || context_budget_check_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("context_budget_check worker join failed: {e}"))?
}

fn context_budget_check_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
async fn context_compact(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || context_compact_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("context_compact worker join failed: {e}"))?
}

fn context_compact_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
#[tauri::command]
async fn toggle_managed_mode(
    project_root: Option<String>,
    session_id: Option<String>,
    enable: bool,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || toggle_managed_mode_blocking(project_root, session_id, enable))
        .await
        .map_err(|e| format!("toggle_managed_mode worker join failed: {e}"))?
}

fn toggle_managed_mode_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    enable: bool,
) -> Result<Value, String> {
...
#[tauri::command]
async fn metrics_snapshot() -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || metrics_snapshot_blocking())
        .await
        .map_err(|e| format!("metrics_snapshot worker join failed: {e}"))?
}

fn metrics_snapshot_blocking() -> Result<Value, String> {
...
#[tauri::command]
async fn memory_anchor_health(project_root: String) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || memory_anchor_health_blocking(project_root))
        .await
        .map_err(|e| format!("memory_anchor_health worker join failed: {e}"))?
}

fn memory_anchor_health_blocking(project_root: String) -> Result<Value, String> {
...
fn dashboard_live_cursor(project_root: String) -> Result<Value, String> {
    ...
    // lane agents), read DIRECTLY from sqlite in Rust — NO python spawn. Kept
    // as the poll fallback for builds/paths where the watcher push (below)
    // isn't running; both share compute_live_cursor.
    Ok(serde_json::json!({ "ok": true, "cursor": compute_live_cursor(&project_root) }))
}

/// Shared cursor signature: execution-event rowid/count + lane rowid/active.
/// "nodb" when the project has no index DB; "err" (stable, non-flapping) when
/// sqlite can't be opened, so a transient lock never looks like a change.
fn compute_live_cursor(project_root: &str) -> String {
    ...
    format!("{ev_max}:{ev_cnt}:{lane_max}:{lane_active}")
}

/// Generation stamp for the cursor-watcher thread: starting a new watcher
/// bumps it; the old thread notices and exits. One live watcher max.
static CURSOR_WATCHER_GEN: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);

#[tauri::command]
fn start_cursor_watcher(app: tauri::AppHandle, project_root: String) -> Result<Value, String> {
    // Dashboard-war (c) literal: Rust watcher thread → Tauri EVENT PUSH,
    // replacing the frontend's 2s cursor-poll invoke. The thread reads the
    // SAME cheap sqlite cursor (no python, no subprocess) once per second and
    // emits `aidocs://live-cursor` ONLY on change — an idle project emits
    // nothing and the webview does zero work. Re-invoking (project switch)
    // bumps the generation so the previous thread exits on its next tick;
    // app exit kills the thread with the process.
    use std::sync::atomic::Ordering;
    use tauri::Emitter;
    if project_root.trim().is_empty() {
        return Err("project_root required".into());
    }
    let my_gen = CURSOR_WATCHER_GEN.fetch_add(1, Ordering::SeqCst) + 1;
    std::thread::spawn(move || {
        let mut last = String::new();
        loop {
            if CURSOR_WATCHER_GEN.load(Ordering::SeqCst) != my_gen {
                return; // superseded by a newer watcher
            }
            let sig = compute_live_cursor(&project_root);
            if sig != "err" && !sig.is_empty() && !last.is_empty() && sig != last {
                let _ = app.emit(
                    "aidocs://live-cursor",
                    serde_json::json!({ "cursor": sig }),
                );
            }
            if sig != "err" && !sig.is_empty() {
                last = sig;
            }
            std::thread::sleep(std::time::Duration::from_millis(1000));
        }
    });
    Ok(serde_json::json!({ "ok": true, "watching": true }))
}
...
async fn conductor_start(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || conductor_start_blocking(project_root, session_id, backend, model))
        .await
        .map_err(|e| format!("conductor_start worker join failed: {e}"))?
}

fn conductor_start_blocking(
    project_root: Option<String>,
    session_id: String,
    backend: Option<String>,
    model: Option<String>,
...
#[tauri::command]
async fn conductor_send(
    project_root: Option<String>,
    session_id: String,
    message: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || conductor_send_blocking(project_root, session_id, message))
        .await
        .map_err(|e| format!("conductor_send worker join failed: {e}"))?
}

fn conductor_send_blocking(
    project_root: Option<String>,
    session_id: String,
    message: String,
) -> Result<Value, String> {
...
#[tauri::command]
async fn conductor_stop(
    project_root: Option<String>,
    session_id: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || conductor_stop_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("conductor_stop worker join failed: {e}"))?
}

fn conductor_stop_blocking(
    project_root: Option<String>,
    session_id: String,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
#[tauri::command]
async fn conductor_output(
    project_root: Option<String>,
    session_id: String,
    since: Option<f64>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || conductor_output_blocking(project_root, session_id, since))
        .await
        .map_err(|e| format!("conductor_output worker join failed: {e}"))?
}

fn conductor_output_blocking(
    project_root: Option<String>,
    session_id: String,
    since: Option<f64>,
) -> Result<Value, String> {
...
#[tauri::command]
async fn opencode_models() -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || opencode_models_blocking())
        .await
        .map_err(|e| format!("opencode_models worker join failed: {e}"))?
}

fn opencode_models_blocking() -> Result<Value, String> {
...
async fn mcp_registry_search(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || mcp_registry_search_blocking(query, limit, cursor))
        .await
        .map_err(|e| format!("mcp_registry_search worker join failed: {e}"))?
}

fn mcp_registry_search_blocking(
    query: String,
    limit: Option<usize>,
    cursor: Option<String>,
) -> Result<Value, String> {
...
#[tauri::command]
async fn execution_clear_tokens(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || execution_clear_tokens_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("execution_clear_tokens worker join failed: {e}"))?
}

// #885 (2026-08-26): this used to call ExecutionIndexStore.clear_token_usage,
// which ran `DELETE FROM execution_events WHERE payload_json LIKE
// '%tokens_in_estimate%'` against a ledger that is append-only and
// hash-chained -- from a subprocess with no operator, no reason and no
// audit-of-audit. Token estimates live only on the tool call's OUTCOME row, so
// the predicate removed outcomes and left attempts, and crash-recovery then
// read every completed call as an orphaned attempt.
//
// "Clear Tokens" is a DISPLAY reset, so it is now an APPEND: the store writes a
// token_usage_reset watermark and the token queries floor on it. Nothing is
// deleted, so this needs no audit-deletion warrant -- there is no deletion.
fn execution_clear_tokens_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
reset = store.reset_token_usage_counter(
    root,
    session_id=sid,
    reason="dashboard Clear Tokens",
    actor="dashboard",
)
print(json.dumps({"ok": True, "cleared": "tokens", **reset}))
"#;
    run_python_with_args(&root, script, &[&root.to_string_lossy(), &sid])
}

#[tauri::command]
async fn execution_clear_tool_calls(
    project_root: Option<String>,
    session_id: Option<String>,
    reason: Option<String>,
) -> Result<Value, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        execution_clear_tool_calls_blocking(project_root, session_id, reason)
    })
    .await
    .map_err(|e| format!("execution_clear_tool_calls worker join failed: {e}"))?
}

// #885: unlike Clear Tokens, this one REALLY deletes -- whole tool-call
// lifecycles out of the append-only ledger. It used to reach
// ExecutionIndexStore directly, bypassing audit_deletion_law entirely. It now
// goes through the law, which is the single chokepoint; the store refuses any
// caller arriving without the warrant the law issues.
//
// A dashboard-spawned `python -c` carries NO operator context, so the law
// answers auth_required and NOTHING is deleted. That refusal is the correct
// answer, not a bug to route around: erasing audit evidence requires an
// authenticated admin operator, and the surface that has one is the governed
// MCP tool `execution_clear_tool_calls`. The verdict dict is returned verbatim
// so the UI shows the operator exactly which clause refused.
fn execution_clear_tool_calls_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    reason: Option<String>,
) -> Result<Value, String> {
...
import json, sys
from pathlib import Path
from aidocs_mcp.audit_deletion_law import run_audit_deletion
from aidocs_mcp.execution_index_store import ExecutionIndexStore
root = Path(sys.argv[1])
sid = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] else None
...
def _no_audit_of_audit():
    # AUDIT-OF-AUDIT, fail-closed. This subprocess has no hub to record the
    # intent-to-delete through, and a no-op here would be a FAKE audit record.
    # Today the auth clause refuses first so this never runs; if this path ever
    # gains a context, it must fail closed rather than delete unaudited.
    raise RuntimeError(
        "the dashboard subprocess cannot record the deletion intent; "
        "use the governed MCP tool execution_clear_tool_calls"
    )

verdict = run_audit_deletion(
    operation="clear_tool_calls",
    reason=reason,
    ctx=None,
    has_permission=False,
    is_subagent=False,
    record_intent=_no_audit_of_audit,
    record_result=lambda _r: _no_audit_of_audit(),
    deleter=lambda: store.clear_tool_calls(root, session_id=sid),
)
print(json.dumps({"cleared": "tool_calls", **verdict}))
"#;
    run_python_with_args(&root, script, &[&root.to_string_lossy(), &sid, &why])
}
...
#[tauri::command]
async fn clear_degraded_state(
    project_root: Option<String>,
    session_id: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || clear_degraded_state_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("clear_degraded_state worker join failed: {e}"))?
}

fn clear_degraded_state_blocking(
    project_root: Option<String>,
    session_id: String,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
#[tauri::command]
async fn execution_prune_events(
    project_root: Option<String>,
    keep_days: Option<i32>,
    max_events: Option<i32>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || execution_prune_events_blocking(project_root, keep_days, max_events))
        .await
        .map_err(|e| format!("execution_prune_events worker join failed: {e}"))?
}

fn execution_prune_events_blocking(
    project_root: Option<String>,
    keep_days: Option<i32>,
    max_events: Option<i32>,
) -> Result<Value, String> {
...
#[tauri::command]
async fn execution_usage_by_identity(project_root: Option<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || execution_usage_by_identity_blocking(project_root))
        .await
        .map_err(|e| format!("execution_usage_by_identity worker join failed: {e}"))?
}

fn execution_usage_by_identity_blocking(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let script = r#"
import json, sys
from pathlib import Path
...
// only. Phoenix 2026-05-12: renamed from cerberus_* (Empire directive).
#[tauri::command]
async fn tauri_msg_send(
    project_root: Option<String>,
    session_id: Option<String>,
    to_roles: Vec<String>,
    body: String,
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_msg_send_blocking(project_root, session_id, to_roles, body, in_reply_to))
        .await
        .map_err(|e| format!("tauri_msg_send worker join failed: {e}"))?
}

fn tauri_msg_send_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    to_roles: Vec<String>,
    body: String,
...
#[tauri::command]
async fn tauri_msg_inbox(
    project_root: Option<String>,
    session_id: Option<String>,
    role: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_msg_inbox_blocking(project_root, session_id, role))
        .await
        .map_err(|e| format!("tauri_msg_inbox worker join failed: {e}"))?
}

fn tauri_msg_inbox_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    role: String,
) -> Result<Value, String> {
...
async fn delete_session(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || {
        delete_session_blocking(project_root, session_id, reason)
    })
    .await
    .map_err(|e| format!("delete_session worker join failed: {e}"))?
}
...
fn delete_session_blocking(
    ...
    // FORWARD THE REASON. session_deletion_law requires >=6 chars and records it
    // in the audit trail; this argv omitted --reason entirely, so every dashboard
    // delete was refused with blocked_by='reason_required' (masked by the
    // operator-token wall, which fires first). Passed through verbatim and NOT
    // defaulted to a placeholder — an absent reason must be refused by the law,
    // not papered over with a synthetic justification in the audit record.
    if let Some(reason) = reason {
        args.push("--reason".to_string());
        args.push(reason);
    }
...
async fn create_session(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || create_session_blocking(project_root, title, session_id, goal))
        .await
        .map_err(|e| format!("create_session worker join failed: {e}"))?
}
...
async fn connect_session(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || connect_session_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("connect_session worker join failed: {e}"))?
}

fn connect_session_blocking(
    project_root: Option<String>,
    session_id: String,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
...
// ─── backlog / todo bridges (agent surface: ai_backlog + ai_task todo modes, #83) ───
// GOVERNED (2026-07-30, operator charter "dashboard needs to be able to CRUD
// backlog items (RBAC)"). These used to marshal a `python -c` payload that
// imported project_backlog_store and called it DIRECTLY — no principal, no
// permission check, no audit row. The dashboard was not a client of an
// authority, it WAS the authority, which sealed law forbids (no user is exempt
// from the gates, including the operator).
//
// Every action now goes through the `dashboard-backlog` CLI command, which
// authenticates the operator token and checks backlog.read / backlog.write /
// backlog.remove before touching the store. Note run_json_cli (NOT
// run_json_cli_checked): a refusal must reach the UI as a STRUCTURED payload
// carrying reason + required_permission, so the page can render it AS A
// REFUSAL. Collapsing it to an Err(String) — or worse, to an empty list — is
// how a 145-item backlog read as "there is nothing here".

/// Shared argv builder: the action plus the operator token, which every
/// backlog action needs because every backlog action is gated.
fn backlog_cli_args(root: &Path, action: &str, extra: Vec<(&str, String)>) -> Vec<String> {
    let mut args = vec![
        "dashboard-backlog".to_string(),
        "--action".to_string(),
        action.to_string(),
        "--json".to_string(),
    ];
...
async fn tauri_backlog_list(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_backlog_list_blocking(project_root, status, priority, limit))
        .await
        .map_err(|e| format!("tauri_backlog_list worker join failed: {e}"))?
}

fn tauri_backlog_list_blocking(
    project_root: Option<String>,
    status: Option<String>,
    priority: Option<String>,
    limit: Option<i64>,
...
#[tauri::command]
async fn tauri_backlog_get(
    project_root: Option<String>,
    backlog_id: i64,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_backlog_get_blocking(project_root, backlog_id))
        .await
        .map_err(|e| format!("tauri_backlog_get worker join failed: {e}"))?
}

fn tauri_backlog_get_blocking(
    project_root: Option<String>,
    backlog_id: i64,
) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let args = backlog_cli_args(&root, "get", vec![("--id", backlog_id.to_string())]);
    run_json_cli(&root, &args)
}

#[tauri::command]
async fn tauri_backlog_add(
    project_root: Option<String>,
    content: String,
    priority: Option<String>,
    status: Option<String>,
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_backlog_add_blocking(project_root, content, priority, status, kind, tags, session_id))
        .await
        .map_err(|e| format!("tauri_backlog_add worker join failed: {e}"))?
}

fn tauri_backlog_add_blocking(
    project_root: Option<String>,
    content: String,
    priority: Option<String>,
    status: Option<String>,
    ...
    let root = resolve_project_root(project_root)?;
    // NOTE: add is deliberately NOT task-gated — the API dropped that
    // requirement for add/update (operator ruling; _BACKLOG_TASK_GATED_MODES is
    // {remove, merge, unmerge}), so the dashboard must not re-impose it. The
    // session id is therefore not an authority input and is not forwarded.
    let args = backlog_cli_args(
        &root,
        "add",
        vec![
            ...
            ("--priority", priority.unwrap_or_default()),
            ("--status", status.unwrap_or_default()),
            // #573 kind. An EMPTY string is dropped by backlog_cli_args, which
            // is the correct behaviour: KIND_UNSET is the stored default and is
            // NOT storable (validate_kind rejects it), so "leave it alone" and
            // "set it to unrated" must never become the same wire value.
...
async fn tauri_backlog_update(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_backlog_update_blocking(project_root, backlog_id, status, priority, content, kind, tags))
        .await
        .map_err(|e| format!("tauri_backlog_update worker join failed: {e}"))?
}

fn tauri_backlog_update_blocking(
    project_root: Option<String>,
    backlog_id: i64,
    status: Option<String>,
    priority: Option<String>,
    ...
    let args = backlog_cli_args(
        ...
        vec![
            ...
            // #573 kind — see the note in tauri_backlog_add_blocking.
            ("--kind", kind.unwrap_or_default()),
            ("--tags", tags.unwrap_or_default().join(",")),
        ],
    );
...
#[tauri::command]
async fn tauri_backlog_remove(
    project_root: Option<String>,
    backlog_id: i64,
    reason: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_backlog_remove_blocking(project_root, backlog_id, reason))
        .await
        .map_err(|e| format!("tauri_backlog_remove worker join failed: {e}"))?
}

fn tauri_backlog_remove_blocking(
    project_root: Option<String>,
    backlog_id: i64,
    reason: String,
) -> Result<Value, String> {
    ...
    let args = backlog_cli_args(
        &root,
        "remove",
        vec![("--id", backlog_id.to_string()), ("--reason", reason)],
    );
    run_json_cli(&root, &args)
}

#[tauri::command]
async fn tauri_todo_list(
    project_root: Option<String>,
    session_id: Option<String>,
    task_id: Option<String>,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_todo_list_blocking(project_root, session_id, task_id))
        .await
        .map_err(|e| format!("tauri_todo_list worker join failed: {e}"))?
}

fn tauri_todo_list_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
    task_id: Option<String>,
) -> Result<Value, String> {
...
async fn tauri_todo_update(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_todo_update_blocking(project_root, todo_id, status, content, tags, task_id, session_id, urgency))
        .await
        .map_err(|e| format!("tauri_todo_update worker join failed: {e}"))?
}

fn tauri_todo_update_blocking(
    project_root: Option<String>,
    todo_id: i64,
    status: Option<String>,
    content: Option<String>,
...
#[tauri::command]
async fn tauri_todo_remove(
    project_root: Option<String>,
    todo_id: i64,
    reason: String,
) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || tauri_todo_remove_blocking(project_root, todo_id, reason))
        .await
        .map_err(|e| format!("tauri_todo_remove worker join failed: {e}"))?
}

fn tauri_todo_remove_blocking(
    project_root: Option<String>,
    todo_id: i64,
    reason: String,
) -> Result<Value, String> {
...
async fn vocab_upsert_group(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || vocab_upsert_group_blocking(project_root, kind, lang, parent_key, tokens, attrs, parent_mode))
        .await
        .map_err(|e| format!("vocab_upsert_group worker join failed: {e}"))?
}

fn vocab_upsert_group_blocking(
    project_root: Option<String>,
    kind: String,
    lang: String,
    parent_key: String,
...
async fn memory_capture(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || memory_capture_blocking(project_root, kind, content, target_hint))
        .await
        .map_err(|e| format!("memory_capture worker join failed: {e}"))?
}

fn memory_capture_blocking(
    project_root: Option<String>,
    kind: String,
    content: String,
    target_hint: Option<String>,
    ...
    if kind.trim().is_empty() || content.trim().is_empty() {
        return Err("kind + content required".into());
    }
    // Governed write (#200, war d): the SAME doctrine-guarded CLI path the
    // agents' memory_capture tool uses (durability rubric, sovereign guard,
    // sqlite-canonical). user_safe class — the guards live in the CLI/store,
    // not in an operator-token wall.
    let root = resolve_project_root(project_root)?;
    let mut args = vec![
        "dashboard-memory-capture".to_string(),
        "--kind".to_string(),
        kind,
        "--content".to_string(),
        content,
    ...
    if let Some(hint) = target_hint {
        if !hint.trim().is_empty() {
            args.push("--target-hint".to_string());
            args.push(hint);
        }
    }
    run_json_cli_checked(&root, &args)
...
async fn palace_maintenance(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || palace_maintenance_blocking(project_root, mode, dry_run, force, session_id))
        .await
        .map_err(|e| format!("palace_maintenance worker join failed: {e}"))?
}

fn palace_maintenance_blocking(
    project_root: Option<String>,
    mode: Option<String>,
    dry_run: Option<bool>,
    force: Option<bool>,
...
#[tauri::command]
async fn capability_profiles(project_root: Option<String>) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || capability_profiles_blocking(project_root))
        .await
        .map_err(|e| format!("capability_profiles worker join failed: {e}"))?
}

fn capability_profiles_blocking(project_root: Option<String>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn governed_bash_status(project_root: Option<String>) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || governed_bash_status_blocking(project_root))
        .await
        .map_err(|e| format!("governed_bash_status worker join failed: {e}"))?
}

fn governed_bash_status_blocking(project_root: Option<String>) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn governed_bash_enable(
    project_root: Option<String>,
    approval_card_json: Option<String>,
    scope: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || governed_bash_enable_blocking(project_root, approval_card_json, scope))
        .await
        .map_err(|e| format!("governed_bash_enable worker join failed: {e}"))?
}

fn governed_bash_enable_blocking(
    project_root: Option<String>,
    approval_card_json: Option<String>,
    scope: Option<String>,
) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn governed_bash_disable(
    project_root: Option<String>,
    scope: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || governed_bash_disable_blocking(project_root, scope))
        .await
        .map_err(|e| format!("governed_bash_disable worker join failed: {e}"))?
}

fn governed_bash_disable_blocking(
    project_root: Option<String>,
    scope: Option<String>,
) -> Result<serde_json::Value, String> {
    let root = resolve_project_root(project_root)?;
...
#[tauri::command]
async fn operator_surface_list(
    project_root: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || operator_surface_list_blocking(project_root))
        .await
        .map_err(|e| format!("operator_surface_list worker join failed: {e}"))?
}

fn operator_surface_list_blocking(
    project_root: Option<String>,
) -> Result<serde_json::Value, String> {
    let root = resolve_project_root(project_root)?;
    run_json_cli(&root, &[
...
#[tauri::command]
async fn operator_surface_status(
    project_root: Option<String>,
    profile_id: String,
    session_id: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || operator_surface_status_blocking(project_root, profile_id, session_id))
        .await
        .map_err(|e| format!("operator_surface_status worker join failed: {e}"))?
}

fn operator_surface_status_blocking(
    project_root: Option<String>,
    profile_id: String,
    session_id: Option<String>,
) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn operator_surface_inspect(
    project_root: Option<String>,
    key: String,
    session_id: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || operator_surface_inspect_blocking(project_root, key, session_id))
        .await
        .map_err(|e| format!("operator_surface_inspect worker join failed: {e}"))?
}

fn operator_surface_inspect_blocking(
    project_root: Option<String>,
    key: String,
    session_id: Option<String>,
) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn operator_surface_rows(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || operator_surface_rows_blocking(project_root, session_id))
        .await
        .map_err(|e| format!("operator_surface_rows worker join failed: {e}"))?
}

fn operator_surface_rows_blocking(
    project_root: Option<String>,
    session_id: Option<String>,
) -> Result<serde_json::Value, String> {
    let root = resolve_project_root(project_root)?;
...
#[tauri::command]
async fn operator_surface_expert_set(
    project_root: Option<String>,
    key: String,
    value_json: String,
    confirm: Option<String>,
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || operator_surface_expert_set_blocking(project_root, key, value_json, confirm, scope))
        .await
        .map_err(|e| format!("operator_surface_expert_set worker join failed: {e}"))?
}

fn operator_surface_expert_set_blocking(
    project_root: Option<String>,
    key: String,
    value_json: String,
    confirm: Option<String>,
...
async fn vocab_delete_group(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || vocab_delete_group_blocking(project_root, kind, lang, parent_key, parent_mode))
        .await
        .map_err(|e| format!("vocab_delete_group worker join failed: {e}"))?
}

fn vocab_delete_group_blocking(
    project_root: Option<String>,
    kind: String,
    lang: String,
    parent_key: String,
...
#[tauri::command]
async fn gate_msg_list(lang: String) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || gate_msg_list_blocking(lang))
        .await
        .map_err(|e| format!("gate_msg_list worker join failed: {e}"))?
}

fn gate_msg_list_blocking(lang: String) -> Result<serde_json::Value, String> {
    let db = empire_db_path_rs();
    if !db.is_file() {
...
async fn gate_msg_upsert(
    ...
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || gate_msg_upsert_blocking(project_root, key, body, lang))
        .await
        .map_err(|e| format!("gate_msg_upsert worker join failed: {e}"))?
}

fn gate_msg_upsert_blocking(
    project_root: Option<String>,
    key: String,
    body: String,
    lang: Option<String>,
...
#[tauri::command]
async fn gate_msg_delete(
    project_root: Option<String>,
    key: String,
    lang: Option<String>,
) -> Result<serde_json::Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || gate_msg_delete_blocking(project_root, key, lang))
        .await
        .map_err(|e| format!("gate_msg_delete worker join failed: {e}"))?
}

fn gate_msg_delete_blocking(
    project_root: Option<String>,
    key: String,
    lang: Option<String>,
) -> Result<serde_json::Value, String> {
    ...
    tauri::Builder::default()
        ...
        .run(|_app, event| {
            if matches!(event, tauri::RunEvent::ExitRequested { .. } | tauri::RunEvent::Exit) {
                ...
                // DO NOT revoke the operator token here (fixed 2026-07-23).
                //
                // This used to call revoke_cached_operator_token() on exit "so
                // the row doesn't outlive the process" — which deleted
                // ~/.aidocs/operator_token.json and revoked the row. The effect
                // was that CLOSING THE APP DESTROYED THE LOGIN: every launch
                // began signed-out at the local password form, and because that
                // form is gate A, the operator never even reached the CodeNexus
                // sign-in behind it. Hours were spent hunting the cloud path
                // while the credential was being deleted on window close.
                //
                // It also contradicted the two rules it sat between: #421
                // ("ONCE-ONLY login — cache the token machine-side"), whose own
                // success message promises the token works "until <expiry>",
                // and #92 ("renew, never re-wall"). A machine cache that does
                // not survive the process is not a cache.
                //
                // The in-process cache dies with the process by definition, and
                // the token keeps its own expiry + revocation. Signing out is
                // still explicit: dashboard_logout / operator_logout revoke.
                let _ = &OPERATOR_TOKEN_CACHE;
            }
        });
}
...
async fn open_url(url: String) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn_blocking(move || open_url_blocking(url))
        .await
        .map_err(|e| format!("open_url worker join failed: {e}"))?
}
...
/// Apply CREATE_NO_WINDOW so a PROBE never flashes a console window.
///
/// #474 defect 2 (process churn, diagnosed live 2026-07-25): opening the dashboard
/// spawned a burst of short-lived CONSOLE processes and the UI froze until the burst
/// drained. Measured on the operator's box: 7 conhost.exe alive alongside the app.
/// The cause is the startup probe sweep — find_python_path tries 3 interpreter
/// candidates (each `--version`, then `where`/`which`), and setup_detect's cli_found
/// tries FOUR candidates per CLI (`.cmd`/`.exe`/`.bat`/bare) for node, npm and each
/// agent CLI. Every one of those was a console spawn: a conhost process, a window
/// flash, and CPU the webview then had to compete with.
///
/// 21 of 30 Command::new sites lacked the flag while 9 already had it, so the idiom
/// existed and the probe paths had simply been missed. Probes are the hot ones —
/// they run at OPEN, in loops, before the operator has done anything.
/// Resolve an executable on PATH **without spawning anything** (#474 defect 2).
///
/// This replaces the probe-by-spawn pattern that froze the dashboard on open. The
/// old code answered "is node installed?" by RUNNING `node --version` — and on
/// Windows it tried four candidates per CLI (`.cmd`/`.exe`/`.bat`/bare), so a
/// handful of CLIs became ~20 console spawns before the operator had clicked
/// anything. Each spawn costs a process, a conhost, a window flash, and CPU the
/// webview then competes with; the UI unfroze only when the burst drained.
///
/// Existence is a FILESYSTEM question, so answer it with the filesystem: walk PATH,
/// apply PATHEXT on Windows, and stat. Microseconds, no processes, no flashes. A
/// version check still needs to execute the binary — but only ONCE, on the winner,
/// instead of once per candidate.
fn resolve_on_path(name: &str) -> Option<PathBuf> {
    let raw = std::env::var_os("PATH")?;
    let exts: Vec<String> = if cfg!(windows) {
        // Honour the real PATHEXT; fall back to the documented default set.
        let pe = std::env::var("PATHEXT")
        ...
        let mut v: Vec<String> = pe
            ...
            .filter(|e| !e.is_empty())
            .collect();
        // An explicit extension in `name` (or a shim with none) must still match.
...
fn silence_console(cmd: &mut Command) -> &mut Command {
    #[cfg(target_os = "windows")]
    {
        use std::os::windows::process::CommandExt;
        cmd.creation_flags(0x08000000); // CREATE_NO_WINDOW
...
fn find_python_path() -> Option<String> {
    // Resolve first (no spawn), then execute ONLY a candidate that exists. The old
    // shape ran `--version` on every name and then `where`/`which` again on the
    // winner — up to 6 console spawns for a question that needs at most one.
    for candidate in &["python", "python3", "py"] {
        let Some(resolved) = resolve_on_path(candidate) else {
            continue;
        };
        let mut probe = Command::new(&resolved);
        probe.args(["--version"]);
        let Ok(output) = silence_console(&mut probe).output() else {
            continue;
        };
        if !output.status.success() {
            continue;
        }
        // `py --version` and some shims print to stderr; read both.
        let mut ver = String::from_utf8_lossy(&output.stdout).trim().to_string();
        if ver.is_empty() {
            ver = String::from_utf8_lossy(&output.stderr).trim().to_string();
        }
        let Some(v) = ver.strip_prefix("Python ") else {
            continue;
        };
...
async fn select_folder() -> Result<Option<String>, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || select_folder_blocking())
        .await
        .map_err(|e| format!("select_folder worker join failed: {e}"))?
}

fn select_folder_blocking() -> Result<Option<String>, String> {
    #[cfg(target_os = "windows")]
    {
        let output = Command::new("powershell")
            .args(["-NoProfile", "-Command",
...
#[tauri::command]
async fn setup_detect(project_root: Option<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || setup_detect_blocking(project_root))
        .await
        .map_err(|e| format!("setup_detect worker join failed: {e}"))?
}

fn setup_detect_blocking(project_root: Option<String>) -> Result<Value, String> {
    ...
    let python_version = python_path
        .as_ref()
        .and_then(|py| {
            let mut vprobe = Command::new(py);
            vprobe.args(["--version"]);
            silence_console(&mut vprobe).output().ok().map(|o| {
    ...
    let cli_found = |cmd: &str| -> bool {
        // ZERO SPAWNS. This used to run `<cmd> --version` across four Windows
        // candidates per CLI; for node + npm + the agent CLIs that was the bulk of
        // the open-time process storm (#474 defect 2). Presence is a filesystem
        // fact, and resolve_on_path already applies PATHEXT — so `.cmd` shims,
        // which plain Command::new cannot launch, are still detected correctly.
        if resolve_on_path(cmd).is_some() {
            return true;
        }
        if cfg!(windows) {
            return ["cmd", "exe", "bat"]
                .iter()
                .any(|ext| resolve_on_path(&format!("{cmd}.{ext}")).is_some());
        }
        false
    };
...
#[tauri::command]
async fn setup_install(targets: Vec<String>) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || setup_install_blocking(targets))
        .await
        .map_err(|e| format!("setup_install worker join failed: {e}"))?
}

fn setup_install_blocking(targets: Vec<String>) -> Result<Value, String> {
...
#[tauri::command]
async fn setup_configure(project_root: String) -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || setup_configure_blocking(project_root))
        .await
        .map_err(|e| format!("setup_configure worker join failed: {e}"))?
}

fn setup_configure_blocking(project_root: String) -> Result<Value, String> {
    ...
    let mut cfg_cmd = Command::new(&python);
    let output = silence_console(&mut cfg_cmd)
...
#[tauri::command]
async fn setup_install_python() -> Result<Value, String> {
    // #474: off the UI thread. See the note above load_dashboard.
    tauri::async_runtime::spawn_blocking(move || setup_install_python_blocking())
        .await
        .map_err(|e| format!("setup_install_python worker join failed: {e}"))?
}

fn setup_install_python_blocking() -> Result<Value, String> {
...
mod tests {
    ...
    // ── #509 token lifetime: iso_utc_to_unix + the tri-state verdict ──

    #[test]
    fn iso_utc_to_unix_epoch() {
        assert_eq!(iso_utc_to_unix("1970-01-01T00:00:00Z"), Some(0.0));
    }

    #[test]
    fn unix_to_iso_utc_epoch() {
        assert_eq!(unix_to_iso_utc(0.0), "1970-01-01T00:00:00Z");
    }

    #[test]
    fn unix_to_iso_utc_known_instant() {
        assert_eq!(unix_to_iso_utc(1_784_937_600.0), "2026-07-25T00:00:00Z");
    }

    /// THE PROPERTY THAT MATTERS: the two are inverses. A stamp this formatter
    /// writes must parse back to the instant it came from, because the Python
    /// side compares it lexicographically against `_iso_now()` to decide
    /// whether a gate bearer is dead. A format that drifts by an hour, or
    /// zero-pads wrongly, silently mis-expires credentials — and the failure
    /// looks exactly like a revoked operator, which is the one confusion this
    /// whole area exists to prevent.
    #[test]
    fn iso_round_trips_across_leap_years_and_month_ends() {
        for stamp in [
            "1970-01-01T00:00:00Z",
            "1999-12-31T23:59:59Z",
            "2000-02-29T12:00:00Z", // leap century
            "2024-02-29T00:00:00Z", // ordinary leap year
            "2026-03-01T00:00:00Z", // day after a non-leap February
            "2026-08-22T10:14:50Z", // the operator's measured gate expiry
            "2038-01-19T03:14:07Z", // 32-bit rollover
            "2100-02-28T23:59:59Z", // non-leap century
        ] {
            let secs = iso_utc_to_unix(stamp).expect("parses");
            assert_eq!(unix_to_iso_utc(secs), stamp, "round trip for {stamp}");
        }
    }

    /// An hour added to a known instant must land an hour later — the actual
    /// arithmetic `webmcp_oauth_complete` performs with `expires_in`.
    #[test]
    fn unix_to_iso_utc_adds_an_expires_in_window() {
        let base = iso_utc_to_unix("2026-08-22T09:14:50Z").expect("parses");
        assert_eq!(unix_to_iso_utc(base + 3600.0), "2026-08-22T10:14:50Z");
    }

    #[test]
    fn iso_utc_to_unix_known_instant() {
        assert_eq!(
            iso_utc_to_unix("2026-07-25T00:00:00Z"),
            Some(1_784_937_600.0)
        );
    }

    #[test]
    fn iso_utc_to_unix_leap_day() {
        assert_eq!(
            iso_utc_to_unix("2024-02-29T12:34:56Z"),
    ...
    fn iso_utc_to_unix_rejects_garbage() {
        ...
        assert_eq!(iso_utc_to_unix("2026-07-25"), None);
    }

    #[test]
    fn token_verdict_unknown_is_not_revoked() {
        // The TYPE is the guarantee (#508): "could not ask" must not be
        // expressible as "the authority said no". A collapse back to bool
        // fails to compile rather than silently re-locking the operator out.
        assert_ne!(TokenVerdict::Unknown, TokenVerdict::Revoked);
        assert_ne!(TokenVerdict::Unknown, TokenVerdict::Valid);
    }

    #[test]
    fn machine_cached_token_is_withheld_only_once_expired() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("operator_token.json");
        let write = |expires: &str| {
            fs::write(
                &path,
                format!(r#"{{"token":"tok-30d","expires_at":"{expires}"}}"#),
            )
            .unwrap();
        };
        std::env::set_var("AIDOCS_TOKEN_CACHE", &path);

        write("2999-01-01T00:00:00Z"); // inside its 30-day life
        assert_eq!(
            machine_cached_token_unexpired(),
            Some("tok-30d".to_string()),
            "an unexpired token must still be handed to the CLI"
        );

        write(""); // legacy row, no expiry recorded
        assert_eq!(
            machine_cached_token_unexpired(),
            Some("tok-30d".to_string()),
            "a missing expiry is not evidence of expiry — the CLI decides"
        );

        write("2000-01-01T00:00:00Z"); // genuinely past its 30 days
        assert_eq!(
            machine_cached_token_unexpired(),
            None,
            "past 30 days the operator logs in again"
        );

        std::env::remove_var("AIDOCS_TOKEN_CACHE");
    }
