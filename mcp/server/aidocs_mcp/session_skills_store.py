...
class SessionSkillsStore(SQLiteIndexStoreBase):
    ...
    shape. Reads (get_row / read_legacy) are pure (#1095); legacy
    ingestion runs only in init_db, on the write path.
    """
    ...
    # ── PURE READS (#1095) ───────────────────────────────────────────
    # A read never creates the table, ingests or deletes a legacy file, or
    # rewrites a row. Migration is init_db's job, reached from the WRITE
    # path (SkillStore.set_selected_skills).

    def get_row(self, project_root: Path, session_id: str) -> list[str] | None:
        """The persisted selection, or None when there is no row (or no
        table yet). Never creates schema."""
        try:
            with self.session(project_root) as conn:
                row = conn.execute(
                    "SELECT selected_skills FROM session_skills WHERE session_id = ?",
    ...
    def read_legacy(self, project_root: Path, session_id: str) -> list[str] | None:
        """A not-yet-migrated legacy skills.json selection, read in place
        (nothing ingested, nothing deleted). None when absent/unreadable."""
        path = self._legacy_json_path(project_root, session_id)
        if not path.is_file():
            return None
        try:
            ...
            return None
        selected = raw.get("selected_skills") if isinstance(raw, dict) else None
        if not isinstance(selected, list):
            return None
        return [str(s) for s in selected if isinstance(s, str) and s]
