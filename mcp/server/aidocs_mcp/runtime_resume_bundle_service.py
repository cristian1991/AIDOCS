...
class RuntimeResumeBundleService:
    ...
    def session_resume_bundle(
        ...
        if journal_last_n == 10:
            # 0 = unlimited (read_journal returns all entries). Read without
            # the `or 10` footgun that turned a configured 0 back into 10.
            _rjl = self.runtime._config_resolver.get(
                "presentation.resume_journal_last_n",
                project_root=project_root,
                session_id=session_id,
            )
            try:
                journal_last_n = int(_rjl) if _rjl is not None else 10
            except (TypeError, ValueError):
                journal_last_n = 10
        # Resume implies session-scoped read authority: SQL session_membership
        # is the sole authority. Heal an unmigrated legacy session once
        # (bounded, marker-gated), then fail closed for a true non-member so
        # resume can't surface an arbitrary session's bundle. File presence is
        # never authority here.
        from .session_membership_store import SessionMembershipStore

        if not SessionMembershipStore().ensure_member_or_heal(project_root, session_id):
            return {
                "ok": False,
                ...
                "error": (
                    ...
                    f"— resume refused (membership is the sole authority). "
                    f"Run `aidocs migrate-control-authority` for legacy "
                    f"sessions, or session create/connect."
                ),
            }
        ...
        imported_skill_state = self.runtime._imported_skill_state(
            project_root,
            session_id,
            selected_state=selected_skills,
        )
        ...
        # Slim bundle: only expose section NAMES on disk artifacts, not their
        # bodies. Full bodies live on disk and are fetched via ai_get_lines
        # when actually needed. Echoing every section body here was the
        # single biggest resume-time context burner (operator flagged
        # 2026-04-20, TIER -5). Keep `path` only on artifacts the agent
        # might want to open next.
        result: dict[str, object] = {
            "session": {
                "session_id": session.session_id,
                "path": str(session.path),
                "section_names": list(session.sections.keys())
                if isinstance(session.sections, dict)
                else [],
            },
            "context": {
                "path": str(context.path),
                "section_names": list(context.sections.keys())
            ...
            "handoff": {
                ...
                "section_names": list(handoff.sections.keys())
                if isinstance(handoff.sections, dict)
                else [],
            },
        ...
        if include_code_bundle:
            result["ai_bundle"] = self.runtime._refresh_session_code_bundle(
                project_root,
                session_id=session_id,
