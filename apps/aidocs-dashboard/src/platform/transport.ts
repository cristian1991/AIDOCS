...
const GATE_TOOL: Readonly<Record<string, string>> = {
  ...
  // Governed memory write (#200): member write on the bound project;
  // two-phase confirm handled by gateCallConfirming below. Gate name is
  // dashboard_memory_capture so it can never shadow the agent tool.
  memory_capture: "dashboard_memory_capture",
  ...
  // War 0: freeze surfacing + recovery (org OWNER/ADMIN). clear_freeze uses
  // the gate's CONSUMABLE confirmation — the confirm_required round-trip is
  // handled by gateCallConfirming like its escalation siblings.
  ...
  // #1061: the dashboard operator message -> governed XAACP operator append.
  tauri_msg_send: "dashboard_operator_message",
};

// VIEW, DON'T BIND (r0b ruling f0bed231-a27, 2026-09-17). The web dashboard used
// to project_select before every load — an account-wide binding change that
// cleared the selected session on every surface. It now VIEWS a project: every
// project-scoped call below carries the viewed project_id, and the gate reads
// (or, for memory_capture, writes) THAT project, re-deciding authority in the
// project's own org. Without this, panels would silently show — and
// memory_capture would silently write — the BOUND project while the page names
// another.
//
// EVERY mapped command must be classified in exactly one of these two sets; a
// test fails on any GATE_TOOL command in neither, so a new panel cannot quietly
// fall back to the bound project.
export const PROJECT_SCOPED_COMMANDS: ReadonlySet<string> = new Set([
  "skill_scan_results",
  "list_mcp_servers",
  "tauri_backlog_list",
  "tauri_todo_list",
  "memory_kg_graph",
  "memory_kg_get",
  // tenant-scoped: the viewed project's OWNING org's vocabulary
  "vocab_list_langs",
  "vocab_get_grouped",
  // the one write: lands in the viewed project, confirm names it
  "memory_capture",
]);

// Deliberately NOT project-scoped: global/public reads (no fake project scoping),
// org-level control plane, and explicit binding/registration actions.
export const NON_PROJECT_COMMANDS: ReadonlySet<string> = new Set([
  "mcp_registry_search",
  "vocab_list_kinds",
  "approve_escalation",
  "deny_escalation",
...
export const MAPPED_WEB_COMMANDS: ReadonlyArray<string> = Object.keys(GATE_TOOL);

let _viewedProjectId: string | null = null;

/** The project the dashboard is VIEWING (never a binding). */
export function setViewedProjectId(projectId: string | null | undefined): void {
  _viewedProjectId = projectId ? String(projectId) : null;
}

export function getViewedProjectId(): string | null {
  return _viewedProjectId;
}

/** Add the viewed project_id to a project-scoped call; an explicit one is never overridden. */
export function withViewedProject(
  command: string,
  args?: Record<string, unknown>,
): Record<string, unknown> | undefined {
  if (!PROJECT_SCOPED_COMMANDS.has(command) || !_viewedProjectId) return args;
  if (args && args["project_id"]) return args;
  return { ...(args || {}), project_id: _viewedProjectId };
}
 ...
 */
export async function gateCallConfirming<T = unknown>(
  tool: string,
  args?: Record<string, unknown>,
): Promise<T> {
...
export async function routeInvoke<T = unknown>(
  ...
  if (tool) return gateCallConfirming<T>(tool, withViewedProject(command, args));
