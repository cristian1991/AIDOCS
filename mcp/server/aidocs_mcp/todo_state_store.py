...
class TodoStateStore:
    ...
    def _resolve_owner(
        ...
        # A CANONICAL ID IS NOT THE SAME AS A NON-EMPTY ONE (#1043b). The check
        # above tests emptiness, and the refused identity is a non-empty string,
        # so it satisfies "requires canonical agent_context_id" while being the
        # opposite of one. Every unstamped subagent carries the same value, so
        # it would become a SHARED todo-ownership key: one unattributable
        # caller reading and overwriting another's todo list.
        try:
            from .task_actor_identity import identity_is_unproven

            if identity_is_unproven(actor_id):
                raise RuntimeError(
                    "worker todo state requires canonical agent_context_id: this "
                    "caller carries worker/subagent evidence but no agent id "
                    "[rule_id=unproven_subagent_identity]"
                )
