...
def _authenticated_uid(project_root: Path, host_session_id: str = "") -> str:
    ...
    try:
        ...
        pass
    try:
        # (b) per-session APPROVED host binding — session-specific, so it
        # outranks the machine-wide login below. Without this ordering,
        # user B's later machine login would hijack a session already
        # bound to user A (specificity must win).
        if host_session_id:
        ...
        pass
    try:
        # (c) #443: machine-wide FALLBACK — a machine-level user login
        # authenticates every local session that has no more-specific
        # credential. The newest live dashboard-login token row in the
        # identity DB (pure DB read — no file reads) resolves this
        # machine's signed-in user, regardless of host_session_id and
        # without any host-binding row.
        from .operator_auth_service import OperatorAuthService
...
def require_admin(
    ...
    *,
    ...
    return Decision(
        ok=False,
        flavor=flavor,
        reason=f"requires {permission}. Sign in as an operator with that grant.",
