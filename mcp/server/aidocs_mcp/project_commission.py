...
def _conn() -> sqlite3.Connection:
    ...
    conn = _canonical_connect(str(p))
    # The DDL ran on EVERY connect, not just on init (#756) -- a write lock and
    # a transaction per connection to learn the table already exists. The memo
    # re-verifies the file, so a deleted DB rebuilds its schema.
    if not _schema_already_ensured(p, "project_commission"):
        conn.execute(
        ...
        _mark_schema_ensured(p, "project_commission")
    return conn
