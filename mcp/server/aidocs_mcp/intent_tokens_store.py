...
Per empire-doctrine §XII + Empire-directive 2026-05-05: NLP intent vocabulary
...
# #756: the canonical CLOSING connect. `with sqlite3.connect(p) as conn:` is
# sqlite3's TRANSACTION context manager — it commits or rolls back and never
# closes, and these handles sit in reference cycles so only a gen-2 collection
# frees them (3 OS handles each: db/-wal/-shm).
#
# row_factory=False at every call site is DELIBERATE, not an oversight. The
# helper defaults to True, but raw sqlite3.connect() yields plain tuples and this
# module depends on that: the memo below stores "the RAW immutable row tuples"
# and every consumer indexes positionally (`r[0]`, `row[0]`). Converting rows to
# sqlite3.Row here would change what is cached and what callers compare, for no
# benefit — nothing in this file reads a column by name.
...
def _request_conn() -> "sqlite3.Connection | None":
    ...
    if cache.conn is None:
        # RUNTIME (the default), and durability is in any case moot: this is
        # the request-scoped READ connection — its only users are the SELECT
        # and the `PRAGMA data_version` memo guard in lemma_rows_for_kind.
        # NOT read_only, deliberately: the empire vocab db is created by
        # init_db on first use and a mode=ro open would refuse rather than
        # let the caller reach that path. row_factory=False preserves the raw
        # tuples this module's memo stores and every consumer indexes
        # positionally (see the header note above).
        #
        # The handle is BORROWED by design — it is cached on the request and
        # closed explicitly by the invalidation branch above — so it is never
        # entered as `with conn:` by a caller and ClosingConnection cannot pull
        # it out from under one.
        conn = _canonical_connect(db_path, row_factory=False)
...
def _seed_from_factory_if_empty(db: Path) -> int:
    ...
    if not factory.is_file():
        ...
        try:
            with _canonical_connect(str(db), row_factory=False) as _c:
    ...
    try:
        with _canonical_connect(str(db), row_factory=False) as conn:
            # PER-TABLE short-circuit (fixed 2026-07-26). This used to test ONLY
            # intent_lemma_sets and then copy BOTH tables, so a live DB holding
            # vocab with an EMPTY gate_message_strings returned early FOREVER and
            # never received the messages — including the CURATED TOOL
            # DESCRIPTIONS that override every tool's docstring.
            #
            # That state was live, not theoretical: the deploy proof root had vocab
            # and no messages, so the server advertised RAW SOURCE DOCSTRINGS and
            # the mode-dispatched tools' wire inputSchema silently differed from
            # what a provisioned server emits. It is why the surface golden was
            # first pinned to the wrong side (#533).
            #
            # A partially-seeded database is precisely what a per-table check
            # exists to repair, and the copies below are already INSERT OR IGNORE,
            # so re-running the seed for one table cannot duplicate the other.
            vocab_seeded = (
                conn.execute("SELECT 1 FROM intent_lemma_sets LIMIT 1").fetchone() is not None
            )
            msgs_seeded = (
                conn.execute("SELECT 1 FROM gate_message_strings LIMIT 1").fetchone() is not None
            )
            if vocab_seeded and msgs_seeded:
...
def has_any_rows(lang: str = "") -> bool:
    ...
    sql += " LIMIT 1"
    with _canonical_connect(str(empire_db_path()), row_factory=False) as conn:
