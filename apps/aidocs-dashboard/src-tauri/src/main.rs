...
/// (aidocs-doctrine §XX — installs are signed/verified operator paths).
...
// Message substrate. #1061: the dashboard's operator SEND no longer runs a
// local python `msg_send(from_role="king")`; it goes through the gate's
// governed `dashboard_operator_message` op (principal/session server-derived,
// bound-operator check, UPS safety head, one origin='operator' stream entry).
#[tauri::command]
