...
# DURABILITY: AUDIT for the writes. palace_disable is a REFUSE-ALL switch
# an operator threw deliberately; a write a power cut un-does re-ENABLES
# the palace, which is an authority handed back to callers it was taken
# from. The read is read_only=True -- and no longer materialises an empty
# db just to discover the row is absent.
...
class PalaceControlStore:
    ...
    def is_palace_disabled(self, project_root: Path) -> bool:
        ...
        try:
            with _canonical_connect(
                str(self.db_path(project_root)), read_only=True, row_factory=False
            ) as conn:
                row = conn.execute(
