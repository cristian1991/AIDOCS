...
def edit_result(
    *,
    ...
    # ── #448 Consumer C rail: blast radius of THIS invocation (#1075) ──
    # _post_edit_reindex_and_grant records a receipt for each edited path; this
    # ack renders ONLY receipts of the same invocation (same project, recorded
    # at or after this call's started_at), ALL of them merged into one bounded
    # note (r0b B2: a batch used to render only its last path), then clears
    # them all, success or failure. It used to drain a process-global slot,
    # which let one actor's ack carry ANOTHER actor's dependent paths and
    # anchored memories. Best-effort — never blocks the response.
    _radius_note = None
    try:
        from .blast_presenter import merge_radii
        from .semantic_enrichment import receipts_for_invocation

        _receipts = receipts_for_invocation(project_root, started_at)
        if _receipts:
            _radius_note = merge_radii([r.get("radius") for r in _receipts])  # type: ignore[misc]
    except Exception:
        _radius_note = None
    finally:
        try:
            from .semantic_enrichment import clear_edit_receipts as _clear_receipts

            _clear_receipts()
        except Exception:
            pass
    if _radius_note is not None and success:
        # #1075 step 4: ONE channel, bounded size. The raw dependent list never
        # rides the ack; a Stop-feedback host gets a pointer (the obligation is
        # in the blast ledger), any other host a compact inline note.
        _note = None
        try:
            from .blast_presenter import channel_for_host, compact_note, resolve_host_kind, summary_line

            _note = compact_note(
                _radius_note, channel=channel_for_host(resolve_host_kind(project_root))
            )
            _radius_summary = summary_line(_note)
        except Exception:
            _note, _radius_summary = None, ""
        if _note is not None and isinstance(structured, dict):
            structured["blast_radius"] = _note
