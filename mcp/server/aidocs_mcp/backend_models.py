...
def _default_run_opencode(oc: str) -> subprocess.CompletedProcess:
    ...
    return audited_run(
        ...
        # OL host-CLI probe: fixed argv `opencode models`, shell=False, read-only
        # catalog query, not agent-controlled, DEVNULL stdin + bounded timeout;
        # registered in shell_egress_service.LEGACY_SUBPROCESS_CALLSITES. The
        # `# nosemgrep:` that used to lead this comment is gone (#1031): the rule
        # now recognises the audited seam, so a routed spawn needs no waiver and
        # the rationale can just be a rationale.
