...
Owns the UPS sub-pipeline library: every host (Claude Code, OpenCode,
future Codex adapter) reaches these sub-pipelines through the ONE
canonical transaction entry, ``PromptSubmitService.evaluate_submit``
(which drives ``hook_pipeline._run_user_prompt_core``); each
sub-pipeline returns a ``PromptMutationResult``.
...
risks behavior drift. Strategy:
  ...
  1. This module defines the **contract** — the ``PromptMutationResult``
     shape returned by every sub-pipeline; the transaction entry all
     hosts target is ``PromptSubmitService.evaluate_submit``.
...
The canonical core composes these sub-pipelines and hosts render
the service result. The contract distinguishes:
...
# ---------------------------------------------------------------------------
# #99 FIX1 (UX half): sticky sink-drop surfacing
# ---------------------------------------------------------------------------


def _sink_drop_block(new_tools: list[str], *, sticky_flag: bool) -> tuple[list[str], str | None]:
    """Which requested STICKY tools the write-sink will drop by T0 doctrine
    (raw shell/file tools are non-sticky), plus an operator-facing explanation
    block (or None when nothing is dropped).

    #99 FIX1: pre-fix the drop was silent — the operator typed a valid sticky
    phrase for e.g. `grep` and saw it "not stick" with no signal. This builds
    the next-UPS additionalContext message naming the dropped tool(s) + the
    tier-2 recovery path. Doctrine is unchanged — this only ADDS feedback; it
    never sticky-grants a raw tool. Uses QueryGateStore._STICKY_RAW_TOOL_FORBIDDEN
    as the single source so the surfaced set matches the actual sink drop.
    """
    if not sticky_flag:
        return [], None
    from .query_gate import QueryGateStore

    forbidden = QueryGateStore._STICKY_RAW_TOOL_FORBIDDEN
    dropped = sorted({str(t) for t in new_tools if str(t).lower() in forbidden})
    if not dropped:
        return [], None
    tools = ", ".join(f"`{t}`" for t in dropped)
    block = (
        f"⚠️ Sticky grant refused at the sink for {tools} — raw shell/file tools "
        "cannot be sticky by doctrine (they must be re-demonstrated each turn). "
        'Use tier-2 for narrowed sticky scope (e.g. "allow bash to run opencode, '
        'persist the grant"), or grant per-turn without the sticky marker. The '
        "per-turn grant for this turn still applies."
    )
...
class PromptMutationResult:
    ...
    def __post_init__(self) -> None:
        if self.decision != "block" or not self.block_reason:
            return
        from .tool_gate_service import refusal_with_affordance

        rule_id = next(
            (str(item) for item in reversed(self.why) if str(item).strip()),
            "prompt_gate.refusal",
        )
        object.__setattr__(
            self,
            "block_reason",
            refusal_with_affordance(self.block_reason, rule_id),
        )
...
# --- A CONFIRMATION MUST BE ANSWERED, NOT MERELY CONTAINED ------------------
# The sticky-grant answer scan used `token in text` -- substring containment over
# the whole prompt. Every one of these silently confirmed EVERY pending grant:
#     "yesterday I broke the build"  contains "yes"
#     "check the eyes on that logo"  contains "yes"
#     "disapprove that change"       contains "approve"  <- GRANTS BY REFUSING
#     "that is unconfirmed"          contains "confirm"  <- GRANTS BY DOUBTING
#     "register a new user"          contains "register"
#
# The asymmetry was the real defect: the DENIAL branch was already tight (" no"
# with a leading space plus an exact "no"/"n" fallback) while the AFFIRMATIVE
# branch was loose -- so the two halves disagreed about how careful to be, and
# the careless half was the one that GRANTS PERMISSION. For a security
# confirmation the bias must run the other way: ambiguity fails CLOSED and the
# operator repeats themselves. (AGT audit S6 called these "generic affirmative
# tokens", which understates it -- there was no affirmative REQUIREMENT at all.)
#
# The affirmative must now be the operator's ANSWER: word-bounded and at the
# START of the prompt, so "yes, go ahead" works and "yesterday..." does not.
# `register` is dropped entirely -- it opens ordinary requests and carries no
# confirming sense alone, unlike "yes"/"confirm".
#
# STANDING TENSION: operator ruling 2026-07-25 (#530) is "remove sticky from
# aidocs completely... i never wanted sticky". If that lands, this matcher goes
# with it. This makes the live path safe meanwhile; it is not a case for keeping
# the feature.
_AFFIRMATIVE_ANSWER = re.compile(
    r"^\s*(?:yes|yeah|yep|yup|y|confirm(?:ed|s)?|approve[ds]?|ok(?:ay)?"
    r"|ok to grant|go ahead|do it)\b",
)
# Denial keeps its historic breadth ON PURPOSE and is checked FIRST by the
# caller: a refusal anywhere in the prompt must win, because failing closed on a
# half-expressed "no" costs a repeated question, while missing one grants.
_NEGATIVE_ANSWER = re.compile(
    r"(?:^\s*(?:n|nope|nah)\b)|\b(?:no|deny|denied|reject|cancel|don't|do not"
    r"|skip grant)\b",
)
# `\bno\b` matches ANYWHERE on purpose, replacing the original " no" (leading
# space) so "yes but no" still refuses -- caught by a test after a first attempt
# anchored it and made the refusal branch NARROWER than the code it replaced.
# Word boundaries keep it off "nothing", "note" and "known"; a standalone "no"
# in a sentence ("there is no rush") will refuse a pending grant, which is
# over-matching on the side that COSTS A REPEATED QUESTION rather than the side
# that grants permission.


def _is_affirmative_answer(prompt: str) -> bool:
    """True only when the prompt OPENS with an affirmative answer."""
    return bool(_AFFIRMATIVE_ANSWER.search((prompt or "").strip().lower()))


def _is_negative_answer(prompt: str) -> bool:
    """True when the prompt contains a refusal anywhere. Deliberately broader
    than the affirmative: this is the side that is safe to over-match."""
    return bool(_NEGATIVE_ANSWER.search((prompt or "").strip().lower()))
...
class PromptMutator:
    ...
    # NOTE (2026-07-14, #377): the `mutate_prompt` compatibility shim over
    # PromptSubmitService was RETIRED. The one canonical prompt-submit entry
    # is ``PromptSubmitService.evaluate_submit`` (see prompt_submit_service);
    # hosts compose the sub-pipelines below only through that transaction.
    ...
    def consume_sticky_grant_answers(
        ...
        *,
        ...
        is_yes = _is_affirmative_answer(text)
        is_no = _is_negative_answer(text)
        ...
        *,
        ...
        try:
            from .intent_grant_detector import (
                detect_grant,
                detect_sticky_grant_consent,
            )

            # #496 defect (B) — provenance floor. Layer-2 detection runs on
            # the OPERATOR-AUTHORED slice only: a harness-injected
            # <task-notification> minted a standing ai_deslop_apply grant
            # pre-fix. Harness text is not the operator's voice, so it
            # surfaces nothing — per-turn or sticky.
            try:
                from .canonical_intent_registry import strip_non_operator_text

                _l2_text = strip_non_operator_text(prompt)
            except Exception:
                _l2_text = ""  # cannot prove provenance → surface nothing
            detection = detect_grant(_l2_text)
            _RAW_TOOLS = {"grep", "bash", "read", "edit", "write", "glob"}
            # High-authority, gate-enforced tools are NEVER auto-granted from prose: a deploy
            # trigger must come through the gate's super_admin + confirm path, not from a casual
            # phrase like "the grep output was confusing" (which would otherwise surface
            # ai_deploy_output via the dynamic full-tool_interface UPS matcher). Defense-in-depth.
            _NEVER_NLP_GRANT = {"ai_deploy", "ai_deploy_output"}
            try:
                ...
                nlp_granted = {
                    ...
                    if t not in _RAW_TOOLS and t not in _NEVER_NLP_GRANT and not is_eager_tool(t)
                }
            except Exception:
                nlp_granted = set(detection.granted_tools) - _RAW_TOOLS - _NEVER_NLP_GRANT
            if nlp_granted and not detection.deny and not detection.has_negation:
                ...
                # #496 defect (B) — MINTING WITHOUT CONSENT. Surfacing a
                # tool because the operator mentioned it is a VISIBILITY
                # mechanism (docs/user-intent.md §8), not consent to
                # STANDING authority. Sticky requires affirmative,
                # present-tense, operator-authored consent carrying a
                # sticky marker (§4); everything else surfaces PER-TURN
                # only — same visibility as before, no permanent row.
                sticky_ok, _sticky_reason = detect_sticky_grant_consent(prompt)
                if sticky_ok:
                    ...
                    current_turn = set(
                        qg.get_user_intent_tools(project_root, managed_session_id) or [],
                    )
                    if not nlp_granted <= current_turn:
                        qg.set_user_intent_tools(
                            project_root,
                            managed_session_id,
                            sorted(current_turn | nlp_granted),
                        )
                        layer2_perturn = len(nlp_granted - current_turn)
                        try:
                            qg.bump_grants_generation(
                                project_root,
                                managed_session_id,
        ...
        if layer2_perturn:
            # #496: surfaced for visibility this turn only — no standing grant.
            side_effects.append(f"Layer-2 NLP per-turn surfacing: {layer2_perturn}")
            why.append("user_intent_layer2_perturn")

        # #99 FIX1 (UX half): if the operator asked to STICKY a raw shell/file
        # tool, the write-sink drops it by doctrine. Surface a next-UPS
        # additionalContext block so the drop isn't silent ("I said it right and
        # it didn't stick"). The per-turn grant for these tools still applies.
        blocks: list[str] = []
        sink_dropped, sink_block = _sink_drop_block(_new_tools, sticky_flag=sticky_flag)
        if sink_dropped:
            side_effects.append(f"sticky grant refused at sink: {', '.join(sink_dropped)}")
            why.append("sticky_sink_dropped")
            if sink_block:
                blocks.append(sink_block)
    ...
    def ensure_host_session_bound(
        self,
        *,
        project_root: Path,
        host_session_id: str,
    ) -> bool:
        """Bind ``host_session_id`` to this project's session if it is not
        already bound. Returns True when a heal was performed.

        THE EARLIEST-POSSIBLE #135 SELF-HEAL (#672 fallout, measured
        2026-08-01). #672 correctly made managed-mode resolution REFUSE a
        caller that NAMES a host session with no per-conductor row
        (``resolved_via="unresolvable_host_session"``) rather than hand back
        the project singleton's session — another actor's identity. Its note
        anticipated the blast radius and relied on "#135 auto-bind self-heals
        on UserPromptSubmit", but that heal lived near the END of
        ``_run_user_prompt_core``. Everything that resolves a session BEFORE
        it — the prompt-submit transaction's own participant set, the UPS
        audit row, the preflight judge, the origin gate, the SEC-001 snapshot
        and the whole SEC-002 cascade — therefore ran against an EMPTY session
        id and silently did nothing.

        The transaction case was the worst of them: participants are built
        per-session, so an unbound host produced a snapshot of NOTHING, and a
        mid-mutation failure then "rolled back" without restoring anything —
        a SEC-002 atomicity hole that no longer announced itself.

        Idempotent and cheap: one PK-shaped read, and it returns immediately
        when the host already resolves. Callers invoke it before any
        session-keyed work. It never substitutes another actor's session on a
        read — ``auto_bind_session`` performs one explicit, audited BIND of
        the project's own activated session to the host that just spoke.
        """
        sid = str(host_session_id or "").strip()
        if not sid:
            # Identity-LESS callers keep the singleton path #672 deliberately
            # left open — there is nothing to bind and nothing substituted.
            return False
        try:
            from .managed_mode_service import resolve_managed_session

            # #1027 authority door: a binding that names no usable session is
            # NOT a reason to skip the heal -- healing it is the whole point.
            if resolve_managed_session(
                ...
                return False
            result = self.auto_bind_session(
                project_root=project_root,
                host_session_id=sid,
            )
        except Exception:
            # Best-effort: a failed heal leaves the caller exactly as it was
            # before this existed (unbound), never worse.
            return False
        return bool(result is not None and result.why and result.why[0] == "auto_bind_session")
    ...
    def auto_bind_session(
        ...
        *,
        ...
        """Any user message in an unbound state should auto-ACTIVATE for an
        AIDOCS-MANAGED project — and NEVER hard-block a non-AIDOCS one.

        "Managed" is decided by the single chokepoint ``is_aidocs_managed`` (today
        the .aidocs marker; migrating to the sqlite code index). Behaviour:
          - bind the most recent ACTIVE session, else resume the most recent
            NON-terminal session (both imply the project is already AIDOCS);
          - if there's NO bindable session but the project IS managed, bootstrap a
            fresh session (activating a genuine AIDOCS project is correct);
          - otherwise return empty — the project is NOT AIDOCS-managed, so the
            CALLER runs the prompt normally (no-op). We NEVER block or force
            managed mode onto a project that does not use AIDOCS (bug #234-2).

        The bound session_id is in ``audit_events[0][1]["bound_session_id"]``.
        Best-effort: any exception returns empty.
        """
        try:
            ...
            def _status(s: object) -> object:
                return s.get("status") if isinstance(s, dict) else getattr(s, "status", None)

            def _sid(obj: object) -> str:
                return str(
                    obj.get("session_id")
                    if isinstance(obj, dict)
                    else getattr(obj, "session_id", ""),
                ).strip()

            # THE PROJECT'S OWN ACTIVATION WINS (#672 fallout, measured
            # 2026-08-01). Before #672 this heal only ran when managed mode was
            # genuinely INACTIVE, so "no session is named yet" was true by
            # construction. #672 added a SECOND trigger: managed mode is active
            # as the project singleton but THIS host has no per-conductor row.
            # In that case a session IS already named — the one the operator
            # activated via the CLI or dashboard — and minting a fresh
            # `auto-<timestamp>` instead would silently DISCARD that activation
            # and strand every grant, sticky row and freeze under a session the
            # operator never named. #672's own note ("a project activated
            # WITHOUT host identity now refuses a host's tool calls until it
            # binds... #135 auto-bind self-heals") only holds if the heal binds
            # THAT session, which is what this does.
            #
            # This is NOT a re-admission of the #672 fail-open: nothing is
            # silently substituted on a read. We perform one explicit, audited
            # BIND (managed_mode_auto_activated) attaching the host that just
            # spoke to the project's own activated session.
            singleton_sid = ""
            try:
                from .managed_mode_service import resolve_managed_session

                # #1027 authority door: this id is what the host gets BOUND
                # TO, so it must name a session that can actually act.
                singleton_sid = resolve_managed_session(
                    self.runtime.hub.managed_mode,
                    project_root,
                )
            except Exception:
            ...
            active = [s for s in (sessions or []) if _status(s) == "active"]
            if singleton_sid:
                bind_sid = singleton_sid
            elif active:
                bind_sid = _sid(active[0])
            else:
                resumable = [
                    s
                    for s in (sessions or [])
                    if _status(s) not in ("active", "archived", "completed", "done", "deleted")
                ]
                if resumable:
                    bind_sid = _sid(resumable[0])
                elif is_aidocs_managed(project_root):
                    # Managed project, no bindable session -> bootstrap one (it IS
                    # an AIDOCS project, so activating is correct, not forcing).
                    from datetime import datetime, timezone

                    bind_sid = "auto-" + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
                    self.runtime.hub.sessions.create_session(
                        project_root,
                        bind_sid,
                        "Auto-activated session",
                        "user_prompt_submit",
                        "Auto-created on first prompt (managed mode)",
                    )
                else:
                    return PromptMutationResult.empty()
            if not bind_sid:
                return PromptMutationResult.empty()
            # #135: resolve THIS conductor's host id so auto-bind writes a
            # PER-CONDUCTOR mapping (isolation), consistent with session_start
            # (#134). Auto-bind is a first-bind seed (managed mode was inactive),
            # so the singleton is still written (restamp_singleton default True)
            # to keep `aidocs status` correct; the per-conductor row is what
            # prevents a second conductor's later reconnect from clobbering this
            # bind. host_session_id="" (no identity) preserves the prior
            # singleton-only behaviour.
            # Prefer the host id the CALLER already holds (the hook payload's
            # session_id). The ContextVar fallback below is only reachable for
            # callers that have no envelope of their own. Passing it explicitly
            # matters: on the hook path the ContextVar is not always populated
            # at the moment the transaction is opened, so relying on it alone
            # wrote the singleton WITHOUT a per-conductor row — the bind then
            # failed to resolve and a second heal had to repeat it.
            _hsid = str(host_session_id or "").strip()
            if not _hsid:
                try:
    ...
    def apply_dnt_grants(
        ...
        *,
        ...
        try:
            ...
            set_turn_read_files([])
            # The agent-research override, derived from THIS prompt. Without this
            # line the flag is never written at all, and the gate's advertised
            # escape hatch ("include 'delegate research' in their prompt") can
            # never fire — which is exactly what shipped, and what nobody noticed,
            # because a dead ALLOW-path is indistinguishable from a working gate:
            # both just refuse. Re-derived every turn, so it evaporates with the
            # turn (doctrine IX — operator intent is present-tense).
            from .protected_file_runtime import set_agent_research_override_from_prompt

            set_agent_research_override_from_prompt(prompt)
        except Exception:
            pass
        ...
        try:
            ...
            # #365: persist the agent-research override to sqlite too, so the
            # MCP-server process (the agent-brief gate) sees what THIS prompt
            # granted. The module-level flag set in the per-process block above
            # never crosses to the MCP server — which is exactly why the
            # 'delegate research' override was dead in production. Re-derive from
            # the ONE canonical phrase table.
            try:
                from .canonical_intent_registry import (
                ...
                # Provenance floor (WAR P Task 2) + #459 delegation-intent
                # SHAPE: the literal table and the lemma/proximity shape both
                # feed the SAME override flag, derived ONLY from operator-
                # authored text (harness-injected blocks stripped first).
                _low = strip_non_operator_text(prompt or "").lower()
                # #396 (WAR U): a UPS whose operator-authored residue is EMPTY
                # (pure harness injection: task-notification / system-reminder)
                # is not a new operator turn — it must not REVOKE the override
                # the operator's own message minted this turn. Skip the write;
                # the next real operator prompt still resets per-turn scope.
                if not ((prompt or "").strip() and not _low.strip()):
                    _research_granted = any(p in _low for p in AGENT_RESEARCH_OVERRIDE_PHRASES)
                    if not _research_granted:
                        _research_granted = detect_research_delegation_v2(_low)
                    gate.set_agent_research_override(
                        project_root, _grant_sid, _research_granted
                    )
            except Exception:
                pass
        except Exception:
    ...
    def record_user_prompt_received(
        ...
        *,
        ...
        Fresh-CLI detection: legacy singleton fallback hosts send a per-process
        session UUID in every UPS payload. A changed UUID raises
        ``requires_reconnect`` before inherited singleton state can be used.
        An authoritative per-conductor binding already proves that reconnect,
        so it never touches the deprecated session-global freshness stamp.
        ...
        try:
            ...
            # `managed` is retained ONLY for `resolved_via` (the fresh-CLI
            # check further down); the SESSION comes from the #1027 door.
            ...
            return PromptMutationResult.empty()
        # #1027: all three refusals this replaces returned the same empty
        # result, so there is no distinction to preserve here.
        if not session_id:
            return PromptMutationResult.empty()
        ...
        try:
            changed = (
                self.runtime.hub.query_gate.check_and_update_host_session_id(
                    project_root,
                    session_id,
                    host_session_id,
                )
                if str(managed.get("resolved_via") or "") != "per_conductor"
                else False
            )
            if changed:
                side_effects.append(
    ...
    def worker_lane_intercept(
        ...
        *,
        ...
        # Doctrine-survives-compaction for SUBAGENTS (#316 property 3):
        # the worker UPS path returns from this intercept BEFORE the
        # conductor-side PromptContextBuilder ever runs, so the
        # doctrine-resurface block never reached lane workers — a worker
        # that read a scroll (conductor-granted ai_skill) lost it on its
        # own compaction. The intercept carries the same lean pointer
        # tail: once per worker epoch, scrolls in the worker's own read
        # ledger re-surface after its epoch rotates. Fail-quiet — a
        # resurface hiccup must never break the lane wake.
        def _doctrine_tail() -> str:
            try:
                from .doctrine_resurface import maybe_doctrine_resurface_blocks

                blocks = maybe_doctrine_resurface_blocks(project_root)
        ...
        if worker_id:
            ...
            if msg is not None:
                injected = (
                    ...
                    f"then exit. The conductor resumes you (host --resume) "
                    f"if the next instruction requires it."
                )
                return PromptMutationResult(
                    decision="allow",
                    rewritten_prompt=injected + _doctrine_tail(),
                    side_effects=(
        ...
        protocol = (
            ...
            f"AIDOCS_EXPERT_RESULT, and exit. The conductor resumes you "
            f"(host --resume <session>) with any new instruction."
        )
        return PromptMutationResult(
            decision="allow",
            rewritten_prompt=protocol + _doctrine_tail(),
    ...
    def apply_per_turn_intent_state(
        ...
        *,
        ...
        try:
            ...
            per_turn = set()
        # STICKY REMOVED (operator ruling 2026-07-25: "remove sticky from aidocs
        # completely ... they were introduced as a bypass to all gates").
        #
        # This line WAS `sorted(per_turn | sticky)`, unioning the subcommands the
        # operator authorised IN THIS PROMPT with standing grants — so a bash
        # subcommand the operator never asked for this turn stayed permitted.
        # That union is precisely the gate bypass. Bash authority is now per-turn
        # only: what the operator said this turn, nothing inherited.
        #
        # Nothing replaces it, because nothing needs to: PERSISTENT bash
        # allowances already live in bash_policy's config-driven allow-list
        # (_evaluate_segment_allow), a separate and properly-governed surface.
        # Sticky's tier-2 bash grants were redundant with it.
        new_bash = sorted(per_turn)
        if new_bash != list(
            ...
            side_effects.append(f"bash_subs set (per_turn={len(per_turn)})")
        ...
        # 5. User-intent ACTION grants (bleed B, 2026-09-24). Canonical
        # {tool, mode, target} tuples the operator's own words asked for,
        # bound to the CURRENT operator turn (minted before this stage) and
        # consumed single-use by the local control-plane act they name.
        # Replaced every turn: an unused grant from the previous prompt is
        # cleared by this write. A gate without the action column (duck-typed
        # doubles) holds no grants, so there is nothing to write or clear.
        _get_actions = getattr(qg, "get_user_intent_actions", None)
        if _get_actions is not None:
            try:
                from .canonical_intent_registry import detect_user_intent_actions_v2

                detected_actions = list(detect_user_intent_actions_v2(prompt))
            except Exception:
                detected_actions = []  # detection doubt mints nothing
            actions: list[dict[str, str]] = []
            if detected_actions:
                try:
                    _turn = str(qg.get_current_turn_id(project_root, managed_session_id) or "")
                except Exception:
                    _turn = ""
                if _turn:  # no operator turn to bind to => mint nothing
                    import hashlib as _hl_actions

                    _content = _hl_actions.sha256((prompt or "").encode("utf-8")).hexdigest()
                    actions = [
                        {
                            "tool": a["tool"],
                            "mode": a["mode"],
                            "target": a["target"],
                            "turn_id": _turn,
                            "origin": "user_prompt_submit",
                            "provenance": "operator_text",
                            "instruction_sha256": _content,
                        }
                        for a in detected_actions
                    ]
            if actions != list(_get_actions(project_root, managed_session_id)):
                plan.add(
                    lambda v=actions: qg.set_user_intent_actions(
                        project_root,
                        managed_session_id,
                        v,
                    ),
                ...
                if actions:
                    side_effects.append(f"user intent actions granted (count={len(actions)})")
                    why.append("user_intent_actions")
    ...
    def _super_admin_preflight_failsafe(
        self,
        project_root: Path,
        *,
        rule_ids: str,
        session_id: str,
    ) -> PromptMutationResult | None:
        """Super-admin failsafe for a FORBIDDEN pre-flight verdict.

        Returns a NON-BLOCKING result (decision stays ``allow``, the prompt is
        never rewritten and never echoed back) carrying a sanitized advisory
        when the AUTHENTICATED caller holds ``security.preflight_failsafe`` —
        a permission only super_admin holds. Returns None (→ normal freeze) for
        every other caller, including an unauthenticated local session. The
        advisory names the rule_ids ONLY; the operator's prompt is never
        re-surfaced. Best-effort: any error returns None so a broken failsafe
        can never SUPPRESS the freeze it was meant to soften.
        """
        try:
            from .mcp_server_runtime_helpers import (
                current_calling_host_session_id,
            )
            from .permission_catalog import PERM_SECURITY_PREFLIGHT_FAILSAFE
            from .project_authority import _has_perm
            ...
            try:
                ...
                host_sid = ""
            if not _has_perm(
                project_root, PERM_SECURITY_PREFLIGHT_FAILSAFE, host_sid
            ):
                return None
        except Exception:
            return None
        ...
        advisory = (
            "🛡️ SUPER-ADMIN FAILSAFE — this prompt tripped a pre-flight safety "
            f"check ({rule_ids}). It was NOT blocked because you hold "
            "security.preflight_failsafe. Before acting on it, briefly confirm "
            "with the operator that this task is intended; if anything looks "
            "unintended, stop and ask rather than proceeding. "
            "(The prompt itself is intentionally not repeated here.)"
        )
        # Auditability rides on audit_events below (rule_ids only, never the
        # prompt); the agent-facing advisory rides on additional_context_blocks
        # THIS turn, so no next-turn block-notice record is needed.
        return PromptMutationResult(
            decision="allow",
            additional_context_blocks=(advisory,),
            audit_events=(
                (
                    "preflight_super_admin_failsafe",
                    {"rule_ids": rule_ids, "session_id": session_id},
                ),
            ),
            why=("preflight_super_admin_failsafe", rule_ids),
        )
    ...
    def preflight_judge(
        ...
        *,
        ...
        """
        # A harness-injected subagent task-notification is NOT an operator prompt
        # — it is a completion REPORT. Security-triage reports legitimately
        # describe bypass / permission / gate concepts and can carry truncation
        # noise, which false-positived the hostile-OPERATOR rules and froze a
        # session (2026-06-29 operator bug report). Skip the preflight for it:
        # matched on the full harness schema (open+close + task-id + status), not
        # a bare prefix, and it is data the agent reads, never an operator command
        # — so this is not a gate bypass. The rest of the UPS pipeline still runs.
        _stripped = (prompt or "").lstrip()
        if (
            _stripped.startswith("<task-notification>")
            and "</task-notification>" in prompt
            and "<task-id>" in prompt
            and "<status>" in prompt
        ):
            return PromptMutationResult.empty()
        # An operator pasting a PRIOR preflight/freeze BANNER back — to ask about
        # it or report a false positive — must not RE-trip the gate on the
        # banner's OWN wording (the rule_ids it names literally contain "BYPASS"/
        # "TAMPER"; the freeze prose says "frozen/recovery"). Strip ONLY the exact
        # banner templates from the JUDGE input (variable slots constrained to
        # rule-id / family / count / freeze-id shapes — no free-text slot a real
        # command could hide in), then judge the residual. Bypass-safe on two
        # counts: it changes only what the JUDGE reads (the agent still receives
        # the full original prompt), and the constrained slots cannot carry a
        # lowercase command — so a forged banner smuggles nothing past the judge.
        if "PRE-FLIGHT " in prompt.upper() or "SESSION LOCKED" in prompt.upper():
            import re as _re

            _opt = (
                r'(?: Freeze id: [\w-]+\. Clear with '
                r'admin_clear_freeze\(freeze_id="[\w-]+"\)\.)?'
            )
            _banner = _re.compile(
                r"🛑?\s*PRE-FLIGHT BLOCKED — hostile operator intent detected "
                r"\([A-Z0-9_, ]+\)\. The agent never saw the prompt\. If this is "
                r"a false positive, file the rule_id and prompt with the AIDOCS "
                r"team\."
                r"|🛑?\s*PRE-FLIGHT FLAG — operator intent flagged "
                r"\([A-Z0-9_, ]+\)\. Re-phrase the request or confirm intent "
                r"explicitly\. \(Freeze pipeline integration lands in #44 "
                r"Batch 7\.\)"
                r"|🛑?\s*SESSION LOCKED — immediate freeze \([a-z0-9_]+\)\. This "
                r"attempt class cannot be retried; the session is frozen on the "
                r"first hit\. Only operator/admin recovery can clear it\." + _opt
                + r"|🛑?\s*SESSION LOCKED — hostile operator prompt "
                r"\([a-z0-9_]+\)\. The session is frozen; only operator/admin "
                r"recovery can clear it\." + _opt
                + r"|🛑?\s*SESSION LOCKED — \d+ security violations "
                r"\([a-z0-9_]+\)\. This session is now frozen; only "
                r"operator/admin recovery can clear it\. The agent cannot lift "
                r"this lock by prompt\." + _opt,
                _re.IGNORECASE,
            )
            _residual = _banner.sub(" ", prompt)
            _residual = _residual.replace(
                "UserPromptSubmit operation blocked by hook:", " "
            )
            if "Original prompt:" in _residual:
                _residual = _residual.split("Original prompt:", 1)[1]
            prompt = _residual.strip()
            if not prompt:
                return PromptMutationResult.empty()
        try:
            ...
            # SECURITY FAIL-CLOSED (War-2 'One Gate & Policy', doctrine). The
            # preflight judge is a hostile-OPERATOR-prompt SECURITY gate; if its
            # evaluator cannot produce a trustworthy verdict, the prompt is
            # REFUSED. A security enforcement check that cannot run must DENY —
            # never let an UNEVALUATED (possibly hostile) prompt through. This is
            # distinct from the read/attribution gates that fail open on infra
            # error; a security judge fails closed. Pinned by
            # test_war2_one_gate_policy::test_preflight_evaluator_failure_fails_closed.
            return PromptMutationResult(
                decision="block",
                block_reason=(
                    "🛑 PRE-FLIGHT UNAVAILABLE (rule_id=preflight.evaluator_unavailable) "
                    "— the prompt evaluator could not produce a trustworthy decision, "
                    f"so the request was refused defensively ({type(exc).__name__}). "
                    "Retry after the evaluator is healthy."
                ),
                why=("preflight.evaluator_unavailable",),
            )
        ...
        if outcome.should_block:
            # Directive-vs-report drop (2026-06-29) applied to the FULL verdict
            # set so BOTH the freeze path and the confirmable fallback honor it.
            # Tamper rules match surface verbs and can't tell "bypass the gate"
            # (command) from a security REPORT describing a bypass — a subagent
            # triage report froze a session this way. Drop tamper-class verdicts
            # ONLY when spaCy confirms every trigger verb is descriptive
            # (3rd-person nominal subject, not imperative). Conservative +
            # fail-closed: NLP-down / any imperative keeps the block.
            try:
                from .aidocs_nlp.consumers.directive_intent import (
                    drop_descriptive_tamper,
                )

                verdicts = drop_descriptive_tamper(
                    prompt, list(outcome.verdicts), project_root=project_root
                )
            except Exception:
                verdicts = list(outcome.verdicts)
            if not verdicts:
                # Every should_block verdict was a confirmed descriptive report.
                return PromptMutationResult.empty()
            forbidden = [v for v in verdicts if v.risk == "critical"]
            # Resolve the managed session up front: needed both for the freeze
            # below AND to read security.judge_override at SESSION scope.
            sid = ""
            try:
                from .managed_mode_service import resolve_managed_session

                # #1027 authority door: this id both FREEZES a session and
                # reads security.judge_override at SESSION scope.
                sid = resolve_managed_session(self.runtime.hub.managed_mode, project_root)
            except Exception:
                sid = ""
            # Operator override (2026-06-29): a session may suppress specific
            # preflight rule_ids via security.judge_override — the SAME lever the
            # tool-call judge honors (agent_orchestrator:2310). Lets an operator
            # silence a preflight rule for one isolated session (e.g. red-team)
            # without disabling the gate everywhere. judge_override is dashboard-
            # only + security-sensitive, so only an operator grant sets it — an
            # agent cannot self-suppress. If nothing critical survives the
            # override, the prompt is NOT frozen.
            if forbidden:
                try:
                    from .config import get_setting
                    ...
                    _suppressed = set()
                if _suppressed:
                    forbidden = [v for v in forbidden if v.rule_id not in _suppressed]
            if forbidden:
                rule_ids = ", ".join(v.rule_id for v in forbidden[:3])
                # SUPER-ADMIN FAILSAFE (operator-requested 2026-07-16): a proven
                # super_admin (the sole holder of security.preflight_failsafe) is
                # NOT frozen on a forbidden UPS verdict. The prompt PASSES THROUGH
                # (decision stays allow) and the agent receives a sanitized
                # advisory — rule_ids only, never the prompt text — telling it to
                # confirm intent with the operator before acting. Fail-closed: an
                # unauthenticated or non-super_admin caller falls straight through
                # to the immediate freeze below.
                failsafe = self._super_admin_preflight_failsafe(
                    project_root, rule_ids=rule_ids, session_id=sid
                )
                ...
                if strike_msg:
                    block_reason += f"\n\n{strike_msg}"
                # #198: leave the agent a SANITIZED next-turn awareness notice
                # (rule_ids only — never the hostile prompt). Surfaced on the
                # agent's next non-blocked UPS. Best-effort; never breaks block.
                try:
                    if sid:
                        from .preflight_block_notice_store import (
                            PreflightBlockNoticeStore,
                        )

                        PreflightBlockNoticeStore().record(
                            project_root, sid, kind="forbidden", rule_ids=rule_ids
                        )
                except Exception:
                    pass
                return PromptMutationResult(
            ...
            confirmable_ids = ", ".join(v.rule_id for v in verdicts[:3])
            # #198: sanitized awareness notice for the FLAGGED (confirmable) path too.
            try:
                if sid:
                    from .preflight_block_notice_store import PreflightBlockNoticeStore

                    PreflightBlockNoticeStore().record(
                        project_root, sid, kind="confirmable", rule_ids=confirmable_ids
                    )
            except Exception:
                pass
            return PromptMutationResult(
    ...
    def escalation_scrub(
        ...
        *,
        ...
        try:
            ...
            # #1027 authority door: this id selects WHOSE escalation rows
            # scrub_and_process is allowed to mutate.
            session_id = (
                resolve_managed_session(self.runtime.hub.managed_mode, project_root) or None
            )
        ...
        *,
        ...
        if host_session_id:
            try:
                ...
                # `resolved_via` is still a get_mode fact; the SESSION comes
                # from the #1027 authority door.
                if m_pc.get("resolved_via") == "per_conductor":
                    session_id = resolve_managed_session(
                        self.runtime.hub.managed_mode,
                        project_root,
                        host_session_id=host_session_id,
        ...
        try:
            ...
            _host = (_cchsi() or "").strip()
        except Exception:
            _host = ""
        freeze = sfs.get_active_freeze(project_root, session_id, _host)
        if freeze is None:
            return PromptMutationResult.empty()
    ...
    def resolve_chat_unfreeze(
        ...
        *,
        ...
        try:
            ...
            # `resolved_via` is still a get_mode fact; the SESSION comes from
            # the #1027 authority door.
            if m_pc.get("resolved_via") == "per_conductor":
                session_id = resolve_managed_session(
                    self.runtime.hub.managed_mode,
                    project_root,
        ...
        # AUTHORITY: host-operator binding (RBAC) — no flavor path (#404).
    ...
    def notifications_drain(
        ...
        """Surface only notifications owned by this prompt's actor.

        NOT AN INDEPENDENT COLLECTOR ANY MORE (empire law
        `promoted-9f9756ad9462`). This used to peek `run_notifications` and
        `lane_completion_review_store` ITSELF, which made it a genuine SECOND
        emitter of the same event classes the MCP rail emits: it used
        `peek_for_session` (non-consuming), so it announced a background run and
        left the record pending, and the MCP rail then surfaced the very same
        run up to `notifications.max_displays` (default 3) more times. That is
        the operator-measured defect — one run notice, `r_34c5200a55cc3517`,
        announced on THREE separate tool results.

        It now DELEGATES to the one canonical composition authority,
        `notification_injector.compose_notices`, and shares that authority's
        DURABLE announce-once ledger. Because both surfaces claim the same
        dedup key in the same scope, whichever fires first wins and the other
        goes quiet — duplication is structurally impossible rather than merely
        avoided by two emitters happening to agree.

        WHICH SURFACE IS THE RAIL is decided by WHO CONTROLS THE HOST: an
        operator-controlled host has hooks, so this prompt path is its rail; a
        non-hook actor and an AIDOCS-spawned agent read the SAME obligation via
        the MCP result rail and `wait_next`. Neither path is the only path for
        an actor class that cannot use it.
        """
        if os.environ.get("AIDOCS_EXPERT_LANE_ID", "").strip():
            return PromptMutationResult.empty()

        host_session_id = str(payload.get("session_id") or "").strip()
        host_kind = str(payload.get("host_kind") or "claude_code").strip()
        managed_session_id = ""
        actor_id = ""
        try:
            ...
            # #1027 authority door: notifications are read FOR this session.
        ...
        blocks: list[str] = []
        # #1061 two-phase delivery: notices whose stream claims are confirmed
        # ONLY once their text is in the returned additionalContext blocks.
        placed_candidates: list = []
        if managed_session_id:
            # THE DELEGATED PATH — one authority, one durable dedup.
            try:
                from . import notification_injector as _ni

                notices = _ni.compose_notices(
                    project_root,
                    managed_session_id,
                    actor_id,
                    "",
                    # THIS PATH KNOWS THE HOST SESSION ID; the MCP rail has to
                    # read it off the calling thread. Passing it explicitly
                    # preserves the lane-review OR-match (a conductor that
                    # swapped sessions mid-run still sees its pending reviews).
                    host_session_id=host_session_id,
                )
                notices = _ni.announce_once(
                    project_root,
                    actor_id or managed_session_id,
                    notices,
                )
                rendered = _ni.render_notice_payload(notices)
                # KIND DETERMINES PLACEMENT, even on a text-only surface.
                # must_act goes in its OWN block so enforced-workflow traffic is
                # never concatenated with a weather report.
                must_act = rendered.get(_ni.MUST_ACT_KEY) or []
                if must_act:
                    # ONE BLOCK PER OBLIGATION, plus a leading marker.
                    # `additional_context_blocks` is a TUPLE OF BLOCKS, so
                    # separation on this surface means separate ELEMENTS — not a
                    # single concatenated string with a header. Concatenating
                    # them would also silently break every existing assertion of
                    # the form `"<store text>" in r.additional_context_blocks`,
                    # which is exact element equality, not a substring test.
                    blocks.append(
                        "⛔ ACTION REQUIRED — each block below demands a "
                        "specific action from you. Announced ONCE; the "
                        "obligation persists and stays queryable.",
                    )
                    blocks.extend(
                        str(entry.get("text") or "")
                        for entry in must_act
                        if str(entry.get("text") or "")
                    )
                info = str(rendered.get(_ni.NOTIFICATIONS_KEY) or "")
                if info:
                    blocks.append(info)
                placed_candidates = list(notices)
            except Exception:
                pass
        else:
            # UNBOUND-HOST FLOOR. No managed session resolved (e.g. a fresh CLI
            # host with no bound session), so the session-scoped composer has
            # nothing to read. Fall back to peek-all so notifications still
            # surface — baseline 17086d88 contract, pinned by
            # test_prompt_mutator_parity::test_unresolved_managed_falls_back_to_peek_all.
            #
            # DELIBERATELY NOT DEDUPED: with no session and no actor there is no
            # scope to anchor "once" against, and a floor that silently drops
            # traffic is worse than one that repeats it. Named as an exception
            # rather than left to look like the deduped path.
            try:
        ...
        # #1061 PHASE TWO on the UPS surface: confirm a stream claim only when
        # its notice text is actually one of the returned blocks. A render or
        # placement failure leaves it unconfirmed -> re-claimable once.
        if placed_candidates:
            try:
                from . import notification_injector as _ni

                placed_blocks = set(result.additional_context_blocks)
                _ni.confirm_surfaced_claims(
                    project_root,
                    [n for n in placed_candidates if n.claims and n.text in placed_blocks],
                )
