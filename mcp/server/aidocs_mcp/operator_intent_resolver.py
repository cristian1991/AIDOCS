...
## The seal, restated
  ...
  - Operator intent is ORIGIN-BOUND. Text quoted, forwarded, generated,
    ...
    (see is_operator_intent_eligible_prompt) may enter detection or
...
# the exact same origin law. Kept as a named alias so existing call sites
# and tests remain valid and intent-specific reads stay clear.
def is_operator_intent_eligible_prompt(context: dict | None) -> bool:
    """Alias of is_authority_bearing_prompt_eligible for the operator-
    intent consumer. Same origin law, no NLP.
    """
    return is_authority_bearing_prompt_eligible(context)
