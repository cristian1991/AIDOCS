...
class ActionSurfaceService:
    ...
    def compare(self, project_root: Path, query: str, session_id: str | None = None, limit: int = 20) -> dict[str, Any]:
        needle = query.strip()
        if not needle:
            return {
                "query": query,
        ...
        capabilities = self.hub.capabilities.find_capabilities(project_root, query=needle, limit=limit)
        all_capabilities = self.hub.capabilities.find_capabilities(project_root, query=None, limit=1000)
        events = self.hub.execution.list_events(project_root, query=needle, session_id=session_id, limit=limit)
        runs = self._filter_runs(
            self.hub.execution.list_runs(project_root, session_id=session_id, limit=max(limit * 5, 50)),
            needle=needle,
            limit=limit,
        )
        ...
        for procedure in procedures:
            ...
            if not procedure_id:
                continue
            for link in self.hub.procedure_links.list_links(project_root, procedure_id=procedure_id, limit=limit):
                seen_links.setdefault(str(link.get("link_id") or ""), link)
                capability_name = str(link.get("capability_name") or "").strip()
                if capability_name and not any(item.get("name") == capability_name for item in capabilities):
        ...
        resolved_link_count = sum(1 for item in links if item.get("resolution_status") == "resolved")
        unresolved_link_count = sum(1 for item in links if item.get("resolution_status") == "unresolved")
    ...
    def assess(self, project_root: Path, query: str, session_id: str | None = None, limit: int = 20) -> dict[str, Any]:
        ...
        assessment = comparison.get("assessment") if isinstance(comparison.get("assessment"), dict) else {}
        coverage = comparison.get("coverage") if isinstance(comparison.get("coverage"), dict) else {}
        history = comparison.get("history_summary") if isinstance(comparison.get("history_summary"), dict) else {}
        gaps = comparison.get("gap_summary") if isinstance(comparison.get("gap_summary"), dict) else {}
    ...
    def status_bundle(
        ...
        assessments = [self.assess(project_root, query=query, session_id=session_id, limit=limit) for query in normalized_queries]
        ...
        if attention_items:
            if any(item.get("state") in {"definition_without_capability", "execution_only", "unmapped"} for item in attention_items):
    ...
    def session_status_bundle(
        ...
        queries = self._derive_session_queries(project_root, session_id=session_id, max_queries=max_queries)
        bundle = self.status_bundle(project_root, queries=queries, session_id=session_id, limit=limit)
    ...
    def current_session_bundle(
        ...
        if not session_id:
            active = [item for item in self.hub.sessions.list_sessions(project_root) if item.status == "active"]
            if len(active) == 1:
        ...
        bundle = self.session_status_bundle(project_root, session_id=session_id, limit=limit, max_queries=max_queries)
        bundle["ready"] = True
        bundle["resolution"] = resolution
        bundle["managed_mode_active"] = bool(managed.get("active"))
        return bundle

    def _filter_runs(self, runs: list[dict[str, Any]], needle: str, limit: int) -> list[dict[str, Any]]:
        lower = needle.lower()
        matched = []
        for run in runs:
            haystacks = [
    ...
    def _build_history_summary(self, events: list[dict[str, Any]], runs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "latest_run_at": latest_run.get("completed_at") if latest_run else (latest_run.get("started_at") if latest_run else None),
        ...
        unresolved_actions = sorted(
            {
                ...
                for item in links
                if item.get("resolution_status") == "unresolved" and str(item.get("action_kind") or "").strip()
            }
        )
        return {
    ...
    def _suggest_capabilities(self, action_kind: str, capabilities: list[dict[str, Any]]) -> list[str]:
        ...
        for capability in capabilities:
            ...
            aliases = [str(item).strip().lower() for item in capability.get("aliases") or [] if str(item).strip()]
            score = 0
            if family and family in tokens:
                score += 3
            if any(alias in action_kind.lower() or action_kind.lower() in alias for alias in aliases):
                score += 4
    ...
    def _build_assessment(
        ...
        *,
        ...
        candidate_capabilities = gap_summary.get("candidate_capabilities") or {}
        if not has_definition:
            recommended_next_steps.append("Define or capture the intended procedure/workflow for this action.")
        if not has_capability and unresolved_actions:
            recommended_next_steps.append("Add or declare a callable capability for the unresolved workflow action.")
        if unresolved_actions:
            recommended_next_steps.append("Review candidate capabilities before adding any new alias or direct mapping.")
        if has_capability and not has_execution:
            recommended_next_steps.append("Capture real execution evidence for this capability through MCP invocation history.")
        if unresolved_link_count > 0 and not unresolved_actions:
            recommended_next_steps.append("Inspect unresolved procedure-capability links and verify whether the gap is real or naming-related.")
        if history_summary.get("run_count", 0) > 0 and resolved_link_count == 0 and has_capability:
            recommended_next_steps.append("Link observed execution back to a procedure or explicitly accept it as ad-hoc work.")
    ...
    def _build_findings(
        ...
        *,
        ...
        candidate_summary = gaps.get("candidate_capabilities") if isinstance(gaps.get("candidate_capabilities"), dict) else {}
        for action, candidates in list(candidate_summary.items())[:2]:
            if candidates:
                findings.append(f"Candidates for `{action}`: {', '.join(candidates[:3])}.")
        return findings[:6]

    def _derive_session_queries(self, project_root: Path, session_id: str, max_queries: int) -> list[str]:
