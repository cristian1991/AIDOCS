...
import { setViewedProjectId } from "./platform/transport";
...
export function useDashboardData() {
  ...
  // VIEWED project for every project-scoped web call (transport.withViewedProject).
  // Set during RENDER, not in an effect, on purpose: React runs child effects
  // BEFORE parent effects, so panel components mounting under this hook would
  // otherwise make their first gate calls before the viewed project was known —
  // and those calls would read the bound project instead. This writes a plain
  // module value (no React state, no re-render) and is idempotent, so it is
  // safe here.
  // Outside web scope it is RESET, not left holding the last web project: a
  // stale viewed id must never be waiting for the next web call.
  setViewedProjectId(getScope() === "web" ? selectedProjectRoot ?? null : null);
  ...
  useEffect(
    () =>
      onScopeChange(() => {
        ...
        // Drop the PREVIOUS source's data too (2026-09-17). The reload below
        // replaces it on success, but on FAILURE the catch only sets an error,
        // so the old snapshot stayed on screen under the NEW mode's label —
        // reading as current. Measured: an expired web token in WebAgent mode
        // left the LOCAL dashboard visible. Operator, verbatim: "i was clicking
        // one and the other and i'm not completely sure the data was changing".
        // Empty-then-loading is honest; stale-but-labelled-new is not.
        setSnapshot(null);
        setProjects([]);
        setTomlDocuments([]);
        setError(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Web scope: VIEW the current (or first) project when nothing is chosen
        // yet — this is the dashboard's own UI selection, passed to
        // dashboard_snapshot as project_id. It is NOT a gate bind: since
        // 2026-09-17 viewing never calls project_select, because that rewrote the
        // account binding and cleared the selected session on every surface.
