...
class RuntimeBootstrapOrchestrationService:
    ...
    def project_bootstrap_or_resume(
        ...
        # and multi-language (§IX), not a hardcoded English regex. Self-heals a
        ...
        # #1095 (r0b2 655e9231-9d9): the skill catalog READ no longer seeds the
        # empire library or migrates the legacy skill-providers.json; an
        # explicit bootstrap is a lifecycle rail, so it converges both through
        # the ONE helper (fail-open; never fails the bootstrap).
        try:
            from .skill_store import run_skill_authority_lifecycle

            run_skill_authority_lifecycle(project_root, self.hub.skills)
        except Exception as exc:
            self._logger.debug("run_skill_authority_lifecycle: %s", exc)
    ...
    def _heal_empty_palace(self, project_root: Path) -> dict | None:
        """Per-launch palace recovery for staged intents and legacy emptiness.

        Durable staged projections are drained even when the palace is already
        populated. This heals prior read-back failures and saturated capture
        queues. If the palace was empty at entry, the stamped receipted
        migrator runs so pre-staging projects converge too. Recovery remains
        best-effort and never blocks bootstrap.
        """
        palace = getattr(self.hub, "palace", None)
        if palace is None:
            return None
        try:
            from .memory_body_staging_store import drain_staged
            from .memory_home_migrator import migrate_bodies_with_ingest_summary
            from .memory_sqlite_store import list_entries
            ...
            was_empty = int(getattr(status, "total_drawers", 0) or 0) == 0
            ...
            if was_empty and list_entries(project_root):
                stats = migrate_bodies_with_ingest_summary(
                    project_root,
                    palace,
                    hub_ctx=ctx,
                )
                result = {"healed": True, **stats}
            if recovery.get("scanned"):
                if result is None:
                    result = {"healed": bool(recovery.get("landed"))}
                result["staged_recovery"] = recovery
            if result is not None:
                self._logger.info("palace heal ingest: %s", result)
            return result
        except Exception:
