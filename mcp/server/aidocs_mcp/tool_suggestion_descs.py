...
TOOL_USAGE_DESCS: dict[str, str] = {
    ...
    "schema_query": "DB schema: entities, columns, relationships",
    "ai_replace": (
        "unified edit: mode=anchor(between two unique anchors, PRIMARY) | "
        "string(old≤200 chars) | symbol('Class.method'+new_body) | lines"
    ),
    ...
    "ai_task": "lifecycle: begin before non-trivial work → update at steps → complete with result+evidence",
    ...
    "git_ops": "governed git operations (status/diff/commit/push)",
}
