...
def sticky_grant_worthy(tool: str) -> bool:
    """True iff a STANDING grant for ``tool`` could ever confer anything.

    #496 / operator ruling 2026-07-23: "sticky should only exist for
    UNALLOWED commands — commands an agent cannot call without user
    intent". A standing grant for an already-callable tool is pure risk
    with zero benefit and must never be minted.

    "Already callable" is not a guess — it is read off the gate. The
    ONLY consumer of a tool-name grant is agent_orchestrator's
    ``user_granted`` flag, and that flag is consulted by exactly the raw
    shell / raw file / raw edit gates in access_gate, plus the
    conductor-only tool list. Every other tool (all MCP tools, deferred
    ones included — those are reachable via ToolSearch) is callable with
    no grant at all, so a grant for one is a no-op with a permanent
    audit footprint.

    Fails CLOSED: if the gate's own tables cannot be read we cannot
    prove the tool is denied by default, so the answer is "not worthy"
    ...
    try:
        ...
        denied: set[str] = set()
        denied |= {t.lower() for t in _BLOCKED_RAW_FILE_TOOLS}
        denied |= {t.lower() for t in _RAW_EDIT_TOOLS}
        denied |= {t.lower() for t in _RAW_SHELL_TOOLS}
        denied |= {t.lower() for t in AccessGate._CONDUCTOR_ONLY_TOOLS}
    except Exception:
        return False
    return name in denied
...
class QueryGateStore:
    ...
    def _write_sticky(
        ...
        *,
        ...
        try:
            ...
            # #496 note: grant-WORTHINESS (is this tool even deniable?)
            # is enforced one level up, at add_sticky_user_intent_tools —
            # the Layer-2 NLP door every one of the 148 unconsented
            # standing grants came through (registration_source=
            # layer2_direct). It is deliberately NOT enforced here: this
            # sink is also the revoke-sync path, and filtering the whole
            # set would revoke pre-existing operator-facing rows as a
            # side effect of a normal write. The remaining doors
            # (set_user_intent_tools(sticky=True), snapshot_restore, the
            # cli operator grant) are fed only by explicit raw-tool
            # detection or a deliberate operator command.
            # Add tools that appeared this turn.
            for tool in new_set - existing:
                try:
                    # PROVENANCE, not a hardcoded blank. A grant is a consent
                    # record; these fields are the only trace of WHO consented
                    # and WITH WHAT WORDS. They were pinned to "" for every path
                    # — operator-phrase grants included — which is why 91% of
                    # rows in a live install (161 of 176) name no consenting
                    # party.
                    #
                    # judge_verdict records the REAL write path. It used to be
                    # stamped "legacy-write-path", which is a euphemism, not a
                    # fact: it labelled a defect and then blessed it as a
                    # recognised class, making consented and auto-minted grants
                    # indistinguishable while implying the state was acceptable.
                    # The honest signal needs no marker — a grant with an empty
                    # phrase AND an empty user simply HAS no provenance, and that
                    # is queryable directly. So the source is recorded as-is and
                    # nothing is labelled "legacy".
                    store.register_grant(
                        ...
                        judge_verdict=source,
                        confirmation_answer=phrase,
                        registration_source=source,
                    )
                except Exception:
            ...
            for tool in existing - new_set:
                try:
                    store.revoke_tool(
                        ...
                        session_id=session_id,
                        tool=tool,
                        # States the FACT — the tool left the sticky set on this
                        # turn's full-replacement write. The old
                        # "legacy-write-path-removed" named a code path we had
                        # decided to call legacy instead of naming what happened,
                        # which tells an auditor reading the revoke reason nothing.
                        revoked_reason=f"removed from sticky set by {source}",
                    )
                except Exception:
                    pass
        except Exception:
    ...
    def add_sticky_user_intent_tools(
        ...
        if not new_tools:
            return
        # #496 defect (A) — OVER-MINTING BY TIER. Operator ruling
        # 2026-07-23: "sticky should only exist for UNALLOWED commands —
        # commands an agent cannot call without user intent." This is the
        # Layer-2 NLP door, and it is where all 148 unconsented standing
        # grants came from (registration_source=layer2_direct): the
        # detector surfaced already-callable MCP tools from ambient prose
        # and each one became a permanent tier-1 row conferring nothing.
        # Refuse them here, BEFORE the union — a standing grant for an
        # already-callable tool is pure risk with zero benefit. Fails
        # closed (sticky_grant_worthy returns False when the gate's own
        # default-deny tables can't be read).
        refused = sorted(t for t in new_tools if not sticky_grant_worthy(t))
        new_tools = {t for t in new_tools if sticky_grant_worthy(t)}
        if refused:
            try:
                from .execution_index_store import ExecutionIndexStore
                ...
                ExecutionIndexStore().record_event(
                    project_root,
                    event_kind="sticky_grant_refused_already_callable",
                    source_kind="query_gate.add_sticky_user_intent_tools",
                    session_id=session_id or None,
                    capability_name="",
                    action_kind="grant_worthiness",
                    target_entity=session_id,
                    status="refused",
                    payload={
                        "session_id": session_id,
                        "registration_source": "layer2_direct",
                        "refused_tokens": refused,
                        "kept_tokens": sorted(new_tools),
                        "reason": (
                            "These tools are callable WITHOUT a grant — no "
                            "default-deny gate keys on their tool name, and "
                            "deferred MCP tools are reachable via ToolSearch. "
                            "A standing tier-1 grant would confer nothing "
                            "while leaving permanent authority in the ledger "
                            "(#496 defect A)."
                        ),
                    },
                )
    ...
    def set_current_task_id(
        ...
        """Audit-stamp hook: task_begin writes; the empty string is the
        WHOLESALE reset (every open-task holder purged — #599's reclaim path).
        To close ONE actor's task use :meth:`close_session_task`."""
        self._init(project_root)
    ...
    def close_session_task(
        ...
        """#599: close one task without revoking a co-hosted peer's.

        Returns the session's new current task id — the most recent still-open
        holder, or "" when this was the last one.
        """
        self._init(project_root)
        return self._store.close_session_task(project_root, session_id, task_id)
    ...
    def set_user_intent_actions(
        ...
        """Replace this turn's operator action grants (UPS writer only)."""
        self._init(project_root)
        self._store.set_user_intent_actions(project_root, session_id, actions)
    ...
    def get_user_intent_actions(
        ...
        self._init(project_root)
        return self._store.get_user_intent_actions(project_root, session_id)
    ...
    def consume_user_intent_action(
        ...
        *,
        ...
        """Atomic single-use consumption; see the store for the contract."""
        self._init(project_root)
        return self._store.consume_user_intent_action(
            project_root,
            session_id,
            tool=tool,
            mode=mode,
