...
class ReadOnlyTierRExecutor:
    ...
    def _ensure_server(self):
        if self._server is None:
            if self._server_factory is not None:
                ...
                self._server = create_server(tools_profile="read_only")
        return self._server
    ...
    def _validate_binding(self, root: Path) -> None:
        if not _is_aidocs_project(root):
            raise ExecutorRefusal(
                "project_not_aidocs",
                f"bound project {root} is not an AIDOCS project",
            )
        if not _index_populated(root):
            raise ExecutorRefusal(
                "project_unindexed",
    ...
    def run(
        ...
        *,
        ...
        with self._lock:
            ...
            prev_hid = current_calling_host_session_id()
            set_calling_conductor_host_session_id(hid)
            try:
                ...
                set_calling_conductor_host_session_id(prev_hid)
    ...
    def _ensure_remote_context(srv, root: Path, session_id: str, host_session_id: str = "") -> None:
        ...
        try:
            ...
            ManagedModeService().set_mode(
                ...
                session_id,
                source="outer_gate",
                cli_session_id=host_session_id or session_id,
            )
