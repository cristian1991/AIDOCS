...
import re
import sqlite3  # still needed for sqlite3.Error below
import time
from pathlib import Path

# #814/#755: the canonical connect establishes journal_mode=WAL (persisted in
# the FILE HEADER), synchronous, busy_timeout and foreign_keys=ON. On the
# default rollback journal a writer takes an EXCLUSIVE lock over the whole
# file, so a concurrent reader cannot overlap and gets SQLITE_BUSY — the bare
# "database is locked" #746 chased. This store is on the governed prompt path,
# which is exactly where that degradation was observed.
#
# It also closes a second bug: every call site below uses `with connect(...)`,
# and a plain sqlite3 connection's `with` is a TRANSACTION context manager that
# never CLOSES the handle. ClosingConnection commits and then closes (#756).
...
class PreflightBlockNoticeStore:
    ...
    def record(self, project_root: Path, session_id: str, *, kind: str, rule_ids: str) -> bool:
        ...
        with _canonical_connect(self.db_path(project_root)) as conn:
            conn.execute(
