...
# Task|Agent: Claude Code's subagent-spawn tool is surfaced as `Agent`; older
# builds / other hosts call it `Task`. The gate logic already handles BOTH
# (tool_gate_service.agent_dispatch_brief fires for tool_name in {task, agent}),
# but the matcher listed only `Task` — so on current CC the PreToolUse hook never
# fired for an `Agent` spawn and the brief gate was silently bypassed ("agents
# cheat"): spawns escaped gating, auditing, and parent-linkage. Both names are
# matched now so every subagent dispatch is governed. (2026-07-09)
_PRE_MATCHER = (
    "Read|Edit|Write|Glob|Grep|Bash|MultiEdit|Patch|Search|ListDir|Task|Agent|"
