...
class GateDecision:
    ...
    def __post_init__(self) -> None:
        ...
        # names the condition + recovery instead of a generic block.
        self.reason = decorate_refusal(self.reason)
    ...
    if _is_external_path(normalized):
        ...
        try:
            ...
            if _managed and normalized.lower().startswith(_managed.lower() + "/"):
                ...
                try:
                    from .session_artifact import is_own_session_workspace

                    _own = is_own_session_workspace(
                        normalized,
                        project_root=gate_state.get("project_root"),
                        host_session_ids=gate_state.get("host_session_ids"),
                    )
                ...
                return GateDecision(
                    ...
                    level="read_gate",
                    reason=(
                        f"Read blocked: '{normalized}' is another session's managed "
                        f"task-artifact output (cross-session isolation)."
                    ),
                )
        except Exception:
            pass
...
def check_bash_allowed(
    ...
    *,
    ...
    the canonical [bash] declarative table. This shim now hard-fails
    ...
    raise NotImplementedError(
        ...
        "bash_policy.evaluate_bash_policy with the canonical `bash` "
