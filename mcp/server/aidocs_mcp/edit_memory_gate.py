...
class GateResult:
    ...
    #: #910 -- which of the states above the palace axis reached, and why. The
    #: POLICY is unchanged (an unreachable palace still does not block an edit);
    #: what changes is that "not consulted" is now VISIBLE instead of being
    #: inferred from a None that also means "consulted and clean".
...
def check_edit_memory_gate(
    ...
    *,
    ...
    if palace_axis in PALACE_DEFECT_STATES:
        # LOUD, because the whole point is that this used to be inaudible. The
        # edit still proceeds; somebody just gets to find out that a security
        # axis did not run.
    ...
    if not memories and (palace_result is None or palace_result.allowed):
        return GateResult(
            allowed=True, palace_axis=palace_axis, palace_note=palace_note
        )
