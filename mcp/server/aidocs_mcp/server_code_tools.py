...
def _text_empty_envelope(
    ...
    *,
    ...
    return {
        ...
        "results": [],
        "empty_reason": reason,
        # Back-compat: pre-#482 consumers keyed on 'no_text_match'.
        "empty_reason_legacy": "no_text_match",
...
def deslop_apply_execute(
    ...
    *,
    ...
    # The first live mutation proved the extract path does no import
    # management — it can leave the target using names it never imports and
    # the source calling the symbol it just removed. A refactor that changes
    # which names resolve is NEVER silently acceptable: scan every changed
    # .py file for NEW unresolved names vs its pre-mutation version; on any
    # hit, roll the filesystem back to the captured before-bytes (removing
    # created files) and REFUSE.
    if applied:
        ...
        if new_unresolved:
            ...
            return {
                ...
                "error": (
                    ...
                    "removed symbol still referenced) — filesystem rolled back to "
                    "the pre-mutation state; nothing was applied. Fix the plan "
                    "(imports must move with the code) and retry."
                ),
                "diff": diffs,
            }
    ...
    *,
    ...
    def ai_text_search(
        ...
        if not matches:
            ...
            return _text_empty_envelope(
                hub, root, scope=(glob or None), include_tests=include_tests,
            )
    ...
    def ai_find(
        ...
        if m in ("text", "regex", "string"):
            ...
            if not matches:
                ...
                return _text_empty_envelope(
                    hub, root, scope=(path or None), include_tests=include_tests,
                )
        ...
        def _grant(
            ...
            return _surface_knowledge(
                _inject_index_staleness(result, root), query=query, root=root
            )
    ...
    @server.tool(
        annotations={
            "destructiveHint": True,
            "openWorldHint": False,
            "title": "Promote Memory to Empire",
        },
    )
    def memory_promote(
        content: str = "",
        ...
        the archive; law enters only through the throne):
          ...
          * RBAC admin.manage_config gates the actor.
    ...
    def memory_capture(
        ...
        # the archive; law enters only through the throne. Promotion to the
        # empire tier stays an operator/trusted-path act.
        _empire_candidate, _empire_reason = is_empire_candidate(kind, content)
        ...
        if _palace is not None and target_rel:
            ...
            def _do_palace_ingest():
                from .memory_body_staging_store import project_staged_entry
                from .mcp_server_runtime_helpers import with_target_project_root
                from .palace_hub_extension import build_palace_context

                # Pin the CAPTURE's already-resolved project_root for the whole
                # ingest (build_palace_context + PalaceService path resolvers all
                # call resolve_project_root again). The worker copies the
                # enqueuing context too, but this binding makes the closure
                # self-sufficient regardless of where it runs — under
                # mcp.multitenant_strict an unpinned re-resolution REFUSES and
                # the projection silently died (2026-07-17 stall).
                with with_target_project_root(project_root):
                    ...
                    # #375 Phase 3 (A): the projector leg — ingest, read the
                    # drawer back, receipt, THEN retire the staged row and
                    # flip body_home='palace'. Any failure leaves the staged
                    # row intact/retriable (verify-then-retire).
                    return project_staged_entry(
                        project_root,
                        _palace,
                        target_rel,
                        hub_ctx=_palace_ctx,
        ...
        if _empire_candidate:
            notes.append(
                "empire-worthy — promote via ai_memory mode='promote' "
