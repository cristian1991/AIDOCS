...
# #755/#756: the ONE canonical connect. The single site here opened a
# handle with no pragmas at all (foreign_keys OFF, no busy_timeout,
# synchronous=FULL). The gate only ever SELECTs -- code_teaches hits and
# similarity over memory_index -- so read_only=True is the truthful mode:
# sqlite itself now refuses a write from the capture gate, and the reader
# still carries the three per-connection pragmas it had none of.
