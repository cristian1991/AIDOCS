...
def classify_frontend_symbol_kind(symbol: str, fallback_kind: str = "function") -> str:
    if symbol.startswith("use") and len(symbol) > 3 and symbol[3:4].isupper():
        return "hook"
    if symbol[:1].isupper() and symbol.endswith("Provider"):
        return "context_provider"
    if symbol[:1].isupper() and fallback_kind == "function":
        return "component"
...
class FrontendAstExtractor:
    ...
    def extract_outline(
        ...
        text: str,
        language: str,
    ) -> list[tuple[str, str, int, str | None, bool]] | None:
    ...
    def _detect_support(self) -> FrontendAstSupport:
        reason = tree_sitter_support_reason("tree_sitter_javascript", "tree_sitter_typescript")
        if reason is not None:
            return FrontendAstSupport(False, reason)
    ...
    def _get_parser(self, language: str):
        ...
        try:
            if language in {"javascript", "jsx"}:
                parser = create_tree_sitter_parser("tree_sitter_javascript", "language")
            elif language == "tsx":
                parser = create_tree_sitter_parser("tree_sitter_typescript", "language_tsx")
            else:
                parser = create_tree_sitter_parser("tree_sitter_typescript", "language_typescript")
        except Exception:
    ...
    def _frontend_kind(self, symbol: str) -> str:
        return classify_frontend_symbol_kind(symbol)
