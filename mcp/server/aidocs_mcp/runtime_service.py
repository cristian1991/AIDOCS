...
from .skill_resolution import (
    ...
    selected_skill_trigger_identity,
)
...
def _resolve_intent_tokens_dir() -> Path:
    ...
    seed dir post-Phase 5b. Per king-directive 2026-05-13: intent vocabulary
...
class RuntimeService:
    ...
    def _resolve_skill_trigger_state(
        ...
        selected_skills = [str(item) for item in selected.get("selected_skills", [])]
        available_skills = {
            str(item.get("skill_id")): item for item in self.hub.skills.list_skills(project_root)
        }
    ...
    def host_state(
        ...
        *,
        ...
        return self._session_state.host_state(
            project_root, session_id, prompt_text, verify_index=verify_index,
        )
    ...
    def _result_artifacts_root(self, project_root: Path, session_id: str | None = None) -> Path:
    ...
    def _sync_bootstrap_indexes(self, project_root: Path, include_tests: bool) -> dict[str, object]:
        ...
        # Push canonical bootstrap-law memory (rules/domains/roadmaps) into
        # the palace as drawers so PalaceService.get_bootstrap_context can
        # serve them at bootstrap WITHOUT reading .MEMORY/*.md. memory_index
        # is the source; markdown is export only. Best-effort: an ingest
        # failure leaves the prior palace state untouched (incremental-only).
        palace_ingest = {"ingested": 0, "skipped": 0, "failed": 0}
        palace = getattr(self.hub, "palace", None)
        if palace is not None:
            try:
                from .memory_sqlite_store import palace_ingest_from_canonical
                from .palace_hub_extension import build_palace_context
                ...
                palace_ingest = palace_ingest_from_canonical(
                    project_root,
                    palace,
                    only_kinds=("rule", "domain", "roadmap"),
                    hub_ctx=ctx,
                )
            except Exception:
    ...
    def aidocs_route_prompt(
        ...
        *,
        ...
        managed = self.hub.managed_mode.get_mode(
            project_root, host_session_id=host_session_id
        )
    ...
    def grant_raw_tools_for_lane(
        ...
        try:
            ...
            in_managed_mode = bool(managed.get("active"))
        except Exception:
    ...
    def verification_gate(
        ...
        if not tdd_mode:
            return {
                ...
                "commands_run": [
                    str(item).strip()
                    for item in ((verification_evidence or {}).get("commands_run") or [])
                    if str(item).strip()
                ],
                "command_results": [],
            }
        ...
        required_commands = self._clean_bullets(context_sections.get("Relevant Commands", []))
        ...
        if not commands_run:
            ...
            if required_commands:
                ...
                hint = (
                    f" Pass verification_evidence={{'commands_run': [...]}} "
                    f"with each required command: {preview}"
                )
            return {
                "session_id": session_id,
                "lane_id": lane_id,
                ...
                "reason": (
                    "evidence.commands_run is empty — no verification "
                    "commands recorded for this task." + hint
                ),
                "required_commands": required_commands,
                "commands_run": commands_run,
                "command_results": command_results,
    ...
    def task_begin(
        ...
        try:
            ...
            task_id = ""  # Non-fatal — task_begin still succeeds.
        ...
        if not _open_task:
            return {
                ...
                "error": (
                    ...
                    "task_begin first, do the work, then task_complete."
                ),
            }
