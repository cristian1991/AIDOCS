...
shell adapter evaluates it as ``ai_run``), so there is no separate "native is
unusable" table — ``raw_bash`` is the inert legacy namespace.
...
still flows through (``evaluate_bash_policy`` → judge). It never resurrects
``raw_bash``/``allow_raw_shell``/ungated shell.
"""
