...
def run_deploy(
    ...
    *,
    ...
    # §5: deploy the PINNED immutable commit recorded at enqueue, not the mutable ref (which may have
    # moved since the operator approved). Fall back to the ref only for a legacy request with no pin.
    target = str(payload.get("commit_sha") or ref)
    ...
    rc, log = dev_test(target)
    if rc != 0:
        _to(states.advance(states.TESTING, states.FAILED))
        queue.write_log(queue_dir, deploy_id, log or "")
    ...
    rc, plog = promote(target)
    final = states.advance(states.PROMOTING, states.OK if rc == 0 else states.ROLLED_BACK)
    _to(final)
    full_log = (log or "") + (plog or "")
    queue.write_log(queue_dir, deploy_id, full_log)
    return {"ok": rc == 0, "rc": rc, "state": final, "log": full_log}
...
def run_post_sign_deploy(
    ...
    *,
    ...
    # §5: deploy the PINNED immutable commit recorded at enqueue, not the mutable ref (the runner
    # already re-validated it is a 40-hex commit_sha). Fall back to ref only for a legacy request.
    target = str(payload.get("commit_sha") or ref)
