"""SQL store for lane completion reviews — aidocs-doctrine §VIII.
...
# DURABILITY: AUDIT, i.e. the FULL this file already had. record_verdict
# writes the CONDUCTOR'S DECISION -- the approve that lets a lane call
# task_complete, or the deny that recycles it. That is an authority
# decision with a named decider, and its absence after a crash is itself
# the finding ("no lane self-declares done"). Verdicts arrive at human
# speed, so keeping FULL costs nothing.
...
def init_db(project_root: Path) -> None:
    ...
    with _canonical_connect(
        path, durability=_Durability.AUDIT, row_factory=False
    ) as conn:
        conn.executescript("""
...
def get_review(project_root: Path, review_id: str) -> dict[str, Any] | None:
    ...
    init_db(project_root)
    with _canonical_connect(
        _db_path(project_root), durability=_Durability.AUDIT
    ) as conn:
...
def submit_verdict(
    ...
    *,
    ...
    updated = get_review(project_root, review_id) or {"error": "post-update fetch failed"}
    # WAR D (#452) Task 4: the verdict lands in execution_events with
    # actor attribution (record_event stamps user_id/agent_epoch). Store-
    # level so EVERY caller of submit_verdict is audited (one gate).
    if not updated.get("error"):
        try:
            ...
            ExecutionIndexStore().record_event(
                project_root,
                event_kind="lane_review_verdict",
                source_kind="mcp",
                session_id=conductor_session_id
                or str(updated.get("session_id") or "")
                or None,
                capability_name="ai_review",
                action_kind="review",
                target_entity=str(updated.get("lane_id") or review_id),
                status=verdict,
                payload={
                    "review_id": review_id,
                    "verdict": verdict,
                    "lane_id": str(updated.get("lane_id") or ""),
                    "worker_id": str(updated.get("worker_id") or ""),
