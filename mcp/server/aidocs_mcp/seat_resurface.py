...
def _full_block(granted: tuple[str, ...]) -> str:
    lines = [
        ...
        "- Law — cross-project: ai_skill('empire-doctrine'); project: ai_skill('aidocs-doctrine')",
        "- Field manual: scratch/co-co reports/v5.md (retired curriculum: archive/)",
...
def _lean_block(granted: tuple[str, ...]) -> str:
    ...
    return (
        ...
        "ai_skill('head-conductor'), ai_skill('empire-doctrine'), ai_skill('aidocs-doctrine'), "
        "v5.md, ai_soul('head-conductor-soul')" + g + "</aidocs-seat-resurface>"
    )
