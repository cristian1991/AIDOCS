...
#: Vendored modules that carry a RUNTIME-REACHABLE CAPABILITY (process spawn,
#: shell, or network egress) and are NOT reachable from this package (#925g).
#:
#: THESE ARE EXACT CALLSITE EXEMPTIONS, NOT A TREE EXEMPTION. `aidocs_mcp/_vendor`
#: ships upstream code inside the signed package, and a blanket `_vendor/**`
#: skip on the spawn/egress laws would hide real capability behind the word
#: "vendored" — a census that omits executable paths is not a census. So each
#: module is named, with the reason, and `contract_import_closure` PROVES the
#: reason on every run: the moment one of these becomes reachable from
#: MEMPALACE_CONTRACT_MODULES, the proof fails and the exemption dies with it.
#:
#: Reachability is what makes them exempt. AIDOCS imports six mempalace modules
#: (above); these seven are upstream's own CLI, daemon, hook installer, scanner
#: and LLM clients, none of which appear in the import closure of those six.
#: They ship because the engine ships whole — removing files from a signed
#: vendored tree would break byte-equivalence with the canonical tree, which is
#: the property the release signature rests on.
VENDORED_UNREACHABLE_CAPABILITY_MODULES: tuple[tuple[str, str], ...] = (
    ("mempalace/cli.py", "upstream CLI entry point (os.execve + subprocess.run re-exec)"),
    ("mempalace/daemon.py", "upstream daemon supervisor (subprocess.Popen)"),
    ("mempalace/hooks_cli.py", "upstream hook installer (subprocess.Popen/run)"),
    ("mempalace/project_scanner.py", "upstream repo scanner (subprocess.run: git)"),
    ("mempalace/closet_llm.py", "upstream LLM client (urllib egress)"),
    ("mempalace/entity_registry.py", "upstream entity lookup (urllib egress)"),
    ("mempalace/llm_client.py", "upstream LLM client (urllib egress)"),
)


def contract_import_closure(vendor_pkg) -> frozenset[str]:
    """Every mempalace submodule reachable from MEMPALACE_CONTRACT_MODULES.

    THE PROOF BEHIND EVERY EXEMPTION ABOVE, recomputed rather than asserted. A
    static import walk over the vendored tree, seeded with the modules this
    package actually imports; anything outside the result cannot execute as a
    consequence of embedding the engine.

    Static is sufficient HERE and only here: the vendored tree performs no
    dynamic import of its own submodules (verified — the only `importlib`
    mentions are `util`/`metadata` and a reload comment). A tree that started
    doing so would need this replaced, not extended.

    Returns module names relative to ``mempalace`` (e.g. "palace", "miner").
    """
    import ast
    from pathlib import Path

    root = Path(vendor_pkg)
    seed = [m.split(".", 1)[1] for m in MEMPALACE_CONTRACT_MODULES if "." in m]

    def _imports(mod: str) -> list[str]:
        path = root / f"{mod}.py"
        if not path.is_file():
            path = root / mod / "__init__.py"
            if not path.is_file():
                return []
        try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                if (node.module or "").startswith("mempalace"):
                    if "." in (node.module or ""):
                ...
                found.extend(
                    a.name.split(".", 1)[1]
                    for a in node.names
                    if a.name.startswith("mempalace.")
                )
    ...
    while stack:
        ...
        if mod in seen:
            continue
        seen.add(mod)
        stack.extend(d for d in _imports(mod) if d not in seen)
