...
def _ensure_pair_files_read(
    ...
    if missing:
        raise ValueError(
            f"🛑 PAIR-FILE CONTEXT MISSING: {protected_path}. Before "
            f"editing this protected file you must first read the pair "
            f"files named in its header: {', '.join(missing)}. Call "
            f"ai_get_lines on each, then retry.",
        )
    ...
    *,
    ...
    if max_old_chars > 0 and len(old_str) > max_old_chars:
        return {
            ...
            "replacements": 0,
            "error": (
                f"old_string too long ({len(old_str)} chars, limit "
                f"{max_old_chars}). Use ai_replace(mode='anchor', "
                f"start_anchor, end_anchor, replacement) for big "
                f"spans OR ai_replace(mode='symbol', symbol, new_body) "
                f"for whole bodies."
            ),
        }
    ...
    try:
        ...
        with abs_path.open(encoding="utf-8", newline="") as _f:
            content = _f.read()
...
def anchor_replace(
    ...
    *,
    ...
    new_content = content[:span_left] + replacement + content[span_right:]
