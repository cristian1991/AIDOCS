...
class MemoryWriteResult:
    ...
    # Backlog #220 observability: True when the capture APPENDED to an
    # existing canonical row (row count stays the same, only updated_at
    # moves — a created_at-ordered query MISSES the add). False when a
    # brand-new row was created. Surfaced so agents stop misreading a
    # silent consolidation as a failed write.
