...
async function loadDashboardFromGate(projectId?: string): Promise<DashboardSnapshot> {
  ...
  // VIEW, DON'T BIND (2026-09-17). This used to call project_select before
  // every load, because dashboard_snapshot could only report the BOUND
  // project. Selecting a project is an account-wide binding change: it demanded
  // a confirmation naming the same project on both ends, and committing it
  // cleared the selected session on EVERY surface of the account — the web
  // agent and a separate desktop session both went no_session_selected the
  // moment the operator opened this page. Looking at a project must never
  // change what is bound, so the id is passed to the read instead.
  const res = await tauriFetch(WEBMCP_GATE_MCP, {
    ...
    body: JSON.stringify({
      ...
      id: 1,
      method: "tools/call",
      params: { name: "dashboard_snapshot", arguments: projectId ? { project_id: projectId } : {} },
    }),
  });
  if (!res.ok) {
...
export async function msgSend(
  ...
  // #1061: operator messages enter XAACP through the gate's governed
  // dashboard_operator_message op. Principal, project and session are derived
  // server-side from the authenticated request + bound selection; the payload
  // carries only the target roles, the body and an immutable message id.
  void projectRoot;
  void sessionId;
  void inReplyTo;
  ...
  return routeInvoke("tauri_msg_send", {
    to_roles: toRoles,
    body,
