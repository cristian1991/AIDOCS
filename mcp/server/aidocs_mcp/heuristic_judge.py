...
def _inline_protected_path_rules(inline_code: str, project_root: Path | None) -> list[RuleVerdict]:
    ...
    # INSPECTION THAT DID NOT FINISH IS UNKNOWN, NOT CLEAN (r0b b4bde671-155).
    # This family used to end in `except Exception: pass`, so ANY exception —
    # from the fold, extraction, candidate iteration or classify_path — became
    # "no protected-path verdicts". That seam turned a single parser exception
    # into a working bypass: an oversized numeric literal in a discarded earlier
    # expression made the fold throw, and a later encoded read of a secrets path
    # was never classified. Making that one producer total fixed the instance,
    # not the class. Now a failure yields an explicit fail-closed verdict, and a
    # failure on one candidate never erases verdicts already found for others.
    # Evidence carries only the phase and the exception TYPE — never the
    # candidate text, which may itself be the sensitive path.
    phase = "import"

    def _inspection_failed(stage: str, exc: BaseException) -> RuleVerdict:
        return RuleVerdict(
            "INLINE_PROTECTED_PATH_INSPECTION_FAILED",
            "critical",
            f"Protected-path inspection could not complete ({stage}: "
            f"{type(exc).__name__})",
            evidence=f"phase={stage} error={type(exc).__name__}",
            recommendation=(
                "The judge could not finish checking this inline code for "
                "protected paths, so it is treated as unverified rather than "
                "clean. Rewrite the inline body into a form the protected-path "
                "inspector can analyze, then retry."
            ),
        )
    ...
    try:
        ...
        if project_root is not None:
            try:
                ...
                allow_secrets = False
        phase = "extract"
        candidates = list(extract_paths_from_inline_code(inline_code))
        phase = "classify"
        _classify_failed = False
        for candidate in candidates:
            # PER-CANDIDATE: one candidate the classifier cannot handle must not
            # erase what earlier candidates already proved. Record the failure
            # once, keep going, and let the fail-closed verdict ride alongside.
            try:
                pc = classify_path(
                    candidate,
                    project_root=project_root,
                ...
                if not _classify_failed:
                    verdicts.append(_inspection_failed("classify", exc))
        ...
        # FAIL-CLOSED, and it KEEPS what was already found. The old comment
        # here said "classifier failure must not block legit work", and that
        # trade was the bypass: availability was bought by treating "could not
        # inspect" as "nothing to report". The verdict is its own taxonomy
        # class: an invocation-only security hard block with NO confirm path
        # and NO strike — the defect is the judge's, so it is neither liftable
        # by intent nor charged to the operator (see judge_taxonomy).
        verdicts.append(_inspection_failed(phase, exc))
    return verdicts




# Global options git accepts BEFORE the subcommand. The two-token forms must
# be skipped with their value so `git -C /x reset --hard` still resolves
# `reset` as the subcommand.
_GIT_GLOBAL_OPT_WITH_VALUE: frozenset[str] = frozenset(
...
# command positions, never hide one. `su` is deliberately absent: it takes its
# command as a STRING argument to -c, which is a different shape from "the
# rest of argv is the command" and would need its own quoting analysis.
_WRAPPER_OPTIONS: dict[str, dict[str, object]] = {
    "sudo": {
        "value": frozenset(
            {
...
# (the round-2 exclusion was precisely "its command is a string argument").
...
# resolved to no git at all — a MISS). Every case-collapsed spelling is
# therefore AMBIGUOUS and is followed BOTH ways.
_WRAPPER_TABLES: dict[str, dict[str, object]] = {}
for _name, _spec in _WRAPPER_OPTIONS.items():
    ...
    _cmd_l = frozenset(o.lower() for o in _COMMAND_VALUE_OPTIONS.get(_name, frozenset()))
    _WRAPPER_TABLES[_name] = {
        "value": _value_l,
        "bool": _bool_l,
...
def _strip_one_quote_layer(value: str) -> tuple[str, bool]:
...
def _env_split_string_tokens(value: str) -> list[str] | None:
    ...
    """
    content, expansion = _strip_one_quote_layer(value)
    if expansion:
...
def _command_value_name(token: str, command_value: frozenset[str]) -> str | None:
...
def _opaque_command_value(cmd_lower: str) -> str | None:
    """Evidence for a command-bearing option value that CANNOT be normalised.

    `env FOO=bar -S "$CMD"`, `sh -c "$@"`, `eval "$(…)"` — the value IS a
    command line that the shell/env resolves at run time. No static parse is
    faithful, so the judge must not treat it as inert data.

    #893 round 5: this used to be a SECOND, backward scan with its own
    grammar, and that grammar did not know env's leading `NAME=VALUE`
    assignments — the walk stopped at `FOO=bar` and never reached `env`, so
    `env FOO=bar -S "$CMD"` was graded neither destructive nor opaque. The
    decision is now read off the SAME forward walk that
    :func:`_peel_wrapper_candidates` performs (it already honours
    assignments, option arity and wrapper nesting), so the two parses cannot
    disagree again: one grammar, one traversal, two outputs.
    """
    for segment in _DANGEROUS_SEGMENT_SPLIT_RE.split(cmd_lower or ""):
        toks = segment.split()
        while toks and (_ENV_ASSIGN_RE.match(toks[0]) or toks[0] in _DANGEROUS_LEAD_KEYWORDS):
            toks = toks[1:]
        notes: list[str] = []
        _peel_wrapper_candidates(toks, notes=notes)
        if notes:
            return notes[0]
    return None
...
def _peel_wrapper_candidates(
    ...
    if not tokens or depth >= 4:
        return [tokens]
    base_name = _basename_token(tokens[0])
    spec = _WRAPPER_TABLES.get(base_name)
    if spec is None:
        return [tokens]
    if base_name == "eval" and notes is not None and len(tokens) > 1:
        ...
        if _strip_one_quote_layer(operand)[1]:
    ...
    while rest:
        ...
        cmd_opt = _command_value_name(tok, command_value)
        ...
        if tok.startswith("-") and len(tok) > 1:
            ...
            rest = rest[1:]
            continue
        if assignments and _ENV_ASSIGN_RE.match(tok):
            rest = rest[1:]
            continue
    ...
    for branch in [rest, *branches]:
        if not branch:
            continue
        for peeled in _peel_wrapper_candidates(branch, depth + 1, notes):
            if peeled and peeled not in out:
                out.append(peeled)
            if len(out) >= _PEEL_MAX_CANDIDATES:
                return out
    return out


def _git_subcommand_invocations(cmd_lower: str) -> list[tuple[str, list[str]]]:
    """Every ``(subcommand, argument tokens)`` a ``git`` in COMMAND POSITION
    runs in ``cmd_lower``.

    Segment splitting and command position come from the same primitives as
    :func:`_dangerous_rule_hit` (``_DANGEROUS_SEGMENT_SPLIT_RE`` +
    ``_dangerous_command_positions``); wrapper peeling is done by
    :func:`_peel_wrapper_candidates`, which — unlike the shared helper —
    keeps flag tokens (so `sudo git reset --hard` keeps its `--hard`) and
    understands wrapper options that take VALUES. A `git` that is not in
    command position — e.g. the word `git` inside a grep PATTERN — yields
    nothing.
    """
    ...
    for segment in _DANGEROUS_SEGMENT_SPLIT_RE.split(cmd_lower or ""):
        ...
        while toks and (_ENV_ASSIGN_RE.match(toks[0]) or toks[0] in _DANGEROUS_LEAD_KEYWORDS):
            toks = toks[1:]
        for peeled in _peel_wrapper_candidates(toks):
            if peeled and _basename_token(peeled[0]) == "git":
                candidates.append(("git", peeled[1:]))

        for _base, args in candidates:
            i = 0
            while i < len(args) and args[i].startswith("-"):
                if args[i] in _GIT_GLOBAL_OPT_WITH_VALUE:
                    i += 2
                else:
                    i += 1
            if i >= len(args):
                continue
            invocation = (args[i].strip("\"'"), [t.strip("\"'") for t in args[i + 1 :]])
...
def _git_opt(
    ...
    """True when ``option`` is one of ``subcommand``'s OWN option tokens.

    A whole token, not a substring: `--hard` inside a quoted operand
    (`git grep "reset --hard"`) is an operand, not an option. Tokens after a
    bare `--` are pathspec operands and never count as options.
    """
...
def _short_flag(token: str, letters: str) -> bool:
    """True when ``token`` is a short-flag bundle carrying one of ``letters``."""
...
def _check_git_rules(
    ...
    *,
    ...
    command = mask_data_windows(command)
    lower = command.lower()
    # #893 (2026-09-17): every rule below parses the actual git ARGV —
    # subcommand in command position, flags among THAT subcommand's own
    # option tokens. The previous shape was raw substring containment
    # (`"git" in lower and "reset" in lower and "--hard" in lower`), which
    # graded a READ-ONLY `git grep 'reset --hard'` as a hard reset, asked
    # for confirmation and froze the session. Same parser as
    # `_dangerous_rule_hit` (`_dangerous_command_positions`), so command
    # position, wrapper peeling (`sudo`/`xargs`/…) and segment splitting
    # behave identically here and there.
    invocations = _git_subcommand_invocations(lower)

    # HIGH: an option value that IS a command line but cannot be normalised
    # (`env -S "$CMD"`, `eval "$(…)"`). It may be anything, including a hard
    # reset, so it is never inert. An unclassified rule_id maps to
    # confirmable_destructive by judge_taxonomy's documented default — the
    # fail-closed grade this needs.
    opaque = _opaque_command_value(lower)
    if opaque:
        verdicts.append(
            RuleVerdict(
                "EXEC_OPAQUE_COMMAND_VALUE",
                "high",
                "An option value that is executed as a command cannot be parsed.",
                evidence=opaque,
                recommendation=(
                    "`env -S`/`sh -c`/`eval` values that expand a variable or "
                    "use syntax the gate cannot normalise are opaque: the gate "
                    "cannot tell a hard reset from a read. Inline the literal "
                    "command instead of expanding it."
                ),
            ),
        )

    # HIGH: force push
    if _git_opt(invocations, "push", "--force"):
        verdicts.append(
            RuleVerdict(
                "GIT_FORCE_PUSH",
                "high",
    ...
    # HIGH: hard reset
    if _git_opt(invocations, "reset", "--hard"):
        verdicts.append(
            RuleVerdict(
                "GIT_RESET_HARD",
                "high",
    ...
    for sub, args in invocations:
        ...
        if head == "--":
            ...
            break
        if head == "." or head.startswith("head") or head.startswith("origin/"):
            verdicts.append(
                RuleVerdict(
                    "GIT_CHECKOUT_OVERWRITE",
                    "high",
    ...
    # MEDIUM: git clean
    if any(
        sub == "clean" and any(_short_flag(tok, "fdx") for tok in args)
        for sub, args in invocations
    ):
        verdicts.append(
            RuleVerdict(
    ...
    if any(
        sub == "branch" and any(_short_flag(tok, "d") or tok == "--delete" for tok in args)
        for sub, args in invocations
    ):
        verdicts.append(
            RuleVerdict(
    ...
    if any(
        sub == "stash" and args and args[0] in ("drop", "clear") for sub, args in invocations
    ):
        verdicts.append(
            RuleVerdict(
                "GIT_STASH_DROP",
    ...
    if _git_opt(invocations, "push", "--force-with-lease"):
        verdicts.append(
            RuleVerdict(
                "GIT_FORCE_LEASE",
                "medium",
    ...
    for sub, args in invocations:
        if sub != "restore" or not args:
            continue
        # A restore that touches the WORKING TREE discards uncommitted work.
        # `--worktree` / `-W` says so in as many words; otherwise fall back to
        # the operand check (`.`, or the `--` separator before a pathspec),
        # which is what a bare `git restore .` looks like.
        #
        # WHY NOT `args[1:] if args[0] == "--staged"`: that stripped exactly ONE
        # known flag and then demanded the operand sit at position 0, so
        # `git restore --staged --worktree src/` — a declared
        # DECLARED_COMMAND_SHAPES entry, and the shape that wipes BOTH index and
        # worktree — saw `--worktree` in the operand slot and fired nothing.
        # The old regex `restore\s+(--staged\s+)?(\.|--)` caught it only by
        # accident, because `--worktree` happens to start with `--`.
        worktree = any(tok == "--worktree" or _short_flag(tok, "W") for tok in args)
        operands = [tok for tok in args if not tok.startswith("-")]
        if worktree or (operands and operands[0] == ".") or "--" in args:
            verdicts.append(
                RuleVerdict(
                    "GIT_RESTORE_DISCARD",
                    "high",
...
def evaluate_tool_call(
    ...
    *,
    ...
    # never runs on this path (§XXVII — no verdict may DEPEND on the guest).
    try:
    ...
    locked.update(
        {
            ...
            "BASH_EVAL_SUBSHELL",
            "FILE_PEM_PRIVATE_KEY",
            # An unfinished inspection is not a rule anyone may opt out of:
            # disabling it restores the fail-open it exists to close.
            "INLINE_PROTECTED_PATH_INSPECTION_FAILED",
        },
    )
    try:
