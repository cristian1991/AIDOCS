...
_DEFAULT_CONFIG: dict[str, dict[str, object]] = {
    ...
    "security": {
        ...
        # 2026-04-25 — superseded by [bash] / [raw_bash] tables.
    ...
    "edit": {
        # King doctrine 2026-05-01: cap tightened from 2000 → 200 to
    ...
    # Raw Bash (host-native tool) policy — separate tier from ai_run.
    # Empty by default so raw Bash requires explicit operator opt-in;
    # ai_run remains the wide audited surface. Backlog #7 fix:
    # granting `bash.allow.git = true` for ai_run no longer implicitly
    # lets raw Bash run git without audit.
    #
    # Operators who want raw Bash for specific commands add entries
    # here explicitly (dashboard or TOML). Same grammar as [bash].
    # The _JUDGE_DENYLIST (rm, sudo, dd, mkfs, etc.) still trumps any
    # allow entry at this layer too — adding raw rm to raw_bash.allow
    # has no effect.
    "raw_bash": {
        "default": "block",
        "allow": {},
...
def _resolve_gate_messages_dir() -> Path | None:
    """Legacy TOML directory resolver — retained for callers that still
    want to inspect the on-disk seed files. The runtime loader no
    longer reads these; gate-message strings are sourced from the
    empire SQLite store (`gate_message_strings`) seeded via
    `intent_tokens_store`.

    Post-Phase 5b (2026-05-14): TOMLs live inside the python package
    at mcp/server/aidocs_mcp/seed/gate_messages/. Repo-root
    gate_messages/ no longer exists.
    """
    pkg_seed = Path(__file__).resolve().parent / "seed" / "gate_messages"
    if pkg_seed.is_dir():
        return pkg_seed
    return None
...
# (see enforcement.is_dev_flavor / dev_mode_authorized), not a config flag.
ALLOW_CONFIG_EDIT: bool = _parse_bool(
    _get_dotted(_DEFAULT_EFFECTIVE_CONFIG, "security.allow_config_edit"),
    default=False,
)
GATE_ENFORCE: bool = _parse_bool(
    _get_dotted(_DEFAULT_EFFECTIVE_CONFIG, "security.enforce"),
    default=True,
)
ALLOW_SUBAGENTS: bool = _parse_bool(
    _get_dotted(_DEFAULT_EFFECTIVE_CONFIG, "agents.allow_subagents"),
    default=False,
)
...
# Read deny-list + per-role/user read ACL (king 2026-06-13). Reads are
...
# legacy substring allowlist superseded by [bash] / [raw_bash] tables.
