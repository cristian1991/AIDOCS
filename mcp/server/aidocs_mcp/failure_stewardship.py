...
def _now_stamp() -> str:
    """Wall-clock epoch seconds as a STRING.

    History entries are ``dict[str, str]`` and are persisted as JSON by
    ``save_ledger``; a float here would make one key's type differ from
    every other in the same dict for no gain. Readers parse it back (see
    ``_history_window``) and treat an unparseable or absent value as
            ...
            row.history.append(
                {
                    ...
                    # WHEN. The diagnosis door has to answer "first seen / last
                    # seen" and the history carried no clock at all, so the
                    # only honest answer for an existing row was "unknown".
                    # Stamped from here forward; rows already in a live ledger
                    # stay honestly unknown rather than being backfilled with
                    # the time somebody happened to read them.
                    "at": _now_stamp(),
                },
            )
            return row
...
def _connect(db_path: Path) -> sqlite3.Connection:
    ...
    # through the ONE canonical connect (#755, empire-doctrine VI) rather than
...
#: How many pre-filled diagnosis calls the block offers. A Stop-block that
#: pastes 133 invocations is the same unreadable wall as the 116KB listing,
#: one layer up -- so the block names a few and then names the LISTING,
#: which is now bounded and pageable.
MAX_ADVERTISED_DOORS = 3


def _diagnosis_door(rows: list[FailureRow]) -> str:
    """The investigation instrument, with the signature ALREADY SUBSTITUTED.

    Named FIRST in the block because it is the first step: the old block's
    ordering implied the agent already knew what the failure was and merely
    had to file paperwork about it.
    """
    shown = rows[:MAX_ADVERTISED_DOORS]
    calls = " ".join(
        f"ai_failures(mode='diagnose', signature={r.failure_signature[:12]!r})" for r in shown
    )
    more = ""
    if len(rows) > len(shown):
        more = (
            f" ({len(rows) - len(shown)} more — enumerate them with "
            "ai_failures(mode='list'), which is bounded and pages with "
            "limit/offset.)"
        )
    return (
        "FIRST, FIND OUT WHAT IT IS — do not triage a signature you have not read. "
        f"{calls}{more} That returns the nodeid, the full error type AND message, "
        "the assertion line, the duty and causal origin, the row's history, and a "
...
#: Default page size for `list_session_failures`. Measured 2026-09-10: the
#: unbounded listing returned 116,665 characters on ONE SINGLE LINE -- over
#: the tool-result token cap, so it spilled to a file; 1 real blocker buried
#: under 133 `blocked_elsewhere` rows; no limit, no offset, no filter and no
#: terse mode. `jq` is not installed on this box, Grep answered
#: "[Omitted long matching line]", and the gate refused a Bash call because
#: it parsed the quoted spill filename as a command name. The answer came
#: from a hand-written python one-liner. An enforcement surface that cannot
#: be read by any tool in the estate -- INCLUDING ITS OWN -- is not a
#: surface, so the listing is bounded by default and the caller pages.
DEFAULT_LIST_LIMIT = 20

#: Hard ceiling on one page regardless of what the caller asks for. The cap
#: that was missing is the whole defect; an opt-out would restore it.
MAX_LIST_LIMIT = 200

#: How many history entries one diagnosis renders. Newest last (the list is
#: append-ordered), so the trim drops the OLDEST -- a row re-observed 400
#: times must still show the disposition attempts at its end.
MAX_DIAGNOSIS_HISTORY = 20


def _terse_summary(row: "FailureRow") -> dict[str, object]:
    """The narrowest honest row: enough to ADDRESS it and to ask about it.

    Deliberately carries `short` (what every disposition and diagnosis call
    accepts) and the nodeid, and deliberately omits the error text -- that
    is what `mode='diagnose'` is for. A listing's job is to enumerate; a
    diagnosis's job is to explain. Conflating them is how the listing grew
    to 116KB.
    """
    return {
        # THE FULL SIGNATURE STAYS. It is 64 hex characters, and three
        # existing consumers key on `signature` (test_failure_stewardship_673,
        # test_failure_stewardship_disposition_surface) -- dropping the
        # canonical id to save bytes would break every reader to fix a
        # payload problem that lives in the PROSE, not in the ids.
        "signature": row.failure_signature,
        "short": row.failure_signature[:12],
        "nodeid": row.nodeid,
        "error_type": row.error_type,
...
def _repro_command(row: "FailureRow") -> dict[str, object]:
    """How to REPRODUCE this failure, as a runnable thing.

    The hook's block text has always named the DISCHARGE surface and never
    the investigation. "Find out why it fails" with no instrument is not an
    instruction, it is a demand.

    Mirrors `_default_reverify_runner`'s own argv (`-v`, no cache provider)
    so the agent reproduces what the ledger re-verifies rather than a
    neighbouring run -- but `--tb=short` instead of `--tb=no`, because a
    human-or-agent investigation needs the traceback the automated
    re-verify deliberately suppresses.

    A FILE-LEVEL signature (#743: a pure collection error, no `::`) is NOT
    a collectable nodeid and must not be handed to anything expecting one.
    It gets its own command and says why.
    """
    nodeid = row.nodeid
    if "::" not in nodeid:
        return {
            "kind": "collection_error",
            "pytest": f"python -m pytest {nodeid} --collect-only -q",
            "ai_test": f"ai_test(paths={nodeid!r})",
            "note": (
                "This signature names a FILE, not a test: the whole module failed "
                "to import, so no test was ever collected (#743). Every test in "
                "that file is missing from the run -- reproduce the IMPORT, not a "
                "single test."
            ),
        }
    safe = _is_safe_pytest_nodeid(nodeid)
    out: dict[str, object] = {
        "kind": "nodeid",
        "pytest": (
            f"python -m pytest {nodeid} -v --tb=short --no-header -p no:cacheprovider"
        ),
        "ai_test": f"ai_test(paths={nodeid.split('::', 1)[0]!r}, name_filter={nodeid.rsplit('::', 1)[-1]!r})",
        "runs_from": "the directory holding the project's pyproject.toml (mcp/ in this repo)",
    }
    if not safe:
        # Preserve the fact rather than silently offering a command the
        # ledger itself refuses to run.
        out["warning"] = (
            "this nodeid is NOT re-verifiable by the ledger (it fails "
            "_is_safe_pytest_nodeid -- flag-shaped or malformed, probably scraped "
            "from truncated console text). It will never auto-clear on a green "
            "run, so it must be disposed explicitly."
        )
    return out
...
def diagnose_failure(
    ...
    """WHAT IS THIS FAILURE? -- addressable by the same short prefix the
    disposition modes accept.

    OPERATOR DIRECTIVE (backlog #1056, verbatim): "you should have a tool to
    find out what that fail was. that tool should be advertised by the
    failure steward hook itself."

    MEASURED the same day: the Stop hook blocked a turn naming
    `f4d3d97b871a` and one surface -- the DISCHARGE surface -- with no way
    to discover what `f4d3d97b871a` WAS. Diagnosing it took five calls and
    three gate refusals. A gate that demands understanding must hand over
    the instrument; this is the instrument.

    BOUNDED BY CONSTRUCTION: one row, one error message, one assertion
    line, at most `MAX_DIAGNOSIS_HISTORY` history entries. It can never
    become the 116KB listing it exists to replace.

    A signature that matches NOTHING, or matches AMBIGUOUSLY, returns the
    refusal `_resolve_signature` raises -- never a best guess. Diagnosing
    the wrong failure is worse than diagnosing none, because the agent then
    disposes of a row it never looked at.
    """
    ledger = load_ledger(project_root)
    try:
        sig = _resolve_signature(ledger, signature)
    except StewardshipError as exc:
        return {
            "ok": False,
            "error": str(exc),
            "mode": "diagnose",
            "signature_asked": (signature or "").strip(),
            "how_to_find_one": (
                "ai_failures(mode='list') lists the signatures under your duty "
                "(bounded; page with limit/offset). Every `short` it prints is "
                "accepted here."
            ),
        }
    row = ledger.get(sig)
    if row is None:  # pragma: no cover - _resolve_signature already guarantees it
        return {"ok": False, "error": f"no ledger row for {sig!r}", "mode": "diagnose"}

    first_at, last_at = _history_window(row)
    return {
        "ok": True,
        "mode": "diagnose",
        "signature": row.failure_signature,
        "short": row.failure_signature[:12],
        "nodeid": row.nodeid,
        # THE ERROR, BOTH HALVES. `error_type` is the exception CLASS and
        # `top_assertion_line` is its MESSAGE -- parse_pytest_failures splits
        # pytest's `FAILED <nodeid> - <Class>: <message>` on the first colon.
        # The listing showed the class and called it the error, which is how
        # an agent ends up triaging "AssertionError" with no idea what was
        # asserted.
        "error_type": row.error_type,
        "error_message": row.top_assertion_line,
        "assertion_line": row.top_assertion_line,
        "disposition": row.disposition,
        "duty": {
            "current_duty": row.current_duty,
            "owner_key": row.owner_key,
            "asking_session": session_id,
            "disposable_by_you": bool(
                row.current_duty and session_id and row.current_duty == session_id
            ),
            "next_step": (
                _foreign_duty_next_step(row, session_id)
                if row.current_duty and row.current_duty != session_id
                else (
                    "claim a disposition with proof: ai_failures(mode='fixed'|"
                    "'preserve_baseline'|'quarantine'|'escalate', "
                    f"signature={row.failure_signature[:12]!r}, ...)"
                )
            ),
        },
        "causal_origin": row.causal_origin,
        "first_seen": {
            "sha": row.first_seen_sha,
            "tree_hash": row.first_seen_tree_hash,
            # UNKNOWN STAYS UNKNOWN. Rows registered before history entries
            # carried an `at` stamp have no timestamp to report, and a
            # fabricated one would be indistinguishable from an observed one.
            "at": first_at,
            "at_unknown_because": (
                None
                if first_at is not None
                else "this row was registered before history entries carried an "
                "`at` stamp; its registration time was never recorded and "
                "cannot be recovered. Rows registered from now on carry one."
            ),
        },
        "last_seen_at": last_at,
        "reverify_note": row.reverify_note,
        "cleared_by": row.cleared_by,
        "followup_ref": row.followup_ref,
        "proof": {"command": row.proof_command, "log_hash": row.proof_log_hash},
        "reproduce": _repro_command(row),
        "history": list(row.history)[-MAX_DIAGNOSIS_HISTORY:],
        "history_total": len(row.history),
    }


def _history_window(row: "FailureRow") -> tuple[float | None, float | None]:
    """(first_at, last_at) from history stamps, or Nones.

    Two Nones mean the stamps are absent, NOT that the row is new. The
    caller says which it is; this helper never guesses.
    """
    stamps: list[float] = []
...
def list_session_failures(
    ...
    *,
    ...
    """
    ledger = load_ledger(project_root)
    # `limit=0` means THE CALLER DID NOT ASK. It is not "no limit": the two
    # are different requests and collapsing them is how the 116KB page got
    # shipped to somebody who never chose it.
    asked_for_a_limit = bool(limit)
    lim = max(1, min(int(limit or DEFAULT_LIST_LIMIT), MAX_LIST_LIMIT))
    off = max(0, int(offset or 0))
    needle = (signature or "").strip().lower()

    def _match(r: "FailureRow") -> bool:
        if not needle:
            return True
        return r.failure_signature.lower().startswith(needle) or needle in r.nodeid.lower()

    def _page(
        rows: list["FailureRow"], *, foreign: bool, bounded: bool = True
    ) -> tuple[list[dict], dict]:
        kept = [r for r in rows if _match(r)]
        window = kept[off : off + lim] if bounded else kept[off:]
        render = _terse_summary if terse else _row_summary
        out = [dict(render(r)) for r in window]
        if foreign:
            # THE REACHABLE NEXT STEP SURVIVES TERSE MODE. It is ~450
            # characters of near-identical prose per row and it was most of
            # the measured 116KB -- but law 311bf3e6 (#673) says a named
            # remedy must be REACHABLE, per row, and a foreign-duty row whose
            # only instruction is "claim a disposition" is the dead end that
            # law exists to forbid. The fix for the payload is the PAGE BOUND
            # above, not deleting the remedy: 20 rows of it is 9KB, 133 was
            # 60KB. Trimming honesty to save bytes would be fixing the wrong
            # thing.
            for rendered, r in zip(out, window, strict=False):
                rendered["next_step"] = _foreign_duty_next_step(r, session_id)
        meta = {
            "total": len(kept),
            "returned": len(out),
            "offset": off,
            "limit": lim if bounded else None,
            "has_more": (off + len(out)) < len(kept),
        }
        return out, meta

    blockers, blockers_page = _page(ledger.seal_blockers(session_id), foreign=False)
    elsewhere, elsewhere_page = _page(
        ledger.blockers_under_other_duty(session_id), foreign=True
    )
    payload: dict[str, object] = {
        "ok": True,
        "session_id": session_id,
        "blockers": blockers,
        "blocked_elsewhere": elsewhere,
        "page": {
            "blockers": blockers_page,
            "blocked_elsewhere": elsewhere_page,
            "terse": bool(terse),
            "signature_filter": needle,
        },
        # THE INSTRUMENT, NAMED ON THE SURFACE THAT ENUMERATES. A terse row
        # says WHICH failure; it deliberately does not say why. This is where
        # the reader learns the second half exists.
        "diagnose": (
            "ai_failures(mode='diagnose', signature='<short>') - full error type AND "
            "message, assertion line, duty, causal origin, history and the "
            "reproducing command for ONE failure. Accepts any `short` above."
        ),
    }
    if blockers_page["has_more"] or elsewhere_page["has_more"]:
        payload["page_next"] = (
            f"ai_failures(mode='list', offset={off + lim}, limit={lim}"
            + (f", signature={needle!r}" if needle else "")
            + ")"
        )
    if include_all:
        # #852's AUDIT VIEW STAYS WHOLE. `include_all=True` is an explicit
        # opt-in to the dump — the caller asked for every row and is entitled
        # to every row, so it is paged only when a `limit` was actually
        # passed. The thing that MUST stay bounded is the critical path:
        # `blockers` / `blocked_elsewhere` ship on every turn that touches a
        # test, nobody opts into them, and that is where the 116KB landed.
        all_rows, all_page = _page(
            ledger.all(), foreign=False, bounded=asked_for_a_limit
        )
        payload["all"] = all_rows
        payload["page"]["all"] = all_page  # type: ignore[index]
    return payload
...
def _evaluate_turn_locked(
    ...
    *,
    ...
    if blockers:
        ...
        # ── THE DIAGNOSIS DOOR, HANDED OVER (operator directive #1056) ────
        # "you should have a tool to find out what that fail was. that tool
        # should be advertised by the failure steward hook itself."
        #
        # MEASURED 2026-09-10: this block named `f4d3d97b871a`, demanded
        # "find out why it fails", and pointed at exactly ONE surface -- the
        # DISCHARGE surface. Discovering what `f4d3d97b871a` actually WAS
        # took five calls and three gate refusals, because the only listing
        # available returned 116,665 characters on a single line. A gate that
        # demands understanding must hand over the instrument, and it must
        # hand it over ALREADY LOADED: the signature is substituted into the
        # call, so the agent copies a working invocation instead of
        # assembling one.
        if mine:
            parts.append(_diagnosis_door(mine))
            parts.append(
        ...
        if unowned:
            if not mine:
                # Only once per block: it is the same door. Repeating the
                # instrument three times would push the block past the point
                # an agent reads it.
                parts.append(_diagnosis_door(unowned))
            parts.append(
