...
class ProtectedFileRegistryStore:
    ...
    def init_db(self, project_root: Path) -> None:
        ...
        with sqlite3.connect(str(path)) as conn:
            ...
            # for.
            #
            # Historical column name `cli_session_id` stores the
            # derived agent_memory_epoch (sha256 over host_kind +
            # project_root + aidocs_work_session_id + host_session_id +
            # compaction_count). Column kept under the legacy name to
            # avoid a broad migration; see dnt_banner_injector.py and
            ...
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS dnt_banners_shown (
                    ...
                    PRIMARY KEY (cli_session_id, dnt_id)
                );
            """)
            conn.commit()
        ...
        *,
        ...
        with sqlite3.connect(str(self.db_path(project_root))) as conn:
            conn.execute(
                ...
                " pair_files, machine_id) VALUES (?, ?, ?, ?, ?, ?)",
                (rec_path, protected_by_user_id, ts, why, pair_json, machine_id),
            )
            conn.commit()
    ...
    def mark_banner_shown(
        ...
        *,
        ...
        marks are a no-op.
        """
        if not cli_session_id or not dnt_id:
            return
        self.init_db(project_root)
        ts = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        with sqlite3.connect(str(self.db_path(project_root))) as conn:
            conn.execute(
                ...
                "(cli_session_id, dnt_id, first_shown_at) "
                "VALUES (?, ?, ?)",
                (cli_session_id, dnt_id, ts),
            )
            conn.commit()
    ...
    def was_banner_shown(
        ...
        *,
        ...
        with sqlite3.connect(str(self.db_path(project_root))) as conn:
            row = conn.execute(
                "SELECT 1 FROM dnt_banners_shown WHERE cli_session_id = ? AND dnt_id = ? LIMIT 1",
                (cli_session_id, dnt_id),
            ).fetchone()
    ...
    def clear_banners_shown_for_conductor(
        ...
        """Clear all banner-shown rows for one cli_session_id. Used
        ...
        with sqlite3.connect(str(self.db_path(project_root))) as conn:
            cur = conn.execute(
                "DELETE FROM dnt_banners_shown WHERE cli_session_id = ?",
                (cli_session_id,),
            )
            conn.commit()
