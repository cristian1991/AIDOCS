...
def category_of(lemma: str) -> set[str]:
    """Return all categories a lemma belongs to. Lowercased input
    expected (caller normalizes).
    """
    out: set[str] = set()
...
def in_category(lemma: str, category: str) -> bool:
    members = SEMANTIC_CATEGORIES.get(category, frozenset())
    return lemma in members
