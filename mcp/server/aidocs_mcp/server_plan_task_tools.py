    ...
    *,
    ...
    def ai_models(backend: str) -> dict[str, Any]:
        ...
        if backend == "opencode":
            ...
            if not oc:
                return {"backend": backend, "models": [], "error": "opencode CLI not found"}
            try:
                r = subprocess.run(
                    [oc, "models"],
                    capture_output=True,
                ...
                if r.returncode == 0:
                    ...
                    return {"backend": backend, "models": models, "source": "opencode models"}
                return {"backend": backend, "models": [], "error": (r.stderr or "")[-500:]}
            except Exception as exc:
                return {"backend": backend, "models": [], "error": str(exc)}
        if backend == "claude":
            return {
                "backend": backend,
                "models": [
                ...
                "source": "hardcoded (claude CLI has no --models flag)",
            }
        if backend == "codex":
            return {
                "backend": backend,
                "models": ["gpt-5", "gpt-4o", "o3", "o3-mini"],
                "source": "hardcoded (codex CLI has no --models flag)",
            }
