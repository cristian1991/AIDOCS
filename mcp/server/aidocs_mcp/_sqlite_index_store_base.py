...
class SQLiteIndexStoreBase:
    ...
    def connect(self, project_root: Path) -> sqlite3.Connection:
        ...
        try:
            # journal_mode=WAL is persisted on the DB file (cheap no-op after the
            # first); synchronous=NORMAL is the safe WAL companion; busy_timeout
            # is belt-and-suspenders with the connect timeout above.
