...
#
# MOVED 2026-08-20 to output_guard.scrub_persisted_text. The floor was always
# about the artifact being COMMITTED, not about it being a session file, but
# living privately here meant .MEMORY/sync/events/ — equally committed — never
# got it, and a daemon boot token reached git history through that gap. Both
# names stay re-exported: they are this module's existing surface and #363's
# tests import them from here.
from .output_guard import (  # noqa: E402,F401 - deliberate re-export, see above
    GUARD_UNAVAILABLE_MASK,
)
from .output_guard import scrub_persisted_text as _scrub_credential_text
...
class SessionStore:
    ...
    def __init__(self, templates_root: Path | None) -> None:
        # Optional since #628: the canonical resolver returns None rather than
        # fabricating a nonexistent templates dir. create_session re-resolves.
    ...
    def create_session(
        ...
        # #628: the ONE template read in the codebase. Resolve through the
        # canonical resolver WITH the project root in hand — templates_root was
        # fixed at hub-construction time, before any project was known, and on
        # an installed runtime it can be a stale/absent guess. The resolver
        # raises TemplatesRootUnresolved (naming what it probed and the
        # remedies) instead of letting a fabricated path ENOENT here.
        from .host_services.path_resolver_service import require_context_template

        context_template = require_context_template(
            self.templates_root,
