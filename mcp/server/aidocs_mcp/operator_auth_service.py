...
CLI_COMMAND_CLASS: dict[str, str] = {
    ...
    # control-plane mutation (adds agent tools), so admin-gated. Solo/dev
    # are local-mintable (auto-token); corpo requires login.
...
class OperatorAuthService:
    ...
    def resolve_token_from_args(
        ...
        *,
        ...
        """Pull the operator token from CLI args or env. Returns
        empty string when neither carries a value. Never raises.
        """
        if flag in args:
            try:
                idx = args.index(flag)
    ...
    def mint_local_operator_token(
        ...
        *,
        ...
        """Bootstrap + issue a local operator token for the desktop
        dashboard. Returns ``(token, reason)``.

        Authority model by flavor (plan §9):
          - solo / dev: the local operator IS super_admin (no login).
            The physically-present desktop user is trusted; we
            bootstrap the local admin user on first call and issue a
            token. The token is still the mechanism the auth wall
            checks, so every mutation remains audited + RBAC-gated.
          - corpo: login required. We DO NOT auto-mint; returns
            (None, 'login_required') so the caller surfaces the
            sign-in flow.

        This is the desktop-app equivalent of "first-run local
        admin/bootstrap" from the plan. It does NOT weaken the wall:
        a token still maps to a real user_id with the admin role, so
        audit attribution and permission checks are genuine. What it
        avoids is forcing a password UI on a single-operator solo box.
        """
        f = (flavor or "solo").strip().lower()
        # Universal login (security.require_login): when set, NO flavor auto-mints
        # a local operator token — the physically-present operator is no longer
        # auto-super_admin; the desktop must sign in (begin_desktop_login). This is
        # the desktop half of project_authority's passthrough suppression.
        try:
            from .config import get_setting

            if bool(
                get_setting("security.require_login", project_root=project_root, default=False)
            ):
        ...
        if f == "corpo":
            return None, "login_required"
        try:
            from .identity_store import ROLE_ADMIN, IdentityStore
            from .permission_catalog import seed_rbac
            from .rbac_store import RBACStore
        except Exception:
            return None, "identity_unavailable"
        ident = IdentityStore()
        local_email = "local-operator@aidocs.local"
        try:
            user = ident.get_user_by_email(project_root, local_email)
        except Exception:
            user = None
        if user is None:
            try:
                ...
                # Already exists (race) — re-resolve.
                try:
                    user = ident.get_user_by_email(project_root, local_email)
                except Exception:
                    user = None
        if user is None:
            return None, "bootstrap_failed"
        # Ensure the local operator holds the admin role.
        try:
            seed_rbac(project_root)
            rbac = RBACStore()
            admin_role = rbac.get_role_by_name(project_root, "admin")
            ...
            pass
        # GC expired tokens on every mint so the table stays bounded
        # even if a caller (or an old un-cached build) minted per-call.
        try:
            ident.purge_expired_tokens(project_root)
        except Exception:
            pass
        try:
            session = ident.issue_token(project_root, user.user_id)
            return session.token, "minted"
        except Exception:
            return None, "issue_failed"
    ...
    def auth_status(
        ...
        role, flavor}. ``auth_mode`` distinguishes the unauthenticated
        cases so the UI never shows a false error in solo/dev:

          * "authenticated"  — a valid token is present.
          * "local_mintable" — no cached token YET, but the install is
            solo/dev: a local operator token is minted on demand (no
            manual login). NOT an error state.
          * "login_required" — corpo: the operator must sign in first.
        """
        ...
        flavor = _auth_status_flavor()
        if flavor in ("dev", "solo"):
            return {
                "authenticated": False,
                "auth_mode": "local_mintable",
                "login_required": False,
                "user_id": "",
                "role": "",
                "flavor": flavor,
                "message": (
                    "No cached operator token yet — a local "
                    "operator token is minted on demand "
                    "(solo/dev). No manual login required."
                ),
            }
        return {
            "authenticated": False,
            ...
            "role": "",
            "flavor": flavor,
            "message": "Login required (corpo) — sign in to obtain an operator token.",
        }
