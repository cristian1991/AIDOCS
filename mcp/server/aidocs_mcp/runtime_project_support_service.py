...
class RuntimeProjectSupportService:
    ...
    def ensure_claude_mcp_config(self, project_root: Path) -> dict[str, object]:
        ...
        # If aidocs_mcp is pip-installed (importable without PYTHONPATH), don't set it
        # This makes .mcp.json portable and avoids hardcoded source checkout paths
        env: dict[str, str] = {}
        try:
            import importlib.util
            spec = importlib.util.find_spec("aidocs_mcp")
            if spec is None:
        ...
        new_entry = {
            ...
            "env": env if env else {"AIDOCS_PROJECT_ROOT": str(project_root)},
        }
