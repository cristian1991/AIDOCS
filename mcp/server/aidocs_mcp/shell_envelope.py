...
# Tool basename → DIALECT (#561 phase 2). The provider map above is
# deliberately BROAD so PreToolUse intercepts sh/zsh/wsl at all; this
# map is deliberately NARROW, because it answers a different question:
# which grammar will actually parse the command. Only `bash` is bash.
# `sh` is a POSIX sh. `zsh` and `wsl` are neither, and get
# DIALECT_UNKNOWN rather than a guess — AIDOCS has no law written for
# them, and recording that honestly is what keeps them off the
# native-execution path instead of quietly judging them as bash.
_TOOL_DIALECTS: dict[str, str] = {
    "bash": _dialect.DIALECT_BASH,
    "sh": _dialect.DIALECT_POSIX_SH,
    "powershell": _dialect.DIALECT_POWERSHELL,
    "pwsh": _dialect.DIALECT_POWERSHELL,
    "cmd": _dialect.DIALECT_CMD,
}
...
class ShellCommandEnvelope:
    ...
    # GRAMMAR the command will be parsed in (#561 phase 2). Distinct
    # from `provider`, which is the interception tag. Empty string means
    # "not set by this caller" — ShellPolicy then derives it from the
    # provider, so an envelope built by hand keeps its pre-phase-2
    # verdict exactly.
    dialect: str = ""
...
def detect_dialect(tool_name: str) -> str:
    """Map a tool name to the GRAMMAR its command will be parsed in.

    Deliberately NOT derived from ``detect_provider_and_transport`` —
    that map folds sh/zsh/wsl into provider=bash so they get intercepted,
    and folding them again here would reproduce the exact confusion #561
    phase 2 exists to end. ai_run is bash because phase 1 made a resolved
    bash the interpreter at the governed chokepoint; monitor rides the
    same runner.
    """
    base = _normalize_tool_basename(tool_name)
    if base in ("ai_run", "ai_run_output", "ai_run_status", "ai_run_kill", "monitor"):
        return _dialect.DIALECT_BASH
    return _TOOL_DIALECTS.get(base, _dialect.DIALECT_UNKNOWN)
...
def normalize(
    *,
    ...
    return ShellCommandEnvelope(
        tool_name=tool_name,
        provider=provider,
        dialect=detect_dialect(tool_name),
        transport=transport,
        command=command,
