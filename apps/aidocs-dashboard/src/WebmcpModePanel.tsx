...
import { setScope, saveGateConnection, getScope, onScopeChange, loadGateConnection, conductorLockReason } from "./webmcpScope";

// WebAgent mode + login (DoD #6). The LOCAL dashboard runs as a solo superadmin
// with NO login (dev/solo). WebAgent (cloud) mode authenticates against the
...
export function WebmcpModePanel() {
  // WEB build: render the browser PKCE mode/connect panel — the desktop Tauri login flow
  // below never runs in a browser. __AIDOCS_WEB__ is a compile-time constant, so hook order
  // stays consistent within each build.
  ...
  return (
    <>
      <button
        ...
        title={onWeb ? "WebAgent cloud scope — manage login / switch to Local" : "Local scope — log in to use WebAgent"}
        className={
      ...
      >
        ...
        />
        <span className="font-semibold">{onWeb ? "WebAgent" : "Local"}</span>
        {onWeb && session ? (
    ...
    >
      ...
      >
        <div className="mb-1 flex items-center justify-between">
          <h2 className="text-lg font-black tracking-tight text-white">WebAgent access</h2>
        ...
        <p className="mb-4 text-xs text-castle-mute">
          ...
          codenexus.cloud account to use WebAgent (cloud) mode.
        </p>
        ...
        {session ? (
          <div className="space-y-3">
            <div className="rounded-xl border border-castle-line bg-black/20 p-3 text-sm">
              ...
              {!session.webmcp.entitled ? (
                <p className="mt-2 text-[11px] text-castle-deny">
                  No active WebAgent seat in any of your orgs. Ask an org owner/admin to add you,
            ...
            {session.orgs.length ? (
              <div className="rounded-xl border border-castle-line bg-black/20 p-3 text-sm">
                ...
                <p className="mt-2 text-[10px] text-castle-mute">
                  Pick the active org in the WebAgent chat / dashboard via org_select. Org data via
            ...
            {/* Third mode (3-mode local dashboard): CloudAgent — a server-hosted agent +
                MCP. Locked until ADB AI-agent + dockerization lands. */}
            <button
              type="button"
              disabled
              title={conductorLockReason("cloudagent")}
              className="w-full cursor-not-allowed rounded-xl border border-castle-line bg-black/10 px-4 py-2 text-sm text-castle-mute opacity-60"
            >
              CloudAgent — coming soon (server-hosted agent; conductor locked until ADB)
            </button>
            <button
              type="button"
              onClick={() => {
          ...
          <>
            ...
            <form onSubmit={tab === "login" ? doLogin : doRegister} className="space-y-2">
              ...
              >
                {busy ? "…" : tab === "login" ? "Log in to WebAgent" : "Create account & sign in"}
              </button>
            </form>
...
// Switching the dashboard to WebAgent needs a live gate OAuth token (the cloud
  ...
  return (
    <div>
      <button
        ...
        className="w-full rounded-xl border border-castle-allow/40 bg-castle-allow/15 px-4 py-2 text-sm font-semibold text-white hover:bg-castle-allow/20 disabled:opacity-50"
      >
        {busy ? "Connecting to gate…" : "Switch dashboard to WebAgent (cloud) →"}
      </button>
