...
SPAWN_SITES: tuple[SpawnSite, ...] = (
    ...
    SpawnSite(
        ...
        'AR',
        'ruff-edit-gate',
        "Ruff edit gate: `ruff check --stdin-filename <rel> --force-exclude --no-fix --no-cache --output-format json -` on the BEFORE and AFTER text of a .py edit, so an edit may not introduce findings under the project's own Ruff config. Routed through audited_run; passthrough-lambda seam (#345). argv[0] is the ruff console script of the ACTIVE interpreter's environment (sysconfig scripts dir) — never PATH, never a project venv; source travels on stdin, never as argv; no shell; bounded 10s timeout; CREATE_NO_WINDOW. Agent-reachable because every agent .py edit triggers it; FAILS CLOSED (a broken ruff refuses the edit).",
    ),
    SpawnSite(
        'runtime_service.py',
...
FILE_NOTES: dict[str, str] = {
    ...
    'ruff_edit_gate.py': "Ruff edit gate — lints the BEFORE/AFTER text of a .py edit on stdin with the active environment's own ruff (sysconfig scripts dir, never PATH); fixed argv, no shell, bounded timeout, fail-closed",
