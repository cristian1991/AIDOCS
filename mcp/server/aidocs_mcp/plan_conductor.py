...
from .types import PlanConductorGraph, PlanConductorLane, RunnableLaneResult
...
class PlanConductor:
    ...
    def graph(self) -> dict[str, object]:
        ...
        graph = PlanConductorGraph(
            ...
            lanes=lanes,
            dependencies={lane.lane_id: list(lane.depends_on) for lane in self.plan.lanes if lane.depends_on},
    ...
    def runnable_lanes(self) -> dict[str, object]:
        ...
        completed_lanes = {lane.lane_id for lane in self.plan.lanes if self._lane_is_completed(lane)}
        ...
        for phase in self.plan.phases:
            ...
            for lane in phase.lanes:
                if lane.lane_id in completed_lanes:
                    continue
                result.blocked_reasons[lane.lane_id] = [f"waiting-on-phase:{active_phase}"]
        ...
        overlap_reasons = self._candidate_overlap_reasons(candidate_lanes)
        ...
        for lane in candidate_lanes:
            ...
            unresolved_dependencies = [
                ...
                if dependency not in completed_lanes and not self._dependency_is_satisfied_by_contract_ready(dependency, lane)
            ]
    ...
    def _lane_is_completed(self, lane: PlanConductorLane) -> bool:
        matching_lane = next((plan_lane for plan_lane in self.plan.lanes if plan_lane.lane_id == lane.lane_id), None)
        if matching_lane is None or not matching_lane.steps:
            return False
        return all(step.status == "completed" for step in matching_lane.steps)

    def _dependency_is_satisfied_by_contract_ready(self, dependency: str, lane: PlanConductorLane) -> bool:
        if dependency not in self.contract_ready_lane_ids or not self._lane_is_contract_like(lane):
            return False
        dependency_lane = next((plan_lane for plan_lane in self.plan.lanes if plan_lane.lane_id == dependency), None)
