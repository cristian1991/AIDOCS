    ...
    *,
    ...
    Returns:
      - EnforceResult(refusal=...) when DECISION_BLOCK_STRIKE — block,
        ...
        actor = "agent", lane_id passed via tool_input if present.
    ...
    actor = "agent"
    lane_id = str(tool_input.get("lane_id") or "") if tool_input else ""
    triggering_rule = decision.triggering_rule_id
    msg_extra = ""
    try:
...
def enforce_tool_call(
    ...
    *,
    ...
    try:
        ...
        # Q3-A: latch the degraded gate (king-rendered 2026-05-04).
        try:
...
def _enforce_tool_call_inner(
    ...
    *,
    ...
    # 0. [BREAK-GLASS] dev.kill_switch — the emergency key.
    #    Castle law: the emergency key must hang OUTSIDE the prison
    #    cell, not inside it. Checked BEFORE session_id resolution and
    #    the freeze short-circuit so a frozen session can be unlocked
    #    on a dev-flavor install. Mirrors claude_hook.PreToolUse and
    #    AgentOrchestrator.check_tool's own kill_switch check (one
    #    source of truth: enforcement.is_kill_switch_active).
    if is_kill_switch_active(project_root):
        record_kill_switch_bypass(
            project_root,
            source="gate_tool",
            target=tool_name,
            payload={"tool_input_keys": sorted(tool_input_dict.keys())},
        )
        return EnforceResult(
            refusal=None,
            session_id="",
            decision_blocked_by="",
            decision_reason="enforcement_disabled_bypass",
        )
