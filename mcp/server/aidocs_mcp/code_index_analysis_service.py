...
class CodeIndexAnalysisService:
    ...
    def preview_extraction_deps(
        ...
        if ext == ".py":
            return self._preview_deps_python(
                path,
                text,
                block_text,
                outside_text,
    ...
    def _preview_deps_python(
        ...
        try:
            ...
            for node in full_tree.body:
                ...
                elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) or isinstance(
                    node, ast.ClassDef
                ):
    ...
    def suggest_extractions(
        ...
        lang_row = conn.execute(
            "SELECT language FROM code_files WHERE path = ?",
            (path.replace("\\", "/"),),
        ).fetchone()
    ...
    def find_dead_code(
        ...
        if ext == ".cs":
            return self._find_dead_code_cs(path, text)
        return {
            "path": path,
            "dead_imports": [],
            "unused_locals": [],
            "error": f"Unsupported language: {ext}",
        }
    ...
    def _find_dead_code_python(path: str, text: str) -> dict[str, object]:
        ...
        for node in ast.walk(tree):
            if (
                ...
                and node.lineno in type_checking_lines
            ):
                continue
    ...
    def _find_dead_code_js(path: str, text: str) -> dict[str, object]:
        ...
        for i, line in enumerate(all_lines, 1):
            ...
            if m.group(1):
                names = [
                    n.strip().split(" as ")[-1].strip() for n in m.group(1).split(",") if n.strip()
                ]
            elif m.group(2):
                names = [m.group(2)]
            elif m.group(3):
    ...
    @staticmethod
    def _find_dead_code_cs(path: str, text: str) -> dict[str, object]:
        """Regex-based detection of unused `using` *aliases* in a C# file.

        Scope is deliberately narrow: only `using Alias = Fully.Qualified.Name;`
        is analyzed, because the alias identifier is always how the imported
        thing is referenced, making "used vs unused" decidable from the file
        alone. Namespace imports (`using System.Linq;`) and static imports
        (`using static System.Math;`) bring in members by bare name, so
        deciding unused-ness requires the BCL catalog plus full symbol
        resolution — false positives there would cause build breaks when
        agents remove "dead" usings, so we don't report them. `global using`
        lines have project-wide scope and are skipped for the same reason.
        """
        import re
        ...
        alias_using = re.compile(r"^\s*using\s+(\w+)\s*=\s*[^;]+;\s*$")
        global_prefix = re.compile(r"^\s*global\s+using\b")

        alias_entries: list[tuple[int, str]] = []
        for idx, line in enumerate(lines, 1):
            if global_prefix.match(line):
                continue
            m = alias_using.match(line)
            if m:
                alias_entries.append((idx, m.group(1)))

        if not alias_entries:
            return {"path": path, "dead_imports": [], "unused_locals": []}

        last_alias_line = max(idx for idx, _ in alias_entries)
        body_text = "\n".join(lines[last_alias_line:])
        body_text_no_strings = re.sub(
            r'"(?:[^"\\]|\\.)*"|@"(?:[^"]|"")*"|\$"(?:[^"\\{]|\\.|\{[^}]*\})*"',
            "",
            body_text,
        )
        body_text_no_comments = re.sub(
            r"//.*?$|/\*.*?\*/",
            "",
            body_text_no_strings,
            flags=re.MULTILINE | re.DOTALL,
        )

        for line_num, alias in alias_entries:
            if not re.search(rf"\b{re.escape(alias)}\b", body_text_no_comments):
                dead_imports.append({"name": alias, "line": line_num, "kind": "alias"})
