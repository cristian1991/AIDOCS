...
class CodeIndexRouteQueryService:
    ...
    def find_routes(self, project_root: Path, query: str | None = None, limit: int = 50) -> dict[str, object]:
        ...
        symbol_matches = self.store.search_symbols(project_root, needle or "controller", limit=max(limit * 3, 100)) if needle else self.store.search_symbols(project_root, "controller", limit=max(limit * 3, 100))
        code_matches = self.store.search_code(project_root, needle or "route", limit=max(limit * 3, 100)) if needle else self.store.search_code(project_root, "route", limit=max(limit * 3, 100))
        ...
        for item in symbol_matches:
            ...
            score = 0
            if needle:
                score += self.store._score_text_match(needle, symbol, exact=110, prefix=80, contains=50)
    ...
    def trace_api_to_ui(self, project_root: Path, concept: str, limit: int = 50) -> dict[str, object]:
        ...
        touchpoints = self.store.find_ui_backend_touchpoints(project_root, concept=query, limit=limit)["matches"]
        clusters = self.store.find_domain_clusters(project_root, concept=query, limit=limit)["cluster"]

        if method_name:
            refs = self.store.find_references(project_root, symbol=method_name, limit=limit * 3).get("matches", [])
            for ref in refs:
        ...
        if service_name:
            service_symbols = self.store.search_symbols(project_root, query=service_name, limit=limit * 3)
            for item in service_symbols:
                ...
                text_match = self.store._score_text_match(service_name, symbol, exact=80, prefix=50, contains=30)
                container_match = self.store._score_text_match(service_name, container, exact=60, prefix=40, contains=20)
    ...
    def find_ui_backend_touchpoints(self, project_root: Path, concept: str, limit: int = 50) -> dict[str, object]:
        ...
        for item in symbol_matches:
            ...
            symbol = str(item["symbol"])
            container = str(item.get("container") or "")
            text_score = self.store._score_text_match(needle, symbol, exact=120, prefix=90, contains=60)
            # Also check container match (e.g., query="CompleteItemAsync" in container "DocumentService")
            container_score = self.store._score_text_match(needle, container, exact=80, prefix=50, contains=30) if container else 0
    ...
    def find_entrypoints(self, project_root: Path, concept: str | None = None, limit: int = 50) -> dict[str, object]:
        ...
        symbol_matches = self.store.search_symbols(project_root, needle or "init", limit=max(limit * 2, 50)) if needle else []
        code_matches = self.store.search_code(project_root, needle or "main", limit=max(limit * 2, 50)) if needle else []
