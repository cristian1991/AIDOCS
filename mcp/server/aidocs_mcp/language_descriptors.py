...
def _load_descriptor_file(file: Path, source: str) -> LanguageDescriptor | None:
    ...
    if not name:
        return None
    extensions = tuple(ext if ext.startswith(".") else f".{ext}" for ext in _normalize_list(data.get("extensions")))
    suffixes = tuple(sfx if sfx.startswith(".") else f".{sfx}" for sfx in _normalize_list(data.get("suffixes")))
...
def load_language_descriptors(project_root: Path, *, force_reload: bool = False) -> dict[str, object]:
    ...
    for directory in _descriptor_dirs(project_root):
        if not directory.is_dir():
            continue
        source = "built_in" if directory == builtin_descriptor_root() / "index_languages" else "project"
        for file in sorted(directory.glob("*.toml")):
    ...
    for directory in _descriptor_dirs(project_root):
        ...
        for file in sorted(directory.glob("*.toml")):
            ...
            if not extensions and not suffixes and not include_globs:
                entry["valid"] = False
                entry["issues"].append("descriptor must declare extensions, suffixes, or include_globs")
            ...
            for ext in extensions:
                ...
                if previous and previous != name:
                    entry["issues"].append(f"extension collision: {normalized} already owned by {previous}")
            ...
            for suffix in suffixes:
                ...
                if previous and previous != name:
                    entry["issues"].append(f"suffix collision: {normalized} already owned by {previous}")
                seen_suffixes[normalized.lower()] = name

            if outline_family and str(outline_family).strip() and not isinstance(outline_family, str):
            ...
            extractor_family = str(data.get("extractor_family") or "").strip() or None
            if extractor_family and extractor_family not in {"python_ast", "javascript_ast", "typescript_ast", "jsx_ast", "tsx_ast", "csharp_rich", "resx_rich", "css_rich"}:
...
def descriptor_semantics_summary() -> dict[str, object]:
    ...
    built_in_items = [item for item in built_in.values() if isinstance(item, LanguageDescriptor) and item.source == "built_in"]
    embedded = sorted({value for item in built_in_items for value in item.embedded_semantics})
    return {
        "extractor_families": ["python_ast", "javascript_ast", "typescript_ast", "jsx_ast", "tsx_ast", "csharp_rich", "resx_rich", "css_rich"],
        ...
        "built_in_with_extractor_family": sum(1 for item in built_in_items if item.extractor_family),
        "built_in_with_outline_family": sum(1 for item in built_in_items if item.outline_family),
        "built_in_with_outline_patterns": sum(1 for item in built_in_items if item.outline_patterns),
        "built_in_with_role_semantics": sum(1 for item in built_in_items if item.role_hint or item.role_patterns),
...
def descriptor_match_summary(project_root: Path, relative_path: str) -> dict[str, object]:
    ...
    return {
        ...
        "descriptor": {
            ...
            "line_patterns": [{"pattern": pattern, "kind": kind} for pattern, kind in descriptor.line_patterns],
            ...
            "role_patterns": [{"glob": glob, "role": role} for glob, role in descriptor.role_patterns],
            "uses_semantic_tags": bool(descriptor.semantic_tags),
            "uses_raw_outline_patterns": bool(descriptor.outline_patterns),
            "uses_raw_role_patterns": bool(descriptor.role_patterns) and not bool(descriptor.semantic_tags),
        } if descriptor is not None else None,
    }


def language_for_custom_descriptor(project_root: Path, relative_path: str, suffix: str) -> str | None:
    data = load_language_descriptors(project_root)
    extensions = data["extensions"] if isinstance(data.get("extensions"), dict) else {}
    suffixes = data["suffixes"] if isinstance(data.get("suffixes"), dict) else {}
    include_globs = data["include_globs"] if isinstance(data.get("include_globs"), list) else []
...
def descriptor_for_language(project_root: Path, relative_path: str, suffix: str) -> LanguageDescriptor | None:
    data = load_language_descriptors(project_root)
        ...
        items.append(
            {
                ...
                "extractor_family": descriptor.extractor_family,
                "outline_family": descriptor.outline_family,
                "outline_patterns": [{"pattern": pattern, "kind": kind} for pattern, kind in descriptor.outline_patterns],
                "line_patterns": [{"pattern": pattern, "kind": kind} for pattern, kind in descriptor.line_patterns],
                "role_hint": descriptor.role_hint,
                "role_patterns": [{"glob": glob, "role": role} for glob, role in descriptor.role_patterns],
                "module_hints": list(descriptor.module_hints),
