...
def init_db(project_root: Path) -> None:
    ...
    with _canonical_connect(db, row_factory=False) as conn:
        ...
        # table once (copy-first, empire §XVIII): new schema, rows copied with
        ...
        # are rebuilt once (atomic, copy-first — empire §XVIII/#242), PRESERVING every
...
def canonical_add_payload(
    *,
    ...
    """Validate + normalize an add payload to EXACTLY what the row will carry.

    Returns `(payload, None)` or `(None, error)`. Extracted from `add()` (which
    now calls it, so there is one normalizer and not two) because #1055's
    approval binding must hash the bytes that LAND, not the bytes a caller typed.

    THE GAP THIS CLOSES WAS MEASURED, NOT IMAGINED. Three of these normalizations
    change the value between the tool boundary and the INSERT, and an approval
    taken out over the pre-normalization text would therefore approve a preimage
    that differs from the stored row:

      * `priority` — `_canon_priority` folds ALIASES ('medium' -> 'normal'), so
        approving priority='medium' and landing 'normal' rules on a row that is
        not the row that appears.
      * `kind` — a missing kind becomes `KIND_UNSET`, not ''.
      * `content` — `repair_agent_prose` REWRITES the body (#684 sink screen).
        This is the dangerous one: the approved text and the stored text are
        genuinely different strings whenever the screen repairs anything.

    And one NON-normalization matters just as much: `tags` is stored as
    `json.dumps(tags or [])`, so ORDER AND DUPLICATES ARE PRESERVED VERBATIM.
    Any binding that sorted or de-duplicated tags would treat ['b','a'] and
    ['a','b'] as the same approval while the store writes two different rows.
    So this function deliberately does NOT tidy the list, and the digest must
    not either.

    `status` is validated and then DROPPED: the INSERT hardcodes 'open', so
    'open' is the only value a new row can carry (#818 clause 2).
    """
    ...
    return (
        {
            ...
            "kind": kind,
            "difficulty": difficulty,
            # NOT sorted and NOT de-duplicated: see the docstring. This is the
            # exact list the INSERT serializes.
            "tags": list(tags or []),
        },
        None,
    )
...
def add(
    ...
    *,
    ...
    # ONE NORMALIZER, called rather than inlined (see `canonical_add_payload`):
    # #1055's approval binding hashes that SAME function's output, so the
    # approved preimage and the stored row cannot diverge. A field normalized in
    # one place and not the other is exactly how an approval comes to authorize
    # a row nobody ruled on.
    canon, canon_err = canonical_add_payload(
        content=content,
        priority=priority,
        kind=kind,
        difficulty=difficulty,
    ...
    if canon_err is not None or canon is None:
        return {"ok": False, "error": canon_err or "invalid backlog payload"}
...
# ONE public max page size for ai_backlog(mode='list'), and the store fetch cap
# derived from it. The handler detects truncation by asking for limit+1 rows;
# when the store clamped at the SAME number the public surface accepted (500),
# the probe row could never arrive and a 501-row backlog listed at limit=500
# answered truncated:false (q3, 2026-09-24). The fetch cap is therefore
# public max + 1 -- never equal to it.
LIST_PUBLIC_MAX_LIMIT = 500
LIST_FETCH_MAX_ROWS = LIST_PUBLIC_MAX_LIMIT + 1
    ...
    *,
    ...
    with _canonical_connect(_db_path(project_root)) as conn:
        ...
        rows = conn.execute(
            ...
            f"  created_at ASC "
            f"LIMIT ?",
            (*params, max(1, min(int(limit), LIST_FETCH_MAX_ROWS))),
        ).fetchall()
    ...
    *,
    ...
    if status is not None:
        ...
        if _unmerge:
            ...
            # The legacy 'merged' → 'open' un-fold is an unmerge too: it must
            # release the authority's relation, not only the local pointer.
            _changed["merged_into_global_id"] = None
...
def _merge_locked(
    ...
    *,
    ...
    for b in absorbed_ids:
        if gid_of.get(b):
            # `merged_into` stays the LOCAL display pointer (#N — what a human
            # reads, and it renumbers). The relation's IDENTITY travels as
            # `merged_into_global_id`, captured HERE while the umbrella row is
            # in hand: resolving #N later could name a different item (#972).
            _fields: dict[str, Any] = {
                "status": status_of[b],
                "merged_into": target,
                "updated_at": now,
            }
            # A legacy umbrella with no global_id has no identity to capture:
            # OMIT the key (the wire mapper then blocks it as unproven) rather
            # than send None, which means "not merged" — the authority would
            # record a merged item as free.
            if gid_of.get(target):
                _fields["merged_into_global_id"] = gid_of[target]
            _emit_backlog(project_root, gid_of[b], "update", _fields)
    for prev_rowid, prev_body in repoint_body.items():
    ...
    if gid:
        _emit_backlog(
            project_root, gid, "update",
            {
                "status": new_status,
                "merged_into": None,
                # Clears the relation BY IDENTITY too, so the authority's copy
                # is released and not only the local pointer (#972).
                "merged_into_global_id": None,
                "updated_at": now,
            },
        )
    ...
    for row in rows:
        ...
        _emit_backlog(
            project_root, gid, "add",
            {
                ...
                # #972: the seed claims to carry the row's CURRENT STATE, so it
                # carries every durable field, not the pre-#573 subset.
                "kind": d["kind"], "difficulty": d["difficulty"],
                "linked_task_id": d["linked_task_id"],
                "promoted_from_todo_id": d["promoted_from_todo_id"],
                # The display pointer only. NO merged_into_global_id: a stored
                # #N may no longer name the umbrella it was written for (ids
                # renumber), so identity is never reconstructed from it — the
                # wire mapper blocks such a row as migration debt.
