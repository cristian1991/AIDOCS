...
class ClaudeHookHandler:
    ...
    def _handle_pre_tool_use(
        ...
        # Check conductor comms — lane state + pending messages
        advisory_parts: list[str] = []
        if decision.advisory:
            advisory_parts.append(decision.advisory)
        ...
        try:
            ...
            if managed.get("active"):
                ...
                if lane_id:
                    ...
                    # Lane paused → block
                    if comms["lane_state"] == "paused":
                        return {
                            "decision": "block",
                            "reason": f"Lane '{lane_id}' is paused: {comms['lane_reason']}. Wait for conductor to resume.",
                        }
                    if comms["lane_state"] == "canceled":
                        return {
                            "decision": "block",
                            "reason": f"Lane '{lane_id}' has been canceled: {comms['lane_reason']}. Stop working.",
                        }
                    # Pending messages → inject as context
                    for msg in comms.get("pending_messages", []):
                        advisory_parts.insert(0, f">>> CONDUCTOR MESSAGE: {msg['content']}")
        except Exception:
            pass
