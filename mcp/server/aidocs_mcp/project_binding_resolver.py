...
def _resolve_local(project_root: Any) -> ProjectBinding:
    ...
    THE LOCAL WRITER (#988). This docstring used to say no PRODUCTION caller
    registers the local box's own project; that has been false since #988.
    LOCAL ``ai_project(mode='connect', project_root=..., project_id=...)`` is a
    two-phase "confirm cloud link" act that calls
    ``local_cloud_link.link_project``: it obtains the gate credential, asks the
    authority which projects the caller is entitled to, refuses an unanswered,
    rejected, unentitled or org-less state WITHOUT writing, and only then
    persists through ``GateProjectStore.link_local`` — the same per-home
    ``gate_projects`` store read here, one canonical project id per root. That
    confirmed, entitlement-checked link is the ONLY authority transition.

    An unlinked root still answers ``local_unregistered``, never a guess. A
    repo key (remote URL/ref) may one day help DISCOVER candidates to offer the
    operator, but it is never an identity and never writes a registration by
    itself (r0b, ae192db4-231).
