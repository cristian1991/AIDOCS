...
#: #956 — file kinds that CANNOT contain a call site, ranked below those that
#: can in find_references. Split by what the kind can hold, never by where the
#: file lives: mcp/scratch/fbinv holds both .jsonl dumps AND the .py scanners
#: that produced them, and scratch/ holds load-bearing doctrine .md next to
#: session-transcript .jsonl. A path rule would delete real code and real prose
#: to remove data.
#:
#: DATA is demoted harder than PROSE on purpose. A .md discussing a symbol is
#: frequently what a reader wants and often carries the reasoning trail; a
#: machine-written .jsonl scan dump is never the answer to "who calls this".
_DATA_SUFFIXES = frozenset({".json", ".jsonl", ".ndjson", ".csv", ".tsv", ".lock"})
...
class CodeIndexSymbolSearchService:
    ...
    def decorate_hits(
        ...
        for h in hits:
            ...
            sym = str(h["symbol"])
            own_container = str(h.get("container") or "")
            ...
            for ln, cont in outlines.get(str(h["path"]), []):
                ...
                # #481: containment is a scope-CHAIN test, not name membership —
                # a member named after its container (every C# constructor) made
                # its siblings look nested and swallowed the rest of the file.
                if row_is_inside_scope(cont, own_container, sym):
    ...
    def search_concept(
        ...
        """AND-first concept search over symbols AND file summaries (#1039 A+ 5).

        Collects candidates PER TERM (a whole-phrase LIKE never matches
        "draft cleanup" against ``StaleDraftCleanupJob`` or against the
        summary "removes abandoned empty draft documents"), then ranks every
        candidate by how many distinct query terms it matches across its
        identifier tokens, path segments, container and file summary
        (``concept_rank.and_rank``). Returns {"terms", "results"} where each
        result carries facet ("symbol" | "file"), terms_matched, terms_total.
        """
        from .concept_rank import and_rank, query_terms

        self.store.init_db(project_root)
        terms = query_terms(query)
        if not terms:
            return {"terms": [], "results": []}
        per_term = max(limit * 2, 20)
        ...
        def _summary(path: str) -> str:
            if path not in summaries:
                try:
                    with self.store.connect(project_root) as conn:
                        row = conn.execute(
                            "SELECT summary FROM code_files WHERE path = ? LIMIT 1", (path,),
                        ).fetchone()
                    summaries[path] = str(row["summary"] or "") if row else ""
                except Exception:
                    summaries[path] = ""
        ...
        for term in terms:
            try:
                sym_rows = self.search_symbols(project_root, term, limit=per_term)
            except Exception:
                sym_rows = []
            for r in sym_rows:
                key = (str(r.get("path")), str(r.get("symbol")))
                if key in seen:
                    continue
                seen.add(key)
                candidates.append(
                    {
                        ...
                        "summary": _summary(str(r.get("path") or "")),
                    },
                )
            try:
                file_rows = self.store.search_code(project_root, term, limit=per_term)
            except Exception:
                file_rows = []
            for r in file_rows:
                key = (str(r.get("path")), "")
        ...
        ranked = and_rank(
            ...
            terms,
            lambda r: (
                # identity first: the symbol name, or the file basename
                str(r.get("symbol") or str(r.get("path") or "").rsplit("/", 1)[-1]),
                str(r.get("path") or ""),
                str(r.get("container") or ""),
                str(r.get("summary") or ""),
            ),
        )
        # Symbols outrank bare file matches at equal term coverage: a named
        # thing is a better next step than a file that merely mentions it.
        ranked.sort(
            key=lambda r: (
                -int(r.get("terms_matched") or 0),
                -int(r.get("identity_terms") or 0),
                0 if r.get("facet") == "symbol" else 1,
            ),
        )
        for r in ranked:
            r.pop("summary", None)
        return {"terms": terms, "results": ranked[:limit]}
    ...
    def find_references(
        ...
        for item in matches:
            ...
            stripped = str(item["line"]).lstrip()
            suffix = Path(str(item["path"])).suffix.lower()

            # #956 — WHAT CAN THIS FILE KIND CONTAIN? A .jsonl/.json dump can
            # only ever MENTION a symbol; a .md/.txt can only DISCUSS it.
            # Neither can be a CALL SITE, which is the one thing this mode
            # exists to find, so they rank below anything that can.
            #
            # DEMOTED, NEVER EXCLUDED: a doc mentioning the symbol is often
            # exactly what a reader wants, and whole evidence trails live in
            # .md. The bug was ORDER, not presence.
            #
            # NOT BY PATH, deliberately (operator ruling 2026-08-28, correcting
            # my first proposal): mcp/scratch/fbinv holds the .py scanners that
            # PRODUCED its .jsonl dumps, and scratch/ holds load-bearing
            # doctrine .md. Excluding the folder would delete real code and real
            # prose to remove data. The discriminator is the file KIND.
            if suffix in _DATA_SUFFIXES:
                score -= 800
            elif suffix in _PROSE_SUFFIXES:
                score -= 400

            # WHAT IS THIS LINE DOING? A call outranks a definition outranks a
            # comment. Cheap and syntactic on purpose — this is a ranked text
            # sweep (source="file_content"), not a resolved call graph, and a
            # heuristic that mis-orders is recoverable where an exclusion is
            # not.
            if re.search(rf"\b{re.escape(lower_symbol)}\s*\(", line_lower):
                score += 200  # call-shaped
            if re.match(rf"(?:async\s+)?(?:def|class)\s+{re.escape(lower_symbol)}\b", stripped):
                score += 100  # the definition — wanted, but it is not a caller
            if stripped.startswith(("#", "//", "*", '"""', "'''")):
                score -= 150  # prose that happens to live in a source file

            # The old `+120` for "the line contains the symbol" was DEAD: the
            # sweep pattern above already matched, so it fired for every single
            # result and carried no signal. Removed rather than kept as a
            # constant, so the scores below mean something.
            if (
                ...
                score += 40
            # Depth kept ONLY as a tiebreak. It used to be the sole live term,
            # which made shallow repo-root docs structurally outrank deep
            # production source — agents/notes.md (1 slash) beat
            # mcp/server/pkg/caller.py (3) on every query, forever.
        ...
        # #1039: source="file_content" cannot see what the index refused. Say
        # how many source files were NOT searched — loudly when empty.
        try:
            out = self.store.annotate_unindexed(project_root, out)
        except Exception:
            pass
        return out
