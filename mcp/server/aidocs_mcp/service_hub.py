...
class AidocsServiceHub:
    ...
    def require_permission(
        ...
        *,
        ...
        # DEV-FLAVOR break-glass passthrough — a contributor build is the
        # developer's own machine, so the local operator is admin (matches
        # is_dev_flavor / is_kill_switch_active break-glass precedent and
        # makes this gate's long-standing docstring true: dev flavor was
        # claimed to short-circuit but never actually did). solo + corpo
        # still enforce RBAC below. Flavor is GLOBAL-locked, so a hostile
        # repo cannot self-elevate. Audited.
        try:
            from .enforcement import is_dev_flavor

            _dev = is_dev_flavor(project_root)
        except Exception:
            _dev = False
        if _dev:
            try:
                self.execution.record_event(
                    project_root,
                    event_kind="rbac_dev_flavor_allow",
                    ...
                    payload={
                        "reason": "dev_flavor_break_glass",
                        "required_permission": permission_name,
                    },
                )
            except Exception:
                pass
            return {"ok": True, "dev_flavor": True}
