...
# The canonical connect pays both debts in one edit: its factory closes
# on __exit__ and it applies the full pragma set.
# DURABILITY: AUDIT. The one-shot tenant RBAC bootstrap STAMP and the
# grant probes around it. The stamp is what stops a later, different
# admin being auto-granted org-admin on a tenant project; losing it to
# a power cut reopens a grant surface that was deliberately closed.
...
def _identity_db_path(project_root: Path) -> Path:
    """Where the one-shot BOOTSTRAP STAMP lives — the PROJECT's own DB (#516).

    The stamp answers "has THIS project's RBAC been bootstrapped?", which is a
    project fact, not an operator fact. #488 pointed it at the machine-global
    identity home, where one bare key is shared by every project: after the
    FIRST tenant project was stamped, every later project reported
    already-bootstrapped and its creator was granted NOTHING (the heal-forward
    bridge silently closed machine-wide). Keeps the pre-#488 spelling, so an
    existing install's stamp is still found where it was written.
    """
    from .identity_db import legacy_project_identity_path

    return legacy_project_identity_path(Path(project_root))


def _rbac_db_path(project_root: Path) -> Path:
    """The GRANT store — machine-global by #488 ruling."""
...
def _store_has_any_role_grants(project_root: Path) -> bool:
    ...
    #516: SCOPED to this project. The store is machine-global (#488), so an
    unscoped "any row at all" probe sees every OTHER project's grants and would
    declare a brand-new tenant project "already configured" — closing the
    heal-forward bridge machine-wide. A global-scope grant (the machine owner's
    break-glass) still counts: it really is in force here.
    """
    try:
        from .project_authority import project_scope_key

        db = _rbac_db_path(project_root)
        if not db.exists():
            return False
        with _canonical_connect(db, durability=_Durability.AUDIT, row_factory=False) as conn:
            row = conn.execute(
                "SELECT 1 FROM rbac_user_roles WHERE scope_type = 'global' "
                "OR (scope_type = 'project' AND scope_id = ?) LIMIT 1",
                (project_scope_key(Path(project_root)),),
            ).fetchone()
        return row is not None
    except sqlite3.Error:
        return False
...
def _grant_project_scoped_super_admin(store: Any, root: Path, uid: str) -> bool:
    """CLASS 2 grant (#516): org-admin authority in THIS project only.

    #488 pooled every project's grants into one machine-global store, and this
    bootstrap wrote its grant at GLOBAL scope — so a tenant admin provably held
    ``super_admin`` inside a FOREIGN org's project (measured cross-tenant
    privilege escalation). The operator's ruling splits the classes: the
    MACHINE OWNER's break-glass stays global (``assign_role_to_user_by_name``,
    #488), while a TENANT's project bootstrap must be written at
    ``scope_type='project'`` with the ``project_scope_key`` spelling #500's
    reads use — never visible from another org's project.

    Returns True iff a new grant row was created (INSERT OR IGNORE on the
    (user, role, scope_type, scope_id) key keeps re-bootstrap idempotent).
    """
    from .project_authority import project_scope_key

    role = store.get_role_by_name(root, "super_admin")
    if role is None:
        return False
    return bool(
        store.assign_role_to_user_scoped(
            ...
            role.role_id,
            scope_type="project",
            scope_id=project_scope_key(root),
...
def bootstrap_project_creator_rbac(
    ...
    * FAIL-CLOSED: an empty/unresolvable creator grants NOTHING and writes NO
    ...
    seed_rbac(root)
    store = RBACStore()
    granted = _grant_project_scoped_super_admin(store, root, uid)
