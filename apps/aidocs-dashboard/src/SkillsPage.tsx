      ...
      <div className="page-fixed-header config-header-row">
        ...
        {tab === "user" && onUploadSkill && (
          <div className="config-header-actions">
            <button type="button" className="action-button action-button-compact" disabled={!configEditingAvailable()} onClick={onUploadSkill}>
              Upload Skill
            </button>
          </div>
        )}
                ...
                <div key={status.provider_id} className="setting-row" style={{ borderLeft: `3px solid ${providerStateColor(status.provider_state)}`, paddingLeft: "10px", background: "rgba(255,255,255,0.02)" }}>
                  ...
                  <div className="setting-control" style={{ display: "flex", gap: "6px", justifyContent: "flex-end", alignItems: "center", flexWrap: "wrap" }}>
                    {status.choices.includes("keep_enabled_anyway") ? (
                      <button type="button" className={`action-button action-button-compact ${status.user_choice === "keep_enabled_anyway" ? "action-button-active" : ""}`} disabled={pending || !configEditingAvailable()} onClick={() => onSetProviderOverride?.(status.provider_id, "keep_enabled_anyway")}>
                        Keep Enabled
                      </button>
                    ) : null}
                    {status.choices.includes("disable") ? (
                      <button type="button" className={`action-button action-button-compact ${status.user_choice === "disable" ? "action-button-active" : ""}`} disabled={pending || !configEditingAvailable()} onClick={() => onSetProviderOverride?.(status.provider_id, "disable")}>
                        Disable
                      </button>
                    ) : null}
                    {status.user_choice ? (
                      <button type="button" className="action-button action-button-compact" disabled={pending || !configEditingAvailable()} onClick={() => onSetProviderOverride?.(status.provider_id, null)}>
                        Auto
                      </button>
                    ) : null}
                  </div>
              ...
              <div key={skillId} className={`setting-row${item.scan.finding_count > 0 ? " setting-row-danger" : ""}`} style={item.scan.finding_count > 0 ? { borderLeft: "3px solid #f87171", paddingLeft: "8px" } : undefined}>
                ...
                <div className="setting-control" style={{ display: "flex", gap: "6px", justifyContent: "flex-end", alignItems: "center" }}>
                  <button
                    ...
                    disabled={selectDisabled || !configEditingAvailable()}
                  ...
                  {isUser && onDeleteSkill && (
                    <button
                      ...
                      disabled={!configEditingAvailable()} onClick={() => onDeleteSkill(skillId)}
                    >
                      Del
                    </button>
                  )}
