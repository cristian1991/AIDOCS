...
# Preflight only reads (capabilities, code, memory, tests, memory links),
# so read_only=True; the explicit row_factory=None below is preserved by
# passing row_factory=False.
...
def preflight(project_root: Path, task: str) -> dict:
    ...
    with _canonical_connect(str(db_path), read_only=True, row_factory=False) as conn:
