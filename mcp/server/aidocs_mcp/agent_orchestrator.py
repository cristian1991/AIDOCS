...
class AgentOrchestrator:
    ...
    def _bash_unauthenticated_refusal(
        ...
        """#257: return a fail-closed ToolDecision when the INTERACTIVE operator's
        host session is unauthenticated AND auth is required (corpo install).
        Returns None (execution proceeds) for delegated callers (subagent / lane
        worker — they inherit the conductor's auth), local solo/dev installs
        (machine presence IS the authority — no dashboard login exists), or an
        authenticated session. Mirrors the config-write auth gate
        (operator_intent_resolver.resolve_operator_context_from_host_session,
        which already refuses `unauthenticated_host_session`) — closing the gap
        where the persisted bash allow-table kept authorizing exec past logout."""
        # Delegated execution inherits the conductor's authority — never gate a
        # subagent / lane worker on its own (empty/different) host binding.
        if not is_privileged_caller:
            return None
        try:
            from .operator_auth_service import _auth_status_flavor

            if _auth_status_flavor() in ("dev", "solo"):
                return None  # local single-tenant: machine presence is the authority
        except Exception:
            # _auth_status_flavor itself fails closed to 'corpo'; fall through and
            # require auth (the safe direction) if the read blew up.
            pass
        try:
            ...
            ctx = OperatorAuthService().resolve_operator_context_from_host_session(
                host_sid, project_root
            )
        except Exception:
            ctx = None  # fail closed
        if ctx is not None:
            return None  # authenticated operator — proceed
        try:
            self._record_event(
                project_root,
                "bash_policy_block",
                ...
                reason="Unauthenticated host session — shell execution requires an authenticated operator.",
        ...
        return ToolDecision(
            ...
            reason=(
                ...
                "session, but the host session is unauthenticated (logged out or "
                "dashboard TTL expired). Re-authenticate in the dashboard, then retry."
            ),
    ...
    def check_tool(
        ...
        if _gate_uid:
            managed = self.hub.managed_mode.get_mode(
                project_root, host_session_id=_gate_uid, strict=True
            )
        else:
        ...
        try:
            ...
            if raw_target:
                ...
                if _is_write_family:
                    try:
                        from .session_artifact import is_foreign_session_workspace

                        _foreign_ws = is_foreign_session_workspace(
                            raw_target,
