...
def register_palace_tools(*, server: Any, hub: Any, runtime: Any) -> None:
    ...
    _ti_c20.register_impl("ai_palace_search", ai_palace_search)
    _ti_c20.register_impl("ai_palace_status", ai_palace_status)
    ...
    def ai_palace_maintenance(
        ...
        Modes:
          ...
          - ``reingest_canonical`` — bulk-project EVERY active memory_index row
            into the palace and rebuild its KG triples (the #382 graph
            backfill). Not redundant with the legacy backfill: that one touches
            no graph, and since the palace is a REBUILDABLE PROJECTION this is
            what makes the graph rebuildable from canonical rows. Admin-gated.
            ``dry_run`` is REFUSED by name (blocked_by="dry_run_unsupported"):
            the bulk ingest has no preview path, and silently writing drawers
            when the caller asked to preview is the worst direction for a bulk
            mutation.
...
def run_palace_maintenance(
    ...
    *,
    ...
    # Both remaining modes share the spine below (palace-availability check,
    # hub_ctx build, run, audit) and differ only in which bulk function runs.
    if normalized not in ("backfill_legacy_memory_drawers", "reingest_canonical"):
        return {
            "ok": False,
            "blocked_by": "unknown_mode",
            "reason": (
                f"unknown maintenance mode {mode!r}; supported: "
                "backfill_legacy_memory_drawers, reingest_canonical, mine"
            ),
        }
    # #523 item A ruling (2026-07-26) — RE-WIRE, not retire.
    # palace_ingest_from_canonical sat with ZERO production callers after the
    # #763 home-flip unwired it. It is NOT redundant with the legacy backfill:
    # it is also the palace-KG backfill adapter (#382), writing each entry's
    # extract_kg_facts into <palace>/kg.sqlite3 and reporting kg_triples. The
    # legacy backfill touches no graph. Since the palace is a REBUILDABLE
    # PROJECTION, dropping this would have left the graph unrebuildable from
    # canonical rows.
    #
    # FAIL CLOSED ON dry_run: palace_ingest_from_canonical takes no dry_run
    # parameter. Silently ingesting when the caller asked for a preview is the
    # worst possible direction for a bulk mutation, so refuse by name instead.
    if normalized == "reingest_canonical" and dry_run:
        return {
            "ok": False,
            "blocked_by": "dry_run_unsupported",
            "reason": (
                "mode='reingest_canonical' cannot preview: the bulk canonical "
                "ingest has no dry-run path, and proceeding would WRITE drawers "
                "and KG triples. Re-run without dry_run to apply it, or use "
                "mode='backfill_legacy_memory_drawers' (dry_run=true supported) "
                "to preview the drawer-reconciliation half."
            ),
            "authorized_via": via,
        }
    ...
    if not dry_run:
        try:
            ...
            ctx = None
    if normalized == "reingest_canonical":
        # Bulk canonical -> palace projection + KG backfill (#382). No dry_run
        # leg exists and dry_run was already refused above, so this only ever
        # runs in apply mode.
        stats = _msq.palace_ingest_from_canonical(
            project_root,
            hub.palace,
            hub_ctx=ctx,
        )
...
def _soul_forbidden_roots() -> list[Path] | None:
    ...
    moves, this exclusion moves with it (doctrine XI: structural, not
    filtered). Returns ``None`` when the locations cannot be resolved — the
    caller FAIL-CLOSES on ``None`` and refuses to mine at all.
    """
    try:
        from .skill_store import empire_db_path, soul_chamber_paths

        empire_home = empire_db_path().parent.resolve()
        return [*soul_chamber_paths(), empire_home]
    except Exception:
        return None


def _mine_content_guard(source_file: str, text: str) -> tuple[str, dict]:
    """Tier 1/2 ingest guard (doctrine XII): output_guard's regex+entropy scan
    ...
    - injection hits   -> ingest the text UNCHANGED and stamp
                          ``guard_injection_categories`` on the drawers
                          (#648/#651). Injection SHAPE in stored knowledge is
                          DATA, not an attack on the miner: this project's own
                          scanner pattern tables, red-team corpus, and security
                          docs are exactly the material a palace must be able
                          to recall. Refusing them would blind the palace to
                          its own security documentation, and redacting them
                          would destroy the knowledge. So the drawer carries a
                          mark instead — a reader of that drawer learns the
                          content is quoted DATA, never a directive. This scan
                          used to be filtered down to ``credential:`` here and
                          the injection findings were DISCARDED.
    ...
    injection_meta: dict = {}
    injection = _og.injection_findings(result)
    if injection:
        injection_meta["guard_injection_categories"] = ",".join(
            sorted({str(f.category) for f in injection})
        )
    credential = [
        f for f in result.findings if str(f.category).startswith("credential:")
    ]
    if not credential:
        return text, injection_meta
...
def _run_palace_mine(
    ...
    *,
    ...
    # Sovereign souls: structural exclusion (doctrine XI). Cannot resolve the
    ...
    result = {
        ...
        "evidence_note": (
            "mined drawers are EVIDENCE, never law (empire-doctrine XII): they surface "
            ...
            "(doctrine XXII) — never a second source of truth"
        ),
