...
from .shell_provider_identity import _PROVIDER_BASENAMES as _IDENTITY_BASENAMES

# #561 phase 4. THE identity law's bash set, NAMED — never restated.
#
# This module's auto-enrolment gate used to carry its own inline copy of the
# bash basename pair, under a comment asserting the identity law governed it.
# Prose said "read the law"; the branch said "here is the law again". Two
# consumers of one ruling, and only one of them would follow a narrowing of it —
# so the day ``_PROVIDER_BASENAMES["bash"]`` tightens, auto-enrolment would keep
# minting signed offers for a binary the identity law had just refused.
#
# Bound to the SAME frozenset object (not a copy) so divergence is not merely
# unlikely but unrepresentable, and so the seam test can assert it with ``is``.
_BASH_IDENTITY_BASENAMES = _IDENTITY_BASENAMES["bash"]
...
def machine_presence_native_ok(
    ...
    """Authorize the provider-less native route only for a logged-in operator.

    The function name is retained as an internal compatibility seam, but
    machine presence is never authority. The proven Claude Code + bash adapter
    remains eligible only when the shared project-authority seam resolves an
    authenticated principal (token, enabled approved binding, or machine login).
    """
    if project_root is None:
        return (False, "no bound project context for authenticated native execution")
    try:
        from .shell_capability_matrix import HOST_CLAUDE_CODE, PROVIDER_BASH
    except Exception:
    ...
    if (host or "") != HOST_CLAUDE_CODE or (provider or "") != PROVIDER_BASH:
        return (
            False,
            ("provider-less native execution is limited to the proven "
            "claude_code+bash adapter; other hosts/providers route to ai_run"),
        )
    try:
        ...
        uid = _authenticated_uid(Path(project_root), host_session_id)
    except Exception:
        uid = ""
    if not uid:
        return (
            False,
            ("native shell execution requires an authenticated operator login; "
            "machine presence alone is not authority"),
        )
    return (
        True,
        "authenticated operator on the proven claude_code+bash adapter",
    )
...
def discover_supported_provider(
    *,
    ...
    for cand in _system_owned_candidates():
        ...
        # the identity law's _PROVIDER_BASENAMES, READ here rather than
        # restated; see _BASH_IDENTITY_BASENAMES above).
        if cand.name.lower() not in _BASH_IDENTITY_BASENAMES:
            continue
