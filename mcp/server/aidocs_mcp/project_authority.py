...
def require_cross_project_session(
    ...
    *,
    ...
    source_permission: str = PERM_ADMIN_MANAGE_SESSIONS,
    target_project_permission: str = PERM_ADMIN_MANAGE_SESSIONS,
    target_session_permission: str = PERM_ADMIN_MANAGE_SESSIONS,
) -> Decision:
    """Full target-side authority for a cross-project session WRITE/BIND
    (connect / handoff / lane spawn / XAACP message into another project's
    session).
    ...
    PER-STAGE PERMISSIONS (8d84 phase 3, r0b2 644959cc-d65 D3). Each stage's
    permission is named separately and each DEFAULTS to admin.manage_sessions,
    so every caller that predates this is unchanged. XAACP passes
    xaacp.message_session for all three: create/archive/transfer is the wrong
    power for sending a message. Naming a permission never relaxes anything
    else -- the approved relation, target membership and target session RBAC
    stay mandatory whatever permission is asked.
    ...
    ts = require_session(
        ...
        host_session_id=host_session_id,
        project_permission=target_project_permission,
        session_permission=target_session_permission,
    )
    if not ts.ok:
        return Decision(
            ok=False,
