...
Three-tier ladder (#461's mechanism):
  ...
  TIER 3 EXECUTION RECEIPT  a control that must run in ANOTHER process writes a
                            ...
                            deploy gate already embodies it (doctrine §XV:
                            "the crown-head marker is the verdict"); a truth
                            declares ``receipt=True`` to demand it.

THE ABSENT CASE WAITS — IT NEVER FAILS (doctrine §XXV, KISS). The registry is
...
(doctrine §XXV: a seam with a clean input->output boundary).
"""
...
# contract test — the registry starts GREEN on purpose (§XXV: the absent
...
# build-time tool (doctrine §XXVI: dev/CI toolchain, never in the runtime), and
...
def resolve_declaration(declaration: str) -> "tuple[object | None, str]":
    ...
    is a REPORT-level fact, and the report fails open (doctrine §XXVII — an
...
def contract_test_paths() -> "tuple[str, ...]":
    ...
    acceptance suite (doctrine §XXV, "the boundary is enumerable").
    """
