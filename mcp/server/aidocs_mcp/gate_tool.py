...
_PRECHECK_LOG_MAX = 200


def _log_precheck_exception(kind: str, exc: BaseException) -> None:
    """Privileged server-log diagnostics for a raised judge precheck (#1097).

    The ONLY sink for the exception message: never the tool result, never the
    audit payload. Bounded, then passed through the output-guard credential
    redactor. Best-effort -- a logging failure must never soften the refusal.
    """
    try:
        import logging

        msg = str(exc)[:_PRECHECK_LOG_MAX]
        try:
            from .output_guard import scan_text

            guarded = scan_text(msg, redact=True)
            if guarded.redacted_text is not None:
                msg = guarded.redacted_text
        except Exception:
            msg = "<unredactable; omitted>"
        logging.getLogger(__name__).warning(
            "judge_precheck_unavailable: %s: %s",
            kind,
            msg,
        )
    except Exception:
...
def _flat_judge_refusal(
    ...
    *,
    ...
    """A structured no-confirm, no-strike judge refusal.

    Mints nothing: no confirmation, no freeze row, no strike. The audit event
    is best-effort; its failure must never soften the refusal.
    """
    reason = f"{reason} (blocked_by={blocked_by})"
    try:
        hub.execution.record_event(
            ...
            action_kind="block",
            target_entity=str(rule_id or "")[:200],
            status="blocked",
            payload={
                "tool_name": tool_name,
                "rule_id": rule_id,
    ...
    return EnforceResult(
        refusal={"ok": False, "blocked_by": blocked_by, "reason": reason},
...
def _judge_taxonomy_precheck(
    ...
    *,
    ...
    from .judge_taxonomy import (
        ...
        DECISION_BLOCK_SECURITY_FAILURE,
    ...
    if session_id:
        try:
            ...
            if query_gate is not None:
                # #893: the verdicts are passed so intent must SPEAK FOR the
                # rule that fired ("clean up the imports" is not consent to a
                # hard reset). Unmapped families keep the legacy behaviour.
    ...
    if decision.decision == DECISION_BLOCK_FREEZE_NO_CONFIRM:
        ...
        # Carry the judge's own recommendation ("Use ai_create_file / ai_replace
        # ...") so a refused shell write is POINTED at the governed tool instead
        # of only told it lacked intent (operator, 2026-08-25). Additive text.
        _rec = ""
        for _v in judge.verdicts:
            if getattr(_v, "rule_id", "") == triggering_rule:
                _rec = str(getattr(_v, "recommendation", "") or "").strip()
                break
        reason = (
            f"refused: confirmable_destructive rule {triggering_rule!r} "
            f"fired without matching operator destructive intent. "
            f"{decision.reason}"
            + (f" Do this: {_rec}" if _rec else "")
    ...
    if decision.decision == DECISION_BLOCK_SECURITY_FAILURE:
        # The judge could not finish a security inspection. Hard refusal of
        # this invocation: no confirm path (intent was ignored upstream), and
        # NO strike — SecurityViolationService is deliberately not called,
        # because the defect is the judge's, not the caller's.
        return _flat_judge_refusal(
            hub,
            project_root,
            session_id=session_id,
            tool_name=tool_name,
            blocked_by="judge_security_inspection_failure",
            event_kind="judge_security_inspection_failure_block",
            rule_id=decision.triggering_rule_id,
            verdict_class=decision.triggering_class,
            reason=(
                f"refused: security inspection did not complete for rule "
                f"{decision.triggering_rule_id!r} — the command cannot be "
                "shown safe, so it does not run. Not confirmable; no strike "
                "recorded. Rewrite the command without the construct the "
                "inspector could not read."
            ),
        )
    if decision.decision != DECISION_BLOCK_STRIKE:
        # An unrecognised decision is an unfinished decision: refuse, never
        # fall through to a cascade that would treat it as clean.
        return _flat_judge_refusal(
            hub,
            project_root,
            session_id=session_id,
            tool_name=tool_name,
            blocked_by="judge_taxonomy_unknown_decision",
            event_kind="judge_taxonomy_unknown_decision_block",
            rule_id=decision.triggering_rule_id,
            verdict_class=decision.triggering_class,
            reason=(
                f"refused: the judge returned an unrecognised decision "
                f"{str(decision.decision)[:60]!r} — failing closed. Not "
                "confirmable; no strike recorded."
            ),
        )
...
class EnforceResult:
    ...
    """WHICH policy rule produced the refusal, carried verbatim from
    ``ToolDecision.matched_rule`` (local backlog 984).

    PROVENANCE CHAIN, and it runs one way only:

        bash_policy -> ToolDecision.matched_rule -> ToolGateResult /
        EnforceResult -> the outer gate's confirmation intent

    Never synthesized higher up. A rule id re-derived at a flattening layer
    would be a SECOND answer to "which rule asked", and the confirmation intent
    binds to it — so a divergence would mint handles that satisfy a decision the
    policy never made.

    ADDITIVE and ignorable: existing callers read `decision_blocked_by` and
    `decision_reason` unchanged."""

    decision_needs_confirmation: bool = False
    """Whether the DECISION asked for confirmation, from
    ``ToolDecision.needs_confirmation`` (local backlog 984).

    CARRIED SEPARATELY FROM `decision_blocked_by` ON PURPOSE. The outer gate
    admits a confirmation challenge only when BOTH are true — needs_confirmation
    AND blocked_by == "bash_policy_ask" — and that conjunction is only worth
    anything while the two halves have INDEPENDENT provenance. Deriving this
    from the blocked_by string would make the test of both a test of one, and
    the guard would agree with itself forever: any future refusal that borrowed
    the string would arrive pre-approved for click-through.

    Refusals that are NOT confirmable leave this False, which is what keeps hard
    deny / dangerous chain / sensitive read / control plane / judge-locked
    primitive outranking `ask` by construction rather than by ordering luck."""
...
def enforce_tool_call(
    ...
    *,
    ...
    """
    # BRANCH-TRANSITION BARRIER (r0b send-back on 7cb98b4d9, blocker 2): the
    # MCP-direct / outer-gate cascade honours the same project-scoped barrier
    # as the host pretool gate. Reads pass; the holder's own call passes.
    try:
        from .branch_transition_barrier import barrier_refusal

        _bt_why = barrier_refusal(project_root, tool_name, tool_input)
    except Exception as _bt_exc:  # noqa: BLE001 - fail closed
        _bt_why = f"AIDOCS branch-transition barrier check failed ({_bt_exc!r}); refusing closed."
    if _bt_why:
        return EnforceResult(
            refusal={"ok": False, "blocked_by": "branch_transition_barrier", "reason": _bt_why},
            session_id="",
            decision_blocked_by="branch_transition_barrier",
            decision_reason=_bt_why,
        )
    try:
...
def _enforce_tool_call_inner(
    ...
    *,
    ...
    session_id = ""
    try:
        session_id = resolve_managed_session(ManagedModeService(), project_root)
    except Exception:
        session_id = ""
    ...
    except Exception as _pc_exc:  # noqa: BLE001 -- the refusal below IS the handling
        # #1097: A JUDGE THAT RAISED IS NOT A JUDGE THAT SAID ALLOW. This
        # branch used to set `_precheck = None` -- the same value the precheck
        # returns for ALLOW/ASK -- and fall through to the cascade. Unknown
        # laundered into proceed: the shape DRT-06/07/08 closed at three other
        # gates, still open here. What that lost: the durable strike and
        # no-confirm refusal for malicious_forbidden, and the unknown-decision
        # fail-closed path. Worse, when the cascade then crashed on the same
        # broken judge, the refusal said `evaluator_exception` -- blaming the
        # half of the gate that was healthy.
        #
        # Same class as the inspection-failure precedent (64df): the judge
        # could not finish, so this call does not run. Structured, audited,
        # NO confirm path, NO strike -- unknown is not malice, the defect is
        # ours. The exception CLASS is the audit target and is named in the
        # reason, so the operator can see WHY the gate is degraded rather
        # than only that it is. `None` below now means exactly one thing: the
        # precheck ran and decided ALLOW or ASK.
        #
        # The exception MESSAGE is never host-facing (#1097 release hold,
        # r0b2): the refusal is model-visible, and str(exc) from a security
        # evaluator is an untrusted tunnel -- internal paths, parser fragments,
        # credential-like text. Same floor as `evaluator_exception`: stable
        # reason + class only, in the refusal AND the audit payload. The
        # message goes to the privileged server log only, bounded and
        # passed through the output-guard redactor.
        #
        # BOUND, stated: this is also the outer gate's cascade, so a judge
        # crash refuses calls through the gate too -- the bound the
        # inspection-failure class already accepts.
        _pc_kind = type(_pc_exc).__name__
        _log_precheck_exception(_pc_kind, _pc_exc)
        _precheck = _flat_judge_refusal(
            hub,
            project_root,
            session_id=session_id,
            tool_name=tool_name,
            blocked_by="judge_precheck_unavailable",
            event_kind="judge_precheck_unavailable_block",
            rule_id=f"exception:{_pc_kind}",
            verdict_class="",
            reason=(
                f"refused: the judge could not evaluate this call "
                f"({_pc_kind}) -- with no decision available the call does "
                "not run. Not confirmable; no strike recorded. The gate is "
                "degraded, not the caller at fault; details are in the "
                "server log."
            ),
        )
    ...
    # 4-pre. #571 THREE-WAY ROUTING. Resolve the rung BEFORE any of the
    # session/freeze machinery below, because two of the three outcomes need
    # none of it:
    #   allow — detected user-intent already covered the action (law c09cba5b:
    #           "the agent should not even be blocked").
    #   block — rung 3. No freeze row, no escalation request, no strike, and
    #           critically no session_id requirement, so an unlisted `pwd`
    #           no longer depends on a managed session or on the freeze mint's
    #           identity resolution to produce a truthful answer.
    #   freeze — rungs 1-2, unchanged pipeline below.
    _refusal_outcome = ""
    _refusal_class = ""
    if include_freeze and getattr(decision, "needs_confirmation", False):
        from .verdict_class import OUTCOME_ALLOW, OUTCOME_BLOCK, outcome_for

        _refusal_outcome, _refusal_class = outcome_for(
        ...
        if _refusal_outcome == OUTCOME_ALLOW:
            return EnforceResult(refusal=None, session_id=session_id)
        if _refusal_outcome == OUTCOME_BLOCK:
            ...
            _blk = build_workflow_block_response(
                ...
                verdict_class=_refusal_class,
                recommendation=str(getattr(decision, "advisory", "") or ""),
                # #588 D5: let the envelope READ BACK what is actually
                # latched instead of asserting what this branch intends to
                # write. Both are optional — a session-less block still
                # renders, it just declines to certify the freeze state.
            ...
            return EnforceResult(
                ...
                session_id=session_id,
                # THE DECISION'S REASON, NOT THE ENVELOPE'S CLASS (local
                # backlog 984). These are two different questions and this field
                # answers the second one by name:
                #
                #   refusal["blocked_by"] = "workflow_block"   what KIND of
                #                                              refusal this is
                #   decision.blocked_by   = "bash_policy_ask"  WHY it was
                #                                              refused
                #
                # This line read `_blk["blocked_by"]`, so every confirmable
                # decision arrived upstairs relabelled "workflow_block" — the
                # transport class of EVERY rung-3 block, identical for an ask, a
                # policy block and a lifecycle block alike. The outer gate tests
                # this value against "bash_policy_ask" to decide whether an ask
                # may become a two-phase confirmation, so the relabelling
                # silently disabled that path in full: measured on the live gate
                # as `ai_run sleep 30` still answering "requires operator
                # confirmation ... there is no approval phrase to type".
                #
                # The envelope below is UNCHANGED and still says workflow_block,
                # which is what downstream renderers and the block_state
                # contract read. Nothing about how this refusal presents moves.
                decision_blocked_by=str(getattr(decision, "blocked_by", "") or ""),
                decision_reason=str(_blk["permissionDecisionReason"]),
                # THE FLATTENING POINT (local backlog 984). A genuine
                # `bash_policy_ask` resolves to OUTCOME_BLOCK here and becomes a
                # workflow refusal — correct on a HOOK-CAPABLE host, where the
                # operator answers the ask through the host's native channel.
                # WebMCP has no such channel, so its ask had no way to become
                # approved and the refusal named a confirmation the surface
                # could not give.
                #
                # Carried FROM THE DECISION, never re-derived: the provenance
                # chain is bash_policy -> ToolDecision.matched_rule -> here ->
                # the outer gate's confirmation intent. A rule id synthesized at
                # this layer would be a second answer to "which rule asked", and
                # the confirmation binds to it.
                decision_matched_rule=str(getattr(decision, "matched_rule", "") or ""),
                decision_needs_confirmation=bool(
                    getattr(decision, "needs_confirmation", False)
                ),
            )
    ...
    if include_freeze and getattr(decision, "needs_confirmation", False) and not session_id:
        return EnforceResult(
            refusal={
                ...
                # Law 311bf3e6: the branch tests "no session is BOUND", so the
                # remedy must be a BIND, not `/aidocs` — that command
                # COMMISSIONS a project, and an uncommissioned project is a
                # deliberate operator choice, not the fault being reported
                # here. `ai_session` is BOOTSTRAP_EXEMPT and so is reachable
                # from inside this exact state; a slash command is not.
                "reason": (
                    "action requires operator approval but no active managed "
                    "session exists; bind one with "
                    "`ai_session(mode='connect', session_id='<id>')` "
                    "(`ai_session(mode='list')` shows them)"
                ),
            },
            session_id="",
    ...
    if include_freeze and getattr(decision, "needs_confirmation", False) and session_id:
        ...
        try:
            env = build_freeze_response(
                ...
                # #571: hand the mint site the REAL refusal identity so
                # verdict_class.classify has something to classify. Before
                # this, the only signal that crossed was risk_class — which
                # `or "destructive_action"` above had already flattened to a
                # literal — so every freeze arrived looking destructive.
                # blocked_by is passed unflattened, on purpose.
                blocked_by=str(getattr(decision, "blocked_by", "") or ""),
                matched_rule=str(getattr(decision, "matched_rule", "") or ""),
                user_intent_detected=bool(
                    getattr(decision, "user_intent_detected", False),
                ),
                # Reuse the class resolved by the 4-pre routing above rather
                # than classifying a second time — one refusal must not be able
                # to produce two different classes.
                verdict_class=_refusal_class,
            )
        except Exception:
            return EnforceResult(
