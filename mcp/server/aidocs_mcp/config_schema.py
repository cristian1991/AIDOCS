...
SETTINGS_CATALOG: dict[str, SettingMetadata] = {
    ...
    "index.lsp_materialize": _setting(
        type="boolean",
        default=False,
        description="Call the aidocs_lsp door (lsp_drain_and_evict, scoped to the files changed in the pass) at the end of a code-index sync to materialize semantic_ref edges. Dormant by default; fail-open — any door failure leaves the sync result unchanged (doctrine XXVII guest oracle).",
        scope=["global", "project", "session"],
    ),
    ...
    "security.read_grounding_evidence_unavailable": _setting(
        type="string",
        default="closed",
        allowed_values=["closed", "degraded_allow", "open"],
        description=(
            "What happens when the edit gate cannot READ the store that proves "
            "you looked at a file before editing it (DRT-07, backlog #893). "
            "closed (default): the edit is refused and the refusal names the "
            "underlying error — usually sqlite lock contention, so retrying "
            "normally works. degraded_allow: the edit proceeds but the verdict "
            "is stamped as NOT grounded. open: the old behaviour, where an "
            "unreadable evidence store counted as proof that you had read the "
            "file. Dashboard-only. (§6)"
        ),
        value_descriptions={
            "closed": "Refuse the edit and name the cause (default).",
            "degraded_allow": "Allow, stamped explicitly as not grounded.",
            "open": "Treat 'cannot check' as 'checked and fine'. Not advised.",
        },
        security_sensitive=True,
        dashboard_only=True,
        is_t0=True,
    ...
    "security.palace_axis_unavailable": _setting(
        type="string",
        default="closed",
        allowed_values=["closed", "degraded_allow", "open"],
        description=(
            "What happens when a palace IS configured but its blocking-anchor "
            "authority cannot answer — a half-wired anchor store, a failed "
            "mempalace import, or a consult that raised (DRT-08). Running with "
            "NO palace is unaffected: that is a supported setup and stays "
            "permissive. closed (default): governed edits are refused, because "
            "pinned blockers for the target are unknown rather than absent. "
            "degraded_allow: edits proceed with the verdict stamped as "
            "unverified. open: the old behaviour, where a broken palace meant "
            "the same as no palace. Dashboard-only. (§6)"
        ),
        value_descriptions={
            "closed": "Refuse governed edits and name the cause (default).",
            "degraded_allow": "Allow, stamped as palace-axis unverified.",
            "open": "A broken palace counts as no palace. Not advised.",
        },
        security_sensitive=True,
        dashboard_only=True,
        is_t0=True,
    ...
    "security.memory_ack_requires_read_receipt": _setting(
        type="boolean",
        default=True,
        description=(
            "Whether acknowledging anchored memory before an edit requires "
            "proof that the memory was actually READ (DRT-06). On (default): "
            "the ack must be backed by a server-minted receipt from "
            "memory_read, scoped to this agent and this memory epoch — "
            "retyping the route ids from the refusal is no longer enough. Off: "
            "the ack id list alone unblocks the edit, as it did before. "
            "Dashboard-only. (§6)"
        ),
        security_sensitive=True,
        dashboard_only=True,
    ...
    **{
    ...
    "observability.backlog_sync_stale_seconds": _setting(
        type="integer",
        default=900,
        description=(
            "Seconds since the last successful backlog sync after which the "
            "sync_receipt reports STALE and the sync alarm surfaces (#1063 "
            "liveness). Lower = earlier alarm on a stuck sync; higher = fewer "
            "false alarms on slow links."
        ),
        scope=["global", "project"],
    ),
