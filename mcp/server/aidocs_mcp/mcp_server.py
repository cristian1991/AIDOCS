...
def create_server(
    ...
    async def conductor_mode_enter(
        ...
        if not _host_sid and sid:
            try:
                # Direct accessor — gate.get() omits last_host_session_id.
                _host_sid = hub.query_gate.get_last_host_session_id(
                    project_root,
                    sid,
                )
            except Exception:
        ...
        if _host_sid:
            ...
            try:
                ...
                msg_register_role(project_root, _host_sid, "conductor")
            except Exception:
                pass
        try:
            hub.managed_mode.set_mode(
    ...
    async def coconductor_mode_enter(session_id: str = "") -> Any:
        project_root = _project_root()
        sid = session_id or _resolve_session_id(hub, project_root)
        from .mcp_server_runtime_helpers import current_calling_host_session_id
        ...
        if not _host_sid and sid:
            try:
                _host_sid = hub.query_gate.get_last_host_session_id(project_root, sid)
            except Exception:
                _host_sid = ""
        ...
        if _host_sid:
            ...
            try:
                ...
                msg_register_role(project_root, _host_sid, "co_conductor")
        ...
        try:
            _role = hub.skills.read_role("co-conductor")
            if _role and _role.get("content_text"):
                terse["role"] = _role["content_text"]
        except Exception:
            pass
    ...
    async def conductor_start(
        ...
        context_parts = [
            ...
            "You are the Empire's right hand — you WIN THE WAR. A General wins his own wars: drive "
            "the work end to end yourself (plan, execute, verify, seal). If the war stands on one "
            "front, you win it — you do not wait for the army. You also COMMAND the army: dispatch "
            "lane agents when the war splits into multiple parallel battlefields (dashboard, gates, "
            "nlp/user-intent, tools, outergate). Both are your job. The enemy is DUMB laziness — "
            "handing off or hand-waving what you should win yourself; smart laziness (future-proof "
            "seams) is the edge (120% §11: the conductor is not just task dispatch).",
            "",
            "== HOW YOU WORK ==",
            ...
            "3. ONE FRONT (the default): WIN IT YOURSELF, end to end — edit (ai_replace / "
            "ai_batch_edit), run (ai_test / ai_run), verify, seal. Do not hand off or hand-wave "
            "what you can finish.",
            "4. WAR SPLITS into multiple parallel battlefields: COMMAND the army — dispatch lanes "
            "through the SINGLE surface ai_lane(action=…): spawn to dispatch, guide to nudge a "
            "running worker, pause, resume a stalled worker (by lane), kill a runaway (by lane), "
            "review to decide a lane's completion",
    ...
    async def ai_seat(
        ...
        if m in ("co-enter", "co_enter", "coenter"):
            return await coconductor_mode_enter(session_id=session_id)
        if m == "exit":
    ...
    async def ai_version(mode: str = "local") -> Any:
        """Which build is this? Three truth-sources, pick with `mode`:
          local   (default) — the build ACTUALLY RUNNING on this process:
                    pyproject version + live git HEAD. On a dev box the signed
                    manifest lags real code (only `--deploy` rewrites it) while
                    the marker-poll watchdog has already reloaded newer code, so
                    this is the honest "what am I running right now".
          deploy  — what the last local `--deploy` sealed (.deploy-reports:
                    crown.reports-head commit + test tally).
          release — the signed release manifest (build_number bumped per deploy,
                    commit/version/created_at/builder) — the last BLESSED build.
        Benign, read-only; no org/project selection needed. Registered HERE in
        create_server (not the expose_deploy-gated register_deploy_tools) so
        surface=BOTH is honest — reachable on stdio AND the gate."""
        from . import build_info

        return build_info(mode)
    ...
    async def ai_agents(
        ...
        whose process has exited (for audit). For the lane-worker roster use
        ai_lane(action='agents'). No file changes.
        """
        from .agent_audit import connected_agents_audit

        return connected_agents_audit(
            _project_root(),
...
def main() -> None:
    ...
    if args.http:
        ...
        server.run(transport="http", host="127.0.0.1", port=int(args.port))
    else:
        server.run()
