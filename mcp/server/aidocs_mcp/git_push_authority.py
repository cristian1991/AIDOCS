...
def push_refusal(principal: dict | None) -> str:
    """'' when a GATE principal may push; else the refusal reason. Fail-closed."""
    ...
    if SCOPE_GIT_PUSH not in scope:
        return (
            f"token lacks {SCOPE_GIT_PUSH} scope — push is a dedicated capability, "
            "stricter than tier_m_edit (default for org owners/admins; granted to "
            "members only explicitly)"
        )
...
REASON_TOKEN_SCOPE = "token_scope"
...
def live_push_authority(
    ...
    *,
    ...
    static = push_refusal(principal)
    if static:
        return refuse(REASON_TOKEN_SCOPE, static)
    if not isinstance(principal, dict):  # unreachable: push_refusal refused it
        return refuse(REASON_TOKEN_SCOPE, "no principal")
