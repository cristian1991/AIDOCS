...
def read_global_law(
    law_id: str, *, include_retired: bool = False, follow_alias: bool = True
) -> dict | None:
    """Read one global law by id. Retired rows are hidden unless asked for.

    A legacy id (doctrine_global_law_seed.LEGACY_LAW_ID_ALIASES — the one
    canonical alias map) whose own row is not ACTIVE resolves to its stable
    successor; an active row under the legacy id (operator-owned) still wins."""
    lid = (law_id or "").strip()
    if not lid:
        return None
    if follow_alias:
        from .doctrine_global_law_seed import canonical_law_id

        new = canonical_law_id(lid)
        if new != lid:
            own = read_global_law(lid, include_retired=True, follow_alias=False)
            if own is None or own["status"] != _ACTIVE:
                moved = read_global_law(
                    new, include_retired=include_retired, follow_alias=False
                )
                if moved is not None:
                    return moved
            if own is not None and (include_retired or own["status"] == _ACTIVE):
                return own
            return None
    conn = _conn()
    try:
