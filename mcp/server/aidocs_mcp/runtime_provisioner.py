...
def _is_source_root(candidate: Path) -> bool:
    """Does ``candidate`` carry the SOURCE-CHECKOUT marker pair?

    ``<candidate>/pyproject.toml`` + ``<candidate>/server/aidocs_mcp/`` — i.e. the
    repo's ``mcp/`` dir. Extracted so the walk-up and the declared-authority
    fallback in ``local_source_root`` cannot drift apart: two markers checked two
    ways is how one of them eventually accepts a tree the other would refuse.
    """
    try:
...
def base_interpreter() -> str:
    """The DURABLE base interpreter a new generation must be created from.

    THE DEFECT THIS CLOSES (measured 2026-09-10, operator box). Provisioning runs
    inside the watchdog, under the LIVE generation's interpreter, and this used to
    be ``sys.executable``. CPython writes ``pyvenv.cfg`` from TWO DIFFERENT
    attributes, and only one of them is the runtime edge::

        home       <- sys._base_executable's prefix   the RUNTIME edge
        executable <- sys.executable                  the INVOKING PARENT
        command    <- sys.executable + the argv

    so each child recorded a SIBLING GENERATION as its invoking parent. The live
    generation's own file says it outright::

        home       = ...\\uv\\python\\cpython-3.13-windows-x86_64-none
        executable = ...\\generations\\gen-20260909T040257Z-7093a2c9ac\\venv\\Scripts\\pythonw.exe
        command    = ...same pythonw.exe... -m venv ...\\gen-20260910T033529Z-e2f3588f93\\venv

    11 of 13 generations on that box record an invoking parent that is not their
    runtime base; one records ``mcp/.venv``, a DEVELOPER WORKING TREE; and the
    recorded parent is reached through ``pythonw.exe``, a binary this estate
    blocks outright. ``_provision_venv`` then copies the same value into
    ``runtime.json``'s ``base_python``.

    The comment eighteen lines below the old assignment condemned it already:
    "Paying honestly for an independent tree beats seeding from the live one,
    which would embed the old venv's interpreter paths and make B quietly depend
    on A." The author guarded the dependency TREE and missed the metadata field.

    WHAT THIS IS NOT. It is NOT a fragility fix, and it must not be prioritised as
    if the estate were about to break. Measured: a child whose recorded
    ``executable``/``command`` name a parent that is then DELETED ENTIRELY still
    runs, with ``base_prefix`` and ``_base_executable`` resolving outside that
    parent. Nor is it a pruning fix — there is no generation GC to mislead
    (``_discard_generation`` discards only the generation its caller just built,
    by path) and nothing in the server package reads ``pyvenv.cfg`` at all. This
    is PROVENANCE CORRECTNESS, so that a prover, health check or replay consumer
    reading those fields is never handed a dependency that does not exist.

    ``sys._base_executable`` is the ONE attribute that already names the real base
    from inside a venv (measured on the live generation: the uv cpython, outside
    ``generations/``). It is private, so this never trusts it blindly: a missing,
    empty or DANGLING value degrades to ``sys.executable`` — the previous
    behaviour, weaker provenance, never a failed provision. That fallback is
    deliberate and is not an error path: refusing to provision would brick the
    update path for the box, which is strictly worse than an honestly weak base.
    """
    cand = (getattr(sys, "_base_executable", "") or "").strip()
    if cand:
        try:
...
def local_source_root() -> Path | None:
    ...
    for candidate in list(pkg.parents)[1:4]:
        if _is_source_root(candidate):
            return candidate
    # THE WALK-UP ALONE WENT PERMANENTLY BLIND AT #1030, and that blindness is
    # why five bodies of green work were invisible to the running server all day
    # (measured 2026-09-10). #1030 moved the RUNNING package out of the checkout
    # and into `~/.aidocs/runtime/generations/<id>/venv/Lib/site-packages`, and no
    # ancestor of a site-packages path is ever a checkout — so this returned None
    # on a box that demonstrably HAS one. `freshness_report` then selected
    # AXIS_RELEASE by construction, `ai_version` reported
    # `runtime_fresh: null` / "not a source machine?", and NOTHING IN THE ESTATE
    # COULD OBSERVE LOCAL SOURCE DRIFT. That is the two-axis trap at its worst:
    # one axis says "current", the other says "cannot tell", and the detector
    # consults the wrong one.
    #
    # The fix is an EXPLICIT, DURABLE authority rather than a cleverer search.
    # AIDOCS_PATH is already that lever — honored as the source root by
    # host_services/path_resolver_service.py and by
    # runtime_project_support_service.py — and it is already set on the operator
    # box, so the axis becomes answerable with nothing invented.
    #
    # IT IS VALIDATED, NOT TRUSTED. A declared path that does not carry the
    # marker pair yields None, exactly as before: fabricating a source root would
    # make `runtime_freshness` fingerprint a tree that is not the law and report
    # the verdict as authoritative — a fail-green strictly worse than blindness.
    # Unknown stays unknown. And the walk-up is tried FIRST, so the package's own
    # location (stronger evidence) always outranks a stale env var.
    declared = (os.environ.get("AIDOCS_PATH") or "").strip()
    if declared:
        try:
            root = Path(declared).resolve()
        except OSError:
            return None
        # Operators point this at the REPO ROOT; accepting the `mcp/` dir itself
        # costs nothing and is gated by the same markers.
        for candidate in (root / "mcp", root):
            if _is_source_root(candidate):
                return candidate
    return None
...
def _stamp_provenance_current(src: Path, inst: Path) -> tuple[bool, str]:
    ...
    try:
        # The ONE audited git helper (Article VI) — already fingerprinted for
...
def runtime_freshness(
    *,
    ...
    try:
        ...
        # differ.
        # ── AND THE PACKAGED VENDORED ENGINE (#925) ───────────────────────
        #
        # `_vendor/mempalace` is staged INTO the package at release build time so
        # the artefact can install its own engine, and it is covered by the
        # RELEASE SIGNATURE because packaging writes it before the manifest is
        # fingerprinted. A source checkout never carries it — exactly the shape
        # STAMP_REL already has, for exactly the same reason.
        #
        # So it must be excluded HERE, on both sides, or fixing #925 would
        # manufacture permanent drift: every installed runtime would carry a tree
        # its source reference lacks, `fresh` would be False forever, and the
        # refresh loop would reinstall on every tick trying to reach a parity it
        # can never have. That is the failure the stamp-in-digest rule already
        # produced once (dev-1236, rc=5 "still stale after runtime --fix").
        #
        # EXCLUDING IT HERE DOES NOT UNSIGN IT. This is the FRESHNESS digest,
        # which asks "does the CODE differ"; the SIGNATURE digest is a separate
        # call with no exclusions, and vendored bytes are third-party code this
        # axis was never asking about.
        excl: tuple[str, ...] = (STAMP_REL, _PACKAGED_VENDOR_REL + "/")
...
#: Where a release carries the palace engine INSIDE the signed package (#925),
#: POSIX-relative to the aidocs_mcp package dir. One spelling, shared by the
#: builder that writes it, the freshness axis that must ignore it, and
#: ``aidocs_mcp._package_vendor_root`` that imports from it — three places that
#: silently disagreeing would each look correct on its own.
_PACKAGED_VENDOR_REL = "_vendor"


def stage_packaged_vendor(pkg_dir: Path | str, engine: Path | str) -> Path:
    """Copy the signed engine into ``<package>/_vendor/mempalace`` (#925).

    CALLED BY THE RELEASE BUILDER, BEFORE THE MANIFEST IS FINGERPRINTED — which
    is the entire reason this is safe. `compute_package_fingerprint` digests
    everything under the package, so an engine written here is covered by the
    RELEASE SIGNATURE. Written afterwards it would be unsigned bytes riding
    beside signed ones, which is the supply-chain hole #925 refuses, and the
    vendored CLAIM cannot be repointed to cover it without SIG_MISMATCHing every
    already-signed release.

    Idempotent: the destination is removed first, so a rebuild over a dirty tree
    cannot leave a stale module behind to be signed and shipped.
    """
    import shutil

    dest = Path(pkg_dir) / _PACKAGED_VENDOR_REL / "mempalace"
    if dest.exists():
        shutil.rmtree(dest)
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copytree(
        Path(engine), dest, ignore=shutil.ignore_patterns("__pycache__", "*.pyc")
    )
    return dest


def packaged_vendor_digest(pkg_dir: Path | str) -> str | None:
    """Fingerprint of the engine carried INSIDE the package, or None."""
    from .package_integrity import compute_package_fingerprint

    tree = Path(pkg_dir) / _PACKAGED_VENDOR_REL / "mempalace"
    if not (tree / "__init__.py").is_file():
        return None
    return str(compute_package_fingerprint(tree, version="mempalace")["fingerprint"])


def packaged_vendor_matches_claim(pkg_dir: Path | str, engine: Path | str) -> dict:
    """Is the PACKAGED engine byte-equivalent to the CANONICAL claimed tree?

    THE BOOTSTRAP COPY MUST NOT BECOME A SECOND SOURCE OF TRUTH. `_vendor` is a
    copy of `third_party/mempalace/mempalace`, and the whole reason it is safe
    is that it carries the SAME BYTES the vendored claim already vouches for.
    A copy that drifted — staged from a stale tree, or edited after staging —
    would ship an engine nobody claimed, under a signature that covers it: the
    signature would be honest and the PROVENANCE would be a lie.

    Deliberately computed with `compute_package_fingerprint(..., version=
    "mempalace")` — the SAME primitive and the SAME version salt
    `compute_vendored_fingerprints` uses for this tree. So equality here means
    equality under the exact digest the claim is made with, not under a second
    hashing scheme that could agree by luck and diverge later.

    LEGACY CLAIM SEMANTICS ARE UNTOUCHED: this compares two trees on the
    builder's disk. It adds no manifest key, changes no VENDORED_TREES entry,
    and is invisible to every client — old or new — reading a signed release.

    Returns {ok, packaged, canonical, reason}. ok=False on ANY doubt, including
    an unreadable tree: an equivalence that cannot be computed has not been
    demonstrated.
    """
    from .package_integrity import compute_package_fingerprint

    out: dict = {"ok": False, "packaged": None, "canonical": None, "reason": ""}
    try:
        packaged = packaged_vendor_digest(pkg_dir)
        src = Path(engine)
        canonical = (
            str(compute_package_fingerprint(src, version="mempalace")["fingerprint"])
            if (src / "__init__.py").is_file()
            else None
        )
    except Exception as exc:  # noqa: BLE001 — uncomputable is not equivalent
        out["reason"] = f"could not fingerprint both trees: {type(exc).__name__}"
        return out

    out["packaged"] = packaged
    out["canonical"] = canonical
    if packaged is None:
        out["reason"] = "no packaged engine at <package>/_vendor/mempalace"
        return out
    if canonical is None:
        out["reason"] = "no canonical vendored tree to compare against"
        return out
    if packaged != canonical:
        out["reason"] = (
            "packaged engine differs from the canonical vendored tree — the "
            "release would ship bytes the vendored claim does not vouch for"
        )
        return out
    out["ok"] = True
    return out
...
def signed_mempalace_package(pkg_target: str | None) -> Path | None:
    """The SIGNED importable ``mempalace/`` package inside a pulled release.

    `vendored_mempalace_dir` above answers the CHECKOUT posture and returns None
    for everything else. That "everything else" includes the posture every
    operator box actually updates through, and it is why a released build could
    never install itself (#913/#925):

      * SHAPE. ``VENDORED_TREES`` signs ``third_party/mempalace/mempalace`` —
        the IMPORTABLE PACKAGE ALONE, deliberately, so the fingerprint covers
        exactly the code that gets imported. There is no pyproject.toml in the
        artefact because there is no project in the claim.
      * PATH. The checkout resolver probes ``<project>/../third_party`` (right
        for ``<repo>/mcp`` beside ``<repo>/third_party``). In a pulled release
        the tree sits INSIDE the project root, beside pyproject.toml.

    Both layouts are accepted here, by EVIDENCE rather than by guessing which
    one we are on — the same accommodation `vendored_contract._release_root_for`
    already makes, and bounded to those two candidates so this can never wander
    into a parent that merely happens to contain a third_party/.

    Returns the package dir (…/third_party/mempalace/mempalace) or None.
    """
    if not pkg_target:
        return None
    try:
        ...
        for base in (root, root.parent):
            pkg = base / "third_party" / "mempalace" / "mempalace"
...
def stage_vendored_mempalace_project(pkg_dir: Path, staging_root: Path) -> Path:
    """Wrap the signed package in generated metadata so pip can install it.

    THE METADATA IS GENERATED ON THE CLIENT, NEVER SHIPPED. That is the whole
    design, and it is what keeps this out of #925's trap: adding a pyproject to
    the SERVED artefact would either change what ``VENDORED_TREES`` covers — so
    a client recomputes the digest over a different directory, gets
    SIG_MISMATCH, refuses the whole artefact and can then never update again —
    or ride along UNCLAIMED beside a claimed tree, which is the supply-chain
    hole the vendored claim exists to close. Generating it here transports
    nothing new, so there is nothing new to sign and no legacy claim changes
    meaning.

    NOR IS ANYTHING BORROWED FROM THE SERVING GENERATION. The bytes come from
    the pulled artefact; only the ~40 bytes of packaging boilerplate are
    synthesized, from a template in this file.

    THE VERSION IS THE TRUST IDENTITY. There is no version to read — the
    artefact carries no project metadata — so rather than invent one or hardcode
    a stale literal, the dist version is derived from the tree's own
    fingerprint, computed with the SAME audited primitive the vendored claim
    uses (`compute_package_fingerprint`). Two consequences, both wanted:
    a changed engine always changes the version (the `--force-reinstall`
    comment below exists precisely because "the vendored tree's version rarely
    bumps"), and `pip list` names the exact bytes installed. It also answers
    #925's objection to installing without pip — there IS dist-info, so the
    install is queryable and uninstallable like any other.

    Returns the staged project directory, ready for ``pip install``.
    """
    import shutil
    ...
    fingerprint = str(
        compute_package_fingerprint(pkg_dir, version="mempalace")["fingerprint"]
    )
    local = "".join(ch for ch in fingerprint if ch.isalnum())[-12:] or "unknown"

    project = Path(staging_root) / "vendored-mempalace"
    if project.exists():
        shutil.rmtree(project, ignore_errors=True)
    (project / "mempalace").parent.mkdir(parents=True, exist_ok=True)
    shutil.copytree(pkg_dir, project / "mempalace")
    (project / "pyproject.toml").write_text(
        ...
        "[project]\n"
        'name = "mempalace"\n'
        f'version = "0+vendored.{local}"\n'
        'description = "Vendored palace engine, staged from a signed release."\n'
        "\n"
        "[tool.setuptools]\n"
        'packages = ["mempalace"]\n'
        "\n"
...
def vendored_mempalace_install_target(
    pkg_target: str | None,
    staging_root: Path | str | None,
) -> Path | None:
    """The ONE thing to ``pip install`` for the palace engine, either posture.

    Checkout first, unchanged, so the legacy lane keeps its exact behaviour and
    the tests that pin it (test_mempalace_installed_posture_733) still describe
    reality. Only when that answers None — i.e. the pulled-release posture that
    previously fell straight through to "install nothing" — is a project staged
    from the signed package.

    None still means "there is genuinely no vendored tree here", which
    `verify_interpreter` then catches as mempalace_unresolvable. This narrows
    that verdict to the case it was always meant for; it does not silence it.
    """
    direct = vendored_mempalace_dir(pkg_target)
    if direct is not None:
        return direct
    if not staging_root:
        return None
    pkg = signed_mempalace_package(pkg_target)
    if pkg is None:
        return None
    try:
        return stage_vendored_mempalace_project(pkg, Path(staging_root))
...
_VERIFY_SNIPPET = (
    ...
    "except Exception:\n"
    "    v = None\n"
    # RESOLUTION IS NOT EXECUTION, and the first cut of this check FELL FOR THAT
    # EXACT TRAP while naming it. `require_mempalace()` returns early on
    # `_MEMPALACE_RESOLUTION is not None` — it is a FLAG READ, set at import by
    # the wiring, and it never imports the engine. A candidate whose
    # mempalace/__init__.py raises resolves fine, sets the flag, passes the
    # guard, and dies at `import mempalace` one line later — after activation.
    #
    # So EXECUTE THE SEAM `create_server` executes, in the order it uses it
    # (mcp_server.py:3553-3571): the guard, then the hard `import mempalace`,
    # then the three palace modules whose import failure is the #910 silent
    # death — hub.palace becomes None and the edit gate stops evaluating its
    # palace blockers while still answering allowed=True.
    #
    # `warm_palace_embedder` is NOT called: create_server only schedules it,
    # and running an ONNX load inside a 25s probe would trade a real check for
    # a timeout. Importing these modules executes their module-level code,
    # which is the part that actually breaks.
    #
    # BaseException, not Exception: a candidate that dies on SystemExit or a
    # recursion/memory error during import is no more activatable than one that
    # raises ValueError, and the narrower catch would let it through as ok.
    "boots = None\n"
    "try:\n"
    "    m.require_mempalace()\n"
    "    import mempalace\n"
    "    import aidocs_mcp.palace_hub_extension\n"
    "    import aidocs_mcp.server_palace_tools\n"
    "    import aidocs_mcp.server_recall_tools\n"
    "    boots = True\n"
    "except BaseException as exc:\n"
    "    boots = 'startup:' + type(exc).__name__ + ':' + str(exc)[:160]\n"
...
def verify_interpreter(
    ...
    *,
    ...
    # DOES IT ACTUALLY BOOT — the question `mempalace` above cannot answer.
    #
    # That check is `find_spec`, which by design never executes module code, so
    # a runtime whose engine RESOLVES but EXPLODES on import verified clean and
    # died at startup — after the swap. The candidate now EXECUTES the seam
    # `create_server` executes: the guard, the hard `import mempalace`, and the
    # three palace modules whose import failure is #910's silent death.
    # Measured 2.1s against a 25s budget on a healthy runtime.
    #
    # `.get(..., True)` for the same reason the line above uses it: a probe from
    # an older package predates the field, and a runtime must not be refused for
    # failing to answer a question its build never knew about. Only an explicit
    # non-True answer — the guard actually raised — is a refusal.
    _boots = payload.get("boots", True)
    if _boots is not True:
        return {
            "ok": False,
            "imports": True,
            "version": version,
            "reason": f"does_not_boot: the candidate resolved the palace engine but the "
            f"server's own startup guard refused it ({_boots}) — activating this "
            f"generation would swap in a runtime that cannot start (#925)",
        }
...
def _provision_venv(
    ...
    *,
    ...
    # AN EXPLICIT BASE, NEVER THE AMBIENT ONE. `base_python` is the operator's
    # `--base-python` and outranks everything. Otherwise `base_interpreter()`
    # resolves the DURABLE base rather than `sys.executable`, which inside the
    # watchdog IS the live generation — see that function for the measured
    # pyvenv.cfg contamination this removes. `src` is also what lands in
    # runtime.json's `base_python` below, so this is the same fix twice.
    src = base_python or base_interpreter()
    ...
    # Staged INSIDE the generation being built, so a failure discards it with
    # the generation and nothing leaks into the serving one (#1030).
    vendor = vendored_mempalace_install_target(target, gen_root)
    ...
    try:
        ...
        try:
            if installer is not None:
                ...
                vendor = vendored_mempalace_install_target(pkg, staging)
                if vendor is not None:
                    code, out, err = (runner or (lambda a: _default_runner(a)))(
                        [
                            staged_py,
