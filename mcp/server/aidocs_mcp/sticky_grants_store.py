...
class StickyGrant:
    ...
    # #99 FIX2: agent_memory_epoch captured at registration. None on legacy
    # rows (pre-migration) — treated as non-matching under epoch-binding.
    registered_epoch: str | None = None
...
class StickyGrantsStore(SQLiteIndexStoreBase):
    ...
    def init_db(self, project_root: Path) -> None:
        ...
        # hook event. Same shape as SessionQueryGateStore's _schema_ready memo,
        # hoisted to the shared helper so every store spells it once. The memo
        # re-verifies the file exists, so a deleted DB is rebuilt.
        _db = self.db_path(project_root)
        if _schema_already_ensured(_db, "sticky_grants"):
            return
        with self.session(project_root) as conn:
            ...
            # #99 FIX2: agent_memory_epoch binding column (idempotent migration
            # for DBs created before the feature — legacy rows get NULL, which
            # reads as non-matching under epoch-binding = fail toward clearing).
            self._ensure_column(conn, "sticky_grants", "registered_epoch", "TEXT")
        _mark_schema_ensured(_db, "sticky_grants")
    ...
    # ── #99 FIX2: agent_memory_epoch binding ──────────────────────────────
    #
    # A sticky grant demonstrated once must NOT ride silently across an
    # unbounded number of compactions (the "sticky-as-backdoor" regression,
    # Empire report 2026-05-01). Binding the grant to the agent_memory_epoch at
    # registration means a compaction (which rotates the epoch) clears it from
    # the ENFORCEMENT surface — the operator re-grants intentionally in the new
    # conversation context. Enforcement only; audit/display stays unfiltered.

    def _resolve_epoch(self, project_root: Path) -> str:
        """Best-effort current agent_memory_epoch for the calling host session.
        Mirrors ExecutionIndexStore._resolve_audit_epoch. Returns "" when no
        host session resolves (fail-open at the enforcement site)."""
        # Resolved by the ONE authority (#525/#539) rather than re-deriving the
        # ladder locally.
        from .agent_memory_epoch import resolve_epoch

        return resolve_epoch(project_root) or ""

    def _sticky_epoch_bound(self, project_root: Path) -> bool:
        """Whether sticky grants are epoch-bound. Default TRUE (Empire preference /
        AIDOCS doctrine — compaction should clear demonstrated intent). Operators
        opt out (security.sticky_epoch_bound=false) for persist-until-revoke."""
        try:
            from .config import _parse_bool, get_setting

            return _parse_bool(
                get_setting(
                    "security.sticky_epoch_bound", project_root=project_root, default=True
                ),
                default=True,
            )
        except Exception:
            # Fail toward the SECURE default (bound) if the setting can't be read.
            return True

    def _epoch_enforce(
        self, project_root: Path, grants: list[StickyGrant]
    ) -> list[StickyGrant]:
        """Drop grants whose registered_epoch != the current epoch when
        epoch-binding is on AND a current epoch resolves. Fail-open when the
        epoch is unresolvable ("") so a transiently-absent host session never
        silently strips a legitimate grant (user-access safeguard).

        SYMMETRIC fail-open (#377, 2026-07-14): a grant whose epoch was
        unresolvable AT REGISTRATION (registered_epoch == "") is also kept.
        Dropping it would permanently brick a legitimately-registered grant
        the moment any later turn resolves an epoch — the exact silent-strip
        the enforcement-side fail-open exists to prevent. Grants registered
        through the prompt-submit pipeline always carry a real epoch (the
        transaction binds request identity before the grant stages run), so
        epoch-rotation clearing (sticky-as-backdoor closure, #99 FIX2) is
        unaffected for them."""
        if not self._sticky_epoch_bound(project_root):
            return grants
        current = self._resolve_epoch(project_root)
        if not current:
            return grants  # unresolvable → cannot bind → keep (fail open)
        return [g for g in grants if (g.registered_epoch or "") in ("", current)]
    ...
    def register_grant(
        ...
        *,
        ...
        # PROVENANCE (was TODO(provenance, 2026-07-23), now threaded). A grant is
        # a CONSENT RECORD: `phrase` and `registered_by_user_id` are the only
        # trace of WHO consented and WITH WHAT WORDS. Every row in a live install
        # had neither, because query_gate._write_sticky hardcoded both to "" on
        # every path — operator-phrase grants included. _write_sticky now accepts
        # and forwards them, so a caller holding the operator's words records them.
        #
        # Not validated HERE, deliberately: this store does not second-guess the
        # judge (see the docstring), and the restore paths
        # (prompt_submit_rollback / snapshot_restore) re-materialise earlier
        # grants rather than capturing a fresh consent. Absence of provenance is
        # queryable on the facts — empty phrase AND empty user — so it needs no
        # marker; nothing is stamped "legacy", which would dress a defect up as
        # an accepted class.
        self.init_db(project_root)
        # DEDUP AGAINST ACTIVE GRANTS. _write_sticky advertises itself as
        # "idempotent by design", but that dedup was read-side only: the grant_id
        # is time-seeded (see _gen_grant_id), so every re-registration minted a
        # new primary key and the table accumulated duplicates behind clean-looking
        # reads. A live install held 19 active grants covering 17 tools.
        #
        # This matters beyond tidiness: revoke_grant() revokes ONE grant_id, so a
        # duplicate survives it and the tool stays authorised after a revocation
        # that reports success. (revoke_tool() sweeps all rows and was unaffected.)
        #
        # Scoped to (session, tool, subcommand) because that triple IS the grant's
        # identity: consent is per-session, and a tier-2 grant authorises one
        # subcommand. Only ACTIVE rows suppress a write — re-granting after a
        # revoke is a genuinely new consent decision and still mints a new row,
        # which is why the id is time-seeded in the first place.
        _norm_tool = tool.strip()
        _norm_sub = subcommand.strip() if subcommand else None
        with self.session(project_root) as conn:
            existing = conn.execute(
                "SELECT grant_id FROM sticky_grants "
                "WHERE session_id = ? AND tool = ? AND revoked_at IS NULL "
                "AND ((subcommand IS NULL AND ? IS NULL) OR subcommand = ?) "
                "ORDER BY registered_at LIMIT 1",
                (session_id, _norm_tool, _norm_sub, _norm_sub),
            ).fetchone()
        if existing is not None:
            # Return the EXISTING id so callers keep a usable handle rather than
            # an id for a row that was never written.
            return str(existing["grant_id"])
        grant_id = self._gen_grant_id(session_id, tool, subcommand)
        registered_at = self._now()
        # #99 FIX2: stamp the current agent_memory_epoch so the grant can be
        # cleared from the enforcement surface once the epoch rotates.
        registered_epoch = self._resolve_epoch(project_root)
        with self.session(project_root) as conn:
            conn.execute(
                "INSERT INTO sticky_grants (grant_id, session_id, tier, tool, "
                "subcommand, phrase, registered_at, registered_by_user_id, "
                "judge_verdict, confirmation_answer, registered_epoch) VALUES "
                "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    grant_id,
                    session_id,
    ...
    def active_grants_for_session(
        ...
        with self.session(project_root) as conn:
            rows = conn.execute(
                ...
                "revoked_reason, registered_epoch FROM sticky_grants "
                "WHERE session_id = ? "
    ...
    def active_tools_for_session(
        ...
        """
        grants = self._epoch_enforce(
            project_root, self.active_grants_for_session(project_root, session_id)
        )
    ...
    def snapshot_prompt_submit_state(
        ...
        *,
        ...
        self.init_db(project_root)
        scopes = {
            "sticky_grants": ("session_id = ?", (session_id,)),
            "sticky_grants_pending": ("session_id = ?", (session_id,)),
        }
        with self.session(project_root) as conn:
    ...
    def _row_to_grant(row: Any) -> StickyGrant:
        return StickyGrant(
            ...
            registered_epoch=(str(row["registered_epoch"]) if row["registered_epoch"] else None),
        )
