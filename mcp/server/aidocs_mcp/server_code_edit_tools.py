...
from .path_trust_zone import PathTrustZone
...
def _sync_body(registered: Any) -> Any:
    """The undecorated SYNC body behind a `@server.tool`-registered name.

    For one tool calling a sibling tool in-process. The registered binding
    may be an async wrapper (the universal cancel-safety offload makes every
    sync tool one), and calling it from sync code yields an unawaited
    coroutine instead of a result. Refuses loudly rather than hand back a
    coroutine if the body itself is async -- that caller must await it.
    """
    body = inspect.unwrap(registered)
    if inspect.iscoroutinefunction(body):
        raise TypeError(
            f"{getattr(body, '__qualname__', body)!r} is async; a sync tool cannot "
            "delegate to it by direct call"
        )
    return body


# #78 "wrote it = read it": these tools carry FILE-LEVEL read evidence
# because writing a file proves the agent had its content loaded. The claim
# only holds for a write that LANDED - see _applied_edit_paths.
_EDIT_EVIDENCE_TOOLS = frozenset(
    {
        "ai_str_replace",
        "ai_anchor_replace",
        "ai_edit_lines",
...
def _applied_edit_paths(events: list[Any]) -> set[str]:
    """Paths a write ACTUALLY LANDED on, per the post-edit audit rows.

    #893, MEASURED 2026-08-26 on the live project and then reproduced in
    tests/security/test_edit_must_have_read.py. The audit middleware writes a
    ``tool_call_completed`` row for EVERY call that returns - refusals and
    engine failures included - and that row carries the canonical read-evidence
    stamp (``evidence`` = full_file for an edit tool). So an edit REFUSED for
    lack of grounding minted, as its own side effect, the exact evidence the
    gate looks for, and the identical retry passed. Refuse once, call again,
    through - on every editor, not only the ungated batch ones.

    ``tool_edit_completed`` / ``status='applied'`` is the honest signal: it is
    emitted by ``post_edit_reindex_and_grant``, which runs only after the edit
    engine reports success. Requiring the two to agree keeps #78 intact for
    real writes and removes the self-satisfying rung.

    Read from the SAME event list the grounding query already fetched - the
    row's payload names the path, so it matches that path-substring query.
    """
    applied: set[str] = set()
    for ev in events:
        if not isinstance(ev, dict):
            continue
        if str(ev.get("event_kind") or "") != "tool_edit_completed":
            continue
        if str(ev.get("status") or "").lower() != "applied":
            continue
        ...
        if not raw:
            raw = str(ev.get("target_entity") or "")
        if raw:
            applied.add(raw.replace("\\", "/").lstrip("/"))
    return applied



def _evidence_filed_under(hub, project_root, target: str, checked: str) -> str:
    """Name the session(s) whose events DO carry read evidence for ``target``.

    The grounding query filters by session_id. When the RECORDER and the
    READER disagree about which session is current, the refusal becomes
    unexplainable: the agent did read the file, was told it had not, and
    re-reading cannot help because the new read is filed under the same
    wrong session. Law 311bf3e6 -- "read it first" is not a reachable
    remedy under those conditions, so the refusal must say WHICH session
    holds the evidence.

    Reported live 2026-08-26: two agents hit this and neither could name a
    cause, because the message that exists to name it (#893) shipped with
    an EMPTY f-string interpolation -- prose promising a value it did not
    carry. Diagnostic only; never widens what counts as grounded.
    """
    try:
        events = hub.execution.list_events(project_root, query=target, limit=200)
    except Exception:  # noqa: BLE001 - diagnostics must never mask the refusal
        return "unknown: the unfiltered evidence query failed too"
    rows = [ev for ev in events if isinstance(ev, dict)]
    if not rows:
        return "nowhere: no session recorded a read of this path"
    seen = {str(ev.get("session_id") or "").strip() for ev in rows}
    unattributed = "" in seen
    seen.discard("")
    others = sorted(seen - {checked})
    if others:
        return ", ".join(f"`{s}`" for s in others)
    if checked in seen:
        # #930: "nowhere" used to swallow this case, because the old set
        # discarded `checked` before testing for emptiness. Events under the
        # checked session DO exist -- so the honest answer is that none of
        # them qualifies, not that nothing was recorded. Saying "nowhere"
        # here sends the reader hunting for a broken recorder that is
        # working fine.
        return (
            f"`{checked}` itself — events for this path exist under the session "
            f"being checked, but none qualifies as read evidence for this edit "
            f"(wrong tool, uncovered line range, or a refused/failed call)"
        )
    if unattributed:
        # #930: every row for this path was recorded WITHOUT a session. That
        # is precisely what the recorder does when managed mode is bound to a
        # session that is not a real member -- the split-brain seal in
        # mcp_server._real_instrumented_call_tool nulls the attribution on a
        # STALE BIND. The gate still queries by the bound id, so no read can
        # ever match and re-reading cannot help. Law 311bf3e6: name the
        # condition, because "read it first" is not a reachable remedy here.
        return (
            f"NO SESSION AT ALL — the reads were recorded but stamped "
            f"UNATTRIBUTED, which is what the recorder does when managed mode "
            f"is bound to a session that does not exist (a stale bind on "
            f"`{checked}`). Re-binding to a real session is the fix; "
            f"re-reading is not"
        )
    return "nowhere: no session recorded a read of this path"
...
#: DRT-07 policy key. The read-grounding evidence store is the project sqlite;
#: the failure this is about is overwhelmingly TRANSIENT (79 `database is
#: locked` errors measured in one day of concurrent lanes), which is why the
#: default refusal names a RETRY rather than an operator repair -- and why an
#: operator who is stuck can still set `degraded_allow` and keep working with a
#: stamped, non-grounded verdict instead of being bricked.
_GROUNDING_POLICY_KEY = "security.read_grounding_evidence_unavailable"


def _grounding_unavailable_verdict(
    project_root: Path,
    *,
    target: str,
    cause: BaseException,
    ...
    """DRT-07 — "COULD NOT CHECK" IS NOT "CHECKED AND CLEAN".

    Both grounding helpers used to end an evidence-store failure with

        except Exception:
            return True, "events-unavailable"

    i.e. "I cannot determine whether this agent read the target" spelled as
    "the edit is grounded", on an ENFORCEMENT check, with a reason string the
    callers never looked at. Backlog #893 has carried it as open/critical.

    The replacement refuses by default and the reason carries the marker, the
    exception type and message, and the route -- so the refusal can never be
    mistaken for a grounded pass (``evidence_authority.is_unavailable_reason``)
    and never becomes an unattributed "no".
    """
    ...
    policy = resolve_unavailable_policy(
        _GROUNDING_POLICY_KEY, project_root, default=POLICY_CLOSED
    )
    reason = format_unavailable_reason(
        axis=f"read_before_edit:{target}",
        cause=cause,
        recovery=(
            f"The evidence store is the project sqlite and this failure is "
            f"usually transient (lock contention): RETRY the edit. If it "
            f"persists, {retry_hint} and check the execution_events store; an "
            f"operator can set {_GROUNDING_POLICY_KEY}=degraded_allow to "
            f"proceed with an explicitly UNGROUNDED verdict."
        ),
        policy=policy,
    )
    if policy in (POLICY_DEGRADED, POLICY_OPEN):
        # Allowed, but the reason still carries the marker. A caller that logs
        # or renders the reason -- or asks is_unavailable_reason -- can tell
        # this from "file was read this session". POLICY_OPEN is byte-for-byte
        # the old behaviour except that it is now a CHOICE with a name.
        return True, reason
    return False, reason


def _grounding_refusal_structured(reason: str, path: str) -> dict[str, Any]:
    """Structured verdict for a read-before-edit refusal.

    The MACHINE-READABLE half of DRT-07. The three single-edit doors used to
    emit `{ok, error, path}` only, so a host that renders structured_content
    (or any programmatic caller) could not tell "you never read this file" from
    "we could not find out whether you read this file" -- the second needs a
    RETRY, the first needs a read. `is_unavailable_reason` is the single
    authority on which one this is; the token is derived from it, never typed
    twice.
    """
    from .evidence_authority import is_unavailable_reason

    blocked_by = (
        "evidence_authority_unavailable"
        if is_unavailable_reason(reason)
        else "edit_not_grounded"
    )
    return {"ok": False, "error": reason, "path": path, "blocked_by": blocked_by}
...
def register_code_edit_tools(
    *,
    ...
    def _file_was_read(project_root: Path, path: str) -> tuple[bool, str]:
        ...
        session_id, _mm_reason = explain_managed_session(hub.managed_mode, project_root)
        if not session_id:
            # Keep the two reasons distinct: "the binding names no usable
            # session" is not the same story as "this project is unmanaged".
            names_no_session = _mm_reason.startswith("stale_bind:") or (
                _mm_reason == "managed_binding_names_no_session"
            )
            return True, ("no-session" if names_no_session else "unmanaged")
        ...
        except Exception as exc:  # noqa: BLE001 - DRT-07: unknown is not a pass
            return _grounding_unavailable_verdict(
                project_root,
                target=target,
                cause=exc,
                retry_hint="the file-level grounding query could not run",
            )
        # #893: which of these paths did a write actually LAND on. An edit
        # tool's completion row is written even when the edit was refused.
        applied_paths = _applied_edit_paths(events)
        _READ_TOOLS = {
            "ai_get_lines",
        ...
        for ev in events:
            ...
            if ev_path != target:
                continue
            # #893: an edit tool's completion row is written even when the
            # edit was REFUSED or matched nothing, so it proves a read only
            # when a write actually landed on this path.
            if (
                str(ev.get("capability_name") or "") in _EDIT_EVIDENCE_TOOLS
                and ev_path not in applied_paths
            ):
                continue
            return True, "file was read this session"
        # Name the session the evidence was filed under. Law 311bf3e6: the
        # remedy has to be reachable, and "read it first" is unreachable
        # advice when the reader and the recorder disagree about WHICH
        # session that is -- saying which one turns an unexplainable refusal
        # into a diagnosable one.
        return False, (
            f"edit not grounded: no read of `{target}` in session `{session_id}` "
            f"(the session read evidence is filed under "
            f"{_evidence_filed_under(hub, project_root, target, session_id)}). "
            f"Read it first — "
    ...
    def _lines_were_read(project_root: Path, path: str, start: int, end: int) -> tuple[bool, str]:
        ...
        try:
            ...
            return _grounding_unavailable_verdict(
                project_root,
                target=f"{target}:L{start}-L{end}",
                cause=exc,
                retry_hint="the line-range grounding query could not run",
            )
        # #893: see _applied_edit_paths -- a refused edit is not a read.
        applied_paths = _applied_edit_paths(events)
        ...
        for ev in events:
            ...
            # #893: an edit tool's completion row is written even for a REFUSED
            # or failed edit, so it grounds only when a write actually landed.
            if (
                cap in _FALLBACK_READ_TOOLS
                and evidence_is_file_level(evidence)
                and not (cap in _EDIT_EVIDENCE_TOOLS and target not in applied_paths)
            ):
                if fresh is True:
                    file_fresh = True
                elif stamped:
        ...
        return False, (
            f"no read covers `{target}` L{start}-L{end} in session `{session_id}` "
            f"(the session read evidence is filed under "
            f"{_evidence_filed_under(hub, project_root, target, session_id)}). "
            f"Read it first — "
            f"ai_get_lines(path='{target}', start_line={start}, count={end - start + 1})."
    ...
    def _check_batch_grounded(
        ...
        *,
        ...
        """Read-before-edit for the BATCH editors -- PER EDIT PATH (#893).

        ai_replace / ai_insert_lines refused an ungrounded edit while
        ai_batch_edit performed the identical edit on the identical file,
        first try. A rule one governed editor enforces and another does not
        is advisory: any agent that met a grounding refusal could satisfy the
        same intent by switching tools, and nothing in either tool's output
        said a rule had been skipped.

        PER PATH, not per call -- a batch that mixes a read file with an
        unread one must not carry the unread one through on the read one's
        evidence. Batches are atomic, so one ungrounded edit refuses the
        whole batch and nothing is written.

        Line edits get the RANGE check (_lines_were_read), the same rule
        ai_replace(mode='lines') applies; string edits get the file-level
        check (_file_was_read), the same rule ai_replace(mode='string')
        applies. The batch door gets the single-edit door's rule, not a
        weaker batch-only one.
        """
        reasons: list[str] = []
        seen: set[str] = set()
        for e in edits or []:
            ...
            covered = False
            reason = ""
            if line_mode and start is not None and end is not None:
                try:
                    covered, reason = _lines_were_read(
                        project_root, path, int(start), int(end)
                    )
                except (TypeError, ValueError):
                    covered, reason = _file_was_read(project_root, path)
            else:
                # Insert-shaped line edits carry no end_line, and string edits
                # match at runtime -- neither has a range knowable up front, so
                # the file-level rule is the honest one to apply.
                covered, reason = _file_was_read(project_root, path)
            if not covered and reason not in seen:
                seen.add(reason)
        ...
        # DRT-07: a batch refused because the evidence store was unreachable is
        # not a batch refused because nothing was read. The #930 machine verdict
        # stays for the real thing; the outage gets its own token so a caller
        # knows to RETRY rather than to go and read twenty files.
        degraded = any(is_unavailable_reason(r) for r in reasons)
        verdict = "evidence_authority_unavailable" if degraded else "edit_not_grounded"
        head = (
            f"{len(reasons)} edit target(s) could not be PROVEN grounded"
            if degraded
            else f"{len(reasons)} edit target(s) not grounded"
        )
        msg = f"{head} — " + "; ".join(reasons)
        return edit_result(
            content_blocks=[
                TextContent(type="text", text=f"❌ {tool_name} refused: {msg}"),
            ],
            structured={
                "ok": False,
                "error": msg,
                "blocked_by": verdict,
                "reason": f"batch.{verdict}",
            },
    ...
    def ai_get_lines(
        ...
        # #648 (P2 provenance stamp): ai_get_lines is a gated reader
        # (read_pipeline.gate above) like ai_read_raw/pdf/excel/docx/
        # sqlite/jsonl, which all stamp requested_path/resolved_path/
        # zone onto their result. ai_get_lines used to drop the zone
        # after gating on it — carry it through so the agent can see
        # which trust zone the read resolved into.
        structured["zone"] = str(gate_result.zone)
        if isinstance(result, dict):
        ...
        # The zone earns footer space only when it changes what the
        # caller may do with the bytes. A project-internal read is the
        # unremarkable default, so stamping it on EVERY read is pure
        # noise (and a raw enum repr at that). Reads that resolved
        # outside the project keep the stamp — as the zone value, not
        # the Python repr. The structured payload keeps zone always.
        meta_bits: list[str] = []
        if gate_result.zone is not PathTrustZone.PROJECT_INTERNAL:
            meta_bits.append(f"zone={gate_result.zone.value}")
    ...
    def ai_read_sqlite(
        ...
        if gate_result.refusal is not None:
            return gate_result.refusal
        # #222 leak 2: the path gate above is satisfied by WHERE the file
        # sits; this asks what it CARRIES. An empire ledger relocated via
        # AIDOCS_EMPIRE_DB passes the former and must still fail the latter.
        # Runs before every mode — 'tables' and 'schema' expose the soul
        # store's shape, 'query' its rows.
        soul_refusal = _rp.sovereign_soul_db_block(gate_result.resolved_path)
        if soul_refusal is not None:
            return soul_refusal
        try:
    ...
    def ai_read_jsonl(
        ...
        `since` / `until` bound the row `timestamp` (ISO-8601, or relative '30h'/'2d' back from now);
        with either set, rows lacking a parseable timestamp are excluded and counted in `untimed_skipped`.
        Returns {path, rows, row_count, scanned_lines, invalid_lines, matched_total, truncated, offset, limit}.
        Designed for Claude Code session jsonl (~/.claude/projects/<this project's slug>/*.jsonl) and MCP telemetry streams where blob reads would waste context.
        """
        from . import read_pipeline as _rp
    ...
    @server.tool(
        annotations={
            "readOnlyHint": False,
            # FALSE, matching the twins' measured reason (tool_interface
            # :2160-2166): a host card on a MULTI-MODE tool cannot be honest —
            # `create` is not destructive and `restore` is recovery — and
            # stacking one on AIDOCS' own two-phase was measured to make a tool
            # unreachable rather than guarded. The confirmation lives in
            # confirm_modes, where it can name exactly `rename` and `delete`.
            "destructiveHint": False,
            "openWorldHint": False,
            "title": "File",
        },
    ...
    def ai_file(
        mode: Annotated[
            ...
            Field(
                description=(
                    "'create' (new file), 'rename' (move within the project), "
                    "'delete' (to .TRASH/), 'restore' (bring one back)."
                ),
            ),
        ] = "",
        path: Annotated[
            ...
            Field(
                description=(
                    "Project-relative path. create/delete: the target. "
                    "rename: the SOURCE. restore: optional destination "
                    "override."
                ),
            ),
        ] = "",
        to: Annotated[
            str,
            Field(description="mode='rename' only: the destination path."),
        ] = "",
        content: Annotated[
            str,
            Field(description="mode='create' only: the exact file content."),
        ] = "",
        reason: Annotated[
            str,
            Field(
                description=(
                    "Why. Required for delete and rename (operator-visible in "
                    'the audit log). Be specific — "renamed to match the '
                    'surviving verb", not "cleanup".'
                ),
            ),
        ] = "",
        trash_id: Annotated[
            str,
            Field(description="mode='restore' only: the id the delete returned."),
        ] = "",
        config_edit_mode: str | None = None,
        ack_memory_ids: list[int] | None = None,
        ack_drawer_ids: list[str] | None = None,
    ) -> Any:
        """One tool for a file's IDENTITY and EXISTENCE — create, rename,
        delete, restore (#958).

        BUILT BECAUSE A RENAME HAD NO GOVERNED PATH AT ALL. `git mv` is refused
        by the edit gate (a shell write "bypasses every gate by hitting the FS
        at OS level") and no rename service existed, so the only route was
        create+delete — which rewrites the whole file through the model and
        risks silent transcription drift on files nobody re-reads.

        DELEGATES TO THE SIBLING TOOLS rather than reimplementing them, which is
        what makes the fold safe: create/delete/restore call ai_create_file and
        ai_delete directly, inheriting their active-task gate, grounding checks,
        reindex, audit rows and refusal surface unchanged. Only `rename` is new
        code, and even its validation is FileDeleteService's ladder re-run —
        against BOTH endpoints, because a destination is an attack surface a
        deletion does not have (the same call that moves a file out of a
        reserved prefix could move one in).
        """
        m = (mode or "").strip().lower()
        # CALL THE SIBLINGS' SYNC BODIES, NOT THEIR REGISTERED BINDINGS. The
        # names `ai_create_file` / `ai_delete` in this scope are what
        # `@server.tool(...)` returned, and the universal cancel-safety offload
        # registers every sync tool as an `async def offloaded` wrapper. Called
        # from here (a sync body already running in a worker thread) they
        # returned an UNAWAITED coroutine: the sibling never ran and the caller
        # got "<coroutine object ...>" with no error (2026-09-22). Every wrapper
        # on that chain is one of the server.tool patches and keeps
        # `__wrapped__`, so unwrapping reaches the undecorated body; this call's
        # OWN wrapper still does the thread offload and the notification drain,
        # once, for the whole ai_file call.
        if m == "create":
            return _sync_body(ai_create_file)(
                path=path,
                content=content,
                config_edit_mode=config_edit_mode,
                ack_memory_ids=ack_memory_ids,
        ...
        if m in ("delete", "restore"):
            return _sync_body(ai_delete)(path=path, reason=reason, mode=m, trash_id=trash_id)
        if m != "rename":
            return edit_result(
                content_blocks=[
                    TextContent(
                        type="text",
                        text=(
                            f"❌ ai_file: unknown mode {mode!r} "
                            "(create|rename|delete|restore)"
                        ),
                    ),
                ],
                structured={"ok": False, "error": "unknown_mode"},
            )

        _t0_file = time.perf_counter()
        project_root = resolve_project_root()
        gate = require_active_task(hub, project_root, "ai_file")
        if gate is not None:
            return gate

        if not (path or "").strip() or not (to or "").strip():
            return edit_result(
                content_blocks=[
                    TextContent(
                        type="text",
                        text="❌ ai_file(rename) requires path (source) and to (destination)",
                    ),
                ],
                structured={"ok": False, "error": "missing_argument"},
            )
        if not (reason or "").strip():
            # Same bar ai_delete sets: a move that leaves no trace of WHY is the
            # one a forensic reader cannot reconstruct, and a rename leaves even
            # less behind than a delete (no .TRASH entry to find).
            return edit_result(
                content_blocks=[
                    TextContent(
                        type="text",
                        text="❌ ai_file(rename) requires reason (audit-visible)",
                    ),
                ],
                structured={"ok": False, "error": "missing_argument"},
            )

        from .file_rename_service import FileRenameService

        result = FileRenameService(hub).rename(project_root, path, to, reason=reason)
        if not result.ok:
            return edit_result(
                content_blocks=[
                    TextContent(
                        type="text",
                        text=f"❌ ai_file(rename): {result.error_message}",
                    ),
                ],
                structured=result.to_dict(),
                project_root=project_root,
        ...
        # BOTH ENDPOINTS REINDEX. The index keys on path: reindexing only the
        # destination leaves the SOURCE name resolvable in ai_find, pointing at
        # a file that is no longer there — the stale-authority failure #956 was
        # filed for. evict_known_path on the source because that path must never
        # be treated as a fresh read again.
        post_edit_reindex_and_grant(
            hub, project_root, "ai_file", result.src, evict_known_path=True
        )
        post_edit_reindex_and_grant(hub, project_root, "ai_file", result.dst)

        msg = f"✓ ai_file(rename): `{result.src}` → `{result.dst}`"
        if result.anchored_memories:
            # Surfaced, not fatal: the rename succeeded, and these anchors now
            # point at a path that no longer exists. Saying so is the only
            # chance the caller gets — unlike a delete, a rename looks from the
            # outside like nothing happened.
            msg += (
                f"\n⚠ {len(result.anchored_memories)} anchored memory/ies still "
                f"reference the OLD path; re-anchor them: "
                + ", ".join(
                    str(a.get("target_path") or "?")
                    for a in result.anchored_memories[:5]
                )
            )
        return edit_result(
            content_blocks=[TextContent(type="text", text=msg)],
    ...
    def ai_insert_lines(
        ...
        if not covered:
            return edit_result(
                ...
                structured=_grounding_refusal_structured(reason, path),
                project_root=project_root,
                tool_name="ai_insert_lines",
    ...
    def ai_batch_edit(
        ...
        dry_run is honoured on mode='line' ONLY. mode='string' cannot
        preview, so it REFUSES dry_run=True rather than silently applying
        the edits (#540) — a safety flag is never downgraded to a write.
        ...
        if mode == "string":
            # #540: the string engine has no dry_run — neither
            # ai_batch_str_replace nor file_ops.batch_str_replace takes the
            # flag. Dropping it at this switch and applying anyway turned
            # every PREVIEW request into a real write, and repeated
            # "previews" compounded the mutation. An unsupported SAFETY flag
            # must fail closed: refuse, name the alternative, write nothing.
            if dry_run:
                project_root = resolve_project_root()
                msg = (
                    "dry_run is not supported for mode='string' — refusing rather "
                    "than applying the edits. Preview with mode='line' (dry_run is "
                    "honoured there), or re-run mode='string' without dry_run to apply."
                )
                return edit_result(
                    content_blocks=[
                        TextContent(type="text", text=f"❌ batch_edit: {msg}"),
                    ],
                    structured={
                        "ok": False,
                        "error": msg,
                        "blocked_by": "dry_run_unsupported",
                        "reason": "batch.dry_run_unsupported",
                        "mode": "string",
                    },
                    project_root=project_root,
        ...
        if not dry_run:
            ...
            if dnt_gate is not None:
                return dnt_gate
            # #893: read-before-edit, per edit path. Dry runs stay exempt --
            # they are previews and write nothing, matching ai_edit_lines.
            ground_gate = _check_batch_grounded(
                project_root,
                edits,
                line_mode=True,
                tool_name="ai_batch_edit",
    ...
    def ai_replace(
        ...
        inclusive: Annotated[
            ...
            Field(
                description=(
                    "mode='anchor' only: True consumes both anchors - the replaced "
                    "span runs from the start of start_anchor through the end of "
                    "end_anchor and `replacement` takes their place. Default False "
                    "replaces only the content BETWEEN the anchors."
                ),
            ),
        ] = False,
        ...
        PRIMARY CHOICE for surgical edits — mode='anchor':
          ...
          before, a stable closing brace after, etc.). Partial-line
           ...
           (expert/debug escape only) to bypass. inclusive=True consumes
           the anchors too: the span runs from the START of start_anchor
           through the END of end_anchor and `replacement` takes their
           place (use it to rewrite a block INCLUDING its boundary lines).
    ...
    def ai_batch_str_replace(
        ...
        if dnt_gate is not None:
            return dnt_gate
        # #893: read-before-edit, per edit path -- the same rule
        # ai_str_replace applies. This is also the door ai_batch_edit
        # (mode='string') dispatches through, so both are covered here.
        ground_gate = _check_batch_grounded(
            project_root,
            edits,
            line_mode=False,
