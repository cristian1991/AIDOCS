...
- **Persistent memory** — per-project `/.MEMORY/` tree, routed bootstrap, typed memory capture with auto-linking, journal auto-eviction to archive
- **Indexed retrieval** — symbol / reference / text search, **local embedding-based semantic search** (no external API), trace + bundle tools, schema-query; raw read/grep gated in managed mode so agents read with intent
...
- **Security gate cascade** — lifecycle/task gate, taxonomy-based heuristic judge (structured rule verdicts), output guard with auto-redaction, a single supervised shell-egress chokepoint, tool policies + RBAC
- **Dashboard** — Tauri desktop app: monitoring, conductor chat + lane control, scoped settings, skills + MCP registry, setup wizard
