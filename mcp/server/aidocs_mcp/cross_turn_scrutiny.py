...
into a shared or guessed bucket is worse than not accumulating at all (§XXI: a
...
#: an unstable identity, and never collapsed into it: §XXI forbids overclaiming,
...
def observe_prompt(
    ...
    *,
    ...
    that cannot compute must not invent a verdict (§IX drop-on-doubt).
    ...
    if not session_key:
        # TIER 2 REFUSAL (§XXI). Nothing is written — not into a shared bucket,
