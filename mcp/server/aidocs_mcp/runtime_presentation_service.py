...
class RuntimePresentationService:
    ...
    def dashboard_sticky_grants(
        self,
        project_root: Path,
        session_id: str | None,
    ) -> dict[str, object]:
        """Active sticky user-intent grants for the selected session
        (backlog #21 — always-visible sticky-perms indicator).

        Display surface: NEVER epoch-filtered (audit/display sees every
        non-revoked grant; only enforcement filters by epoch). Empty
        grants list when no session is selected.
        """
        ...
        if session_id:
            try:
                ...
                for g in StickyGrantsStore().active_grants_for_session(
                    project_root,
                    session_id,
                ):
                    grants.append(
                        {
                            ...
                            "registered_by": g.registered_by_user_id,
                            "judge_verdict": g.judge_verdict,
                            "confirmation_answer": g.confirmation_answer,
                        },
                    )
            except Exception:
                grants = []
    ...
    def dashboard_judge_overrides(
        self,
        project_root: Path,
        session_id: str | None,
    ) -> dict[str, object]:
        """Judge-rule registry + family-split override state (backlog
        #19 data contract, #22 class-grouped editor surface).

        Each rule row carries its family (bash/git/file_write/network/
        dangerous/credential/general), its verdict class (A credential /
        B sensitive-read / C destructive via judge_taxonomy.classify),
        the locked flag, and whether the current config suppresses it.
        """
        try:
            from .heuristic_judge import JUDGE_FAMILIES, list_judge_rules
            from .judge_overrides import (
                flatten_judge_overrides,
                get_judge_overrides,
            )

            rules = list_judge_rules(project_root)
            overrides = get_judge_overrides(project_root, session_id=session_id)
            suppressed = flatten_judge_overrides(overrides, project_root)
            try:
                from .judge_taxonomy import classify
            except Exception:
                def classify(_rule_id: str) -> str:  # type: ignore[misc]
                    return ""
            ...
            for rule in rules:
                ...
                rule_rows.append(
                    {
                        **rule,
                        "verdict_class": classify(rule_id),
                        "overridden": rule_id in suppressed,
                    },
                )
            return {
                "rules": rule_rows,
            ...
            return {"rules": [], "overrides": {}, "families": []}
    ...
    def dashboard_bash_policy(
        ...
        def _cmd_state(layer_value: Any, cmd: str) -> tuple[str, list[str] | None]:
            ...
            Per Empire: deny overpowers allow within a layer.
            """
        ...
        for layer, (db, scope_filter, scope_key) in (
            (LAYER_GLOBAL, (_global_db_path(project_root), "global", "")),
    ...
    def dashboard_config_entries(
        ...
        is_t0 (the catalog FIELD — #749), effective_layer (which scope
            ...
            entries.append(
                {
                    ...
                    # THE TIER IS A FIELD, NOT A SUBSTRING (#749).
                    # This read used to be `"[T0" in description`, which made
                    # the dashboard's guardrail badge depend on a bracket
                    # surviving inside an English sentence — so rewriting a
                    # description for readability could silently un-flag a
                    # protected setting, with every test still green.
                    #
                    # The first cut kept `"[T0" in description` as the .get()
                    # fallback "for entries built before the field existed".
                    # There are none — is_t0 is a required key of
                    # SettingMetadata and _setting() always emits it — so the
                    # fallback was unreachable in fact and load-bearing in
                    # appearance: it left a live prose dependency sitting in
                    # the security read path for the next reader to trust.
                    # Removed 2026-08-27. The description is now prose, and
                    # only prose.
                    "is_t0": bool(metadata["is_t0"]),
                    ...
                    # switch). Without this OR the Empire couldn't flip
    ...
    def dashboard_snapshot(
        ...
        *,
        ...
        connected_agents = _t(
            "connected_agents",
            lambda: connected_agents_audit(project_root),
        )
        # #210 slice-loads: a live_only call serves the on-change refresh and
        # computes ONLY the live sections (plans, execution, token_usage,
        # degraded_state, freezes). The heavy static reads (repo summary,
        # project overview, config entries/rbac, selected-session detail) are
        # skipped; the frontend merges the slice into its last full snapshot.
        repo_summary = (
            None
            if live_only
            else _t(
        ...
        # #1012 selection law: explicit wins; otherwise the managed-mode binding
        # is trusted ONLY when it names a session list_sessions can see. The
        # project singleton (get_mode without host identity) can outlive its
        # session (probe-593 on the live tree) — a binding that points at
        # nothing must not become the selection, or every per-session read
        # below raises FileNotFoundError and the whole dashboard dies.
        listed_ids = {item.session_id for item in sessions}
        # #1027 authority door: the binding may SELECT a session only when it
        # names one that could actually act. The list_sessions re-check below
        # stays -- it is NOT made redundant by the door: the door's staleness
        # test reads SQL membership, the guard reads what list_sessions can
        # actually open, and the dashboard dies on the second if it trusts
        # only the first.
        #
        # REFUSING TO SELECT IS NOT REFUSING TO NAME. `resolve_managed_session`
        # answers "" for a stale bind, which is right for SELECTION and wrong
        # for REPORTING: this surface exists to tell an operator WHICH session
        # the binding points at when that session is gone
        # (`managed_session_missing:<id>` — #1012). Taking the plain resolver
        # here silently degraded that to a bare "none", and
        # test_dashboard_json_survives_stale_managed_binding caught it on the
        # VPS. So the id is recovered from the refusal reason: never selected,
        # always named.
        from .managed_mode_service import explain_managed_session

        managed_sid, _managed_why = explain_managed_session(
            runtime.hub.managed_mode, project_root
        )
        dangling_sid = ""
        if not managed_sid and _managed_why.startswith("stale_bind:"):
            dangling_sid = _managed_why.split(":", 1)[1]
        selection_reason = "none"
        if selected_session_id:
            selection_reason = "explicit"
        elif managed_sid:
            if managed_sid in listed_ids:
                selected_session_id = managed_sid
                selection_reason = "managed_binding"
            else:
                selection_reason = f"managed_session_missing:{managed_sid}"
        elif dangling_sid:
            selection_reason = f"managed_session_missing:{dangling_sid}"
        if not selected_session_id and len(sessions) == 1:
            selected_session_id = sessions[0].session_id
            selection_reason = "single_session"
        ...
        try:
            ...
            if _rdb.is_file():
                # read_only=True — a dashboard SELECT over RBAC grants. A
                # presentation path must never be able to write the authority
                # store, and read_only makes that sqlite's rule rather than a
                # convention. Also closes the #756 leak: `with
                # sqlite3.connect(p) as _c:` commits and never closes.
                with _canonical_connect(str(_rdb), read_only=True, row_factory=False) as _c:
                    _rows = _c.execute(
        ...
        # Project-level conductor plans - EVERY session's lane graph (active OR
        # finished), not only the selected one, so a plan surfaces regardless of
        # which session the dashboard picked (per-session conductor state persists
        # after completion, so completed plans show too). Capped to bound the
        # per-refresh read cost on large projects.
        def _conductor_view(st: object) -> dict[str, object] | None:
            # plan_conductor_status returns a FLAT dict (lanes + runnable_lane_ids +
            # blocked_reasons at the top level), but the dashboard expects nested
            # conductor.graph.lanes + conductor.runnable. Reshape so the lane GRAPH
            # (which persists for finished plans) renders, with the live runnable/
            # blocked overlay applied where present. None when there are no lanes.
            ...
            if not lanes:
                return None
            return {
                "graph": {"lanes": lanes, "dependencies": st.get("dependencies") or {}},
                "runnable": {
                    "runnable_lane_ids": st.get("runnable_lane_ids") or [],
                    "blocked_reasons": st.get("blocked_reasons") or {},
                },
            }

        def _collect_project_plans() -> list[dict[str, object]]:
            out: list[dict[str, object]] = []
            for _it in sessions[:60]:
                _sid = _it.session_id
                try:
                    _view = _conductor_view(runtime.plan_conductor_status(project_root, _sid))
                    if not _view:
                        continue
                    out.append(
                        {
        ...
        project_plans = _t("project_plans", _collect_project_plans)
        ...
        if selected_session_id and not live_only:
            try:
                selected_session = self._dashboard_selected_session(
                    project_root,
                    selected_session_id,
                    execution_summary=execution_summary,
                    conductor_view=_conductor_view,
                    timed=_t,
                )
            except (FileNotFoundError, OSError) as exc:
                # #1012 degrade, never crash: an explicit --session (or a
                # binding that slipped past the list check) naming a session
                # whose files are gone renders as a missing card; every other
                # section of the snapshot still paints.
                selection_reason = f"session_missing:{selected_session_id}"
                selected_session = {
                    "missing": True,
                    "session_id": selected_session_id,
                    "path": str(getattr(exc, "filename", None) or exc),
        ...
        if selected_session_id:
            try:
                ...
                pass
        # GATE LIVENESS — the Empire's literal requirement: log in, stamp a
        # session, and SEE whether the security gate is actually running.
        # Before this, the dashboard could show a perfectly green, perfectly
        # UNGOVERNED session (hook declining itself on package drift; hooks not
        # firing; the NLP security surface dead). snapshot_gate_health never
        # raises and NEVER fabricates green — a computation it cannot complete
        # reports UNKNOWN, and unknown renders as a WARNING, not a pass. The
        # belt-and-braces except here keeps that floor even if the import
        # itself fails: the section is always present and never lies.
        try:
            gate_health = _t(
                "gate_health",
                lambda: __import__(
                    "aidocs_mcp.gate_health", fromlist=["snapshot_gate_health"]
                ).snapshot_gate_health(project_root),
            )
        except Exception as exc:  # noqa: BLE001 — unknown, never absent, never green
            gate_health = {
                "status": "unknown",
                "reason": f"gate health unavailable: {type(exc).__name__}: {exc}",
                "probes": {},
            }
        ...
        _recent_partial_live = len(recent_execution) >= event_limit
        if live_only:
            return {
                "live_only": True,
                "selected_session_id": selected_session_id,
                ...
                "execution": {
                    ...
                    "recent_partial": _recent_partial_live,
                },
        ...
        config_judge_overrides = _t(
            "config_judge_overrides",
            lambda: self.dashboard_judge_overrides(project_root, selected_session_id),
        )
        sticky_grants = _t(
            "sticky_grants",
            lambda: self.dashboard_sticky_grants(project_root, selected_session_id),
        )
        ...
        snapshot: dict[str, object] = {
            ...
            "connected_agents": connected_agents,
            "selected_session_id": selected_session_id,
            # #1012: why this session (or none) is selected —
            # explicit | managed_binding | single_session |
            # managed_session_missing:<id> | session_missing:<id> | none
            "selection_reason": selection_reason,
            "selected_session": selected_session,
            # Every conductor plan in the project (active + finished), independent
            # of the selected session - rendered so a plan is never hidden just
            # because a different session is picked in the dashboard.
            "plans": project_plans,
            "degraded_state": degraded_state,
            # Gate liveness (see the computation above): is the security gate
            # actually running? ok / degraded / unknown — never a fake green.
            "gate_health": gate_health,
            ...
            "config": {
                ...
                "rbac": config_rbac,
                "judge_overrides": config_judge_overrides,
                "available_edit_modes": available_config_edit_modes("release"),
            },
            # Backlog #21: active sticky grants for the selected session —
            # the always-visible sticky-perms indicator reads this.
            "sticky_grants": sticky_grants,
            "partial": partial,
        }
        if with_timings:
    ...
    def _dashboard_selected_session(
        self,
        project_root: Path,
        ...
        *,
        ...
        """The selected-session detail block of dashboard_snapshot. Every
        per-session file read lives here so the caller can catch a missing
        session (FileNotFoundError/OSError) in ONE place (#1012)."""
        runtime = self.runtime
        session = timed(
            "session_read",
        ...
        if getattr(plan, "lanes", None):
            try:
                conductor = timed(
                    "conductor",
                    lambda: conductor_view(runtime.plan_conductor_status(project_root, selected_session_id)),
                )
            except Exception as exc:
                conductor_error = str(exc)
        session_overview = timed(
            "session_overview",
    ...
    def write_result_artifact(
        ...
        *,
        ...
        try:
            ...
            # RUNTIME (the default). A result artifact is a large tool payload
            # spilled out of the response; the except-branch below already
            # falls back to writing the SAME bytes to a JSON file, so this
            # store is explicitly not the only copy and losing the last commit
            # to a power cut costs a cached payload, never an authorisation.
            # This is also the write that spills on the hot tool-result path,
            # so it is exactly where synchronous=NORMAL's 8-10x lands.
            conn = _canonical_connect(str(db_path), row_factory=False)
            conn.execute(
