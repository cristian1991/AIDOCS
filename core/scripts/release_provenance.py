...
def public_changelog_record(changelog: Path | None, version: str) -> dict:
    """Hash the per-version PUBLIC changelog section that ships as the GitHub
    Release body (#427). Folding its digest into provenance makes a later edit
    of the published body tamper-evident: the release asset, the release body
    and this hash must all agree. Empty dict when no changelog was supplied —
    explicit absence, never a silently missing field."""
...
def provenance(
    ...
    artifacts: list[Path], changelog: Path | None = None,
) -> dict:
    ...
    return {
        ...
        "public_changelog": public_changelog_record(changelog, version),
        "artifacts": [
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--changelog", default="",
                    help="per-version PUBLIC changelog section published as "
                         "the GitHub Release body; its sha256 is folded into "
                         "provenance.json (#427 immutability)")
