...
# aidocs-doctrine §XIV (intent detection is multi-language by
...
CONFIG_GRANT_PATTERNS: tuple[tuple[str, str], ...] = (
    ...
    # "__toggle__" against the current bool (§XXII extend-don't-fork). The
...
#     system text.
# Fail direction is DROP (§X drop-on-doubt): text whose provenance
...
# Doctrine §XIV: detectors match semantic SHAPE — lemma sets + token
...
#
# Doctrine §X negatives DROP (fail-SAFE, drop-on-doubt):
...
# sent / spawned / dispatched / had) are deliberately absent so the §X
# past-tense DROP holds by construction. Romanian imperatives included
# (§XIV multi-language by shape, not translation tables).
...
# fable. Canonical voice (backlog #461); subject to the same §X guards.
...
def detect_research_delegation_v2(prompt: str) -> bool:
    ...
    (≤ proximity), evaluated per sentence, with §X negatives dropped
