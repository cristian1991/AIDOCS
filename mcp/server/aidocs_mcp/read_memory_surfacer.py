...
Per emperor-doctrine §XI memory injection triggers, §XV evidence
...
# ---------------------------------------------------------------------------
# Tool name vocabulary (closed sets — emperor-doctrine §VI edit-tool ecology)
# ---------------------------------------------------------------------------
...
# emperor-doctrine §XII "memory injection triggers" / once-per-
...
class ReadMemorySurfacer:
    ...
    def surface_on_read(
        ...
        *,
        ...
        """Pre-read goggles. Empty SurfacingResult when:
          ...
          - data layer yields no hints (no anchored memory), OR
          - every hint was already surfaced this epoch (once-per-epoch).
        Fail-mode: any exception is swallowed and returns empty —
        the calling tool MUST be allowed to proceed even when
        surfacing fails.
        """
        ...
        # Fail-open time budget: a slow discovery is skipped, never blocks
        # the read (tools.memory_surfacing_timeout_ms).
        try:
        ...
        # Once-per-epoch dedup + display cap. Filter unseen, cap to the
        # display limit, and mark ONLY the displayed hints — capped-out
        # hints stay unmarked so they can surface on a later call.
        ...
        lines = tuple(self._format_memory_line(h) for h in hints)
    ...
    def surface_on_prompt(
        ...
        *,
        ...
        already_surfaced_tools: Iterable[str] | None = None,
        host_kind: str | None = None,
        host_session_id: str | None = None,
    ) -> SurfacingResult:
        """Prompt-time surfacing. Two layers:
          - NLP tool surfacing via ``intent_grant_detector.detect_grant``
            ...
            session (caller passes ``already_used_tools``) and already
            sticky-surfaced (caller passes ``already_surfaced_tools``).
        ...
        ``already_used_tools`` and ``already_surfaced_tools``
        themselves — they're cheap reads of runtime state and
        threading them in keeps this service stateless on session
        identity. Hosts that don't need dedup pass ``None`` for
        both and get the full surface.
        """
        ...
        try:
            ...
            granted = detect_grant(prompt).granted_tools
        except Exception:
            granted = frozenset()
        if granted and project_root is not None:
            used = set(already_used_tools or ())
            already = set(already_surfaced_tools or ())
            fresh = sorted(t for t in granted if t not in used and t not in already)[
                :MAX_TOOL_SUGGESTIONS_PER_PROMPT
            ]
            if fresh:
                # 2026-06-11 (operator): names alone tell the agent WHICH
                # tool but not HOW — annotate each with its usage one-liner
                # from the static catalog (hook-budget cheap, no schema build).
                try:
                    from .tool_suggestion_descs import describe_tools

                    lines.append(describe_tools(fresh))
                except Exception:
        ...
        if project_root is not None:
            try:
                from .memory_discovery import (
                    discover_relevant_memory,
                    format_memory_hints,
                )

                memory_hints = discover_relevant_memory(
                    prompt,
                    project_root,
    ...
    def tools_used_in_session(self, project_root: Path) -> set[str]:
        """Tool names already used in the current managed session.

        Reads ``native_tool_use`` rows from ``execution_events``.
        Unmanaged sessions return empty — no filtering happens.
        Exceptions suppressed so hint generation never fails on a
        store hiccup.

        Hosts that have already computed this (e.g. claude_hook's
        ``_tools_used_in_session``) MAY skip this helper and pass
        their own set. The behavior is intentionally identical so
        either path produces the same SurfacingResult.
        """
        try:
    ...
    def sticky_surfaced_tools(
        self,
        project_root: Path,
    ) -> set[str]:
        """Tools already in the sticky ``user_intent_tools`` set for
        the current managed session. The "agent already knows these"
        suppression layer for prompt surfacing.

        Returns empty for unmanaged sessions or on store error.
        """
        try:
            managed = self.runtime.hub.managed_mode.get_mode(project_root)
            if not managed.get("active"):
            ...
            if not session_id:
                return set()
            return set(
                self.runtime.hub.query_gate.get_sticky_user_intent_tools(project_root, session_id)
                or [],
            )
        except Exception:
            return set()
