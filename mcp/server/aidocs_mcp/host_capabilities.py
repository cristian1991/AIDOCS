...
STOP_FEEDBACK_BLOCK_REASON = "block_reason"
STOP_FEEDBACK_CONTEXT = "context"
STOP_FEEDBACK_NONE = "none"
_STOP_FEEDBACK_VALUES = (STOP_FEEDBACK_BLOCK_REASON, STOP_FEEDBACK_CONTEXT, STOP_FEEDBACK_NONE)

# host_kind -> how a Stop hook's text reaches the AGENT (#1075). Claude Code's
# Stop hook `decision:"block"` + `reason` is fed back to the model and the turn
# continues (verified). Unregistered hosts are `none`: their Stop output is not
# known to reach the agent, so anything owed must ride inline instead.
_STOP_FEEDBACK_TO_AGENT: dict[str, str] = {
    "claude_code": STOP_FEEDBACK_BLOCK_REASON,
    "claude": STOP_FEEDBACK_BLOCK_REASON,
    "cc": STOP_FEEDBACK_BLOCK_REASON,
}


def stop_feedback_to_agent(host_kind: str | None) -> str:
    """``block_reason`` | ``context`` | ``none`` — fail closed to ``none``."""
    if not host_kind:
        return STOP_FEEDBACK_NONE
    return _STOP_FEEDBACK_TO_AGENT.get(str(host_kind).strip().lower(), STOP_FEEDBACK_NONE)
...
def register_host_capability(
    ...
    *,
    ...
    if stop_feedback is not None:
        if stop_feedback not in _STOP_FEEDBACK_VALUES:
            raise ValueError(f"stop_feedback must be one of {_STOP_FEEDBACK_VALUES}")
        _STOP_FEEDBACK_TO_AGENT[key] = stop_feedback
