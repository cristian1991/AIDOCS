...
def _is_blank_semantic_hit(hit: Any) -> bool:
    """True when a palace hit carries NEITHER a drawer_id NOR a snippet — a blank shell
    from a degraded/empty palace search. Such hits are pure noise to the agent; ai_recall
    drops them so the result is [] or real hits, never filler (#66, Empire 2026-06-20)."""
    return not (
        str(getattr(hit, "drawer_id", "") or "").strip()
        or str(getattr(hit, "snippet", "") or "").strip()
    )
...
def run_clustered_recall(
    *,
    ...
    for unit_id, cluster in clusters_by_unit.items():
        ...
        code_ref: ClusterCodeRef = cluster["code"]
        try:
            # direction="both" is REQUIRED, not a widening: the KG writer
            # emits `memory:<path> -anchors_symbol-> Symbol`, so a code
            # symbol only ever sits in OBJECT position. The default
            # "outgoing" asks for edges FROM the symbol and therefore
            # matches nothing — this lane stayed empty even once hub.kg
            # was wired (lane-B forensics 2026-07-29).
            triples = kg.query_entity(code_ref.symbol, direction="both")
        except Exception:
            triples = []
        ...
        try:
            ...
            if isinstance(search_result, SearchResult):
                ...
                for hit in raw_semantic:
                    if _is_blank_semantic_hit(hit):
                        continue  # #66: drop blank-shell hits (no drawer_id/snippet)
