...
import { isConnected } from "./platform/webAuth";
...
function App() {
  ...
  const checkDesktopAuth = useCallback(async () => {
    ...
    try {
      ...
      setDesktopAuthed(Boolean(st.authenticated));
    } catch {
      setDesktopAuthed(false);
    }
  }, [selectedProjectRoot]);
  useEffect(() => {
  ...
  // Host-operator bindings rail + auto-bind (Empire 2026-07-17: 1 dashboard =
  // 1 user = bind). Keyed on `snapshot` so it rides every data load/refresh
  // (no second timer). Fail-closed: web build or unauthenticated => nothing.
  const [bindings, setBindings] = useState<HostBindingRow[]>([]);
  const autoBindInFlight = useMemo(() => new Set<string>(), []);
  const autoBindEnabled = readAutoBindSetting(snapshot?.config?.effective);
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setBindings(rows);
        for (const id of selectAutoBindable(rows, {
          authenticated: true,
          enabled: autoBindEnabled,
        })) {
          if (autoBindInFlight.has(id)) continue;
          autoBindInFlight.add(id);
          try {
            await bindingApprove(id, selectedProjectRoot ?? undefined); // SAME audited path
          } catch {
            /* fail-closed: server refuses if the token is invalid */
          } finally {
            autoBindInFlight.delete(id);
          }
        }
      } catch {
  ...
  }, [snapshot, selectedProjectRoot, desktopAuthed, autoBindEnabled, autoBindInFlight]);
  ...
  function renderActivePage() {
    ...
    if (activeNav === "sessions") {
      return <SessionsPage sessions={snapshot.sessions ?? []} connectedAgents={snapshot.connected_agents} sessionValue={sessionValue} onSelectSession={setSelectedSessionId} deletingSessionId={deletingSessionId} degradedState={snapshot?.degraded_state ?? null} gateHealth={snapshot?.gate_health ?? null} onDeleteSession={async (targetSessionId) => {
          ...
          !isWebBuild() && desktopAuthed ? (
            <div className="flex flex-col gap-4">
              <BindingsPanel
                ...
                onChanged={refresh}
              />
              <label className="flex items-start gap-2 text-xs text-castle-mute">
                <input
                  type="checkbox"
                  checked={autoBindEnabled}
                  onChange={(e) => {
                    const w = autoBindToggleWrite(e.target.checked);
                ...
                />
                <span>
                  Auto-bind new local sessions while logged in — while you're
                  signed in, new Claude Code sessions in this project are
                  approved automatically (audited); off by default.
                </span>
              </label>
            </div>
          ) : undefined
