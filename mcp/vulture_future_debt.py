...
# ── #1039 S2 late-result cache: the instruments, not yet the dashboard ──
# Vulture is RIGHT about all three — test-only, no production consumer. They go
# HERE rather than in the allowlist for exactly that reason: the allowlist is
# for symbols vulture is WRONG about, and calling these false positives would be
# a lie that also buried the finding.
#
# `attached` is the sharpest of the three. #1042b's own commit note said: "reuse
# is now silently inert wherever stable_actor_id cannot resolve … it means the
# optimisation can be off without anyone noticing. Worth a counter before anyone
# tunes it." The counter EXISTS — incremented on every attach — and NOTHING
# READS IT, so the blind spot that note warned about is still open. #1043b
# widened it: an unproven caller now gets no cache key at all, which is correct
# and invisible.
attached  # noqa: F821 (direction=add — backlog #1044: the attach counter is written on every late_attach and never read; the consumer is a late-cache health surface that reports attach/park counts and how often reuse was DISABLED for want of a principal.)
late_status  # noqa: F821 (direction=add — backlog #1044: "none"|"running"|"finished" for a parked call; test-only today. Same coming consumer as `attached`.)
remaining_seconds  # noqa: F821 (direction=add — backlog #1044: the documented extension point for implementations that return PARTIAL results before the hard kill, tool_latency.py:71-72. No tool returns partials yet, so nothing calls it.)

# ── native-lease lifecycle: the per-call release landed, the session sweep did not ──
# Vulture is RIGHT — zero callers anywhere, tests included, so this is not an
# allowlist false positive. The absence is tracked, not accidental: the helper's
# docstring names its consumer verbatim ("Stop / SubagentStop: no tool of that
# host session is still running"), and claude_hook's Stop/SubagentStop arm
# exists but only ever releases per tool call
# (`_release_native_mutation_lease` on PostToolUse/PostToolUseFailure).
# Native leases carry NO pid and NO heartbeat — claude_hook's own comment says
# expiry therefore proves nothing — so a host that dies mid-tool leaves a
# `native:` lease blocking `ai_git switch` for the whole TTL. Stop is the host's
# own statement that the tool is over: the evidence the sweep needs already
# arrives at a handler that ignores it. Deleting the mop would discard the fix,
# not the debt.
release_native_session_leases  # noqa: F821 (direction=add — backlog #1082: sweep every `native:%` lease of a host session when its Stop/SubagentStop arrives; claude_hook's Stop arm is the named, already-present consumer.)
