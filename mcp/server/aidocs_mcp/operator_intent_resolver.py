...
# mirrors aidocs_nlp.substance._NEGATION_LEMMAS (doctrine IX: match
...
_NONDIRECTIVE_TOKENS: frozenset[str] = frozenset(
    {
        ...
        # Romanian / Spanish seed (doctrine IX: match by shape, not
...
def _negated_words(text: str) -> frozenset[str]:
    """FLOOR 2 (doctrine IX polarity). Words a negator reaches.

    Doctrine IX: past / future / conditional / hypothetical / quoted /
...
def _nondirective_words(text: str) -> frozenset[str]:
    """FLOOR 4 (doctrine IX framing). Words a non-directive frame reaches.

    #610, second sweep. The NEGATED arm of doctrine IX was enforced; the
    ...
    def parse(
        ...
        for route in self._routes.values():
            ...
            if negated and verbs & negated:
                # Doctrine IX: a negated action verb mints NOTHING.
                continue
            if framed and verbs & framed:
                # Doctrine IX: a conditionally/hypothetically framed
            ...
            # Drop-on-doubt (doctrine IX): the cost is one operator retype;
