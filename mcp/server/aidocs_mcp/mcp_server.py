...
def _run_with_timeout(
    ...
    **kwargs,
    ...
    #1039 S2 — LATE RATHER THAN LOST. With a ``late_key`` the deadline no
    longer discards the work: a call that times out is PARKED under its
    key, a re-issue of the same call attaches to the still-running worker
    instead of starting a second copy, and once the worker has finished
    the retry returns its answer immediately (dict payloads carry
    ``late_result: true``). The duration of every run is recorded for the
    adaptive default (``tool_latency``), timed out or not — the true cost
    of the call is what the next deadline should be based on.
    """
    import contextvars as _cv

    from . import tool_latency as _tl

    wait = timeout_seconds if (timeout_seconds and timeout_seconds > 0) else None

    # A parked worker for this exact call? Attach — never run it twice.
    if late_key:
        parked = _tl.late_lookup(late_key)
        if parked is not None:
            if parked.done():
                _tl.late_consume(late_key)
                result = parked.result()
                if isinstance(result, dict):
                    result = {**result, "late_result": True}
                return result
            try:
                result = parked.result(timeout=wait)
            except FuturesTimeoutError:
                raise TimeoutError(_late_timeout_message(tool, timeout_seconds)) from None
            _tl.late_consume(late_key)
            if isinstance(result, dict):
                result = {**result, "late_result": True}
            return result

    # BRANCH-TRANSITION WORKER LEASE (r0b 5620f244): the shared lease must
    # follow the WORKER, not the RPC. A timed-out call is parked and keeps
    # running, so the lease is taken HERE (before submit, overlapping the RPC
    # lease) and released by the worker itself when the work actually ends.
    from .branch_transition_barrier import acquire_worker_lease as _acquire_worker_lease

    _release_worker_lease = _acquire_worker_lease(tool or getattr(fn, "__name__", ""), kwargs)
    ...
    def _measured(*a, **kw):
        ...
        try:
            ...
            if tool:
                _tl.record(tool, time.perf_counter() - started)

    try:
        deadline_token = _tl.set_deadline(wait)
        try:
            ctx = _cv.copy_context()
        finally:
            _tl.reset_deadline(deadline_token)
        future = _tool_executor.submit(ctx.run, _measured, *args, **kwargs)
    ...
    try:
        ...
        if late_key:
            _tl.late_park(late_key, tool, future)
            raise TimeoutError(_late_timeout_message(tool, timeout_seconds)) from None
        future.cancel()
        raise TimeoutError(
...
def _late_timeout_message(tool: str, timeout_seconds: int) -> str:
    """Name the measured facts and the retry door — never a bare 'timed out'."""
    from . import tool_latency as _tl

    facts = _tl.explain(tool) if tool else {}
    p95 = facts.get("p95_seconds")
    measured = (
        f" measured p95 for {tool}: {p95}s over {facts.get('samples')} call(s);"
        f" {facts.get('inflight')} other tool call(s) in flight."
        if p95 is not None
        else (f" {facts.get('inflight', 0)} other tool call(s) in flight." if tool else "")
    )
    return (
        f"Tool call timed out after {timeout_seconds}s — the work is STILL RUNNING "
        f"server-side, not lost.{measured} Re-issue the SAME call (identical "
        f"arguments) to attach to it: if it has finished you get the answer "
        f"immediately (late_result=true), otherwise the retry waits its own "
        f"deadline. Pass timeout= for a longer deadline (0 = unlimited)."
    )


def _live_max_ceiling() -> int:
    """tools.max_timeout read LIVE (dashboard edits apply without restart);
    the import-time constant only when the config store is unreachable.
    0 means no ceiling."""
...
def _resolve_timeout(
    ...
    *,
    ...
    (no watchdog). Semantics:
      ...
      - #1039 S2: when a ``tool`` name is given and the CATEGORY default
        applies, it is the ADAPTIVE default — raised to the tool's measured
        p95 x 1.5 and by current in-flight load, never below the configured
        value, never above the ceiling (``tool_latency.adaptive_default``).
        A caller-supplied timeout is never adapted: the caller's number is
        the caller's number.
    """
    ...
    if tool and fallback > 0:
        from . import tool_latency as _tl

        fallback = _tl.adaptive_default(tool, fallback, ceiling=_live_max_ceiling())
    ...
    # take effect without MCP restart. A ceiling of 0 means "no ceiling"
    # — the caller value passes through uncapped.
    max_ceiling = _live_max_ceiling()
...
def _make_timed_decorator(
    ...
    *,
    ...
    def decorator(fn):
        _tool_name = str(getattr(fn, "__name__", "") or "")

        @functools.wraps(fn)
        def wrapper(*args, **kwargs):
            from . import tool_latency as _tl

            # Resolve BEFORE entering the in-flight count so the load factor
            # measures the OTHER calls, not this one.
            ...
            # WHO ASKED is part of the key (#1042). `_LATE` is process-global and
            # this process serves a conductor AND its subagents; without the
            # principal, two callers with identical arguments attach to one
            # another's parked result. See tool_latency.late_key.
            _key = (
                _tl.late_key(
                    _tool_name, args, kwargs, principal=_tl.calling_principal()
                )
                if _tool_name
                else None
            )
            _tl.inflight_enter(_tool_name)
            try:
                return _run_with_timeout(
                    fn, timeout, *args, late_key=_key, tool=_tool_name, **kwargs,
                )
...
def _post_edit_reindex_and_grant(
    ...
    # #1075 pin c: the index row still holds the PRE-edit checksum until the
    # refresh below — the only in-process witness of what this edit replaced.
    _pre_sha = None
    try:
        from .blast_envelope import indexed_checksum as _indexed_checksum

        _pre_sha = _indexed_checksum(project_root, canonical)
    except Exception:
        _pre_sha = None
    ...
    # code_edges (import ∪ semantic_ref, the owned stores the §XXVII
    ...
    try:
        ...
        _blast_radius = None
    # ── #1046 Phase 1 (observe only): mutation epoch + blast envelope ──
    # The radius above is EDIT-TIME PROSE — it is printed and forgotten,
    # which is exactly how #925j shipped: `ensure_claude_hooks` had a
    # production consumer in cli.py `cmd_setup` that nobody opened. This
    # records the same computation as a DURABLE obligation instead, so
    # Stop can ask whether the connected surfaces were accounted for.
    #
    # Cheap by construction: the file radius is memoized and the symbol
    # sweep is DEFERRED to Stop (see surfaces_for_mutation's
    # resolve_consumers). Nothing here blocks anything in Phase 1, and
    # the whole thing is fail-quiet — a broken ledger must never fail an
    # edit that already succeeded on disk.
    _mutation: dict[str, Any] = {}
    try:
        from .blast_envelope import record_mutation as _record_blast_mutation
        from .blast_envelope import symbol_changes_for_edit as _symbol_changes

        # #1075 steps 2-3: the symbols THIS edit changed (baseline diff) with
        # their severity, so the ledger stops owing a sweep for every symbol in
        # the file. None (first governed edit of the file, no outline) keeps
        # the over-inclusive path.
        try:
            _severity = _symbol_changes(project_root, canonical)
        except Exception:
            _severity = None
        _mutation = _record_blast_mutation(
            project_root,
            canonical,
            changed_symbols=None if _severity is None else list(_severity),
            symbol_severity=_severity,
            pre_sha=_pre_sha,
        )
    except Exception:
        _mutation = {}
    # #1075: a per-INVOCATION receipt, never a process-global slot. Recorded
    # for EVERY edit (a dependency-free one included), so this ack can only
    # ever render what this edit touched. Recorded AFTER the ledger so an exact
    # undo (pin c) is not announced a second time: its receipt carries no radius.
    try:
        from .semantic_enrichment import record_edit_receipt

        record_edit_receipt(
            project_root,
            canonical,
            None if _mutation.get("inverse_of_epoch") else _blast_radius,
        )
    except Exception:
        pass
    ...
    try:
        ...
        pass
    # #1075 step 4: the full radius stays in the audit payload above and in the
    # edit receipt; it is NOT returned here, so no caller can merge the raw
    # dependent list into an agent-facing result. tool_display presents it.
...
def create_server(
    ...
    # #1095: the project-bind store's schema + legacy (pre-ttl) migration runs
    # HERE, on the boot rail -- reads no longer create or migrate it (r0b2
    # 0ec88c0b-ce2). Best-effort: a failure never blocks startup, and the pure
    # reader tolerates an unmigrated table.
    try:
        from .session_project_bind_store import ensure_bind_store_schema

        ensure_bind_store_schema()
    except Exception:  # noqa: BLE001
        import logging as _bind_log

        _bind_log.getLogger("aidocs.mcp_server").exception(
            "project-bind store schema step failed at startup"
        )

    # #1095 step 2: install-wide startup steps (the legacy known-projects
    # registry merge) -- the same bootstrap every AIDOCS process entry runs.
    # Listing and recording never migrate. Never raises.
    from .process_bootstrap import run_process_bootstrap
    ...
    async def _instrumented_call_tool(
        ...
        *,
        ...
        # BRANCH-TRANSITION SHARED LEASE (r0b d7137542 gap 2): the common
        # boundary for every in-process tool execution. Whatever project file
        # the tool writes, it writes under a lease a branch switch must drain.
        from .branch_transition_barrier import tool_call_lease as _tool_call_lease

        _lease_root = _project_root_from_args(arguments)
        if _lease_root is None:
            try:
                from .mcp_server_runtime_helpers import resolve_project_root as _rpr

                _lease_root = _rpr()
            except Exception:  # noqa: BLE001 - no bound project: nothing to switch
                _lease_root = None
        try:
            with request_config_scope(), _tool_call_lease(name, arguments, _lease_root):
    ...
    async def _real_instrumented_call_tool(
        ...
        *,
        ...
        # #1095: the project-bind keepalive for hosts WITHOUT hooks, on the
        # call boundary of tools NOT advertised read-only (hosts with hooks
        # also refresh from PreToolUse). Root resolution no longer writes, so
        # this is the one call-boundary refresh; it never creates or revives a
        # bind and never raises.
        from .mcp_server_runtime_helpers import (
            keep_project_bind_alive_for_call,
            keep_xaacp_actor_alive_for_call,
            record_project_for_call,
        )

        keep_project_bind_alive_for_call(name)
        # #1095 step 4a: XAACP actor liveness rides the same boundary (never a
        # messaging call's own side effect); existing rows only.
        keep_xaacp_actor_alive_for_call(name)
        ...
        # #1095 step 2: registering the project (known-projects discovery
        # state + the project_info mirror) is a write a read-only tool must
        # not own. The helper skips tools advertised readOnlyHint=true and
        # never fails the call.
        record_project_for_call(name, project_root, managed_session_id=managed_session_id)
        run_id = f"mcp-{uuid4()}"
    ...
    async def ai_session(
        ...
        if m == "connect":
            ...
            # phrase style. Doctrine VI -- the sibling binding tool already
            ...
            if isinstance(_cgp(), dict):
                ...
                if not (_confirmed and _confirmed[0] == "ai_session"):
                    ...
                    return _refusal
            _connected = await session_connect(session_id=session_id)
            # #1095 step 4 (r0b2 804329fc-66d): a COMPLETE, successful connect is
            # the caller's explicit attachment act, so it attaches the caller's
            # XAACP actor through the ONE authority -- locally for a local
            # project, on the GATE for a cloud-bound one (no local shadow row).
            # Only for a per-conductor conductor bind (never a lane worker, whose
            # spawn owns its row, and never an identity-less singleton bind).
            # The attach result rides the receipt; it never fails the connect.
            if (
                isinstance(_connected, dict)
                and _connected.get("connected")
                and not _connected.get("lane_id")
                and _connected.get("host_identity") != "absent"
            ):
                try:
                    from .conductor_comms import xaacp_attach_on_connect

                    _connected["xaacp_attach"] = xaacp_attach_on_connect(
                        Path(str(_connected.get("project_root") or project_root)),
                        session_id=str(_connected.get("session_id") or ""),
                        confirm_token=confirm_token,
                    )
                except Exception as _att_exc:  # noqa: BLE001
                    _connected["xaacp_attach"] = {
                        "ok": False,
                        "status": "unavailable",
                        "error": type(_att_exc).__name__,
                    }
            # #1095 (r0b2 655e9231-9d9): a successful connect is the explicit
            # activation rail a HOOKLESS host (stdio / Codex / OpenCode) goes
            # through -- it never fires SessionStart. The skill catalog READ no
            # longer converges the empire library or the legacy provider JSON,
            # so the connect does, through the ONE lifecycle helper. Fail-open:
            # it never fails the connect and never reshapes its receipt.
            if isinstance(_connected, dict) and _connected.get("connected"):
                try:
                    from .skill_store import run_skill_authority_lifecycle

                    run_skill_authority_lifecycle(
                        Path(str(_connected.get("project_root") or project_root)),
                        hub.skills,
                    )
                except Exception:  # noqa: BLE001
        ...
        if m == "create":
            ...
            # locally (bleed B, 2026-09-24) it is the operator's own prompt,
            # a single-use user_intent_actions grant -- the spoken phrase is
            # retired on the local surface.
            ...
            from .mcp_server_runtime_helpers import (
                current_gate_principal as _cgp_new,
            )

            _confirmed_new = _cgc_new()
            if _confirmed_new and _confirmed_new[0] == "ai_session":
                # Gate boundary already consumed its minted handle.
                sid = session_id
            elif _cgp_new() is not None:
                # GATE-PRINCIPAL PATH, UNCHANGED (bleed B leaves it alone): it
                # never consults or consumes local user_intent_actions.
                ...
                # LOCAL PATH (bleed B, 2026-09-24). Consent is the operator's
                # own prompt -- a current-turn user_intent_actions grant for
                # (ai_session, create, <this id>), consumed single-use. The
                # fixed phrase grants nothing here and confirm_token is
                # ignored (the field stays on the shared schema for the gate:
                # tests/security/test_session_user_intent_actions_bleed_b.py
                # pins that debt). The target must be NAMED: no id, no
                # derived slug -- refuse before anything changes.
                sid = (session_id or "").strip()
                if not sid:
                    return {
                        "created": False,
                        "_error": "session_id_required",
                        "_detail": (
                            "Local session create needs an explicit session_id, "
                            "and consent comes from the operator's own prompt "
                            "naming that same id (for example: create session "
                            "<id>). No session was created and no id was derived "
                            "from the title."
                        ),
                    }
                ...
                from .local_human_intent import require_local_human_intent

                _refusal_new = require_local_human_intent(
                    hub, project_root, tool="ai_session", mode="create", target=sid
                )
                if _refusal_new is not None:
                    return {"created": False, **_refusal_new}
            if not sid:
        ...
        if m == "delete":
            ...
            #
            # THE LOCAL SPOKEN PHRASE IS RETIRED (bleed B, 2026-09-24). #939
            # established that a published phrase is satisfiable from
            # documentation; the local consent is now the operator's own
            # prompt (user_intent_actions, single-use, consumed atomically by
            # the store). The gate-principal arm keeps its behaviour.
            from .mcp_server_runtime_helpers import (
                current_gate_confirmation as _cgc_del,
            )
            from .mcp_server_runtime_helpers import (
                current_gate_principal as _cgp_del,
            )

            _confirmed_del = _cgc_del()
            if _confirmed_del and _confirmed_del[0] == "ai_session":
                pass  # gate boundary already consumed its minted handle
            elif _cgp_del() is not None:
                # GATE-PRINCIPAL PATH, UNCHANGED (bleed B leaves it alone): it
                # never consults or consumes local user_intent_actions.
                ...
                # LOCAL PATH (bleed B, 2026-09-24): consent is the operator's
                # own prompt -- a current-turn user_intent_actions grant for
                # (ai_session, delete, <this id>), consumed single-use. The
                # fixed phrase grants nothing and confirm_token is ignored.
                # An unknown target is refused BEFORE consumption so a typo
                # does not spend the operator's instruction.
                _pre_base = (Path(project_root) / ".MEMORY" / "sessions").resolve()
                _pre_dir = (_pre_base / _target_sid).resolve()
                if _pre_base != _pre_dir.parent or not _pre_dir.is_dir():
                    return {
                        "ok": False,
                ...
                _refusal_del = require_local_human_intent(
                    hub, project_root, tool="ai_session", mode="delete", target=_target_sid
                )
                if _refusal_del is not None:
                    return _refusal_del
            ...
            # rule reused verbatim (doctrine VI). A resolved child whose parent
    ...
    async def ai_project(
        ...
        if m == "sessions":
            ...
            # consolidation exists to remove (doctrine VI).
    ...
    async def conductor_mode_enter(
        ...
        terse = {
            ...
            # DRT-03: the role BODY reaches this payload EXACTLY ONCE, under
            # "role". This key used to carry a second, rival copy of it.
            "responsibilities": "see 'role' (the head-conductor role scroll)",
            "next": _conductor_doctrine.conductor_next_hint(),
        }

        # The ROLE (what the head-conductor seat does) auto-dumps on entry —
        # one text, one home: the operator-inscribed registry row when present
        # (#479 operator-wins), else the bundled head-conductor scroll body.
        # WHO the seat-holder is lives in the sovereign soul, opened only by
        # the Emperor's word — never dumped here.
        try:
            terse["role"] = _conductor_doctrine.head_conductor_role_text(hub.skills)
        except Exception:
            pass
    ...
    # co-conductor seat's entry. ROLE auto-dumps; SOUL never does (§XI);
    ...
    @renders_as("status", title="conductor mode")
    async def conductor_mode_exit(
        session_id: str = "", role: str = "", reason: str = ""
    ) -> Any:
        """Leave the seat: clear the inline marker AND give back the XAACP seat.

        THE MISSING HALF OF THE LIFECYCLE (2026-09-12). This cleared only the
        in-process inline-conductor marker, so a co-conductor that was done with
        a session left its AUTHORITATIVE `xaacp_actors` row held — and the next
        conversation, being a different actor, was refused `occupied` with no
        remedy but to wait out a six-hour succession grace for a seat nobody was
        using. 'exit' now releases the caller's own seat through the same
        authority the claim uses.

        ONLY ITS OWN. The holder is derived from the caller's XAACP route; no
        argument names an actor, so this can never become an eviction door.
        Leaves any subprocess-spawned conductor untouched.
        """
        prev = _conductor_process.pop("inline", None)
        seat_release: dict[str, Any] = {}
        try:
            from .conductor_comms import xaacp_release_seat_authority

            seat_release = xaacp_release_seat_authority(
                _project_root(),
                session_id=session_id,
                role=role,
                reason=reason,
        ...
        out: dict[str, Any] = {
            "seat_released": bool(seat_release.get("ok")),
            "seat_release": seat_release,
        }
        if prev is None:
            return {**out, "exited": False, "reason": "no inline conductor was active"}
        return {
            **out,
            "exited": True,
    ...
    async def ai_git(
        ...
        """Basic git operations. op: status, log, diff, add, commit, push, pull, branch, switch, stash.

        op=switch REQUIRES `branch`; create=True creates it (`git switch -c`),
        otherwise `git switch --no-guess`. The name is validated strictly before
        git runs; never --force/--discard-changes. The receipt reports
        head_branch + head_sha read back after the switch; ok=False unless HEAD
        is the requested branch. `branch` and `create` are honoured ONLY by
        op=switch.

        op=push (#762) pushes an EXPLICIT `remote` + `refspec` (source:destination).
        Pass both, or neither to derive them from the current branch and its
        tracked upstream; the resolved `push_plan` is returned. Detached HEAD,
        no upstream, force (`+src:dst`), delete (`:dst`) and flag shapes are
        refused. On the GATE surface fetch/pull/push run through the credentialed
        gate git primitive (org GitHub token in the child env only); push there
        also needs the `git.push` capability and is ledgered.
        ...
        `range` and `path` ARE WIRED PER-OP, and refused where they are not
        (#1079 defect 1 / #762 law, r0b 51cd6cff-e7e). Until this change
        op=diff was the bare string "git diff --stat" and read NEITHER, so a
        range asked about the working tree (EMPTY on a clean checkout, which
        reads exactly like "nothing changed") and a path scope was ignored
        (returning the whole tree, .MEMORY noise included). The matrix:
          diff        — range YES, path YES   -> git diff --stat [range] -- [path]
          diff_staged — path YES, range NO    -> git diff --cached --stat -- [path]
          log         — range YES, path NO
          add         — path YES, range NO
          any other op — both refused
        diff_staged refuses `range` deliberately: --cached compares against the
        index, and what a two-endpoint range means there is not invented here.
        Both operands go through ONE validator: metachars refused, whitespace
        refused (they are NOT shell-quoted, so a space would split one operand
        into several), and a range may not start with '-' (git would read it as
        an option).
        ...
        Examples:
          ...
          git_ops(op="diff")
          git_ops(op="diff", range="HEAD..origin/main")
          git_ops(op="diff", path="mcp/server/aidocs_mcp/mcp_server.py")
          git_ops(op="diff", range="abc123..HEAD", path="mcp/gate_checks/webmcp_smoke.py")
          git_ops(op="diff_staged", path="src/foo.py")
        ...
        # op=switch is the ONE op that honours `branch` (and `create`); every
        # other op refuses both before any command is built.
        if o != "switch" and create:
            return {
                "ok": False,
                "error": (
                    f"git_ops refused: `create` is only honoured by op='switch', "
                    f"not op={o!r} (#762: never accept-and-ignore)."
                ),
            }
        _branch_guarded = o != "switch"
        if _branch_guarded:
            if branch.strip():
                return {
                    "ok": False,
                    "error": (
                        f"git_ops refused: the `branch` argument is NOT WIRED for "
                        f"op={o!r} — only op='switch' reads it (backlog #762). It "
                        f"would be accepted and then ignored — "
                        f"op=push runs `git push <remote> <refspec>` from `remote`/"
                        f"`refspec`, NOT branch={branch.strip()!r}. "
                        f"Callers that passed it have advanced the trunk while "
                        f"believing they pushed to a side branch.\n"
                        f"Do instead:\n"
                        f"  * to push somewhere explicit — pass remote + refspec, or "
                        f"run the refspec directly in "
                        f"a shell: `git push origin HEAD:refs/heads/<branch>`;\n"
                        f"  * to switch/create a branch — git_ops(op='switch', "
                        f"branch=..., create=True|False);\n"
                        f"  * to LIST branches — git_ops(op='branch') (that op takes no "
                        f"argument; it runs `git branch -a`)."
                    ),
                }

        # THE SAME #762 LAW, NOW FOR `range` AND `path` (r0b 51cd6cff-e7e).
        # op=diff was a BARE CONSTANT ("git diff --stat") that read NEITHER
        # argument, while the only range guard below is scoped to op=log. So
        # ai_git(op="diff", range="HEAD..origin/main") silently ran the
        # WORKING-TREE diff: on a clean tree it answered EMPTY, which a caller
        # cannot distinguish from "nothing changed" — and ai_git(op="diff",
        # path=...) returned the whole tree including .MEMORY dirt whatever
        # scope was asked for. That surface degraded three of r0b's reviews
        # before anyone traced it (#1079 defect 1); he byte-reviewed the loaded
        # source by hand instead of trusting it.
        #
        # THE MATRIX IS EXPLICIT so no op can drift back into accept-and-ignore:
        #   diff        -> range HONOURED, path HONOURED
        #   diff_staged -> path HONOURED, range REFUSED (the index has a
        #                  different contract; a two-endpoint `range` against
        #                  --cached is not quietly invented here)
        #   log         -> range HONOURED (below), path REFUSED
        #   add         -> path HONOURED (below), range REFUSED
        #   everything else -> both REFUSED
        _RANGE_OPS = {"diff", "log"}
        _PATH_OPS = {"diff", "diff_staged", "add"}
        _rng = (range or "").strip()
        _pth = (path or "").strip()
        if _rng and o not in _RANGE_OPS:
            return {
                "ok": False,
                "error": (
                    f"git_ops refused: `range` is NOT WIRED for op={o!r} — only "
                    f"{sorted(_RANGE_OPS)} read it (#762: never accept-and-ignore). "
                    f"It would be accepted and then ignored, and a range that is "
                    f"silently dropped answers about the WRONG tree.\n"
                    f"Do instead:\n"
                    f"  * a committed delta — git_ops(op='diff', range=...) or "
                    f"git_ops(op='log', range=...);\n"
                    f"  * the STAGED tree — git_ops(op='diff_staged', path=...) "
                    f"(no range: --cached has no two-endpoint contract here)."
                ),
            }
        if _pth and o not in _PATH_OPS:
            return {
                "ok": False,
                "error": (
                    f"git_ops refused: `path` is NOT WIRED for op={o!r} — only "
                    f"{sorted(_PATH_OPS)} read it (#762: never accept-and-ignore). "
                    f"A dropped path scope reports the WHOLE tree while the caller "
                    f"believes it asked about one file."
                ),
            }

        def _git_operand_refusal(kind: str, value: str) -> str:
            """ONE validator for the UNQUOTED git operands (`range`, `path`).

            Deliberately a single function rather than per-op checks: the old
            code had the metachar floor written out separately for `add` and
            `log`, which is how `diff` ended up with neither. Returns "" when
            the operand is acceptable, else the refusal text.

            REFUSE, DO NOT QUOTE — the house rule for this function. These
            operands are interpolated into a string command, and Windows shell
            quoting is fragile, so anything ambiguous is rejected instead of
            escaped. Whitespace is refused for BOTH kinds: `path` is singular in
            the schema, so a space would silently split one requested path into
            several pathspecs.
            """
            v = (value or "").strip()
            if any(c in forbidden_metachars for c in v):
                return (
                    f"git_ops op={o} refused: {kind} contains a shell "
                    f"metacharacter. Values with {sorted(forbidden_metachars)!s} "
                    f"are rejected (Windows-safe; quoting is fragile)."
                )
            if any(ch.isspace() for ch in v):
                return (
                    f"git_ops op={o} refused: {kind}={v!r} contains whitespace. "
                    f"The operand is not shell-quoted, so it would be split into "
                    f"multiple arguments rather than the single {kind} requested."
                )
            if kind == "range" and v.startswith("-"):
                return (
                    f"git_ops op={o} refused: range={v!r} starts with '-', which "
                    f"git would read as an OPTION, not a revision range."
                )
            return ""

        def _git_local(argv: list[str]) -> tuple[int, str]:
            """Plain local git via ai_run (no credential). Git-internal tokens only."""
        ...
        if o == "switch":
            # Branch name is validated (strict charset, no leading '-') inside
            # run_git_switch BEFORE any git argv is built, so the joined command
            # carries no caller metacharacters. Gate admission: 'switch' is not
            # a read op, so it takes the same tier_m_edit + session wall as
            # add/commit in outer_gate.
            from .git_switch import project_liveness_blockers, run_git_switch
            from .mcp_server_runtime_helpers import (
            ...
            def _git_run3(argv: list[str]) -> tuple[int, str, str]:
                res = ai_run(project_root, "git " + " ".join(argv), timeout=30)
                rc = res.exit_code if res.exit_code is not None and res.exit_code >= 0 else (
                    0 if res.success else 1
                )
                return rc, (res.stdout_preview or ""), (res.stderr_preview or "")

            def _switch_exec(argv: list[str]) -> tuple[int, str, str]:
                # The ONLY door that carries the sanctioned-switch allowance:
                # argv form, exact shape re-verified inside the egress service.
                from .shell_egress_service import (
                    SANCTIONED_GIT_SWITCH,
                    SANCTIONED_GIT_SWITCH_METADATA_KEY,
                    ShellEgressRequest,
                    default_service,
                )
                ...
                res = default_service().execute(
                    ShellEgressRequest(
                        ...
                        timeout_s=30.0,
                        audit_tag="ai_git.switch",
                        metadata={SANCTIONED_GIT_SWITCH_METADATA_KEY: SANCTIONED_GIT_SWITCH},
                    )
                )
                rc = res.rc if res.rc is not None else 1
                return rc, (res.stdout or ""), (res.stderr or "")

            _principal = current_gate_principal() or {}
            _caller_hsid = (current_calling_host_session_id() or "").strip()
            _caller_agent = (current_calling_agent_id() or "").strip()
            try:
                from .mcp_server_runtime_helpers import current_calling_host_kind

                _caller_host_kind = (current_calling_host_kind() or "").strip()
            except Exception:
                _caller_host_kind = ""
            try:
                _sid = str(_principal.get("session_id") or "") or (
                    _resolve_session_id(hub, project_root) or ""
                )
            except Exception:
                _sid = ""
            try:
                _actor = str(_principal.get("user_id") or "") or current_calling_agent_context_id(
                    project_root
                )
            except Exception:
                _actor = _caller_hsid
            ...
            def _liveness() -> list[str]:
                # Self is proven at ACTOR level (r0b blocker 3): the caller's
                # canonical XAACP actor id, never the shared host session id.
                try:
                    from .conductor_comms import MSG_ROLE_UNMAPPED, xaacp_resolve_caller_actor

                    _me = str(xaacp_resolve_caller_actor(project_root) or "").strip()
                    if _me == MSG_ROLE_UNMAPPED:
                        _me = ""
                except Exception:
                    _me = ""
                return project_liveness_blockers(
                    project_root,
                    caller_host_session_id=_caller_hsid,
                    caller_agent_id=_caller_agent,
                    caller_actor_id=_me,
                    caller_host_kind=_caller_host_kind,
                    session_id=_sid,
                )

            def _barrier():
                from .branch_transition_barrier import holding

                return holding(
                    project_root,
                    owner=f"ai_git.switch actor={_actor or '?'} hsid={_caller_hsid or '?'} agent={_caller_agent or '-'}",
                )

            def _audit(stage: str, receipt: dict) -> bool:
                from .gate_git_ops import _ledger

                return _ledger(
                    hub,
                    project_root,
                    critical=(stage == "requested"),
                    status=stage,
                    event_kind="git_switch",
                    source_kind="ai_git",
                    capability_name="git.switch",
                    action_kind="switch",
                    target_entity=str(receipt.get("requested_branch") or ""),
                    principal_type="user" if _principal else "agent",
                    user_id=str(_principal.get("user_id") or ""),
                    session_id=_sid,
                    payload={
                        "actor": _actor,
                        "host_session_id": _caller_hsid,
                        "host_agent_id": _caller_agent,
                        "from_branch": receipt.get("from_branch"),
                        "from_sha": receipt.get("from_sha"),
                        "to_branch": receipt.get("requested_branch"),
                        "head_branch": receipt.get("head_branch"),
            ...
            return run_git_switch(
                branch,
                bool(create),
                _git_run3,
                project_root=project_root,
                liveness=_liveness,
                switch_exec=_switch_exec,
                audit=_audit,
                barrier=_barrier,
            )

        def _local_drift_runner(argv: list[str]):
            """Adapt ai_run to the compute_origin_drift runner contract.

            Bounded (15s) so a slow/unreachable remote can never hang the
            status read. Returns (returncode, stdout, failure) — failure carries
            the category + SCRUBBED stderr so origin_check=unreachable says why.
            The argv is composed of git-internal tokens only (fetch/rev-list +
            a ref) — no caller input, so no shell-metachar exposure."""
            from .gate_git_transport import classify_git_failure, scrub_git_text

            res = ai_run(project_root, "git " + " ".join(argv), timeout=15)
            rc = res.exit_code if res.exit_code is not None and res.exit_code >= 0 else (
                0 if res.success else 1
            )
            err = scrub_git_text(res.stderr_preview or "")
            return rc, (res.stdout_preview or ""), {
                "category": "" if rc == 0 else classify_git_failure(err),
                "reason": err.strip()[-400:],
            }

        # Gate surface: a gate-resolved principal is bound for this dispatch.
        # Its tenant selects the org credential for the network verbs; the local
        # surface (no principal) keeps plain git + the host credential manager.
        from .mcp_server_runtime_helpers import current_gate_principal as _cgp

        _gate_principal = _cgp()
        if _gate_principal is not None:
            from .gate_git_ops import gate_drift_runner as _gdr

            _git_drift_runner = _gdr(
                project_root, str(_gate_principal.get("tenant_id") or ""), _git_local
            )
        else:
            _git_drift_runner = _local_drift_runner

        if _gate_principal is not None and o in ("fetch", "pull", "push"):
            from .gate_git_ops import run_gate_git_op

            if o == "push":
                triggers_pending = [
                    a
                    for t in ("before_git_push",)
                    for a in hub.workflow.pending_actions_for_trigger(project_root, t)
                    if not _is_workflow_action_satisfied(project_root, a)
                ]
                if triggers_pending:
                    return {
                        "op": o,
                        "blocked": True,
                        "reason": "Workflow rule requires pending actions before push.",
                        "pending_actions": triggers_pending,
                    }
            return run_gate_git_op(
                o,
                project_root=project_root,
                ...
                refspec=refspec,
                hub=hub,
                git_local=_git_local,
            )

        # #762: local push is explicit too — resolve the plan BEFORE cmd_map.
        push_cmd = ""
        push_plan: dict[str, Any] | None = None
        if o == "push":
            from .gate_git_ops import derive_push_plan

            _plan = derive_push_plan(remote, refspec, _git_local)
            if not _plan.get("ok"):
                return {"ok": False, "error": _plan["error"]}
            push_plan = {
                k: _plan[k] for k in ("remote", "refspec", "source", "destination", "derived")
            }
            push_cmd = f"git push {_plan['remote']} {_plan['refspec']}"
        ...
        if o == "add":
            ...
            # THE SHARED VALIDATOR, not a second bespoke one (r0b aa28dd5b-b20).
            # This block had its own metachar check and NO whitespace check, so
            # `path="a b.txt"` was accepted and became `git add -- a b.txt` —
            # TWO pathspecs from one singular argument, staging a file the
            # caller never named. Keeping a private validator here is also what
            # made the "ONE validator" claim false while it read as true.
            _bad = _git_operand_refusal("path", p)
            if _bad:
                return {
                    "ok": False,
                    "error": _bad + " Stage via shell directly if legitimately needed.",
                }
            add_cmd = f"git add -- {p}"

        # op=log range=...: validated by the SHARED operand validator now, so
        # log, diff and add cannot drift apart again. Empty range falls back to
        # legacy `-N` recent-commits.
        log_cmd = f"git log --oneline -{count}"
        if o == "log" and _rng:
            _bad = _git_operand_refusal("range", _rng)
            if _bad:
                return {"ok": False, "error": _bad}
            log_cmd = f"git log --oneline -{count} {_rng}"

        # op=diff / diff_staged: the operands are READ here (r0b 51cd6cff-e7e).
        # `--` separates revisions from pathspecs so a path can never be parsed
        # as a revision, and neither operand is shell-quoted — the validator
        # refuses anything that would need quoting.
        diff_cmd = "git diff --stat"
        diff_staged_cmd = "git diff --cached --stat"
        if o in ("diff", "diff_staged"):
            for _kind, _val in (("range", _rng), ("path", _pth)):
                if _val:
                    _bad = _git_operand_refusal(_kind, _val)
                    if _bad:
                        return {"ok": False, "error": _bad}
            if o == "diff":
                diff_cmd = "git diff --stat"
            ...
            try:
                ...
                if br.success and cur_branch and cur_branch != "HEAD":
                    ...
                    if up.success and (up.stdout_preview or "").strip():
                        ...
                        if drift.get("origin_check_category"):
                            response["origin_check_category"] = drift["origin_check_category"]
                            response["origin_check_reason"] = drift.get("origin_check_reason", "")
    ...
    async def conductor_start(
        ...
        context_parts = [
            ...
            "",
            "== YOUR ROLE ==",
            # DRT-03: the role text has ONE home — the bundled head-conductor
            # role scroll, rendered through conductor_doctrine. This block used
            # to be followed by a hand-written seven-step restatement of the
            # same body; the tool-specific steps live in the situation→tool map
            # (conductor_onboarding) and the rest was a rival copy.
            ...
            "take the next task when it arrives, and never exit unless the operator says to stop."),
            ("When a task completes, report what changed, what was tested and what needs attention — "
    ...
    async def ai_seat(
        ...
        mode='co-enter' — current agent takes the CO-CONDUCTOR seat (#167 Phase 4):
                         ...
                         Souls never auto-load (§XI); the seat's soul opens only
                         through ai_soul on the Empire's word.
        mode='succeed'  — SUCCESSION, not eviction (DRT-10). The current agent inherits
                         a seat whose holder is gone: a terminal Web-ChatGPT
                         co-conductor conversation used to strand `co_conductor`
                         forever, because the replacement conversation is a
                         different actor and gets `occupied` with no remedy.
                         Required: role ('conductor'|'co_conductor'). Optional:
                         session_id, reason. GRANTED only when the incumbent is
                         not provably live AND its own heartbeat is stale past a
                         fixed grace period this caller cannot shorten. A
                         PROVABLY LIVE incumbent is an authority transfer that
                         only r0 or above may order (peers cannot authorize it,
                         and that path currently refuses: the rank ladder is not
                         landed). Silent-but-not-yet-stale also refuses — unknown
                         is not death. The successor is derived from the caller's
                         XAACP route and can never be named; a spawned agent
                         (lane/subagent) never holds or inherits a seat.
        ...
        if m == "succeed":
            # DRT-10. NOTHING about identity is assembled here: an omitted
            # session_id is filled from the caller's own XAACP route inside
            # `xaacp_succeed_seat`, and the SUCCESSOR has no parameter at all.
            #
            # THROUGH THE AUTHORITY, exactly as `co-enter` claims. This called
            # `xaacp_succeed_seat` directly on local SQLite, so on a cloud-bound
            # project the claim asked the gate and the succession asked the
            # local copy — and on 2026-09-12 they answered 'occupied' and
            # 'vacant' about the same seat within one minute.
            from .conductor_comms import xaacp_succeed_seat_authority

            return xaacp_succeed_seat_authority(
                _project_root(),
                session_id=session_id,
                role=role,
                reason=reason,
        ...
        if m == "exit":
            return await conductor_mode_exit(session_id=session_id, role=role, reason=reason)
        if m == "status":
        ...
        return {
            "error": (
                ...
                "(valid: enter|co-enter|succeed|exit|status|overview)"
            )
        }
    ...
    async def ai_failures(
        ...
        """Failure-stewardship disposition AND DIAGNOSIS surface — the
        agent-callable CONSUMER half of the failure ledger (the Stop hook is
        the producer).

        mode='diagnose'          — WHAT IS THIS FAILURE? One row, by full signature or
                                   unambiguous short prefix: nodeid, full error type AND
                                   message, assertion line, duty + causal origin, first/
                                   last seen, history, and the command that REPRODUCES
                                   it. Bounded by construction. Required: signature.
                                   (#1056 — the Stop block now advertises this call with
                                   the signature already substituted.)
        mode='list'              — failures THIS session owns (seal blockers), plus the
                                   still-open rows under ANOTHER duty (blocked_elsewhere,
                                   #673). BOUNDED: terse rows, limit/offset paging and a
                                   signature/nodeid filter. The full ledger ships only
                                   with include_all=True (#852), and is paged too.
        ...
        if m in ("diagnose", "explain", "why"):
            return fs.diagnose_failure(project_root, sid, signature)
        if m in ("list", "blockers", ""):
            return fs.list_session_failures(
                project_root,
                sid,
                include_all=include_all,
                # 0 is passed through UNCHANGED: it means "the caller did not
                # ask", which list_session_failures distinguishes from a real
                # limit (it keeps the #852 include_all audit view whole).
        ...
        # #1039 S4: a `fixed` disposition the ledger ACCEPTED unlocks one rerun
        # window on the test-retry gate — the counter is keyed on the MANAGED
        # session (the gate's axis), not the ledger's duty uuid.
        #
        # `fixed` ONLY, and `autoclear` DELIBERATELY NOT (#1042). The first cut
        # unlocked on either, describing both as "the ledger ACCEPTED" — but
        # autoclear is never refused: apply_disposition returns
        # {"ok": True, ...} unconditionally for it, even when cleared == 0, and
        # failure_stewardship says why in as many words — "an agent asserting a
        # green run is a CLAIM, not an observation (#673)". So any agent wanting
        # a third run could call autoclear and get it with no proof and no edit,
        # which is the one thing this gate exists to prevent.
        #
        # `fixed` carries a real cost: the ledger transition raises without
        # proof_log. That is what makes it evidence rather than an assertion,
        # and it is also exactly what the operator's report asked for — "accept
        # an ai_failures mode=fixed claim as the unlock".
        if m == "fixed" and isinstance(_receipt, dict) and _receipt.get("ok"):
            try:
                from .agent_orchestrator import (
                    authenticated_actor_lane,
                ...
                record_test_retry_unlock(
                    hub,
                    project_root,
                    _resolve_session_id(hub, project_root),
                    reason=f"ai_failures:{m}",
                    # The caller's authenticated lane, the counter's own axis.
                    lane_id=authenticated_actor_lane(project_root),
                )
                _receipt["test_retry_unlocked"] = True
            except Exception:
                pass
        return _receipt
    ...
    async def ai_msg(
        ...
        confirm_token: str = "",
        reason: str = "",
        # #1063: declared on tool_interface.ai_msg and accepted by
        # ai_msg_dispatch, but missing HERE -- the implementation _delegate
        # calls -- so the declared-schema path passed an argument this handler
        # rejected with TypeError. Every declared parameter must reach it.
        pending_only: bool = False,
        # 8d84 phase 3: cross-project send target (canonical project id).
        target_project_id: str = "",
    ) -> Any:
        """One inter-agent channel for role messages and actor-routed XAACP."""
        # Imported INSIDE the call, so a test patching the module attribute
        # still takes effect -- and in this exact `from .mod import name` form,
        # which test_mode_contract_parity follows to find the real dispatcher.
...
def main() -> None:
    ...
    # #1063: backlog sync starts at BOOT for every root a prior process synced;
    # session connect stays an idempotent ensure. Never blocks a boot.
    try:
        from .backlog_sync_sitter import boot_backlog_sync

        boot_backlog_sync(server._aidocs_test_hub)
    except Exception:  # noqa: BLE001
        pass
    if args.http:
