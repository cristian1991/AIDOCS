...
function Remove-AidocsHookGroups {
  ...
  foreach ($group in (Normalize-HookGroups $Groups)) {
    ...
    foreach ($hook in (Normalize-HookGroups $group.hooks)) {
      if (($hook.command -and ($hook.command -match "claude-hook\.ps1" -or $hook.command -match "aidocs_mcp\.claude_hook")) -or ($hook.statusMessage -and $hook.statusMessage -like "AIDOCS *")) {
        $isAidocsGroup = $true
        break
      }
    }
...
# Hook command — uses python -m for portability (works from any CWD)
$claudeHookCommand = "python -m aidocs_mcp.claude_hook"
...
$postCompactHookGroup = [pscustomobject]@{
  hooks = @(
    [pscustomobject]@{
      type = "command"
      command = $claudeHookCommand
      timeout = 15
      statusMessage = "AIDOCS context reset"
    }
  )
}
...
$postCompactGroups = Remove-AidocsHookGroups (Normalize-HookGroups $claudeSettings.hooks.PostCompact)
...
$claudeSettings.hooks | Add-Member -Force -NotePropertyName PostCompact -NotePropertyValue @($postCompactGroups + $postCompactHookGroup)
