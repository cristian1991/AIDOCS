...
class CodeIndexTraceSurfacesService:
    ...
    def find_mutation_points(
        ...
        with self.store.connect(project_root) as conn:
            mutation_like_clauses = " OR ".join(
                ["LOWER(co.symbol) LIKE ?" for _ in mutation_tokens],
            )
        ...
        for item in symbol_matches:
            ...
            container_match = (
                self.store._score_text_match(needle, container, exact=50, prefix=35, contains=20)
                if container
                else 0
            )
            ...
            if layer in {"logic", "api", "ui"}:
                score += 10
            if any(token in lower_path for token in factory_tokens) or any(
                token in lower_symbol for token in factory_tokens
            ):
                score -= 25
        ...
        for item in code_matches:
            ...
            if any(token in lower_path for token in factory_tokens) or any(
                token in lower_summary for token in factory_tokens
            ):
                score -= 25
            if "/test" in lower_path or "tests/" in lower_path:
                score -= 20
    ...
    def find_async_boundaries(
        ...
        symbol_matches = (
            self.store.search_symbols(project_root, needle or "async", limit=max(limit * 2, 100))
            if needle
            else self.store.search_symbols(project_root, "async", limit=max(limit * 2, 100))
        )
        code_matches = (
            self.store.search_code(project_root, needle or "task", limit=max(limit * 2, 100))
            if needle
            else self.store.search_code(project_root, "task", limit=max(limit * 2, 100))
        )
