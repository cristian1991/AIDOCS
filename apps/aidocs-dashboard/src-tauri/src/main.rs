...
/// the caller validates it against the server via `_token_is_valid`, which
/// enforces expiry/revocation authoritatively). Returns None when the file is
/// absent/corrupt or holds no token.
...
///   3. None => the operator is signed out and the frontend MUST show LOGIN
...
fn resolve_operator_token(project_root: &Path) -> Option<String> {
    ...
    // 3. no valid token anywhere => signed out.
    None
}
...
#[tauri::command]
fn update_status() -> Result<Value, String> {
    let script = r#"
import json
...
/// Password sign-in for the desktop dashboard (Empire directive 2026-07-17:
/// 1 dashboard = 1 user = bind). Delegates to the SANCTIONED, password-gated
/// `dashboard-login --emit-token` CLI: it authenticates via IdentityStore.login,
/// writes the SHARED machine cache, and (only under --emit-token) returns the
/// bearer token, which we seed into the in-process cache so subsequent admin
/// commands ride it. The token is NEVER returned to the frontend.
#[tauri::command]
fn operator_login(
    ...
    // run_json_cli_checked: a refused login ({ok:false}, e.g. invalid_credentials)
    // surfaces as an Err the login view renders — never a false success.
    let v = run_json_cli_checked(&root, &args)?;
    // Seed the in-process cache, then strip the token out of the payload.
...
#[tauri::command]
fn operator_logout(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    revoke_cached_operator_token(&root);
    Ok(serde_json::json!({"ok": true, "logged_out": true}))
}

/// Pending + approved host-operator bindings for this project (read-only).
/// Feeds the dashboard Bindings panel. No token required — `bindings` is a
/// pure read of the bindings ledger.
#[tauri::command]
fn bindings_list(project_root: Option<String>) -> Result<Value, String> {
    let root = resolve_project_root(project_root)?;
    let args = vec!["bindings".to_string(), "--json".to_string()];
    run_json_cli(&root, &args)
}
