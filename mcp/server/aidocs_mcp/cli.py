...
def _coerce_setting_value(setting_path: str, value: object) -> object:
    ...
    (e.g. the canonical bash.* runtime policy namespace).
    """
...
def cmd_dashboard_set_config(args: list[str]) -> int:
    ...
    if not allowed:
        ...
        message_map = {
            "unauthenticated": (
                "dashboard-set-config refused: no operator token found "
                "in env AIDOCS_OPERATOR_TOKEN, the --operator-token "
                "flag, or the machine token cache. Sign in once with "
                "'aidocs operator-login' (caches the token); the "
                "Dashboard UI attaches its own token automatically."
            ),
...
def _require_operator_for_admin_command(
    ...
    msg = {
        "unauthenticated": (
            ...
            f"AIDOCS_OPERATOR_TOKEN, the --operator-token flag, or the "
            f"machine token cache. Sign in once with "
            f"'aidocs operator-login' (as ADMIN/SUPERADMIN) — the token "
            f"is cached for this machine until it expires."
        ),
        "missing_admin_manage_config": (
...
def cmd_service(args: list[str]) -> int:
    ...
    stop    — ask the watchdog to terminate the daemon and exit.
    ...
    if sub == "start":
        ...
        if health.get("status") == "up" and health.get("daemon_alive"):
            ...
            return 0
        # pythonw.exe (GUI-subsystem) so the detached watchdog never flashes a
        # console/Terminal tab; DETACHED_PROCESS keeps it out of our console.
        cmd = [windowless_python(), "-m", "aidocs_mcp.cli", "service", "run", "--port", str(port)]
        creation = 0
        if os.name == "nt":
        ...
        # callsite ('cli.py','cmd_service','subprocess.Popen').
        from .shell_egress_service import audited_popen

        proc = audited_popen(
            cmd,
            fingerprint=("cli.py", "cmd_service", "subprocess.Popen"),
        ...
        print(f'point .mcp.json at: {{"aidocs": {{"url": "http://127.0.0.1:{port}/mcp"}}}}')
        return 0
    if sub == "stop":
        request_stop()
        print("stop requested (watchdog drains the daemon and exits)")
        return 0
    if sub == "status":
        health = service_status()
    ...
    if sub == "install":
        if os.name != "nt":
            ...
            return 0
        run_cmd = f'"{sys.executable}" -m aidocs_mcp.cli service run --port {port}'
        ...
        if startup.is_dir():
            ...
            launcher.write_text(
                "@echo off\n"
                f'start "" /min "{sys.executable}" -m aidocs_mcp.cli service run --port {port}\n',
                encoding="utf-8",
            )
    ...
    if sub == "uninstall":
        ...
        return result.returncode
    print("usage: aidocs service {start|stop|status|run|install|uninstall} [--port N]")
    return 2
...
def cmd_operator_login(args: list[str]) -> int:
    """Password login → operator token (the sanctioned mint). Usage:
        aidocs operator-login --email <e> --password <pw> [--no-cache]
                              [--cache-path <file>]
    ONCE-ONLY login (#421): by default the minted token is cached to the
    per-user machine cache (~/.aidocs/operator_token.json) so every later
    CLI command / binding approval / session rides it until expiry — no
    credential re-entry per surface. --no-cache prints the token only
    (export as AIDOCS_OPERATOR_TOKEN or pass --operator-token).
    """
    ...
    if not email or not password:
        payload = {"ok": False, "reason": "missing_args",
                   "message": "--email and --password are required"}
        print(_safe_json_dumps(payload, indent=2) if as_json else payload["message"])
        return 2
    from .identity_store import IdentityStore

    tok = IdentityStore().login(root, email, password)
    if tok is None:
        payload = {"ok": False, "reason": "invalid_credentials",
                   "message": "invalid email or password"}
        print(_safe_json_dumps(payload, indent=2) if as_json else payload["message"])
        return 1
    # ONCE-ONLY login (#421): cache the token machine-side (default ON).
    cached = False
    cache_file = ""
    if "--no-cache" not in args:
        ...
        try:
            ...
            # World/group-readable CUSTOM path: refuse loudly. The token
            # row was minted but NOT persisted anywhere readable.
            payload = {
                "ok": False,
                "reason": "cache_refused",
                "message": str(exc),
            }
            print(_safe_json_dumps(payload, indent=2) if as_json else payload["message"])
            return 1
    ...
    payload = {
        "ok": True, "token": tok.token, "user_id": tok.user_id,
        "expires_at": tok.expires_at,
        "cached": cached, "cache_path": cache_file,
        "message": message,
    }
    print(_safe_json_dumps(payload, indent=2) if as_json else (payload["token"]))
    return 0


def cmd_dashboard_login(args: list[str]) -> int:
    """Desktop dashboard sign-in (Empire directive 2026-07-17: 1 dashboard =
    1 user = bind). Usage:

        aidocs dashboard-login --email <e> --password <pw>
                               --project-root <path> [--emit-token] [--json]

    Authenticates via the SANCTIONED password-gated mint
    (``IdentityStore.login``) and writes the per-machine operator-token cache
    (``~/.aidocs/operator_token.json``) so every later CLI surface / binding
    approval rides it — the ONE resolution door. Returns
    ``{ok, user_id, expires_at, cached}``. The bearer token is NEVER echoed
    in JSON unless ``--emit-token`` is passed (the Tauri side uses that to seed
    its in-process cache); the machine cache still holds it either way.
    """
    root = _resolve_login_root(args)
    as_json = _wants_json(args)
    emit_token = "--emit-token" in args
    email = _option_value(args, "--email", "").strip()
    password = _option_value(args, "--password", "").strip()
    if not email or not password:
        payload = {
    ...
    if tok is None:
        ...
        print(_safe_json_dumps(payload, indent=2) if as_json else payload["message"])
        return 1
    # Reconcile RBAC to the identity role on every login (#434, 2026-07-17):
    # create_user records identity_users.role but never created the matching
    # rbac_user_roles grant, so a provisioned operator authenticated yet held
    # ZERO permissions (the role-less-owner lockout that made source self-edit
    # unsatisfiable). Login is the natural self-healing seam — idempotent, so
    # an already-correct account is untouched. Best-effort: an RBAC hiccup never
    # blocks a valid sign-in.
    try:
        _u = IdentityStore().get_user_by_id(root, tok.user_id)
        if _u is not None and _u.role:
            from .rbac_store import RBACStore

            RBACStore().assign_role_to_user_by_name(root, tok.user_id, _u.role)
    except Exception:
        pass
    cached = False
    cache_file = ""
    ...
    try:
        ...
        payload = {"ok": False, "reason": "cache_refused", "message": str(exc)}
        print(_safe_json_dumps(payload, indent=2) if as_json else payload["message"])
        return 1
    except Exception:
        cached = False  # best-effort: an IO hiccup never blocks login
    ...
    if as_json:
        ...
        print(f"Signed in as {tok.user_id} (expires {tok.expires_at}).")
    return 0
    ...
    if ctx is None:
        ...
        payload = {
            ...
            "message": (
                ...
                "'aidocs operator-login' (caches the token), or approve "
                "from the Dashboard."
            ),
        }
        print(_safe_json_dumps(payload, indent=2) if as_json else payload["message"])
